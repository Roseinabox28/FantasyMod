...
public class PreRenderShadersHookTransformer extends TransformerModule {
	...
	public void transformMethodInstruction(MethodNode method, AbstractInsnNode node, int index) {
		if (node instanceof MethodInsnNode) {
			...
			if (methodCallNode.name.equals(this.getMappedName("c", "renderEntityOutlineFramebuffer"))
					&& methodCallNode.desc.equals("()V") && methodCallNode.owner
							.equals(this.getMappedName("buy", "net/minecraft/client/renderer/RenderGlobal"))) {
				...
				insertions.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "thebetweenlands/client/ClientHooks",
						"onPreRenderShaders", "(F)V", false));
