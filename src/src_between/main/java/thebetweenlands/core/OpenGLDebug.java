...
public class OpenGLDebug implements IClassTransformer {
	...
	private static final String[] checkExclusions = new String[] { "glBegin", "glEnd", "glVertex", "glColor",
			"glSecondaryColor", "glIndex", "glNormal", "glFogCoord", "glTexCoord", "glMultiTexCoord", "glVertexAttrib",
				...
				if (insnNode.getOpcode() == Opcodes.INVOKESTATIC) {
					...
					if (!this.isExcluded(methodInsn.name) && methodInsn.owner.startsWith("org/lwjgl/opengl")) {
						...
						method.instructions.insert(methodInsn,
								new MethodInsnNode(INVOKESTATIC, OpenGLDebug.class.getCanonicalName().replace(".", "/"),
										"checkThrowError", "()V", false));
						System.out.println(String.format("[OpenGLDebug] Hooked method '%s' at '%s' (%s)",
								classNode.name + "#" + method.name, methodInsn.name + methodInsn.desc, i));
					}
				}
			}
		}
		...
		if (checkGlContext()) {
			...
			if (!foundErrors.isEmpty()) {
				...
				for (int i = 0; i < foundErrors.size(); i++) {
					errors.append(GLU.gluErrorString(foundErrors.get(i))).append(" (").append(foundErrors.get(i))
							.append(")");
