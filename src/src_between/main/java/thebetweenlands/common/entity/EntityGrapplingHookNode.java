...
public class EntityGrapplingHookNode extends Entity implements IEntityAdditionalSpawnData {
	private static final DataParameter<Integer> DW_PREV_NODE = EntityDataManager
			.createKey(EntityGrapplingHookNode.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> DW_NEXT_NODE = EntityDataManager
			.createKey(EntityGrapplingHookNode.class, DataSerializers.VARINT);
	private static final DataParameter<Float> DW_CURRENT_ROPE_LENGTH = EntityDataManager
			.createKey(EntityGrapplingHookNode.class, DataSerializers.FLOAT);
	private static final DataParameter<Boolean> DW_ATTACHED = EntityDataManager.createKey(EntityGrapplingHookNode.class,
	...
	/**
	 * Number of nodes the grappling hook rope has. Only updated on mount node!
	 */
	...
	public void onEntityUpdate() {
		if (this.ticksExisted < 2) {
			...
			// Tracker updates server side position but *does not* send the change to the
			// client
			// when tracker.updateCounter == 0, causing a desync until the next force
			// teleport
			// packet.......
		...
		if (!this.world.isRemote) {
			if (nextNode != null) {
				...
				if (nextNode instanceof EntityGrapplingHookNode && ((EntityGrapplingHookNode) nextNode).isMountNode()) {
					...
					if (mountNode.isExtending && nextNode.posY < this.posY && nextNode.getDistance(this.posX, this.posY,
							this.posZ) > this.getDefaultRopeLength() - 0.2D) {
						if (mountNode.nodeCount < mountNode.maxNodeCount) {
							Vec3d connection = this.getConnectionToNext();
							if (connection != null) {
								Vec3d newPos = mountNode.getPositionVector().add(connection.scale(-0.5D)).add(0, 0.1D,
										0);

								RayTraceResult result = this.world.rayTraceBlocks(
										mountNode.getPositionVector().add(0, mountNode.height, 0), newPos, false);

								if (result != null && result.typeOfHit == Type.BLOCK
										&& result.hitVec.squareDistanceTo(mountNode.getPositionVector().add(0,
												mountNode.height, 0)) < newPos.squareDistanceTo(
														mountNode.getPositionVector().add(0, mountNode.height, 0))) {
									newPos = result.hitVec
											.add(result.hitVec.subtract(this.getPositionVector().add(0, this.height, 0))
													.normalize().scale(0.1D));
								}

								EntityGrapplingHookNode newNode = this.extendRope(mountNode, newPos.x, newPos.y,
										newPos.z);
							...
							if (controller instanceof EntityPlayer) {
								((EntityPlayer) controller).sendStatusMessage(
										new TextComponentTranslation("chat.grappling_hook.max_length"), true);
							}
						}
					}
				}

				if (nextNode.getDistance(this.posX, this.posY + this.height - nextNode.height, this.posZ) > this
						.getMaxRopeLength()) {
					...
					if (mountNode != null) {
						...
						if (controller instanceof EntityPlayer) {
							((EntityPlayer) controller).sendStatusMessage(
									new TextComponentTranslation("chat.grappling_hook.disconnected"), true);
						}
		...
		if (!attached) {
			...
			this.move(MoverType.SELF, this.motionX + this.correctionX, this.motionY + this.correctionY,
					this.motionZ + this.correctionZ);
		...
		if (prevNode instanceof EntityGrapplingHookNode) {
			...
			Vec3d diff = prevNode.getPositionVector().add(0, prevNode.height, 0)
					.subtract(this.getPositionVector().add(0, this.height, 0));
		...
		if (controller != null && this.isMountNode()) {
			...
			while (it.hasNext()) {
				ItemStack stack = it.next();
				if (!stack.isEmpty() && stack.getItem() instanceof ItemGrapplingHook
						&& ((ItemGrapplingHook) stack.getItem()).canRideGrapplingHook(stack, controller)) {
					hasValidUser = true;
					break;
				}
			}
	...
	protected void constrainMotion(Entity parentNode, Entity constraintNode, double ropeFriction, double constraintMin,
			double constraintDampening) {
		Vec3d nextPoint = new Vec3d(this.posX + this.motionX - parentNode.motionX,
				this.posY + this.height + this.motionY - parentNode.motionY,
				this.posZ + this.motionZ - parentNode.motionZ);

		Vec3d tetherPoint = new Vec3d(constraintNode.posX, constraintNode.posY + constraintNode.height,
				constraintNode.posZ);
		...
		if (tetherPoint.distanceTo(nextPoint) >= currentRopeLength) {
			...
			constrainedPoint = constrainedPoint.scale(currentRopeLength).add(tetherPoint.x, tetherPoint.y,
					tetherPoint.z);

			Vec3d fwd = tetherPoint.subtract(constrainedPoint).normalize();

			Vec3d relMotion = new Vec3d(this.motionX - parentNode.motionX, this.motionY - parentNode.motionY,
					this.motionZ - parentNode.motionZ);

			Vec3d side = fwd.crossProduct(new Vec3d(0, 1, 0)).normalize();
			Vec3d up = side.crossProduct(fwd).normalize();

			Vec3d newMotion = side.scale(side.dotProduct(relMotion) * 1F).add(up.scale(up.dotProduct(relMotion) * 1F))
					.add(fwd.scale(Math.max(constraintMin, fwd.dotProduct(relMotion) * constraintDampening)));
		...
		if (!this.world.isRemote) {
			if (controller.isJumping) {
				if (controller.moveForward > 0) {
					...
					if (canReelIn) {
						...
						if (prevNode instanceof EntityGrapplingHookNode) {
							...
							if (prevNode.getEntityBoundingBox().intersects(this.getEntityBoundingBox())) {
								...
								this.setCurrentRopeLength(Math.min((float) this.getDefaultRopeLength() - 0.1F,
										(float) prevNode.getDistance(this.posX,
												this.posY + this.height - prevNode.height, this.posZ)));
							}

							if (this.pullCounter % 24 == 0) {
								this.world.playSound(null, controller.posX, controller.posY, controller.posZ,
										SoundRegistry.ROPE_PULL, SoundCategory.PLAYERS, 1.5F, 1);
							}
					...
					this.setCurrentRopeLength(
							Math.min((float) this.getDefaultRopeLength() - 0.1F, this.getCurrentRopeLength() + 0.2F));
					this.isExtending = true;
				}
			} else {
				this.pullCounter = 0;

				if ((Math.abs(controller.moveForward) > 0.05D || Math.abs(controller.moveStrafing) > 0.05D)
						&& !this.onGround) {
					int count = 0;
	...
	protected void checkForEntityCollisions(Entity prevNode, Entity nextNode) {
		if (!this.world.isRemote && prevNode != null) {
			double velocity = Math.sqrt((this.posX - this.prevPosX) * (this.posX - this.prevPosX)
					+ (this.posY - this.prevPosY) * (this.posY - this.prevPosY)
			...
			if (mountNode != null) {
				...
				if (user != null && velocity > 0.25D) {
					List<EntityLivingBase> entities = this.world.getEntitiesWithinAABB(EntityLivingBase.class,
							new AxisAlignedBB(this.posX, this.posY, this.posZ, prevNode.posX, prevNode.posY,
									prevNode.posZ));

					for (EntityLivingBase entity : entities) {
						if (entity != user) {
							RayTraceResult intersect = entity.getEntityBoundingBox().calculateIntercept(
									new Vec3d(this.posX, this.posY, this.posZ),
									new Vec3d(prevNode.posX, prevNode.posY, prevNode.posZ));
	...
	public boolean isAttached() {
		...
		return !this.isMountNode() /* && this.getPreviousNode() == null */ && !this.world
				.getCollisionBoxes(this, this.getEntityBoundingBox().grow(0.1D, 0.1D, 0.1D)).isEmpty();
	}
	...
	public Vec3d getConnectionToNext() {
		...
		if (nextNode != null) {
			return new Vec3d(nextNode.posX - this.posX, nextNode.posY + nextNode.height - (this.posY + this.height),
					nextNode.posZ - this.posZ);
		}
		return null;
	}
	...
	public Entity getNextNodeByUUID() {
		if (this.cachedNextNodeEntity != null && this.cachedNextNodeEntity.isEntityAlive()
				&& this.cachedNextNodeEntity.getUniqueID().equals(this.nextNodeUUID)) {
	...
	public Entity getPreviousNodeByUUID() {
		if (this.cachedPrevNodeEntity != null && this.cachedPrevNodeEntity.isEntityAlive()
				&& this.cachedPrevNodeEntity.getUniqueID().equals(this.prevNodeUUID)) {
	...
	public Entity getNextNodeClient() {
		if (this.cachedNextNodeEntity == null || !this.cachedNextNodeEntity.isEntityAlive()
				|| this.cachedNextNodeEntity.getEntityId() != this.getDataManager().get(DW_NEXT_NODE)) {
	...
	public Entity getPreviousNodeClient() {
		if (this.cachedPrevNodeEntity == null || !this.cachedPrevNodeEntity.isEntityAlive()
				|| this.cachedPrevNodeEntity.getEntityId() != this.getDataManager().get(DW_PREV_NODE)) {
	...
	private Entity getEntityByUUID(UUID uuid) {
		for (Entity entity : (List<Entity>) this.world.getEntitiesWithinAABB(Entity.class,
				this.getEntityBoundingBox().grow(24, 24, 24))) {
	...
	public Vec3d getWeightPos(float partialTicks) {
		if (this.weightPos == null) {
			return new Vec3d(this.prevPosX + (this.posX - this.prevPosX) * partialTicks,
