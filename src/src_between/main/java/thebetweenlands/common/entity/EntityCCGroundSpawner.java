...
public class EntityCCGroundSpawner extends EntityProximitySpawner {
	private static final DataParameter<Boolean> IS_WORLD_SPANWED = EntityDataManager
			.createKey(EntityCCGroundSpawner.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> SPAWN_COUNT = EntityDataManager.createKey(EntityCCGroundSpawner.class,
			DataSerializers.VARINT);
	private static final DataParameter<Boolean> CAN_BE_REMOVED_SAFELY = EntityDataManager
			.createKey(EntityCCGroundSpawner.class, DataSerializers.BOOLEAN);
	...
	/*
	 * @Override public AxisAlignedBB getCollisionBoundingBox() { return
	...
	public void onUpdate() {
		...
		if (!getEntityWorld().isRemote) {
			...
			if (getEntityWorld().getTotalWorldTime() % 60 == 0)
				checkArea();
			List<EntityFallingBlock> listPlug = getEntityWorld().getEntitiesWithinAABB(EntityFallingBlock.class,
		...
		this.setPosition(MathHelper.floor(this.posX) + 0.5D, MathHelper.floor(this.posY),
				MathHelper.floor(this.posZ) + 0.5D);
	...
	protected Entity checkArea() {
		if (!getEntityWorld().isRemote) {
			...
			if (getEntityWorld().getDifficulty() != EnumDifficulty.PEACEFUL) {
				if (isWorldSpawned() && !isBloodSky(getEntityWorld()))
					return null;
				List<EntityLivingBase> list = getEntityWorld().getEntitiesWithinAABB(EntityLivingBase.class,
						proximityBox());
	...
	/*
	 * @Override protected boolean isMovementBlocked() { return true; }
	 */
	@Override
	...
	protected void performPreSpawnaction(Entity targetEntity, Entity entitySpawned) {
		...
		getEntityWorld().playSound((EntityPlayer) null, getPosition(), getDigSound(), SoundCategory.HOSTILE, 0.5F,
				1.0F);
		entitySpawned.setPosition(getPosition().getX() + 0.5F, getPosition().getY() - 1.5F,
				getPosition().getZ() + 0.5F);
	}
	...
	protected void performPostSpawnaction(Entity targetEntity, @Nullable Entity entitySpawned) {
		if (!getEntityWorld().isRemote) {
			TheBetweenlands.networkWrapper.sendToAll(
					new PacketParticle(ParticleType.GOOP_SPLAT, (float) posX, (float) posY + 0.25F, (float) posZ, 0F));
	...
	public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata) {
		if (!getEntityWorld().isRemote) {
			...
			getEntityWorld().setBlockState(getPosition().add(-1, 0, -1),
					blockHelper.COMPACTED_MUD_SLOPE.withProperty(BlockCompactedMudSlope.FACING, EnumFacing.NORTH)
							.withProperty(BlockCompactedMudSlope.HALF, EnumHalf.BOTTOM));
			getEntityWorld().setBlockState(getPosition().add(0, 0, -1),
					blockHelper.COMPACTED_MUD_SLOPE.withProperty(BlockCompactedMudSlope.FACING, EnumFacing.NORTH)
							.withProperty(BlockCompactedMudSlope.HALF, EnumHalf.BOTTOM));
			getEntityWorld().setBlockState(getPosition().add(1, 0, -1),
					blockHelper.COMPACTED_MUD_SLOPE.withProperty(BlockCompactedMudSlope.FACING, EnumFacing.NORTH)
							.withProperty(BlockCompactedMudSlope.HALF, EnumHalf.BOTTOM));
			getEntityWorld().setBlockState(getPosition().add(-1, 0, 1),
					blockHelper.COMPACTED_MUD_SLOPE.withProperty(BlockCompactedMudSlope.FACING, EnumFacing.SOUTH)
							.withProperty(BlockCompactedMudSlope.HALF, EnumHalf.BOTTOM));
			getEntityWorld().setBlockState(getPosition().add(0, 0, 1),
					blockHelper.COMPACTED_MUD_SLOPE.withProperty(BlockCompactedMudSlope.FACING, EnumFacing.SOUTH)
							.withProperty(BlockCompactedMudSlope.HALF, EnumHalf.BOTTOM));
			getEntityWorld().setBlockState(getPosition().add(1, 0, 1),
					blockHelper.COMPACTED_MUD_SLOPE.withProperty(BlockCompactedMudSlope.FACING, EnumFacing.SOUTH)
							.withProperty(BlockCompactedMudSlope.HALF, EnumHalf.BOTTOM));
			getEntityWorld().setBlockState(getPosition().add(-1, 0, 0),
					blockHelper.COMPACTED_MUD_SLOPE.withProperty(BlockCompactedMudSlope.FACING, EnumFacing.WEST)
							.withProperty(BlockCompactedMudSlope.HALF, EnumHalf.BOTTOM));
			getEntityWorld().setBlockState(getPosition().add(1, 0, 0),
					blockHelper.COMPACTED_MUD_SLOPE.withProperty(BlockCompactedMudSlope.FACING, EnumFacing.EAST)
							.withProperty(BlockCompactedMudSlope.HALF, EnumHalf.BOTTOM));
		}
		return livingdata;
	}
	...
	public void loadOriginBlocks(World world, NBTTagCompound tag) {
		...
		getEntityWorld().playSound((EntityPlayer) null, origin, SoundRegistry.ROOF_COLLAPSE, SoundCategory.BLOCKS, 1F,
				1.0F);
	}
