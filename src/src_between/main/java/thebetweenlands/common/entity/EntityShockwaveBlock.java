...
public class EntityShockwaveBlock extends Entity implements IEntityAdditionalSpawnData {
	private static final DataParameter<String> OWNER_DW = EntityDataManager
			.<String>createKey(EntityShockwaveBlock.class, DataSerializers.STRING);
	...
	public void onUpdate() {
		...
		if (this.motionY > 0.1D && !this.world.isRemote) {
			...
			List<EntityLivingBase> entities = this.world.getEntitiesWithinAABB(EntityLivingBase.class,
					getEntityBoundingBox().grow(0.1D, 0.1D, 0.1D));
