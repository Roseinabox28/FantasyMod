...
public class EntityWeedwoodRowboat extends EntityBoat implements IEntityAdditionalSpawnData {
	...
	private static final EnumMap<ShipSide, DataParameter<Float>> ROW_PROGRESS = ShipSide.newEnumMap(
			(Class<DataParameter<Float>>) (Class<?>) DataParameter.class, defineId(DataSerializers.FLOAT),
			defineId(DataSerializers.FLOAT));
	...
	private static final EnumMap<ShipSide, SoundEvent> SOUND_ROW = ShipSide.newEnumMap(SoundEvent.class,
			SoundRegistry.ROWBOAT_ROW_STARBOARD, SoundRegistry.ROWBOAT_ROW_PORT);

	private static final EnumMap<ShipSide, SoundEvent> SOUND_ROW_START = ShipSide.newEnumMap(SoundEvent.class,
			SoundRegistry.ROWBOAT_ROW_START_STARBOARD, SoundRegistry.ROWBOAT_ROW_START_PORT);

	private EnumMap<ShipSide, Float> rowForce = ShipSide.newEnumMap(float.class);

	private EnumMap<ShipSide, Integer> rowTime = ShipSide.newEnumMap(int.class, FORCE_SETTLE_DURATION,
			FORCE_SETTLE_DURATION);

	private EnumMap<ShipSide, Float> prevRowProgress = ShipSide.newEnumMap(float.class, RESTING_ROW_PROGRESS,
			RESTING_ROW_PROGRESS);

	private EnumMap<ShipSide, Float> rowProgress = ShipSide.newEnumMap(float.class, RESTING_ROW_PROGRESS,
			RESTING_ROW_PROGRESS);
	...
	@Override
	public void setPositionAndRotationDirect(double x, double y, double z, float yaw, float pitch, int t,
			boolean teleport) {
		serverX = x;
		serverY = y;
		serverZ = z;
	...
	public boolean attackEntityFrom(DamageSource source, float amount) {
		...
		if (!world.isRemote && !isDead) {
			...
			markVelocityChanged();
			boolean creative = source.getTrueSource() instanceof EntityPlayer
	...
	public boolean processInitialInteract(EntityPlayer player, EnumHand hand) {
		...
		if (EnumItemMisc.TAR_DRIP.isItemOf(stack) && !isTarred()) {
			if (!world.isRemote) {
				setIsTarred(true);
				stack.shrink(1);
				playSound(SoundRegistry.TAR_BEAST_STEP, 0.9F + rand.nextFloat() * 0.1F,
	...
	public void onUpdate() {
		double pow = 1 - SPEED_WAVE_POWER
				.eval(MathHelper.sqrt((posX - prevPosX) * (posX - prevPosX) + (posZ - prevPosZ) * (posZ - prevPosZ)));
		if (!world.isRemote) {
			setFlag(6, isGlowing());
		}
		onEntityUpdate();
		...
		if (canPassengerSteer()) {
			...
			if (world.isRemote) {
				...
				TheBetweenlands.networkWrapper
						.sendToServer(new MessageRow(oarState.get(ShipSide.STARBOARD), oarState.get(ShipSide.PORT),
								rowProgress.get(ShipSide.STARBOARD), rowProgress.get(ShipSide.PORT)));
			}
		...
		if (!world.isRemote) {
			world.getEntitiesWithinAABBExcludingEntity(this, getEntityBoundingBox().grow(0.2, 0.05, 0.2))
	...
	private void hitWaves(double pow) {
		...
		if (!isTarred() && canPassengerSteer()) {
			...
			if (strength > 0) {
				...
				rotationalVelocity += Math.signum(MathUtils.modularDelta(rotationYaw, dir - 90, 360))
						* Math.min((1 - normal.y) * 60 * roughness, roughness);
			}
		}
	}
	...
	private void updatePilotPull() {
		...
		if (timeStarboard > 20 && timePort > 20 && getAppropriateOarState(ShipSide.STARBOARD)
				&& getAppropriateOarState(ShipSide.PORT)
				&& getAppropriateRowProgress(ShipSide.STARBOARD) == getAppropriateRowProgress(ShipSide.PORT)) {
			if (pilotPower < 1) {
				pilotPower += 0.2F;
				if (pilotPower > 1) {
					pilotPower = 1;
	...
	private void pullOarByWave(ShipSide side, Vec3d normal) {
		...
		float angle = nf.length() < 1e-12 ? 0
				: (float) Math.acos(Math.max(Math.min(nf.dotProduct(of) / (nf.length() * of.length()), 1), -1));
		...
		oarXWavePull.put(side, x + (MathHelper.clamp(yaw * align * (float) nf.length() * 2, -0.3F, 0.3F) - x) * 0.7F
				* (float) nf.length());
		float z = oarZWavePull.get(side);
		oarZWavePull.put(side,
				z + ((pitch - MathUtils.PI / 2) * (1 - align) * (getOarElevation(side) + 1) / 2 - z) * 0.4F);
	}
	...
	private Vec3d applyRowForce() {
		...
		if (currentMotion.length() < 0.1
				&& rotation.x * rotation.x + rotation.y * rotation.y + rotation.z + rotation.z > 0) {
	...
	private void animateHullWaterInteraction() {
		...
		if (velocity > 0.2625) {
			...
			for (int p = 0; p < 1 + velocity * 60; p++) {
				...
				world.spawnParticle(EnumParticleTypes.WATER_SPLASH, splashX, Math.ceil(posY) - 0.125, splashZ, motionX,
						0.01, motionZ);
			}
		}
	}
	...
	private void animateOar(ShipSide side, double motion) {
		...
		if (raytrace != null && raytrace.typeOfHit == RayTraceResult.Type.BLOCK) {
			if (motion > 0.175) {
				for (int p = 0; p < motion; p++) {
					...
					world.spawnParticle(EnumParticleTypes.WATER_SPLASH, raytrace.hitVec.x + x, raytrace.hitVec.y + y,
							raytrace.hitVec.z + z, motionX, 0.01, motionZ);
				}
			}
		...
		if (bladeInAir && rand.nextFloat() < 0.4F) {
			for (int p = 0, count = (int) (1 + motion * 3); p < count; p++) {
				...
				float x = (float) (oarVector.x * point
						+ MathUtils.linearTransformf(rand.nextFloat(), 0, 1, -0.1F, 0.1F));
				float y = (float) (oarVector.y * point
						+ MathUtils.linearTransformf(rand.nextFloat(), 0, 1, -0.4F, -0.2F));
				float z = (float) (oarVector.z * point
						+ MathUtils.linearTransformf(rand.nextFloat(), 0, 1, -0.1F, 0.1F));
				world.spawnParticle(EnumParticleTypes.WATER_SPLASH, oarlock.x + x, oarlock.y + y, oarlock.z + z, 0,
						1e-8, 0);
			}
		}
	}
		...
		if (raytrace != null && raytrace.typeOfHit == RayTraceResult.Type.BLOCK) {
			...
			if (amountInAir < LOOM_LENGTH) {
				...
				if (oarInAir.get(side) || start) {
					...
					world.playSound(null, raytrace.hitVec.x, raytrace.hitVec.y, raytrace.hitVec.z, sound,
							SoundCategory.NEUTRAL, volume, 0.8F + rand.nextFloat() * 0.3F);
				}
			}
		}
	...
	public boolean handleWaterMovement() {
		...
		if (world.handleMaterialAcceleration(getEntityBoundingBox(), Material.WATER, this)) {
			if (mX != motionX && mZ != motionZ && canPassengerSteer()) {
				...
				rotationalVelocity += (MathHelper
						.clamp(MathUtils.modularDelta(rotationYaw, dir - 90, 360) * Math.min(speed * 1.1, 0.3), -12, 12)
						- rotationalVelocity) * 0.75;
			}
			if (!inWater) {
				float volume = MathHelper.sqrt(motionX * motionX * 0.2 + motionY * motionY + motionZ * motionZ * 0.2)
						* 0.2F;
				if (volume > 0.15) {
					if (volume > 1) {
						volume = 1;
					...
					for (int i = 0; i < 1 + width * 20; i++) {
						...
						world.spawnParticle(EnumParticleTypes.WATER_BUBBLE, posX + x, min + 1, posZ + z, motionX,
								motionY - rand.nextFloat() * 0.2F, motionZ);
					}
					for (int i = 0; i < 1 + width * 20; i++) {
						float x = (rand.nextFloat() * 2 - 1) * width;
						float z = (rand.nextFloat() * 2 - 1) * width;
						world.spawnParticle(EnumParticleTypes.WATER_SPLASH, posX + x, min + 1, posZ + z, motionX,
								motionY, motionZ);
					}
				}
			}
	...
	public float getRowProgress(ShipSide side, float delta) {
		...
		return delta * (MathUtils.mod(progress - prevProgress + ROW_PROGRESS_PERIOD / 2, ROW_PROGRESS_PERIOD)
				- ROW_PROGRESS_PERIOD / 2) + prevProgress;
	}
	...
	public float getOarRotationX(ShipSide side, float theta, float delta) {
		return MathHelper.sin(theta * EntityWeedwoodRowboat.OAR_ROTATION_SCALE) * 0.6F + prevOarXWavePull.get(side)
				+ (oarXWavePull.get(side) - prevOarXWavePull.get(side)) * delta;
	}

	public float getOarRotationY(ShipSide side, float theta) {
		float angle = MathUtils.linearTransformf(
				MathHelper.sin(theta * EntityWeedwoodRowboat.OAR_ROTATION_SCALE + MathUtils.PI / 2), -1, 1,
				MathUtils.PI / 2, 0);
	...
	public float getOarRotationZ(ShipSide side, float theta, float delta) {
		float angle = MathHelper.cos(theta * EntityWeedwoodRowboat.OAR_ROTATION_SCALE) * 0.45F - MathUtils.PI / 2.5F
				+ prevOarZWavePull.get(side) + (oarZWavePull.get(side) - prevOarZWavePull.get(side)) * delta;
		if (side == ShipSide.PORT) {
			angle = -angle;
		}
		return angle;
	...
	public static void onLivingAttacked(LivingAttackEvent event) {
		...
		if (ridingEntity instanceof EntityWeedwoodRowboat && event.getSource().getTrueSource() != null) {
			...
			if (location != null && location.y + (attacker != null ? attacker.getEyeHeight() : 0) < ridingEntity.posY
					+ ridingEntity.height / 2) {
