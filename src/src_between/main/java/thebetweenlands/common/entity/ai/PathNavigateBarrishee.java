...
public class PathNavigateBarrishee extends PathNavigateGround {
	...
	protected void checkForStuck(Vec3d entityPos) {
		...
		if (this.currentPath != null && !this.currentPath.isFinished()) {
			Vec3d target = this.currentPath.getVectorFromIndex(this.barrishee,
					Math.min(this.currentPath.getCurrentPathLength() - 1, this.currentPath.getCurrentPathIndex() + 0));
			...
			loop: for (int yo = 0; yo < height; yo++) {
				for (int xzo = -ceilHalfWidth; xzo <= ceilHalfWidth; xzo++) {
					BlockPos pos = new BlockPos(checkPos.x + (!isFacingX ? xzo : 0), checkPos.y + yo,
							checkPos.z + (isFacingX ? xzo : 0));

					IBlockState state = this.barrishee.world.getBlockState(pos);

					PathNodeType nodeType = state.getBlock().isPassable(this.barrishee.world, pos) ? PathNodeType.OPEN
							: PathNodeType.BLOCKED;

					if (nodeType == PathNodeType.BLOCKED && state.getCollisionBoundingBox(this.barrishee.world, pos)
							.offset(pos).intersects(this.barrishee.getEntityBoundingBox()
									.expand(Math.signum(diff.x) * 0.2D, 0, Math.signum(diff.z) * 0.2D))) {
						blocked = true;
						break loop;
					}
				}
