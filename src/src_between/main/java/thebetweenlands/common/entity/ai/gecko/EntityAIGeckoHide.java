...
public abstract class EntityAIGeckoHide extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		target = findNearBush();
		if (target == null) {
			Vec3d target = fleeingCausePos != null
					? RandomPositionGenerator.findRandomTargetBlockAwayFrom(gecko, 16, 7, fleeingCausePos)
					: RandomPositionGenerator.findRandomTargetBlockAwayFrom(gecko, 16, 7, gecko.getPositionVector());
			if (target != null) {
		...
		if (target == null) {
			return false;
		} else if (!bushBound && fleeingCausePos != null && fleeingCausePos.squareDistanceTo(target.getX(),
				target.getY(), target.getZ()) < fleeingCausePos.squareDistanceTo(gecko.getPositionVector())) {
			return false;
		} else {
			...
			if (path != null) {
				...
				return finalPathPoint.x == target.getX() && finalPathPoint.y == target.getY()
						&& finalPathPoint.z == target.getZ()
						|| bushBound && doesPathDestinationNeighborBush(target, path);
			}
			return false;
		}
	}
	...
	private boolean doesPathDestinationNeighborBush(BlockPos target, Path path) {
		for (EnumFacing facing : EnumFacing.VALUES) {
			...
			PathPoint finalPathPoint = path.getFinalPathPoint();
			if (finalPathPoint.x == nearTarget.getX() && finalPathPoint.y == nearTarget.getY()
					&& finalPathPoint.z == nearTarget.getZ()) {
				return true;
			}
		}
		return false;
		...
		for (int dx = -radius; dx <= radius; dx++) {
			for (int dy = -radius / 2; dy <= radius; dy++) {
				for (int dz = -radius; dz <= radius; dz++) {
					...
					if (state.getBlock() == BlockRegistry.WEEDWOOD_BUSH
							&& gecko.world.isBlockNormalCube(pos.down(), false)
							&& gecko.world
									.getEntitiesWithinAABB(EntityGecko.class, new AxisAlignedBB(pos), e -> e != gecko)
									.isEmpty()) {
	...
	public boolean shouldContinueExecuting() {
		if (target != null && !gecko.world
				.getEntitiesWithinAABB(EntityGecko.class, new AxisAlignedBB(target), e -> e != gecko).isEmpty()) {
			return false;
		}
