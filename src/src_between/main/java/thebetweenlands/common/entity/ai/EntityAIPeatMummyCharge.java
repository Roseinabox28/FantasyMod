...
public class EntityAIPeatMummyCharge extends EntityAIBase {
	...
	public void updateTask() {
		if (!this.mummy.isCharging()) {
			...
			this.chargingTime++;
			if (this.chargingTime >= this.mummy.getEntityAttribute(EntityPeatMummy.CHARGING_TIME_ATTRIB)
					.getAttributeValue()) {
	...
	public void resetTask() {
		this.chargingTime = 0;
		this.chargingCooldown = this.mummy.getMaxChargingCooldown()
				+ this.mummy.world.rand.nextInt(this.mummy.getMaxChargingCooldown() / 2 + 1);
	}
}
