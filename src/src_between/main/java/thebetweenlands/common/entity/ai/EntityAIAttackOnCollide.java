...
public class EntityAIAttackOnCollide extends EntityAIBase {
	...
	public static boolean useStandardAttack(EntityLiving attacker, Entity target, boolean knockback) {
		...
		if (attacker.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE) != null) {
			attackDamage = (float) attacker.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE)
					.getAttributeValue();
		} else {
			attackDamage = 2.0F;
		}
	...
	/**
	 * Attacks the target with the standard attack implementation of
	 * {@link EntityMob#attackEntityAsMob(Entity)}
	 * 
	 * @param attacker
	 * @param target
	 * @return
	...
	public static boolean useStandardAttack(EntityLiving attacker, Entity target, float attackDamage,
		...
		if (target instanceof EntityLivingBase) {
			attackDamage += EnchantmentHelper.getModifierForCreature(attacker.getHeldItemMainhand(),
		...
		if (attacked) {
			if (knockback && knockBackModifier > 0 && target instanceof EntityLivingBase) {
				((EntityLivingBase) target).knockBack(attacker, (float) knockBackModifier * 0.5F,
			...
			if (target instanceof EntityPlayer) {
				...
				if (attackerItem != null && defenderItem != null && attackerItem.getItem() instanceof ItemAxe
						&& defenderItem.getItem() == Items.SHIELD) {
					float efficiencyModifier = 0.25F
							+ (float) EnchantmentHelper.getEfficiencyModifier(attacker) * 0.05F;
