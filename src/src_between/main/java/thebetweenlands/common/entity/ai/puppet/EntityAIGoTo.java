...
public class EntityAIGoTo extends EntityAIBase {
	...
	public void updateTask() {
		if (this.pos != null) {
			this.taskOwner.getLookHelper().setLookPosition(this.pos.getX() + 0.5D, this.pos.getY() + 0.5D,
					this.pos.getZ() + 0.5D, 30.0F, 30.0F);
			...
			if (this.delayCounter <= 0) {
				...
				if (this.taskOwner.getNavigator().getPath() != null) {
					...
					if (finalPathPoint != null
							&& this.pos.getDistance(finalPathPoint.x, finalPathPoint.y, finalPathPoint.z) < 1) {
						this.failedPathFindingPenalty = 0;
					} else {
				...
				if (!this.taskOwner.getNavigator().tryMoveToXYZ(this.pos.getX() + 0.5D, this.pos.getY() + 0.5D,
						this.pos.getZ() + 0.5D, this.speed)) {
