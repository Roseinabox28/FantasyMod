...
public class EntityAIBLAvoidEntity extends EntityAIBase {
	...
	public EntityAIBLAvoidEntity(EntityCreature entity, Class<?> clazz, float targetDistanceFromEntity, double farSpeed,
			double nearSpeed) {
		this.entity = entity;
	...
	public boolean shouldExecute() {
		if (targetEntityClass == EntityPlayer.class) {
			...
			List list = entity.world.getEntitiesWithinAABB(targetEntityClass,
					entity.getEntityBoundingBox().grow(targetDistanceFromEntity, 3, targetDistanceFromEntity),
					Predicates.and(EntitySelectors.IS_ALIVE, EntitySelectors.CAN_AI_TARGET));
			if (list.isEmpty()) {
				return false;
			}
			closestLivingEntity = (Entity) list.get(0);
		}
		Vec3d pos = RandomPositionGenerator.findRandomTargetBlockAwayFrom(entity, 16, 7,
				new Vec3d(closestLivingEntity.posX, closestLivingEntity.posY, closestLivingEntity.posZ));
		if (pos == null) {
			return false;
