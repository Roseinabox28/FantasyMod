...
public class EntityAIApproachItem extends EntityAIBase {
	public final Predicate<Entity> entitySelector = new Predicate<Entity>() {
		...
		public boolean apply(Entity entity) {
			return entity.isEntityAlive()
					&& entity.getDistance(EntityAIApproachItem.this.entity) <= EntityAIApproachItem.this.targetDistance
					&& entity instanceof EntityItem
					&& (EntityAIApproachItem.this.ignoreDamage
							? ((EntityItem) entity).getItem().getItem() == EntityAIApproachItem.this.targetItem
									.getItem()
							: ((EntityItem) entity).getItem().isItemEqual(EntityAIApproachItem.this.targetItem));
		}
	};
	...
	public EntityAIApproachItem(EntityCreature entity, Item item, int distractionTime, float targetDistance,
			double farSpeed, double nearSpeed) {
		this(entity, new ItemStack(item), distractionTime, targetDistance, farSpeed, nearSpeed, true);
	}

	public EntityAIApproachItem(EntityCreature entity, ItemStack item, int distractionTime, float targetDistance,
			double farSpeed, double nearSpeed, boolean ignoreDamage) {
		this.entity = entity;
	...
	public boolean shouldExecute() {
		List<EntityItem> list = this.entity.world.getEntitiesWithinAABB(EntityItem.class,
				this.entity.getEntityBoundingBox().grow(this.targetDistance, 3, this.targetDistance),
				this.entitySelector);
		...
		this.entityPathEntity = this.entityPathNavigate.getPathToXYZ(this.targetEntity.posX, this.targetEntity.posY,
	...
	public boolean shouldContinueExecuting() {
		boolean cont = (this.entityPathEntity != null && this.targetEntity != null && this.targetEntity.isEntityAlive())
				|| !this.entityPathNavigate.noPath();
		return cont;
	}
