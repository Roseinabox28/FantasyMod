...
public class EntityAIHurtByTargetImproved extends EntityAITarget {
	...
	public void startExecuting() {
		...
		if (this.taskOwner.getAttackTarget() != null && this.entityCallsForHelp) {
			...
			List<EntityCreature> list = this.taskOwner.world.getEntitiesWithinAABB(this.taskOwner.getClass(),
					new AxisAlignedBB(this.taskOwner.posX, this.taskOwner.posY, this.taskOwner.posZ,
							this.taskOwner.posX + 1.0D, this.taskOwner.posY + 1.0D, this.taskOwner.posZ + 1.0D)
									.grow(dist, 10.0D, dist));
			for (EntityCreature creature : list) {
				if (this.taskOwner != creature && creature.getAttackTarget() == null
						&& !creature.isOnSameTeam(this.taskOwner.getAttackTarget())
						&& creature != this.taskOwner.getAttackTarget()) {
