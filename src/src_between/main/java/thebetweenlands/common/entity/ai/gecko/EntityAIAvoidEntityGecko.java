...
public class EntityAIAvoidEntityGecko extends EntityAIGeckoHide {
	...
	public EntityAIAvoidEntityGecko(EntityGecko gecko, Class<? extends Entity> avoidingEntityClass, float distance,
	...
	protected boolean shouldFlee() {
		if (avoidingEntityClass == EntityPlayer.class) {
			...
			List<Entity> list = gecko.world.getEntitiesWithinAABB(avoidingEntityClass,
					gecko.getEntityBoundingBox().grow(distance, 3.0D, distance), viableSelector);
			if (list.isEmpty()) {
				return false;
			}
