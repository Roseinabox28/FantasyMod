...
public class EntityAIFollowTarget extends EntityAIBase {
	public static class FollowClosest implements Supplier<EntityLivingBase> {
		...
		public EntityLivingBase get() {
			List<EntityLivingBase> entities = this.taskOwner.world.getEntitiesWithinAABB(this.type,
					this.taskOwner.getEntityBoundingBox().grow(this.range));
	...
	public EntityAIFollowTarget(EntityLiving taskOwner, Supplier<EntityLivingBase> target, double speed, float minDist,
			float maxDist, boolean teleport) {
		this.taskOwner = taskOwner;
	...
	public boolean shouldContinueExecuting() {
		EntityLivingBase target = this.target.get();
		return target != null && !this.navigator.noPath()
				&& this.taskOwner.getDistanceSq(target) > (double) (this.maxDist * this.maxDist);
	}
	...
	public void updateTask() {
		...
		if (target != null) {
			this.taskOwner.getLookHelper().setLookPositionWithEntity(target, 10.0F,
					(float) this.taskOwner.getVerticalFaceSpeed());
							...
							for (int l = 0; l <= 4; ++l) {
								for (int i1 = 0; i1 <= 4; ++i1) {
									if ((l < 1 || i1 < 1 || l > 3 || i1 > 3)
										...
										this.taskOwner.setLocationAndAngles((double) ((float) (i + l) + 0.5F),
												(double) k, (double) ((float) (j + i1) + 0.5F),
												this.taskOwner.rotationYaw, this.taskOwner.rotationPitch);
										this.navigator.clearPath();
										return;
									}
								}
