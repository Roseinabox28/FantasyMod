...
public class EntityAIFlyRandomly<T extends EntityLiving> extends EntityAIBase {
	...
	public void startExecuting() {
		...
		this.entity.getMoveHelper().setMoveTo(this.getTargetX(random, this.distanceMultiplier),
				this.getTargetY(random, this.distanceMultiplier), this.getTargetZ(random, this.distanceMultiplier),
				this.getFlightSpeed());
	}
	 ...
	 * 
	 * @param rand               Random
	 * @param distanceMultiplier Decreases if path is blocked or can't find a valid
	 *                           path. Ranges from 0.1 to 1.0
	 * @return
	 */
