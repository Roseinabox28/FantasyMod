...
public class FlyingNodeProcessorBL extends NodeProcessor {
	...
	public PathPoint getStart() {
		...
		if (this.entity.getPathPriority(startNodeType) < 0.0F) {
			...
			set.add(new BlockPos(this.entity.getEntityBoundingBox().minX, startY,
					this.entity.getEntityBoundingBox().minZ));
			set.add(new BlockPos(this.entity.getEntityBoundingBox().minX, startY,
					this.entity.getEntityBoundingBox().maxZ));
			set.add(new BlockPos(this.entity.getEntityBoundingBox().maxX, startY,
					this.entity.getEntityBoundingBox().minZ));
			set.add(new BlockPos(this.entity.getEntityBoundingBox().maxX, startY,
					this.entity.getEntityBoundingBox().maxZ));

			for (BlockPos blockpos : set) {
				PathNodeType pathnodetype = this.isFree(blockpos.getX(), blockpos.getY(), blockpos.getZ());// this.getPathNodeType(this.entity,
																											// blockpos);
	...
	public int findPathOptions(PathPoint[] pathOptions, PathPoint currentPoint, PathPoint targetPoint,
		...
		for (EnumFacing enumfacing : EnumFacing.VALUES) {
			PathPoint pathpoint = this.getAirNode(currentPoint.x + enumfacing.getXOffset(),
					currentPoint.y + enumfacing.getYOffset(), currentPoint.z + enumfacing.getZOffset(), targetPoint);
		...
		if (pathnodetype == PathNodeType.OPEN) {
			...
			if (this.preferredMinHeight > 0) {
				...
				if (height <= this.preferredMinHeight && height > 0) {
					double heightCostPenalty = (height / (double) this.preferredMinHeight * 4 + 1) * 2.0D
							* Math.sqrt(MathHelper.clamp((targetPoint.distanceTo(pathPoint) - height) / (double) height,
									0.0D, 1.0D));
		...
		for (int i = x; i < x + this.entitySizeX; ++i) {
			for (int j = y; j < y + this.entitySizeY; ++j) {
				for (int k = z; k < z + this.entitySizeZ; ++k) {
					...
					state.addCollisionBoxToList(this.entity.world, checkPos, new AxisAlignedBB(x, y, z,
							x + this.entitySizeX, y + this.entitySizeY, z + this.entitySizeZ), collidingAABBs,
							this.entity, true);
