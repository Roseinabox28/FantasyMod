...
public class PathNavigateFlyingBL extends PathNavigate {
	...
	protected void pathFollow() {
		...
		if (currentPosition.squareDistanceTo(this.currentPath.getVectorFromIndex(this.entity,
		...
		for (int j = Math.min(this.currentPath.getCurrentPathIndex() + 6,
			...
			if (pathNodePosition.squareDistanceTo(currentPosition) <= 256.0D
					&& this.isDirectPathBetweenPoints(currentPosition, pathNodePosition, 0, 0, 0)) {
	...
	protected boolean isDirectPathBetweenPoints(Vec3d start, Vec3d end, int sizeX, int sizeY, int sizeZ) {
		RayTraceResult raytraceresult = this.world.rayTraceBlocks(start,
				new Vec3d(end.x, end.y + (double) this.entity.height * 0.5D, end.z), false, true, false);
	...
	public Path getPathToEntityLiving(Entity entityIn) {
		if (!this.canNavigate()) {
			...
			if (this.currentPath != null && !this.currentPath.isFinished() && blockpos.equals(this.targetPos)) {
				...
				int i = (int) (f + 16.0F);
				ChunkCache chunkcache = new ChunkCache(this.world, blockpos1.add(-i, -i, -i), blockpos1.add(i, i, i),
						0);
				Path path = this.getPathFinder().findPath(chunkcache, this.entity, new BlockPos(entityIn.posX,
						entityIn.getEntityBoundingBox().minY + entityIn.height / 2.0D, entityIn.posZ), f);
				this.world.profiler.endSection();
				return path;
			}
		}
