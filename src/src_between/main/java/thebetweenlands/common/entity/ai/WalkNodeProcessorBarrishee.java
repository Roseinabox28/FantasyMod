...
public class WalkNodeProcessorBarrishee extends WalkNodeProcessor {
			...
			for (int j = 0; j < ySize; ++j) {
				for (int k = 0; k < zSize; ++k) {
					...
					if (nodeType == PathNodeType.RAIL
							&& !(world.getBlockState(entityPos).getBlock() instanceof BlockRailBase)
							&& !(world.getBlockState(entityPos.down()).getBlock() instanceof BlockRailBase)) {
	...
	protected PathNodeType getPathNodeTypeRaw(IBlockAccess world, int x, int y, int z) {
		...
		List<LocationSludgeWormDungeon> locations = BetweenlandsWorldStorage.forWorld(this.barrishee.world)
				.getLocalStorageHandler().getLocalStorages(LocationSludgeWormDungeon.class, new AxisAlignedBB(pos),
						location -> location.getGuard() != null);
