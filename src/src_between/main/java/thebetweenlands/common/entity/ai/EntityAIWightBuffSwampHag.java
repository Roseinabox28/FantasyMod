...
public class EntityAIWightBuffSwampHag extends EntityAIBase {
	...
	public boolean shouldExecute() {
		boolean canBuff = this.wight.getAttackTarget() != null && !this.wight.isRiding()
				&& this.getTargetSwampHag() != null;
		if (canBuff) {
	...
	public void updateTask() {
		if (this.hag != null) {
			if (!this.wight.isVolatile()) {
				...
				TheBetweenlands.networkWrapper.sendToAllAround(new MessageWightVolatileParticles(this.wight),
						new TargetPoint(this.wight.dimension, this.wight.posX, this.wight.posY, this.wight.posZ, 32));
				this.world.playSound(null, this.wight.posX, this.wight.posY, this.wight.posZ,
						SoundRegistry.WIGHT_ATTACK, SoundCategory.HOSTILE, 1.6F, 1.0F);
			}
			...
			if (!this.wight.isRiding()) {
				...
				this.wight.getLookHelper().setLookPositionWithEntity(this.hag, 10.0F,
						(float) this.wight.getVerticalFaceSpeed());
	...
	protected EntitySwampHag getTargetSwampHag() {
		...
		if (target != null) {
			...
			for (EntitySwampHag hag : nearby) {
				if (hag.getAttackTarget() == target && hag.getPassengers().isEmpty()
						&& hag.getDistance(this.wight) <= range && (closestSuitableToTarget == null
								|| hag.getDistance(target) <= closestSuitableToTarget.getDistance(target))) {
