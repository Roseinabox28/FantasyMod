...
public class EntitySapSpit extends EntityThrowable {
	...
	public void onUpdate() {
		...
		if (this.world.isRemote) {
			this.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, this.posX, this.posY, this.posZ, 0.0D, 0.0D, 0.0D,
					Item.getIdFromItem(ItemRegistry.SAP_SPIT));
		}
	}
		...
		if (id == 3) {
			for (int i = 0; i < 16; ++i) {
				this.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, this.posX, this.posY, this.posZ, 0.0D, 0.0D,
						0.0D, Item.getIdFromItem(ItemRegistry.SAP_SPIT));
			}
		}
	}
	...
	protected void onImpact(RayTraceResult result) {
		if (!this.world.isRemote && result.entityHit != this.thrower
				&& result.entityHit instanceof EntitySpiritTreeFace == false
				&& result.entityHit instanceof EntityRootGrabber == false) {
