...
public class EntityElixir extends EntityThrowable {
	private static final DataParameter<ItemStack> ITEM = EntityDataManager.createKey(EntityElixir.class,
	...
	protected void onImpact(RayTraceResult result) {
		if (!this.world.isRemote) {
			...
			if (!hitEntities.isEmpty()) {
				...
				while (hitEntitiesIT.hasNext()) {
					...
					if (entityDst < 16.0D) {
						...
						SplashPotionEvent event = new SplashPotionEvent(this, affectedEntity, effect,
			...
			this.entityDropItem(
					new ItemStack(ItemRegistry.ELIXIR.getDentrothystType(getElixirStack()) == EnumDentrothyst.GREEN
							? ItemRegistry.DENTROTHYST_SHARD_GREEN
							: ItemRegistry.DENTROTHYST_SHARD_ORANGE, this.world.rand.nextInt(2) + 2),
					this.height / 2);
			if (this.world.rand.nextInt(2) == 0)
				this.entityDropItem(EnumItemMisc.RUBBER_BALL.create(1), this.height / 2);
			this.setDead();
		}
	}
