...
public class EntityPredatorArrowGuide extends Entity {
	private static final DataParameter<Integer> TARGET = EntityDataManager.createKey(EntityPredatorArrowGuide.class,
	...
	public boolean isInRangeToRenderDist(double distance) {
		Entity ridingEntity = this.getRidingEntity();
		return ridingEntity != null ? ridingEntity.isInRangeToRenderDist(distance)
				: super.isInRangeToRenderDist(distance);
	}
	...
	protected void updateHomingTrajectory(Entity mountedEntity) {
		if (!this.world.isRemote) {
			...
			double distanceMovedSq = (mountedEntity.prevPosX - mountedEntity.posX)
					* (mountedEntity.prevPosX - mountedEntity.posX)
					+ (mountedEntity.prevPosY - mountedEntity.posY) * (mountedEntity.prevPosY - mountedEntity.posY)
					+ (mountedEntity.prevPosZ - mountedEntity.posZ) * (mountedEntity.prevPosZ - mountedEntity.posZ);
			...
			if (speed > 0.1D && distanceMovedSq > 0.01D && !mountedEntity.onGround) {
				...
				List<EntityLivingBase> nearbyEntities = this.world.getEntitiesWithinAABB(EntityLivingBase.class,
						this.getEntityBoundingBox().grow(maxFollowReach),
						entity -> entity.isEntityAlive() && entity instanceof IMob);
			...
			if (bestTarget != null) {
				Vec3d newMotion = bestTarget.getPositionEyes(1).subtract(mountedEntity.getPositionEyes(1)).normalize()
						.scale(speed * correctionMultiplier).add(motion.scale(1 - correctionMultiplier));
