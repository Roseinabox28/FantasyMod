...
public class EntitySludgeBall extends EntityThrowable {
	...
	protected void onImpact(RayTraceResult collision) {
		if (collision.typeOfHit == RayTraceResult.Type.BLOCK) {
			...
			if (state.getBlock().canCollideCheck(state, false)) {
				...
				state.addCollisionBoxToList(this.world, collision.getBlockPos(),
						this.getEntityBoundingBox().offset(this.motionX, this.motionY, this.motionZ), aabbs, this,
						true);
				if (!aabbs.isEmpty()) {
					...
					if (collision.sideHit.getAxis() == Axis.Y) {
						...
						if (this.bounces >= 3) {
							...
							getEntityWorld().playSound(null, getPosition(), SoundEvents.ENTITY_SLIME_SQUISH,
									SoundCategory.HOSTILE, 1, 0.9f);
		...
		if (collision.typeOfHit == RayTraceResult.Type.ENTITY) {
			if (!(collision.entityHit instanceof EntityPeatMummy)
					&& !(collision.entityHit instanceof EntityDreadfulMummy)) {
	...
	private void explode() {
		if (!this.getEntityWorld().isRemote) {
			float radius = 3;
			AxisAlignedBB region = new AxisAlignedBB(this.posX - radius, this.posY - radius, this.posZ - radius,
			...
			for (Entity entity : entities) {
				if (entity instanceof EntityLivingBase && !(entity instanceof EntityPeatMummy)
						&& !(entity instanceof EntityDreadfulMummy) && getDistanceSq(entity) < radiusSq) {
					this.attackEntity(entity);
				}
			}
			getEntityWorld().playSound(null, getPosition(), SoundEvents.ENTITY_SLIME_SQUISH, SoundCategory.HOSTILE, 1,
					0.5f);
			getEntityWorld().playSound(null, getPosition(), SoundEvents.ENTITY_SMALL_SLIME_SQUISH,
	...
	private void spawnBounceParticles(int amount) {
		for (int i = 0; i <= amount; i++) {
			this.getEntityWorld().spawnParticle(EnumParticleTypes.SLIME,
					this.posX + (amount / 8) * (this.rand.nextFloat() - 0.5), this.posY + 0.3,
					this.posZ + (amount / 8) * (this.rand.nextFloat() - 0.5), 0, 0, 0);
		}
	}
