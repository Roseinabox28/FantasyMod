		...
		if (result.entityHit != null) {
			if (result.entityHit instanceof EntityLivingBase && !(result.entityHit instanceof EntityBloodSnail)) {
				if (result.entityHit.attackEntityFrom(
						getThrower() != null ? DamageSource.causeIndirectDamage(this, getThrower()).setProjectile()
								: DamageSource.causeThrownDamage(this, null),
						1.0F)) {
					if (!world.isRemote) {
						((EntityLivingBase) result.entityHit)
								.addPotionEffect(new PotionEffect(MobEffects.POISON, 5 * 20, 0));
						this.setDead();
					}
				} else {
					this.motionX *= -0.1D;
			...
			if (result.typeOfHit == Type.BLOCK) {
				...
				if (collisionBox != null
						&& collisionBox.offset(result.getBlockPos()).intersects(this.getEntityBoundingBox())) {
					this.setDead();
				}
			} else {
				this.setDead();
