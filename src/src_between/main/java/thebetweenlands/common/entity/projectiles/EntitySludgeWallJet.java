...
public class EntitySludgeWallJet extends EntityThrowable {
	...
	public void onUpdate() {
		...
		if (getEntityWorld().isRemote)
			for (int i = 0; i < 8; ++i) {
				...
				getEntityWorld().spawnParticle(EnumParticleTypes.ITEM_CRACK, posX, posY, posZ, velX, velY, velZ,
						Item.getIdFromItem(Item.getItemFromBlock(BlockRegistry.MUD_BRICK_STAIRS_DECAY_3)));
			}
	}
	...
	public void handleStatusUpdate(byte id) {
		if (id == 3)
			for (int i = 0; i < 16; ++i)
				getEntityWorld().spawnParticle(EnumParticleTypes.ITEM_CRACK, posX, posY, posZ, 0.0D, 0.0D, 0.0D,
						Item.getIdFromItem(Item.getItemFromBlock(BlockRegistry.MUD_BRICK_STAIRS_DECAY_3)));
	}
	...
	protected void onImpact(RayTraceResult mop) {
		if (!getEntityWorld().isRemote) {
			if (!playedSound) {
				getEntityWorld().playSound((EntityPlayer) null, getPosition(), getSplashSound(), SoundCategory.HOSTILE,
						0.25F, 2.0F);
				playedSound = true;
			}
