...
public class EntitySpikeWave extends Entity implements IEntityAdditionalSpawnData {
	...
	public void addPosition(BlockPos pos) {
		...
		this.renderingBounds = this.blockEnclosingBounds.offset(this.posX - (this.origin.getX() + 0.5D),
				this.posY - this.origin.getY(), this.posZ - (this.origin.getZ() + 0.5D));
	}
		...
		if (this.modelParts == null) {
			...
			for (BlockPos pos : this.positions) {
				...
				for (int i = 0; i < models; i++) {
					Vec3d offset = new Vec3d(pos.getX() + this.rand.nextDouble() * 0.6D - 0.3D - this.posX,
							pos.getY() + 0.25D - this.posY,
							pos.getZ() + this.rand.nextDouble() * 0.6D - 0.3D - this.posZ);
					float scale = 0.4F + this.rand.nextFloat() * 0.2F;
					SpikeRenderer renderer = new SpikeRenderer(2, scale * 0.5F, scale, 1, this.rand.nextLong(),
							offset.x, offset.y, offset.z).build(DefaultVertexFormats.OLDMODEL_POSITION_TEX_NORMAL,
	...
	public void onUpdate() {
		...
		if (this.ticksExisted >= this.delay) {
			if (this.world.isRemote && this.ticksExisted == this.delay) {
				...
				this.world.playSound(this.posX, this.posY, this.posZ, SoundRegistry.SPIRIT_TREE_SPIKES,
						SoundCategory.HOSTILE, 0.7F, 0.9F + this.rand.nextFloat() * 0.2F, false);
			}
		...
		if (this.motionY > 0.1D && !this.world.isRemote) {
			...
			for (BlockPos pos : this.positions) {
				AxisAlignedBB aabb = new AxisAlignedBB(pos).offset(this.posX - (this.origin.getX() + 0.5D),
						this.posY - this.origin.getY(), this.posZ - (this.origin.getZ() + 0.5D)).shrink(0.1D)
						.offset(0, 0.2D, 0);
	...
	private void spawnEmergeParticles() {
		if (!this.positions.isEmpty()) {
			...
			for (int i = 0; i < particles; i++) {
				...
				ParticleRootSpike particle = (ParticleRootSpike) BLParticles.ROOT_SPIKE.spawn(this.world, x, y, z,
						ParticleArgs.get().withMotion(mx, my, mz));
			...
			for (BlockPos pos : this.positions) {
				...
				if (!state.getBlock().isAir(state, this.world, pos)) {
					...
					for (int i = 0; i < dustParticles; i++) {
						...
						this.world.spawnParticle(EnumParticleTypes.BLOCK_DUST, x, y, z, mx, my, mz,
