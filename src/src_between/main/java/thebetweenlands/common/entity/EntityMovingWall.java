...
public class EntityMovingWall extends Entity implements IEntityScreenShake {
	private static final DataParameter<Boolean> IS_NEW_SPAWN = EntityDataManager.createKey(EntityMovingWall.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> HOLD_STILL = EntityDataManager.createKey(EntityMovingWall.class,
			DataSerializers.BOOLEAN);
	...
	private final ItemStack renderStack1 = new ItemStack(BlockRegistry.MUD_BRICKS_CARVED.getDefaultState().getBlock(),
			1, 8);
	private final ItemStack renderStack2 = new ItemStack(BlockRegistry.MUD_BRICKS_CARVED.getDefaultState().getBlock(),
			1, 2);
	private final ItemStack renderStack3 = new ItemStack(BlockRegistry.MUD_BRICKS_CARVED.getDefaultState().getBlock(),
			1, 12);
					...
					if (!entity.canBePushed() && entity instanceof EntityMovingWall == false) {
						...
						if (squished) {
							...
							if (!this.world.isRemote) {
								...
								setHoldStill(true);
								holdCount = 20;
								getEntityWorld().playSound(null, getPosition(), SoundRegistry.WALL_SLAM,
										SoundCategory.HOSTILE, 0.5F, 0.75F);
							}
						}
					}
				}
	...
	private void checkSpawnArea() {
		...
		if (isUnbreakableBlock(getEntityWorld().getBlockState(posEntity.add(2, 0, 0)).getBlock())
				&& isUnbreakableBlock(getEntityWorld().getBlockState(posEntity.add(-2, 0, 0)).getBlock())) {
			motionZ = 0.05F;
			setIsNewSpawn(false);
		} else if (isUnbreakableBlock(getEntityWorld().getBlockState(posEntity.add(0, 0, 2)).getBlock())
				&& isUnbreakableBlock(getEntityWorld().getBlockState(posEntity.add(0, 0, -2)).getBlock())) {
			motionX = 0.05F;
			setIsNewSpawn(false);
		} else {
			setDead();
	...
	public void calculateAllCollisions(double posX, double posY, double posZ) {
		...
		Vec3d vec3d1 = new Vec3d(posX + motionX * 12D, posY + motionY, posZ + motionZ * 12D); // adjust multiplier
																								// higher for slower
																								// speeds
		RayTraceResult raytraceresult = world.rayTraceBlocks(vec3d, vec3d1);
		...
		List<Entity> list = world.getEntitiesWithinAABBExcludingEntity(this,
				getCollisionBoundingBox().expand(motionX, motionY, motionZ).grow(1.0D));
		double d0 = 0.0D;
			...
			if (isUnbreakableBlock(state.getBlock())) { // not sure of all the different states so default will do
				if (result.sideHit.getIndex() == 2 || result.sideHit.getIndex() == 3) {
					...
					if (!getEntityWorld().isRemote) {
						setHoldStill(true);
						holdCount = 20;
						getEntityWorld().playSound(null, getPosition(), SoundRegistry.WALL_SLAM, SoundCategory.HOSTILE,
								0.5F, 0.75F);
					}
	...
	public AxisAlignedBB getEntityBoundingBox() {
		if (getHorizontalFacing() == EnumFacing.NORTH || getHorizontalFacing() == EnumFacing.SOUTH)
			return new AxisAlignedBB(posX - 0.5D, posY - 0.5D, posZ - 0.5D, posX + 0.5D, posY + 0.5D, posZ + 0.5D)
					.grow(1D, 1D, 0D).offset(0D, 0.5D, 0D);
		return new AxisAlignedBB(posX - 0.5D, posY - 0.5D, posZ - 0.5D, posX + 0.5D, posY + 0.5D, posZ + 0.5D)
				.grow(0D, 1D, 1D).offset(0D, 0.5D, 0D);
	}
