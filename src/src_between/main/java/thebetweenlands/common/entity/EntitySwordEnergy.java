...
public class EntitySwordEnergy extends Entity implements IEntityAdditionalSpawnData {
	...
	private static final DataParameter<Float> PART_POS_1 = EntityDataManager.<Float>createKey(EntitySwordEnergy.class,
			DataSerializers.FLOAT);
	private static final DataParameter<Float> PART_POS_2 = EntityDataManager.<Float>createKey(EntitySwordEnergy.class,
			DataSerializers.FLOAT);
	private static final DataParameter<Float> PART_POS_3 = EntityDataManager.<Float>createKey(EntitySwordEnergy.class,
			DataSerializers.FLOAT);
	private static final DataParameter<Float> PART_POS_4 = EntityDataManager.<Float>createKey(EntitySwordEnergy.class,
			DataSerializers.FLOAT);
	...
	public void onUpdate() {
		...
		if (!world.isRemote) {
			if (ticksExisted % 140 == 0)
				world.playSound(null, posX, posY, posZ, SoundRegistry.FORTRESS_PUZZLE_ORB, SoundCategory.BLOCKS, 1.0F,
						1.0F);
			...
			if (getSwordPart1Pos() <= 0 && getSwordPart2Pos() <= 0 && getSwordPart3Pos() <= 0
					&& getSwordPart4Pos() <= 0) {
				world.playSound(null, posX, posY, posZ, SoundRegistry.FORTRESS_PUZZLE_SWORD, SoundCategory.BLOCKS, 1.0F,
						1.0F);
				EntityItem entityItem = new EntityShockwaveSwordItem(world, posX, posY, posZ,
						new ItemStack(ItemRegistry.SHOCKWAVE_SWORD));
