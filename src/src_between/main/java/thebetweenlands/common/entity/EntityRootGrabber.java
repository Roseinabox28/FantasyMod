...
public class EntityRootGrabber extends Entity implements IEntityAdditionalSpawnData {
	public static final DataParameter<Float> DAMAGE = EntityDataManager.createKey(EntityRootGrabber.class,
			DataSerializers.FLOAT);
	public static final DataParameter<Boolean> RETRACT = EntityDataManager.createKey(EntityRootGrabber.class,
		...
		if (this.modelParts == null) {
			...
			for (int j = 0; j < rings; j++) {
				...
				for (int i = 0; i < roots; i++) {
					...
					if (!this.isChains) {
						final SpikeRenderer renderer = new SpikeRenderer(3, scale * 0.5F, scale, 1,
								this.rand.nextLong(), -scale * 0.5F * 1.5F, 0, -scale * 0.5F * 1.5F).build(
										DefaultVertexFormats.OLDMODEL_POSITION_TEX_NORMAL,
										Minecraft.getMinecraft().getTextureMapBlocks()
												.getAtlasSprite(ParticleRootSpike.SPRITE.toString()));
	...
	public int getBrightnessForRender() {
		BlockPos.MutableBlockPos pos = new BlockPos.MutableBlockPos(MathHelper.floor(this.posX), 0,
	...
	public void onUpdate() {
		...
		if (this.attackTicks >= this.delay) {
			if (this.attackTicks == this.delay) {
				if (!this.world.isRemote) {
					List<EntityLivingBase> targets = this.world.getEntitiesWithinAABB(EntityLivingBase.class,
							...
							e -> !e.getIsInvulnerable() && (e instanceof EntityPlayer == false
									|| (!((EntityPlayer) e).isSpectator() && !((EntityPlayer) e).isCreative())));
					if (!targets.isEmpty()) {
						this.grabbedEntity = targets.get(this.rand.nextInt(targets.size()));
						this.grabbedEntity.setLocationAndAngles(this.posX, this.posY + 1, this.posZ,
								this.grabbedEntity.rotationYaw, this.grabbedEntity.rotationPitch);
						...
						if (this.grabbedEntity instanceof EntityPlayerMP) {
							((EntityPlayerMP) this.grabbedEntity).connection.setPlayerLocation(this.grabbedEntity.posX,
									this.grabbedEntity.posY, this.grabbedEntity.posZ, this.grabbedEntity.rotationYaw,
									this.grabbedEntity.rotationPitch);
						}
					}
				} else {
					this.spawnExtendParticles();
					this.world.playSound(this.posX, this.posY, this.posZ, SoundRegistry.SPIRIT_TREE_SPIKE_TRAP,
							SoundCategory.HOSTILE, 1, 0.9F + this.rand.nextFloat() * 0.2F, false);
				}
			}
			...
			if (!this.world.isRemote && this.grabbedEntity != null && !this.dataManager.get(RETRACT)) {
				if (this.getEntityBoundingBox().intersects(this.grabbedEntity.getEntityBoundingBox())) {
					this.grabbedEntity
							.addPotionEffect(new PotionEffect(ElixirEffectRegistry.ROOT_BOUND, 5, 0, true, false));
				} else {
		...
		if (this.world.isRemote && (this.attackTicks <= 5 || retracting)) {
			...
			if (this.emergeSound && !retracting) {
				...
				this.world.playSound(this.posX, this.posY, this.posZ, SoundRegistry.SPIRIT_TREE_SPIKE_TRAP_EMERGE,
						SoundCategory.HOSTILE, 1, 0.9F + this.rand.nextFloat() * 0.2F, false);
			}
		...
		if (!this.isChains) {
			for (int i = 0; i < 64; i++) {
				...
				if (!state.getBlock().isAir(state, this.world, pos)) {
					this.world.spawnParticle(EnumParticleTypes.BLOCK_DUST, this.posX + dx,
							MathHelper.floor(this.posY + dy) + 1 + this.rand.nextDouble() * 0.5D, this.posZ + dz, mx,
							my, mz, Block.getStateId(state));
				}
			}
			...
			for (int i = 0; i < 8; i++) {
				...
				ParticleRootSpike particle = (ParticleRootSpike) BLParticles.ROOT_SPIKE.spawn(this.world,
						this.posX + dx, this.posY + dy, this.posZ + dz,
						ParticleArgs.get().withMotion(mx, my, mz).withScale(0.4F));
	...
	protected void spawnBlockDust() {
		for (int i = 0; i < 8; i++) {
			...
			IBlockState state = this.world.getBlockState(pos);
			if (!state.getBlock().isAir(state, this.world, pos)) {
				this.world.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + dx,
						MathHelper.floor(this.posY + dy) + 1, this.posZ + dz, mx, my, mz, Block.getStateId(state));
			}
		}
	}
	...
	public void handleStatusUpdate(byte id) {
		...
		if (id == EVENT_BROKEN) {
			for (int i = 0; i < 128; i++) {
				...
				double my = (this.rand.nextDouble() - 0.5D) * 0.15D;
				double mz = (this.rand.nextDouble() - 0.5D) * 0.15D;
				this.world.spawnParticle(EnumParticleTypes.BLOCK_DUST, this.posX + dx, this.posY + dy, this.posZ + dz,
						mx, my, mz, Block.getStateId(BlockRegistry.LOG_SPIRIT_TREE.getDefaultState()));
			}
			...
			for (int i = 0; i < 8; i++) {
				...
				double my = (this.rand.nextDouble() - 0.5D) * 0.15D;
				double mz = (this.rand.nextDouble() - 0.5D) * 0.15D;
				this.world.spawnParticle(EnumParticleTypes.BLOCK_CRACK, this.posX + dx, this.posY + dy, this.posZ + dz,
						mx, my, mz, Block.getStateId(BlockRegistry.LOG_SPIRIT_TREE.getDefaultState()));
			}

			SoundType soundType = SoundType.WOOD;
			this.world.playSound(this.posX, this.posY, this.posZ, soundType.getHitSound(), SoundCategory.NEUTRAL,
					(soundType.getVolume() + 1.0F) / 8.0F, soundType.getPitch() * 0.5F, false);
		}
	}
