...
public class EntityVolarkite extends Entity {
	...
	public void onEntityUpdate() {
		...
		if (this.isInWater() || this.isInLava()) {
			invFriction *= 0.8F;
			if (this.world.getBlockState(new BlockPos(this.posX, this.posY + this.height + 0.75D, this.posZ))
		...
		Vec3d kiteDir = new Vec3d(Math.cos(Math.toRadians(this.rotationYaw + 90)), 0,
				Math.sin(Math.toRadians(this.rotationYaw + 90)));
		...
		if (controller != null) {
			...
			if (this.motionY < 0 && !this.onGround) {
				double speedBoost = -this.motionY * 0.1D
						+ MathHelper.clamp(Math.sin(Math.toRadians(this.rotationPitch)) * 0.5F, -0.02D, 0.02D);
			...
			Vec3d controllerDir = new Vec3d(Math.cos(Math.toRadians(controller.rotationYaw + 90)), 0,
					Math.sin(Math.toRadians(controller.rotationYaw + 90)));
			double rotDiff = Math.toDegrees(Math.acos(kiteDir.dotProduct(controllerDir)))
					* -Math.signum(kiteDir.crossProduct(controllerDir).y);
			...
			while (it.hasNext()) {
				...
				if (!stack.isEmpty() && stack.getItem() instanceof ItemVolarkite
						&& ((ItemVolarkite) stack.getItem()).canRideKite(stack, controller)) {
					hasValidUser = true;
					break;
				}
			}
	...
	protected void updateUpdraft() {
		...
		for (int i = 0; i <= range; i++) {
			...
			} else if (state.getMaterial() == Material.FIRE || state.getMaterial() == Material.LAVA
					|| block instanceof BlockFire || block == BlockRegistry.OCTINE_ORE
					|| block == BlockRegistry.OCTINE_BLOCK) {
				this.updraftTicks = 25;
				hasSource = true;
			} else if (state.getMaterial() == Material.ICE || state.getMaterial() == Material.SNOW
					|| state.getMaterial() == Material.CRAFTED_SNOW || state.getMaterial() == Material.PACKED_ICE) {
				this.downdraftTicks = 25;
				hasSource = true;
		...
		if (this.updraftTicks > 0 || this.downdraftTicks > 0) {
			...
			if (this.world.isRemote) {
				for (int i = 0; i < (this.downdraftTicks > 0 ? 2 : 10); i++) {
					...
					this.world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, this.posX + offsetX,
							this.draftSourcePos
									+ (this.posY + (this.downdraftTicks > 0 ? 2.4D : 1) - this.draftSourcePos)
											* this.world.rand.nextFloat(),
							this.posZ + offsetZ, this.motionX,
							this.motionY + (this.downdraftTicks > 0 ? -0.15D : 0.25D), this.motionZ);
				}
			}
		}
	...
	public boolean isInRangeToRenderDist(double distance) {
		return super.isInRangeToRenderDist(distance) || (this.getControllingPassenger() != null
				&& this.getControllingPassenger().isInRangeToRenderDist(distance));
	}
}
