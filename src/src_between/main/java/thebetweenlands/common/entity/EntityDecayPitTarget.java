...
public class EntityDecayPitTarget extends Entity implements IEntityMultiPartPitTarget, IEntityMusic {
	...
	private static final DataParameter<Float> ANIMATION_TICKS = EntityDataManager.createKey(EntityDecayPitTarget.class,
			DataSerializers.FLOAT);
	private static final DataParameter<Boolean> IS_RAISING = EntityDataManager.createKey(EntityDecayPitTarget.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> IS_MOVING = EntityDataManager.createKey(EntityDecayPitTarget.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> IS_SLOW = EntityDataManager.createKey(EntityDecayPitTarget.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> PROGRESS = EntityDataManager.createKey(EntityDecayPitTarget.class,
			DataSerializers.VARINT);
	private static final DataParameter<Boolean> TARGET_N_ACTIVE = EntityDataManager
			.createKey(EntityDecayPitTarget.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> TARGET_E_ACTIVE = EntityDataManager
			.createKey(EntityDecayPitTarget.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> TARGET_S_ACTIVE = EntityDataManager
			.createKey(EntityDecayPitTarget.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> TARGET_W_ACTIVE = EntityDataManager
			.createKey(EntityDecayPitTarget.class, DataSerializers.BOOLEAN);
	...
	public EntityDecayPitTarget(World world) {
		super(world);
		setSize(5F, 5F);
		parts = new EntityDecayPitTargetPart[] { shield_1 = new EntityDecayPitTargetPart(this, "part1", 1F, 1F, true),
	...
	public void onUpdate() {
		...
		if (!getEntityWorld().isRemote) { // upsy-daisy
			if (getProgress() > MIN_PROGRESS && getEntityWorld().getTotalWorldTime() % 400 == 0
					&& attackDamageTicks == 0)
				moveUp();
	...
	protected void setHangingLength(EntityDecayPitTargetPart chain, float extended) {
		...
		AxisAlignedBB axisalignedbb = new AxisAlignedBB(chain.posX - chain.width * 0.5D, posY + height,
				chain.posZ - chain.width * 0.5D, chain.posX + chain.width * 0.5D,
				posY + height + 2F + getProgress() * MOVE_UNIT, chain.posZ + chain.width * 0.5D);
	...
	protected void setNewShieldHitboxPos(float animationTicks, EntityDecayPitTargetPart shield) {
		...
		float wobble = 0F;
		if (shield == shield_1 || shield == shield_3 || shield == shield_5 || shield == shield_7 || shield == shield_9
				|| shield == shield_11 || shield == shield_13 || shield == shield_15)
		...
		if (squarePoint == -1F)
			wobble = 0F;
		shield.setPosition(posX + offSetX, target_north.posY + target_north.height / 2.0D - shield.height + wobble,
				posZ + offSetZ);
	...
	public void setDead() {
		...
		/*
		 * for(EntityDecayPitTargetPart shieldPart : parts) if(shieldPart.isShield)
		 * getEntityWorld().playEvent(null, 2001, shieldPart.getPosition(),
		 * Block.getIdFromBlock(BlockRegistry.SMOOTH_PITSTONE));
		 */
		super.setDead();
	}
	...
	public boolean attackEntityFromPart(EntityDecayPitTargetPart part, DamageSource source, float damage) {
		...
		if (source instanceof EntityDamageSource) {
			...
			if (attackingEntity == null) {
				...
				Vec3d pos = new Vec3d(attackingEntity.posX, attackingEntity.posY + attackingEntity.getEyeHeight(),
	...
	public EntityDecayPitTargetPart rayTraceShields(Vec3d pos, Vec3d dir) {
		...
		for (EntityDecayPitTargetPart shieldPart : parts) {
			if (shieldPart.isShield) {
				...
				if (EntityFortressBoss.rayTraceTriangle(relPos, relRay, v0, v1, v2)
						|| EntityFortressBoss.rayTraceTriangle(relPos, relRay, v2, v3, v0)) {
					return shieldPart;
				}
			}
		}
	...
	public TileEntityDecayPitHangingChain getHangingChains() {
		...
		for (int x = -1; x < 1; x++)
			for (int y = 0; y < 15; y++)
				for (int z = -1; z < 1; z++) {
					if (getWorld()
							.getTileEntity(getPosition().add(x, y, z)) instanceof TileEntityDecayPitHangingChain) {
	...
	public boolean canEntityBeSeen(Entity entity) {
		return getEntityWorld().rayTraceBlocks(new Vec3d(posX, posY + (double) getEyeHeight(), posZ),
