...
public class EntityRopeNode extends Entity {
	private static final DataParameter<Integer> DW_PREV_NODE = EntityDataManager.createKey(EntityRopeNode.class,
			DataSerializers.VARINT);
	private static final DataParameter<Integer> DW_NEXT_NODE = EntityDataManager.createKey(EntityRopeNode.class,
			DataSerializers.VARINT);
	...
	public void onEntityUpdate() {
		...
		if (attached && !prevAttached) {
			this.world.playSound((EntityPlayer) null, this.posX, this.posY, this.posZ, SoundEvents.BLOCK_METAL_STEP,
		...
		if (!this.world.isRemote) {
			...
			if (this.isEntityAlive()) {
				if (attached) {
					...
					if (this.lightBlock != null && (this.lightBlock.getX() != pos.getX()
							|| this.lightBlock.getY() != pos.getY() || this.lightBlock.getZ() != pos.getZ())) {
						if (this.world.isBlockLoaded(this.lightBlock) && this.world.getBlockState(this.lightBlock)
								.getBlock() == BlockRegistry.CAVING_ROPE_LIGHT) {
		...
		if (!this.world.isRemote) {
			if (nextNode instanceof EntityPlayer) {
				...
				if (this.pickUp && nextNode.getEntityBoundingBox().grow(0.4D, 0.4D, 0.4D)
						.intersects(this.getEntityBoundingBox())) {
					...
					if (player.inventory.addItemStackToInventory(new ItemStack(ItemRegistry.CAVING_ROPE, 1))) {
						this.world.playSound((EntityPlayer) null, player.posX, player.posY, player.posZ,
								SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.PLAYERS, 0.2F,
								((this.rand.nextFloat() - this.rand.nextFloat()) * 0.7F + 1.0F) * 2.0F);
					} else {
						EntityItem itemEntity = new EntityItem(this.world, this.posX, this.posY, this.posZ,
								new ItemStack(ItemRegistry.CAVING_ROPE, 1));
				...
				if (this.canExtend && nextNode.getDistance(this) > ROPE_LENGTH + 1) {
					...
					for (int i = 0; i < invSize; ++i) {
						...
						if (!stack.isEmpty() && stack.getItem() == ItemRegistry.CAVING_ROPE) {
							...
							if (connection != null) {
								Vec3d newPos = nextNode.getPositionVector().add(connection.scale(-0.5D)).add(0, 0.1D,
										0);
								RayTraceResult result = this.world.rayTraceBlocks(nextNode.getPositionVector(), newPos,
										false);
								if (result != null && result.typeOfHit == Type.BLOCK
										&& result.hitVec.squareDistanceTo(nextNode.getPositionVector()) < newPos
												.squareDistanceTo(nextNode.getPositionVector())) {
									newPos = result.hitVec.add(
											result.hitVec.subtract(this.getPositionVector()).normalize().scale(0.1D));
								}
								...
								if (rope.isAttached()) {
									this.world.playSound((EntityPlayer) null, this.posX, this.posY, this.posZ,
											SoundEvents.BLOCK_METAL_STEP, SoundCategory.PLAYERS, 1, 1.5F);
								}
								break;
							}
						}
				...
				if (nextNode.getDistance(this) > ROPE_LENGTH_MAX) {
					if (nextNode instanceof EntityPlayer) {
						((EntityPlayer) nextNode)
								.sendStatusMessage(new TextComponentTranslation("chat.rope.disconnected"), true);
					}
		...
		if (!attached && !this.onGround && !this.inWater) {
			...
			if (nextNode != null && this.getDistance(nextNode) >= ROPE_LENGTH) {
				...
				if (connection != null) {
					...
					if (prevNode != null
							&& prevNode.getDistance(this.posX + mx, this.posY + my, this.posZ + mz) < ROPE_LENGTH + 1) {
						this.motionX += mx;
						this.motionZ += mz;
						this.motionY += my;
						isFloating = true;
			...
			if (nextNode != null) {
				...
				if (tetherPoint.distanceTo(nextPoint) >= ROPE_LENGTH) {
					...
					constrainedPoint = new Vec3d(constrainedPoint.x * ROPE_LENGTH, constrainedPoint.y * ROPE_LENGTH,
	...
	public void setDead() {
		...
		if (this.lightBlock != null && this.world.isBlockLoaded(this.lightBlock)
				&& this.world.getBlockState(this.lightBlock).getBlock() == BlockRegistry.CAVING_ROPE_LIGHT) {
			this.world.setBlockToAir(this.lightBlock);
		}
