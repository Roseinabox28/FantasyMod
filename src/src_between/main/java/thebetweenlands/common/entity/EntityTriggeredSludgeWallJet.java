...
public class EntityTriggeredSludgeWallJet extends EntityProximitySpawner {
	private static final DataParameter<Integer> ANIMATION_TICKS_SYNC = EntityDataManager
			.createKey(EntityTriggeredSludgeWallJet.class, DataSerializers.VARINT);
	...
	protected void performPreSpawnaction(Entity targetEntity, Entity entitySpawned) {
		if (targetEntity instanceof EntityPlayer) {
			float angle = (float) Math
					.toDegrees(Math.atan2(targetEntity.posX - this.posX, targetEntity.posZ - this.posZ));
			float angleDiff = Math.abs(
					MathHelper.wrapDegrees(MathHelper.wrapDegrees(angle) - MathHelper.wrapDegrees(-this.rotationYaw)));

			if (angleDiff < 55)
				((EntitySludgeWallJet) entitySpawned).setDead();
			else {
				((EntitySludgeWallJet) entitySpawned).setPosition(posX, posY + height * 0.5D, posZ);
				((EntitySludgeWallJet) entitySpawned).shoot(targetEntity.posX - posX,
						targetEntity.posY + targetEntity.height - posY, targetEntity.posZ - posZ, 0.5F, 0F);
			}
		}
	}
	...
	public boolean attackEntityFrom(DamageSource source, float damage) {
		if (source instanceof EntityDamageSource) {
			...
			if (immediateSource != null) {
				float angle = (float) Math
						.toDegrees(Math.atan2(immediateSource.posX - this.posX, immediateSource.posZ - this.posZ));
				float angleDiff = Math.abs(MathHelper
						.wrapDegrees(MathHelper.wrapDegrees(angle) - MathHelper.wrapDegrees(-this.rotationYaw)));
	...
	protected AxisAlignedBB proximityBox() {
		return new AxisAlignedBB(getPosition()).grow(getProximityHorizontal(), getProximityVertical(),
