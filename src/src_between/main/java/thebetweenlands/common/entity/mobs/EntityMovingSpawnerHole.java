...
public class EntityMovingSpawnerHole extends EntityMovingWallFace implements IMob {
	...
	protected boolean isValidBlockForMovement(BlockPos pos, IBlockState state) {
		return state.isOpaqueCube() && state.isNormalCube() && state.isFullCube()
				&& state.getBlockHardness(this.world, pos) > 0 && state.getMaterial() == Material.ROCK;
	}
	...
	protected Entity createEntity(Vec3d holeBottom, double frontOffset, EnumFacing facing) {
		...
		entity.move(MoverType.SELF, facing.getXOffset() * 0.35D, facing == EnumFacing.UP ? 1 : 0,
				facing.getZOffset() * 0.35D);
		return entity;
	}
	...
	public void startSpawningMob() {
		BlockPos pos = this.getPosition();
		Entity entity = this.createEntity(new Vec3d(pos.getX() + 0.5D, pos.getY() + 0.1D, pos.getZ() + 0.5D), 0.55D,
				this.getFacing());
	...
	public boolean canSpawnMobs() {
		...
		if (target != null && !this.isSpawningMob() && this.getDistance(target) < this.maxTargetRange
				&& this.canEntityBeSeen(target)) {
			Predicate<Entity> pred = this.getNearbySpawnedEntitiesPredicate();
			int others = this.world
					.getEntitiesWithinAABB(Entity.class, this.getEntityBoundingBox().grow(this.countCheckRange), pred)
					.size();
	...
	protected static class AISpawnMob extends EntityAIBase {
		...
		public boolean shouldExecute() {
			if (--this.spawnCooldown <= 0) {
				this.spawnCooldown = this.minSpawnCooldown
						+ this.entity.rand.nextInt(this.maxSpawnCooldown - this.minSpawnCooldown);
				return this.entity.isEntityAlive() && !this.entity.isMoving() && this.entity.isAnchored()
						&& this.entity.canSpawnMobs();
			}
			return false;
		}
