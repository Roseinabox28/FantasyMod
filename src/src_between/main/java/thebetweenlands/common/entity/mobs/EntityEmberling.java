...
public class EntityEmberling extends EntityMob implements IEntityMultiPart, IEntityBL {
	...
	private static final DataParameter<Boolean> IS_CASTING_SPELL = EntityDataManager
			.<Boolean>createKey(EntityEmberling.class, DataSerializers.BOOLEAN);
	...
	public EntityEmberling(World world) {
		...
		stepHeight = 1F;
		isImmuneToFire = true;
		tailPart = new MultiPartEntityPart[] { new MultiPartEntityPart(this, "tail", 0.5F, 0.5F) }; // may use more
																									// parts?
	}
	...
	protected Entity checkCollision() {
		List<EntityLivingBase> list = getEntityWorld().getEntitiesWithinAABB(EntityLivingBase.class,
				tailPart[0].getEntityBoundingBox());
		for (int entityCount = 0; entityCount < list.size(); entityCount++) {
			Entity entity = list.get(entityCount);
			if (entity != null && entity == getAttackTarget() && !(entity instanceof IEntityMultiPart)
					&& !(entity instanceof MultiPartEntityPart))
				if (entity instanceof EntityLivingBase) {
					attackEntityAsMob(entity);
					entity.addVelocity(-MathHelper.sin(tailPart[0].rotationYaw * 3.141593F / 180.0F) * 0.5D, 0.3D,
							MathHelper.cos(tailPart[0].rotationYaw * 3.141593F / 180.0F) * 0.5D);
	...
	static class EntityAIFireballColumn extends EntityAIBase {
		...
		public void startExecuting() {
			missileCount = 0;
			shootCount = 0;
			emberling.getEntityWorld().playSound(null, emberling.getPosition(), SoundRegistry.EMBERLING_FLAMES,
		...
		public void updateTask() {
			...
			if (missileCount % 5 == 0) {
				...
				flame_jet.setPosition(emberling.posX + (double) MathHelper.cos(f) * d2, emberling.posY,
						emberling.posZ + (double) MathHelper.sin(f) * d2);
				emberling.getEntityWorld().spawnEntity(flame_jet);
				emberling.getEntityWorld().playSound(null, emberling.getPosition(), SoundEvents.ITEM_FIRECHARGE_USE,
						SoundCategory.HOSTILE, 0.5F,
						1F + (emberling.getEntityWorld().rand.nextFloat() - emberling.getEntityWorld().rand.nextFloat())
								* 0.8F);
			}
	...
	static class EntityAIHoverSpinAttack extends EntityAIBase {
		...
		public void startExecuting() {
			if (emberling.getIsCastingSpell())
				emberling.setIsCastingSpell(false);
			emberling.getEntityWorld().playSound(null, emberling.getPosition(), SoundRegistry.EMBERLING_JUMP,
					SoundCategory.HOSTILE, 1F, 1F);
			double d0 = target.posX - emberling.posX;
			double d1 = target.posZ - emberling.posZ;
			float f = MathHelper.sqrt(d0 * d0 + d1 * d1);
			if ((double) f >= 1.0E-4D) {
				emberling.motionX += d0 / (double) f * 0.5D * 0.800000011920929D
						+ emberling.motionX * 0.20000000298023224D;
				emberling.motionZ += d1 / (double) f * 0.5D * 0.800000011920929D
						+ emberling.motionZ * 0.20000000298023224D;
			}
