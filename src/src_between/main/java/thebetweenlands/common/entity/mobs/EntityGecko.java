...
public class EntityGecko extends EntityCreature implements IEntityBL, WeedWoodBushUncollidableEntity {
	private static final DataParameter<Boolean> HIDING = EntityDataManager.createKey(EntityGecko.class,
			...
			if (isHiding()) {
				if (hasValidHiding()) {
					...
					if (rand.nextFloat() < 0.01F) {
						playSound(SoundRegistry.GECKO_HIDE, rand.nextFloat() * 0.05F + 0.02F,
								rand.nextFloat() * 0.2F + 0.8F);
						if (rand.nextFloat() < 0.3F)
							sendRustleEffect((rand.nextFloat() + 0.2F) * 0.06F);
					}
					if (timeHiding > MIN_HIDE_TIME) {
						List<EntityPlayer> players = world.getEntitiesWithinAABB(EntityPlayer.class,
								this.getEntityBoundingBox().grow(PLAYER_MIN_DISTANCE, PLAYER_MIN_DISTANCE,
										PLAYER_MIN_DISTANCE));
	...
	public void readEntityFromNBT(NBTTagCompound compound) {
		...
		if (compound.hasKey("isHiding")) {
			...
			if (isHiding()) {
				setHidingBush(new BlockPos(compound.getInteger("hidingBushX"), compound.getInteger("hidingBushY"),
						compound.getInteger("hidingBushZ")));
			}
		}
	}
