...
public class EntityGasCloud extends EntityFlyingMob implements IEntityBL {
	public static final IAttribute GAS_CLOUD_COLOR_R = (new RangedAttribute(null, "bl.gasCloudColorRed", 104, 0, 255))
			.setDescription("Gas cloud color red component").setShouldWatch(true);
	public static final IAttribute GAS_CLOUD_COLOR_G = (new RangedAttribute(null, "bl.gasCloudColorGreen", 196, 0, 255))
			.setDescription("Gas cloud color green component").setShouldWatch(true);
	public static final IAttribute GAS_CLOUD_COLOR_B = (new RangedAttribute(null, "bl.gasCloudColorBlue", 179, 0, 255))
			.setDescription("Gas cloud color blue component").setShouldWatch(true);
	public static final IAttribute GAS_CLOUD_COLOR_A = (new RangedAttribute(null, "bl.gasCloudColorAlpha", 170, 0, 255))
			.setDescription("Gas cloud color alpha component").setShouldWatch(true);

	protected double aboveLayer = 6.0D;
	protected int targetBlockedTicks = 0;

	public static final DamageSource damageSourceSuffocation = (new DamageSource("suffocation"))
	...
	private void initParticleBatch() {
		this.particleBatch = BatchedParticleRenderer.INSTANCE.createBatchType(new ParticleBatchTypeBuilder().pass()
				.depthMaskPass(true).lit(true).texture((ResourceLocation) null).end().build());
	}
	...
	public void onUpdate() {
		...
		if (this.isInWater()) {
			...
			if (this.getAttackTarget() != null) {
				this.moveHelper.setMoveTo(this.getAttackTarget().posX,
						this.getAttackTarget().posY + this.getAttackTarget().getEyeHeight(),
						this.getAttackTarget().posZ, 1.0D);
			}
		}

		if (!this.world.isRemote && this.isEntityAlive()) {
			List<EntityLivingBase> targets = this.world.getEntitiesWithinAABB(EntityLivingBase.class,
					this.getEntityBoundingBox().grow(0.5D, 0.5D, 0.5D));
			for (EntityLivingBase target : targets) {
				if (!(target instanceof EntityGasCloud) && !(target instanceof IEntityBL)) {
					...
					if (target.ticksExisted % 10 == 0)
						target.attackEntityFrom(damageSourceSuffocation, (float) this
	...
	private void spawnCloudParticle(boolean strongMotion) {
		if (strongMotion) {
			...
			ParticleGasCloud particle = (ParticleGasCloud) BLParticles.GAS_CLOUD.create(this.world, x, y, z,
					ParticleFactory.ParticleArgs.get().withData(this)
							.withMotion((this.rand.nextFloat() - 0.5F) * this.rand.nextFloat() * 0.25F,
									(this.rand.nextFloat() - 0.5F) * this.rand.nextFloat() * 0.25F,
									(this.rand.nextFloat() - 0.5F) * this.rand.nextFloat() * 0.25F)
			...
			ParticleGasCloud particle = (ParticleGasCloud) BLParticles.GAS_CLOUD.create(this.world, x, y, z,
					ParticleFactory.ParticleArgs.get().withData(this).withMotion(mx, my, mz)
							.withColor(color[0] / 255.0F, color[1] / 255.0F, color[2] / 255.0F, color[3] / 255.0F));
