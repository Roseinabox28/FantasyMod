...
public class EntityTarminion extends EntityTameable implements IEntityBL {
	public static final IAttribute MAX_TICKS_ATTRIB = (new RangedAttribute(null, "bl.maxAliveTicks", 7200.0D, 0,
			Integer.MAX_VALUE)).setDescription("Maximum ticks until the Tar Minion despawns");
		...
		if (!this.isDead && this.dropContentsWhenDead) {
			if (this.getAttackTarget() != null) {
				if (this.world.isRemote) {
					for (int i = 0; i < 200; i++) {
						...
						vec = vec.normalize();
						BLParticles.SPLASH_TAR.spawn(this.world, this.posX + rx + 0.1F, this.posY + ry,
								this.posZ + rz + 0.1F,
								ParticleArgs.get().withMotion(vec.x * 0.4F, vec.y * 0.4F, vec.z * 0.4F));
					}
				} else {
					for (int i = 0; i < 8; i++) {
						this.playSound(SoundRegistry.TAR_BEAST_STEP, 1F, (this.rand.nextFloat() * 0.4F + 0.8F) * 0.8F);
					}
					List<EntityCreature> affectedEntities = (List<EntityCreature>) this.world.getEntitiesWithinAABB(
							EntityCreature.class, this.getEntityBoundingBox().grow(5.25F, 5.25F, 5.25F));
					for (EntityCreature e : affectedEntities) {
						if (e == this || e.getDistance(this) > 5.25F || !e.canEntityBeSeen(this)
								|| e instanceof EntityTarminion)
							continue;
						double dst = e.getDistance(this);
						e.attackEntityFrom(DamageSource.causeMobDamage(this), (float) this
								.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getAttributeValue() * 4);
						e.addPotionEffect(
								new PotionEffect(MobEffects.SLOWNESS, (int) (20 + (1.0F - dst / 5.25F) * 150), 1));
					}
				}
			}
			...
			if (this.world.isRemote) {
				for (int i = 0; i < 100; i++) {
					...
					BLParticles.SPLASH_TAR.spawn(this.world, this.posX + rx + 0.1F, this.posY + ry,
							this.posZ + rz + 0.1F,
							ParticleArgs.get().withMotion(vec.x * 0.2F, vec.y * 0.2F, vec.z * 0.2F));
				}
			}
		}
	...
	public boolean attackEntityFrom(DamageSource source, float amount) {
		if (source == DamageSource.DROWN
				&& this.world.getBlockState(new BlockPos(this.posX, this.posY + this.height, this.posZ))
						.getBlock() == BlockRegistry.TAR) {
			return false;
		}
	...
	protected boolean attack(Entity entity) {
		if (!this.world.isRemote) {
			...
			((EntityLivingBase) entity)
					.addPotionEffect(new PotionEffect(MobEffects.SLOWNESS, world.getDifficulty().getId() * 50, 0));
			return true;
		}
		return true;
	}
