...
public class EntitySwampHag extends EntityMob implements IEntityBL {
	private static final DataParameter<Byte> TALK_SOUND = EntityDataManager.createKey(EntitySwampHag.class,
			DataSerializers.BYTE);
	private static final DataParameter<Boolean> SHOULD_JAW_MOVE = EntityDataManager.createKey(EntitySwampHag.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> LIVING_SOUND_TIMER = EntityDataManager.createKey(EntitySwampHag.class,
	...
	public void onLivingUpdate() {
		...
		if (this.getAttackTarget() != null && !this.getRecursivePassengersByType(EntityWight.class).isEmpty()) {
			if (this.pathingCooldown <= 0) {
				// No idea why the swamp hag doesn't want to move when possessed, this seems to
				// work as hackish solution
