...
public class EntityRootSprite extends EntityCreature implements IEntityBL {
	...
	protected void initEntityAI() {
		...
		this.tasks.addTask(3, new EntityAIFollowTarget(this,
				new EntityAIFollowTarget.FollowClosest(this, EntitySporeling.class, 10), 0.65D, 0.5F, 10.0F, false));
		this.tasks.addTask(4, new EntityAIJumpRandomly(this, 10, () -> !EntityRootSprite.this.world
				.getEntitiesWithinAABB(EntitySporeling.class, this.getEntityBoundingBox().grow(1)).isEmpty()) {
			@Override
	...
	private void spawnLeafParticles() {
		for (int i = 0; i < 1 + this.rand.nextInt(3); i++) {
			BLParticles.WEEDWOOD_LEAF
					.spawn(this.world, this.posX + this.motionX, this.posY + 0.1F + this.rand.nextFloat() * 0.3F,
							this.posZ + this.motionZ,
							ParticleArgs.get()
									.withMotion(this.motionX * 0.5F + this.rand.nextFloat() * 0.1F - 0.05F, 0.05F,
											this.motionZ * 0.5F + this.rand.nextFloat() * 0.1F - 0.05F)
									.withScale(0.5F));
		}
	}
}
