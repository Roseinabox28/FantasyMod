...
public class EntitySludge extends EntityLiving implements IMob, IEntityBL {
	public static final DataParameter<Boolean> IS_ACTIVE = EntityDataManager.createKey(EntitySludge.class,
			DataSerializers.BOOLEAN);

	public static final IAttribute SLUDGE_TRAIL = (new BooleanAttribute(null, "bl.sludgeTrail", false))
			.setDescription("Whether this Sludge should leave a Sludge trail");
	...
	public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, IEntityLivingData livingdata) {
		this.setActive(this.world.rand.nextInt(5) == 0
				|| !this.canHideIn(this.world.getBlockState(this.getPosition().down())));
	...
	public void onUpdate() {
		...
		if (!this.world.isRemote) {
			...
			if (this.isActive()) {
				...
				if (this.getAttackTarget() == null && this.onGround && this.world.rand.nextInt(350) == 0
						&& !this.isInWater() && this.canHideIn(this.world.getBlockState(this.getPosition().down()))) {
	...
	protected void dealDamage(EntityLivingBase entityIn) {
		if (this.isActive() && this.canEntityBeSeen(entityIn) && this.getDistanceSq(entityIn) < 2.5D
	...
	protected boolean getIsPlayerNearby(double distanceX, double distanceY, double distanceZ, double radius) {
		List<Entity> entities = this.world.getEntitiesWithinAABBExcludingEntity(this,
				this.getEntityBoundingBox().grow(distanceX, distanceY, distanceZ));
		for (Entity entityNeighbor : entities) {
			if (entityNeighbor instanceof EntityPlayer && this.getDistance(entityNeighbor) <= radius
					&& (!((EntityPlayer) entityNeighbor).capabilities.isCreativeMode
							&& !((EntityPlayer) entityNeighbor).isSpectator()
							&& this.getEntitySenses().canSee(entityNeighbor)))
				return true;
		}
		return false;
	}
	...
	public AxisAlignedBB getRenderBoundingBox() {
		return Minecraft.getMinecraft().player.capabilities.isCreativeMode || this.isActive()
	...
	static class AISludgeAttack extends EntityAIBase {
		...
		public void updateTask() {
			...
			if (target != null) {
				...
				((EntitySludge.SludgeMoveHelper) this.sludge.getMoveHelper()).setDirection(this.sludge.rotationYaw,
						true);
			}
		}
	}
	...
	static class AISludgeFaceRandom extends EntityAIBase {
		...
		@Override
		public boolean shouldExecute() {
			return this.sludge.getAttackTarget() == null && (this.sludge.onGround || this.sludge.isInWater()
					|| this.sludge.isInLava() || this.sludge.isPotionActive(MobEffects.LEVITATION));
		}
				...
				if (this.entity.onGround) {
					...
					if (this.jumpDelay-- <= 0) {
						...
						if (this.sludge.makesSoundOnJump()) {
							this.sludge.playSound(this.sludge.getJumpSound(), this.sludge.getSoundVolume(),
									((this.sludge.getRNG().nextFloat() - this.sludge.getRNG().nextFloat()) * 0.2F
											+ 1.0F) * 0.8F);
						}
					} else {
