...
public class EntitySludgeWorm extends EntityMob implements IEntityMultiPart, IMob, IEntityBL {
	...
	public EntitySludgeWorm(World world) {
		...
		isImmuneToFire = true;
		parts = new MultiPartEntityPart[] { new MultiPartEntityPart(this, "part1", 0.4375F, 0.3125F),
	...
	public void spawnParticles(World world, double x, double y, double z, Random rand) {
		for (int count = 0; count < 10; ++count) {
			...
			BLParticles.TAR_BEAST_DRIP.spawn(world, x + offSetX, y, z + offSetZ).setRBGColorF(0.4118F, 0.2745F,
					0.1568F);
		}
	}
	...
	public void movePiecePos(MultiPartEntityPart targetPart, MultiPartEntityPart destinationPart, float speed,
			float yawSpeed) {
		...
		Vec3d diff = new Vec3d(destinationPart.posX, 0, destinationPart.posZ)
				.subtract(new Vec3d(targetPart.posX, 0, targetPart.posZ));
