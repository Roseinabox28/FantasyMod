...
public class EntityLeech extends EntityMob implements IEntityBL {
	...
	private static final DataParameter<Byte> BLOOD_CONSUMED = EntityDataManager.createKey(EntityLeech.class,
	...
	protected void initEntityAI() {
		this.aiAttackTarget = new EntityAINearestAttackableTarget<EntityLivingBase>(this, EntityLivingBase.class, true,
				true);
	...
	public void onUpdate() {
		if (!world.isRemote) {
			...
			if (getRidingEntity() != null) {
				...
				if (getRidingEntity().isBurning() && getRidingEntity() instanceof EntityLivingBase) {
					PotionEffect stomachContents = new PotionEffect(MobEffects.POISON,
							120 + getBloodConsumed() * 200 / MAX_BLOOD_LEVEL, 0);
		...
		if (getRidingEntity() != null) {
			...
			if (rand.nextInt(10) == 0) {
				for (int i = 0; i < 8; i++) {
					world.spawnParticle(EnumParticleTypes.REDSTONE, posX + (rand.nextFloat() - rand.nextFloat()),
							posY + rand.nextFloat(), posZ + (rand.nextFloat() - rand.nextFloat()), 0, 0, 0);
				}
			}
		...
		if (getRidingEntity() != null && getRidingEntity() instanceof EntityLivingBase
				&& getBloodConsumed() < MAX_BLOOD_LEVEL) {
			drainage++;
	...
	public boolean canAttackClass(Class entity) {
		return entity != EntityLeech.class
				&& (entity == EntityPlayer.class || entity == EntityPlayerMP.class || entity == EntitySwampHag.class);
	}
