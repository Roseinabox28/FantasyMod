...
public class EntityCryptCrawler extends EntityMob implements IEntityBL {
	...
	private static final DataParameter<Boolean> IS_BIPED = EntityDataManager.createKey(EntityCryptCrawler.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> IS_STANDING = EntityDataManager.createKey(EntityCryptCrawler.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> IS_CHIEF = EntityDataManager.createKey(EntityCryptCrawler.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> IS_BLOCKING = EntityDataManager
			.<Boolean>createKey(EntityCryptCrawler.class, DataSerializers.BOOLEAN);
	...
	public EntityCryptCrawler(World world) {
		super(world);
		setSize(0.5F, 1.0F); // Width must be < 1 otherwise path finder will not path through one block wide
								// gaps!!
		stepHeight = 1F;
	}
	...
	protected void initEntityAI() {
		...
		targetTasks.addTask(0, new EntityAINearestAttackableTarget<>(this, EntityPlayer.class, 3, true, true, null)
	...
	public void onLivingUpdate() {
		if (this.isBlocking() && !this.getHeldItemOffhand().isEmpty()
				&& this.getHeldItemOffhand().getItem().isShield(this.getHeldItemOffhand(), this)) {
		...
		if (!getEntityWorld().isRemote && !isBiped()) {
			if (getAttackTarget() != null) {
				double distance = getDistance(getAttackTarget().posX, getAttackTarget().getEntityBoundingBox().minY,
	...
	public boolean attackEntityAsMob(Entity entity) {
		if (!this.isBlocking() && this.canEntityBeSeen(entity)) {
			boolean hasHitTarget = entity.attackEntityFrom(DamageSource.causeMobDamage(this),
			...
			if (hasHitTarget) {
				...
				// entity.addVelocity(-MathHelper.sin(rotationYaw * 3.141593F / 180.0F) * 0.5F,
				// 0.2D, MathHelper.cos(rotationYaw * 3.141593F / 180.0F) * 0.5F);

				if (!getEntityWorld().isRemote)
					getEntityWorld().playSound((EntityPlayer) null, posX, posY, posZ,
							SoundRegistry.CRYPT_CRAWLER_LIVING, SoundCategory.HOSTILE, 1F, 0.5F);
			}
			return hasHitTarget;
		}
		return false;
	...
	public boolean attackEntityFrom(DamageSource source, float amount) {
		...
		if (this.isBlocking() && !wasAttackBlocked) {
			...
			// Play shield block sound to all listeners. For some reason shield block sound
			// from item doesn't seem to work.
	...
	public void handleStatusUpdate(byte id) {
		...
		if (id == EVENT_SHIELD_BLOCKED) {
			this.world.playSound(this.posX, this.posY, this.posZ, SoundEvents.ITEM_SHIELD_BLOCK, SoundCategory.NEUTRAL,
					1.0F, 0.8F + this.world.rand.nextFloat() * 0.4F, false);
		}
	}
	...
	static class AIShieldBlock extends EntityAIBase {
		...
		public boolean shouldExecute() {
			...
			if (target != null && !crawler.isSwingInProgress
					&& crawler.hurtResistantTime <= Math.max(0, crawler.maxHurtResistantTime - 5)) {
		...
		@Override
		public boolean shouldContinueExecuting() {
			return this.isHoldingShield() && !crawler.recentlyBlockedAttack && blockingCount != -1
					&& crawler.hurtResistantTime <= Math.max(0, crawler.maxHurtResistantTime - 5)
					&& !crawler.isSwingInProgress && !(blockingCount > blockingCountMax
							|| (meleeBlockingCounterMax >= 0 && meleeBlockingCounter > meleeBlockingCounterMax));
		}
