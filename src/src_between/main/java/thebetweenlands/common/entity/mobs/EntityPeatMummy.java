...
public class EntityPeatMummy extends EntityMob implements IEntityBL, IEntityScreenShake {
	public static final IAttribute SPAWN_LENGTH_ATTRIB = (new RangedAttribute(null, "bl.spawnLength", 100.0D, 0.0D,
			Integer.MAX_VALUE)).setDescription("Spawning Length");
	public static final IAttribute SPAWN_OFFSET_ATTRIB = (new RangedAttribute(null, "bl.spawnOffset", 2.0D,
			-Integer.MAX_VALUE, Integer.MAX_VALUE)).setDescription("Spawning Y Offset");
	public static final IAttribute SPAWN_RANGE_ATTRIB = (new RangedAttribute(null, "bl.spawnRange", 8.0D, 0,
			Double.MAX_VALUE)).setDescription("Spawning Range");

	public static final IAttribute CHARGING_COOLDOWN_ATTRIB = (new RangedAttribute(null, "bl.chargingCooldown", 160.0D,
			0, Integer.MAX_VALUE)).setDescription("Charging Cooldown");
	public static final IAttribute CHARGING_PREPARATION_SPEED_ATTRIB = (new RangedAttribute(null,
			"bl.chargingPreparationSpeed", 60.0D, 0, Integer.MAX_VALUE)).setDescription("Charging Preparation Speed");
	public static final IAttribute CHARGING_TIME_ATTRIB = (new RangedAttribute(null, "bl.chargingTime", 320.0D, 0,
			Integer.MAX_VALUE)).setDescription("Charging Time");
	public static final IAttribute CHARGING_SPEED_ATTRIB = (new RangedAttribute(null, "bl.chargingSpeed", 0.55D, 0,
			Double.MAX_VALUE)).setDescription("Charging Movement Speed");
	public static final IAttribute CHARGING_DAMAGE_MULTIPLIER_ATTRIB = (new RangedAttribute(null,
			"bl.chargingDamageMultiplier", 1.65D, 0, Double.MAX_VALUE)).setDescription("Charging Damage Multiplier");

	public static final IAttribute CARRY_SHIMMERSTONE = (new BooleanAttribute(null, "bl.carryShimmerstone", false))
			.setDescription("Whether this Peat Mummy carries a Shimmerstone");
	public static final IAttribute IS_BOSS = (new BooleanAttribute(null, "bl.isDreadfulPeatMummyBoss", false))
			.setDescription("Whether this Peat Mummy was spawned by a Dreadful Peat Mummy");
	...
	private static final DataParameter<Integer> SPAWNING_TICKS = EntityDataManager
			.<Integer>createKey(EntityPeatMummy.class, DataSerializers.VARINT);

	private int chargingPreparation;
	private static final DataParameter<Byte> CHARGING_STATE = EntityDataManager.<Byte>createKey(EntityPeatMummy.class,
	...
	public void initEntityAI() {
		...
		this.inactiveTargetTasks
				.add(new EntityAINearestAttackableTarget<EntityPlayer>(this, EntityPlayer.class, false) {
					@Override
	...
	public void onUpdate() {
		...
		if (!this.world.isRemote) {
			...
			if (this.isInValidSpawn() && !this.isSpawningFinished()) {
				...
				if ((getSpawningTicks() - breakPoint / 2 - 1) % breakPoint == 0) {
					...
					this.playSound(blockState.getBlock().getSoundType().getBreakSound(),
			...
			if (this.getSpawningProgress() != 0.0F && this.getSpawningProgress() != 1.0F) {
				...
				if ((getSpawningTicks() - breakPoint / 2 - 1) % breakPoint == 0) {
					...
					for (int i = 0, amount = this.rand.nextInt(20) + 15; i < amount; i++) {
						...
						this.world.spawnParticle(EnumParticleTypes.BLOCK_DUST, px + ox, py, pz + oz, motionX, motionY,
								motionZ, Block.getStateId(blockState));
					}
				}
			}
		}
		...
		if (this.isSpawningFinished()) {
			...
			if (!this.world.isRemote) {
				...
				if (this.isCharging()) {
					this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).setBaseValue(BASE_DAMAGE
							* this.getEntityAttribute(CHARGING_DAMAGE_MULTIPLIER_ATTRIB).getAttributeValue());
					this.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED)
							.setBaseValue(this.getEntityAttribute(CHARGING_SPEED_ATTRIB).getAttributeValue());
				} else {
					this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).setBaseValue(BASE_DAMAGE);
					this.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue(BASE_SPEED);
				}
		...
		for (int y = -MathHelper.ceil(this.getMaxSpawnOffset()); y < 0; y++) {
			for (int x = -1; x <= 1; x++) {
				for (int z = -1; z <= 1; z++) {
					...
					if (!(y == -1 ? (SPAWN_BLOCKS.contains(cb))
							: (cb.isOpaqueCube(blockState) || SPAWN_BLOCKS.contains(cb)))) {
						return false;
					}
				}
			}
	...
	/**
	 * Sets the charging state: - 0: Not charging - 1: Preparing - 2: Charging
	 * 
	 * @param state
	 */
	...
	public float getPreparationProgress() {
		return 1.0F / (int) this.getEntityAttribute(CHARGING_PREPARATION_SPEED_ATTRIB).getAttributeValue()
				* this.chargingPreparation;
	}
	...
	public AxisAlignedBB getRenderBoundingBox() {
		return Minecraft.getMinecraft().player.capabilities.isCreativeMode || this.getSpawningTicks() > 0
