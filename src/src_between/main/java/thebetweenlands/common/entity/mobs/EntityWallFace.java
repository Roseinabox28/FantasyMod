...
public abstract class EntityWallFace extends EntityCreature implements IEntityBL {
	private static final DataParameter<EnumFacing> FACING = EntityDataManager.createKey(EntityWallFace.class,
			DataSerializers.FACING);
	private static final DataParameter<EnumFacing> FACING_UP = EntityDataManager.createKey(EntityWallFace.class,
			DataSerializers.FACING);
	private static final DataParameter<BlockPos> ANCHOR = EntityDataManager.createKey(EntityWallFace.class,
	...
	private static final DataParameter<Boolean> MOVING = EntityDataManager.createKey(EntityWallFace.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Float> MOVE_SPEED = EntityDataManager.createKey(EntityWallFace.class,
			DataSerializers.FLOAT);
	private static final DataParameter<EnumFacing> MOVE_FACING = EntityDataManager.createKey(EntityWallFace.class,
			DataSerializers.FACING);
	private static final DataParameter<EnumFacing> MOVE_FACING_UP = EntityDataManager.createKey(EntityWallFace.class,
			DataSerializers.FACING);
	private static final DataParameter<BlockPos> MOVE_ANCHOR = EntityDataManager.createKey(EntityWallFace.class,
			DataSerializers.BLOCK_POS);
	private static final DataParameter<Byte> MOVE_REASON = EntityDataManager.createKey(EntityWallFace.class,
			DataSerializers.BYTE);
	private static final DataParameter<Boolean> ANCHORED = EntityDataManager.createKey(EntityWallFace.class,
			DataSerializers.BOOLEAN);
	...
	public int getBrightnessForRender() {
		...
		BlockPos.MutableBlockPos pos = new BlockPos.MutableBlockPos(MathHelper.floor(this.posX) + facing.getXOffset(),
				0, MathHelper.floor(this.posZ) + facing.getZOffset());
	...
	public EntityItem entityDropItem(ItemStack stack, float offsetY) {
		if (stack.isEmpty()) {
			...
			EnumFacing facing = this.getFacing();
			Vec3d dropPos = this.getFrontCenter().add(facing.getXOffset() * entityItem.width,
					facing.getYOffset() * entityItem.height, facing.getZOffset() * entityItem.width);
	...
	public boolean canEntityBeSeen(Entity entity) {
		...
		return this.world.rayTraceBlocks(frontCenter,
				new Vec3d(entity.posX, entity.posY + (double) entity.getEyeHeight(), entity.posZ), false, true,
				false) == null;
	}
	...
	protected void updatePositioning(boolean isAnchored) {
		...
		if (isAnchored) {
			if (facing == EnumFacing.UP || facing == EnumFacing.DOWN) {
				...
				this.prevRenderYawOffset = this.renderYawOffset = facingUp.getHorizontalAngle()
						+ (facing == EnumFacing.DOWN ? 0.0F : 180.0F);
			} else {
		...
		if (!this.world.isRemote) {
			if (!this.isMoving()) {
				...
				if (!this.isTravelBlocked() && !this.isMoving()
						&& (this.targetFacing != null || this.targetAnchor != null)) {
	...
	protected void updateMovement() {
		...
		if (this.isMoving()) {
			...
			if (movementProgress < 0.5F) {
				...
				double pz = this.posZ;
				this.setPosition(position.x, position.y - this.height / 2.0D, position.z);
				if ((this.posX - px) * (this.posX - px) + (this.posY - py) * (this.posY - py)
						+ (this.posZ - pz) * (this.posZ - pz) >= 1.0D) {
	...
	public float getMovementProgress(float partialTicks) {
		return MathHelper.clamp(this.lastMoveProgress + (this.moveProgress - this.lastMoveProgress) * partialTicks, 0,
				1);
	}

	public Vec3d getOffset(float movementProgress) {
		float offsetLength = this.getHalfMovementProgressFromRegular(movementProgress);
		return new Vec3d(this.getFacing().getDirectionVec())
				.scale(this.getPeek() + (this.getFacing().getAxis().isHorizontal() ? (this.getBlockWidth() - this.width)
						: (this.getBlockHeight() - this.height)) / 2.0D)
				.scale(offsetLength);
	}
	...
	public Vec3d getCenter() {
		return new Vec3d(this.getAnchor()).add(this.getBlockWidth() / 2.0D, this.getBlockHeight() / 2.0D,
				this.getBlockWidth() / 2.0D);
	}

	public Vec3d getFrontCenter() {
		EnumFacing facing = this.getFacing();
		Vec3d center = this.getCenter();
		return center.add(this.getOffset(this.getMovementProgress(1))).add(facing.getXOffset() * this.width / 2.0F,
				facing.getYOffset() * this.height / 2.0F, facing.getZOffset() * this.width / 2.0F);
	}
	...
	public static class AnchorChecks {
		...
		/**
		 * Checks whether the entities around the anchor and the entity's face at the
		 * anchor are valid
		 */
	...
	public int checkAnchorAt(Vec3d pos, Vec3d lookDir, int checks) {
		...
		BlockPos anchor = new BlockPos(pos.x - (this.getBlockWidth() / 2), pos.y - (this.getBlockHeight() / 2),
				pos.z - (this.getBlockWidth() / 2));
	...
	public int checkAnchorAt(BlockPos anchor, EnumFacing facing, EnumFacing facingUp, int checks) {
		...
		if ((checks & AnchorChecks.ENTITIES) != 0) {
			if (!this.world.getEntitiesWithinAABB(EntityWallFace.class,
					this.getEntityBoundingBox().offset(anchor.subtract(this.getAnchor())).expand(
							facing.getXOffset() * this.getPeek(), facing.getYOffset() * this.getPeek(),
							facing.getZOffset() * this.getPeek()),
					e -> e != this).isEmpty()) {
				...
				outer: for (int xo = 0; xo < this.getBlockWidth(); xo++) {
					for (int zo = 0; zo < this.getBlockWidth(); zo++) {
						for (int yo = 0; yo < MathHelper.ceil(this.getPeek()); yo++) {
							pos.setPos(anchor.getX() + xo, anchor.getY() + y + facing.getYOffset() * yo,
									anchor.getZ() + zo);
				...
				outer: for (int xo = 0; xo < this.getBlockWidth(); xo++) {
					for (int yo = 0; yo < this.getBlockHeight(); yo++) {
						for (int zo = 0; zo < MathHelper.ceil(this.getPeek()); zo++) {
							pos.setPos(anchor.getX() + xo, anchor.getY() + yo,
									anchor.getZ() + z + facing.getZOffset() * zo);
							if (!this.canMoveFaceInto(pos, facing, facingUp)) {
								violations |= AnchorChecks.FACE_BLOCKS;
								break outer;
							}
				...
				outer: for (int zo = 0; zo < this.getBlockWidth(); zo++) {
					for (int yo = 0; yo < this.getBlockHeight(); yo++) {
						for (int xo = 0; xo < MathHelper.ceil(this.getPeek()); xo++) {
							pos.setPos(anchor.getX() + x + facing.getXOffset() * xo, anchor.getY() + yo,
									anchor.getZ() + zo);
							if (!this.canMoveFaceInto(pos, facing, facingUp)) {
								violations |= AnchorChecks.FACE_BLOCKS;
								break outer;
	...
	public static final class LookHelper extends EntityLookHelper {
		...
		public void onUpdateLook() {
			if (this.lookingMode == 1) {
				...
				EnumFacing[] facing = this.face
						.getFacingForLookDir(new Vec3d(this.x - center.x, this.y - center.y, this.z - center.z));
		...
		public void setSpeed(double speed) {
			if (!this.face.isMoving() && this.face.targetAnchor == null) {
				this.face.setAIMoveSpeed((float) (speed
						* this.face.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).getAttributeValue()));
			}
		}
	}
	...
	public static class MoveHelper extends EntityMoveHelper {
		...
		public void onUpdateMoveHelper() {
			if (this.action == EntityMoveHelper.Action.STRAFE && this.moveStrafe != 0) {
				Vec3i horDir = this.face.getFacing().getDirectionVec()
						.crossProduct(this.face.getFacingUp().getDirectionVec());
				int strafeDir = -(int) Math.signum(this.moveStrafe);
				this.face.targetAnchorTimeout = 30 + this.entity.world.rand.nextInt(30);
				this.face.targetAnchor = this.face.getAnchor().add(horDir.getX() * strafeDir, horDir.getY() * strafeDir,
						horDir.getZ() * strafeDir);
				this.setSpeed(this.speed);
			} else if (this.action == EntityMoveHelper.Action.MOVE_TO) {
				this.face.targetAnchorTimeout = 30 + this.entity.world.rand.nextInt(30);
				this.face.targetAnchor = new BlockPos(this.posX - this.face.getBlockWidth() / 2.0D,
						this.posY - this.face.getBlockHeight() / 2.0D, this.posZ - this.face.getBlockWidth() / 2.0D);
				this.setSpeed(this.speed);
			}
