...
public class EntityDarkDruid extends EntityMob {
	private static final DataParameter<Boolean> CASTING = EntityDataManager.createKey(EntityDarkDruid.class,
				...
				if (this.attackDelayCounter <= 0 || this.attackCounter > 0) {
					if (getEntitySenses().canSee(getAttackTarget())) {
						if (attackCounter == 0) {
							...
							this.attackDelayCounter = MIN_ATTACK_DELAY
									+ this.rand.nextInt(MAX_ATTACK_DELAY - MIN_ATTACK_DELAY + 1) + 1;
							attackCounter = 0;
							stopCasting();
							if (!world.isRemote) {
	...
	public boolean teleportNearEntity(Entity entity) {
		...
		if (world.isBlockLoaded(pos)) {
			...
			if (validBlock) {
				...
				newDruid.attackDelayCounter = MIN_ATTACK_DELAY
						+ this.rand.nextInt(MAX_ATTACK_DELAY - MIN_ATTACK_DELAY + 1) + 1;
				if (world.getCollisionBoxes(newDruid, newDruid.getEntityBoundingBox()).isEmpty()
						&& !world.containsAnyLiquid(newDruid.getEntityBoundingBox())) {
					successful = true;
					setDead();
	...
	private void druidParticlePacketTarget(EntityDarkDruid newDruid) {
		...
		if (world instanceof WorldServer) {
			...
			TheBetweenlands.networkWrapper.sendToAllAround(new MessageDruidTeleportParticles(newDruid),
					new NetworkRegistry.TargetPoint(dim, newDruid.posX + 0.5D, newDruid.posY + 1.0D,
							newDruid.posZ + 0.5D, 64D));
		}
	}
	...
	private void druidParticlePacketOrigin() {
		...
		if (world instanceof WorldServer) {
			int dim = ((WorldServer) world).provider.getDimension();
			TheBetweenlands.networkWrapper.sendToAllAround(new MessageDruidTeleportParticles(this),
					new NetworkRegistry.TargetPoint(dim, this.posX + 0.5D, this.posY + 1.0D, this.posZ + 0.5D, 64D));
		}
	}
	...
	public void spawnParticles() {
		...
		BLParticles.DRUID_CASTING.spawn(world, posX + offsetX, posY + offsetY, posZ + offsetZ,
				ParticleFactory.ParticleArgs.get().withMotion(motionX, motionY, motionZ)
						.withScale(rand.nextFloat() + 0.5F));
	}
	...
	public float getAttackAnimationTime(float partialRenderTicks) {
		return (prevAttackAnimationTime + (attackAnimationTime - prevAttackAnimationTime) * partialRenderTicks)
				/ MAX_ATTACK_ANIMATION_TIME;
	}
