...
public class EntityLurker extends EntityCreature implements IEntityBL, IMob {
	private static final DataParameter<Boolean> IS_LEAPING = EntityDataManager.createKey(EntityLurker.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> SHOULD_MOUTH_BE_OPEN = EntityDataManager.createKey(EntityLurker.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Float> MOUTH_MOVE_SPEED = EntityDataManager.createKey(EntityLurker.class,
	...
	public boolean isNotColliding() {
		return this.getEntityWorld().getCollisionBoxes(this, this.getEntityBoundingBox()).isEmpty()
				&& this.getEntityWorld().checkNoEntityCollision(this.getEntityBoundingBox(), this);
	}
	...
	private Block getRelativeBlock(int offsetY) {
		return getEntityWorld().getBlockState(new BlockPos(MathHelper.floor(posX),
				MathHelper.floor(getEntityBoundingBox().minY) + offsetY, MathHelper.floor(posZ))).getBlock();
	}
	...
	public void onLivingUpdate() {
		...
		float motionPitch = MathHelper.clamp((float) Math.atan2(magnitude, motionY) / (float) Math.PI * 180 - 90,
				-10.0F, 10.0F);
		if (magnitude > 1) {
			magnitude = 1;
		}
		float newRotationPitch = isLeaping() ? 90
				: leapFallTime > 0 ? -45
						: MathHelper.clamp((rotationPitchBody - motionPitch) * magnitude * 4 * (inWater ? 1 : 0),
								-30.0F, 30.0F);
	...
	private void breachWater() {
		...
		while (ring-- > 0) {
			...
			for (int p = 0; p < particleCount; p++) {
				...
				double x = posX + dx * ring * 1 * MathUtils.linearTransformd(rand.nextDouble(), 0, 1, 0.6, 1.2)
						+ rand.nextDouble() * 0.3 - 0.15;
				double y = posY - rand.nextDouble() * 0.2;
				double z = posZ + dz * ring * 1 * MathUtils.linearTransformd(rand.nextDouble(), 0, 1, 0.6, 1.2)
						+ rand.nextDouble() * 0.3 - 0.15;
				...
				BLParticles.SPLASH.spawn(this.getEntityWorld(), x, y, z,
						ParticleArgs.get().withMotion(motionX, motionY, motionZ).withColor(waterColorMultiplier));
			}
		}
	}
	...
	private int getWaterColor() {
		...
		if (blockState.getMaterial().isLiquid()) {
			...
			// TODO: automatically build a map of all liquid blocks to the average color of
			// there texture to get color from
			...
			int multiplier = blockState.getMapColor(getEntityWorld(), new BlockPos(blockX, blockY, blockZ))
					.getMapColor(1);
			return 0xFF000000 | (r * (multiplier >> 16 & 0xFF) / 255) << 16 | (g * (multiplier >> 8 & 0xFF) / 255) << 8
					| (b * (multiplier & 0xFF) / 255);
		}
		return 0xFFFFFFFF;
	}
	...
	protected void updateMovementAndPathfinding() {
		...
		if (this.isInWater() && !this.world
				.isAirBlock(new BlockPos(this.posX, this.getEntityBoundingBox().maxY + 0.25D, this.posZ))) {
	...
	public void onUpdate() {
		...
		if (!this.getEntityWorld().isRemote) {
			...
			if (target instanceof EntityDragonFly && attackTime <= 0 && target.getDistance(this) < 3.2D
					&& target.getEntityBoundingBox().maxY >= getEntityBoundingBox().minY
					&& target.getEntityBoundingBox().minY <= getEntityBoundingBox().maxY
					&& ticksUntilBiteDamage == -1) {
				...
				if (entityBeingBit != null) {
					if (!entityBeingBit.isDead) {
						...
						if (getRidingEntity() == entityBeingBit) {
							getRidingEntity().attackEntityFrom(DamageSource.causeMobDamage(this),
									((EntityLivingBase) entityBeingBit).getMaxHealth());
						}
					}
					entityBeingBit = null;
				}
		...
		float movementSpeed = MathHelper.sqrt((prevPosX - posX) * (prevPosX - posX)
				+ (prevPosY - posY) * (prevPosY - posY) + (prevPosZ - posZ) * (prevPosZ - posZ));
		if (movementSpeed > 1) {
			movementSpeed = 1;
	...
	public boolean attackEntityAsMob(Entity entityIn) {
		...
		if (attackTime <= 0 && distance < 3.5D && entityIn.getEntityBoundingBox().maxY >= getEntityBoundingBox().minY
				&& entityIn.getEntityBoundingBox().minY <= getEntityBoundingBox().maxY && ticksUntilBiteDamage == -1) {
			setShouldMouthBeOpen(true);
			setMouthMoveSpeed(10);
			ticksUntilBiteDamage = 10;
			attackTime = 10;
	...
	public boolean attackEntityFrom(DamageSource source, float damage) {
		...
		if (attacker instanceof EntityPlayer) {
			List<EntityLurker> nearLurkers = getEntityWorld().getEntitiesWithinAABB(EntityLurker.class,
					getEntityBoundingBox().grow(16, 16, 16));
		...
		public void onUpdateMoveHelper() {
			if (action == EntityMoveHelper.Action.MOVE_TO && !lurker.getNavigator().noPath()) {
				...
				float f1 = (float) (speed
						* lurker.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).getAttributeValue());
				...
				if (Math.abs(lurker.motionY) < 0.35) {
					lurker.motionY += (double) lurker.getAIMoveSpeed() * d1 * 0.1D
							* (2 + (d1 > 0 ? 0.4 : 0) + (lurker.collidedHorizontally ? 20 : 0));
				}
				...
				lurker.getLookHelper().setLookPosition(d10 + (d7 - d10) * 0.125D, d11 + (d8 - d11) * 0.125D,
