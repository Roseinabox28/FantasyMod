...
public class EntityFortressBossBlockade extends EntityMob implements IEntityBL {
	protected static final DataParameter<Optional<UUID>> OWNER = EntityDataManager
			.<Optional<UUID>>createKey(EntityFortressBossBlockade.class, DataSerializers.OPTIONAL_UNIQUE_ID);
	protected static final DataParameter<Float> SIZE = EntityDataManager
			.<Float>createKey(EntityFortressBossBlockade.class, DataSerializers.FLOAT);
	protected static final DataParameter<Float> ROTATION = EntityDataManager
			.<Float>createKey(EntityFortressBossBlockade.class, DataSerializers.FLOAT);
	...
	public void onUpdate() {
		...
		if (!this.world.isRemote) {
			...
			List<EntityPlayer> targets = this.world.getEntitiesWithinAABB(EntityPlayer.class,
					this.getEntityBoundingBox().grow(this.getTriangleSize() * 2, 0, this.getTriangleSize() * 2));
			for (EntityPlayer target : targets) {
				Vec3d[] vertices = this.getTriangleVertices(1);
				if (EntityFortressBoss.rayTraceTriangle(new Vec3d(target.posX - this.posX, 1, target.posZ - this.posZ),
						new Vec3d(0, -2, 0), vertices[0], vertices[1], vertices[2])) {
					float damage = (float) this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE)
							.getAttributeValue();
					if (target.attackEntityFrom(DamageSource.MAGIC, damage) && this.getOwner() != null
							&& this.getOwner() instanceof EntityLivingBase) {
			...
			for (int c = 0; c < 4; c++) {
				...
				if (this.getOwner() != null) {
					...
					sy = this.getOwner().getEntityBoundingBox().minY + (this.getOwner().getEntityBoundingBox().maxY
							- this.getOwner().getEntityBoundingBox().minY) / 2.0D;
	...
	public boolean isEntityInvulnerable(DamageSource source) {
		return !DamageSource.OUT_OF_WORLD.getDamageType().equals(source.getDamageType())
				&& source.getImmediateSource() instanceof EntityShockwaveBlock == false;
	}
