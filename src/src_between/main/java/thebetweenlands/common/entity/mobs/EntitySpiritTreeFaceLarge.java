...
public class EntitySpiritTreeFaceLarge extends EntitySpiritTreeFace implements IEntityWithLootModifier, IBLBoss {
	...
	protected static final UUID STRENGTH_MULTIPLIER_ATTRIBUTE_UUID = UUID
			.fromString("8a8dccae-273d-445d-b581-81d4a8a979a5");
	protected static final UUID HEALTH_MULTIPLIER_ATTRIBUTE_UUID = UUID
			.fromString("e29b66a3-2ed2-4598-a44c-ed82a8f03eb2");

	private static final DataParameter<Integer> BLOW_STATE = EntityDataManager
			.createKey(EntitySpiritTreeFaceLarge.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> SPIT_STATE = EntityDataManager
			.createKey(EntitySpiritTreeFaceLarge.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> ROTATING_WAVE_STATE = EntityDataManager
			.createKey(EntitySpiritTreeFaceLarge.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> CRAWLING_WAVE_STATE = EntityDataManager
			.createKey(EntitySpiritTreeFaceLarge.class, DataSerializers.VARINT);
	private static final DataParameter<Float> WISP_STRENGTH_MODIFIER = EntityDataManager
			.createKey(EntitySpiritTreeFaceLarge.class, DataSerializers.FLOAT);
	private static final DataParameter<BlockPos> LAST_LOCKED_WISP = EntityDataManager
			.createKey(EntitySpiritTreeFaceLarge.class, DataSerializers.BLOCK_POS);
	private static final DataParameter<Optional<UUID>> BOSSINFO_ID = EntityDataManager
			.createKey(EntitySpiritTreeFaceLarge.class, DataSerializers.OPTIONAL_UNIQUE_ID);
	...
	protected void initEntityAI() {
		...
		this.tasks.addTask(2, new AISpit(this, 4.5F) {
			...
			protected float getSpitDamage() {
				return (float) EntitySpiritTreeFaceLarge.this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE)
						.getAttributeValue() / 3.0F;
			}
		});
	...
	public List<BlockPos> findNearbyBlocksForMovement() {
		List<LocationSpiritTree> locations = BetweenlandsWorldStorage.forWorld(this.world).getLocalStorageHandler()
				.getLocalStorages(LocationSpiritTree.class, this.getEntityBoundingBox(), loc -> loc.isInside(this));
	...
	public void handleStatusUpdate(byte id) {
		...
		if (id == EVENT_ATTACKED) {
			...
			for (int i = 0; i < 16; i++) {
				...
				vec = vec.normalize();
				this.world.spawnParticle(EnumParticleTypes.BLOCK_CRACK, frontCenter.x + rx, frontCenter.y + ry,
						frontCenter.z + rz, vec.x * 1.5F, vec.y * 1.5F, vec.z * 1.5F,
						Block.getIdFromBlock(BlockRegistry.LOG_SPIRIT_TREE));
			}
			...
			for (int i = 0; i < 64; i++) {
				...
				Vec3d vec = new Vec3d(rx, ry, rz);
				vec = vec.normalize();
				ParticleRootSpike particle = (ParticleRootSpike) BLParticles.ROOT_SPIKE.spawn(world, frontCenter.x,
						frontCenter.y - 0.25D, frontCenter.z,
						ParticleArgs.get().withMotion(vec.x * 0.45F, vec.y * 0.45F + 0.2F, vec.z * 0.45F));
			...
			for (int i = 0; i < 32; i++) {
				...
				Vec3d vec = new Vec3d(rx, ry, rz);
				vec = vec.normalize();
				BLParticles.MOTION_ITEM_BREAKING.spawn(world, frontCenter.x, frontCenter.y - 0.25D, frontCenter.z,
						ParticleArgs.get().withMotion(vec.x * 0.25F, vec.y * 0.25F, vec.z * 0.25F)
								.withData(ItemRegistry.SAP_SPIT, 0));
			}
		}
	}
	...
	public void onDeath(DamageSource cause) {
		...
		if (!this.world.isRemote) {
			List<SpiritTreeKillToken> killTokens = BetweenlandsWorldStorage.forWorld(this.world)
					.getSpiritTreeKillTokens();
			...
			if (!locations.isEmpty()) {
				...
				List<EntitySpiritTreeFaceSmall> smallFaces = this.world
						.getEntitiesWithinAABB(EntitySpiritTreeFaceSmall.class, location.getEnclosingBounds());
						...
						for (int yo = -radius; yo <= radius; yo++) {
							for (int zo = -radius; zo <= radius; zo++) {
								if (xo * xo + yo * yo + zo * zo <= radius * radius) {
									...
									if (SurfaceType.GRASS_AND_DIRT.matches(state)
											&& !this.world.getBlockState(pos.up()).isNormalCube()
											&& this.world.rand.nextInt(3) == 0) {
										this.world.setBlockState(pos,
												BlockRegistry.SPREADING_SLUDGY_DIRT.getDefaultState());
									}

									if (state.getBlock() == BlockRegistry.LOG_SPIRIT_TREE
											&& this.world.rand.nextInt(5) == 0) {
										this.world.setBlockState(pos,
												BlockRegistry.LOG_SPREADING_ROTTEN_BARK.getDefaultState());
									}
								}
							}
						}
	...
	protected void wispLockEffect(BlockPos pos) {
		for (int i = 0; i < 64; i++) {
			Vec3d dir = new Vec3d(this.rand.nextFloat() - 0.5F, this.rand.nextFloat() - 0.5F + 0.25F,
					this.rand.nextFloat() - 0.5F);
			dir = dir.normalize().scale(2);
			BLParticles.CORRUPTED.spawn(this.world, pos.getX() + 0.5D + this.rand.nextFloat() / 2.0F - 0.25F,
					pos.getY() + 0.5D + this.rand.nextFloat() / 2.0F - 0.25F,
					pos.getZ() + 0.5D + this.rand.nextFloat() / 2.0F - 0.25F,
					ParticleArgs.get().withMotion(dir.x, dir.y, dir.z));
		}
		this.world.playSound(pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D, SoundRegistry.DAMAGE_REDUCTION,
				SoundCategory.HOSTILE, 0.65F, 0.5F, false);
	}
	...
	public void onUpdate() {
		...
		if (this.dataManager.get(BLOW_STATE) != 0 || this.dataManager.get(ROTATING_WAVE_STATE) != 0
				|| this.dataManager.get(CRAWLING_WAVE_STATE) != 0 || this.dataManager.get(SPIT_STATE) != 0) {
		...
		if (!this.world.isRemote) {
			if (this.isActive() && this.isEntityAlive() && this.getAttackTarget() != null) {
				...
				if (this.ticksExisted % 10 == 0) {
					List<LocationSpiritTree> locations = BetweenlandsWorldStorage.forWorld(this.world)
							.getLocalStorageHandler().getLocalStorages(LocationSpiritTree.class,
									this.getEntityBoundingBox(), loc -> loc.isInside(this));
					if (!locations.isEmpty()) {
						LocationSpiritTree location = locations.get(0);
						...
						for (BlockPos wispPosition : wispPositions) {
							if (!location.getGuard().isGuarded(this.world, null, wispPosition)
									&& this.world.getBlockState(wispPosition).getBlock() == BlockRegistry.WISP) {
			...
			attackAttribute.applyModifier(new AttributeModifier(STRENGTH_MULTIPLIER_ATTRIBUTE_UUID,
					"Wisp strength modifier", strengthModifier - 1.0F, 2));
			...
			if (this.blowTicks > 0) {
				if (this.blowTicks > 20 + this.blowDelay) {
					...
					if ((this.blowTicks - (21 + this.blowDelay)) % 15 == 0) {
						...
						this.playSound(SoundRegistry.SPIRIT_TREE_FACE_SPIT_ROOT_SPIKES, 1,
				...
				if (this.rotatingWaveTicks > this.rotatingWaveDelay) {
					...
					if ((this.rotatingWaveTicks - 1 - this.rotatingWaveDelay) % 3 == 0) {
						for (int i = 0; i < 2; i++) {
							...
							double a1 = (this.rotatingWaveStart
									+ (this.rotatingWaveTicks - this.rotatingWaveDelay) / 3 * increment)
									* (i == 0 ? 1 : -1);
							double a2 = (this.rotatingWaveStart
									+ ((this.rotatingWaveTicks - this.rotatingWaveDelay) / 3 + 1) * increment)
									* (i == 0 ? 1 : -1);

							double start = Math.min(a1, a2);
							double end = Math.max(a1, a2);

							List<BlockPos> blocks = BlockShapeUtils.getRingSegment(this.getAnchor(), start, end,
									WorldGenSpiritTreeStructure.RADIUS_INNER_CIRLCE + 0.5D,
									WorldGenSpiritTreeStructure.RADIUS_OUTER_CIRCLE + 0.5D, false, new ArrayList<>());
				...
				if (this.crawlingWaveTicks > this.crawlingWaveDelay) {
					...
					if (this.getAttackTarget() != null) {
						if ((this.crawlingWaveTicks - 1 - this.crawlingWaveDelay) % ticksPerWave == 0) {
							this.crawlingWaveAngle = (float) Math.atan2(this.getAttackTarget().posZ - this.posZ,
									this.getAttackTarget().posX - this.posX);
						}

						if ((this.crawlingWaveTicks - 1 - this.crawlingWaveDelay) % 4 == 0) {
							int dist = WorldGenSpiritTreeStructure.RADIUS_OUTER_CIRCLE
									+ ((this.crawlingWaveTicks - 1 - this.crawlingWaveDelay) / 4 * 3)
											% CRAWLING_WAVE_RANGE;

							double a1 = this.crawlingWaveAngle - Math.PI / 16;
							double a2 = this.crawlingWaveAngle + Math.PI / 16;

							List<BlockPos> blocks = BlockShapeUtils.getRingSegment(this.getAnchor(), a1, a2, dist,
									dist + 3, false, new ArrayList<>());
			...
			if (blowState == 2) {
				...
				for (int i = 0; i < 4; i++) {
					...
					Vec3d vec = new Vec3d(rx, ry, rz);
					vec = vec.normalize();
					this.world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, frontCenter.x + rx,
							frontCenter.y - 0.75D + ry, frontCenter.z + rz, -vec.x * 0.5F, -vec.y * 0.5F,
							-vec.z * 0.5F);
				}
			}
		}
	...
	public boolean isTargetInBlowRange(EntityLivingBase target) {
		if (target.getDistanceSq(this) <= 5 * 5) {
			...
			Vec3d facing = new Vec3d(this.getFacing().getXOffset(), this.getFacing().getYOffset(),
					this.getFacing().getZOffset());
	...
	public void doBlowAttack() {
		...
		List<EntityLivingBase> targets = this.world.getEntitiesWithinAABB(EntityLivingBase.class,
				new AxisAlignedBB(frontCenter.x - 5, frontCenter.y - 5, frontCenter.z - 5, frontCenter.x + 5,
						frontCenter.y + 5, frontCenter.z + 5));
	...
	public void startRotatingWaveAttack() {
		...
		if (this.getWispStrengthModifier() > 1.0F) {
			this.rotatingWaveDelay = (int) (DEFAULT_ROTATING_WAVE_DELAY
					/ (1.0F + (this.getWispStrengthModifier() - 1.0F) * 2.0F));
		} else {
	...
	public boolean isTargetInCrawlingWaveAttackRange(EntityLivingBase target) {
		...
		int outerSq = (WorldGenSpiritTreeStructure.RADIUS_OUTER_CIRCLE + CRAWLING_WAVE_RANGE)
				* (WorldGenSpiritTreeStructure.RADIUS_OUTER_CIRCLE + CRAWLING_WAVE_RANGE);
		return dstSq >= innerSq && dstSq <= outerSq;
	}
	...
	public void startCrawlingWaveAttack() {
		...
		if (this.getWispStrengthModifier() > 1.0F) {
			this.crawlingWaveDelay = (int) (DEFAULT_CRAWLING_WAVE_DELAY
					/ (1.0F + (this.getWispStrengthModifier() - 1.0F) * 2.0F));
		} else {
	...
	public boolean startGrabAttack() {
		if (this.getAttackTarget() != null) {
			...
			for (int i = 0; i < 6; i++) {
				BlockPos pos = new BlockPos(target.posX + this.rand.nextInt(3) - 1, target.posY - 1,
						target.posZ + this.rand.nextInt(3) - 1);
	...
	protected void updateWispStrengthModifier() {
		...
		if (!locations.isEmpty()) {
			...
			this.dataManager.set(WISP_STRENGTH_MODIFIER,
					decay * newModifier + (1 - decay) * this.dataManager.get(WISP_STRENGTH_MODIFIER));
		}
	}
	...
	public Vec3d getMiniBossTagOffset(float partialTicks) {
		return new Vec3d(this.getFacing().getXOffset() * (this.width / 2), this.height + 0.5D,
				this.getFacing().getZOffset() * (this.width / 2));
	}
	...
	public static class AIRespawnSmallFaces extends EntityAIBase {
		...
		protected int countSmallFaces() {
			return this.entity.world.getEntitiesWithinAABB(EntitySpiritTreeFaceSmall.class,
					this.entity.getEntityBoundingBox().grow(40.0D)).size();
		}
		...
		public void updateTask() {
			if (this.spawnCooldown <= 0) {
				this.spawnCooldown = (int) ((180 + this.entity.rand.nextInt(100))
						/ this.entity.getWispStrengthModifier());
				...
				if (!blocks.isEmpty()) {
					...
					spawnLoop: for (int i = 0; i < 16; i++) {
						...
						for (EnumFacing facing : facings) {
							EnumFacing facingUp = facing.getAxis().isVertical()
									? EnumFacing.HORIZONTALS[this.entity.rand.nextInt(EnumFacing.HORIZONTALS.length)]
									: EnumFacing.UP;
	...
	public static class AIBlowAttack extends EntityAIBase {
		...
		@Override
		public boolean shouldExecute() {
			if (this.entity.isActive() && !this.entity.isAttacking() && this.entity.getAttackTarget() != null
					&& this.entity.isTargetInBlowRange(this.entity.getAttackTarget())) {
				if (this.cooldown <= 0) {
	...
	public static class AIRotatingWaveAttack extends EntityAIBase {
		...
		@Override
		public boolean shouldExecute() {
			if (this.entity.isActive() && this.entity.rotatingWaveTicks == 0 && this.entity.getAttackTarget() != null
					&& this.entity.isTargetInRotatingWaveAttackRange(this.entity.getAttackTarget())) {
				if (this.cooldown <= 0) {
	...
	public static class AICrawlingWaveAttack extends EntityAIBase {
		...
		@Override
		public boolean shouldExecute() {
			if (this.entity.isActive() && this.entity.crawlingWaveTicks == 0 && this.entity.getAttackTarget() != null
					&& this.entity.isTargetInCrawlingWaveAttackRange(this.entity.getAttackTarget())) {
				if (this.cooldown <= 0) {
					this.cooldown = (int) ((60 + this.entity.rand.nextInt(80)) / this.entity.getWispStrengthModifier());
					return true;
				}
	...
	public static class AIGrabAttack extends EntityAIBase {
		...
		public boolean shouldExecute() {
			if (this.entity.isActive() && !this.entity.isAttacking() && this.entity.getAttackTarget() != null
					&& this.entity.getAttackTarget().onGround
					&& this.entity.isTargetInGrabAttackRange(this.entity.getAttackTarget())) {
				if (this.cooldown <= 0) {
					this.cooldown = (int) ((60 + this.entity.rand.nextInt(80)) / this.entity.getWispStrengthModifier());
					return true;
				}
