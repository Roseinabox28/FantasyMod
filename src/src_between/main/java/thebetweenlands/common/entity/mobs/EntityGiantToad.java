...
public class EntityGiantToad extends EntityCreature implements IEntityBL {
	private static final DataParameter<Byte> DW_SWIM_STROKE = EntityDataManager.createKey(EntityGiantToad.class,
			DataSerializers.BYTE);
	private static final DataParameter<Boolean> DW_TAMED = EntityDataManager.createKey(EntityGiantToad.class,
	...
	public void onUpdate() {
		...
		if (this.isBeingRidden()) {
			this.setEntityBoundingBox(this.getEntityBoundingBox()
					.setMaxY(this.getEntityBoundingBox().minY + this.height + this.getControllingPassenger().height));
		}
		super.onUpdate();
		...
		if (!world.isRemote) {
						...
						if (distance > 1) {
							if (this.ticksOnGround > 20) {
								...
								motionY += speedMultiplier * 0.6
										+ MathHelper.clamp((nextHopSpot.y - this.posY) / 6.0D, 0.0D, 0.3D);
			...
			if (this.isBeingRidden()) {
				List<EntityLivingBase> targets = this.world.getEntitiesWithinAABB(EntityLivingBase.class,
						this.getEntityBoundingBox().grow(0.6D, 0.6D, 0.6D));
				EntityLivingBase closestTarget = null;
				float lastAngDiff = 0.0F;
				Entity controllingPassenger = this.getControllingPassenger();
				for (EntityLivingBase target : targets) {
					if (target.getRevengeTarget() == controllingPassenger
							|| (controllingPassenger instanceof EntityLivingBase
									&& ((EntityLivingBase) controllingPassenger).getRevengeTarget() == target)) {
						float x = (float) (target.posX - posX);
						float z = (float) (target.posZ - posZ);
						float angle = (float) (Math.atan2(z, x));
						float angDiff = (float) Math
								.abs(this.rotationYaw % 360.0F - Math.toDegrees(angle) % 360.0F + 90) % 360.0F;
				...
				if (closestTarget != null) {
					...
					if (closestTarget.attackEntityFrom(damageSource, attackDamage)) {
						...
						if (doesJump && ((this.onGround && this.ticksOnGround >= 5)
								|| (this.inWater && this.strokeTicks == 0))) {
							float angle = (float) (Math.atan2(z, x));
							if (!this.inWater)
								motionY += 0.4;
	...
	public float getSwimProgress(float partialRenderTicks) {
		return Math.min((1F - (float) Math.pow(1 - this.swimmingAnim.getAnimationFraction(partialRenderTicks), 2))
				* 2.5F * this.swimmingAnim.getAnimationFraction(partialRenderTicks), 1.0F);
	}
	...
	protected boolean processInteract(EntityPlayer player, EnumHand hand) {
		boolean holdsEquipment = hand == EnumHand.MAIN_HAND && !player.getHeldItem(hand).isEmpty()
				&& (player.getHeldItem(hand).getItem() instanceof IEquippable
						|| player.getHeldItem(hand).getItem() == ItemRegistry.AMULET_SLOT);
		if (holdsEquipment)
			return true;
		...
		if (!this.isBeingRidden() && this.isTamed() && (!holdsWings || this.getHealth() >= this.getMaxHealth())
				&& !player.isSneaking() && !this.world.isRemote) {
	...
	public boolean attackEntityFrom(DamageSource source, float damage) {
		...
		return !(this.isBeingRidden() && this.getControllingPassenger() != null
				&& this.getControllingPassenger().equals(entity)) && super.attackEntityFrom(source, damage);
	}
	...
	public void travel(float strafing, float up, float forward) {
		...
		if (this.isBeingRidden() && controllingPassenger != null && controllingPassenger instanceof EntityLivingBase) {
			...
			boolean onWaterSurface = this.world.getBlockState(new BlockPos(MathHelper.floor(this.posX),
					MathHelper.floor(this.posY + 1), MathHelper.floor(this.posZ))).getMaterial().isLiquid();
			...
			if (!this.inWater || !onWaterSurface) {
				...
				if (this.motionY < 0.0F) {
					if (this.world.getBlockState(new BlockPos(MathHelper.floor(this.posX),
							MathHelper.floor(this.posY + 0.9D), MathHelper.floor(this.posZ))).getMaterial()
							.isLiquid()) {
						this.motionY *= 0.05F;
					}
				}
				if (this.world.getBlockState(new BlockPos(MathHelper.floor(this.posX),
						MathHelper.floor(this.posY + 1.1D), MathHelper.floor(this.posZ))).getMaterial().isLiquid()) {
					this.motionY += 0.02F;
				}
