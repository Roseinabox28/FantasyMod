...
public class EntityFrog extends EntityCreature implements IEntityBL {
	public static final IAttribute FROG_SKIN_ATTRIB = (new RangedAttribute(null, "bl.frogSkin", 0, 0, 5))
			.setDescription("Frog skin").setShouldWatch(true);

	private static final DataParameter<Byte> DW_SWIM_STROKE = EntityDataManager.createKey(EntityFrog.class,
		...
		if (!this.world.isRemote) {
			...
			if (!this.world.isRemote) {
				if (this.motionY < 0.0F
						&& this.world
								.getBlockState(new BlockPos(MathHelper.floor(this.posX),
										MathHelper.floor(this.posY + 0.4D), MathHelper.floor(this.posZ)))
				...
				if ((path == null || path.isFinished())
						&& this.world
								.getBlockState(new BlockPos(MathHelper.floor(this.posX),
										MathHelper.floor(this.posY + 0.5D), MathHelper.floor(this.posZ)))
								.getMaterial().isLiquid()) {
					this.motionY += 0.04F;
					this.velocityChanged = true;
				}
	...
	public void onCollideWithPlayer(EntityPlayer player) {
		...
		if (getSkin() == 4) {
			if (!world.isRemote && !player.capabilities.isCreativeMode
					&& player.getEntityBoundingBox().maxY >= getEntityBoundingBox().minY
					&& player.getEntityBoundingBox().minY <= getEntityBoundingBox().maxY
					&& player.getEntityBoundingBox().maxX >= getEntityBoundingBox().minX
					&& player.getEntityBoundingBox().minX <= getEntityBoundingBox().maxX
					&& player.getEntityBoundingBox().maxZ >= getEntityBoundingBox().minZ
					&& player.getEntityBoundingBox().minZ <= getEntityBoundingBox().maxZ) {
				int duration = 0;
