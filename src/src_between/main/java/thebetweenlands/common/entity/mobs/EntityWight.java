...
public class EntityWight extends EntityMob implements IEntityBL {
	...
	public static final IAttribute VOLATILE_HEALTH_START_ATTRIB = (new RangedAttribute(null, "bl.volatileHealthStart",
			1.0D, 0.0D, 1.0D)).setDescription("Volatile Health Percentage Start");
	public static final IAttribute VOLATILE_COOLDOWN_ATTRIB = (new RangedAttribute(null, "bl.volatileCooldown", 400.0D,
			10.0D, Integer.MAX_VALUE)).setDescription("Volatile Cooldown");
	public static final IAttribute VOLATILE_FLIGHT_SPEED_ATTRIB = (new RangedAttribute(null, "bl.volatileFlightSpeed",
			0.32D, 0.0D, 5.0D)).setDescription("Volatile Flight Speed");
	public static final IAttribute VOLATILE_LENGTH_ATTRIB = (new RangedAttribute(null, "bl.volatileLength", 600.0D,
			0.0D, Integer.MAX_VALUE)).setDescription("Volatile Length");
	public static final IAttribute VOLATILE_MAX_DAMAGE_ATTRIB = (new RangedAttribute(null, "bl.volatileMaxDamage",
			20.0D, 0.0D, Double.MAX_VALUE)).setDescription("Volatile Max Damage");

	protected static final DataParameter<Boolean> HIDING_STATE_DW = EntityDataManager.createKey(EntityWight.class,
			DataSerializers.BOOLEAN);
	protected static final DataParameter<Boolean> VOLATILE_STATE_DW = EntityDataManager.createKey(EntityWight.class,
			DataSerializers.BOOLEAN);
	...
	public EntityWight(World world) {
		...
		this.flightMoveHelper = new FlightMoveHelper(this) {
			...
			protected double getFlightSpeed() {
				return this.entity.getAttributeMap().getAttributeInstance(VOLATILE_FLIGHT_SPEED_ATTRIB)
						.getAttributeValue();
			}
		};
	...
	protected void initEntityAI() {
		...
		this.tasks.addTask(3, new EntityAIMoveToDirect<EntityWight>(this,
				this.getAttributeMap().getAttributeInstance(VOLATILE_FLIGHT_SPEED_ATTRIB).getAttributeValue()) {
			@Override
		...
		this.tasks.addTask(9, new EntityAIFlyRandomly<EntityWight>(this) {
			...
			public boolean shouldExecute() {
				return this.entity.isVolatile() && this.entity.volatileTicks >= 20
						&& this.entity.getAttackTarget() == null && super.shouldExecute();
			}
	...
	public void onUpdate() {
		if (!this.world.isRemote) {
			if (this.getAttackTarget() == null) {
				...
				if (this.canTurnVolatile && !this.isVolatile() && !this.isRiding()
						&& this.canPossess(this.getAttackTarget()) && this.canTurnVolatileOnTarget) {
					if (this.volatileCooldownTicks > 0) {
						this.volatileCooldownTicks--;
					}

					if (this.getHealth() <= this.getMaxHealth()
							* this.getEntityAttribute(VOLATILE_HEALTH_START_ATTRIB).getAttributeValue()
							&& this.volatileCooldownTicks <= 0) {
						this.setVolatile(true);
						this.didTurnVolatileOnPlayer = true;
						this.volatileReceivedDamage = 0.0F;
						this.volatileCooldownTicks = this.getMaxVolatileCooldown()
								+ this.world.rand.nextInt(this.getMaxVolatileCooldown()) + 20;
						this.volatileTicks = 0;

						TheBetweenlands.networkWrapper.sendToAllAround(new MessageWightVolatileParticles(this),
								new TargetPoint(this.dimension, this.posX, this.posY, this.posZ, 32));
						this.world.playSound(null, this.posX, this.posY, this.posZ, SoundRegistry.WIGHT_ATTACK,
								SoundCategory.HOSTILE, 1.6F, 1.0F);
					}
				} else if (this.didTurnVolatileOnPlayer && this.isVolatile()
						&& !this.canPossess(this.getAttackTarget())) {
				...
				if (this.getAttackTarget() != null) {
					...
					if (this.getRidingEntity() == null) {
						...
						if (attackTarget instanceof EntityLivingBase) {
							...
							dy = entitylivingbase.posY + (double) entitylivingbase.getEyeHeight()
									- (this.posY + (double) this.getEyeHeight());
						} else {
							dy = (attackTarget.getEntityBoundingBox().minY + attackTarget.getEntityBoundingBox().maxY)
						...
						if (this.ticksExisted % 5 == 0 && this.canEntityBeSeen(this.getAttackTarget())
								&& !this.isWearingSkullMask(this.getAttackTarget())) {
							List<EntityVolatileSoul> existingSouls = this.world.getEntitiesWithinAABB(
									EntityVolatileSoul.class, this.getEntityBoundingBox().grow(16.0D, 16.0D, 16.0D));
							if (existingSouls.size() < 16) {
								...
								soul.setLocationAndAngles(this.posX + dir.x * 0.5D, this.posY + dir.y * 1.5D,
										this.posZ + dir.z * 0.5D, 0, 0);
	...
	public boolean isEntityInvulnerable(DamageSource source) {
		boolean isCreative = source instanceof EntityDamageSourceIndirect
				&& ((EntityDamageSourceIndirect) source).getTrueSource() instanceof EntityPlayer
				&& ((EntityPlayer) ((EntityDamageSourceIndirect) source).getTrueSource()).isCreative();
	...
	public boolean attackEntityFrom(DamageSource source, float damage) {
		...
		if (this.didTurnVolatileOnPlayer && this.isVolatile() && this.getRidingEntity() != null) {
			...
			if (this.volatileReceivedDamage >= this.getEntityAttribute(VOLATILE_MAX_DAMAGE_ATTRIB)
					.getAttributeValue()) {
				this.setVolatile(false);
				this.didTurnVolatileOnPlayer = false;
			}
		}
		if (this.getAttackTarget() != null && source instanceof EntityDamageSource
				&& source.getTrueSource() == this.getAttackTarget()) {
	...
	public float getHidingAnimation(float partialTicks) {
		return (this.lastHidingAnimationTicks
				+ (this.hidingAnimationTicks - this.lastHidingAnimationTicks) * partialTicks) / 12.0F;
	}
