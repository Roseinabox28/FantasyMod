...
public class EntityPyrad extends EntityFlyingMob implements IEntityBL {
	public static final IAttribute FLAMES_PER_ATTACK = (new RangedAttribute(null, "bl.flamesPerAttack", 6.0D, 1.0D,
			64.0D)).setDescription("Number range of flames per attack");
	public static final IAttribute AGRESSIVE = (new BooleanAttribute(null, "bl.pyradAgressive", false))
			.setDescription("Whether the Pyrad is agressive and doesn't go inactive");

	private static final DataParameter<Boolean> CHARGING = EntityDataManager.<Boolean>createKey(EntityPyrad.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> ACTIVE = EntityDataManager.<Boolean>createKey(EntityPyrad.class,
			DataSerializers.BOOLEAN);
		...
		this.activeTasks.add(new EntityAIMoveToDirect<EntityPyrad>(this, 1.0D) {
			...
			protected Vec3d getTarget() {
				...
				if (target != null) {
					...
					Vec3d dir = new Vec3d(target.posX - this.entity.posX,
							target.posY + 1 - this.entity.rand.nextFloat() * 0.3 - this.entity.posY - 1,
							target.posZ - this.entity.posZ);
					double dst = dir.length();
					if (dst > 10) {
						dir = dir.normalize();
						this.setSpeed(0.75D);
						return new Vec3d(this.entity.posX + dir.x * (dst - 10),
								Math.min(this.entity.posY + dir.y * (dst - 10),
										Math.max(groundHeight + 2, target.posY + 2)),
								this.entity.posZ + dir.z * (dst - 10));
					} else if (dst < 5) {
						dir = dir.normalize();
						this.setSpeed(1.0D);
						return new Vec3d(this.entity.posX - dir.x * 2,
								Math.min(this.entity.posY - dir.y * 2,
										Math.max(groundHeight + (this.entity.isCharging() ? 6 : 2), target.posY + 2)),
								this.entity.posZ - dir.z * 2);
					}
				}
				return null;
			}
	...
	public void onLivingUpdate() {
		...
		if (!this.world.isRemote) {
			...
			if (this.isEntityAlive()
					&& (this.getEntityAttribute(AGRESSIVE).getAttributeValue() == 1 || !day || this.isInWater())
					&& !this.isActive()) {
				this.setActive(true);
			}

			if (this.getEntityAttribute(AGRESSIVE).getAttributeValue() == 0 && this.isEntityAlive() && this.isActive()
					&& this.getAttackTarget() == null && this.rand.nextInt(800) == 0) {
		...
		if (this.world.isRemote && this.isActive()) {
			...
			if (this.isCharging() || this.rand.nextInt(10) == 0) {
				ParticleArgs<?> args = ParticleArgs.get().withMotion((this.rand.nextFloat() - 0.5F) / 4.0F,
						(this.rand.nextFloat() - 0.5F) / 4.0F, (this.rand.nextFloat() - 0.5F) / 4.0F);
				if (this.isCharging()) {
					args.withColor(0.9F, 0.35F, 0.1F, 1);
					args.withData(60);
				} else {
				...
				BLParticles.WEEDWOOD_LEAF.spawn(this.world, this.posX, this.posY + this.getEyeHeight(), this.posZ,
						args);
			}
		}
	...
	public AxisAlignedBB getCollisionBox(Entity entity) {
		return !this.isActive() && this.activeTicks == 0 ? entity.getEntityBoundingBox()
				: super.getCollisionBox(entity);
	}

	@Override
	@Nullable
	public AxisAlignedBB getCollisionBoundingBox() {
		return !this.isActive() && this.activeTicks == 0 ? this.getEntityBoundingBox()
				: super.getCollisionBoundingBox();
	}
				...
				for (int i = 0; i < 10; i++) {
					if (this.world.isRemote) {
						ParticleArgs<?> args = ParticleArgs.get()
								.withMotion((this.rand.nextFloat() - 0.5F) / 2.0F,
										(this.rand.nextFloat() - 0.5F) / 2.0F, (this.rand.nextFloat() - 0.5F) / 2.0F)
								.withColor(1F, 0.65F, 0.25F, 1);
					...
					for (int i = 0; i < 10; i++) {
						ParticleArgs<?> args = ParticleArgs.get()
								.withMotion((this.rand.nextFloat() - 0.5F) / 2.0F,
										(this.rand.nextFloat() - 0.5F) / 2.0F, (this.rand.nextFloat() - 0.5F) / 2.0F)
								.withScale(2.0F);
						args.withColor(1F, 0.25F + this.rand.nextFloat() * 0.5F, 0.05F + this.rand.nextFloat() * 0.25F,
								1);
						BLParticles.WEEDWOOD_LEAF.spawn(this.world, this.posX, this.posY + 0.8D, this.posZ, args);
						args = ParticleArgs.get().withMotion((this.rand.nextFloat() - 0.5F) / 2.0F,
								(this.rand.nextFloat() - 0.5F) / 2.0F, (this.rand.nextFloat() - 0.5F) / 2.0F);
			...
			if (target != null) {
				...
				if (distSq < 4.0D) {
					...
					double dy = target.getEntityBoundingBox().minY + (double) (target.height / 2.0F)
							- (this.pyrad.posY + (double) (this.pyrad.height / 2.0F));
					...
					if (this.attackTime <= 0) {
						...
						if (this.attackStep > 1) {
							...
							this.pyrad.world.playEvent((EntityPlayer) null, 1018,
									new BlockPos((int) this.pyrad.posX, (int) this.pyrad.posY, (int) this.pyrad.posZ),
									0);

							int numberFlames = (int) this.pyrad.getEntityAttribute(FLAMES_PER_ATTACK)
									.getAttributeValue();

							for (int i = 0; i < (numberFlames > 1 ? this.pyrad.rand.nextInt(numberFlames) : 0)
									+ 1; ++i) {
								EntityPyradFlame flame = new EntityPyradFlame(this.pyrad.world, this.pyrad,
										dx + this.pyrad.getRNG().nextGaussian() * (double) f, dy,
										dz + this.pyrad.getRNG().nextGaussian() * (double) f);
