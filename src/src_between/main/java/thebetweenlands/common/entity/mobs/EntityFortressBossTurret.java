...
public class EntityFortressBossTurret extends EntityMob implements IEntityBL {
	protected static final DataParameter<Optional<UUID>> OWNER = EntityDataManager
			.<Optional<UUID>>createKey(EntityFortressBossTurret.class, DataSerializers.OPTIONAL_UNIQUE_ID);
	protected static final DataParameter<Optional<UUID>> TARGET = EntityDataManager
			.<Optional<UUID>>createKey(EntityFortressBossTurret.class, DataSerializers.OPTIONAL_UNIQUE_ID);
	protected static final DataParameter<Boolean> DEFLECTION_STATE = EntityDataManager
			.<Boolean>createKey(EntityFortressBossTurret.class, DataSerializers.BOOLEAN);
	...
	public Entity getOwner() {
		...
		} else if (this.cachedOwner == null || !this.cachedOwner.isEntityAlive()
				|| !this.cachedOwner.getUniqueID().equals(uuid)) {
			this.cachedOwner = null;
			for (Entity entity : this.getEntityWorld().getEntitiesWithinAABB(Entity.class,
					this.getEntityBoundingBox().grow(64.0D, 64.0D, 64.0D))) {
	...
	public Entity getTarget() {
		...
		} else if (this.cachedTarget == null || !this.cachedTarget.isEntityAlive()
				|| !this.cachedTarget.getUniqueID().equals(uuid)) {
			this.cachedTarget = null;
			for (Entity entity : this.getEntityWorld().getEntitiesWithinAABB(Entity.class,
					this.getEntityBoundingBox().grow(64.0D, 64.0D, 64.0D))) {
				if (entity.getUniqueID().equals(uuid)) {
	...
	public void onUpdate() {
		if (!this.world.isRemote && (this.world.getDifficulty() == EnumDifficulty.PEACEFUL
				|| (this.getOwner() != null && !this.getOwner().isEntityAlive()))) {
			this.setDead();
			return;
		}
		this.motionX = 0;
			...
			if (this.attackTicks > this.attackDelay) {
				if (!this.world.isRemote) {
					if (!this.isObstructedByBoss()) {
						Vec3d diff = new Vec3d(this.posX, this.posY, this.posZ).subtract(new Vec3d(
								this.getTarget().getEntityBoundingBox().minX
										+ (this.getTarget().getEntityBoundingBox().maxX
												- this.getTarget().getEntityBoundingBox().minX) / 2.0D,
								this.getTarget().getEntityBoundingBox().minY
										+ (this.getTarget().getEntityBoundingBox().maxY
												- this.getTarget().getEntityBoundingBox().minY) / 2.0D,
								this.getTarget().getEntityBoundingBox().minZ
										+ (this.getTarget().getEntityBoundingBox().maxZ
												- this.getTarget().getEntityBoundingBox().minZ) / 2.0D))
								.normalize();
	...
	public boolean isObstructedByBoss() {
		...
		List<Entity> hitEntities = this.world.getEntitiesWithinAABBExcludingEntity(this,
				this.getEntityBoundingBox().grow(64, 64, 64));
		double minDist = 0.0D;
	...
	private void spawnFlameParticles() {
		BLParticles.GREEN_FLAME.spawn(this.world, this.posX, this.posY + 0.2F, this.posZ,
				ParticleArgs.get().withMotion((this.world.rand.nextFloat() - 0.5F) / 5.0F,
						(this.world.rand.nextFloat() - 0.5F) / 5.0F, (this.world.rand.nextFloat() - 0.5F) / 5.0F));
	}
