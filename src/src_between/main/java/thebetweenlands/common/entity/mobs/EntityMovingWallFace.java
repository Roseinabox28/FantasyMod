...
public abstract class EntityMovingWallFace extends EntityWallFace {
	public static final IAttribute BLOCK_TRACKING_SEARCH_RANGE = (new RangedAttribute((IAttribute) null,
			"bl.blockTrackingSearchRange", 8.0D, 0.0D, 32.0D)).setDescription("Block Tracking Search Range");
	public static final IAttribute BLOCK_FIX_SEARCH_RANGE = (new RangedAttribute((IAttribute) null,
			"bl.blockFixSearchRange", 8.0D, 0.0D, 32.0D)).setDescription("Block Fix Search Range");
			...
			if (this.ticksExisted % 3 == 0) {
				...
				for (int i = 0; i < 24; i++) {
					...
					if (!state.getBlock().isAir(state, this.world, pos)) {
						...
						this.world.spawnParticle(EnumParticleTypes.BLOCK_DUST, px + rx, py + ry, pz + rz, mx, my, mz,
	...
	protected void fixUnsuitablePosition(int violatedChecks) {
		if (this.ticksExisted % 3 == 0) {
			...
			for (int i = 0; i < 50; i++) {
				...
				BlockPos rndPos = new BlockPos(this.posX + this.world.rand.nextInt(searchRange * 2) - searchRange,
						this.posY + this.height / 2 + this.world.rand.nextInt(searchRange * 2) - searchRange,
						this.posZ + this.world.rand.nextInt(searchRange * 2) - searchRange);
	...
	protected static abstract class AITrackTarget<T extends EntityMovingWallFace> extends EntityAIBase {
		...
		protected boolean isTargetVisibleAndInRange() {
			return this.entity.getEntitySenses().canSee(this.entity.getAttackTarget()) && (!this.stayInRange
					|| this.entity.getAttackTarget().getDistanceSq(this.entity) <= this.maxRangeSq);
		}

		@Override
		public boolean shouldExecute() {
			return this.entity.isEntityAlive() && this.canMove() && !this.entity.isMoving()
					&& this.entity.getAttackTarget() != null && this.entity.getAttackTarget().isEntityAlive()
					&& !this.isTargetVisibleAndInRange();
		}
				...
				if (this.trackingBlocks != null && !this.trackingBlocks.isEmpty() && this.checkCooldown <= 0) {
					...
					for (int i = 0; i < 16; i++) {
						...
						if (!this.stayInRange || target.getDistanceSqToCenter(pos) <= this.maxRangeSq) {
							Vec3d center = new Vec3d(pos.getX() + this.entity.getBlockWidth() / 2.0D,
									pos.getY() + this.entity.getBlockHeight() / 2.0D,
									pos.getZ() + this.entity.getBlockWidth() / 2.0D);
							Vec3d lookDir = target.getPositionVector().add(0, target.getEyeHeight(), 0)
									.subtract(center);

							EnumFacing facing = EnumFacing.getFacingFromVector((float) lookDir.x, (float) lookDir.y,
									(float) lookDir.z);

							if (this.canSeeFrom(pos, facing, target)
									&& this.entity.checkAnchorAt(center, lookDir, AnchorChecks.ALL) == 0) {
								this.entity.moveHelper.setMoveTo(center.x, center.y, center.z, 1);
								this.entity.lookHelper.setLookDirection(facing.getXOffset(), facing.getYOffset(),
										facing.getZOffset());
								break;
							} else {
								for (EnumFacing otherFacing : EnumFacing.HORIZONTALS) {
									if (otherFacing != facing) {
										...
										if (this.canSeeFrom(pos, otherFacing, target)
												&& this.entity.checkAnchorAt(center, lookDir, AnchorChecks.ALL) == 0) {
											this.entity.moveHelper.setMoveTo(center.x, center.y, center.z, 1);
											this.entity.lookHelper.setLookDirection(otherFacing.getXOffset(),
													otherFacing.getYOffset(), otherFacing.getZOffset());
											break;
										}
									}
								}
		...
		protected boolean canSeeFrom(BlockPos pos, EnumFacing facing, Entity entity) {
			return this.entity.world.rayTraceBlocks(
					new Vec3d(
							pos.getX() + this.entity.getBlockWidth() / 2.0D
									+ facing.getXOffset() * (this.entity.width / 2 + this.entity.getPeek()),
							pos.getY() + this.entity.getBlockHeight() / 2.0D
									+ facing.getYOffset() * (this.entity.height / 2 + this.entity.getPeek()),
							pos.getZ() + this.entity.getBlockWidth() / 2.0D
									+ facing.getZOffset() * (this.entity.width / 2 + this.entity.getPeek())),
		...
		public boolean shouldContinueExecuting() {
			return !this.entity.isMoving() && this.entity.getAttackTarget() != null
					&& this.entity.getAttackTarget().isEntityAlive() && !this.isTargetVisibleAndInRange();
		}
	}
	...
	protected static abstract class AIWander<T extends EntityMovingWallFace> extends EntityAIBase {
		...
		@Override
		public boolean shouldExecute() {
			return this.canMove() && !this.entity.isMoving() && this.entity.getIdleTime() < 100
		...
		public void updateTask() {
			...
			if (this.trackingBlocks != null && !this.trackingBlocks.isEmpty() && this.checkCooldown <= 0) {
				...
				for (int i = 0; i < 16; i++) {
					...
					if (this.entity.getDistanceSqToCenter(pos) <= this.rangeSq
							&& this.entity.isWithinHomeDistanceFromPosition(pos)) {
						Vec3d center = new Vec3d(pos.getX() + this.entity.getBlockWidth() / 2.0D,
								pos.getY() + this.entity.getBlockHeight() / 2.0D,
								pos.getZ() + this.entity.getBlockWidth() / 2.0D);
	...
	protected static class AIAttackMelee extends EntityAIAttackMelee {
		...
		protected boolean isInReach(EntityLivingBase enemy, double reach) {
			double dist = Math.max(0, RingOfDispersionEntityCapability
					.calculateAABBDistance(this.attacker.getEntityBoundingBox(), enemy.getEntityBoundingBox()));
			return dist <= reach;
		}
