...
public class EntityFortressBossTeleporter extends Entity implements IEntityScreenShake {
	protected static final DataParameter<Integer> TARGET_ID = EntityDataManager
			.<Integer>createKey(EntityFortressBossTeleporter.class, DataSerializers.VARINT);
	...
	public void onUpdate() {
		...
		if (!this.world.isRemote) {
			if (this.world.getDifficulty() != EnumDifficulty.PEACEFUL) {
				if (this.target == null) {
					AxisAlignedBB checkAABB = new AxisAlignedBB(this.posX - radius, this.posY - radius,
							this.posZ - radius, this.posX + radius, this.posY + radius, this.posZ + radius);
					List<EntityPlayer> players = this.world.getEntitiesWithinAABB(EntityPlayer.class, checkAABB);
					EntityPlayer closestPlayer = null;
					for (EntityPlayer player : players) {
						if ((closestPlayer == null || player.getDistance(this) < closestPlayer.getDistance(this))
								&& player.getDistance(this) < radius && player.canEntityBeSeen(this)) {
							Vec3d playerLook = player.getLook(1.0F).normalize();
							Vec3d vecDiff = new Vec3d(this.posX - player.posX, this.getEntityBoundingBox().minY
					...
					if (this.target.getDistance(this) > radius) {
						...
						Vec3d vecDiff = new Vec3d(this.posX - this.target.posX,
								this.getEntityBoundingBox().minY + (double) (this.height / 2.0F)
										- (this.target.posY + (double) this.target.getEyeHeight()),
			...
			if (target instanceof EntityPlayer) {
				if (this.target == null) {
					for (int i = 0; i < 60; i++) {
						this.spawnSmokeParticle(this.posX, this.posY + this.height / 2.0D, this.posZ,
								(this.world.rand.nextFloat() - 0.5F) / 2.5F,
								(this.world.rand.nextFloat() - 0.5F) / 2.5F,
								(this.world.rand.nextFloat() - 0.5F) / 2.5F);
					}
				}
		...
		if (this.target != null) {
			...
			if (!this.world.isRemote && this.teleportTicks > this.maxTeleportTicks) {
				...
				if (this.target instanceof EntityPlayerMP) {
					...
					player.connection.setPlayerLocation(this.teleportDestination.x, this.teleportDestination.y,
							this.teleportDestination.z, player.rotationYaw, player.rotationPitch);
				} else {
					this.target.dismountRidingEntity();
					this.target.setLocationAndAngles(this.teleportDestination.x, this.teleportDestination.y,
							this.teleportDestination.z, this.target.rotationYaw, this.target.rotationPitch);
				}
				if (!this.spawnedBoss) {
					this.spawnBoss();
				...
				this.spawnSmokeParticle((float) this.posX - this.width / 2.0F + rx, (float) this.posY + ry,
						(float) this.posZ - this.width / 2.0F + rz, (rx - 0.5D) / len * 0.2D, (ry - 0.5D) / len * 0.2D,
						(rz - 0.5D) / len * 0.2D);
			}
		} else {
			this.teleportTicks = 0;
			AxisAlignedBB checkAABB = new AxisAlignedBB(this.posX - lookRadius, this.posY - lookRadius,
					this.posZ - lookRadius, this.posX + lookRadius, this.posY + lookRadius, this.posZ + lookRadius);
			List<EntityPlayer> players = this.world.getEntitiesWithinAABB(EntityPlayer.class, checkAABB);
			EntityPlayer closestPlayer = null;
			for (EntityPlayer player : players) {
				if ((closestPlayer == null || player.getDistance(this) < closestPlayer.getDistance(this))
						&& player.getDistance(this) < lookRadius && player.canEntityBeSeen(this)) {
					closestPlayer = player;
				}
			}
			if (closestPlayer != null) {
	...
	public void faceEntity(Entity target) {
		...
		if (target instanceof EntityLivingBase) {
			...
			dy = (target.getEntityBoundingBox().minY + target.getEntityBoundingBox().maxY) / 2.0D
	...
	protected void spawnBoss() {
		...
		boss.setPosition(this.bossSpawnPosition.getX() + 0.5D, this.bossSpawnPosition.getY() + 0.5D,
				this.bossSpawnPosition.getZ() + 0.5D);
