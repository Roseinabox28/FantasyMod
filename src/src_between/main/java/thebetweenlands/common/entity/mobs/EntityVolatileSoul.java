...
public class EntityVolatileSoul extends Entity implements IProjectile, IEntityBL {
	...
	protected static final DataParameter<Optional<UUID>> OWNER_UUID_DW = EntityDataManager
			.createKey(EntityVolatileSoul.class, DataSerializers.OPTIONAL_UNIQUE_ID);
	...
	protected void onImpact(RayTraceResult target) {
		if (target.entityHit != null && target.entityHit instanceof EntityLivingBase
				&& target.entityHit instanceof EntityWight == false
				&& target.entityHit instanceof EntitySwampHag == false) {
			if (!this.world.isRemote) {
				if (target.entityHit instanceof EntityPlayer
						&& ((EntityPlayer) target.entityHit).isActiveItemStackBlocking()
						&& ((EntityPlayer) target.entityHit).getItemInUseCount() <= 15) {
					this.motionX *= -6;
					this.motionY *= -6;
					this.motionZ *= -6;
					this.strikes++;
				...
				if (!this.isDead && target.entityHit instanceof EntityPlayer
						&& (target.entityHit.isDead || ((EntityLivingBase) target.entityHit).getHealth() <= 0.0F)) {
					target.entityHit.setDead();
					/*
					 * EntityWight wight = new EntityWight(this.world);
					 * wight.setLocationAndAngles(target.entityHit.posX, target.entityHit.posY +
					 * 0.05D, target.entityHit.posZ, target.entityHit.rotationYaw,
					 * target.entityHit.rotationPitch);
	...
	public void onUpdate() {
		if (!this.world.isRemote && (this.world.getDifficulty() == EnumDifficulty.PEACEFUL || this.getOwner() == null
		...
		if (!this.world.isRemote) {
			if ((this.getOwner() == null || !this.getOwner().isEntityAlive()
					|| this.getOwner() instanceof EntityWight == false || !((EntityWight) this.getOwner())
							.isVolatile()) /* || this.target instanceof EntityFortressBoss */)
				this.setDead();
		}
		...
		if (!this.isDead) {
			...
			if (this.target == null || this.target.isDead) {
				List<Entity> targetList = this.world.getEntitiesWithinAABB(EntityLivingBase.class,
			...
			if (this.target != null && this.ticksInAir >= 10) {
				double dx = this.target.getEntityBoundingBox().minX
						+ (this.target.getEntityBoundingBox().maxX - this.target.getEntityBoundingBox().minX) / 2.0D
						- this.posX;
				double dy = this.target.getEntityBoundingBox().minY
						+ (this.target.getEntityBoundingBox().maxY - this.target.getEntityBoundingBox().minY) / 2.0D
						- this.posY;
				double dz = this.target.getEntityBoundingBox().minZ
						+ (this.target.getEntityBoundingBox().maxZ - this.target.getEntityBoundingBox().minZ) / 2.0D
						- this.posZ;
			...
			List<Entity> hitEntities = this.world.getEntitiesWithinAABBExcludingEntity(this,
					this.getEntityBoundingBox().grow(this.motionX, this.motionY, this.motionZ).grow(2.0D, 2.0D, 2.0D));
			double minDist = 0.0D;
