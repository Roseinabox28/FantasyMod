...
public class EntityDreadfulMummy extends EntityMob
		implements IEntityBL, IBLBoss, IEntityScreenShake, IEntityCameraOffset, IEntityMusic {
	...
	public static final IAttribute SPAWN_LENGTH_ATTRIB = (new RangedAttribute(null, "bl.spawnLength", 180.0D, 0.0D,
			Integer.MAX_VALUE)).setDescription("Spawning Length");
	public static final IAttribute SPAWN_OFFSET_ATTRIB = (new RangedAttribute(null, "bl.spawnOffset", 3.0D,
			-Integer.MAX_VALUE, Integer.MAX_VALUE)).setDescription("Spawning Y Offset");

	private static final DataParameter<Integer> SPAWNING_STATE_DW = EntityDataManager
			.<Integer>createKey(EntityDreadfulMummy.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> PREY = EntityDataManager.<Integer>createKey(EntityDreadfulMummy.class,
			DataSerializers.VARINT);
	private static final DataParameter<Float> Y_OFFSET = EntityDataManager.<Float>createKey(EntityDreadfulMummy.class,
			DataSerializers.FLOAT);
	private static final DataParameter<Optional<UUID>> BOSSINFO_ID = EntityDataManager
			.createKey(EntityDreadfulMummy.class, DataSerializers.OPTIONAL_UNIQUE_ID);
	...
	/*
	 * @Override public String pageName() { return null; }
	 */
	@Override
	...
	public float getSpawningProgress(float delta) {
		...
		return 1.0F / getSpawningLength()
				* (getSpawningState() + (getSpawningState() == getSpawningLength() ? 0 : delta));
	}
	...
	public void onUpdate() {
		...
		if (getEntityWorld().isRemote) {
			if (getSpawningProgress() < 1.0F) {
				...
				if ((getSpawningState() - breakPoint / 2 - 1) % breakPoint == 0) {
					...
					for (int i = 0, amount = rand.nextInt(20) + 15; i < amount; i++) {
						...
						getEntityWorld().spawnParticle(EnumParticleTypes.BLOCK_DUST, px + ox, py, pz + oz, motionX,
								motionY, motionZ, Block.getStateId(state));
					}
				}
			...
			if (getSpawningProgress() < 1.0F) {
				...
				if (getSpawningState() == 0) {
					getEntityWorld().playSound(null, getPosition(), SoundRegistry.DREADFUL_PEAT_MUMMY_EMERGE,
				...
				if ((getSpawningState() - breakPoint / 2 - 1) % breakPoint == 0) {
					...
					playSound(blockState.getBlock().getSoundType(blockState, this.world, pos, this).getBreakSound(),
							this.rand.nextFloat() * 0.3F + 0.3F, this.rand.nextFloat() * 0.15F + 0.7F);
				}

				if (getAttackTarget() != null)
					faceEntity(getAttackTarget(), 360, 360);
				if (getSpawningState() == getSpawningLength() - 1) {
					setPosition(posX, posY, posZ);
				}
		...
		if (!getEntityWorld().isRemote && isEntityAlive() && this.getSpawningProgress() >= 1) {
			if (getAttackTarget() != null) {
				...
				List<EntityPeatMummy> peatMummies = getEntityWorld().getEntitiesWithinAABB(EntityPeatMummy.class,
						checkAABB);
				int mummies = 0;
	...
	private void spawnMummy() {
		...
		if (mummy.getEntityWorld().checkNoEntityCollision(mummy.getEntityBoundingBox())
				&& mummy.getEntityWorld().getCollisionBoxes(mummy, mummy.getEntityBoundingBox()).isEmpty()) {
			...
			getEntityWorld().playSound(null, getPosition(), SoundRegistry.DREADFUL_PEAT_MUMMY_SCREAM,
					SoundCategory.HOSTILE, 1F, 1.0F);
			TheBetweenlands.networkWrapper.sendToAllAround(new MessageSummonPeatMummyParticles(mummy),
					new TargetPoint(this.dimension, mummy.posX, mummy.posY, mummy.posZ, 64));
		} else {
	...
	private void spawnSludge() {
		...
		getEntityWorld().playSound(null, getPosition(), SoundRegistry.DREADFUL_PEAT_MUMMY_RETCH, SoundCategory.HOSTILE,
				1F, 0.7F + rand.nextFloat() * 0.6F);
	...
	public boolean attackEntityAsMob(Entity target) {
		...
		if (attacked && rand.nextInt(6) == 0 && target != currentEatPrey && target instanceof EntityLivingBase
		...
		if (attacked)
			getEntityWorld().playSound(null, getPosition(), SoundRegistry.DREADFUL_PEAT_MUMMY_SWIPE,
					SoundCategory.HOSTILE, 1F, 0.7F + rand.nextFloat() * 0.6F);
		return attacked;
	}

	private void updateEatPrey() {
		double direction = Math.toRadians(renderYawOffset);
		currentEatPrey.setPositionAndRotation(posX - Math.sin(direction) * 1.7, posY + 0.25,
				posZ + Math.cos(direction) * 1.7, (float) (Math.toDegrees(direction) + 180), 0);
		currentEatPrey.rotationYawHead = currentEatPrey.prevRotationYawHead = currentEatPrey.rotationYaw = currentEatPrey.prevRotationYaw = ((float) (Math
				.toDegrees(direction) + 180));
		currentEatPrey.fallDistance = 0;
		if (ticksExisted % 10 == 0) {
			if (!getEntityWorld().isRemote) {
				currentEatPrey.attackEntityFrom(DamageSource.causeMobDamage(this), 3);
				getEntityWorld().playSound(null, getPosition(), SoundRegistry.DREADFUL_PEAT_MUMMY_BITE,
						SoundCategory.HOSTILE, 1F, 0.7F + rand.nextFloat() * 0.6F);
			}
		}
	...
	protected void onDeathUpdate() {
		...
		if (deathTicks == 0) {
			if (!getEntityWorld().isRemote) {
				getEntityWorld().playSound(null, getPosition(), SoundRegistry.DREADFUL_PEAT_MUMMY_DEATH,
		...
		if (!getEntityWorld().isRemote) {
			...
			if (deathTicks > 40 && deathTicks % 5 == 0) {
				...
				while (xp > 0) {
					...
					xp -= dropXP;
					getEntityWorld()
							.spawnEntity(new EntityXPOrb(getEntityWorld(), posX, posY + height / 2.0D, posZ, dropXP));
				}
			}
			...
			if (getEntityWorld().isRemote && deathTicks % 5 == 0) {
				for (int xo = -1; xo <= 1; xo++) {
					for (int zo = -1; zo <= 1; zo++) {
						int x = MathHelper.floor(posX) + xo, y = MathHelper.floor(posY - getYOffset() - 0.1D),
								z = MathHelper.floor(posZ) + zo;
						...
						if (block != Blocks.AIR) {
							...
							double pz = posZ + rand.nextDouble() - 0.5F;
							getEntityWorld().playSound(null, getPosition(),
									block.getSoundType(state, getEntityWorld(), getPosition().down(), null)
											.getBreakSound(),
									SoundCategory.BLOCKS, rand.nextFloat() * 0.3F + 0.3F,
									rand.nextFloat() * 0.15F + 0.7F);
							for (int i = 0, amount = rand.nextInt(20) + 10; i < amount; i++) {
								...
								double motionY = rand.nextDouble() * 0.25 + 0.1;
								double motionZ = rand.nextDouble() * 0.2 - 0.1;
								getEntityWorld().spawnParticle(EnumParticleTypes.BLOCK_DUST, px + ox + xo, py,
										pz + oz + zo, motionX, motionY, motionZ, Block.getStateId(state));
							}
						}
					}
				}
	...
	public boolean applyOffset(Entity view, float partialTicks) {
		if (currentEatPrey == view) {
			double direction = Math
					.toRadians(prevRenderYawOffset + (renderYawOffset - prevRenderYawOffset) * partialTicks);
