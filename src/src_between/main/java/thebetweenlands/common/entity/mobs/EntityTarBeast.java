...
public class EntityTarBeast extends EntityMob implements IEntityBL {
	...
	public static final IAttribute SHED_COOLDOWN_ATTRIB = (new RangedAttribute(null, "bl.shedCooldown", 70.0D, 10.0D,
			Integer.MAX_VALUE)).setDescription("Shed Cooldown");
	public static final IAttribute SHED_SPEED_ATTRIB = (new RangedAttribute(null, "bl.shedSpeed", 10.0D, 0.0D,
			Integer.MAX_VALUE)).setDescription("Shedding Speed");

	public static final IAttribute SUCK_COOLDOWN_ATTRIB = (new RangedAttribute(null, "bl.suckCooldown", 400.0D, 0.0D,
			Integer.MAX_VALUE)).setDescription("Sucking Cooldown");
	public static final IAttribute SUCK_PREPARATION_SPEED_ATTRIB = (new RangedAttribute(null, "bl.suckPreparationSpeed",
			40.0D, 0.0D, Integer.MAX_VALUE)).setDescription("Sucking Preparation Speed");
	public static final IAttribute SUCK_LENGTH_ATTRIB = (new RangedAttribute(null, "bl.suckLength", 130.0D, 0.0D,
			Integer.MAX_VALUE)).setDescription("Sucking Length");
	...
	protected static final DataParameter<Byte> SUCKING_STATE_DW = EntityDataManager.createKey(EntityTarBeast.class,
			DataSerializers.BYTE);
	protected static final DataParameter<Boolean> SHEDDING_STATE_DW = EntityDataManager.createKey(EntityTarBeast.class,
	...
	public boolean getCanSpawnHere() {
		...
		if (isDifficultyValid) {
			...
			boolean isInTar = this.world.getBlockState(pos.setPos(bx, by, bz)).getBlock() == BlockRegistry.TAR
			...
			return this.world.checkNoEntityCollision(this.getEntityBoundingBox())
					&& this.world.getCollisionBoxes(this, this.getEntityBoundingBox()).isEmpty() && isInTar;
		}
		return false;
	}
	...
	public void onUpdate() {
		...
		if (world.isRemote) {
			...
			if (this.sheddingProgress > this.getSheddingSpeed()) {
				...
				for (int i = 0; i < 200; i++) {
					...
					vec = vec.normalize();
					BLParticles.SPLASH_TAR.spawn(this.world, this.posX + rx + 0.25F, this.posY + ry,
							this.posZ + rz + 0.25F,
							ParticleArgs.get().withMotion(vec.x * 0.5F, vec.y * 0.5F, vec.z * 0.5F));
				}
			...
			if (this.isSucking()) {
				for (int i = 0; i < 5; i++) {
					...
					Vec3d vec = new Vec3d(rx, ry, rz);
					vec = vec.normalize();
					this.world.spawnParticle(EnumParticleTypes.SMOKE_LARGE, this.posX + rx + 0.25F, this.posY + ry,
							this.posZ + rz + 0.25F, -vec.x * 0.5F, -vec.y * 0.5F, -vec.z * 0.5F);
				}
			}
		}
		...
		if (!world.isRemote) {
			...
			if (!this.isSucking() && !this.isPreparing()) {
				if (this.shedCooldown == 0 && this.getAttackTarget() != null
						&& this.getAttackTarget().getDistance(this) < 6.0D
						&& this.canEntityBeSeen(this.getAttackTarget())) {
					this.setShedding(true);
					this.shedCooldown = this.getSheddingCooldown()
							+ this.world.rand.nextInt(this.getSheddingCooldown() / 2);
				}
				...
				if (this.sheddingProgress > this.getSheddingSpeed()) {
					...
					if (this.getAttackTarget() != null) {
						List<EntityLivingBase> affectedEntities = (List<EntityLivingBase>) this.world
						...
						for (EntityLivingBase e : affectedEntities) {
							if (e == this || e.getDistance(this) > 6.0F || !e.canEntityBeSeen(this)
									|| e instanceof EntityTarBeast)
								continue;
							...
							float dmg = (float) (this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE)
									.getAttributeValue() / dst * 7.0F);
							e.attackEntityFrom(DamageSource.causeMobDamage(this), dmg);
							e.addPotionEffect(
									new PotionEffect(MobEffects.SLOWNESS, (int) (20 + (1.0F - dst / 6.0F) * 150), 1));
						}
					}
				}
			...
			if (!this.isShedding()) {
				if (this.suckingCooldown == 0 && this.getAttackTarget() != null
						&& this.getAttackTarget().getDistance(this) <= 10.0D
						&& this.canEntityBeSeen(this.getAttackTarget())) {
					this.setPreparing();
				}
				...
				if (this.isPreparing()) {
					...
					if (this.suckingPreparation > this.getEntityAttribute(SUCK_PREPARATION_SPEED_ATTRIB)
							.getAttributeValue()) {
						this.suckingPreparation = 0;

						this.setSucking(true);
						this.suckingCooldown = this.getSuckingCooldown()
								+ this.world.rand.nextInt(this.getSuckingCooldown() / 2);
				...
				if (this.isSucking()) {
					...
					List<Entity> affectedEntities = (List<Entity>) this.world.getEntitiesWithinAABB(Entity.class,
							this.getEntityBoundingBox().grow(10.0F, 10.0F, 10.0F));
					for (Entity e : affectedEntities) {
						if (e == this || e.getDistance(this) > 10.0F || !this.canEntityBeSeen(e)
								|| e instanceof EntityTarBeast)
							continue;
	...
	public void renderParticles(World world, double x, double y, double z, Random rand) {
		for (int count = 0; count < 3; ++count) {
			...
			BLParticles.SPLASH_TAR.spawn(world, x, y + rand.nextDouble() * 1.9D, z,
					ParticleArgs.get().withMotion(velX * 0.15D, velY * 0.1D, velZ * 0.15D));
