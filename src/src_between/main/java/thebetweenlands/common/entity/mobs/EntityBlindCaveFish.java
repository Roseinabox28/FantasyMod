...
public class EntityBlindCaveFish extends EntityCreature implements IEntityBL {
	...
	public boolean getCanSpawnHere() {
		return this.posY <= WorldProviderBetweenlands.CAVE_WATER_HEIGHT && world
	...
	public boolean isInWater() {
		return world.getBlockState(new BlockPos(MathHelper.floor(posX), MathHelper.floor(posY), MathHelper.floor(posZ)))
				.getBlock() == BlockRegistry.SWAMP_WATER;
	}
		...
		if (this.world.isRemote) {
			...
			if (inWater) {
				...
				if (world.getTotalWorldTime() % 5 == 0)
					world.playSound((EntityPlayer) null, posX, posY, posZ, SoundEvents.ENTITY_GUARDIAN_FLOP,
		...
		public void onUpdateMoveHelper() {
			if (action == EntityMoveHelper.Action.MOVE_TO && !fish.getNavigator().noPath()) {
				...
				float f1 = (float) (speed
						* fish.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).getAttributeValue());
				...
				fish.getLookHelper().setLookPosition(d10 + (d7 - d10) * 0.125D, d11 + (d8 - d11) * 0.125D,
