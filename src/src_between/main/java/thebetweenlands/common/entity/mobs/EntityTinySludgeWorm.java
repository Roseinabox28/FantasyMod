...
public class EntityTinySludgeWorm extends EntitySludgeWorm {
	...
	public EntityTinySludgeWorm(World world) {
		...
		isImmuneToFire = true;
		this.parts = new MultiPartEntityPart[] { new MultiPartEntityPart(this, "part1", 0.1875F, 0.1875F),
	...
	public void onCollideWithPlayer(EntityPlayer player) {
		if (!getEntityWorld().isRemote) {
			...
			if (this.isSquashed) {
				...
				this.world.playSound(null, this.posX, this.posY, this.posZ, getJumpedOnSound(), SoundCategory.NEUTRAL,
						1.0F, 0.5F);
				this.world.playSound(null, this.posX, this.posY, this.posZ, getDeathSound(), SoundCategory.NEUTRAL,
						1.0F, 0.5F);
	...
	public void handleStatusUpdate(byte id) {
		...
		if (id == EVENT_SQUASHED) {
			for (int i = 0; i < 100; i++) {
				...
				vec = vec.normalize();
				BLParticles.SPLASH_TAR
						.spawn(getEntityWorld(), this.posX + rx + 0.1F, this.posY + ry + 0.1F, this.posZ + rz + 0.1F,
