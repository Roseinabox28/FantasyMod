...
public class EntityAshSprite extends EntityMob implements IEntityBL {
	protected static final DataParameter<Byte> ASH_SPRITE_FLAGS = EntityDataManager
			.<Byte>createKey(EntityAshSprite.class, DataSerializers.BYTE);
	@Nullable
	...
	public void spawnParticles(World worldIn, BlockPos pos, Random rand) {
		for (int i = 0; i < 4; i++) {
			worldIn.spawnParticle(EnumParticleTypes.SUSPENDED_DEPTH,
					pos.getX() + 0.5D + (rand.nextBoolean() ? -0.5F : 0.5F) * Math.pow(rand.nextFloat(), 1F),
					pos.getY() + 0.5D + rand.nextFloat() - 0.5F,
					pos.getZ() + 0.5D + (rand.nextBoolean() ? -0.5F : 0.5F) * Math.pow(rand.nextFloat(), 1F), 0, 0.2D,
					0);
		}
	}
	...
	public void readEntityFromNBT(NBTTagCompound compound) {
		...
		if (compound.hasKey("BoundX"))
			boundOrigin = new BlockPos(compound.getInteger("BoundX"), compound.getInteger("BoundY"),
	...
	class AIChargeAttack extends EntityAIBase {
		...
		public boolean shouldExecute() {
			if (ashSprite.getAttackTarget() != null && !ashSprite.getMoveHelper().isUpdating()
					&& ashSprite.rand.nextInt(7) == 0)
		...
		public boolean shouldContinueExecuting() {
			return ashSprite.getMoveHelper().isUpdating() && ashSprite.isCharging()
					&& ashSprite.getAttackTarget() != null && ashSprite.getAttackTarget().isEntityAlive();
		}
			...
			if (action == EntityMoveHelper.Action.MOVE_TO) {
				...
				if (d3 < ashSprite.getEntityBoundingBox().getAverageEdgeLength()) {
					...
					if (ashSprite.getAttackTarget() == null) {
						ashSprite.rotationYaw = -((float) MathHelper.atan2(ashSprite.motionX, ashSprite.motionZ))
								* (180F / (float) Math.PI);
	...
	class AIMoveRandom extends EntityAIBase {
		...
		public void updateTask() {
			...
			for (int i = 0; i < 3; ++i) {
				BlockPos blockpos1 = blockpos.add(ashSprite.rand.nextInt(15) - 7, ashSprite.rand.nextInt(11) - 5,
						ashSprite.rand.nextInt(15) - 7);

				if (ashSprite.world.isAirBlock(blockpos1)) {
					ashSprite.moveHelper.setMoveTo((double) blockpos1.getX() + 0.5D, (double) blockpos1.getY() + 0.5D,
							(double) blockpos1.getZ() + 0.5D, 0.25D);

					if (ashSprite.getAttackTarget() == null) {
						ashSprite.getLookHelper().setLookPosition((double) blockpos1.getX() + 0.5D,
