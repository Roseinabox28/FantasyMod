...
public class EntityFortressBoss extends EntityMob implements IEntityBL, IBLBoss, IEntityMusic {
	...
	protected static final DataParameter<Integer> SHIELD_STATE = EntityDataManager
			.<Integer>createKey(EntityFortressBoss.class, DataSerializers.VARINT);
	protected static final DataParameter<Float> SHIELD_ROTATION = EntityDataManager
			.<Float>createKey(EntityFortressBoss.class, DataSerializers.FLOAT);
	protected static final DataParameter<Boolean> FLOATING_STATE = EntityDataManager
			.<Boolean>createKey(EntityFortressBoss.class, DataSerializers.BOOLEAN);
	protected static final DataParameter<Boolean> GROUND_ATTACK_STATE = EntityDataManager
			.<Boolean>createKey(EntityFortressBoss.class, DataSerializers.BOOLEAN);
	protected static final DataParameter<BlockPos> ANCHOR = EntityDataManager
			.<BlockPos>createKey(EntityFortressBoss.class, DataSerializers.BLOCK_POS);
	protected static final DataParameter<Float> ANCHOR_RADIUS = EntityDataManager
			.<Float>createKey(EntityFortressBoss.class, DataSerializers.FLOAT);
	private static final DataParameter<Optional<UUID>> BOSSINFO_ID = EntityDataManager
			.createKey(EntityFortressBoss.class, DataSerializers.OPTIONAL_UNIQUE_ID);
	...
	public static final double[][] ICOSAHEDRON_VERTICES = new double[][] { { -ICOSAHEDRON_X, 0.0, ICOSAHEDRON_Z },
			{ ICOSAHEDRON_X, 0.0, ICOSAHEDRON_Z }, { -ICOSAHEDRON_X, 0.0, -ICOSAHEDRON_Z },
			{ ICOSAHEDRON_X, 0.0, -ICOSAHEDRON_Z }, { 0.0, ICOSAHEDRON_Z, ICOSAHEDRON_X },
			{ 0.0, ICOSAHEDRON_Z, -ICOSAHEDRON_X }, { 0.0, -ICOSAHEDRON_Z, ICOSAHEDRON_X },
			{ 0.0, -ICOSAHEDRON_Z, -ICOSAHEDRON_X }, { ICOSAHEDRON_Z, ICOSAHEDRON_X, 0.0 },
			{ -ICOSAHEDRON_Z, ICOSAHEDRON_X, 0.0 }, { ICOSAHEDRON_Z, -ICOSAHEDRON_X, 0.0 },
			{ -ICOSAHEDRON_Z, -ICOSAHEDRON_X, 0.0 } };
	public static final int[][] ICOSAHEDRON_INDICES = new int[][] { { 0, 4, 1 }, { 0, 9, 4 }, { 9, 5, 4 }, { 4, 5, 8 },
			{ 4, 8, 1 }, { 8, 10, 1 }, { 8, 3, 10 }, { 5, 3, 8 }, { 5, 2, 3 }, { 2, 7, 3 }, { 7, 10, 3 }, { 7, 6, 10 },
			{ 7, 11, 6 }, { 11, 0, 6 }, { 0, 1, 6 }, { 6, 1, 10 }, { 9, 0, 11 }, { 9, 11, 2 }, { 9, 2, 5 },
			{ 7, 2, 11 } };
	...
	private float shieldRotationYaw, shieldRotationPitch, shieldRotationRoll, lastShieldRotationYaw,
			lastShieldRotationPitch, lastShieldRotationRoll, shieldExplosion, lastShieldExplosion;
	...
	public EntityFortressBoss(World world) {
		...
		this.coreBoundingBox = new AxisAlignedBB(-coreWidth / 2.0F, 0F + height / 4.0F, -coreWidth / 2.0F,
				coreWidth / 2.0F, coreHeight + height / 4.0F, coreWidth / 2.0F);
		for (int i = 0; i < 20; i++) {
	...
	public int rayTraceShield(Vec3d pos, Vec3d ray, boolean back) {
		...
		this.rotationMatrix.setRotations((float) Math.toRadians(-this.shieldRotationPitch),
				(float) Math.toRadians(-this.shieldRotationYaw), (float) Math.toRadians(-this.shieldRotationRoll));

		Vec3d centerPos = new Vec3d(this.posX + SHIELD_OFFSET_X, this.posY + SHIELD_OFFSET_Y,
		...
		for (int i = 0; i <= 19; i++) {
			...
			Vec3d vert1 = new Vec3d(v1[0] + v1Normalized.x * vertexExplode, v1[1] + v1Normalized.y * vertexExplode,
					v1[2] + v1Normalized.z * vertexExplode);
			Vec3d vert2 = new Vec3d(v2[0] + v2Normalized.x * vertexExplode, v2[1] + v2Normalized.y * vertexExplode,
					v2[2] + v2Normalized.z * vertexExplode);
			Vec3d vert3 = new Vec3d(v3[0] + v3Normalized.x * vertexExplode, v3[1] + v3Normalized.y * vertexExplode,
					v3[2] + v3Normalized.z * vertexExplode);
			...
			if (rayTraceTriangle(pos, ray, vert1, vert2, vert3)
					&& (back || normal.normalize().dotProduct(ray.normalize()) < Math.cos(Math.toRadians(90)))) {
				...
				if (shield == -1
						|| (Math.sqrt(dx * dx + dy * dy + dz * dz) < Math.sqrt(pdx * pdx + pdy * pdy + pdz * pdz))) {
					shield = i;
					centroidX = centerX;
					centroidY = centerY;
					centroidZ = centerZ;
		...
		if (source instanceof EntityDamageSource) {
			...
			if (attackingEntity == null) {
				...
				if (attackingEntity instanceof EntityLivingBase) {
					...
					ray = new Vec3d(attackingEntity.motionX, attackingEntity.motionY, attackingEntity.motionZ)
							.normalize();
				}
				ray = ray.scale(64.0D);

				pos = new Vec3d(attackingEntity.posX, attackingEntity.posY + attackingEntity.getEyeHeight(),
						attackingEntity.posZ);

				if (this.hasShield()
						&& (attackingEntity instanceof EntityPlayer == false
								|| !((EntityPlayer) attackingEntity).capabilities.isCreativeMode)
						|| attackingEntity.isSneaking()) {
					int shieldHit = this.rayTraceShield(pos, ray, false);

					if (shieldHit >= 0) {
						if (!this.world.isRemote && attackingEntity.isSneaking()
								&& ((EntityPlayer) attackingEntity).capabilities.isCreativeMode) {
			...
			if (isDeflected) {
				if (!this.world.isRemote) {
					...
					if (damaged) {
						this.world.playSound(null, this.posX, this.posY, this.posZ, SoundRegistry.FORTRESS_BOSS_NOPE,
				...
				if (pos != null && ray != null && this.coreBoundingBox.offset(this.posX, this.posY, this.posZ)
						.calculateIntercept(pos, ray.add(pos.x, pos.y, pos.z)) != null) {
	...
	public void onUpdate() {
		...
		Iterator<Object2IntMap.Entry<Entity>> cooldownIt = this.deflectionDamageCooldowns.object2IntEntrySet()
				.iterator();
		...
		if (this.isEntityAlive()) {
			...
			if (!this.world.isRemote) {
				...
				if (this.isFloating()
						&& (this.getDistance(anchorCenter.x, this.posY, anchorCenter.z) > this.anchorRadius
								|| Math.abs(this.posY - anchorCenter.y) > this.anchorRadius)) {
					this.world.playSound(null, this.posX, this.posY, this.posZ, SoundRegistry.FORTRESS_BOSS_TELEPORT,
				...
				if (this.ticksExisted % 12 == 0 && this.getHealth() < this.getMaxHealth()) {
					List<EntityLivingBase> currentlyTrackedEntities = this.world
							.getEntitiesWithinAABB(EntityPlayer.class, this.getEntityBoundingBox()
									.grow(this.anchorRadius * 2, this.anchorRadius * 2, this.anchorRadius * 2));
					Iterator<EntityLivingBase> it = currentlyTrackedEntities.iterator();
					while (it.hasNext()) {
						EntityLivingBase living = it.next();
						if (living.getDistance(anchorCenter.x, living.posY, anchorCenter.z) > this.anchorRadius + 4
								|| Math.abs(living.posY - anchorCenter.y) > this.anchorRadius)
							it.remove();
					}
			...
			if (!players.isEmpty()) {
				if (!this.world.isRemote) {
					...
					if (this.isFloating() && this.posY >= anchorCenter.y) {
						...
						List<EntityFortressBossSpawner> spawners = this.world
								.getEntitiesWithinAABB(EntityFortressBossSpawner.class, checkAABB);
							...
							if (this.wightSpawnTicks <= 0) {
								if (this.wightSpawnTicks == 0) {
									...
									if (Math.abs(spawnY - this.posY) < this.anchorRadius) {
										EntityFortressBossSpawner spawner = new EntityFortressBossSpawner(this.world,
												this);
										spawner.setLocationAndAngles(MathHelper.floor(this.posX), spawnY,
												MathHelper.floor(this.posZ), 0, 0);
						...
						List<EntityFortressBossBlockade> blockades = this.world
								.getEntitiesWithinAABB(EntityFortressBossBlockade.class, checkAABB);
						if (blockades.isEmpty()) {
							...
							if (this.blockadeSpawnTicks <= 0) {
								if (this.blockadeSpawnTicks == 0) {
									int spawnY = this.world.getHeight(this.getPosition()).getY();
									if (Math.abs(spawnY - this.posY) < this.anchorRadius) {
										EntityFortressBossBlockade blockade = new EntityFortressBossBlockade(this.world,
												this);
						...
						if (this.teleportTicks <= 0) {
							if (this.teleportTicks == 0) {
								this.world.playSound(null, this.posX, this.posY, this.posZ,
										SoundRegistry.FORTRESS_BOSS_TELEPORT, SoundCategory.HOSTILE, 1.0F, 1.0F);
								this.setLocationAndAngles(
										anchorCenter.x
												+ (this.world.rand.nextFloat() - 0.5F) * 2.0F * (this.anchorRadius - 1),
										anchorCenter.y,
										anchorCenter.z
												+ (this.world.rand.nextFloat() - 0.5F) * 2.0F * (this.anchorRadius - 1),
										0, 0);
						...
						if (this.turretStreak <= 0 && this.getHealth() < this.getMaxHealth() / 2.0F) {
							if (this.turretStreak == 0) {
								...
								if (this.turretStreakTicks % turretFrequency == 0) {
									...
									for (int d = 0; d < 2; d++) {
										Vec3d dir = new Vec3d(Math.sin(angle) * (d == 0 ? 1 : -1), 0,
												Math.cos(angle) * (d == 0 ? 1 : -1));
										dir = dir.normalize().scale(this.anchorRadius);
										EntityFortressBossTurret turret = new EntityFortressBossTurret(this.world,
												this);
										turret.setLocationAndAngles(anchorCenter.x + dir.x, anchorCenter.y + dir.y,
												anchorCenter.z + dir.z, 0, 0);
										turret.setAttackDelay(turretFrequency);
										this.world.spawnEntity(turret);
										this.world.playSound(null, this.posX, this.posY, this.posZ,
												SoundRegistry.FORTRESS_BOSS_SUMMON_PROJECTILES, SoundCategory.HOSTILE,
												0.25F, 0.3F + 0.7F / 300.0F * this.turretStreakTicks);
									}
								}
								if (this.turretStreakTicks >= 300) {
									...
									this.world.playSound(null, this.posX, this.posY, this.posZ,
											SoundRegistry.FORTRESS_BOSS_SUMMON_PROJECTILES, SoundCategory.HOSTILE, 1.0F,
											1.0F);
						...
						if (this.groundAttackTicks <= 0) {
							if (this.groundAttackTicks == 0) {
								...
								for (int i = 0; i < 32; i++) {
									...
									dir = dir.normalize();
									float speed = 0.8F;
									EntityFortressBossProjectile bullet = new EntityFortressBossProjectile(this.world,
											this);
	...
	public void travel(float strafe, float up, float forward) {
		if (this.isFloating()) {
			if (this.isInWater()) {
				...
				if (this.onGround) {
					friction = this.world.getBlockState(new BlockPos(MathHelper.floor(this.posX),
	...
	protected void onDeathUpdate() {
		...
		if (this.deathTicks == 0) {
			if (!this.world.isRemote) {
				final Vec3d anchorCenter = this.getAnchorCenter();
				this.world.playSound(null, anchorCenter.x, anchorCenter.y, anchorCenter.z,
						SoundRegistry.FORTRESS_BOSS_TELEPORT, SoundCategory.HOSTILE, 1.0F, 1.0F);
				this.setPosition(anchorCenter.x, anchorCenter.y, anchorCenter.z);
				List<Entity> trackedEntities = this.world.getEntitiesWithinAABB(EntityWight.class,
						this.getEntityBoundingBox().grow(this.anchorRadius * 2, 512, this.anchorRadius * 2));
				Iterator<Entity> it = trackedEntities.iterator();
				while (it.hasNext()) {
					Entity entity = it.next();
					if (entity.getDistance(anchorCenter.x, entity.posY, anchorCenter.z) > this.anchorRadius
							|| Math.abs(entity.posY - anchorCenter.y) > this.anchorRadius)
						it.remove();
				}
				for (Entity entity : trackedEntities) {
					if (entity instanceof EntityWight || entity instanceof EntityFortressBossSpawner
							|| entity instanceof EntityFortressBossProjectile
							|| entity instanceof EntityFortressBossTurret
							|| entity instanceof EntityFortressBossBlockade) {
						entity.setDead();
					}
				}
			}
			...
			if (this.deathTicks > 100 && this.deathTicks % 5 == 0) {
				...
				while (xp > 0) {
					...
					this.world.spawnEntity(
							new EntityXPOrb(this.world, this.posX, this.posY + this.height / 2.0D, this.posZ, dropXP));
				}
			}
	...
	public void playLivingSound() {
		if (!this.world.isRemote && !this.isSilent()) {
			...
			// MessagePlayEntityIdle(this, SoundRegistry.FORTRESS_BOSS_LIVING,
			// SoundCategory.HOSTILE, 1, 1), new TargetPoint(this.dimension, this.posX,
			// this.posY, this.posZ, 16.0D));
		}
	}
