...
public class EntityBarrishee extends EntityMob implements IEntityScreenShake, IEntityBL {
	private static final DataParameter<Boolean> AMBUSH_SPAWNED = EntityDataManager.createKey(EntityBarrishee.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> SCREAM = EntityDataManager.createKey(EntityBarrishee.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> SCREAM_TIMER = EntityDataManager.createKey(EntityBarrishee.class,
			DataSerializers.VARINT);
	private static final DataParameter<Boolean> SCREAM_BEAM = EntityDataManager.createKey(EntityBarrishee.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> SLAMMING_ANIMATION = EntityDataManager.createKey(EntityBarrishee.class,
			DataSerializers.BOOLEAN);
	...
	public boolean isNotColliding() {
		return !getEntityWorld().containsAnyLiquid(getEntityBoundingBox())
				&& getEntityWorld().getCollisionBoxes(this, getEntityBoundingBox()).isEmpty()
				&& getEntityWorld().checkNoEntityCollision(getEntityBoundingBox(), this);
	}
		...
		if (collisionAABB != null) {
			...
			for (Entity entity : entities) {
				if (entity.canBeCollidedWith() && entity.canBePushed()) {
					...
					if (entityAABB != null) {
						double dx = Math.max(collisionAABB.minX - entityAABB.maxX,
								entityAABB.minX - collisionAABB.maxX);
						double dz = Math.max(collisionAABB.minZ - entityAABB.maxZ,
								entityAABB.minZ - collisionAABB.maxZ);
				...
				for (int sizeZ = minZ; sizeZ <= maxZ; ++sizeZ) {
					for (int sizeY = minY; sizeY <= maxY; ++sizeY) {
						...
						if (state.getBlockHardness(this.world, pos) >= 0
								&& state.getBlock().canEntityDestroy(state, this.world, pos, this)) {
							if (state.getBlock() instanceof BlockLootUrn) {
								spawnAshSpriteMinion(getEntityWorld(), pos, state);
								this.world.destroyBlock(pos, true);
							} else if (!LocationGuarded.isLocationGuarded(this.world, this, pos)
	...
	protected void spawnScreamParticles() {
		...
		float speed = 0.6f;
		Particle particle = BLParticles.SONIC_SCREAM.create(this.world, this.posX,
				this.posY + (getScreamTimer() < 25 ? 0.8 + (getScreamTimer() * 0.0125F)
						: 1.25 - (25 - getScreamTimer()) * 0.025F),
				this.posZ, ParticleArgs.get().withMotion(look.x * speed, look.y * speed, look.z * speed).withScale(10)
						.withData(30, MathHelper.floor(this.ticksExisted * 3.3f)).withColor(1.0f, 0.9f, 0.8f, 1.0f));
	...
	private boolean isLookingAtAttackTarget(Entity entity) {
		...
		Vec3d vec3d1 = new Vec3d(entity.posX - posX,
				entity.getEntityBoundingBox().minY + (double) entity.getEyeHeight() - (posY + (double) getEyeHeight()),
				entity.posZ - posZ);
	...
	/**
	 * Called by barrishee path navigator if a path is obstructed and the barrishee
	 * becomes stuck
	 */
	...
	static class AISonicAttack extends EntityAIBase {
		...
		public boolean shouldExecute() {
			...
			if (target != null) {
				...
				if (distance >= 9.0D && distance <= 144.0D && barrishee.onGround && barrishee.isReadyForSpecialAttack()
						&& barrishee.isLookingAtAttackTarget(target)) {
					return this.cooldown-- < 0;
				}
			}
		...
		public void startExecuting() {
			missileCount = 0;
			shootCount = 0;
			barrishee.getEntityWorld().playSound(null, barrishee.getPosition(), barrishee.getScreamSound(),
					SoundCategory.HOSTILE, 0.75F, 0.5F);
			barrishee.setScreamTimer(0);
		}
			...
			if (target != null) {
				...
				if (barrishee.getScreamTimer() >= 25) {
					...
					if (missileCount % 2 == 0) {
						...
						checkIfBeamHitsAnyone(barrishee.getEntityWorld(),
								new BlockPos(barrishee.posX + (double) MathHelper.cos(f) * d2,
										barrishee.posY + barrishee.getEyeHeight(),
										barrishee.posZ + (double) MathHelper.sin(f) * d2));
					}
				}
		...
		public void checkIfBeamHitsAnyone(World world, BlockPos pos) {
			...
			for (EntityLivingBase entity : list) {
				if (barrishee.attackEntityAsMob(entity)) {
					...
					entity.addVelocity(-MathHelper.sin(barrishee.rotationYaw * 3.141593F / 180.0F) * 0.25D, 0D,
							MathHelper.cos(barrishee.rotationYaw * 3.141593F / 180.0F) * 0.25D);
				}
			}
		}
	}
	...
	static class AISlamAttack extends EntityAIBase {
		...
		public boolean shouldExecute() {
			...
			if (target != null) {
				...
				if (distance >= 9.0D && distance <= 81.0D && barrishee.onGround
						&& barrishee.isReadyForSpecialAttack()) {
					return this.cooldown-- < 0;
				}
			}
		...
		public boolean shouldContinueExecuting() {
			EntityLivingBase target = barrishee.getAttackTarget();
			return target != null && shootCount != -1 && missileCount != -1
					&& barrishee.isLookingAtAttackTarget(target);
		}
			...
			if (target != null) {
				...
				if (barrishee.isLookingAtAttackTarget(target)) {
					...
					if (missileCount % 2 == 0) {
						...
						if (shootCount == 1) {
							barrishee.getEntityWorld().playSound(null, barrishee.getPosition(), SoundRegistry.WALL_SLAM,
						...
						if (block.isNormalCube() && !block.getBlock().hasTileEntity(block)
							...
							shockwaveBlock.setLocationAndAngles(origin.getX() + 0.5D, origin.getY(),
									origin.getZ() + 0.5D, 0.0F, 0.0F);
							shockwaveBlock.setBlock(
									Block.getBlockById(Block.getIdFromBlock(
											barrishee.getEntityWorld().getBlockState(origin).getBlock())),
									barrishee.getEntityWorld().getBlockState(origin).getBlock()
											.getMetaFromState(barrishee.getEntityWorld().getBlockState(origin)));
