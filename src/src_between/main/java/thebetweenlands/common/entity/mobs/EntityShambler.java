...
public class EntityShambler extends EntityMob implements IEntityMultiPart, IEntityBL {
	...
	private static final DataParameter<Boolean> JAWS_OPEN = EntityDataManager.createKey(EntityShambler.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> TONGUE_EXTEND = EntityDataManager.createKey(EntityShambler.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> JAW_ANGLE = EntityDataManager.createKey(EntityShambler.class,
			DataSerializers.VARINT);
	private static final DataParameter<Integer> TONGUE_LENGTH = EntityDataManager.createKey(EntityShambler.class,
			DataSerializers.VARINT);
	...
	protected void initEntityAI() {
		...
		this.targetTasks.addTask(1, new EntityAINearestAttackableTarget<>(this, EntityPlayer.class, 3, true, true, null)
	...
	public void onUpdate() {
		...
		for (MultiPartEntityPart part : tongue_array) {
			...
			part.setPosition(posX + offSetX + ((double) vector.x * getTongueLength() * tongueLength),
					posY + this.getEyeHeight() - 0.32 + offsetY
							+ ((double) vector.y * getTongueLength() * tongueLength),
					posZ + offSetZ + ((double) vector.z * getTongueLength() * tongueLength));
	...
	public void updatePassenger(Entity entity) {
		if (entity instanceof EntityLivingBase) {
			...
			entity.setPosition(tongue_end.posX + offSetX, tongue_end.posY - entity.height * 0.3D,
					tongue_end.posZ + offSetZ);
	...
	protected Entity checkCollision() {
		List<EntityLivingBase> list = getEntityWorld().getEntitiesWithinAABB(EntityLivingBase.class,
				tongue_end.getEntityBoundingBox());
