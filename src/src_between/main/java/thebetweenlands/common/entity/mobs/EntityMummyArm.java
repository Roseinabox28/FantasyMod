...
public class EntityMummyArm extends EntityCreature implements IEntityBL {
	private static final DataParameter<Integer> OWNER_ID = EntityDataManager.<Integer>createKey(EntityMummyArm.class,
				...
				for (EntityLivingBase target : targets) {
					if (target != this && target != this.getOwner() && target instanceof EntityMob
						...
						if (target.hurtResistantTime < 10) {
							...
							target.attackEntityFrom(damageSource, (float) this
