...
public class EntityDragonFly extends EntityAmbientCreature implements IEntityBL {
	...
	public void flyAbout() {
		if (currentFlightTarget != null) {
			if (!world.isAreaLoaded(currentFlightTarget.add(-6, -6, -6), currentFlightTarget.add(6, 6, 6))
					|| !world.isAirBlock(currentFlightTarget) || currentFlightTarget.getY() < 1
					|| world.getBlockState(currentFlightTarget.up()).getBlock() == Blocks.WATER) {
				currentFlightTarget = null;
			}
		}
		if (currentFlightTarget == null || rand.nextInt(30) == 0
				|| currentFlightTarget.getDistance((int) posX, (int) posY, (int) posZ) < 8F) {
			BlockPos newTarget = new BlockPos((int) posX + rand.nextInt(7) - rand.nextInt(7),
					(int) posY + rand.nextInt(6) - 1, (int) posZ + rand.nextInt(7) - rand.nextInt(7));
			if (this.spawnPos != null && this.spawnPos.distanceSq(newTarget) > 32 * 32) {
				newTarget = this.spawnPos.add(rand.nextInt(16) - rand.nextInt(16), rand.nextInt(10) - 5,
	...
	protected void onDeathUpdate() {
		deathTime++;
		if (deathTime == 20) {
			if (!world.isRemote && (recentlyHit > 0 || isPlayer()) && !this.isChild()
			...
			if (!isInLurkersMouth()) {
				for (int particle = 0; particle < 20; particle++) {
					...
					world.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL,
							...
							posZ + rand.nextFloat() * width * 2.0F - width, motionX, motionY, motionZ);
				}
			}
		}
	}
