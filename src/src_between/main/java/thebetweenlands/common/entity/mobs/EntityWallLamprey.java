...
public class EntityWallLamprey extends EntityMovingWallFace implements IMob {
	...
	private static final DataParameter<Boolean> HIDDEN = EntityDataManager.createKey(EntityWallLamprey.class,
			DataSerializers.BOOLEAN);

	private static final DataParameter<Float> LOOK_X = EntityDataManager.createKey(EntityWallLamprey.class,
			DataSerializers.FLOAT);
	private static final DataParameter<Float> LOOK_Y = EntityDataManager.createKey(EntityWallLamprey.class,
			DataSerializers.FLOAT);
	private static final DataParameter<Float> LOOK_Z = EntityDataManager.createKey(EntityWallLamprey.class,
			DataSerializers.FLOAT);
	...
	public void onUpdate() {
		...
		if (this.clientHeadLookChanged) {
			this.headLook = new Vec3d(this.dataManager.get(LOOK_X), this.dataManager.get(LOOK_Y),
					this.dataManager.get(LOOK_Z));
		...
		if (this.isSucking()) {
			...
			if (!this.world.isRemote) {
				List<Entity> affectedEntities = (List<Entity>) this.world.getEntitiesWithinAABB(Entity.class,
						this.getEntityBoundingBox().grow(6.0F, 6.0F, 6.0F));
				...
				for (Entity e : affectedEntities) {
					...
					if (e instanceof EntityPlayer) {
						if (((EntityPlayer) e).isActiveItemStackBlocking())
							mod *= 0.18F;
					}
				...
				for (int i = 0; i < 3; i++) {
					...
					Vec3d vec = fwd.scale(rnd.nextFloat() * 5).add(up.scale((rnd.nextFloat() - 0.5F) * 1.2F))
							.add(right.scale((rnd.nextFloat() - 0.5F) * 1.2F));
					...
					this.world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, front.x + rx, front.y + ry, front.z + rz,
	...
	protected void updateWallSprite() {
		...
		if (state.isFullCube()) {
			this.wallSprite = Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelShapes()
					.getTexture(state);
		}
	}
	...
	public boolean attackEntityAsMob(Entity entity) {
		...
		if (cap != null && cap.isDecayEnabled()) {
			float attackDamage = (float) this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE)
					.getAttributeValue();
	...
	public boolean canResideInBlock(BlockPos pos, EnumFacing facing, EnumFacing facingUp) {
		return this.isValidBlockForMovement(pos, this.world.getBlockState(pos)) && this.isValidBlockForMovement(
				pos.offset(facingUp.getOpposite()), this.world.getBlockState(pos.offset(facingUp.getOpposite())));
	}
	...
	public int checkAnchorAt(BlockPos anchor, EnumFacing facing, EnumFacing facingUp, int checks) {
		...
		if ((checks & AnchorChecks.ENTITIES) != 0) {
			if (!this.world.getEntitiesWithinAABB(EntityWallFace.class,
					this.getEntityBoundingBox().offset(anchor.subtract(this.getAnchor()).offset(facingUp.getOpposite()))
							.expand(facing.getXOffset() * this.getPeek(), facing.getYOffset() * this.getPeek(),
	...
	protected boolean isValidBlockForMovement(BlockPos pos, IBlockState state) {
		return state.isOpaqueCube() && state.isNormalCube() && state.isFullCube()
				&& state.getBlockHardness(this.world, pos) > 0
				&& (state.getMaterial() == Material.ROCK || state.getMaterial() == Material.WOOD);
	}
	...
	public float getLampreyHiddenPercent(float partialTicks) {
		return 1 - (1
				- this.easeInOut(this.prevHiddenPercent + (this.hiddenPercent - this.prevHiddenPercent) * partialTicks))
				* this.getHoleDepthPercent(partialTicks);
	}
	...
	public void setHeadLook(Vec3d look) {
		...
		if (Math.abs(curr.x - look.x) >= 0.01F || Math.abs(curr.y - look.y) >= 0.01F
				|| Math.abs(curr.z - look.z) >= 0.01F) {
			if (!this.world.isRemote) {
	...
	public Vec3d getHeadLook(float partialTicks) {
		return new Vec3d(this.prevHeadLook.x + (this.headLook.x - this.prevHeadLook.x) * partialTicks,
	...
	public float[] getRelativeHeadLookAngles(float partialTicks) {
		...
		return new float[] { (float) Math.toDegrees(Math.atan2(right, fwd)),
				(float) Math.toDegrees(Math.atan2(fwd, up)) * (float) Math.signum(fwd) - 90.0F };
	}
	...
	public void startSucking() {
		if (!this.world.isRemote) {
			...
			this.world.playSound(null, this.posX, this.posY, this.posZ, SoundRegistry.WALL_LAMPREY_SUCK,
					SoundCategory.HOSTILE, 0.8F, this.world.rand.nextFloat() * 0.3F + 0.8F);
		}
	...
	public void startSpit(float spitDamage) {
		...
		if (target != null) {
			...
			jet.setPosition(this.posX + facing.getXOffset() * (this.width / 2 + 0.1F),
					this.posY + this.height / 2.0F + facing.getYOffset() * (this.height / 2 + 0.1F),
					this.posZ + facing.getZOffset() * (this.width / 2 + 0.1F));
	...
	protected static class AISuck extends EntityAIBase {
		...
		public boolean shouldExecute() {
			return this.entity.getFacing() != EnumFacing.DOWN && !this.entity.isSucking() && !this.entity.isMoving()
					...
					&& this.entity.getDistance(this.entity.getAttackTarget()) < 6.0F;
		}
			...
			if (!this.entity.isSucking()) {
				if (this.cooldown <= 0) {
					this.cooldown = this.minCooldown
							+ this.entity.rand.nextInt(this.maxCooldown - this.minCooldown + 1);
	...
	protected static class AISpit extends EntityAIBase {
		...
		public boolean shouldExecute() {
			return this.entity.getFacing() == EnumFacing.DOWN && !this.entity.isSucking() && !this.entity.isMoving()
					&& this.entity.getAttackTarget() != null && this.entity.getAttackTarget().isEntityAlive()
					&& this.entity.getEntitySenses().canSee(this.entity.getAttackTarget())
					&& this.isInRange(this.entity.getAttackTarget());
		}
