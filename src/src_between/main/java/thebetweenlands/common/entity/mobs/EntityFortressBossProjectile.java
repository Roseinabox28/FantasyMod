...
public class EntityFortressBossProjectile extends Entity implements IProjectile {
	protected static final DataParameter<Optional<UUID>> OWNER = EntityDataManager
			.<Optional<UUID>>createKey(EntityFortressBossProjectile.class, DataSerializers.OPTIONAL_UNIQUE_ID);
	protected static final DataParameter<Boolean> DEFLECTION_STATE = EntityDataManager
			.<Boolean>createKey(EntityFortressBossProjectile.class, DataSerializers.BOOLEAN);
	...
	public Entity getThrower() {
		...
		} else if (this.cachedThrower == null || !this.cachedThrower.isEntityAlive()
				|| !this.cachedThrower.getUniqueID().equals(uuid)) {
			this.cachedThrower = null;
			for (Entity entity : this.getEntityWorld().getEntitiesWithinAABB(Entity.class,
					this.getEntityBoundingBox().grow(64.0D, 64.0D, 64.0D))) {
				if (entity.getUniqueID().equals(uuid)) {
					...
					if (shieldHit >= 0) {
						if (!this.world.isRemote) {
							...
							this.world.playSound(null, this.posX, this.posY, this.posZ,
									SoundRegistry.FORTRESS_BOSS_SHIELD_DOWN, SoundCategory.HOSTILE, 1.0F, 1.0F);
				...
				if (source.getTrueSource() instanceof EntityPlayer) {
					...
					if (heldItem != null && heldItem.getItem() instanceof ItemSword) {
						if (!this.world.isRemote && source.getTrueSource().getPassengers().isEmpty()) {
							...
							this.getServer().getPlayerList()
									.sendPacketToAllPlayers(new SPacketSetPassengers(source.getTrueSource()));
							return true;
						}
					}
				}
	...
	public void onUpdate() {
		...
		if (!this.isDead) {
			if (this.getRidingEntity() == null) {
				...
				List<Entity> hitEntities = this.world.getEntitiesWithinAABBExcludingEntity(this, this
						.getEntityBoundingBox().grow(this.motionX, this.motionY, this.motionZ).grow(0.1D, 0.1D, 0.1D));
				double minDist = 0.0D;
				...
				for (int i = 0; i < hitEntities.size(); ++i) {
					...
					if (entity.canBeCollidedWith() && entity != this.getThrower() && entity != this) {
						float f = 0.1F;
						AxisAlignedBB axisalignedbb = entity.getEntityBoundingBox().grow((double) f, (double) f,
								(double) f);
