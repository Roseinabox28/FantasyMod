...
public class EntityBloodSnail extends EntityMob implements IEntityBL {
	public static final IAttribute RANGED_ATTACK_MIN_DIST_ATTRIB = (new RangedAttribute(null, "bl.rangedAttackMinDist",
			3.0D, 0, Double.MAX_VALUE)).setDescription("Minimum range at which the ranged attack is used");
	public static final IAttribute RANGED_ATTACK_COOLDOWN_ATTRIB = (new RangedAttribute(null, "bl.rangedAttackCooldown",
			50, 0, Integer.MAX_VALUE)).setDescription("Ranged attack cooldown in ticks");
	...
	public void onUpdate() {
		...
		if (getAttackTarget() != null && this.isEntityAlive()) {
			float distance = (float) getDistance(getAttackTarget().posX, getAttackTarget().getEntityBoundingBox().minY,
