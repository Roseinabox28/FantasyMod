...
public class EntityChiromaw extends EntityFlyingMob implements IEntityBL {
	private static final DataParameter<Boolean> IS_HANGING = EntityDataManager.createKey(EntityChiromaw.class,
	...
	protected void updateAITasks() {
		...
		if (getIsHanging()) {
			if (!this.world.isRemote) {
				...
				if (this.rand.nextInt(250) == 0 || !this.world
						.getBlockState(new BlockPos(this.posX, this.posY + 1, this.posZ)).isNormalCube()) {
					setIsHanging(false);
			...
			if (this.getAttackTarget() == null) {
				if (!this.world.isRemote && this.rand.nextInt(20) == 0
						&& world.getBlockState(new BlockPos(this.posX, this.posY + 1, this.posZ)).isNormalCube()) {
					setIsHanging(true);
				}
			}
		}
