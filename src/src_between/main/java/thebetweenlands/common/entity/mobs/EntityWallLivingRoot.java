...
public class EntityWallLivingRoot extends EntityMovingWallFace implements IMob, IEntityMultiPart {
	...
	public static final IAttribute MAX_ARM_LENGTH = (new RangedAttribute(null, "bl.maxRootArmLength", 2.5D, 0.0D,
			16.0D)).setDescription("Maximum length of root arm").setShouldWatch(true);

	protected static final float ARM_FULL_WIDTH = 0.2F;
	protected static final float[][] ARM_CROSS_SECTION = new float[][] { { -ARM_FULL_WIDTH, ARM_FULL_WIDTH },
			{ -ARM_FULL_WIDTH, -ARM_FULL_WIDTH }, { ARM_FULL_WIDTH, -ARM_FULL_WIDTH },
	...
	private static final DataParameter<Integer> REL_TIP_X = EntityDataManager.createKey(EntityWallLivingRoot.class,
			DataSerializers.VARINT);
	private static final DataParameter<Integer> REL_TIP_Y = EntityDataManager.createKey(EntityWallLivingRoot.class,
			DataSerializers.VARINT);
	private static final DataParameter<Integer> REL_TIP_Z = EntityDataManager.createKey(EntityWallLivingRoot.class,
			DataSerializers.VARINT);
	...
	public Vec3d getTipPos() {
		return new Vec3d(this.posX + this.dataManager.get(REL_TIP_X) / 512.0f,
				this.posY + this.dataManager.get(REL_TIP_Y) / 512.0f,
				this.posZ + this.dataManager.get(REL_TIP_Z) / 512.0f);
	}
	...
	public void onUpdate() {
		...
		float flailingStrength = this.isSwingInProgress ? (1 - this.swingProgress)
				: this.hurtTime > 0 ? (this.hurtTime / (float) this.maxHurtTime) * 0.5F : 0.0f;
		...
		Vec3d dirFwd = new Vec3d(this.getFacing().getXOffset(), this.getFacing().getYOffset(),
				this.getFacing().getZOffset());
		;
		Vec3d dirUp = new Vec3d(this.getFacingUp().getXOffset(), this.getFacingUp().getYOffset(),
				this.getFacingUp().getZOffset());

		Vec3d armStart = new Vec3d(0, this.height / 2, 0).add(-dirFwd.x * (this.width / 2 - 0.1f),
				-dirFwd.y * (this.height / 2 - 0.1f), -dirFwd.z * (this.width / 2 - 0.1f));
		...
		if (!this.rootTipPositionSet) {
			Vec3d tipPos = this.getPositionVector()
					.add(armStart.add(dirFwd.scale(maxArmLength)).add(0, -this.rootTip.height / 2, 0));
			this.setTipPos(tipPos);
			this.rootTip.setPosition(tipPos.x, tipPos.y, tipPos.z);
			this.rootTipPositionSet = true;
		}

		Vec3d armEnd = this.rootTip.getPositionVector().add(0, this.rootTip.height / 2, 0)
		...
		if (!this.world.isRemote) {
			...
			// Idle movement
			targetTipPos = targetTipPos.add(dirUp.scale(idleY)).add(dirFwd.crossProduct(dirUp).scale(idleX))
					.add(dirFwd.scale(offsetZ - idleZ));
			...
			tipPos = armStartWorld.add(tipPos.subtract(armStartWorld).normalize()
					.scale(Math.min(tipPos.subtract(armStartWorld).length(), maxArmLength)));
		...
		endSegment.update(dirUp, armEnd,
				armEnd.subtract(this.armSegments.get(this.armSegments.size() - 2).pos).normalize());

		for (int i = 0; i < NUM_SEGMENTS; i++) {
			ArmSegment segment = this.armSegments.get(i);
			Vec3d pos = segment.pos;
			this.parts[i + 1].setPosition(this.posX + pos.x, this.posY + pos.y - this.parts[i + 1].height / 2.0f,
					this.posZ + pos.z);
		}
	}
	...
	protected void updateMovement() {
		if (!this.world.isRemote && this.isMoving() && this.getMoveReason() != MoveReason.LOOK) {
			...
			if (this.getMovementProgress(1) >= 0.5F && wasFirstHalf) {
				this.world.playSound(null, this.posX, this.posY, this.posZ, SoundRegistry.WALL_LIVING_ROOT_EMERGE,
	...
	public void handleStatusUpdate(byte id) {
		...
		if (id == EVENT_HURT_SOUND || id == EVENT_DEATH) {
			...
			this.world.playSound(this.posX, this.posY, this.posZ, soundType.getBreakSound(), SoundCategory.BLOCKS,
					(soundType.getVolume() + 1.0F) / 1.3F, soundType.getPitch() * 0.8F, false);
			this.world.playSound(this.posX, this.posY, this.posZ, soundType.getHitSound(), SoundCategory.NEUTRAL,
					(soundType.getVolume() + 1.0F) / 4.0F, soundType.getPitch() * 0.5F, false);
		}
	}
		...
		public void updateTask() {
			...
			if (this.attackTicks > 0) {
				this.attackTicks--;
					...
					&& target.getEntityBoundingBox().intersects(this.entity.rootTip.getEntityBoundingBox())) {
