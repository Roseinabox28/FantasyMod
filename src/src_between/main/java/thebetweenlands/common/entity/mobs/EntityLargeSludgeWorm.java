...
public class EntityLargeSludgeWorm extends EntitySludgeWorm {
	private static final DataParameter<Float> EGG_SAC_PERCENTAGE = EntityDataManager
			.<Float>createKey(EntityLargeSludgeWorm.class, DataSerializers.FLOAT);
	...
	protected static final float[][] HULL_CROSS_SECTION = new float[][] { { -HULL_OUTER_WIDTH, HULL_INNER_WIDTH },
			{ -HULL_OUTER_WIDTH, -HULL_INNER_WIDTH }, { -HULL_INNER_WIDTH, -HULL_INNER_WIDTH },
			{ -HULL_INNER_WIDTH, -HULL_OUTER_WIDTH }, { HULL_INNER_WIDTH, -HULL_OUTER_WIDTH },
			{ HULL_INNER_WIDTH, -HULL_INNER_WIDTH }, { HULL_OUTER_WIDTH, -HULL_INNER_WIDTH },
			{ HULL_OUTER_WIDTH, HULL_INNER_WIDTH }, { HULL_INNER_WIDTH, HULL_INNER_WIDTH },
			{ HULL_INNER_WIDTH, HULL_OUTER_WIDTH }, { -HULL_INNER_WIDTH, HULL_OUTER_WIDTH },
	...
	public EntityLargeSludgeWorm(World world) {
		...
		this.parts = new MultiPartEntityPart[] { new MultiPartEntityPart(this, "part1", 0.8F, 0.8F),
				new MultiPartEntityPart(this, "part2", 0.8F, 0.8F), new MultiPartEntityPart(this, "part3", 0.8F, 0.8F),
	...
	protected boolean damageWorm(DamageSource source, float amount) {
		...
		if (!this.world.isRemote && source instanceof EntityDamageSource && this.world.rand.nextInt(6) == 0
				&& amount > 0.5F) {
			...
			entity.setLocationAndAngles(spawnPart.posX, spawnPart.posY, spawnPart.posZ, this.rand.nextFloat() * 360.0F,
					0);
	...
	protected void updateSegmentPositions() {
		...
		Vec3d endPoint;
		if (partDir != null) {
			endPoint = this.parts[this.parts.length - 1].getPositionVector().add(-origin.x + partDir.x,
					-origin.y + partDir.y, -origin.z + partDir.z);
		} else {
			endPoint = this.parts[this.parts.length - 1].getPositionVector().add(-origin.x, -origin.y - 0.0001D,
					-origin.z);
		}
	...
	/**
	 * Sets the egg sac laying progress percentage. Use value < 0 if no egg sac.
	 * 
	 * @param percentage
	 */
	...
	/**
	 * Returns the egg sac laying progress percentage. Value < 0 means no egg sac.
	 * 
	 * @return
	 */
	...
	public static class AILayEggSac extends EntityAIBase {
		...
		public boolean shouldExecute() {
			boolean canLay = this.entity.isEntityAlive() && this.entity.getAttackTarget() != null
					&& this.entity.getEggSacPercentage() < 0;
			if (canLay) {
				if (this.cooldown-- <= 0) {
					List<EntityTinyWormEggSac> nearbyEggSacs = this.entity.world.getEntitiesWithinAABB(
							EntityTinyWormEggSac.class, this.entity.getEntityBoundingBox().grow(16.0D));
					List<EntityTinySludgeWorm> nearbyTinyWorms = this.entity.world.getEntitiesWithinAABB(
							EntityTinySludgeWorm.class, this.entity.getEntityBoundingBox().grow(16.0D));
