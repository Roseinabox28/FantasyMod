...
public class EntitySplodeshroom extends EntityProximitySpawner {
	...
	private static final DataParameter<Boolean> IS_SWELLING = EntityDataManager.createKey(EntityProximitySpawner.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> SWELL_COUNT = EntityDataManager.createKey(EntityProximitySpawner.class,
			DataSerializers.VARINT);
	private static final DataParameter<Boolean> HAS_EXPLODED = EntityDataManager.createKey(EntityProximitySpawner.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Float> AOE_SIZE_XZ = EntityDataManager.createKey(EntityProximitySpawner.class,
			DataSerializers.FLOAT);
	private static final DataParameter<Float> AOE_SIZE_Y = EntityDataManager.createKey(EntityProximitySpawner.class,
			DataSerializers.FLOAT);
	...
	protected Entity checkAreaOfEffect() {
		Entity entity = null;
		if (!getEntityWorld().isRemote && getEntityWorld().getDifficulty() != EnumDifficulty.PEACEFUL) {
			List<EntityPlayer> list = getEntityWorld().getEntitiesWithinAABB(EntityPlayer.class,
	...
	private void explode() {
		...
		TheBetweenlands.networkWrapper.sendToAll(
				new PacketParticle(ParticleType.SPLODE_SHROOM, (float) posX, (float) posY + 0.25F, (float) posZ, 0F));
		getEntityWorld().playSound((EntityPlayer) null, getPosition(), SoundRegistry.SPLODESHROOM_POP,
	...
	protected void setBoundingBoxSize() {
		AxisAlignedBB axisalignedbb = new AxisAlignedBB(posX - getAOESizeXZ() * 0.5D, posY,
				posZ - getAOESizeXZ() * 0.5D, posX + getAOESizeXZ() * 0.5D, posY + getAOESizeY(),
				posZ + getAOESizeXZ() * 0.5D);
	...
	private void setSwelling(boolean swell) {
		...
		if (swell)
			getEntityWorld().playSound((EntityPlayer) null, getPosition(), SoundRegistry.SPLODESHROOM_WINDUP,
					SoundCategory.HOSTILE, 0.5F, 1F + (world.rand.nextFloat() - world.rand.nextFloat()) * 0.8F);
		else
			getEntityWorld().playSound((EntityPlayer) null, getPosition(), SoundRegistry.SPLODESHROOM_WINDDOWN,
					SoundCategory.HOSTILE, 0.5F, 1F + (world.rand.nextFloat() - world.rand.nextFloat()) * 0.8F);
	}
