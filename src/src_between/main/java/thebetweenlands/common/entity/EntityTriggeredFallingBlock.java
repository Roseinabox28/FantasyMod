...
public class EntityTriggeredFallingBlock extends EntityProximitySpawner {
	...
	private static final DataParameter<Boolean> IS_WALK_WAY = EntityDataManager
			.createKey(EntityTriggeredFallingBlock.class, DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> IS_HANGING = EntityDataManager
			.createKey(EntityTriggeredFallingBlock.class, DataSerializers.BOOLEAN);
	...
	public void dustParticles() {
		if (rand.nextInt(16) == 0) {
			...
			if (canFallThrough(getEntityWorld().getBlockState(blockpos))) {
				...
				if (!isWalkway())
					getEntityWorld().spawnParticle(EnumParticleTypes.BLOCK_DUST, d0, d1, d2, 0.0D, 0.0D, 0.0D,
							Block.getStateId(getBlockType(getEntityWorld(), getPosition())));
				else {
					...
					getEntityWorld().spawnParticle(EnumParticleTypes.BLOCK_DUST, d0, d1, d2, motionX, motionY, motionZ,
							Block.getStateId(getBlockType(getEntityWorld(), getPosition())));
				}
			}
		}
	...
	protected void performPreSpawnaction(Entity targetEntity, Entity entitySpawned) {
		...
		if (!getEntityWorld().isRemote) {
			targetEntity.getEntityWorld().playSound(null, targetEntity.getPosition(), SoundRegistry.ROOF_COLLAPSE,
	...
	protected AxisAlignedBB proximityBox() {
		return new AxisAlignedBB(getPosition())
				.grow(getProximityHorizontal(), getProximityVertical(), getProximityHorizontal())
				.offset(0D, isWalkway() ? 1D : -getProximityVertical() * 2, 0D);
	}
	...
	protected Entity getEntitySpawned() {
		if (getBlockType(getEntityWorld(), getPosition()).getBlock() != null) {
			EntityFallingBlock entity = new EntityFallingBlock(getEntityWorld(), posX, posY, posZ,
					getBlockType(getEntityWorld(), getPosition()));
