...
public class RightClickBlockTrigger
		extends BLTrigger<RightClickBlockTrigger.Instance, RightClickBlockTrigger.Listener> {
	...
	public static class Instance extends AbstractCriterionInstance {
		...
		public Instance(ItemPredicate[] items, BlockPredicate[] blocks, LocationPredicate location,
				EnumFacing[] facings) {
		...
		public boolean test(ItemStack stack, IBlockState state, BlockPos pos, EnumFacing face, WorldServer world) {
			...
			boolean matchSide = facings == null || facings.length <= 0
					|| Arrays.stream(facings).anyMatch(enumFacing -> enumFacing.equals(face));
			return matchSide && blockAmount > blockList.size() && itemAmount > itemList.size()
					&& this.location.test(world, (float) pos.getX(), (float) pos.getY(), (float) pos.getZ());
		}
	}
