...
public class TileEntityInfuser extends TileEntityBasicInventory implements IFluidHandler, ITickable {
	...
	public void update() {
		...
		if (this.hasInfusion && this.infusingRecipe != null) {
			...
			if (this.prevInfusionState != this.currentInfusionState && this.currentInfusionState == 2) {
				this.world.playSound(null, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D,
						SoundRegistry.INFUSER_FINISHED, SoundCategory.BLOCKS, 1, 1);
			}
			this.prevInfusionState = this.currentInfusionState;
			if (!this.world.isRemote) {
				if (this.infusionTime > this.infusingRecipe.idealInfusionTime
						+ this.infusingRecipe.infusionTimeVariation) {
					// fail
					if (this.currentInfusionState != 3)
						updateBlock = true;
					this.currentInfusionState = 3;
				} else if (this.infusionTime > this.infusingRecipe.idealInfusionTime
						- this.infusingRecipe.infusionTimeVariation
						&& this.infusionTime < this.infusingRecipe.idealInfusionTime
								+ this.infusingRecipe.infusionTimeVariation) {
					// finished
			...
			if (this.world.isRemote && this.infusionColorGradientTicks > 0 && this.currentInfusionState == 2) {
				for (int i = 0; i < 10; i++) {
					...
					BLParticles.STEAM_PURIFIER.spawn(this.world, x,
							pos.getY() + 1.0D - this.world.rand.nextFloat() * 0.2F, z);
				}
			}
		} else {
			...
			if (this.hasIngredients() && this.temp >= 100) {
				...
				if (!this.world.isRemote && this.infusionColorGradientTicks == 0 && this.currentInfusionState == 0
						&& this.stirProgress == 89) {
					// start gradient animation
					this.infusionColorGradientTicks = 1;
					this.currentInfusionState = 1;
					this.world.playSound(null, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D,
		...
		if (isValidCrystalInstalled()) {
			if (temp >= 100 && evaporation >= 400 && stirProgress >= 90 && this.hasIngredients()) {
				inventory.get(MAX_INGREDIENTS + 1)
						.setItemDamage(inventory.get(MAX_INGREDIENTS + 1).getItemDamage() + 1);
				stirProgress = 0;
			}
			if (!hasCrystal) {
				hasCrystal = true;
	...
	/**
	 * Returns the current infusing state: 0 = no progress, 1 = in progress, 2 =
	 * finished, 3 = failed
	 */
			...
			if (hasInfusion) {
				for (int i = 0; i <= TileEntityInfuser.MAX_INGREDIENTS; i++) {
					...
					if (!stack.isEmpty() && stack.getItem() == ItemRegistry.ASPECT_VIAL) {
						...
						EntityItem entity = new EntityItem(this.world, this.getPos().getX() + 0.5D,
								this.getPos().getY() + 1.0D, this.getPos().getZ() + 0.5D, ret);
				...
				if (evaporation == 600) {
					...
					if (this.infusingRecipe != null) {
						...
						gasCloud.setGasColor((int) (color[0] * 255), (int) (color[1] * 255), (int) (color[2] * 255),
								170);
					}
					gasCloud.setLocationAndAngles(this.pos.getX() + 0.5D, this.pos.getY() + 1D, this.pos.getZ() + 0.5D,
							MathHelper.wrapDegrees(this.world.rand.nextFloat() * 360.0F), 0.0F);
	...
	public int getScaledWaterAmount(int scale) {
		return waterTank.getFluid() != null
				? (int) ((float) waterTank.getFluid().amount / (float) waterTank.getCapacity() * scale)
				: 0;
	}

	public boolean isValidCrystalInstalled() {
		return !inventory.get(MAX_INGREDIENTS + 1).isEmpty()
				&& inventory.get(MAX_INGREDIENTS + 1).getItem() == ItemRegistry.LIFE_CRYSTAL && inventory
						.get(MAX_INGREDIENTS + 1).getItemDamage() < inventory.get(MAX_INGREDIENTS + 1).getMaxDamage();
	}
	...
	public List<IAspectType> getInfusingAspects() {
		...
		for (int i = 0; i <= MAX_INGREDIENTS; i++) {
			if (!inventory.get(i).isEmpty()) {
				ItemAspectContainer container = ItemAspectContainer.fromItem(inventory.get(i),
						AspectManager.get(this.world));
	...
	public boolean isItemValidForSlot(int slot, ItemStack itemstack) {
		return !hasInfusion() && getStackInSlot(slot).isEmpty()
				&& ((slot <= MAX_INGREDIENTS
						&& ItemAspectContainer.fromItem(itemstack, AspectManager.get(world)).getAspects().size() > 0)
						|| (slot == MAX_INGREDIENTS + 1 && itemstack.getItem() == ItemRegistry.LIFE_CRYSTAL));
	}
