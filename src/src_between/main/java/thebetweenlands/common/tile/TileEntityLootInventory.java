...
public abstract class TileEntityLootInventory extends TileEntityBasicInventory implements ISharedLootContainer {
	...
	public static boolean fillInventoryWithLoot(ISharedLootContainer inventory, @Nullable EntityPlayer player,
			long seed) {
		...
		if (lootTableLocation != null && inventory instanceof TileEntity) {
			...
			if (tile.getWorld() instanceof WorldServer) {
				...
				if (inventory.isSharedLootTable()) {
					List<LocationStorage> locations = LocationStorage.getLocations(tile.getWorld(),
							new AxisAlignedBB(tile.getPos()));
	...
	public static boolean fillInventoryRandomly(Random random, List<ItemStack> loot, IInventory itemHandler) {
		...
		while (emptySlotCount > 0 && ((List<ItemStack>) splittableStacks).size() > 0) {
			ItemStack itemstack2 = (ItemStack) splittableStacks
					.remove(MathHelper.getInt(random, 0, splittableStacks.size() - 1));
		...
		for (ItemStack itemstack : loot) {
			...
			if (itemstack.isEmpty()) {
				itemHandler.setInventorySlotContents(((Integer) emptySlots.remove(emptySlots.size() - 1)).intValue(),
						ItemStack.EMPTY);
			} else {
				itemHandler.setInventorySlotContents(((Integer) emptySlots.remove(emptySlots.size() - 1)).intValue(),
						itemstack);
	...
	/**
	 * Tries to read a loot table from the NBT. Returns true if found loot table !=
	 * null
	 * 
	 * @param compound
	 * @return
