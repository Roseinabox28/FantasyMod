...
public class TileEntityPurifier extends TileEntityBasicInventory implements IFluidHandler, ITickable {
	...
	public void update() {
		...
		if (hasFuel() && !outputIsFull()) {
			if (!output.isEmpty() && getWaterAmount() > 0 && inventory.get(2).isEmpty() || !output.isEmpty()
					&& getWaterAmount() > 0 && !inventory.get(2).isEmpty() && inventory.get(2).isItemEqual(output)) {
				time++;
				if (time % 108 == 0)
					world.playSound(null, pos.getX(), pos.getY(), pos.getZ(), SoundRegistry.PURIFIER,
				...
				if (time >= MAX_TIME) {
					...
					time = 0;
					markDirty();
					boolean canRun = !output.isEmpty() && getWaterAmount() > 0 && inventory.get(2).isEmpty()
							|| !output.isEmpty() && getWaterAmount() > 0 && !inventory.get(2).isEmpty()
									&& inventory.get(2).isItemEqual(output);
					if (!canRun)
	...
	public boolean hasFuel() {
		return !getStackInSlot(0).isEmpty() && EnumItemMisc.SULFUR.isItemOf(getStackInSlot(0))
