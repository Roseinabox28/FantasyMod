...
public class TileEntityAnimator extends TileEntityBasicInventory implements ITickable {
	...
	// new WeightedRandomItem(new ItemStack(BLItemRegistry.lifeCrystal), 10), new
	// WeightedRandomItem(ItemGeneric.createStack(EnumItemGeneric.VALONITE_SHARD),
	// 20), new
	// WeightedRandomItem(ItemGeneric.createStack(EnumItemGeneric.OCTINE_INGOT),
	// 30), new WeightedRandomItem(ItemGeneric.createStack(EnumItemGeneric.SULFUR),
	// 40) };
	...
	public void update() {
		...
		if (!world.isRemote) {
			...
			if (!this.itemToAnimate.isEmpty() && isCrystalInslot() && isSulfurInSlot()
					&& fuelConsumed < requiredFuelCount && isValidFocalItem()) {
			...
			if (fuelConsumed >= requiredFuelCount && isSlotInUse(0) && isSlotInUse(1) && !this.itemAnimated) {
				...
				if (result.isEmpty())
					result = recipe.getResult(inventory.get(0));
				if (!result.isEmpty()) {
	...
	protected void playAnimatorSound() {
		Minecraft.getMinecraft().getSoundHandler()
				.playSound(new AnimatorSound(SoundRegistry.ANIMATOR, SoundCategory.BLOCKS, this));
	}
	...
	public boolean isCrystalInslot() {
		return isSlotInUse(1) && inventory.get(1).getItem() == ItemRegistry.LIFE_CRYSTAL
				&& inventory.get(1).getItemDamage() < inventory.get(1).getMaxDamage();
	}
	...
	public boolean isSulfurInSlot() {
		return isSlotInUse(2) && inventory.get(2).getItem() == ItemRegistry.ITEMS_MISC
				&& inventory.get(2).getItemDamage() == ItemMisc.EnumItemMisc.SULFUR.getID();
	}
	...
	public boolean isRunning() {
		return this.isSlotInUse(0) && this.isCrystalInslot() && this.isSulfurInSlot()
				&& this.fuelConsumed < this.requiredFuelCount && lifeCrystalLife >= this.requiredLifeCount
				&& this.isValidFocalItem();
	}
	...
	public boolean canInsertItem(int slot, ItemStack stack, EnumFacing side) {
		if (slot == 1 && !stack.isEmpty() && stack.getItem().equals(ItemRegistry.LIFE_CRYSTAL))
			return true;
		else if (slot == 2 && !stack.isEmpty() && stack.getItem().equals(ItemRegistry.ITEMS_MISC)
				&& stack.getItemDamage() == ItemMisc.EnumItemMisc.SULFUR.getID())
			return true;
