...
public class TileEntityPuffshroom extends TileEntity implements ITickable {
	...
	public boolean active_1 = false, active_2 = false, active_3 = false, active_4 = false, active_5 = false,
			pause = true;
	...
	public void update() {
			...
			if (getWorld().isRemote) {
				if (animation_1 < 3) {
					...
					for (int i = 0, amount = 5 + getWorld().rand.nextInt(2); i < amount; i++) {
						...
						world.spawnParticle(EnumParticleTypes.BLOCK_DUST, px + ox, py, pz + oz, motionX, motionY,
								motionZ, Block.getStateId(BlockRegistry.MUD_TILES.getDefaultState()));
		...
		if (!getWorld().isRemote) {
			if (active_4) {
				if (animation_4 <= 1)
					getWorld().playSound((EntityPlayer) null, getPos().getX() + 0.5D, getPos().getY() + 1D,
							getPos().getZ() + 0.5D, SoundRegistry.PUFF_SHROOM, SoundCategory.BLOCKS, 0.5F,
							0.95F + getWorld().rand.nextFloat() * 0.2F);
				if (animation_4 == 10) {
	...
	protected Entity findEnemyToAttack() {
		if (!active_1 && animation_1 == 0) {
			List<EntityPlayer> list = getWorld().getEntitiesWithinAABB(EntityPlayer.class,
					new AxisAlignedBB(getPos()).grow(2D, 2D, 2D));
