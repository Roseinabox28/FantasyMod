...
public class TileEntityCenser extends TileEntityBasicInventory implements IFluidHandler, ITickable, ICenser {
	...
	public TileEntityCenser() {
		super("container.bl.censer", NonNullList.withSize(INV_SIZE, ItemStack.EMPTY),
				(te, inv) -> new ItemStackHandler(inv) {
					@Override
					...
					public ItemStack extractItem(int slot, int amount, boolean simulate) {
						return slot == ContainerCenser.SLOT_INTERNAL ? ItemStack.EMPTY
	...
	protected void readRecipeNbt(NBTTagCompound nbt, boolean packet) {
		...
		if (nbt.hasKey("recipeId", Constants.NBT.TAG_STRING)) {
			...
			if (recipe != null && this.canRecipeRun(this.isFluidRecipe, recipe)) {
				...
				if (recipeContext != null && id.equals(recipe.getId())
						&& nbt.hasKey("recipeNbt", Constants.NBT.TAG_COMPOUND)) {
	...
	public void update() {
			...
			if (this.checkInternalSlotForRecipes) {
				...
				if (fluid != null && fluid.amount > 0) {
					...
					if (recipe != null && recipe.matchesInput(fluid)) {
						...
						this.maxConsumptionTicks = this.consumptionTicks = this.currentRecipe
								.getConsumptionDuration(this.currentRecipeContext, this);
						this.markDirty();
						IBlockState stat = this.world.getBlockState(this.pos);
						this.world.notifyBlockUpdate(this.pos, stat, stat, 2);
					}
		...
		if (!this.world.isRemote) {
			if (this.checkInputSlotForTransfer) {
				...
				if (!inputStack.isEmpty()) {
					...
					if (this.getStackInSlot(ContainerCenser.SLOT_INTERNAL).isEmpty() && (fillResult = FluidUtil
							.tryEmptyContainer(inputStack, this, Integer.MAX_VALUE, null, true)).isSuccess()) {
		...
		boolean isCreatingDungeonFog = this.isRecipeRunning
				&& this.currentRecipe.isCreatingDungeonFog(this.currentRecipeContext, this);
	...
	protected boolean canRecipeRun(boolean isFluidRecipe, ICenserRecipe<?> recipe) {
		return !((isFluidRecipe
				&& (this.fluidTank.getFluidAmount() <= 0 || !recipe.matchesInput(this.fluidTank.getFluid())))
				|| (!isFluidRecipe && (this.getStackInSlot(ContainerCenser.SLOT_INTERNAL).isEmpty()
						|| this.remainingItemAmount <= 0
						|| !recipe.matchesInput(this.getStackInSlot(ContainerCenser.SLOT_INTERNAL)))));
	}
	...
	public boolean hasFuel() {
		return !getStackInSlot(ContainerCenser.SLOT_FUEL).isEmpty()
				&& EnumItemMisc.SULFUR.isItemOf(getStackInSlot(ContainerCenser.SLOT_FUEL))
				&& getStackInSlot(ContainerCenser.SLOT_FUEL).getCount() >= 1;
	}
	...
	public boolean canExtractItem(int slot, ItemStack stack, EnumFacing side) {
		if (super.canInsertItem(slot, stack, side)) {
			// Only allow automatic extraction from input slot if item has no censer recipe,
			// i.e. was used
	...
	public AxisAlignedBB getFogRenderArea() {
		...
		float height = 12.0F;
		BlockPos pos = this.getPos();
		return new AxisAlignedBB(pos.getX() + 0.5D - width / 2, pos.getY() - 0.1D, pos.getZ() + 0.5D - width / 2,
				pos.getX() + 0.5D + width / 2, pos.getY() - 0.1D + height, pos.getZ() + 0.5D + width / 2);
	}
