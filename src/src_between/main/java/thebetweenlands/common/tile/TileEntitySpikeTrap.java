...
public class TileEntitySpikeTrap extends TileEntity implements ITickable {
	...
	public void update() {
		if (!getWorld().isRemote) {
			...
			if (stateFacing.getBlock() != Blocks.AIR
					&& stateFacing.getBlockHardness(getWorld(), getPos().offset(facing, 1)) >= 0.0F
					&& !(stateFacing.getBlock() instanceof BlockSludge)) {
				setType((byte) 1);
				setActive(true);
				Block block = stateFacing.getBlock();
				getWorld().playEvent(null, 2001, getPos().offset(facing, 1), Block.getIdFromBlock(block));
				block.dropBlockAsItem(getWorld(), getPos().offset(facing, 1),
						getWorld().getBlockState(getPos().offset(facing, 1)), 0);
				getWorld().setBlockToAir(getPos().offset(facing, 1));
			}
			IBlockState stateFacing2 = getWorld().getBlockState(getPos().offset(facing, 2));
			if (stateFacing2.getBlock() != Blocks.AIR
					&& stateFacing2.getBlockHardness(getWorld(), getPos().offset(facing, 2)) >= 0.0F
					&& !(stateFacing.getBlock() instanceof BlockSludge)) {
				setType((byte) 1);
				setActive(true);
				Block block = stateFacing2.getBlock();
				getWorld().playEvent(null, 2001, getPos().offset(facing, 2), Block.getIdFromBlock(block));
				block.dropBlockAsItem(getWorld(), getPos().offset(facing, 2),
						getWorld().getBlockState(getPos().offset(facing, 2)), 0);
		...
		if (active) {
			activateBlock();
			if (animationTicks == 0)
				getWorld().playSound(null, (double) getPos().getX(), (double) getPos().getY(), (double) getPos().getZ(),
						SoundRegistry.SPIKE, SoundCategory.BLOCKS, 1.25F, 1.0F);
	...
	protected Entity isBlockOccupied() {
		...
		BlockPos hitArea = getPos().offset(facing, 1);
		List<EntityLivingBase> list = getWorld().getEntitiesWithinAABB(EntityLivingBase.class,
				new AxisAlignedBB(hitArea).shrink(0.25D));
		for (int i = 0; i < list.size(); i++) {
			Entity entity = list.get(i);
			if (entity != null)
				if (!(entity instanceof IEntityBL))
