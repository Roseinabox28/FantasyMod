...
public abstract class MobSpawnerLogicBetweenlands {
	...
	public MobSpawnerLogicBetweenlands setEntitySpawnList(List<WeightedSpawnerEntity> entitySpawnList) {
		...
		if (!this.entitySpawnList.isEmpty()) {
			this.setNextEntity((WeightedSpawnerEntity) WeightedRandom.getRandomItem(this.getSpawnerWorld().rand,
					this.entitySpawnList));
		} else {
	...
	public boolean isActivated() {
		return this.getSpawnerWorld().getClosestPlayer((double) this.getSpawnerX() + 0.5D,
				...
				(double) this.activatingRangeFromPlayer, false) != null;
	}
	...
	public void updateSpawner() {
		if (!this.isActivated()) {
			...
			if (this.getSpawnerWorld().isRemote) {
				...
				this.lastEntityRotation = this.entityRotation;
				this.entityRotation = (this.entityRotation + (double) (1000.0F / ((float) this.spawnDelay + 200.0F)))
						% 360.0D;
			} else {
				...
				int spawnCount = this.spawnCount > 1 ? this.getSpawnerWorld().rand.nextInt(this.spawnCount) + 1
						: this.spawnCount;
				...
				for (int i = 0; i < 128; ++i) {
					...
					List<Entity> entitiesInReach = this.getSpawnerWorld().getEntitiesWithinAABB(entity.getClass(),
							new AxisAlignedBB((double) this.getSpawnerX(), (double) this.getSpawnerY(),
									(double) this.getSpawnerZ(), (double) (this.getSpawnerX() + 1),
									(double) (this.getSpawnerY() + 1), (double) (this.getSpawnerZ() + 1))
											.grow(this.checkRange, this.checkRange, this.checkRange));
					int nearbyEntities = 0;
					for (Entity e : entitiesInReach) {
						if (e.getDistance(this.getSpawnerX() + 0.5D, this.getSpawnerY() + 0.5D,
								this.getSpawnerZ() + 0.5D) <= this.checkRange) {
							nearbyEntities++;
						}
					}
					...
					if (!canSpawn) {
						...
						if (blockState.getBlock() != Blocks.AIR) {
							AxisAlignedBB boundingBox = blockState.getCollisionBoundingBox(this.getSpawnerWorld(),
									down);
							if (boundingBox != null) {
								boundingBox = boundingBox.offset(down);
								AxisAlignedBB entityBoundingBox = entity.getEntityBoundingBox();
								if (boundingBox.intersects(entityBoundingBox.minX, boundingBox.minY,
										entityBoundingBox.minZ, entityBoundingBox.maxX, boundingBox.maxY,
										entityBoundingBox.maxZ)) {
									RayTraceResult intercept = boundingBox.calculateIntercept(
											entity.getPositionVector(), entity.getPositionVector().add(0, -2, 0));
									if (intercept != null) {
										canSpawn = true;
										entity.setLocationAndAngles(entity.posX, intercept.hitVec.y + 0.1D, entity.posZ,
					...
					if (canSpawn) {
						...
						if (entityLiving == null || ForgeEventFactory.canEntitySpawnSpawner(entityLiving,
								getSpawnerWorld(), (float) entity.posX, (float) entity.posY, (float) entity.posZ)) {
							if (entityLiving != null) {
								if (!ForgeEventFactory.doSpecialSpawn(entityLiving, this.getSpawnerWorld(),
										(float) entity.posX, (float) entity.posY, (float) entity.posZ)) {
									((EntityLiving) entity).onInitialSpawn(
											this.getSpawnerWorld().getDifficultyForLocation(new BlockPos(entity)),
	...
	protected void spawnParticles() {
		...
		double len = Math.sqrt(rx * rx + ry * ry + rz * rz);
		BLParticles.SPAWNER.spawn(this.getSpawnerWorld(), (float) this.getSpawnerX() + rx,
				(float) this.getSpawnerY() + ry, (float) this.getSpawnerZ() + rz, ParticleFactory.ParticleArgs.get()
	...
	public Entity spawnEntity(Entity entity) {
		((EntityLiving) entity).onInitialSpawn(this.getSpawnerWorld().getDifficultyForLocation(new BlockPos(entity)),
				(IEntityLivingData) null);
