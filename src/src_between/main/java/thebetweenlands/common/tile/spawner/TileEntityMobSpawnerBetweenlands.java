...
public class TileEntityMobSpawnerBetweenlands extends TileEntity implements ITickable {
	...
	private final MobSpawnerLogicBetweenlands spawnerLogic = new MobSpawnerLogicBetweenlands() {
		...
		public void broadcastEvent(int eventID) {
			TileEntityMobSpawnerBetweenlands.this.world.addBlockEvent(TileEntityMobSpawnerBetweenlands.this.getPos(),
					BlockRegistry.MOB_SPAWNER, eventID, 0);
		}
		...
		protected void spawnParticles() {
			if (this.getSpawnerWorld().rand.nextInt(2) == 0) {
				...
				BLParticles.SPAWNER.spawn(this.getSpawnerWorld(), (float) this.getSpawnerX() + rx,
						(float) this.getSpawnerY() + ry, (float) this.getSpawnerZ() + rz,
						ParticleFactory.ParticleArgs.get()
								.withMotion((rx - 0.5D) / len * 0.05D, (ry - 0.5D) / len * 0.05D,
										(rz - 0.5D) / len * 0.05D)
								.withColor(1.0F, MathHelper.clamp(4 + (float) Math.sin(counter) * 3, 0, 1),
										MathHelper.clamp((float) Math.sin(counter) * 2, 0, 1), 0.65F));
			}
		}
