...
public class TileEntityDungeonDoorRunes extends TileEntity implements ITickable, IEntityScreenShake {
	...
	private final ItemStack renderStack = new ItemStack(
			BlockRegistry.MUD_TOWER_BEAM_RELAY.getDefaultState().getBlock());
		...
		if (getWorld().isRemote) {
			...
			for (int i = 0, amount = 2 + getWorld().rand.nextInt(2); i < amount; i++) {
				...
				world.spawnParticle(EnumParticleTypes.BLOCK_DUST, px + ox, py, pz + oz, motionX, motionY, motionZ,
						Block.getStateId(BlockRegistry.MUD_TILES.getDefaultState()));
			}
		}
	}
	...
	private void spawnCrashingParticles(BlockPos pos, float ySpikeVel) {
		if (getWorld().isRemote) {
			...
			for (int i = 0, amount = 2 + getWorld().rand.nextInt(2); i < amount; i++) {
				...
				double motionZ = getWorld().rand.nextDouble() * 0.2F - 0.1F;
				world.spawnParticle(EnumParticleTypes.BLOCK_DUST, px + ox, py, pz + oz, motionX, motionY + ySpikeVel,
						motionZ, Block.getStateId(BlockRegistry.MUD_BRICKS.getDefaultState()));
				...
				if (slate_1_rotate <= 0)
					if (!getWorld().isRemote) {
						...
						if (barrishee) {
							...
							entity.setLocationAndAngles(offsetPos.getX() + 0.5D, offsetPos.down().getY(),
									offsetPos.getZ() + 0.5D, 0F, 0.0F);
				...
				if (slate_1_rotate == 0)
					if (!getWorld().isRemote) {
						...
						if (is_gate_entrance) {
							...
							lightTowerBuild.destroyTowerBeamLenses(getWorld(), getPos().add(-15, -2, -14)); // centre
																											// stone of
																											// tower
																											// bottom
																											// floor
	...
	private void playOpenRecessSound(boolean isBigDoor) {
		getWorld().playSound(null, getPos(), SoundRegistry.MUD_DOOR_1, SoundCategory.BLOCKS, isBigDoor ? 1F : 0.5F,
				1.0F);
	}
