...
public class TileEntityDruidAltar extends TileEntityBasicInventory implements ITickable {
	...
	public void update() {
		...
		if (this.world.isRemote) {
			...
			this.renderYOffset = (float) ((double) this.craftingProgress / (double) TileEntityDruidAltar.CRAFTING_TIME
					* (FINAL_HEIGHT - 0.2D) + 1.2D);
		} else {
			if (this.craftingProgress != 0) {
				IDruidAltarRecipe recipe = DruidAltarRecipe.getDruidAltarRecipe(this.inventory.get(1),
						this.inventory.get(2), this.inventory.get(3), this.inventory.get(4));
				...
				if (recipe != null) {
					recipe.onCrafting(this.world, this.pos, new ItemStack[] { this.inventory.get(1),
							this.inventory.get(2), this.inventory.get(3), this.inventory.get(4) });
					if (this.craftingProgress >= CRAFTING_TIME) {
						ItemStack stack = recipe.getOutput(new ItemStack[] { this.inventory.get(1),
								this.inventory.get(2), this.inventory.get(3), this.inventory.get(4) }).copy();
						...
						stopCraftingProcess();
						recipe.onCrafted(this.world, this.pos, new ItemStack[] { this.inventory.get(1),
								this.inventory.get(2), this.inventory.get(3), this.inventory.get(4) }, stack);
					}
				}
			}
		}
	...
	public void setInventorySlotContents(int slot, ItemStack stack) {
		...
		IDruidAltarRecipe recipe = DruidAltarRecipe.getDruidAltarRecipe(this.inventory.get(1), this.inventory.get(2),
				this.inventory.get(3), this.inventory.get(4));
		if (!this.world.isRemote && recipe != null && !stack.isEmpty() && this.inventory.get(0).isEmpty()
				&& this.craftingProgress == 0) {
			recipe.onStartCrafting(world, pos, new ItemStack[] { this.inventory.get(1), this.inventory.get(2),
					this.inventory.get(3), this.inventory.get(4) });
	...
	private void startCraftingProcess() {
		...
		this.world.setBlockState(this.pos,
				this.world.getBlockState(this.pos).withProperty(BlockDruidAltar.ACTIVE, true), 3);
		this.craftingProgress = 1;
		// Packet to start sound
		TheBetweenlands.networkWrapper.sendToAllAround(new MessageDruidAltarProgress(this, -1),
				new NetworkRegistry.TargetPoint(dim, this.pos.getX() + 0.5D, this.pos.getY() + 0.5D,
	...
	private void stopCraftingProcess() {
		...
		int dim = world.provider.getDimension();
		this.world.setBlockState(this.pos,
				this.world.getBlockState(this.pos).withProperty(BlockDruidAltar.ACTIVE, false), 3);
		this.craftingProgress = 0;
		// Packet to cancel sound
		TheBetweenlands.networkWrapper.sendToAllAround(new MessageDruidAltarProgress(this, -2),
				new NetworkRegistry.TargetPoint(dim, this.pos.getX() + 0.5D, this.pos.getY() + 0.5D,
						this.pos.getZ() + 0.5D, 64D));
		// Sets client crafting progress to 0
		TheBetweenlands.networkWrapper.sendToAllAround(new MessageDruidAltarProgress(this, 0),
				new NetworkRegistry.TargetPoint(dim, this.pos.getX() + 0.5D, this.pos.getY() + 0.5D,
						this.pos.getZ() + 0.5D, 64D));
		// Does the metadata stuff for the circle animated textures
		checkDruidCircleBlocks(world);
		...
		List<LocationGuarded> locations = worldStorage.getLocalStorageHandler().getLocalStorages(LocationGuarded.class,
				this.pos.getX(), this.pos.getZ(), (location) -> {
	...
	public void sendCraftingProgressPacket() {
		World world = this.world;
		int dim = world.provider.getDimension();
		TheBetweenlands.networkWrapper.sendToAllAround(new MessageDruidAltarProgress(this),
				new NetworkRegistry.TargetPoint(dim, this.pos.getX() + 0.5D, this.pos.getY() + 0.5D,
						this.pos.getZ() + 0.5D, 64D));
	}
				...
				for (int z = -baseRadius; z <= baseRadius; z++) {
					...
					if (Math.round(Math.sqrt(dSq)) <= baseRadius) {
						...
						if (block instanceof BlockDruidStone) {
							if ((this.craftingProgress == 0 || this.circleShouldRevert)
									&& state.getValue(BlockDruidStone.ACTIVE)) {
	...
	public AxisAlignedBB getRenderBoundingBox() {
		return new AxisAlignedBB(this.pos.getX() - 1, this.pos.getY(), this.pos.getZ() - 1, this.pos.getX() + 2,
				this.pos.getY() + 3, this.pos.getZ() + 2);
	}
