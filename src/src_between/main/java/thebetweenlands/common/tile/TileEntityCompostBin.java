...
public class TileEntityCompostBin extends TileEntity implements ITickable, ISidedInventory {
	...
	@Override
	public void update() {
		this.lidAngle = this.open ? Math.min(this.lidAngle + OPEN_SPEED, MAX_OPEN)
				: Math.max(this.lidAngle - CLOSE_SPEED, MIN_OPEN);
	...
	public int addItemToBin(ItemStack stack, int compostAmount, int compostTime, boolean doSimulate) {
		int clampedAmount = this.getTotalCompostAmount() + compostAmount <= MAX_COMPOST_AMOUNT ? compostAmount
				: MAX_COMPOST_AMOUNT - this.getTotalCompostAmount();
		if (clampedAmount > 0) {
			for (int i = 0; i < this.inventory.size(); i++) {
				if (this.inventory.get(i).isEmpty()) {
					if (!doSimulate) {
	...
	public float getLidAngle(float partialTicks) {
		return this.open ? Math.min(this.lidAngle + OPEN_SPEED * partialTicks, MAX_OPEN)
				: Math.max(this.lidAngle - CLOSE_SPEED * partialTicks, MIN_OPEN);
	}
	...
	public boolean canInsertItem(int index, @Nonnull ItemStack itemStackIn, EnumFacing direction) {
		...
		return recipe != null && this.open && !itemStackIn.isEmpty() && direction == EnumFacing.UP
				&& addItemToBin(itemStackIn, recipe.getCompostAmount(itemStackIn),
						recipe.getCompostingTime(itemStackIn), true) == 1;
	}
	...
	public boolean isItemValidForSlot(int index, ItemStack stack) {
		ICompostBinRecipe recipe = CompostRecipe.getCompostRecipe(stack);
		return recipe != null
				&& addItemToBin(stack, recipe.getCompostAmount(stack), recipe.getCompostingTime(stack), true) == 1;
	}
