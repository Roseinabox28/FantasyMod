...
public class TileEntityBeamOrigin extends TileEntity implements ITickable {
		...
		if (getWorld().getTotalWorldTime() % 10 == 0) {
			if (checkForLitBrazier(getPos().add(3, -1, 3))) {
				...
				if (!world.isRemote)
					if (!beam_1_active) {
						setBeam1Active(true);
						getWorld().playSound((EntityPlayer) null, getPos().add(3, -1, 3), SoundRegistry.PORTAL_ACTIVATE,
								SoundCategory.BLOCKS, 0.125F, 0.25F);
					}
			} else {
				if (!world.isRemote)
					if (beam_1_active)
			...
			if (checkForLitBrazier(getPos().add(3, -1, -3))) {
				...
				if (!world.isRemote)
					if (!beam_2_active) {
						setBeam2Active(true);
						getWorld().playSound((EntityPlayer) null, getPos().add(3, -1, -3),
								SoundRegistry.PORTAL_ACTIVATE, SoundCategory.BLOCKS, 0.125F, 0.25F);
					}
			} else {
				if (!world.isRemote)
					if (beam_2_active)
	...
	private void spawnBrazierParticles(Vec3d target) {
		BatchedParticleRenderer.INSTANCE.addParticle(DefaultParticleBatches.BEAM,
				BLParticles.PUZZLE_BEAM_2.create(world, this.pos.getX() + 0.5 + target.x,
						this.pos.getY() + 0.5 + target.y, this.pos.getZ() + 0.5 + target.z,
						ParticleArgs.get().withMotion(0, 0, 0).withColor(255F, 102F, 0F, 1F).withScale(1.5F)
			...
			if (ShaderHelper.INSTANCE.canUseShaders() && world.rand.nextBoolean()) {
				...
				BatchedParticleRenderer.INSTANCE.addParticle(
						...
						BLParticles.PUZZLE_BEAM.create(world, this.pos.getX() + 0.5 + offset.x,
								this.pos.getY() + 0.5 + offset.y, this.pos.getZ() + 0.5 + offset.z,
								ParticleArgs.get().withMotion(vx, vy, vz).withColor(255F, 102F, 0F, 1F).withScale(scale)
										.withData(100)));
			}
		}
	}
	...
	private void spawnBeamParticles(Vec3d target) {
		BatchedParticleRenderer.INSTANCE.addParticle(DefaultParticleBatches.BEAM,
				BLParticles.PUZZLE_BEAM_2.create(world, this.pos.getX() + 0.5, this.pos.getY() + 0.5,
						this.pos.getZ() + 0.5, ParticleArgs.get().withMotion(0, 0, 0).withColor(40F, 220F, 130F, 1F)
								.withScale(2.5F).withData(30, target)));
		for (int i = 0; i < 3; i++) {
			float offsetLen = this.world.rand.nextFloat();
			Vec3d offset = new Vec3d(target.x * offsetLen + world.rand.nextFloat() * 0.2f - 0.1f,
					target.y * offsetLen + world.rand.nextFloat() * 0.2f - 0.1f,
	...
	public void activateBlock() {
		if (!world.isRemote) {
			...
			if (!state.getValue(BlockBeamOrigin.POWERED)) {
				getWorld().setBlockState(getPos(), BlockRegistry.MUD_TOWER_BEAM_ORIGIN.getDefaultState()
						.withProperty(BlockBeamOrigin.POWERED, true));
				getWorld().playSound((EntityPlayer) null, getPos(), SoundRegistry.BEAM_ACTIVATE, SoundCategory.BLOCKS,
						1F, 1F);
			}
		}
	...
	public void deactivateBlock() {
		...
		if (state.getValue(BlockBeamOrigin.POWERED)) {
			getWorld().setBlockState(getPos(),
					BlockRegistry.MUD_TOWER_BEAM_ORIGIN.getDefaultState().withProperty(BlockBeamOrigin.POWERED, false));
