...
public class TileEntityBarrel extends TileEntity implements IFluidHandler {
	...
	public int fill(FluidStack resource, boolean doFill) {
		if (this.world != null) {
			...
			boolean isFluidHot = resource.getFluid().getTemperature(resource) > 473.15F
					/* 200°C */ || resource.getFluid() == FluidRegistry.LAVA;

			if (!isFluidHot || (state.getBlock() instanceof BlockBarrel
					&& ((BlockBarrel) state.getBlock()).isHeatResistant(this.world, this.pos, state))) {
				if (doFill) {
					this.markDirty();
