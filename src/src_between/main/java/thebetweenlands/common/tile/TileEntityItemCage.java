...
public class TileEntityItemCage extends TileEntity implements ITickable {
	...
	protected Entity isBlockOccupied() {
		List<EntityLivingBase> list = world.getEntitiesWithinAABB(EntityLivingBase.class,
				new AxisAlignedBB(pos.getX() + 0.25D, pos.getY() - 3D, pos.getZ() + 0.25D, pos.getX() + 0.75D,
						pos.getY(), pos.getZ() + 0.75D));
	...
	public Entity isSwordEnergyBelow() {
		List<EntitySwordEnergy> list = world.getEntitiesWithinAABB(EntitySwordEnergy.class,
				new AxisAlignedBB(pos.getX() - 9D, pos.getY() - 2D, pos.getZ() - 9D, pos.getX() + 10D, pos.getY() + 3D,
						pos.getZ() + 10D));
		for (int i = 0; i < list.size(); i++) {
