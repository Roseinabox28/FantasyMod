...
public class TileEntityDugSoil extends TileEntity {
		...
		if (wasComposted != this.isComposted()) {
			...
			if (!this.isFullyDecayed()) {
				this.world.setBlockState(this.pos,
						blockState.withProperty(BlockGenericDugSoil.DECAYED, this.isFullyDecayed())
								.withProperty(BlockGenericDugSoil.COMPOSTED, this.isComposted()),
						3);
			} else {
				this.world.setBlockState(this.pos,
						blockState.withProperty(BlockGenericDugSoil.DECAYED, this.isFullyDecayed())
								.withProperty(BlockGenericDugSoil.COMPOSTED, false),
						3);
			}
		} else {
	...
	public void setDecay(int decay) {
		...
		if (wasDecayed != this.isFullyDecayed()) {
			IBlockState blockState = this.world.getBlockState(this.pos);
			if (this.isFullyDecayed()) {
				blockState = blockState.withProperty(BlockGenericDugSoil.DECAYED, this.isFullyDecayed())
						.withProperty(BlockGenericDugSoil.COMPOSTED, false);
			} else {
				blockState = blockState.withProperty(BlockGenericDugSoil.DECAYED, false)
						.withProperty(BlockGenericDugSoil.COMPOSTED, this.isComposted());
			}
			...
			if (blockUp.getBlock() instanceof BlockGenericCrop) {
				...
				for (int i = 0; i < ((BlockGenericCrop) blockUp.getBlock()).getMaxHeight(); i++) {
					...
					if (cropBlockState.getBlock() instanceof BlockGenericCrop) {
						if (this.isFullyDecayed()) {
							this.world.setBlockState(pos, cropBlockState.withProperty(BlockGenericCrop.DECAYED, true),
									3);
						} else {
							this.world.setBlockState(pos, cropBlockState.withProperty(BlockGenericCrop.DECAYED, false),
									3);
						}
					} else {
						break;
