	...
	public void update() {
		if (!this.world.isRemote) {
			...
			if (this.fuel < 0) {
				...
				AxisAlignedBB affectedBB = new AxisAlignedBB(this.pos.getX() - this.radius - 5.0F,
						this.pos.getY() - this.radius - 5.0F, this.pos.getZ() - this.radius - 5.0F,
						this.pos.getX() + this.radius + 5.0F, this.pos.getY() + this.radius + 5.0F,
						this.pos.getZ() + this.radius + 5.0F);
				List<Entity> affectedEntities = this.world.getEntitiesWithinAABB(Entity.class, affectedBB);
				for (Entity entity : affectedEntities) {
					if (entity instanceof IMob && entity instanceof EntityWight == false
							&& entity instanceof IBLBoss == false) {
						Vec3d closestPoint = this.getClosestAABBCorner(entity.getEntityBoundingBox(), centerX, centerY,
								centerZ);
						if (closestPoint.squareDistanceTo(centerX, centerY, centerZ) < this.radius * this.radius) {
							...
							if (entity instanceof EntityLivingBase) {
								((EntityLivingBase) entity)
										.addPotionEffect(new PotionEffect(MobEffects.SLOWNESS, 10, 6));
							}
					...
					if (entity instanceof IProjectile || entity instanceof EntityFireball) {
						...
						if (closestPoint.squareDistanceTo(centerX, centerY, centerZ) < this.radius * this.radius) {
							double velocity = Math.sqrt(entity.motionX * entity.motionX
									+ entity.motionY * entity.motionY + entity.motionZ * entity.motionZ);
							double diffX = entity.posX - centerX;
							double diffY = entity.posY - centerY;
							double diffZ = entity.posZ - centerZ;
							double len = Math.sqrt(diffX * diffX + diffY * diffY + diffZ * diffZ);
		...
		if (this.running && this.radius < desiredRadius && this.deployTicks < DEPLOY_TIME) {
			...
			this.deployTicks++;
			this.radius = (float) this.easeInOut(this.deployTicks, !this.running ? desiredRadius : this.radius,
					-desiredRadius, DEPLOY_TIME);
