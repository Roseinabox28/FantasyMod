...
public class TileEntityMortar extends TileEntityBasicInventory implements ITickable {
	...
	public void update() {
		...
		if (pestleInstalled() && !outputIsFull()) {
			...
			if (isCrystalInstalled() && getStackInSlot(3).getItemDamage() < getStackInSlot(3).getMaxDamage()
					|| manualGrinding) {
				if ((!output.isEmpty() && inventory.get(2).isEmpty())
						|| (!output.isEmpty() && inventory.get(2).isItemEqual(output)
								&& inventory.get(2).getCount() + output.getCount() <= output.getMaxStackSize())) {
					progress++;
					if (progress == 1)
						world.playSound(null, getPos().getX() + 0.5F, getPos().getY() + 0.5F, getPos().getZ() + 0.5F,
								SoundRegistry.GRIND, SoundCategory.BLOCKS, 1F, 1F);
					if (progress == 64 || progress == 84) {
						world.playSound(null, getPos().getX() + 0.5F, getPos().getY() + 0.5F, getPos().getZ() + 0.5F,
								SoundEvents.BLOCK_GRASS_BREAK, SoundCategory.BLOCKS, 0.3F, 1F);
						world.playSound(null, getPos().getX() + 0.5F, getPos().getY() + 0.5F, getPos().getZ() + 0.5F,
								SoundEvents.BLOCK_STONE_BREAK, SoundCategory.BLOCKS, 0.3F, 1F);
					}
	...
	public boolean isCrystalInstalled() {
		return !getStackInSlot(3).isEmpty() && getStackInSlot(3).getItem() == ItemRegistry.LIFE_CRYSTAL
				&& getStackInSlot(3).getItemDamage() <= getStackInSlot(3).getMaxDamage();
	}
	...
	public boolean isItemValidForSlot(int slot, ItemStack itemstack) {
		return slot == 1 && itemstack.getItem() == ItemRegistry.PESTLE
				|| slot == 3 && itemstack.getItem() == ItemRegistry.LIFE_CRYSTAL
				|| slot != 1 && itemstack.getItem() != ItemRegistry.PESTLE && slot != 3
						&& itemstack.getItem() != ItemRegistry.LIFE_CRYSTAL;
	}
