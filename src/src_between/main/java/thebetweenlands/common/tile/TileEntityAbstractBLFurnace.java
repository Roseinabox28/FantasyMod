...
public abstract class TileEntityAbstractBLFurnace extends TileEntityBasicInventory
	 ...
	 * @param furnaceAmount The amount of furnaces, each one have (input, output,
	 *                      fuel & flux)
	 */
	...
	public static boolean isItemFlux(ItemStack itemstack) {
		return itemstack.getItem() == ItemRegistry.ITEMS_MISC
				&& itemstack.getItemDamage() == EnumItemMisc.LIMESTONE_FLUX.getID();
	}
	...
	public void readFromNBT(NBTTagCompound nbt) {
		...
		for (FurnaceData data : furnaceData) {
			if (nbt.hasKey(NBT_BURN_TIME, Constants.NBT.TAG_SHORT)
					&& nbt.hasKey(NBT_COOK_TIME, Constants.NBT.TAG_SHORT)) {
	...
	public void update() {
		...
		for (FurnaceData data : furnaceData) {
			...
			if (!world.isRemote) {
				...
				if (data.furnaceBurnTime != 0
						|| !fuelStack.isEmpty() && !getStackInSlot(data.getInputSlot()).isEmpty()) {
	...
	private boolean canSmelt(FurnaceData data) {
		...
		if (inputStack.isEmpty())
			...
			return result <= getInventoryStackLimit() && result <= outputStack.getMaxStackSize(); // Forge BugFix: Make
																									// it respect stack
																									// sizes properly.
		}
	}
		...
		if (canSmelt(data)) {
			...
			if (ItemRegistry.isIngotFromOre(inputStack, outputStack)) {
				...
				if (!fluxStack.isEmpty()) {
					...
					if (useFlux && outputStack.getCount() + 1 <= getInventoryStackLimit()
