...
public class TileEntityAlembic extends TileEntity implements ITickable {
	...
	private void loadFromInfusion() {
		this.recipe = null;
		if (this.infusionBucket.isEmpty() || infusionBucket.getTagCompound() == null)
			return;
		int infusionTime = this.infusionBucket.getTagCompound().getInteger("infusionTime");
		this.bucketInfusionTime = infusionTime;
		if (this.infusionBucket.getTagCompound() == null
				|| !this.infusionBucket.getTagCompound().hasKey("ingredients")) {
		...
		this.recipe = recipe;
		if (recipe == null || infusionTime < recipe.idealInfusionTime - recipe.infusionTimeVariation
		...
		if (isPositive) {
			this.producableDuration = recipe.baseDuration
					+ MathHelper.floor(recipe.durationModifier * relDurationAmount);
		} else {
			this.producableDuration = recipe.negativeBaseDuration
					+ MathHelper.floor(recipe.negativeDurationModifier * relDurationAmount);
		}
	}
	...
	private void addInvalidInfusion() {
		...
		this.producableItemAspects.clear();
		if (!this.infusionBucket.isEmpty() && this.infusionBucket.getTagCompound() != null
				&& this.infusionBucket.getTagCompound().hasKey("ingredients")) {
			NBTTagList nbtList = (NBTTagList) this.infusionBucket.getTagCompound().getTag("ingredients");
			List<ItemStack> infusionIngredients = new ArrayList<ItemStack>();
			for (int i = 0; i < nbtList.tagCount(); i++) {
			...
			for (Aspect aspect : infusionAspects) {
				this.producableItemAspects.add(new Aspect(aspect.type,
						MathHelper.floor((aspect.amount * (1.0F - ISOLATION_LOSS_MULTIPLIER)) / 3.0F)));
			}
		}
	}
	...
	public List<IAspectType> getInfusionAspects(List<ItemStack> ingredients) {
		...
		for (ItemStack ingredient : ingredients) {
			...
			// infusingAspects.addAll(AspectManager.get(this.world).getDiscoveredAspectTypes(AspectManager.getAspectItem(ingredient),
			// null));
		}
	...
	private List<Aspect> getInfusionItemAspects(List<ItemStack> ingredients) {
		...
		for (ItemStack ingredient : ingredients) {
			...
			// infusingItemAspects.addAll(AspectManager.get(this.world).getDiscoveredAspects(AspectManager.getAspectItem(ingredient),
			// null));
		}
	...
	/**
	 * Creates an item stack with the elixir in the alembic. Vial types: 0 = green,
	 * 1 = orange
	 *
	 * @param vialType
	 * @return
