...
public class TileEntityDecayPitHangingChain extends TileEntity implements ITickable {
	...
	public void update() {
		...
		if (getEntityCollidedWithChains(getHangingLengthCollision(1, 0, 2F + getProgress() * MOVE_UNIT)) != null)
			checkCollisions(
					getEntityCollidedWithChains(getHangingLengthCollision(1, 0, 2F + getProgress() * MOVE_UNIT)));
		...
		if (getEntityCollidedWithChains(getHangingLengthCollision(0, 1, 2F + getProgress() * MOVE_UNIT)) != null)
			checkCollisions(
					getEntityCollidedWithChains(getHangingLengthCollision(0, 1, 2F + getProgress() * MOVE_UNIT)));
	...
	private void checkCollisions(List<Entity> list) {
		for (Entity entity : list) {
			if (entity instanceof EntityArrow) { // just arrows for now
				...
				getWorld().playSound((EntityPlayer) null, arrow.posX, arrow.posY, arrow.posZ,
						SoundEvents.BLOCK_ANVIL_LAND, SoundCategory.BLOCKS, 0.5F, 3F);
				// this.ticksInAir = 0;
			}
		}
	}
	...
	public AxisAlignedBB getHangingLengthCollision(double offX, double offZ, float extended) {
		return new AxisAlignedBB(getPos().getX() + offX + 0.1875D, getPos().getY() - extended,
				getPos().getZ() + offZ + 0.1875D, getPos().getX() + offX + 0.8125D, getPos().getY(),
				getPos().getZ() + offZ + 0.8125D);
	}
