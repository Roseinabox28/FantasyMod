...
public class TileEntityPossessedBlock extends TileEntity implements ITickable {
	...
	public void update() {
		if (!world.isRemote) {
			...
			if (active) {
				activateBlock();
				if (animationTicks == 0)
					world.playSound(null, getPos(), SoundRegistry.POSSESSED_SCREAM, SoundCategory.BLOCKS, 0.25F,
							1.25F - this.world.rand.nextFloat() * 0.5F);
	...
	private void spawnParticles() {
		...
		BLParticles.SMOKE.spawn(world, (double) (xx - randomOffset), (double) (yy + randomOffset),
	...
	protected Entity findEnemyToAttack() {
		...
		List<EntityLivingBase> list = world.getEntitiesWithinAABB(EntityLivingBase.class, new AxisAlignedBB(
				pos.getX() + x, pos.getY(), pos.getZ() + z, pos.getX() + 1D + x, pos.getY() + 1D, pos.getZ() + 1D + z));
		...
		if (animationTicks == 1)
			for (int i = 0; i < list.size(); i++) {
				...
				if (entity != null)
					if (entity instanceof EntityPlayer) {
						int Knockback = 4;
						entity.addVelocity(MathHelper.sin(entity.rotationYaw * 3.141593F / 180.0F) * Knockback * 0.2F,
								0.3D, -MathHelper.cos(entity.rotationYaw * 3.141593F / 180.0F) * Knockback * 0.2F);
