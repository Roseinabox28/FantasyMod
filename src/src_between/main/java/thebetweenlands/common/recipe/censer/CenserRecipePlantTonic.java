				...
				for (int cz = sz; cz <= ez; cz++) {
					...
					if (chunk != null) {
						...
						for (Entry<BlockPos, TileEntity> entry : tiles.entrySet()) {
							...
							if (te instanceof TileEntityDugSoil && tePos.getX() >= pos.getX() - range
									&& tePos.getX() <= pos.getX() + range && tePos.getY() >= pos.getY() - verticalRange
									&& tePos.getY() <= pos.getY() + verticalRange && tePos.getZ() >= pos.getZ() - range
									&& tePos.getZ() <= pos.getZ() + range) {

								if (((TileEntityDugSoil) te).getDecay() > 5) {
									((TileEntityDugSoil) te).setDecay(0);

									TheBetweenlands.networkWrapper.sendToAllAround(
											new MessageCureDecayParticles(tePos.up()),
											new TargetPoint(world.provider.getDimension(), tePos.getX(), tePos.getY(),
													tePos.getZ(), 32));
