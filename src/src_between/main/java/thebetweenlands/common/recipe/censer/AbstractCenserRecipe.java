	...
	public void getLocalizedEffectText(T context, ICenser censer, List<String> tooltip) {
		...
		if (consumptionAmount > 0) {
			...
			tooltip.addAll(ItemTooltipHandler.splitTooltip(
					TextFormatting.DARK_PURPLE + I18n.translateToLocalFormatted("tooltip.bl.censer.remaining_duration",
							amountLeft, MathHelper.ceil(approxRemainingTicks), MathHelper.ceil(approxRemainingSeconds),
							MathHelper.ceil(approxRemainingMinutes),
							StringUtils.ticksToElapsedTime(MathHelper.ceil(approxRemainingTicks))),
