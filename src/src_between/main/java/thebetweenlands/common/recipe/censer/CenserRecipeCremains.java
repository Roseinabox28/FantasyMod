...
public class CenserRecipeCremains extends AbstractCenserRecipe<Void> {
		...
		if (effectStrength > 0.01F && ShaderHelper.INSTANCE.isWorldShaderActive()) {
			...
			ShaderHelper.INSTANCE.getWorldShader()
					.addGroundFogVolume(new GroundFogVolume(new Vec3d(fogArea.minX, fogArea.minY, fogArea.minZ),
							...
							inScattering, extinction, fogBrightness * 0.7F, fogBrightness * 0.2F,
							fogBrightness * 0.1F));
		}
	}
	...
	private List<EntityLivingBase> getAffectedEntities(World world, BlockPos pos) {
		return world.getEntitiesWithinAABB(EntityLivingBase.class,
				new AxisAlignedBB(pos).grow(6, 0.1D, 6).expand(0, 12, 0));
	}
	...
	public int update(Void context, ICenser censer) {
		...
		if (!world.isRemote && world.getTotalWorldTime() % 10 == 0) {
			...
			for (EntityLivingBase living : affected) {
				if (!living.isInWater() && !living.isEntityInvulnerable(DamageSource.IN_FIRE)
						&& !living.isImmuneToFire()
						&& (living instanceof EntityPlayer == false || !((EntityPlayer) living).isCreative())) {
					living.setFire(1);
				}
			}
		}
	...
	public static void onLivingDrops(LivingDropsEvent event) {
					...
					for (Entry<BlockPos, TileEntity> entry : chunk.getTileEntityMap().entrySet()) {
						...
						if (tile instanceof TileEntityCenser) {
							...
							if (censer.isRecipeRunning()
									&& ((ICenserRecipe<?>) censer.getCurrentRecipe()) instanceof CenserRecipeCremains) {
		...
		if (isDeathFromCenser) {
			...
			for (int i = 0; i < cremains; i++) {
				event.getDrops().add(new EntityItem(entity.world, entity.posX, entity.posY + entity.height / 2,
						entity.posZ, EnumItemMisc.CREMAINS.create(1)));
			}
		}
	}
