...
public class CenserRecipeAspect extends AbstractCenserRecipe<CenserRecipeAspectContext> {
	...
	@Override
	public void render(CenserRecipeAspectContext context, ICenser censer, double x, double y, double z,
			float partialTicks) {
		...
		if (effectStrength > 0.01F && ShaderHelper.INSTANCE.isWorldShaderActive()) {
			...
			ShaderHelper.INSTANCE.getWorldShader()
					.addGroundFogVolume(new GroundFogVolume(new Vec3d(fogArea.minX, fogArea.minY, fogArea.minZ),
							...
							inScattering, extinction, fogBrightness * r, fogBrightness * g, fogBrightness * b));
		}
	}
