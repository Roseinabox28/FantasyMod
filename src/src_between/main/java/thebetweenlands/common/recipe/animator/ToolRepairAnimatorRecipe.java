...
public class ToolRepairAnimatorRecipe implements IAnimatorRecipe {
	...
	public ToolRepairAnimatorRecipe(Item tool, int minRepairLifeCost, int fullRepairLifeCost, int minRepairFuelCost,
			int fullRepairFuelCost) {
		this.tool = tool;
	...
	public boolean matchesInput(ItemStack stack) {
		return !stack.isEmpty() && stack.getItemDamage() > 0 && stack.getItem() == this.tool
				&& (this.tool instanceof IAnimatorRepairable == false
						|| ((IAnimatorRepairable) this.tool).isRepairableByAnimator(stack));
	}
	...
	public int getRequiredFuel(ItemStack stack) {
		...
		return minRepairFuelCost + MathHelper.ceil((fullRepairFuelCost - minRepairFuelCost)
				/ (float) stack.getMaxDamage() * (float) stack.getItemDamage());
	}
	...
	public int getRequiredLife(ItemStack stack) {
		...
		return minRepairLifeCost + MathHelper.ceil((fullRepairLifeCost - minRepairLifeCost)
				/ (float) stack.getMaxDamage() * (float) stack.getItemDamage());
	}
