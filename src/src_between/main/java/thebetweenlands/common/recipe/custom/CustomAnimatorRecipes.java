...
public class CustomAnimatorRecipes extends CustomRecipes<IAnimatorRecipe> {
	public CustomAnimatorRecipes() {
		super("animator",
				ImmutableMap.of("input/item", RecipeArg.ITEM_INPUT, "input/fuel", RecipeArg.INT, "input/life",
						RecipeArg.INT),
				ImmutableMap.of("output", RecipeArg.ITEM_OUTPUT, "output_entity", RecipeArg.ENTITY, "rendered_entity",
						RecipeArg.STRING));
	}
	...
	public IAnimatorRecipe load() {
		...
		if (outputEntity.isPresent()) {
			...
			if (renderedEntity.isPresent()) {
				EntityEntry entry = ForgeRegistries.ENTITIES
						.getValue(new ResourceLocation(renderedEntity.get().create()));
				if (entry != null)
