...
public class RecipeSapSpitCleanTool extends IForgeRegistryEntry.Impl<IRecipe> implements IRecipe {
	...
	public ItemStack getCraftingResult(InventoryCrafting crafter) {
		...
		corrodible.setCorrosion(tool, Math.max(0,
				corrodible.getCorrosion(tool) - MathHelper.ceil(sap * corrodible.getMaxCorrosion(tool) / 3.0F)));
		return tool;
	}
		...
		for (int i = 0; i < remaining.size(); ++i) {
			...
			if (!stack.isEmpty()) {
				if (stack.getItem() instanceof ICorrodible) {
					...
					requiredSap += MathHelper
							.ceil(corrodible.getCorrosion(stack) / (corrodible.getMaxCorrosion(stack) / 3.0F));
				}
			}
		}
