...
public class AnimatorRecipe implements IAnimatorRecipe {
	...
	public AnimatorRecipe(ItemStack input, int requiredFuel, int requiredLife, ItemStack result,
			Class<? extends Entity> resultEntity) {
		this(input, requiredFuel, requiredLife);
	...
	public Entity getRenderEntity(ItemStack stack) {
		if (this.renderEntity != null && this.renderEntity.toString().length() > 0) {
			if (this.renderEntityInstance == null) {
				this.renderEntityInstance = EntityList.createEntityByIDFromName(this.renderEntity,
	...
	/**
	 * Called when the animator has finished animating and is right clicked. Return
	 * true if GUI should be opened on first click
	 * 
	 * @param world
	 * @param pos
	 */
	...
	public boolean onRetrieved(World world, BlockPos pos, ItemStack stack) {
		...
		if (te instanceof TileEntityAnimator) {
			...
			if (spawnEntity != null) {
				Entity entity = null;
				try {
					entity = (Entity) spawnEntity.getConstructor(new Class[] { World.class })
	...
	public boolean matchesInput(ItemStack stack) {
		return this.input.getItemDamage() == OreDictionary.WILDCARD_VALUE ? this.input.getItem() == stack.getItem()
				: this.input.getItem() == stack.getItem() && this.input.getItemDamage() == stack.getItemDamage();
	}
