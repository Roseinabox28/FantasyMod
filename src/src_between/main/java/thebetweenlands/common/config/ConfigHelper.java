...
public class ConfigHelper {
	...
	public static void init() {
		...
		if (remap != null) {
			...
			final File backupFile = new File(oldConfig.getConfigFile().getParentFile(),
					"config (" + (configVersion == null ? "no version" : configVersion) + ").cfg.backup");
			Configuration backup = ConfigRemapper
					.clear(new Configuration(backupFile, oldConfig.getDefinedConfigVersion()));
	...
	private static void initProperties(Class<?> cls, @Nullable Object inst) {
		for (Field f : cls.getDeclaredFields()) {
			...
			} else if (!FieldWrapper.hasWrapperFor(f) && f.getType().getSuperclass() != null
					&& f.getType().getSuperclass().equals(Object.class)) {
