...
public class PortalTargetList extends ConfigProperty {
	...
	protected void init() {
		...
		for (String listed : unparsed) {
			try {
				...
				if (this.customPortalBlockMap.get(block) == null
						|| !this.customPortalBlockMap.get(block).containsKey(OreDictionary.WILDCARD_VALUE)) {
					int meta = 0;
					if (data.length > 2) {
						...
						if (!"*".equals(metaStr)) {
							try {
								...
								TheBetweenlands.logger.error("Failed to parse custom portal entry block: " + listed
										+ ". Invalid metadata: " + metaStr);
								continue;
							}
						} else {
					...
					if (dimData.length < 2) {
						TheBetweenlands.logger
								.error("Failed to parse custom portal entry, missing dimension: " + listed);
						continue;
					}
					int dim;
					try {
	...
	/**
	 * Returns the target dimension for the specified block or the default return
	 * dimension set in the config
	 * 
