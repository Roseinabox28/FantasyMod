...
public abstract class ConfigRemapper implements Comparable<ConfigRemapper> {
	 ...
	 * @return Returns the remapped config and version, or null if nothing was
	 *         remapped
	 */
	@Nullable
	public static Pair<Configuration, String> remap(final Configuration oldConfig, final Configuration newConfig,
		...
		try {
			...
			for (int i = 0; i < sortedRemappers.size(); i++) {
				...
				remapLoop: for (ConfigRemapper remapper : sortedRemappers) {
					if ((workingConfigVersion == NO_VERSION && remapper.acceptedVersions == NO_VERSION)
							|| remapper.acceptedVersions.containsVersion(workingConfigVersion)) {
						TheBetweenlands.logger.info(String.format(
								"### Config remapper '%s' (remaps to '%s') accepted config with version '%s'",
								remapper.getClass().getName(), remapper.outputVersion, workingConfigVersionString));

						Configuration remappedValues = new Configuration();
						if (remapConfig(remapper, workingConfig, remappedValues, propertiesToRemove)) {
							TheBetweenlands.logger.info(String.format("### Remapped config version '%s' to '%s'",
									workingConfigVersionString, remapper.outputVersion));

							removeExclusions(remappedValues, propertiesToRemove);

							workingConfig = mergeRemaps(workingConfig, remappedValues, propertiesToRemove,
									remapper.outputVersion);
			...
			if (workingConfig != oldConfig) {
				TheBetweenlands.logger.info(
						String.format("### Remapped values for config with version '%s':", workingConfigVersionString));
	...
	/**
	 * Copies all properties of the source config that are not in the exclusions
	 * into the target config.
	 * <p>
	 * The format of the exclusions is as follows:
	 * <p>
	 * <ul>
	...
	public static void copy(Configuration source, Configuration target, Set<String> exclusions) {
		traverseConfig(source, sourceCategory -> {
	...
	private static void printConfig(Configuration config) {
		...
		traverseConfig(config, category -> true, (category, property) -> {
	...
	private static void traverseConfig(Configuration config, Function<ConfigCategory, Boolean> categoryTraverser,
			BiConsumer<ConfigCategory, Property> propertyTraverser) {
	...
	private static void traverseCategory(ConfigCategory category, Function<ConfigCategory, Boolean> categoryTraverser,
			BiConsumer<ConfigCategory, Property> propertyTraverser) {
	...
	private static void removeExclusions(Configuration config, Set<String> exclusions) {
		traverseConfig(config, category -> {
	...
	private static boolean remapConfig(ConfigRemapper remapper, Configuration inputConfig, Configuration remappedValues,
			Set<String> remappedProperties) {
		final AtomicBoolean ret = new AtomicBoolean(false);

		traverseConfig(inputConfig, category -> true, (category, property) -> {
			...
			if (remapper.remap(categoryName, property, remappedValues)) {
				...
				TheBetweenlands.logger
						.info(String.format("Remapped property '%s'", categoryName + "/" + property.getName()));
			}
		});
	...
	private static Configuration mergeRemaps(Configuration inputConfig, Configuration remappedValues,
			Set<String> remappedProperties, String newVersion) {
	...
	private static boolean overrideValues(Configuration config, Configuration remappedValues) {
		...
		traverseConfig(remappedValues, remappedCategory -> true,
				(remappedCategory, remappedProperty) -> flattenedRemappedValues
						.put(remappedCategory.getQualifiedName() + "/" + remappedProperty.getName(), remappedProperty));

		traverseConfig(config, category -> true, (category, property) -> {
			Property remappedProperty = flattenedRemappedValues
					.get(category.getQualifiedName() + "/" + property.getName());
	...
	public ConfigRemapper(@Nullable String acceptedVersions, String outputVersion) {
		...
		this.acceptedVersionsComparator = acceptedVersions == null ? NO_VERSION
				: parseVersionRangeComparable(acceptedVersions);
