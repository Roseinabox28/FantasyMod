...
public class AmbienceRegistry {
	...
	public static void preInit() {
		...
		// Locations
		AmbienceManager.INSTANCE.registerAmbience(
				new LocationAmbienceType(EnumLocationAmbience.WIGHT_TOWER, SoundRegistry.AMBIENT_WIGHT_FORTRESS) {
					@Override
					public boolean isActive() {
						return super.isActive()
								&& !this.getAmbience().getLocation().getName().equals("wight_tower_boss");
					}
				});
		AmbienceManager.INSTANCE.registerAmbience(
				new LocationAmbienceType(EnumLocationAmbience.WIGHT_TOWER, SoundRegistry.AMBIENT_WIGHT_FORTRESS) { // Silences
																													// the
																													// other
																													// ambient
																													// tracks
		...
		AmbienceManager.INSTANCE.registerAmbience(new LocationAmbienceType(EnumLocationAmbience.SLUDGE_WORM_DUNGEON,
				SoundRegistry.AMBIENT_SLUDGE_WORM_DUNGEON) {
			@Override
			public boolean isActive() {
				if (super.isActive()) {
					...
					if (ambience != null) {
						...
						if (location instanceof LocationSludgeWormDungeon) {
							return this.getPlayer().getPositionEyes(1).y < ((LocationSludgeWormDungeon) location)
									.getStructurePos().getY();
						}
					}
				}
				return false;
			...
			private float getProgressiveVolume() {
				...
				if (ambience != null) {
					LocationStorage location = ambience.getLocation();
					if (location instanceof LocationSludgeWormDungeon) {
						double dist = ((LocationSludgeWormDungeon) location).getStructurePos().getY()
								- this.getPlayer().getPositionEyes(1).y;
		...
		AmbienceManager.INSTANCE.registerAmbience(
				new LocationAmbienceType(EnumLocationAmbience.SLUDGE_WORM_DUNGEON, SoundRegistry.PIT_OF_DECAY_LOOP) {
					@Override
					public boolean isActive() {
						return super.isActive()
								&& this.getAmbience().getLocation().getName().equals("sludge_worm_dungeon_pit");
					}
				});
		AmbienceManager.INSTANCE.registerAmbience(new SpiritTreeAmbienceType());

		// Events
		AmbienceManager.INSTANCE
				.registerAmbience(new EventAmbienceType(EventSpoopy.class, SoundRegistry.AMBIENT_SPOOPY, 0));
		AmbienceManager.INSTANCE.registerAmbience(
				new EventAmbienceType(EventBloodSky.class, SoundRegistry.AMBIENT_BLOOD_SKY, 1).setDelay(140));
