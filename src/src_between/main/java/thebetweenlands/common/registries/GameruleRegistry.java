...
public final class GameruleRegistry {
	...
	public static void onEntityJoin(EntityJoinWorldEvent event) {
		if (!event.getWorld().isRemote && event.getEntity() instanceof EntityPlayerMP) {
			TheBetweenlands.networkWrapper.sendTo(new MessageSyncGameRules(GameruleRegistry.INSTANCE.gamerules),
	...
	public static void onGameruleChange(GameRuleChangeEvent event) {
		if (GameruleRegistry.INSTANCE.gamerules.contains(event.getRuleName())) {
			TheBetweenlands.networkWrapper
					.sendToAll(new MessageSyncGameRules(Collections.singleton(event.getRuleName())));
		}
	}
}
