			...
			if (target != null && target.hasCapability(CapabilityRegistry.CAPABILITY_EQUIPMENT, null)) {
				...
				case 0:
					...
					if (this.sourceSlot >= -1 && this.sourceSlot < sender.inventory.getSizeInventory()) {
						ItemStack stack = this.sourceSlot == -1 ? sender.getHeldItem(EnumHand.OFF_HAND)
								: sender.inventory.getStackInSlot(this.sourceSlot);
						...
						if (result.isEmpty() || result.getCount() != stack.getCount()) {
							sender.sendStatusMessage(new TextComponentTranslation("chat.equipment.equipped",
				...
				case 1:
					...
					if (this.sourceSlot >= 0) {
						ItemStack stack = EquipmentHelper.unequipItem(sender, target, this.inventory, this.sourceSlot,
								false);
						if (!stack.isEmpty()) {
							sender.sendStatusMessage(new TextComponentTranslation("chat.equipment.unequipped",
									new TextComponentTranslation(stack.getTranslationKey() + ".name")), true);
