			...
			if (cap != null) {
				if (!this.active && cap.isActive()) {
					...
					cap.setCooldownTicks(ItemRingOfSummoning.USE_COOLDOWN);
						...
						&& ItemRingOfSummoning.isRingActive(player)) {
					cap.setActive(true);
					cap.setActiveTicks(0);
					player.world.playSound(null, player.posX, player.posY, player.posZ, SoundRegistry.PEAT_MUMMY_CHARGE,
							SoundCategory.PLAYERS, 0.4F, (player.world.rand.nextFloat() * 0.4F + 0.8F) * 0.8F);
				}
			}
		}
		return null;
