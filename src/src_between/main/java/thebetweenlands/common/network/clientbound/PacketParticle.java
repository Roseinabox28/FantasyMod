...
public class PacketParticle implements IMessage, IMessageHandler<PacketParticle, IMessage> {
	...
	public static enum ParticleType {
		SPORE_JET, SLUDGE_JET, FLAME, BEAM, BEAM_YELLOW, GOOP_SPLAT, SPLODE_SHROOM, FLAME_JET;
		...
		if (world.isRemote) {
			...
			case SLUDGE_JET:
				for (double yy = message.posY; yy < message.posY + 2.5D; yy += 0.5D) {
					...
					BLParticles.TAR_BEAST_DRIP
							.spawn(world, d0, d1, d4, ParticleArgs.get()
									.withMotion(10f * (yy - d8) * (rand.nextFloat() - 0.5f), 4f * (yy - d8),
											10f * (yy - d8) * (rand.nextFloat() - 0.5f))
									.withScale(2.5F).withData(100))
							.setRBGColorF(0.4118F, 0.2745F, 0.1568F);
					BLParticles.TAR_BEAST_DRIP
							.spawn(world, d3, d1, d2, ParticleArgs.get()
									.withMotion(10f * (yy - d8) * (rand.nextFloat() - 0.5f), 4f * (yy - d8),
											10f * (yy - d8) * (rand.nextFloat() - 0.5f))
									.withScale(2.5F).withData(100))
							.setRBGColorF(0.4118F, 0.2745F, 0.1568F);
					BLParticles.TAR_BEAST_DRIP
							.spawn(world, d3, d1, d4, ParticleArgs.get()
									.withMotion(10f * (yy - d8) * (rand.nextFloat() - 0.5f), 4f * (yy - d8),
											10f * (yy - d8) * (rand.nextFloat() - 0.5f))
									.withScale(2.5F).withData(100))
							.setRBGColorF(0.4118F, 0.2745F, 0.1568F);
					BLParticles.TAR_BEAST_DRIP
							.spawn(world, d0, d1, d2, ParticleArgs.get()
									.withMotion(10f * (yy - d8) * (rand.nextFloat() - 0.5f), 4f * (yy - d8),
											10f * (yy - d8) * (rand.nextFloat() - 0.5f))
									.withScale(2.5F).withData(100))
							.setRBGColorF(0.4118F, 0.2745F, 0.1568F);
					BLParticles.TAR_BEAST_DRIP
							.spawn(world, d5, d6, d7, ParticleArgs.get()
									.withMotion(10f * (yy - d8) * (rand.nextFloat() - 0.5f), 4f * (yy - d8),
											10f * (yy - d8) * (rand.nextFloat() - 0.5f))
									.withScale(2.5F).withData(100))
							.setRBGColorF(0.4118F, 0.2745F, 0.1568F);
				...
				break;
			case FLAME:
				TheBetweenlands.proxy.spawnCustomParticle("flame", world, message.posX, message.posY, message.posZ,
						0.0D, 0.00D, 0.0D);
				break;
			case BEAM:
				...
				BatchedParticleRenderer.INSTANCE.addParticle(
						DefaultParticleBatches.TRANSLUCENT_GLOWING_NEAREST_NEIGHBOR,
						BLParticles.PUZZLE_BEAM.create(world, message.posX, message.posY, message.posZ,
								ParticleArgs.get().withMotion(0.0125f * (rand.nextFloat() - 0.5f),
										0.0125f * (rand.nextFloat() - 0.5f), 0.0125f * (rand.nextFloat() - 0.5f))
										.withColor(57F, 255F, 56F, 1F).withScale(0.5F + message.scale).withData(100)));
				break;
			case BEAM_YELLOW:
				counter += rand.nextInt(3);
				if (counter % (Minecraft.getMinecraft().gameSettings.particleSetting == 0 ? 1
				...
				BatchedParticleRenderer.INSTANCE.addParticle(
						...
						BLParticles.PUZZLE_BEAM.create(world, message.posX, message.posY, message.posZ,
								ParticleArgs.get().withMotion(0.0125f * (rand.nextFloat() - 0.5f),
										0.0125f * (rand.nextFloat() - 0.5f), 0.0125f * (rand.nextFloat() - 0.5f))
										.withColor(255F, 102F, 0F, 1F).withScale(0.5F + message.scale).withData(100)));
				break;
			case GOOP_SPLAT:
				for (int count = 0; count <= 200; ++count)
					TheBetweenlands.proxy.spawnCustomParticle("egg_sac", world,
							message.posX + (world.rand.nextDouble() - 0.5D), message.posY + world.rand.nextDouble(),
							message.posZ + (world.rand.nextDouble() - 0.5D), 0, 0, 0);
				break;
			case SPLODE_SHROOM:
				for (int count = 0; count <= 200; ++count)
					TheBetweenlands.proxy.spawnCustomParticle("splode_shroom", world,
							message.posX + (world.rand.nextDouble() - 0.5D), message.posY + world.rand.nextDouble(),
							message.posZ + (world.rand.nextDouble() - 0.5D), 0, 0, 0);
				break;
			case FLAME_JET:
