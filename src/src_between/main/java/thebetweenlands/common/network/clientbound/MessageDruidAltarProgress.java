			...
			if (te instanceof TileEntityDruidAltar) {
				...
				if (this.progress >= 0) {
						...
						for (int y = -8; y <= 8; y++) {
							for (int z = -8; z <= 8; z++) {
								...
								if (block == BlockRegistry.DRUID_STONE_1 || block == BlockRegistry.DRUID_STONE_2
									...
									BLParticles.ALTAR_CRAFTING.spawn(world, pos.getX() + 0.5D, pos.getY() + 0.5D,
											pos.getZ() + 0.5D,
											ParticleArgs.get().withScale(0.25F + te.getWorld().rand.nextFloat() * 0.2F)
													.withData(te));
								}
							}
						}
					...
					Minecraft.getMinecraft().getSoundHandler()
							.playSound(new DruidAltarSound(SoundRegistry.DRUID_CHANT, SoundCategory.BLOCKS, altar));
				}
			}
		}
	}
