	...
	private void handle() {
		...
		if (owner instanceof EntityLivingBase) {
			...
			if (!held.isEmpty() && held.getItem() instanceof ItemLivingWeedwoodShield) {
				...
				Vec3d offset = new Vec3d(bodyForward.x * 0.5F, owner.getEyeHeight(), bodyForward.z * 0.5F)
						.add(right.scale(this.mainhand ? 0.35D : -0.35D).add(0, lookVec.y * 0.5D - 0.4D, 0)
				...
				for (int i = 0; i < 20; i++) {
					...
					living.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, living.posX + offset.x + dx,
							living.posY + offset.y + dy, living.posZ + offset.z + dz, dx * 1.25D, dy, dz * 1.25D,
							itemId);
				}
			}
		}
