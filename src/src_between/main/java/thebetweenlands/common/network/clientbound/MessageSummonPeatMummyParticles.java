	...
	private void handle() {
		...
		if (entity != null) {
			for (int i = 0; i < 250; i++) {
				...
				Vec3d vec = new Vec3d(px, py, pz)
						.subtract(new Vec3d(entity.posX + 0.35F, entity.posY + 1.1F, entity.posZ + 0.35F)).normalize();
				entity.world.spawnParticle(EnumParticleTypes.BLOCK_CRACK, px, py, pz, vec.x * 0.25F, vec.y * 0.25F,
						vec.z * 0.25F, Block.getStateId(BlockRegistry.MUD.getDefaultState()));
			}
		}
	}
}
