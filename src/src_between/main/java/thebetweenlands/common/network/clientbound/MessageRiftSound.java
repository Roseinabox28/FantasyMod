...
public class MessageRiftSound extends MessageBase {
	...
	private void handle() {
		if (Minecraft.getMinecraft().world != null) {
			...
			final float pitchRange = (this.type == RiftSoundType.CREAK
					? (Minecraft.getMinecraft().world.rand.nextFloat() * 0.3f + 0.7f)
					: 1.0f);

			Minecraft.getMinecraft().getSoundHandler().playSound(new PositionedSoundRecord(sound.getSoundName(),
					SoundCategory.AMBIENT, 1, 1, false, 0, ISound.AttenuationType.NONE, 0, 0, 0) {
				@Override
