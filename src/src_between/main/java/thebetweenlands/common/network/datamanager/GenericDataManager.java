...
public class GenericDataManager implements IGenericDataManagerAccess {
	...
	/**
	 * Creates a data parameter with custom de-/serializers. Values will be
	 * de-/serialized on the main thread.
	 * 
	 * @param clazz
	 * @param serializer
	 * @param deserializer
	...
	public static <T> DataParameter<T> createKey(Class<?> clazz, Serializer<T> serializer,
			...
			try {
				...
				if (!callerClass.equals(clazz)) {
					throw new RuntimeException(
							"GenericDataManager#createKey called for: " + clazz + " from " + callerClass);
				}
	...
	/**
	 * Creates a data parameter with a normal serializer. Values will be
	 * de-/serialized on the network thread.
	 * 
	 * @param clazz
	 * @param serializer
	 * @return
	...
	public <T> DataParameter<T> register(DataParameter<T> key, T value) {
		...
		} else if (key.getSerializer() instanceof CustomSerializer == false
				&& DataSerializers.getSerializerId(key.getSerializer()) < 0) {
		...
		try {
			...
			try {
				...
				CrashReport crashreport = CrashReport.makeCrashReport(throwable,
						"Getting synced " + this.owner.getClass().getName() + " data");
				CrashReportCategory crashreportcategory = crashreport
						.makeCategory("Synced " + this.owner.getClass().getName() + " data");
	...
	public <T> EntryAccess<T> set(DataParameter<T> key, T value) {
		...
		if (ObjectUtils.notEqual(value, entry.getValue())) {
			if (this.owner instanceof IDataManagedObject == false
					|| !((IDataManagedObject) this.owner).onParameterChange(key, value, false)) {
	...
	private <T> void serializeEntry(GenericDataManager.DataEntry<T> entry, GenericDataManager.DataEntry<?> copy) {
		...
		try {
			...
			throw new DecoderException(
					"Failed serializing data with custom serializer " + entry.serializer.getClass().getName(), ex);
		} finally {
			buf.release();
		}
	}
	...
	public static List<IDataEntry<?>> readEntries(PacketBuffer buf) throws IOException {
		...
		while ((key = buf.readUnsignedByte()) != 255) {
			...
			GenericDataManager.DataEntry<?> entry = new GenericDataManager.DataEntry(null, serializer.createKey(key),
					value);
				...
				if (entry != null) {
					...
					if (newEntry instanceof GenericDataManager.DataEntry<?> && entry.deserializer != null) {
						...
						if (newGenericEntry.deserializedValue == null) {
							...
							try {
								newGenericEntry.deserializedValue = entry.deserializer
										.deserialize(new PacketBuffer(buf));
							} catch (Exception ex) {
								throw new DecoderException("Failed deserializing data with custom deserializer "
										+ entry.deserializer.getClass().getName(), ex);
							} finally {
								buf.release();
							}
						}
					...
					if (this.owner instanceof IDataManagedObject == false
							|| !((IDataManagedObject) this.owner).onParameterChange(entry.getKey(), newValue, true)) {
	...
	public static class DataEntry<T> implements IDataEntry<T> {
		...
		public GenericDataManager.DataEntry<T> copy() {
			return new GenericDataManager.DataEntry<T>(this.dataManager, this.key,
					this.key.getSerializer().copyValue(this.value), this.trackingTime);
		}
	}
}