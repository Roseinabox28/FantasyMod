	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		...
		case "discover":
			...
			case "held":
				...
				AspectDiscovery discovery = mergedKnowledge.discover(manager,
						AspectManager.getAspectItem(player.getHeldItemMainhand()));
				if (discovery.discovered != null) {
					DiscoveryContainer.addDiscoveryToContainers(player,
							AspectManager.getAspectItem(player.getHeldItemMainhand()), discovery.discovered.type);
					sender.sendMessage(new TextComponentTranslation("command.aspectdiscovery.discover.held",
							new TextComponentString(discovery.result.toString()), new TextComponentString(
									discovery.discovered == null ? "null" : discovery.discovered.type.getName())));
				}
				break;
			case "all":
				List<DiscoveryContainer<?>> discoveryContainers = DiscoveryContainer
						.getWritableDiscoveryContainers(player);
