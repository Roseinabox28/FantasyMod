...
public class CommandBLEvent extends CommandBase {
	...
	private void processToggle(ICommandSender sender, String[] args) throws PlayerNotFoundException, CommandException {
		...
		ResourceLocation eventName = new ResourceLocation(
	...
	private void processOn(ICommandSender sender, String[] args) throws PlayerNotFoundException, CommandException {
		checkArg(args, 2, "on");
		processEventState(sender,
				new ResourceLocation(getChatComponentFromNthArg(sender, args, 1).getUnformattedText()), true);
	}
	...
	private void processOff(ICommandSender sender, String[] args) throws PlayerNotFoundException, CommandException {
		if (args.length < 2) {
			...
			processEventState(sender,
					new ResourceLocation(getChatComponentFromNthArg(sender, args, 1).getUnformattedText()), false);
		}
	}
	...
	private void processEventState(ICommandSender sender, ResourceLocation eventName, boolean isActive)
	...
	public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args,
		...
		return completions == null ? Collections.<String>emptyList()
				: getListOfStringsMatchingLastWord(args, completions.toArray(new String[0]));
	}
	...
	private IEnvironmentEvent getEnvironentEvent(ICommandSender sender, ResourceLocation eventName)
			throws CommandException {
		BLEnvironmentEventRegistry environmentEventRegistry = getEnvironmentEventRegistry(sender);
