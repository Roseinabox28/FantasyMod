...
public class ItemNibblestick extends ItemBLFood {
		...
		if (cap != null && FoodSickness.getSicknessForHatred(cap.getFoodHatred(this)) != FoodSickness.SICK) {
			...
			if (xp > 0) {
				if (!worldIn.isRemote) {
					...
					worldIn.playSound(null, player.posX, player.posY + 0.5D, player.posZ,
							SoundEvents.ENTITY_EXPERIENCE_ORB_PICKUP, SoundCategory.PLAYERS, 0.5F,
	...
	public void addFinishEatingParticles(ItemStack stack, World world, EntityPlayer player) {
		for (int i = 0; i < 20; i++) {
			BLParticles.XP_PIECES.spawn(world, player.posX + world.rand.nextFloat() * 0.6F - 0.3F,
					player.posY + player.getEyeHeight() - 0.1F + world.rand.nextFloat() * 0.6F - 0.3F,
					player.posZ + world.rand.nextFloat() * 0.6F - 0.3F);
		}
	}
}
