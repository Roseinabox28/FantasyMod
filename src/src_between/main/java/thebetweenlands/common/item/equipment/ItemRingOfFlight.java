...
public class ItemRingOfFlight extends ItemRing {
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> list, ITooltipFlag flagIn) {
		...
		if (GuiScreen.isShiftKeyDown()) {
			String toolTip = I18n.format("tooltip.ring.flight", KeyBindRegistry.RADIAL_MENU.getDisplayName(),
						...
						if (isFlying) {
							...
							if (!entity.onGround && entity.world.isRemote) {
								if (cap.getFlightTime() > 40) {
									BLParticles.LEAF_SWIRL.spawn(entity.world, entity.posX, entity.posY, entity.posZ,
											ParticleArgs.get().withData(400, 0.0F, entity));
								} else {
									for (int i = 0; i < 5; i++) {
										BLParticles.LEAF_SWIRL.spawn(entity.world, entity.posX, entity.posY,
												entity.posZ, ParticleArgs.get().withData(400,
														1.0F - (cap.getFlightTime() + i / 5.0F) / 40.0F, entity));
									}
								}
							}
						} else {
	...
	private double getGroundHeight(EntityPlayer player) {
		RayTraceResult result = player.world.rayTraceBlocks(new Vec3d(player.posX, player.posY, player.posZ),
				new Vec3d(player.posX, player.posY - 64, player.posZ), true);
		...
		if (event.player != null) {
			...
			if (!player.capabilities.isCreativeMode) {
				...
				if (cap != null) {
					...
					IEquipmentCapability equipmentCap = player.getCapability(CapabilityRegistry.CAPABILITY_EQUIPMENT,
							null);
								...
								if (player.capabilities.isFlying) {
									...
									if (player == TheBetweenlands.proxy.getClientPlayer()) {
										TheBetweenlands.networkWrapper
												.sendToServer(new MessageFlightState(cap.isFlying()));
									}
								}
							}
