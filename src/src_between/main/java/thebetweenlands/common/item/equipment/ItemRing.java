...
public class ItemRing extends Item implements IEquippable {
	...
	public void drainPower(ItemStack stack, Entity entity) {
		if (stack.getItemDamage() < stack.getMaxDamage() && stack.getItem() instanceof ItemRing
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
		...
		if (!player.isSneaking()) {
			if (stack.getItemDamage() > 0
					&& (player.experienceTotal > 0 || player.experienceLevel > 0 || player.experience > 0)) {
				if (!world.isRemote) {
					...
					stack.setItemDamage(Math.max(0, stack.getItemDamage() - MathHelper
							.ceil(MathHelper.abs(removeXp(player, MathHelper.ceil(requiredRepair))) * conversion)));
				}
	...
	public boolean canEquipOnRightClick(ItemStack stack, EntityPlayer player, Entity target) {
		return stack.getItemDamage() == 0 || player.experienceTotal == 0 || player.experience == 0
				|| player.experienceLevel == 0 || player.isSneaking();
	}
