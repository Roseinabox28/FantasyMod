...
public class ItemRingOfSummoning extends ItemRing {
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> list, ITooltipFlag flagIn) {
		...
		if (GuiScreen.isShiftKeyDown()) {
			String toolTip = I18n.format("tooltip.ring.summoning", KeyBindRegistry.RADIAL_MENU.getDisplayName(),
					KeyBindRegistry.USE_RING.getDisplayName());
				...
				if (cap.getCooldownTicks() > 0) {
					...
					if (cap.isActive()) {
						...
						if (cap.getActiveTicks() > MAX_USE_TIME) {
							...
							int arms = entity.world.getEntitiesWithinAABB(EntityMummyArm.class,
									entity.getEntityBoundingBox().grow(18), e -> e.getDistance(entity) <= 18.0D).size();

							if (arms < MAX_ARMS) {
								List<EntityLivingBase> targets = entity.world.getEntitiesWithinAABB(
										EntityLivingBase.class, entity.getEntityBoundingBox().grow(16),
										e -> e instanceof EntityLiving && e.getDistance(entity) <= 16.0D && e != entity
												&& (e instanceof EntityMob || e instanceof IMob));

								BlockPos targetPos = null;

								if (!targets.isEmpty()) {
									EntityLivingBase target = targets.get(entity.world.rand.nextInt(targets.size()));
									boolean isAttacked = !entity.world
											.getEntitiesWithinAABB(EntityMummyArm.class, target.getEntityBoundingBox())
											.isEmpty();
									if (!isAttacked) {
								...
								if (targetPos == null && entity.world.rand.nextInt(3) == 0) {
									targetPos = entity.getPosition().add(entity.world.rand.nextInt(16) - 8,
											entity.world.rand.nextInt(6) - 3, entity.world.rand.nextInt(16) - 8);
									boolean isAttacked = !entity.world
											.getEntitiesWithinAABB(EntityMummyArm.class, new AxisAlignedBB(targetPos))
											.isEmpty();
									if (isAttacked) {
										targetPos = null;
									}
								...
								if (targetPos != null && entity.world.getBlockState(targetPos.down())
										.isSideSolid(entity.world, targetPos.down(), EnumFacing.UP)) {
									EntityMummyArm arm = new EntityMummyArm(entity.world);
									arm.setLocationAndAngles(targetPos.getX() + 0.5D, targetPos.getY(),
	...
	public static boolean isRingActive(Entity entity) {
		...
		if (cap != null) {
			...
			for (int i = 0; i < inv.getSizeInventory(); i++) {
				...
				if (!stack.isEmpty() && stack.getItem() == ItemRegistry.RING_OF_SUMMONING
						&& ((ItemRing) stack.getItem()).canBeUsed(stack)) {
					hasRing = true;
					break;
				}
			}
