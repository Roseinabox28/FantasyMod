...
public class ItemRingOfDispersion extends ItemRing {
	...
	public BlockPos getLastValidPos(ItemStack stack) {
		return stack.hasTagCompound() && stack.getTagCompound().hasKey(NBT_LAST_VALID_POS, Constants.NBT.TAG_LONG)
				? BlockPos.fromLong(stack.getTagCompound().getLong(NBT_LAST_VALID_POS))
				: null;
	}
	...
	public boolean canPhase(EntityPlayer player, ItemStack stack) {
		return stack.getItemDamage() < stack.getMaxDamage() && !player.isSpectator() && player.isSneaking()
		...
		if (!entity.world.isRemote) {
			...
			if (requiresTeleport && storedPos != null) {
				...
				entity.world.playSound(null, entity.posX, entity.posY, entity.posZ,
						SoundRegistry.RING_OF_DISPERSION_TELEPORT, SoundCategory.PLAYERS, 1.0F, 1.0F);
			}
		}
	}
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> list, ITooltipFlag flagIn) {
		...
		if (GuiScreen.isShiftKeyDown()) {
			String toolTip = I18n.format("tooltip.ring.dispersion", KeyBindRegistry.RADIAL_MENU.getDisplayName(),
