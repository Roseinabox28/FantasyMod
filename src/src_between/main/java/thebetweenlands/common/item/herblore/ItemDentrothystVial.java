...
public class ItemDentrothystVial extends Item implements ItemRegistry.IBlockStateItemModelDefinition {
	private static final class DentrothystVialFluidHandler extends FluidHandlerItemStackSimple {
		...
		public int fill(FluidStack resource, boolean doFill) {
			...
			ItemStack fluidVial = new ItemStack(ItemRegistry.DENTROTHYST_FLUID_VIAL, 1,
					this.container.getItemDamage() == 2 ? 1 : 0);
	...
	/**
	 * Creates an item stack of the specified vial type. Vial types: 0 = green, 1 =
	 * dirty, 2 = orange
	 * 
	 * @param vialType
	 * @return
						...
						if (floor == 5 || floor == 6) {
							if (!world.isRemote) {
								...
								world.playSound(null, pos,
										FluidRegistry.FOG.getFillSound(new FluidStack(FluidRegistry.FOG, 1000)),
										SoundCategory.BLOCKS, 1.0F, 1.0F);
								ItemHandlerHelper.giveItemToPlayer(player, ItemRegistry.DENTROTHYST_FLUID_VIAL
										.withFluid(stack.getItemDamage() == 2 ? 1 : 0, FluidRegistry.FOG));
							}

							return EnumActionResult.SUCCESS;
						} else if (!world.isRemote) {
							player.sendStatusMessage(new TextComponentTranslation("chat.not_enough_fog_for_vial"),
									true);
						}
					}
				}
