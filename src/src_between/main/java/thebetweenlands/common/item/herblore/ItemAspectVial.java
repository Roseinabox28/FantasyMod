...
public class ItemAspectVial extends Item implements ITintedItem, ItemRegistry.IMultipleItemModelDefinition {
	...
	public String getItemStackDisplayName(ItemStack stack) {
		...
		if (itemAspects.size() >= 1) {
			...
			return I18n.translateToLocalFormatted(this.getUnlocalizedNameInefficiently(stack) + ".filled.name",
					aspect.type.getName(), aspect.getRoundedDisplayAmount()).trim();
		}
	...
	public static void placeAspectVial(World world, BlockPos pos, int vialType, Aspect aspect) {
		world.setBlockState(pos, BlockRegistry.ASPECT_VIAL_BLOCK.getDefaultState().withProperty(BlockAspectVial.TYPE,
				BlockDentrothyst.EnumDentrothyst.values()[vialType]), 2);
