...
public class ItemElixir extends Item implements ITintedItem, ItemRegistry.IBlockStateItemModelDefinition {
		...
		if (this.isInCreativeTab(tab)) {
			for (ElixirEffect effect : this.effects) {
				...
				if (recipe != null) {
					...
					int durationModifier = effect.isAntiInfusion() ? recipe.negativeDurationModifier
							: recipe.durationModifier;
					items.add(this.getElixirItem(effect, baseDuration, ElixirEffect.VIAL_INFUSION_MAX_POTENCY - 1, 0));
					items.add(this.getElixirItem(effect,
							baseDuration + MathHelper.floor(durationModifier / 6.0F * 5.0F), 0, 0));
					items.add(this.getElixirItem(effect, baseDuration, ElixirEffect.VIAL_INFUSION_MAX_POTENCY - 1, 1));
					items.add(this.getElixirItem(effect,
							baseDuration + MathHelper.floor(durationModifier / 6.0F * 5.0F), 0, 1));
				}
			}
		}
	}
	...
	public String getItemStackDisplayName(ItemStack stack) {
		if (I18n.canTranslate(stack.getTranslationKey() + ".name")) {
			return I18n.translateToLocalFormatted(stack.getTranslationKey() + ".name",
					TranslationHelper.translateToLocal(this.getElixirFromItem(stack).getEffectName()));
		}
		return I18n.translateToLocalFormatted("item.thebetweenlands.bl.elixir.name",
				TranslationHelper.translateToLocal(this.getElixirFromItem(stack).getEffectName()));
	}

	@Override
	public EnumAction getItemUseAction(ItemStack stack) {
		if (stack.getTagCompound() != null && stack.getTagCompound().hasKey("throwing")
				&& stack.getTagCompound().getBoolean("throwing")) {
	...
	public void onPlayerStoppedUsing(ItemStack stack, World world, EntityLivingBase entityLiving, int timeLeft) {
		if (stack.getTagCompound() != null && stack.getTagCompound().hasKey("throwing")
				&& stack.getTagCompound().getBoolean("throwing")) {
			world.playSound((EntityPlayer) entityLiving, entityLiving.getPosition(), SoundEvents.ENTITY_ARROW_SHOOT,
			...
			if (!world.isRemote) {
				...
				elixir.shoot(entityLiving, ((EntityPlayer) entityLiving).rotationPitch,
						((EntityPlayer) entityLiving).rotationYaw, -20.0F, strength, 1.0F);
	...
	/**
	 * Creates an item stack with the specified effect, duration, strength and vial
	 * type. Vial types: 0 = green, 1 = orange
	 * 
	 * @param effect
	 * @param duration
	 * @param strength
	...
	public ItemStack getElixirItem(ElixirEffect effect, int duration, int strength, int vialType) {
		...
		if (elixirStack.getTagCompound() == null)
			elixirStack.setTagCompound(new NBTTagCompound());
	...
	public ItemStack onItemUseFinish(ItemStack stack, World world, EntityLivingBase entityLiving) {
		...
		if (!world.isRemote) {
			...
			entityplayer.addPotionEffect(
					effect.createEffect(duration == -1 ? 1200 : duration, strength == -1 ? 0 : strength));
		}
		...
		if (entityplayer == null || !entityplayer.capabilities.isCreativeMode) {
			...
			if (entityplayer != null) {
				entityplayer.inventory.addItemStackToInventory(
						ItemRegistry.DENTROTHYST_VIAL.createStack(stack.getItemDamage() % 2 == 0 ? 1 : 2));
			}
		}
	...
	public void addInformation(ItemStack stack, World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
		...
		if (recipe != null) {
			...
			int durationModifier = elixirEffect.isAntiInfusion() ? recipe.negativeDurationModifier
					: recipe.durationModifier;
			durationLevel = Math.max(0, MathHelper.floor((effect.getDuration() - baseDuration)
					/ (float) durationModifier * ElixirEffect.VIAL_INFUSION_MAX_POTENCY));
		} else {
		...
		tooltip.add(I18n.translateToLocalFormatted("tooltip.bl.elixir.duration", durationLevelStr,
				StringUtils.ticksToElapsedTime(effect.getDuration()), effect.getDuration()));
		...
		if (!modifersMap.isEmpty()) {
			for (Entry<IAttribute, AttributeModifier> entry : modifersMap.entrySet()) {
				...
				modifier = new AttributeModifier(modifier.getName(),
						potion.getAttributeModifierAmount(effect.getAmplifier(), modifier), modifier.getOperation());
		...
		if (!modifiers.isEmpty() || hasEffectDescription) {
			...
			for (Tuple<String, AttributeModifier> tuple : modifiers) {
				...
				if (amount > 0.0D) {
					tooltip.add(TextFormatting.BLUE
							+ I18n.translateToLocalFormatted("attribute.modifier.plus." + modifier.getOperation(),
									ItemStack.DECIMALFORMAT.format(adjustedAmount),
									I18n.translateToLocalFormatted("attribute.name." + (String) tuple.getFirst())));
				} else if (amount < 0.0D) {
					adjustedAmount = adjustedAmount * -1.0D;
					tooltip.add(TextFormatting.RED
							+ I18n.translateToLocalFormatted("attribute.modifier.take." + modifier.getOperation(),
									ItemStack.DECIMALFORMAT.format(adjustedAmount),
									I18n.translateToLocalFormatted("attribute.name." + (String) tuple.getFirst())));
				}
			}
			...
			if (hasEffectDescription) {
				tooltip.addAll(ItemTooltipHandler.splitTooltip(
						I18n.translateToLocalFormatted("tooltip." + elixirEffect.getEffectName() + ".effect"), 0));
			}
		}
	}
