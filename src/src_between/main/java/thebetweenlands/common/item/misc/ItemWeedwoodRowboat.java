...
public class ItemWeedwoodRowboat extends Item {
	...
	public ItemWeedwoodRowboat() {
		...
		addPropertyOverride(new ResourceLocation("tarred"),
				(stack, world, entity) -> EntityWeedwoodRowboat.isTarred(stack) ? 1 : 0);
	}
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
		...
		List<Entity> list = world.getEntitiesWithinAABBExcludingEntity(player,
				player.getEntityBoundingBox().grow(look.x * REACH, look.y * REACH, look.z * REACH).grow(1, 1, 1));
		...
		EntityWeedwoodRowboat rowboat = new EntityWeedwoodRowboat(world, hit.hitVec.x,
				liquid ? hit.hitVec.y - 0.3 : hit.hitVec.y, hit.hitVec.z);
