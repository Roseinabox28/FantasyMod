		...
		if (!Minecraft.getMinecraft().isGamePaused()) {
			...
			if (view != null) {
				if (!EquipmentHelper.getEquipment(EnumEquipmentInventory.MISC, view, ItemRegistry.BARK_AMULET).isEmpty()
						|| (view instanceof EntityLivingBase
								&& ((EntityLivingBase) view).isPotionActive(ElixirEffectRegistry.ENLIGHTENED))) {
					final float range = 12.0F;

					List<EntityLivingBase> entities = view.world.getEntitiesWithinAABB(EntityLivingBase.class,
							view.getEntityBoundingBox().grow(range), e -> e.getDistanceSq(view) <= range * range);
					...
					for (EntityLivingBase entity : entities) {
						if (entity != view && entity.ticksExisted % 50 == 0) {
							BatchedParticleRenderer.INSTANCE.addParticle(
									...
									BLParticles.LIFE_ESSENCE.create(entity.world, 0, entity.height + 0.2D, 0,
											ParticleArgs.get().withScale(entity.width / 2.0F).withData(entity,
													entity.ticksExisted)));
						}
					}
				}
