...
public class ItemGrapplingHook extends Item {
	...
	public ItemGrapplingHook() {
		...
		this.addPropertyOverride(new ResourceLocation("grappling_hook_length"),
				(stack, worldIn, entityIn) -> MIN_GRAPPLING_HOOK_LENGTH + stack.getItemDamage());
		this.addPropertyOverride(new ResourceLocation("extended"), (stack, worldIn, entityIn) -> {
			if (entityIn != null && entityIn.getRidingEntity() instanceof EntityGrapplingHookNode) {
				...
				boolean hasOffHand = !entityIn.getHeldItem(EnumHand.OFF_HAND).isEmpty()
						&& entityIn.getHeldItem(EnumHand.OFF_HAND).getItem() instanceof ItemGrapplingHook;
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
		if (!world.isRemote) {
			if (!player.isRiding()) {
				...
				mountNode.setLocationAndAngles(player.posX - player.width / 2, player.posY,
						player.posZ - player.width / 2, 0, 0);
				...
				for (int i = 0; i < thrownNodes; i++) {
					...
					float velocity = 1.5F * (0.4F + 1F * i / (float) thrownNodes) / (float) MAX_GRAPPLING_HOOK_LENGTH
							* maxNodes;
					float upwardsVelocity = 1.0F * (0.4F + 0.6F * (float) Math.sin(Math.PI / 2 / thrownNodes * i))
							/ (float) MAX_GRAPPLING_HOOK_LENGTH * maxNodes;
				...
				world.playSound(null, player.posX, player.posY, player.posZ, SoundRegistry.ROPE_THROW,
						SoundCategory.PLAYERS, 1.5F, 0.8F + world.rand.nextFloat() * 0.3F);
			}
		}
	...
	public void addInformation(ItemStack stack, World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
		...
		tooltip.addAll(ItemTooltipHandler.splitTooltip(
				I18n.format("tooltip.grappling_hook", maxNodes, maxNodes * 2, maxNodes / 2, maxNodes), 0));
