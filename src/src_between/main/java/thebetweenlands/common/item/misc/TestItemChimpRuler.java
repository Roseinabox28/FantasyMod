...
public class TestItemChimpRuler extends Item {
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> list, ITooltipFlag flag) {
		if (hasTag(stack) && stack.getTagCompound().hasKey("homeX")) {
			list.add(TextFormatting.YELLOW + new TextComponentTranslation("tooltip.chimp_ruler.homex",
					stack.getTagCompound().getInteger("homeX")).getFormattedText());
			list.add(TextFormatting.YELLOW + new TextComponentTranslation("tooltip.chimp_ruler.homey",
					stack.getTagCompound().getInteger("homeY")).getFormattedText());
			list.add(TextFormatting.YELLOW + new TextComponentTranslation("tooltip.chimp_ruler.homez",
					stack.getTagCompound().getInteger("homeZ")).getFormattedText());
		}
	}
	...
	public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing,
		...
		if (hasTag(stack) && stack.getTagCompound().hasKey("homeX") && !player.isSneaking()
				&& hand.equals(EnumHand.MAIN_HAND)) {
			...
			if (!world.isRemote && state.getBlock() != null) {
				...
				// String[] name =
				// state.getBlock().getRegistryName().toString().toUpperCase().split(":");
				// CopytoClipboard("rotatedCubeVolume(world, rand, pos, " + x + ", "+ y + ", " +
				// z + ", blockHelper." + name[1] + ", 1, 1, 1, facing);");
				return EnumActionResult.SUCCESS;
			}
		}
	...
	protected void doUseAction(World world, EntityPlayer player, BlockPos home) {
		...
		handler.getLocalStorages(LocationStorage.class,
				new AxisAlignedBB(home.add(-80, -80, -80), home.add(80, 80, 80)), l -> true)
				.forEach(l -> handler.removeLocalStorage(l));
