	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand,
		...
		if (facing != EnumFacing.DOWN && (iblockstate.getMaterial().isSolid() || flag)
			...
			if (playerIn.canPlayerEdit(pos, facing, stack)
					&& BlockRegistry.STANDING_WEEDWOOD_SIGN.canPlaceBlockAt(worldIn, pos)) {
				if (worldIn.isRemote) {
					...
					if (facing == EnumFacing.UP) {
						int i = MathHelper.floor((double) ((playerIn.rotationYaw + 180.0F) * 16.0F / 360.0F) + 0.5D)
								& 15;
						worldIn.setBlockState(pos, BlockRegistry.STANDING_WEEDWOOD_SIGN.getDefaultState()
								.withProperty(BlockStandingSign.ROTATION, Integer.valueOf(i)), 11);
					} else {
						worldIn.setBlockState(pos, BlockRegistry.WALL_WEEDWOOD_SIGN.getDefaultState()
