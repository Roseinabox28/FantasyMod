...
public class ItemOctineIngot extends Item {
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand hand) {
		...
		if (result != null && result.typeOfHit == Type.BLOCK) {
			...
			if (((ItemOctineIngot) itemStackIn.getItem()).isTinder(itemStackIn, ItemStack.EMPTY, blockState)) {
				...
				List<EntityItem> tinder = worldIn.getEntitiesWithinAABB(EntityItem.class, new AxisAlignedBB(offsetPos),
						entity -> !entity.getItem().isEmpty() && ((ItemOctineIngot) itemStackIn.getItem())
								.isTinder(itemStackIn, entity.getItem(), null));
				if (!tinder.isEmpty()) {
					hasTinder = true;
				}
			}
		...
		if (playerIn instanceof EntityPlayer) {
			...
			if (result != null && result.typeOfHit == Type.BLOCK) {
				...
				if (((ItemOctineIngot) stack.getItem()).isTinder(stack, ItemStack.EMPTY, blockState)) {
					...
					List<EntityItem> tinder = worldIn.getEntitiesWithinAABB(EntityItem.class,
							new AxisAlignedBB(offsetPos), entity -> !entity.getItem().isEmpty()
									&& ((ItemOctineIngot) stack.getItem()).isTinder(stack, entity.getItem(), null));
					if (!tinder.isEmpty()) {
						hasTinder = true;
					}
				}
					...
					if (!worldIn.isRemote) {
						if (count <= 1) {
							if (playerIn instanceof EntityPlayerMP) {
								...
								if (worldIn.getBlockState(isBlockTinder ? pos.down() : offsetPos.down())
										.getBlock() == BlockRegistry.PEAT) {
	...
	public boolean isTinder(ItemStack ingot, ItemStack stack, @Nullable IBlockState blockState) {
		if (blockState != null) {
			...
			return block == BlockRegistry.CAVE_MOSS || block == BlockRegistry.MOSS || block == BlockRegistry.LICHEN
					|| block == BlockRegistry.DEAD_MOSS || block == BlockRegistry.DEAD_LICHEN
		...
		if (!stack.isEmpty()) {
			...
			return EnumItemPlantDrop.CAVE_MOSS_ITEM.isItemOf(stack) || EnumItemPlantDrop.MOSS_ITEM.isItemOf(stack)
					|| EnumItemPlantDrop.LICHEN_ITEM.isItemOf(stack) || EnumItemPlantDrop.THORNS_ITEM.isItemOf(stack)
					|| EnumItemCrushed.GROUND_CAVE_MOSS.isItemOf(stack) || EnumItemCrushed.GROUND_MOSS.isItemOf(stack)
					|| EnumItemCrushed.GROUND_LICHEN.isItemOf(stack) || EnumItemCrushed.GROUND_THORNS.isItemOf(stack);
		}
		return false;
	}
