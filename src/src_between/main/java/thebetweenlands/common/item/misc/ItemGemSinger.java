...
public class ItemGemSinger extends Item {
	public static enum GemSingerTarget implements Predicate<IBlockState> {
		...
		LIFE_CRYSTAL(3, state -> state.getBlock() == BlockRegistry.LIFE_CRYSTAL_STALACTITE
				&& state.getValue(BlockLifeCrystalStalactite.VARIANT) == EnumLifeCrystalType.ORE);
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
			...
			if (playerIn.isSneaking()) {
					...
					if (target != null) {
						...
						if (chunk != null) {
							...
							if (storage != null) {
								...
								if (gems.contains(BetweenlandsChunkStorage.getGemSingerTargetIndex(gem.getX(),
										gem.getY(), gem.getZ()))) {
									valid = true;
								}
							}
				...
				if (gem == null) {
					for (int i = 0; i < attempts; i++) {
						Chunk chunk = worldIn.getChunkProvider().getLoadedChunk(
								(MathHelper.floor(playerIn.posX) >> 4) + worldIn.rand.nextInt(chunkRange * 2 + 1)
										- chunkRange,
								(MathHelper.floor(playerIn.posZ) >> 4) + worldIn.rand.nextInt(chunkRange * 2 + 1)
										- chunkRange);
						...
						if (chunk != null) {
							...
							if (storage != null) {
								GemSingerTarget target = GemSingerTarget.values()[worldIn.rand
										.nextInt(GemSingerTarget.values().length)];

								gem = storage.findRandomGem(target, worldIn.rand, playerIn.getPosition(),
										chunkRange * 16);
		...
		if (worldIn.isRemote && !playerIn.isSneaking()) {
			worldIn.playSound(playerIn, playerIn.posX, playerIn.posY, playerIn.posZ, SoundRegistry.GEM_SINGER,
					SoundCategory.PLAYERS, 2, 1);
		}
	...
	protected void spawnEffect(EntityPlayer player, BlockPos target, int maxRangeBlocks, int maxDelay) {
		if (player instanceof EntityPlayerMP) {
			int delay = Math.min((int) (Math.sqrt(player.getDistanceSq(target)) / (float) maxRangeBlocks * maxDelay),
					maxDelay);
