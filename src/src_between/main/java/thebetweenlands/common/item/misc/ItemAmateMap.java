...
public class ItemAmateMap extends ItemMap implements ICustomMeshCallback {
	...
	public void updateMapData(World world, Entity viewer, MapData data) {
		if (world.provider.getDimension() == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId
				&& world.provider.getDimension() == data.dimension && viewer instanceof EntityPlayer
				&& viewer.ticksExisted % 3 == 0) {
			...
			for (int xPixel = viewerPixelX - viewRadiusPixels + 1; xPixel < viewerPixelX + viewRadiusPixels; ++xPixel) {
				if ((xPixel & 15) == (mapInfo.step & 15) || terrainChanged) {
					...
					biomes[primaryBiomesArray] = world.getBiomeProvider().getBiomesForGeneration(
							biomes[primaryBiomesArray], (viewerBlockX + (xPixel - viewerPixelX) * blocksPerPixel) / 4,
							(viewerBlockZ - (viewRadiusPixels - 1) * blocksPerPixel) / 4, 1, biomeViewRange);

					if (updatedColumns == 0 || xPixel - prevXPixel != 1) {
						biomes[previousBiomesArray] = world.getBiomeProvider().getBiomesForGeneration(
								biomes[previousBiomesArray],
								(viewerBlockX + (xPixel - viewerPixelX - 1) * blocksPerPixel) / 4,
								(viewerBlockZ - (viewRadiusPixels - 1) * blocksPerPixel) / 4, 1, biomeViewRange);
					}

					for (int zPixel = viewerPixelZ - viewRadiusPixels - 1; zPixel < viewerPixelZ
							+ viewRadiusPixels; ++zPixel) {
						if (xPixel >= 0 && zPixel >= 0 && xPixel < 128 && zPixel < 128) {
							int xPixelDist = xPixel - viewerPixelX;
							int zPixelDist = zPixel - viewerPixelZ;
							boolean shouldFuzz = xPixelDist * xPixelDist
									+ zPixelDist * zPixelDist > (viewRadiusPixels - 2) * (viewRadiusPixels - 2);
							...
							if (zPixel >= 0 && xPixelDist * xPixelDist + zPixelDist * zPixelDist < viewRadiusPixels
									* viewRadiusPixels && (!shouldFuzz || (xPixel + zPixel & 1) != 0)) {
								...
								int chunkX = worldX >> 4;
								int chunkZ = worldZ >> 4;
								if (!checkedChunks.containsKey(chunkX) || (checkedChunks.containsKey(chunkX)
										&& !checkedChunks.get(chunkX).contains(chunkZ))) {
	...
	private void locateBLLocations(World world, Map<Integer, List<Integer>> posList, int centerX, int centerZ,
			int blocksPerPixel, MapData data) {
		...
		for (Map.Entry<Integer, List<Integer>> chunkX : posList.entrySet()) {
			for (Integer z : chunkX.getValue()) {
				int x = chunkX.getKey();
				List<LocationStorage> localStorages = handler.getLocalStorages(LocationStorage.class, x << 4, z << 4,
						input -> true);
				if (localStorages.size() > 0) {
					for (LocationStorage storage : localStorages) {
						AxisAlignedBB aabb = storage.getEnclosingBounds();
						Vec3d center = new Vec3d(aabb.minX + (aabb.maxX - aabb.minX) * 0.5D,
								aabb.minY + (aabb.maxY - aabb.minY) * 0.5D, aabb.minZ + (aabb.maxZ - aabb.minZ) * 0.5D);
						...
						if (location != Location.NONE) {
							amateMapData
									.addDecoration(new AmateMapData.BLMapDecoration(location, mapX, mapZ, (byte) 8));
							...
							if (done) {
								amateMapData.addDecoration(
										new AmateMapData.BLMapDecoration(Location.CHECK, mapX, mapZ, (byte) 8));
							}
						}
					}
				}
	...
	public Packet<?> createMapDataPacket(ItemStack stack, World worldIn, EntityPlayer player) {
		...
		if (p instanceof SPacketMaps) {
			...
			return TheBetweenlands.networkWrapper
					.getPacketFrom(new MessageAmateMap(stack.getItemDamage(), mapData, (SPacketMaps) p));
		} else {
			return p;
		}
	}
