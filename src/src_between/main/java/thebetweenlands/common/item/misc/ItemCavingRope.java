...
public class ItemCavingRope extends Item {
	...
	public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing,
		...
		if (!world.isRemote) {
			...
			if (connectedRopeNode == null) {
				...
				world.playSound((EntityPlayer) null, ropeNode.posX, ropeNode.posY, ropeNode.posZ,
						SoundEvents.BLOCK_METAL_STEP, SoundCategory.PLAYERS, 1, 1.5F);
				stack.shrink(1);
			} else {
				if (connectedRopeNode.getDistance(pos.getX() + hitX, pos.getY() + hitY,
						pos.getZ() + hitZ) > EntityRopeNode.ROPE_LENGTH) {
					player.sendStatusMessage(new TextComponentTranslation("chat.rope.too_far"), true);

					return EnumActionResult.FAIL;
				} else {
					EntityRopeNode ropeNode = connectedRopeNode.extendRope(player, pos.getX() + hitX, pos.getY() + hitY,
							pos.getZ() + hitZ);
					world.playSound((EntityPlayer) null, ropeNode.posX, ropeNode.posY, ropeNode.posZ,
							SoundEvents.BLOCK_METAL_STEP, SoundCategory.PLAYERS, 1, 1.5F);
					stack.shrink(1);
	...
	public void addInformation(ItemStack stack, World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
		tooltip.addAll(
				ItemTooltipHandler.splitTooltip(
						I18n.format("tooltip.caving_rope", KeyBindRegistry.CONNECT_CAVING_ROPE.getDisplayName(),
								StringUtils.ticksToElapsedTime(BetweenlandsConfig.GENERAL.cavingRopeDespawnTime * 20)),
						0));
	}
}
