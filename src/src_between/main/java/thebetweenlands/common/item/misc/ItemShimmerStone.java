...
public class ItemShimmerStone extends Item {
	...
	public ItemShimmerStone() {
		...
		this.addPropertyOverride(new ResourceLocation("shimmer"),
				(stack, worldIn, entityIn) -> NBTHelper.getStackNBTSafe(stack).getBoolean("shimmering") ? 1.0F : 0.0F);
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
		...
		if (!world.isRemote) {
			...
			double mx = (double) (-MathHelper.sin(player.rotationYaw / 180.0F * (float) Math.PI)
					* MathHelper.cos(player.rotationPitch / 180.0F * (float) Math.PI) * strength);
	...
	protected void triggerAdvancement(EntityPlayer player) {
		if (player != null && !player.world.isRemote && player instanceof EntityPlayerMP
				&& player.world.provider.getDimension() == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId) {
			BlockPos pos = player.getPosition();
			List<EntityPeatMummy> mummies = player.world.getEntitiesWithinAABB(EntityPeatMummy.class,
					new AxisAlignedBB(pos, pos).grow(20));
			if (mummies.size() > 0)
