...
public class ItemBoneWayfinder extends Item implements IRenamableItem, IAnimatorRepairable {
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
		...
		if (player.isSneaking()) {
			if (!world.isRemote) {
				player.openGui(TheBetweenlands.instance, CommonProxy.GUI_ITEM_RENAMING, world,
			...
			if (waystone != null) {
				...
				if (spawnPoint != null) {
					...
					((EntityPlayerMP) entity)
							.sendStatusMessage(new TextComponentTranslation("chat.waystone.obstructed"), true);
				}
			}
		}
		return stack;
	...
	public void addInformation(ItemStack stack, World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
		if (stack.hasTagCompound() && stack.getTagCompound().hasKey("link", Constants.NBT.TAG_LONG)) {
			...
			tooltip.addAll(ItemTooltipHandler.splitTooltip(I18n.translateToLocalFormatted(
					"tooltip.bone_wayfinder_linked", waystone.getX(), waystone.getY(), waystone.getZ()), 0));
		} else {
			tooltip.addAll(
					ItemTooltipHandler.splitTooltip(I18n.translateToLocalFormatted("tooltip.bone_wayfinder"), 0));
		}
	}
	...
	public void onUsingTick(ItemStack stack, EntityLivingBase entity, int count) {
		if (!entity.world.isRemote) {
			...
			if (entity instanceof EntityPlayer && !((EntityPlayer) entity).isCreative() && count < 60
					&& entity.ticksExisted % 3 == 0) {
				int removed = ItemRing.removeXp((EntityPlayer) entity, 1);
				if (removed == 0) {
					entity.stopActiveHand();
					entity.world.playSound(null, entity.posX, entity.posY, entity.posZ,
							SoundEvents.ITEM_FLINTANDSTEEL_USE, SoundCategory.PLAYERS, 1, 1);
				}
			}

			if (count < 90 && count % 20 == 0) {
				entity.world.playSound(null, entity.posX, entity.posY, entity.posZ, SoundRegistry.PORTAL_TRAVEL,
						SoundCategory.PLAYERS, 0.05F + 0.4F * (float) MathHelper.clamp(80 - count, 1, 80) / 80.0F,
						0.9F + entity.world.rand.nextFloat() * 0.2F);
			}
		} else {
			Random rand = entity.world.rand;
			for (int i = 0; i < MathHelper.clamp(60 - count, 1, 60); i++) {
				entity.world.spawnParticle(EnumParticleTypes.SUSPENDED_DEPTH,
						entity.posX + (rand.nextBoolean() ? -1 : 1) * Math.pow(rand.nextFloat(), 2) * 6,
						entity.posY + rand.nextFloat() * 4 - 2,
						entity.posZ + (rand.nextBoolean() ? -1 : 1) * Math.pow(rand.nextFloat(), 2) * 6, 0, 0.2D, 0);
			}
		}
	}
	...
	protected boolean activateWaystone(World world, BlockPos pos, IBlockState state, ItemStack stack) {
		...
		if (block.isValidWaystone(world, pos, state)) {
			...
			if (!world.isRemote) {
				...
				List<LocationStorage> waystoneLocations = BetweenlandsWorldStorage.forWorld(world)
						.getLocalStorageHandler().getLocalStorages(LocationStorage.class,
								new AxisAlignedBB(pos.getX(), pos.getY() + startY, pos.getZ(), pos.getX() + 1,
										pos.getY() + startY + 3, pos.getZ() + 1),
								storage -> storage.getType() == EnumLocationType.WAYSTONE);
	...
	protected void spawnWaystoneParticles(World world, BlockPos pos, BlockWaystone.Part part) {
		...
		for (int yo = startY; yo < startY + 3; yo++) {
			for (int i = 0; i < 4; i++) {
				Vec3d dir = new Vec3d(world.rand.nextFloat() - 0.5F, world.rand.nextFloat() - 0.5F + 0.25F,
						world.rand.nextFloat() - 0.5F);
				dir = dir.normalize().scale(2);
				BLParticles.CORRUPTED.spawn(world, pos.getX() + 0.5D + world.rand.nextFloat() / 2.0F - 0.25F,
						pos.getY() + yo + 0.5D + world.rand.nextFloat() / 2.0F - 0.25F,
						pos.getZ() + 0.5D + world.rand.nextFloat() / 2.0F - 0.25F,
