...
public class TestItemChimp extends Item {
	...
	public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand,
		...
		if (!worldIn.isRemote) {
			/*
			 * IBlockState state = worldIn.getBlockState(pos); if (state.getBlock()
			 * instanceof BlockGenericDugSoil) { TileEntityDugSoil te = (TileEntityDugSoil)
			 * worldIn.getTileEntity(pos); te.setDecay(20); } /* /* WorldGenTarPoolDungeon
			 * gen = new WorldGenTarPoolDungeon(); gen.generate(worldIn, itemRand,
			 * pos.up());
			 */
			/*
			 ...
			 */
			/*
			 * WorldGenIdolHeads head = new WorldGenIdolHeads(); head.generate(worldIn,
			 * itemRand, pos.up());
			 */
			/*
			 ...
			 */
			/*
			 * WorldGenSmallRuins ruins = new WorldGenSmallRuins(); ruins.generate(worldIn,
			 * itemRand, pos.up());
			 */
			/*
			 * if(player.isSneaking()) { BetweenlandsWorldData worldStorage =
			 * BetweenlandsWorldData.forWorld(worldIn); List<SharedStorage> storages =
			 * worldStorage.getSharedStorageAt(SharedStorage.class, (storage) -> {
			 * if(storage instanceof LocationStorage) { return
			 * ((LocationStorage)storage).isInside(pos); } return true; }, pos.getX(),
			 * pos.getZ()); for(SharedStorage storage : storages) {
			 * worldStorage.removeSharedStorage(storage); } } else { WorldGenWightFortress
			 * fortress = new WorldGenWightFortress(); fortress.generate(worldIn, itemRand,
			 * pos.up()); }
			 */
			/*
			 * ItemAspectContainer container =
			 * ItemAspectContainer.fromItem(player.getHeldItem(hand));
			 * if(!player.isSneaking()) { container.add(AspectRegistry.AZUWYNN, 10);
			 * System.out.println("Added: 10"); } else { System.out.println("Drained: " +
			 * container.drain(AspectRegistry.AZUWYNN, 8)); }
			 */
			/*
			 * WorldGenSpawner spawner = new WorldGenSpawner(); if(spawner.generate(worldIn,
			 * itemRand, pos)) { //playerIn.setHeldItem(hand, null); }
			 */
			 ...
			 * 
			 * 
			 * /* ItemStack stack = player.getHeldItem(hand); NBTTagCompound nbt =
			 * stack.getOrCreateSubCompound("pos");
			 * 
			 * if(!nbt.hasKey("x1")) { nbt.setInteger("x1", pos.getX());
			 * nbt.setInteger("y1", pos.getY()); nbt.setInteger("z1", pos.getZ()); } else {
			 * long time = System.nanoTime();
			 * 
			 * WorldGenGiantRoot root = new WorldGenGiantRoot(new
			 * BlockPos(nbt.getInteger("x1"), nbt.getInteger("y1"), nbt.getInteger("z1")),
			 * pos, 14); root.generate(worldIn, itemRand, pos);
			 * 
			 ...
			 */
			/*
			 * WorldGenGiantTree tree = new WorldGenGiantTree(); tree.generate(worldIn,
			 * itemRand, pos.add(0, 10, 0));
			 */
			/*
			 * WorldGenSmallPortal portal = new
			 * WorldGenSmallPortal(player.getHorizontalFacing());
			 ...
			 */
			/*
			 * EntitySplodeshroom trap = new EntitySplodeshroom(worldIn); BlockPos offset =
			 * pos.offset(facing); trap.setPosition(offset.getX() + 0.5F, offset.getY(),
			 * offset.getZ() + 0.5F); worldIn.spawnEntity(trap);
			 */
			/*
			 * EntityCCGroundSpawner trap = new EntityCCGroundSpawner(worldIn);
			 * trap.setPosition(pos.getX() + 0.5F, pos.getY(), pos.getZ() + 0.5F);
			 * trap.onInitialSpawn(worldIn.getDifficultyForLocation(trap.getPosition()),
			 * null); worldIn.spawnEntity(trap);
			 */
			/*
