	...
	public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand,
		...
		if (worldIn.isRemote) {
			...
			if (player.canPlayerEdit(pos, facing, stack) && player.canPlayerEdit(headPos, facing, stack)) {
				...
				if (isFootPlaceable && isHeadPlaceable && worldIn.getBlockState(pos.down()).isTopSolid()
						&& worldIn.getBlockState(headPos.down()).isTopSolid()) {
					IBlockState bedState = BlockRegistry.MOSS_BED.getDefaultState()
							.withProperty(BlockBed.OCCUPIED, Boolean.valueOf(false))
							.withProperty(BlockBed.FACING, placementFacing)
							.withProperty(BlockBed.PART, BlockBed.EnumPartType.FOOT);

					if (worldIn.setBlockState(pos, bedState, 10)) {
						worldIn.setBlockState(headPos, bedState.withProperty(BlockBed.PART, BlockBed.EnumPartType.HEAD),
								10);
					}

					SoundType sound = bedState.getBlock().getSoundType(bedState, worldIn, pos, player);
					worldIn.playSound((EntityPlayer) null, pos, sound.getPlaceSound(), SoundCategory.BLOCKS,
