...
public class ItemMagicItemMagnet extends Item implements IEquippable, IAnimatorRepairable {
	...
	public void onEquipmentTick(ItemStack stack, Entity entity, IInventory inventory) {
		if (stack.getItemDamage() < stack.getMaxDamage()) {
			...
			AxisAlignedBB area = new AxisAlignedBB(entity.posX, entity.posY + entity.height / 2, entity.posZ,
					entity.posX, entity.posY + entity.height / 2, entity.posZ).grow(range);
			List<EntityItem> entities = entity.world.getEntitiesWithinAABB(EntityItem.class, area,
					e -> e.getDistanceSq(entity.posX, entity.posY + entity.height / 2, entity.posZ) <= range * range);
			...
			for (EntityItem item : entities) {
				if (!item.hasNoGravity()) {
					...
					if (nbt.hasKey("thebetweenlands.item_magnet_last_gravity_update", Constants.NBT.TAG_INT)
							&& nbt.getInteger("thebetweenlands.item_magnet_last_gravity_update") == item.ticksExisted) {
	...
	protected void spawnParticles(EntityItem item) {
		if (item.ticksExisted % 4 == 0) {
			BLParticles.CORRUPTED.spawn(item.world, item.posX, item.posY + item.height / 2.0f + 0.25f, item.posZ,
					ParticleArgs.get().withScale(0.5f));
		}
	}
	...
	public static void onItemPickup(ItemPickupEvent event) {
		if (!event.player.world.isRemote) {
			ItemStack magnet = EquipmentHelper.getEquipment(EnumEquipmentInventory.MISC, event.player,
					ItemRegistry.MAGIC_ITEM_MAGNET);
			if (!magnet.isEmpty()) {
