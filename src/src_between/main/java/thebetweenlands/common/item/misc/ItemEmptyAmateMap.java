...
public class ItemEmptyAmateMap extends ItemMapBase {
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
		...
		if (worldIn.provider.getDimension() == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId) {
			ItemStack itemstack = ItemAmateMap.setupNewMap(worldIn, playerIn.posX, playerIn.posZ, (byte) 4, true,
					false);
			itemstack1.shrink(1);
			...
			if (!worldIn.isRemote) {
				playerIn.sendStatusMessage(new TextComponentTranslation("chat.amate_map.invalid")
						.setStyle(new Style().setColor(TextFormatting.RED)), false);
			}
