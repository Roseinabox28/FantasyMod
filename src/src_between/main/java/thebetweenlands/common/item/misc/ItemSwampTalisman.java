...
public class ItemSwampTalisman extends Item implements ItemRegistry.IBlockStateItemModelDefinition {
	...
	public void addInformation(ItemStack stack, World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
		if (EnumTalisman.SWAMP_TALISMAN_5.isItemOf(stack) && stack.hasTagCompound()
				&& stack.getTagCompound().hasKey("link", Constants.NBT.TAG_LONG)) {
			BlockPos otherPortalPos = BlockPos.fromLong(stack.getTagCompound().getLong("link"));
			tooltip.addAll(
					ItemTooltipHandler.splitTooltip(I18n.translateToLocalFormatted("tooltip.swamp_talisman_linked",
							otherPortalPos.getX(), otherPortalPos.getY(), otherPortalPos.getZ()), 0));
		}
	}
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand,
		...
		if (!playerIn.canPlayerEdit(pos, facing, stack)) {
						...
						if (!worldIn.isRemote) {
							...
							for (EnumFacing dir : EnumFacing.VALUES) {
								if (dir.getAxis() == frameAxis) {
									if (closestDir == null || pos.offset(dir).distanceSq(playerIn.posX, playerIn.posY,
											playerIn.posZ) <= pos.offset(closestDir).distanceSq(playerIn.posX,
							...
							if (frameAxis == EnumFacing.Axis.X
									&& BlockTreePortal.isPatternValidX(worldIn, portalPos.up())
									|| frameAxis == EnumFacing.Axis.Z
											&& BlockTreePortal.isPatternValidZ(worldIn, portalPos.up())) {
								...
								if (this.getPortalAt(worldIn, portalPos.up()) == null) {
									...
									LocationPortal location = new LocationPortal(worldStorage,
											new StorageUUID(UUID.randomUUID()), LocalRegion.getFromBlockPos(pos),
											portalPos.offset(closestDir).down());
									location.addBounds(
											new AxisAlignedBB(portalPos.up()).grow(1, 2, 1).expand(0, -0.5D, 0));
								...
								worldIn.playSound(null, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D,
										SoundRegistry.PORTAL_ACTIVATE, SoundCategory.PLAYERS, 0.5F,
			...
			if ((sapling || isCustomListed) && (EnumTalisman.SWAMP_TALISMAN_0.isItemOf(stack)
					|| EnumTalisman.SWAMP_TALISMAN_5.isItemOf(stack))) {
				if (!worldIn.isRemote) {
					if (!BetweenlandsConfig.WORLD_AND_DIMENSION.portalDimensionWhitelistSet
							.isListed(playerIn.world.provider.getDimension())) {
						playerIn.sendStatusMessage(new TextComponentTranslation("chat.talisman.wrongdimension"), true);
					} else {
						WorldGenWeedwoodPortalTree gen;
						if (isCustomListed) {
							int targetDim = BetweenlandsConfig.WORLD_AND_DIMENSION.portalDimensionTargetsList
									.getDimension(state);
							if (targetDim == playerIn.world.provider.getDimension()) {
								gen = new WorldGenWeedwoodPortalTree();
							} else {
								gen = new WorldGenWeedwoodPortalTree(
										BetweenlandsConfig.WORLD_AND_DIMENSION.portalDimensionTargetsList
												.getDimension(state));
							}
						} else {
							gen = new WorldGenWeedwoodPortalTree();
						...
						if (gen.generate(worldIn, itemRand, pos)) {
							worldIn.playSound(null, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D,
									SoundRegistry.PORTAL_ACTIVATE, SoundCategory.PLAYERS, 0.5F,
									itemRand.nextFloat() * 0.4F + 0.8F);
							playerIn.setLocationAndAngles(pos.getX() + 0.5D, pos.getY() + 2D, pos.getZ() + 0.5D,
									playerIn.rotationYaw, playerIn.rotationPitch);
							if (playerIn instanceof EntityPlayerMP) {
								((EntityPlayerMP) playerIn).connection.setPlayerLocation(pos.getX() + 0.5D,
										pos.getY() + 2D, pos.getZ() + 0.5D, playerIn.rotationYaw,
				...
				if (portal != null) {
					if (!worldIn.isRemote) {
						...
						worldIn.playSound(null, playerIn.posX, playerIn.posY, playerIn.posZ,
								SoundEvents.ITEM_FLINTANDSTEEL_USE, SoundCategory.PLAYERS, 0.8F, 0.7F);
					}
					return EnumActionResult.SUCCESS;
				}
				...
				if (stack.hasTagCompound() && stack.getTagCompound().hasKey("link", Constants.NBT.TAG_LONG)
						&& stack.getTagCompound().hasKey("linkDim", Constants.NBT.TAG_INT)) {
					BlockPos otherPortalPos = BlockPos.fromLong(stack.getTagCompound().getLong("link"));
					LocationPortal portal = this.getPortalAt(worldIn, pos);
					if (portal != null) {
						if (worldIn instanceof WorldServer) {
							...
							if (linkDim != worldIn.provider.getDimension()
									&& (linkDim == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId
											|| worldIn.provider
													.getDimension() == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId)) {
								WorldServer otherWorld = ((WorldServer) worldIn).getMinecraftServer().getWorld(linkDim);
								if (otherWorld != null) {
									double moveFactor = otherWorld.provider.getMovementFactor()
											/ worldIn.provider.getMovementFactor();
									if (new Vec3d(portal.getPortalPosition()).distanceTo(new Vec3d(
											otherPortalPos.getX() * moveFactor, portal.getPortalPosition().getY(),
											otherPortalPos.getZ()
													* moveFactor)) <= BetweenlandsConfig.WORLD_AND_DIMENSION.portalMaxLinkDist) {
										LocationPortal linkPortal = this.getLinkPortal(otherWorld, otherPortalPos);
										if (linkPortal != null) {
											linkPortal.setOtherPortalPosition(worldIn.provider.getDimension(),
													portal.getPortalPosition());
											portal.setOtherPortalPosition(linkDim, linkPortal.getPortalPosition());
											playerIn.sendStatusMessage(
													new TextComponentTranslation("chat.talisman.portal_linked"), true);
										} else {
											playerIn.sendStatusMessage(
													new TextComponentTranslation("chat.talisman.cant_link"), true);
										}
									} else {
										playerIn.sendStatusMessage(new TextComponentTranslation("chat.talisman.too_far",
												new TextComponentString(String.valueOf(
														BetweenlandsConfig.WORLD_AND_DIMENSION.portalMaxLinkDist))),
												true);
									}
								}
							} else {
								playerIn.sendStatusMessage(new TextComponentTranslation("chat.talisman.cant_link"),
										true);
							}
						}
					}
	...
	protected LocationPortal getPortalAt(World world, BlockPos pos) {
		...
		List<LocationPortal> portals = worldStorage.getLocalStorageHandler().getLocalStorages(LocationPortal.class,
				pos.getX() + 0.5D, pos.getZ() + 0.5D,
				location -> location.isInside(new Vec3d(pos).add(0.5D, 0.5D, 0.5D)));
		if (!portals.isEmpty()) {
	...
	protected LocationPortal getLinkPortal(WorldServer world, BlockPos portal2Pos) {
		BetweenlandsWorldStorage worldStorage = BetweenlandsWorldStorage.forWorld(world);
		List<LocationPortal> portals = worldStorage.getLocalStorageHandler().getLocalStorages(LocationPortal.class,
				portal2Pos.getX() + 0.5D, portal2Pos.getZ() + 0.5D,
				location -> location.isInside(new Vec3d(portal2Pos).add(0.5D, 0.5D, 0.5D))
						&& portal2Pos.equals(location.getPortalPosition()));
		if (!portals.isEmpty()) {
			return portals.get(0);
		}
		return null;
	...
	protected boolean isBlockSapling(World worldIn, EntityPlayer playerIn, IBlockState state, BlockPos pos,
			EnumFacing facing, float hitX, float hitY, float hitZ) {
	...
	protected EnumFacing.Axis getPortalWoodFrameAxis(World world, BlockPos pos) {
		...
		if (this.isPortalWood(world.getBlockState(pos)) && this.isPortalWood(world.getBlockState(pos.offset(north)))
				&& this.isPortalWood(world.getBlockState(pos.offset(south)))
				&& this.isPortalWood(world.getBlockState(pos.up().offset(north)))
				&& this.isPortalWood(world.getBlockState(pos.up().offset(south)))
				&& this.isPortalWood(world.getBlockState(pos.up(2).offset(north)))
				&& this.isPortalWood(world.getBlockState(pos.up(2).offset(south)))
				&& this.isPortalWood(world.getBlockState(pos.up(3)))
				&& this.isPortalWood(world.getBlockState(pos.up(3).offset(north)))
				&& this.isPortalWood(world.getBlockState(pos.up(3).offset(south)))) {
			return EnumFacing.Axis.X;
		}

		EnumFacing east = EnumFacing.EAST;
		EnumFacing west = EnumFacing.WEST;
		if (this.isPortalWood(world.getBlockState(pos)) && this.isPortalWood(world.getBlockState(pos.offset(east)))
				&& this.isPortalWood(world.getBlockState(pos.offset(west)))
				&& this.isPortalWood(world.getBlockState(pos.up().offset(east)))
				&& this.isPortalWood(world.getBlockState(pos.up().offset(west)))
				&& this.isPortalWood(world.getBlockState(pos.up(2).offset(east)))
				&& this.isPortalWood(world.getBlockState(pos.up(2).offset(west)))
				&& this.isPortalWood(world.getBlockState(pos.up(3)))
				&& this.isPortalWood(world.getBlockState(pos.up(3).offset(east)))
				&& this.isPortalWood(world.getBlockState(pos.up(3).offset(west)))) {
	...
	public enum EnumTalisman implements IGenericItem {
		SWAMP_TALISMAN_0, SWAMP_TALISMAN_1, SWAMP_TALISMAN_2, SWAMP_TALISMAN_3, SWAMP_TALISMAN_4, SWAMP_TALISMAN_5;
