...
public abstract class ItemPlantable extends Item {
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand,
		...
		if (isReplacing || (worldIn.isAirBlock(facingOffset)
				|| worldIn.getBlockState(facingOffset).getBlock().isReplaceable(worldIn, facingOffset))) {
			...
			if (placeBlock != null && block != placeBlock && placeBlock.canPlaceBlockAt(worldIn, newPos)) {
				if (!worldIn.isRemote) {
					...
					worldIn.playSound((EntityPlayer) null, (float) pos.getX() + 0.5F, (float) pos.getY() + 0.5F,
							(float) pos.getZ() + 0.5F, placeBlock.getSoundType().getPlaceSound(), SoundCategory.BLOCKS,
							(placeBlock.getSoundType().getVolume() + 1.0F) / 2.0F,
							placeBlock.getSoundType().getPitch() * 0.8F);
					stack.shrink(1);
				}
	...
	protected IBlockState getBlockState(Block block, ItemStack stack, EntityPlayer playerIn, World worldIn,
			BlockPos pos) {
