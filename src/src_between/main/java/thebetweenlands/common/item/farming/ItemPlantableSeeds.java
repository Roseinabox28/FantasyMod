...
public class ItemPlantableSeeds extends ItemSeeds {
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand,
		...
		BlockBush bush = this.crops.get().getBlock() instanceof BlockBush ? (BlockBush) this.crops.get().getBlock()
				: null;
		BlockPos up = pos.up();
		if (facing == EnumFacing.UP && playerIn.canPlayerEdit(pos.offset(facing), facing, stack)
				&& (bush == null ? state.getBlock().canSustainPlant(state, worldIn, pos, EnumFacing.UP, this)
						: bush.canSustainPlant(state, worldIn, pos, EnumFacing.UP, bush))
				&& worldIn.isAirBlock(up) && (this.soilMatcher == null || this.soilMatcher.test(state))) {
	...
	protected void onPlant(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX,
			float hitY, float hitZ, IBlockState state) {
