...
public class ItemBucketInfusion extends Item implements ITintedItem, ItemRegistry.IMultipleItemModelDefinition {
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> list, ITooltipFlag flagIn) {
		if (hasTag(stack)) {
			if (stack.getTagCompound() != null && stack.getTagCompound().hasKey("infused")
					&& stack.getTagCompound().hasKey("ingredients") && stack.getTagCompound().hasKey("infusionTime")) {
				int infusionTime = stack.getTagCompound().getInteger("infusionTime");
				list.add(I18n.format("tooltip.infusion.time", StringUtils.ticksToElapsedTime(infusionTime),
						infusionTime));
				...
				for (Map.Entry<ItemStack, Integer> stackCount : stackMap.entrySet()) {
					...
					if (!ingredient.isEmpty()) {
						...
						List<Aspect> ingredientAspects = AspectManager.get(TheBetweenlands.proxy.getClientWorld())
								.getDiscoveredStaticAspects(AspectManager.getAspectItem(ingredient),
										DiscoveryContainer.getMergedDiscoveryContainer(
	...
	public EnumActionResult onItemUseFirst(EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hitX,
		...
		if (player.isSneaking()) {
			pos = pos.offset(side);
			world.playSound(player, pos, SoundEvents.BLOCK_FIRE_EXTINGUISH, SoundCategory.BLOCKS, 1.0F,
					itemRand.nextFloat() * 0.4F + 0.8F);
			for (int i = 0; i < 50; i++) {
				world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, pos.getX() + world.rand.nextFloat(),
						pos.getY() + world.rand.nextFloat(), pos.getZ() + world.rand.nextFloat(), 0, 0, 0);
			}
	...
	public List<IAspectType> getInfusingAspects(ItemStack stack) {
		...
		if (hasTag(stack)) {
			if (stack.getTagCompound() != null && stack.getTagCompound().hasKey("infused")
				...
				for (int i = 0; i < nbtList.tagCount(); i++) {
					ItemStack ingredient = new ItemStack(nbtList.getCompoundTagAt(i));
					ItemAspectContainer container = ItemAspectContainer.fromItem(ingredient,
							AspectManager.get(TheBetweenlands.proxy.getClientWorld()));
					for (Aspect aspect : container.getAspects()) {
						infusingAspects.add(aspect.type);
					}
					// infusingAspects.addAll(AspectManager.get(TheBetweenlands.proxy.getClientWorld()).getDiscoveredAspectTypes(AspectManager.getAspectItem(ingredient),
					// null));
				}
			}
		}
		...
		if (tintIndex == 1) {
			...
			if (recipe != null) {
				if (infusionTime > recipe.idealInfusionTime + recipe.infusionTimeVariation) {
					...
					return this.getColorFromRGBA(finishedColor[0], finishedColor[1], finishedColor[2],
							finishedColor[3]);
				} else {
					float startR = 0.2F;
					float startG = 0.6F;
	...
	private int getColorFromRGBA(float r, float g, float b, float a) {
		return ((int) (a * 255.0F) << 24) | ((int) (r * 255.0F) << 16) | ((int) (g * 255.0F) << 8)
				| ((int) (b * 255.0F));
	}
