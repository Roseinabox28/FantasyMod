...
public class ItemSpecificBucket extends ItemBLBucket {
	...
	public void onFillBucket(FillBucketEvent event) {
		...
		if (targetFluidHandler != null) {
			...
			if (fluidStack != null && fluidStack.amount == this.fluidStack.amount
					&& fluidStack.getFluid() == this.fluidStack.getFluid()) {
				fluidStack = targetFluidHandler.drain(this.fluidStack, true);
				world.playSound(null, pos, fluidStack.getFluid().getFillSound(fluidStack), SoundCategory.BLOCKS, 1.0F,
						1.0F);
