...
public class ItemGreataxe extends ItemGreatsword {
	...
	public void onUpdate(ItemStack stack, World world, Entity holder, int slot, boolean isHeldItem) {
		...
		if (holder instanceof EntityPlayerMP && !holder.world.isRemote) {
			...
			if (player.getHeldItemMainhand() == stack && this.isLongSwingInProgress(stack)
					&& this.getSwingStartCooledAttackStrength(stack) > 0.85F) {
				...
				for (int xo = -blockReach; xo <= blockReach; xo++) {
					for (int yo = -blockReach; yo <= blockReach; yo++) {
						for (int zo = -blockReach; zo <= blockReach; zo++) {
							BlockPos pos = new BlockPos(player.posX + xo, player.posY + player.height * 0.5D + yo,
									player.posZ + zo);
							...
							if (dist < breakReach) {
								...
								if (angle >= minAngle && angle < maxAngle) {
									double distXZ = Math.sqrt((center.x - player.posX) * (center.x - player.posX)
											+ (center.z - player.posZ) * (center.z - player.posZ));

									if (player.getLookVec().y * distXZ + player.posY + player.height / 2
											+ 0.25D > pos.getY() - 0.25D
											&& player.getLookVec().y * distXZ + player.posY + player.height / 2
													+ 0.25D < pos.getY() + 1.25D) {
										IBlockState state = player.world.getBlockState(pos);

										if ((state.getBlock().isWood(player.world, pos)
												|| state.getMaterial() == Material.WOOD)
												&& state.getBlockHardness(player.world, pos) <= 2.25F
												&& state.getPlayerRelativeBlockHardness(player, player.world,
														pos) > 0.01F) {
	...
	public Multimap<String, AttributeModifier> getAttributeModifiers(EntityEquipmentSlot equipmentSlot,
		...
		if (equipmentSlot == EntityEquipmentSlot.MAINHAND) {
			...
			multimap.put(SharedMonsterAttributes.ATTACK_SPEED.getName(),
					new AttributeModifier(ATTACK_SPEED_MODIFIER, "Tool modifier", -3.45D, 0));
		}
