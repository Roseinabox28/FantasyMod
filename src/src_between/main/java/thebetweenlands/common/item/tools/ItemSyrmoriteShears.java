...
public class ItemSyrmoriteShears extends ItemShears implements IAnimatorRepairable {
	...
	// TODO: For sickle
	/*
	 * protected List<ItemStack> getDefaultDrops(World worldIn, BlockPos pos,
	 * IBlockState state, int fortune) { List<ItemStack> finalItems = new
	 * ArrayList<>(); List<ItemStack> items = getDrops(worldIn, pos, state,
	 * fortune); float chance =
	 * net.minecraftforge.event.ForgeEventFactory.fireBlockHarvesting(items,
	 * worldIn, pos, state, fortune, 1.0F, false, harvesters.get()); for (ItemStack
	 * item : items) { if (worldIn.rand.nextFloat() <= chance) {
	 * finalItems.add(item); } } return finalItems; }
	 */
}
