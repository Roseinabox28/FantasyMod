...
public class ItemLootSword extends ItemBLSword {
	...
	public boolean hitEntity(ItemStack stack, EntityLivingBase attacked, EntityLivingBase attacker) {
		if (!this.instantKills.isEmpty() && this.instantKills.contains(attacked.getClass())) {
			...
			float corrosionMultiplier = 1.0F - (corrosion > maxCorrosion / 2.0F
					? ((corrosion - maxCorrosion / 2.0F) / (float) (maxCorrosion / 2.0F))
					: 0);
			attacked.attackEntityFrom(DamageSource.causeIndirectMagicDamage(attacker, attacker),
					attacked.getMaxHealth() * corrosionMultiplier);
		}
