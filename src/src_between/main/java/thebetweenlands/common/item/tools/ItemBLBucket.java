...
public class ItemBLBucket extends UniversalBucket implements ItemRegistry.IMultipleItemModelDefinition {
	...
	private static final class FluidBLBucketHandler extends FluidHandlerItemStackSimple {
		...
		public boolean canFillFluidType(FluidStack fluid) {
			boolean canHold = container.getMetadata() == 1
					|| (container.getMetadata() == 0 && fluid.getFluid().getTemperature(fluid) <= 430);
			return canHold && (fluid.getFluid() == FluidRegistry.WATER || fluid.getFluid() == FluidRegistry.LAVA
		...
		protected void setFluid(FluidStack fluid) {
			...
			if (!container.isEmpty() && container
					.isItemEqualIgnoreDurability(((ItemBLBucket) container.getItem()).getEmpty(container))) {
	...
	public String getItemStackDisplayName(ItemStack stack) {
		...
		String fluidUnlocKey = unlocName + "." + fluidStack.getUnlocalizedName() + ".name"; // Unlocalized full bucket
																							// name
		// Try to find localization for this specific fluid bucket, if not found use a
		// generic name with the fluid passed in as %s
		if (I18n.canTranslate(fluidUnlocKey))
			return I18n.translateToLocal(fluidUnlocKey).trim();

		return I18n.translateToLocalFormatted(getEmpty(stack).getTranslationKey() + ".filled.name",
				fluidStack.getFluid().getRarity(fluidStack).color + fluidStack.getLocalizedName()
	...
	public ActionResult<ItemStack> onItemRightClick(final World world, final EntityPlayer player, final EnumHand hand) {
		...
		if (fluidStack != null) {
			...
			if (result.getType() == EnumActionResult.SUCCESS)
				world.playSound(null, pos, fluidStack.getFluid().getEmptySound(fluidStack), SoundCategory.BLOCKS, 1.0F,
						1.0F);
			return result;
		}
		...
		if (world.isBlockModifiable(player, pos) && player.canPlayerEdit(pos, target.sideHit, heldItem)) {
			...
			final FluidActionResult filledResult = FluidUtil.tryPickUpFluid(singleBucket, player, world, pos,
					target.sideHit);
			if (filledResult.isSuccess()) {
				FluidStack fluidStack1 = getFluid(filledResult.getResult());
				world.playSound(null, pos, fluidStack1.getFluid().getFillSound(fluidStack1), SoundCategory.BLOCKS, 1.0F,
						1.0F);
	...
	private ActionResult<ItemStack> tryPlaceTreeTap(final World world, final EntityPlayer player,
		...
		if (result != null && result.typeOfHit == RayTraceResult.Type.BLOCK
			...
			if (player.canPlayerEdit(pos, result.sideHit, itemStack)) {
				...
				Block rubberTap = itemStack.getMetadata() == 0 ? BlockRegistry.WEEDWOOD_RUBBER_TAP
						: BlockRegistry.SYRMORITE_RUBBER_TAP;
				...
				if (blockState.getBlock() == BlockRegistry.LOG_RUBBER && blockState.getValue(BlockRubberLog.NATURAL)) {
					...
					if (world.getBlockState(offset).getBlock().isReplaceable(world, offset)
							&& rubberTap.canPlaceBlockAt(world, offset)) {
						world.setBlockState(offset,
								rubberTap.getDefaultState().withProperty(BlockRubberTap.FACING, result.sideHit));
						itemStack.shrink(1);
		...
		for (int i = 0; i < 2; i++) {
			for (final Fluid fluid : FluidRegistry.getRegisteredFluids().values()) {
				if (fluid != FluidRegistry.WATER && fluid != FluidRegistry.LAVA && !fluid.getName().equals("milk")
						&& !ItemSpecificBucket.hasSpecificBucket(fluid)) {
					if (!BetweenlandsConfig.COMPATIBILITY.showNonBLFluids
							&& !thebetweenlands.common.registries.FluidRegistry.REGISTERED_FLUIDS.contains(fluid))
						continue;
	...
	public ItemStack withFluid(int meta, Fluid fluid) {
		...
		final IFluidHandlerItem fluidHandler = stack.getCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY,
				null);
