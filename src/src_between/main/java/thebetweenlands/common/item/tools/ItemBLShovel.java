...
public class ItemBLShovel extends ItemSpade implements ICorrodible, IAnimatorRepairable {
	...
	public Multimap<String, AttributeModifier> getAttributeModifiers(EntityEquipmentSlot slot, ItemStack stack) {
		return CorrosionHelper.getAttributeModifiers(super.getAttributeModifiers(slot, stack), slot, stack,
				ItemTool.ATTACK_DAMAGE_MODIFIER, this.attackDamage);
	}
	...
	public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing,
		...
		if (facing == EnumFacing.UP) {
			...
			if (dug) {
				if (world.isRemote) {
					for (int i = 0; i < 80; i++) {
						world.spawnParticle(EnumParticleTypes.BLOCK_CRACK, pos.getX() + 0.5F, pos.getY() + 1,
								pos.getZ() + 0.5F, (world.rand.nextFloat() - 0.5F) * 0.1F,
								world.rand.nextFloat() * 0.3f, (world.rand.nextFloat() - 0.5F) * 0.1F,
								new int[] { Block.getStateId(blockState) });
					}
				}
				...
				for (int i = 0; i < 3; i++) {
					world.playSound(null, pos.getX() + hitX, pos.getY() + hitY, pos.getZ() + hitZ,
							sound.getBreakSound(), SoundCategory.PLAYERS, 1, 0.5f + world.rand.nextFloat() * 0.5f);
				}
