...
public class ItemBLShield extends ItemShield implements IAnimatorRepairable {
	 ...
	 * @return
	 */
	public int getShieldBlockingCooldown(ItemStack stack, EntityLivingBase attacked, float damage,
			DamageSource source) {
		return 0;
	}
	...
	public void onAttackBlocked(ItemStack stack, EntityLivingBase attacked, float damage, DamageSource source) {
		if (!attacked.world.isRemote) {
			...
			if (source.getTrueSource() instanceof EntityLivingBase) {
				...
				if (!attackerItem.isEmpty()
						&& attackerItem.getItem().canDisableShield(attackerItem, stack, attacked, attacker)) {
					float attackStrength = attacker instanceof EntityPlayer
							? ((EntityPlayer) attacker).getCooledAttackStrength(0.5F)
							: 1.0F;
	...
	public float getBlockedDamage(ItemStack stack, EntityLivingBase attacked, float damage, DamageSource source) {
		// float multiplier = 0.4F - Math.min(this.material.getAttackDamage() / 3.0F,
		// 1.0F) * 0.4F;
	 ...
	 * @return
	 */
	public float getDefenderKnockbackMultiplier(ItemStack stack, EntityLivingBase attacked, float damage,
			DamageSource source) {
	 ...
	 * @return
	 */
	public boolean canBlockDamageSource(ItemStack stack, EntityLivingBase attacked, EnumHand hand,
			DamageSource source) {
		if (attacked.isHandActive() && attacked.getActiveHand() == hand && !source.isUnblockable()
				&& attacked.isActiveItemStackBlocking()) {
			Vec3d vec3d = source.getDamageLocation();
			if (vec3d != null) {
				Vec3d vec3d1 = attacked.getLook(1.0F);
				Vec3d vec3d2 = vec3d.subtractReverse(new Vec3d(attacked.posX, attacked.posY, attacked.posZ))
						.normalize();
				...
				if (!stack.isEmpty() && stack.getItem() instanceof ItemBLShield) {
					...
					if (shield.canBlockDamageSource(stack, attacked, hand, source)) {
						...
						if (!attacked.world.isRemote) {
							...
							float defenderKbMultiplier = shield.getDefenderKnockbackMultiplier(stack, attacked,
							...
							if (newDamage > 0.0F) {
								...
								if (source instanceof EntityDamageSourceIndirect) {
									newSource = new EntityDamageSourceIndirect(source.damageType,
							...
							if (source.getTrueSource() != null) {
								// Knock back defender
								double prevMotionY = attacked.motionY;
								attacked.knockBack(source.getTrueSource(), defenderKbMultiplier,
										source.getTrueSource().posX - attacked.posX,
										source.getTrueSource().posZ - attacked.posZ);
								attacked.motionY = prevMotionY;
						...
						if (!attacked.world.isRemote) {
							if (source.getTrueSource() == source.getImmediateSource()
								...
								float attackerKbMultiplier = shield.getAttackerKnockbackMultiplier(stack, attacked,
										event.getAmount(), source);
								if (attackerKbMultiplier > 0.0F) {
									((EntityLivingBase) source.getTrueSource()).knockBack(attacked,
											attackerKbMultiplier, attacked.posX - source.getTrueSource().posX,
											attacked.posZ - source.getTrueSource().posZ);
								}
							}
						}

						if (attacked instanceof EntityPlayer) {
							int cooldown = shield.getShieldBlockingCooldown(stack, (EntityPlayer) attacked,
									event.getAmount(), source);
							if (cooldown > 0) {
