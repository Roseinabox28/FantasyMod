...
public class ItemVoodooDoll extends Item {
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
		...
		List<EntityLivingBase> living = world.getEntitiesWithinAABB(EntityLivingBase.class,
				new AxisAlignedBB(player.posX, player.posY, player.posZ, player.posX, player.posY, player.posZ).grow(5,
						5, 5));
		...
		for (EntityLivingBase entity : living) {
			if (entity.isEntityAlive() && !(entity instanceof IBLBoss) && entity instanceof EntityPlayer == false) {
				if (!world.isRemote) {
					...
					attacked = true;
					for (int i = 0; i < 20; i++)
						BLParticles.SWAMP_SMOKE.spawn(world, entity.posX, entity.posY + entity.height / 2.0D,
								entity.posZ,
								ParticleFactory.ParticleArgs.get().withMotion((world.rand.nextFloat() - 0.5F) * 0.5F,
										(world.rand.nextFloat() - 0.5F) * 0.5F, (world.rand.nextFloat() - 0.5F) * 0.5F)
										.withColor(1, 1, 1, 1));
				}
			}
		}
		if (!world.isRemote && attacked) {
			...
			world.playSound(null, player.posX, player.posY, player.posZ, SoundRegistry.VOODOO_DOLL,
					SoundCategory.PLAYERS, 0.5F, 1.0F - world.rand.nextFloat() * 0.3F);
		}
