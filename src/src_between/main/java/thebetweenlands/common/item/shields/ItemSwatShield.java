...
public class ItemSwatShield extends ItemBLShield {
	public ItemSwatShield(ToolMaterial material) {
		...
		this.addPropertyOverride(new ResourceLocation("charging"), (stack, worldIn,
				entityIn) -> entityIn != null && entityIn.isHandActive() && entityIn.getActiveItemStack() == stack
						&& (getRemainingChargeTicks(stack, entityIn) > 0 || isPreparingCharge(stack, entityIn)) ? 1.0F
								: 0.0F);
	}
	...
	/**
	 * Returns for how many ticks the user can charge for the specified preparation
	 * ticks
	 * 
	 * @param stack
	 * @param user
	...
	public void onChargingUpdate(ItemStack stack, EntityLivingBase user) {
		...
		if (Math.sqrt(user.motionX * user.motionX + user.motionZ * user.motionZ) > 0.2D) {
			...
			List<EntityLivingBase> targets = user.world.getEntitiesWithinAABB(EntityLivingBase.class,
					user.getEntityBoundingBox().grow(1), e -> e != user);
	...
	public void onUsingTick(ItemStack stack, EntityLivingBase user, int count) {
		...
		if (preparing && !user.isSneaking() && runningTicks <= 0) {
			this.setRemainingChargeTicks(stack, user,
					this.getChargeTime(stack, user, this.getPreparingChargeTicks(stack, user)));
	...
	public static void onUpdateFov(FOVUpdateEvent event) {
		...
		if (!activeItem.isEmpty() && activeItem.getItem() instanceof ItemSwatShield
				&& (swatShield = (ItemSwatShield) activeItem.getItem()).isPreparingCharge(activeItem,
						event.getEntity())) {
			int preparingTicks = swatShield.getPreparingChargeTicks(activeItem, event.getEntity());
			float progress = Math.min(swatShield.getChargeTime(activeItem, event.getEntity(), preparingTicks)
					/ (float) swatShield.getMaxChargeTime(activeItem, event.getEntity()), 1);
