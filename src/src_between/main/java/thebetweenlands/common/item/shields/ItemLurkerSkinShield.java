...
public class ItemLurkerSkinShield extends ItemBLShield {
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
		...
		if (rayTrace != null) {
			...
			List<Entity> entities = worldIn.getEntitiesWithinAABBExcludingEntity(playerIn, playerIn
					.getEntityBoundingBox().expand(lookVec.x * 5.0D, lookVec.y * 5.0D, lookVec.z * 5.0D).grow(1.0D));
			...
			if (!entityColliding && rayTrace.typeOfHit == RayTraceResult.Type.BLOCK) {
				...
				if (isWater) {
					EntityBoat boat = new EntityLurkerSkinRaft(worldIn, rayTrace.hitVec.x, rayTrace.hitVec.y - 0.12D,
							rayTrace.hitVec.z, stack);
