...
public class ItemWeedwoodShield extends ItemBLShield {
	public ItemWeedwoodShield() {
		...
		this.addPropertyOverride(new ResourceLocation("burning"), (stack, worldIn,
				entityIn) -> ((ItemWeedwoodShield) stack.getItem()).getBurningTicks(stack) > 0 ? 1.0F : 0.0F);
	}
	...
	public void onAttackBlocked(ItemStack stack, EntityLivingBase attacked, float damage, DamageSource source) {
		...
		if (!attacked.world.isRemote && source.getTrueSource() != null) {
			...
			if ((attacker.isBurning() || attacker instanceof EntitySmallFireball)
					&& attacked.world.rand.nextFloat() < 0.5F) {
				...
				if (!activeItem.isEmpty()) {
					...
					if (item == ItemRegistry.OCTINE_AXE || item == ItemRegistry.OCTINE_PICKAXE
							|| item == ItemRegistry.OCTINE_SHIELD || item == ItemRegistry.OCTINE_SHOVEL
							|| item == ItemRegistry.OCTINE_SWORD)
	...
	public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
		if (!worldIn.isRemote && entityIn != null) {
			...
			if (burningTicks > 0) {
				...
				if (burningTicks % 5 == 0)
					worldIn.playSound((EntityPlayer) null, (double) ((float) entityIn.posX),
							(double) ((float) entityIn.posY + entityIn.getEyeHeight()),
							(double) ((float) entityIn.posZ), SoundEvents.BLOCK_FIRE_AMBIENT, SoundCategory.BLOCKS,
							1.0F + worldIn.rand.nextFloat(), worldIn.rand.nextFloat() * 0.7F + 0.3F);
	...
	public boolean onEntityItemUpdate(EntityItem entityItem) {
		...
		if (!entityItem.world.isRemote) {
			...
			if (burningTicks > 0) {
				this.setBurningTicks(stack, burningTicks - 1);
				if (burningTicks % 5 == 0)
					entityItem.world.playSound((EntityPlayer) null, (double) ((float) entityItem.posX),
							(double) ((float) entityItem.posY + entityItem.height / 2.0F),
							(double) ((float) entityItem.posZ), SoundEvents.BLOCK_FIRE_AMBIENT, SoundCategory.BLOCKS,
							1.0F + entityItem.world.rand.nextFloat(), entityItem.world.rand.nextFloat() * 0.7F + 0.3F);
				if (burningTicks % 3 == 0) {
					if (stack.attemptDamageItem(1, entityItem.world.rand, null)) {
						this.renderBrokenItemStack(entityItem.world, entityItem.posX,
								entityItem.posY + entityItem.height / 2.0F, entityItem.posZ, stack);
						stack.shrink(1);
						if (stack.getCount() < 0) {
							stack.setCount(0);
						}
	...
	public boolean shouldCauseReequipAnimation(ItemStack oldStack, ItemStack newStack, boolean slotChanged) {
		...
		boolean isBurning = newStack.getItem() instanceof ItemWeedwoodShield
				? ((ItemWeedwoodShield) newStack.getItem()).getBurningTicks(newStack) > 0
				: false;
		return (super.shouldCauseReequipAnimation(oldStack, newStack, slotChanged) && !isBurning
				|| isBurning != wasBurning)
				|| !NBTHelper.areItemStackTagsEqual(oldStack, newStack, STACK_NBT_EXCLUSIONS);
	}

	protected void renderBrokenItemStack(World world, double x, double y, double z, ItemStack stack) {
		world.playSound((EntityPlayer) null, x, y, z, SoundEvents.ENTITY_ITEM_BREAK, SoundCategory.NEUTRAL, 0.8F,
				0.8F + world.rand.nextFloat() * 0.4F);
		for (int i = 0; i < 5; ++i) {
			Vec3d motion = new Vec3d(((double) world.rand.nextFloat() - 0.5D) * 0.1D, Math.random() * 0.1D + 0.1D,
					0.0D);
			world.spawnParticle(EnumParticleTypes.ITEM_CRACK, x, y, z, motion.x, motion.y + 0.05D, motion.z,
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand hand) {
		...
		return isBurning ? new ActionResult<ItemStack>(EnumActionResult.PASS, itemStackIn)
