...
public class ItemLivingWeedwoodShield extends ItemWeedwoodShield {
			...
			if (!worldIn.isRemote) {
				...
				if (living.isActiveItemStackBlocking() && (mainhand || offhand)) {
					...
					if (worldIn.rand.nextInt(60) == 0) {
						worldIn.playSound(null, entityIn.posX, entityIn.posY + entityIn.height / 2, entityIn.posZ,
								SoundRegistry.SPIRIT_TREE_FACE_SMALL_LIVING, SoundCategory.PLAYERS, 0.35F, 1.4F);
					}
				}
			} else {
	...
	public void setSpitTicks(ItemStack stack, int ticks) {
		ILivingWeedwoodShieldCapability cap = stack.getCapability(CapabilityRegistry.CAPABILITY_LIVING_WEEDWOOD_SHIELD,
				null);
		if (cap != null) {
	...
	protected boolean trySpit(ItemStack stack, World world, EntityLivingBase owner, EnumHand hand) {
		...
		List<EntityLivingBase> entities = world.getEntitiesWithinAABB(EntityLivingBase.class,
				owner.getEntityBoundingBox().grow(8, 3, 8), IMob.MOB_SELECTOR::apply);

		Vec3d lookVec = owner.getLookVec().normalize();

		for (EntityLivingBase e : entities) {
			if (Math.toDegrees(Math.acos(
					lookVec.dotProduct(e.getPositionEyes(1).subtract(owner.getPositionEyes(1)).normalize()))) <= 70) {
		...
		if (target != null) {
			...
			Vec3d bodyForward = new Vec3d(MathHelper.sin(-yaw * 0.017453292F - (float) Math.PI), 0,
					MathHelper.cos(-yaw * 0.017453292F - (float) Math.PI));
			Vec3d up = new Vec3d(0, 1, 0);
			Vec3d right = bodyForward.crossProduct(up);
			Vec3d offset = new Vec3d(bodyForward.x, owner.getEyeHeight(), bodyForward.z)
					.add(right.scale(hand == EnumHand.MAIN_HAND ? 0.35D : -0.35D).add(0, lookVec.y * 0.5D - 0.4D, 0)
							.add(bodyForward.scale(-0.1D)));

			EntitySapSpit spit = new EntitySapSpit(world, owner, 4.5F);
			spit.setPosition(owner.posX + owner.motionX + offset.x, owner.posY + offset.y,
					owner.posZ + owner.motionZ + offset.z);
			...
			world.playSound(null, spit.posX, spit.posY, spit.posZ, SoundRegistry.SPIRIT_TREE_FACE_SMALL_SPIT,
					SoundCategory.PLAYERS, 1, 1);

			TheBetweenlands.networkWrapper.sendToAllAround(
					new MessageLivingWeedwoodShieldSpit(owner, hand == EnumHand.MAIN_HAND, 15),
					new TargetPoint(owner.dimension, owner.posX, owner.posY, owner.posZ, 64));
