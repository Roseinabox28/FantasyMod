...
public class ItemBLArmor extends ItemArmor implements IAnimatorRepairable {
	...
	public ItemBLArmor(ArmorMaterial materialIn, int renderIndexIn, EntityEquipmentSlot equipmentSlotIn,
			String armorName) {
	...
	public void renderHelmetOverlay(ItemStack stack, EntityPlayer player, ScaledResolution resolution,
		...
		if (overlay != null) {
			...
			renderRepeatingOverlay((float) resolution.getScaledWidth_double(),
					(float) resolution.getScaledHeight_double(), overlay,
					this.getOverlaySideTexture(stack, player, partialTicks, true),
					this.getOverlaySideTexture(stack, player, partialTicks, false));
	...
	public static void renderRepeatingOverlay(float width, float height, ResourceLocation overlay,
			@Nullable ResourceLocation sideOverlayLeft, @Nullable ResourceLocation sideOverlayRight) {
		if (overlay != null) {
