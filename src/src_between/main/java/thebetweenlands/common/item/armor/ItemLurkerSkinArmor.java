	...
	public void onArmorTick(World world, EntityPlayer player, ItemStack itemStack) {
		if (!player.isSpectator()) {
			...
			if (itemStack.getItem() == ItemRegistry.LURKER_SKIN_BOOTS && player.isInWater()) {
				IBlockState blockState = player.world.getBlockState(
						new BlockPos(player.posX, player.getEntityBoundingBox().maxY + 0.1D, player.posZ));
				...
				if (armorPieces >= 4) {
					...
					if (player.getAir() <= -20) {
						...
						for (int i = 0; i < 8; ++i) {
							...
							player.world.spawnParticle(EnumParticleTypes.WATER_BUBBLE, player.posX + (double) rx,
									player.posY + (double) ry, player.posZ + (double) rz, player.motionX,
