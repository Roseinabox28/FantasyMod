...
public class SharedLootTableView extends LootTableView {
	...
	protected List<ItemStack> generateLootFromSharedPool(Random rand, LootContext context) {
		...
		if (lootTable != null) {
			...
			long sharedPoolSeed = rand.nextLong(); // Always generate this long so that the generation order doesn't
													// matter
			...
			for (EntryKey lootEntryKey : poolItems.keySet()) {
				...
				lootStackLoop: while (lootStackIT.hasNext()) {
					...
					sharedStackLoop: for (Entry<EntryKey, List<ItemStack>> sharedLootEntry : sharedPoolItems
							.entrySet()) {
						...
						while (sharedLootStackIT.hasNext()) {
							...
							if (lootStack.isEmpty() && sharedLootStack.isEmpty()) {
								...
								int removedItems = this.primarySharedPool.getRemovedItems(sharedLootEntryKey.pool,
										sharedLootEntryKey.poolRoll, sharedLootEntryKey.entry);
								this.primarySharedPool.setRemovedItems(sharedLootEntryKey.pool,
										sharedLootEntryKey.poolRoll, sharedLootEntryKey.entry, removedItems + 1);
							...
							} else if (!lootStack.isEmpty() && !sharedLootStack.isEmpty()
									&& lootStack.getItem() == sharedLootStack.getItem()) {
								int additionalAvailableCount = Math.min(lootStack.getCount() - availableLootCount,
										sharedLootStack.getCount());
								...
								if (additionalAvailableCount > 0) {
									...
									this.primarySharedPool.setRemovedItems(sharedLootEntryKey.pool,
											sharedLootEntryKey.poolRoll, sharedLootEntryKey.entry,
											removedItems + additionalAvailableCount);
	...
	protected void generateLootForPoolFromSharedPool(Map<EntryKey, List<ItemStack>> stacks, Random rand,
			LootContext context, LootPool pool, boolean isSharedPool) {
	...
	protected void generateLootFromSharedPool(Map<EntryKey, List<ItemStack>> stacks, Random rand, LootContext context,
			LootPool pool, boolean isSharedPool) {
		...
		if (this.testAllConditions(poolConditions, rand, context)) {
			...
			int count = rolls.generateInt(poolRand)
					+ MathHelper.floor(bonusRolls.generateFloat(poolRand) * context.getLuck());
	...
	@SuppressWarnings("unchecked")
	protected boolean createLootRollFromSharedPool(Map<EntryKey, List<ItemStack>> stacks, Random rand,
			LootContext context, LootPool pool, boolean isSharedPool, int poolRoll) {
		int totalWeights = 0;
		...
		for (LootEntry lootEntry : lootEntries) {
			LootConditionFromSharedPool fromSharedPoolCondition = LootConditionFromSharedPool.getCondition(rand,
					context, this.getEntryConditions(lootEntry, true));

			boolean isGuaranteed = !isSharedPool && fromSharedPoolCondition != null
					&& fromSharedPoolCondition.isGuaranteed(guaranteePercentage, guaranteeCounter);
			...
			if (isGuaranteed || this.testAllConditions(this.getEntryConditions(lootEntry, false), rand, context)) {
				...
				if (isGuaranteed || weight > 0) {
					...
					if (isSharedPool) {
						// Generated loot must be consistently the same for the same shared pool, roll
						// and entry
						entryRand = new Random(this.primarySharedPool.getLootEntrySeed(rand, pool.getName(), poolRoll,
								lootEntry.getEntryName()));
					}
		...
		if (!validEntries.isEmpty()) {
			for (Tuple<LootEntry, List<ItemStack>> validEntry : validEntries) {
				...
				if (isSharedPool) {
					int removedItems = this.primarySharedPool.getRemovedItems(pool.getName(), poolRoll,
							lootEntry.getEntryName());
				...
				if (isLootEntryStillAvailable) {
					...
					if (isSharedPool) {
						...
						key = new EntryKey(pool.getName(), poolRoll, lootEntry.getEntryName(),
								!LootConditionFromSharedPool.isFromSharedPool(rand, context,
										this.getEntryConditions(lootEntry, true)));
					}
