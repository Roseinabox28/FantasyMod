...
public class SharedLootPool implements ISharedLootPool {
	...
	public float getGuaranteePercentage() {
		int lootInventories = this.location != null ? this.location.getSharedLootInventories(this.lootTableLocation)
				: 0;
		return lootInventories <= 0 ? 1.0F
				: Math.min((float) this.getGuaranteeCounter() / (float) lootInventories, 1.0F);
	}
