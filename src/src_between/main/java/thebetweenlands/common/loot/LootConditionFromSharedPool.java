...
public class LootConditionFromSharedPool implements LootCondition {
	...
	public LootConditionFromSharedPool(LootCondition[] conditions, float guaranteedAfterPercentage,
			int guaranteedAfter) {
	...
	public boolean isGuaranteed(float guaranteePercentage, int guaranteeCounter) {
		return (this.guaranteedAfterPercentage >= 0 && this.guaranteedAfterPercentage <= guaranteePercentage)
				|| (this.guaranteedAfter >= 0 && this.guaranteedAfter <= guaranteeCounter);
	}

	/**
	 * Returns whether the loot entry with the specified loot conditions gets items
	...
	@Nullable
	public static LootConditionFromSharedPool getCondition(Random rand, LootContext context,
			List<LootCondition> conditions) {
		for (LootCondition condition : conditions) {
			if (condition instanceof LootConditionFromSharedPool && LootConditionManager
					.testAllConditions(((LootConditionFromSharedPool) condition).conditions, rand, context)) {
	...
	public static class Serializer extends LootCondition.Serializer<LootConditionFromSharedPool> {
		...
		public LootConditionFromSharedPool deserialize(JsonObject json, JsonDeserializationContext context) {
			...
			if (json.has("conditions")) {
				conditions = (LootCondition[]) JsonUtils.deserializeClass(json, "conditions", context,
						LootCondition[].class);
			} else {
