...
public class LootConditionSharedPool implements LootCondition {
	...
	/**
	 * Returns whether the pool with the specified loot conditions belongs to a
	 * shared pool
	 * 
	 * @param rand
	 * @param context
	...
	public static boolean isSharedPool(Random rand, LootContext context, List<LootCondition> conditions) {
		for (LootCondition condition : conditions) {
			if (condition instanceof LootConditionSharedPool && LootConditionManager
					.testAllConditions(((LootConditionSharedPool) condition).conditions, rand, context)) {
				return true;
			}
		}
