...
public class BiomeSludgePlainsClearing extends BiomeBetweenlands {
	public BiomeSludgePlainsClearing() {
		super(new ResourceLocation(ModInfo.ID, "sludge_plains_clearing"),
				new BiomeProperties("Sludge Plains Clearing").setBaseHeight(WorldProviderBetweenlands.LAYER_HEIGHT + 4)
						.setHeightVariation(0F).setWaterColor(0x3A2F0B).setTemperature(0.8F).setRainfall(0.9F));

		this.setWeight(0);
		this.getBiomeGenerator().setTopBlockState(BlockRegistry.COMPACTED_MUD.getDefaultState()).setFillerBlockHeight(1)
				.setDecorator(new BiomeDecoratorSludgePlainsClearing(this))
