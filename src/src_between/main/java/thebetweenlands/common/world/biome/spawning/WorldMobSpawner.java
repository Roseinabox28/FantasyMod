...
public class WorldMobSpawner extends AreaMobSpawner {
	...
	private static final int MAX_SPAWN_CHUNKS_PER_AREA = (SPAWN_CHUNK_MAX_RANGE * 2 + 1)
			* (SPAWN_CHUNK_MAX_RANGE * 2 + 1) - (SPAWN_CHUNK_MIN_RANGE * 2 + 1) * (SPAWN_CHUNK_MIN_RANGE * 2 + 1);
				...
				for (int xo = -SPAWN_CHUNK_MAX_RANGE; xo <= SPAWN_CHUNK_MAX_RANGE; ++xo) {
					for (int zo = -SPAWN_CHUNK_MAX_RANGE; zo <= SPAWN_CHUNK_MAX_RANGE; ++zo) {
						...
						if (world.getWorldBorder().contains(chunkpos)) {
							PlayerChunkMapEntry playerchunkmapentry = world.getPlayerChunkMap().getEntry(chunkpos.x,
									chunkpos.z);
	...
	public void populateChunk(WorldServer world, int chunkX, int chunkZ) {
		...
		if (world.getGameRules().getBoolean("doMobSpawning")) {
			...
			spawnedEntities += this.populateChunk(world, new ChunkPos(chunkX, chunkZ), spawnHostiles, spawnAnimals,
					false, true, this.getSpawningAttempsPerChunk() * CHUNK_GEN_SPAWN_RUNS, 60,
					this.getSpawningAttemptsPerGroup(), this.getHardEntityLimit(), 1.0F);
			// System.out.println("Spawned: " + spawnedEntities + " Time: " +
