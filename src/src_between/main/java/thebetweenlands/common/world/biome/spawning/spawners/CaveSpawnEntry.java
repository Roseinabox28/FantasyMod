...
/**
 * Makes entities spawn only spawn below cave level. Spawning weight gradually
 * increases as the y value decreases. Spawning weight at bedrock level is
 * baseWeight. Mostly used for hostile entities.
 */
public class CaveSpawnEntry extends BLSpawnEntry {
	...
	public CaveSpawnEntry(int id, Class<? extends EntityLiving> entityType,
			Function<World, ? extends EntityLiving> entityCtor, short baseWeight) {
	...
	@Override
	public boolean canSpawn(World world, Chunk chunk, BlockPos pos, IBlockState spawnBlockState,
			IBlockState surfaceBlockState) {
		return !spawnBlockState.isNormalCube() && ((SurfaceType.MIXED_GROUND_AND_UNDERGROUND.matches(surfaceBlockState)
				&& !spawnBlockState.getMaterial().isLiquid())
				|| (this.canSpawnInWater && spawnBlockState.getMaterial().isLiquid()) || (this.canSpawnOnWater
						&& surfaceBlockState.getMaterial().isLiquid() && !spawnBlockState.getMaterial().isLiquid()));
	}
}
