...
public final class TeleporterBetweenlands extends Teleporter {
		...
		if (!this.isToEnd || BetweenlandsConfig.WORLD_AND_DIMENSION.generatePortalInEnd) {
			if (!this.placeInExistingPortal(entity, rotationYaw)) {
				if (this.isToEnd) {
					...
					entity.setLocationAndAngles(entity.posX + this.world.rand.nextInt(32) - 16, entity.posY,
							entity.posZ + this.world.rand.nextInt(32) - 16, entity.rotationYaw, entity.rotationPitch);
				}
				...
				if (!this.makePortal(entity)) {
					if (this.setSpawn) {
						if (!this.makePortal) {
							...
							this.setEntityLocation(entity, pos.getX() + 0.5D, pos.getY(), pos.getZ() + 0.5D,
							...
							for (int xo = -1; xo <= 1; xo++) {
								for (int zo = -1; zo <= 1; zo++) {
									this.toWorld.setBlockState(pos.add(xo, -1, zo), BlockRegistry.LOG_PORTAL
											.getDefaultState().withProperty(BlockLog.LOG_AXIS, BlockLog.EnumAxis.NONE));
								}
							}

							this.setEntityLocation(entity, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D,
									rotationYaw, 0);
		...
		for (int zo = -2; zo <= 2; ++zo) {
			for (int xo = -2; xo <= 2; ++xo) {
				for (int yo = -1; yo < (setAir ? 3 : 0); ++yo) {
					...
					int bz = z + zo;
					boolean air = yo < 0;
					this.world.setBlockState(new BlockPos(bx, by, bz),
							air ? Blocks.OBSIDIAN.getDefaultState() : Blocks.AIR.getDefaultState());
				}
			}
		}
	}
	...
	public boolean placeInExistingPortal(Entity entity, float rotationYaw) {
		if (this.setSpawn) {
			...
			if (existingPortal != null) {
				...
				this.setEntityLocation(entity, existingPortal.getX() + 0.5D, existingPortal.getY() + 2.0D,
						existingPortal.getZ() + 0.5D, rotationYaw, 0);
	...
	protected LocationPortal getPortalLocation() {
		BetweenlandsWorldStorage worldStorage = BetweenlandsWorldStorage
				.forWorld(this.toWorld.getMinecraftServer().getWorld(this.fromDim));
		List<LocationPortal> portals = worldStorage.getLocalStorageHandler().getLocalStorages(LocationPortal.class,
				this.fromBounds, loc -> loc.intersects(this.fromBounds));
	...
	protected LocationPortal getOtherPortalLocation(@Nullable BlockPos pos) {
		if (pos != null) {
			...
			List<LocationPortal> otherPortals = worldStorage.getLocalStorageHandler().getLocalStorages(
					LocationPortal.class, pos.getX(), pos.getZ(), loc -> loc.getPortalPosition().equals(pos));
	...
	/**
	 * Validates a list of portals and if invalid they are removed from the list and
	 * the world
	 * 
	 * @param portals
	 * @return
	...
	protected BlockPos findSuitableBetweenlandsPortalPos(BlockPos start) {
		...
		Biome[] biomes = this.toWorld.getBiomeProvider().getBiomesForGeneration(new Biome[0], searchStartX,
				searchStartZ, searchWidth, searchDepth);
		...
		for (int i = 0; i < searchWidth * searchDepth; ++i) {
			...
			if (validBiomes.contains(biome) && bx > border.minX() + 16 && bz > border.minZ() + 16
					&& bx < border.maxX() - 16 && bz < border.maxZ() - 16
	...
	protected BlockPos findSuitableNonBLStartPos(BlockPos start) {
		...
		for (int i = 0; i < searchWidth * searchDepth; ++i) {
			...
			if (!unsafeBiomes.contains(biome.getRegistryName().toString()) && bx > border.minX() + 16
					&& bz > border.minZ() + 16 && bx < border.maxX() - 16 && bz < border.maxZ() - 16
					&& (suitablePos == null || this.toWorld.rand.nextInt(counter + 1) == 0)) {
				suitablePos = new BlockPos(bx, 0, bz);
				++counter;
			}
	...
	protected boolean generateBetweenlandsTreePortal(Entity entity, BlockPos center) {
		...
		return this.spiralGenerate(center, 64, 0, 0, checkPos -> {
			...
			if (checkPos.getX() > border.minX() + 16 && checkPos.getZ() > border.minZ() + 16
					&& checkPos.getX() < border.maxX() - 16 && checkPos.getZ() < border.maxZ() - 16) {
				Chunk chunk = this.getDecoratedChunk(this.toWorld, checkPos); // Force chunk to generate
				checkPos.setY(chunk.getHeight(checkPos) - 1);

				if (SurfaceType.MIXED_GROUND.matches(this.toWorld.getBlockState(checkPos))
						&& this.toWorld.isAirBlock(checkPos.up())
						&& this.canGeneratePortalTree(this.toWorld, checkPos)) {
	...
	protected boolean generateTreePortal(Entity entity, BlockPos center) {
		...
		return this.spiralGenerate(center, 64, Math.min(center.getY() - 2, 8), 8, checkPos -> {
			...
			if (checkPos.getX() > border.minX() + 16 && checkPos.getZ() > border.minZ() + 16
					&& checkPos.getX() < border.maxX() - 16 && checkPos.getZ() < border.maxZ() - 16
					&& this.toWorld.getBlockState(checkPos).isNormalCube() && this.toWorld.isAirBlock(checkPos.up())
					&& this.canGeneratePortalTree(this.toWorld, checkPos)) {
	...
	protected boolean generateSmallPortal(Entity entity, BlockPos center) {
		...
		if (this.spiralGenerate(center, 64, Math.min(center.getY() - 2, 8), 8, checkPos -> {
			...
			if (checkPos.getX() > border.minX() + 6 && checkPos.getZ() > border.minZ() + 6
					&& checkPos.getX() < border.maxX() - 6 && checkPos.getZ() < border.maxZ() - 6
					&& this.toWorld.getBlockState(checkPos).isNormalCube() && this.toWorld.isAirBlock(checkPos.up())
					&& this.canGenerateSmallPortalInOpen(this.toWorld, checkPos.up())) {
		...
		if (this.spiralGenerate(center, 32, Math.min(center.getY() - 2, 8), 8, checkPos -> {
			...
			if (checkPos.getX() > border.minX() + 6 && checkPos.getZ() > border.minZ() + 6
					&& checkPos.getX() < border.maxX() - 6 && checkPos.getZ() < border.maxZ() - 6
					&& this.toWorld.getBlockState(checkPos).isNormalCube()
					&& !this.isSmallPortalObstructedByOthersOrLiquid(this.toWorld, checkPos)) {
				BlockPos pos = checkPos.toImmutable().up();
				if (genPortal.generate(this.toWorld, this.toWorld.rand, pos)) {
					this.lonkPortalsTogetherAndTeleport(entity, pos, 0.5D, 1.0D, -0.5D);
					if (this.isToEnd) {
	...
	protected void lonkPortalsTogetherAndTeleport(Entity entity, BlockPos newPortalPos, double playerOffsetX,
			double playerOffsetY, double playerOffsetZ) {
		LocationPortal portal = this.getPortalLocation();

		if (portal != null) {
			BetweenlandsWorldStorage worldStorage = BetweenlandsWorldStorage.forWorld(this.toWorld);
			List<LocationPortal> newPortals = worldStorage.getLocalStorageHandler().getLocalStorages(
					LocationPortal.class, newPortalPos.getX(), newPortalPos.getZ(),
					loc -> loc.isInside(newPortalPos.up(2)));
		...
		if (this.setSpawn) {
			this.setEntityLocation(entity, newPortalPos.getX() + playerOffsetX, newPortalPos.getY() + playerOffsetY,
					newPortalPos.getZ() + playerOffsetZ, entity.rotationYaw, 0);
			this.setDefaultPlayerSpawnLocation(newPortalPos, entity);
		}
	}

	protected boolean spiralGenerate(BlockPos center, int radius, int yDown, int yUp,
			Function<MutableBlockPos, Boolean> gen) {
		MutableBlockPos checkPos = new MutableBlockPos();

		// Spiral from center outwards to stay as close to the preferred position as
		// possible
		int xo = 0, zo = 0;
		int[] dir = new int[] { 0, -1 };
			...
			for (int zo = -maxRadius; zo <= maxRadius; zo++) {
				if (Math.sqrt(xo * xo + zo * zo) <= maxRadius) {
					for (int yo = -3; yo < height; yo++) {
						...
						if ((yo >= 2 && (blockState.getMaterial().isLiquid() || blockState.isNormalCube()))
	...
	protected boolean canGenerateSmallPortalInOpen(World world, BlockPos pos) {
		for (MutableBlockPos p : BlockPos.getAllInBoxMutable(pos.getX() - 3, pos.getY(), pos.getZ() - 3, pos.getX() + 3,
				pos.getY() + 7, pos.getZ() + 3)) {
			IBlockState blockState = world.getBlockState(p);
			boolean isOutside = Math.abs(pos.getX() - p.getX()) >= 2 || Math.abs(pos.getZ() - p.getZ()) >= 2
					|| p.getY() - pos.getY() >= 5;
			if (!isOutside && (blockState.getMaterial().isLiquid() || blockState.isNormalCube()
					|| blockState.getBlock() instanceof BlockLeaves)) {
				return false;
