...
public class BetweenlandsChunkStorage extends ChunkStorageImpl {
	...
	/**
	 * Encodes a position into an index that might be contained in
	 * {@link #findGems(thebetweenlands.common.item.misc.ItemGemSinger.GemSingerTarget)}
	 * 
	 * @param x X
	 * @param y Y
	 * @param z Z
	...
	/**
	 * Decodes a position index retrieved from
	 * {@link #findGems(thebetweenlands.common.item.misc.ItemGemSinger.GemSingerTarget)}
	 ...
	 * 
	 * @param target Gem target type
	 * @return an unmodifiable set of all gem positions which can be decoded by
	 *         {@link #getGemSingerTargetPosition(int)}
	 */
	 ...
	 * @return position of a gem <b>in world coordinates</b>, or null if none was
	 *         found
	 */
	@Nullable
	...
	/**
	 * @see #markGem(int, int, int,
	 *      thebetweenlands.common.item.misc.ItemGemSinger.GemSingerTarget)
	 * 
	 * @param world  World
	 ...
	 */
	@Nullable
	public static BlockPos findRandomGem(World world, ItemGemSinger.GemSingerTarget target, Random rand, BlockPos pos,
			float range) {
		BetweenlandsChunkStorage storage = forChunk(world, world.getChunk(pos));
		if (storage != null) {
