...
public class BetweenlandsWorldStorage extends WorldStorageImpl {
	...
	protected void init() {
		...
		if (!this.getWorld().isRemote) {
			...
			this.aspectManager.loadAndPopulateStaticAspects(null,
					AspectManager.getAspectsSeed(this.getWorld().getWorldInfo().getSeed()));
		}
	...
	public void readFromNBT(NBTTagCompound nbt) {
		if (!this.getWorld().isRemote) {
			...
			this.aspectManager.loadAndPopulateStaticAspects(nbt.getCompoundTag("itemAspects"),
					AspectManager.getAspectsSeed(this.getWorld().getWorldInfo().getSeed()));
			...
			for (int i = 0; i < spiritTreeKillTokensNbt.tagCount(); i++) {
				this.spiritTreeKillTokens
						.add(SpiritTreeKillToken.readFromNBT(spiritTreeKillTokensNbt.getCompoundTagAt(i)));
			}
		}
	}
	...
	public void tick() {
		...
		if (this.getWorld().isRemote
				&& this.getWorld().provider.getDimension() == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId) {
	...
	protected boolean playAmbientCaveSounds(EntityPlayer player, int x, int z, Chunk chunk) {
		...
		if (state.getMaterial() == Material.AIR && world.getLight(pos) <= world.rand.nextInt(8)
				&& world.getLightFor(EnumSkyBlock.SKY, pos) <= 0) {
			double dst = player.getDistanceSq((double) pos.getX() + 0.5D, (double) pos.getY() + 0.5D,
					(double) pos.getZ() + 0.5D);

			if (dst > 4.0D && dst < 256.0D) {
				world.playSound((double) pos.getX() + 0.5D, (double) pos.getY() + 0.5D, (double) pos.getZ() + 0.5D,
						SoundRegistry.AMBIENT_CAVE_SPOOK, SoundCategory.AMBIENT, 0.85F,
						0.8F + world.rand.nextFloat() * 0.2F, false);
	...
	public static BetweenlandsWorldStorage forWorld(World world) {
		...
		if (storage == null) {
			throw new RuntimeException(String.format("World %s (%s) does not have BetweenlandsWorldStorage capability",
					world.getWorldInfo().getWorldName(), world.provider.getClass().getName()));
		}
		return storage;
	}
