...
public class AmateMapData extends MapData {
	...
	public void addDecoration(BLMapDecoration deco) {
		...
		if (!this.decorations.containsKey(index)) {
			...
			for (int i = -area; i <= area; i++) {
				for (int j = -area; j <= area; j++) {
					if (i * i + j * j <= area * area) {
						int offsetIndex = ((((x >> gridSize) + i) + ((y >> gridSize) + j) * (128 >> gridSize)) << 8)
								| deco.location.id;
			...
			if (!occupied) {
				this.occupiedSpots
						.add((((x >> gridSize) + (y >> gridSize) * (128 >> gridSize)) << 8) | deco.location.id);
				this.decorations.put(index, deco);
			} else {
				this.decorations.put(index, new BLMapDecoration(BLMapDecoration.Location.SMALL_MARKER, deco.getX(),
						deco.getY(), deco.getRotation()));
			}
		}
	}
	...
	public static class BLMapDecoration extends MapDecoration implements Comparable<BLMapDecoration> {
		...
		private static final ResourceLocation MAP_ICONS = new ResourceLocation(ModInfo.ID,
				"textures/gui/map_icon_sheet.png");
		...
		public boolean render(int index) {
			...
			// We don't care about depth, just the rendering order which is already sorted
			// out
		...
		public enum Location {
			NONE(0, 0, 0, 0, 0), SMALL_MARKER(12, 32, 16, 16, 16), PORTAL(1, 0, 0, 16, 16), SPAWN(2, 16, 0, 16, 16),
			SHRINE(3, 32, 0, 16, 16), GIANT_TREE(4, 48, 0, 16, 16), RUINS(5, 64, 0, 16, 16), TOWER(6, 80, 0, 16, 16),
			IDOL(7, 96, 0, 16, 16), WAYSTONE(8, 112, 0, 16, 16), BURIAL_MOUND(9, 0, 16, 16, 16),
			SPIRIT_TREE(10, 16, 16, 16, 16), FORTRESS(11, 0, 104, 22, 24),
