...
public class LocationCragrockTower extends LocationGuarded {
	...
	protected static final DataParameter<Boolean> CRUMBLING = GenericDataManager.createKey(LocationCragrockTower.class,
			DataSerializers.BOOLEAN);
	protected static final DataParameter<Integer> CRUMBLING_TICKS = GenericDataManager
			.createKey(LocationCragrockTower.class, DataSerializers.VARINT);
	...
	/**
	 * Sets whether the top was reached and changes all glowing cragrock blocks
	 * accordingly
	 * 
	 * @param reached
	 */
	...
	/**
	 * Adds an inactive glowing cragrock block that is activated once the top is
	 * reached
	 * 
	 * @param pos
	 */
	...
	/**
	 * Sets whether the tower was entered. Wisps are changed accordingly in the next
	 * tick
	 * 
	 * @param entered
	 * @return
	...
	public void restoreBlockade(int level) {
		...
		if (blocks != null && blocks.length != 0) {
			...
			for (BlockPos pos : blocks) {
				world.setBlockState(pos, BlockRegistry.SMOOTH_CRAGROCK_SLAB.getDefaultState()
				...
				if (this.wispUpdateTicks >= 10) {
					...
					for (int i = 0; i < 4; i++) {
						if (!this.inactiveWisps.isEmpty()) {
							...
							if ((closest.getY() - this.structurePos.getY() < 16
									&& player.posY - this.structurePos.getY() < 45)
									|| (closest.getY() - this.structurePos.getY() >= 16
											&& player.posY - this.structurePos.getY() >= 45)) {
								canLightUp = true;
							}

							if (canLightUp) {
								world.setBlockState(closest, BlockRegistry.WISP.getDefaultState()
										.withProperty(BlockWisp.COLOR, world.rand.nextInt(4)));
								world.playSound(null, closest.getX(), closest.getY(), closest.getZ(),
										SoundRegistry.IGNITE, SoundCategory.AMBIENT,
										1.6F + world.rand.nextFloat() * 0.45F, 1.0F + world.rand.nextFloat() * 0.4F);
	...
	public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
		...
		for (int i = 0; i < 5; i++) {
			...
			this.saveBlockList(blockadeNbt, "blocks",
					blocks != null && blocks.length != 0 ? Arrays.asList(blocks) : new ArrayList<>());
	...
	public void update() {
		...
		if (!world.isRemote) {
			List<EntityPlayer> players = world.getEntitiesWithinAABB(EntityPlayer.class, this.getEnclosingBounds(),
					player -> this.isInside(player) && !player.isCreative() && !player.isSpectator());

			if (!players.isEmpty()) {
				if (this.isTopConquered() && !this.isCrumbling() && this.getCrumblingTicks() == 0) {
					List<EntityPlayer> topPlayers = world.getEntitiesWithinAABB(EntityPlayer.class,
							this.getLevelBounds(5).grow(5, 3, 5),
				...
				for (EntityPlayer player : players) {
					...
					if (this.isTopConquered() && player instanceof EntityPlayerMP
							&& this.getLevel(MathHelper.floor(player.posY)) == 5) {
						AdvancementCriterionRegistry.CRAGROCK_TOP.trigger((EntityPlayerMP) player);
					} else if (!this.isTopReached
							&& !this.getInnerBoundingBox().grow(0.5D, 0.5D, 0.5D).contains(player.getPositionVector())
							&& player.posY - structurePos.getY() > 12) {
						// Player trying to bypass tower, teleport to entrance

						player.dismountRidingEntity();
						if (player instanceof EntityPlayerMP) {
							EntityPlayerMP playerMP = (EntityPlayerMP) player;
							playerMP.connection.setPlayerLocation(structurePos.getX() + 0.5D, structurePos.getY(),
									structurePos.getZ() + 0.5D, player.rotationYaw, player.rotationPitch);
						} else {
							player.setLocationAndAngles(structurePos.getX() + 0.5D, structurePos.getY(),
									structurePos.getZ() + 0.5D, player.rotationYaw, player.rotationPitch);
						}
						player.fallDistance = 0.0F;
						player.addPotionEffect(new PotionEffect(MobEffects.BLINDNESS, 60, 2));
						player.world.playSound(null, player.posX, player.posY, player.posZ,
								SoundRegistry.FORTRESS_BOSS_TELEPORT, SoundCategory.AMBIENT, 1, 1);
					}
				}
			...
			if (!this.isTopConquered()) {
				for (int i = 0; i < 5; i++) {
					if (this.getBlockadeState(i) && !this.getSpawnerState(i)) {
						List<EntityLivingBase> entities = world.getEntitiesWithinAABB(EntityLivingBase.class,
								this.getLevelBounds(i),
								entity -> entity instanceof EntityMob || entity instanceof IMob);
						if (entities.isEmpty()) {
				...
				if (this.topSpawners == 0) {
					List<EntityLivingBase> entities = world.getEntitiesWithinAABB(EntityLivingBase.class,
							this.getLevelBounds(5).grow(3, 2, 3),
							entity -> entity instanceof EntityMob || entity instanceof IMob);
					if (entities.isEmpty()) {
			...
			if (this.isCrumbling()) {
				for (int i = 0; i < Math.min(Math.pow(this.getCrumblingTicks() / 400.0f, 4) * 30.0f, 30) + 1; i++) {
					...
					if (blockState.getBlock() != Blocks.AIR && world.isAirBlock(pos.down())
							&& world.isAirBlock(pos.down(2))) {
						// TODO: Fix falling blocks and replace with falling smooth cragrock
						EntityFallingBlock fallingBlock = new EntityFallingBlock(world, pos.getX() + 0.5D, pos.getY(),
								pos.getZ() + 0.5D, BlockRegistry.WEAK_BETWEENSTONE_TILES.getDefaultState());
						fallingBlock.fallTime = -60;
						fallingBlock.shouldDropItem = false;
						fallingBlock.setHurtEntities(true);
						world.spawnEntity(fallingBlock);

						world.playSound(null, pos, SoundRegistry.CRUMBLE, SoundCategory.BLOCKS, 0.2F,
	...
	protected BlockPos getRandomPosInTower() {
		...
		return new BlockPos(x + world.rand.nextInt(width + 1), y + world.rand.nextInt(height + 1),
				z + world.rand.nextInt(depth + 1));
	}
