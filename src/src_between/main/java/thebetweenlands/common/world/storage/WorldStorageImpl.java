...
public abstract class WorldStorageImpl implements IWorldStorage {
	...
	public static class Handler {
		...
		public static void onWorldCapability(final AttachCapabilitiesEvent<World> event) {
			event.addCapability(new ResourceLocation(ModInfo.ID, "world_data"),
					new ICapabilitySerializable<NBTTagCompound>() {
						...
						public NBTTagCompound serializeNBT() {
							return (NBTTagCompound) CAPABILITY_INSTANCE.getStorage().writeNBT(CAPABILITY_INSTANCE,
									this.instance, null);
						}
	...
	public static void register() {
		CapabilityManager.INSTANCE.register(IWorldStorage.class, new IStorage<IWorldStorage>() {
			...
			@Override
			public void readNBT(Capability<IWorldStorage> capability, IWorldStorage instance, EnumFacing side,
					NBTBase nbt) {
	...
	public void loadChunk(Chunk chunk) {
		if (!this.storageMap.containsKey(chunk.getPos())) {
			try {
				...
				TheBetweenlands.logger.error(
						String.format("Failed creating chunk storage at %s", "[x=" + chunk.x + ", z=" + chunk.z + "]"),
						ex);
			}
		}
	}
		...
		if (this.storageMap.containsKey(chunk.getPos())) {
			if (BetweenlandsConfig.DEBUG.debug)
				TheBetweenlands.logger
						.warn(String.format("Reading chunk storage at %s, but chunk storage is already loaded!",
								"[x=" + chunk.x + ", z=" + chunk.z + "]"));
		} else {
			try {
				ChunkStorageImpl storage = new BetweenlandsChunkStorage(this, chunk);
				...
				TheBetweenlands.logger.error(
						String.format("Failed reading chunk storage at %s", "[x=" + chunk.x + ", z=" + chunk.z + "]"),
						ex);
			}
		}
	}
	...
	public void unloadChunk(Chunk chunk) {
		if (!this.storageMap.containsKey(chunk.getPos())) {
			if (BetweenlandsConfig.DEBUG.debug)
				TheBetweenlands.logger
						.warn(String.format("Unloading chunk storage at %s, but chunk storage is not loaded!",
								"[x=" + chunk.x + ", z=" + chunk.z + "]"));
		} else {
	...
	public NBTTagCompound saveChunk(Chunk chunk) {
		if (!this.storageMap.containsKey(chunk.getPos())) {
			if (BetweenlandsConfig.DEBUG.debug)
				TheBetweenlands.logger
						.warn(String.format("Saving chunk storage at %s, but chunk storage is not loaded!",
								"[x=" + chunk.x + ", z=" + chunk.z + "]"));
		} else {
			try {
				...
				TheBetweenlands.logger.error(
						String.format("Failed saving chunk storage at %s", "[x=" + chunk.x + ", z=" + chunk.z + "]"),
						ex);
			}
		}
		return null;
