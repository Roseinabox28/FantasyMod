...
public class LocalStorageHandlerImpl implements ILocalStorageHandler {
	...
	public LocalStorageHandlerImpl(IWorldStorage worldStorage) {
		...
		this.localStorageDir = new File(this.world.getSaveHandler().getWorldDirectory(),
				(dimFolder != null && dimFolder.length() > 0 ? dimFolder + File.separator : "") + "data"
						+ File.separator + "local_storage" + File.separator);
	...
	public <T extends ILocalStorage> List<T> getLocalStorages(Class<T> type, double x, double z,
		...
		if (chunkStorage != null) {
			for (LocalStorageReference ref : chunkStorage.getLocalStorageReferences()) {
				...
				if (localStorage != null && localStorage.getBoundingBox() != null
						&& type.isAssignableFrom(localStorage.getClass())
			...
			for (int cz = sz; cz <= ez; cz++) {
				...
				if (chunkStorage != null) {
					for (LocalStorageReference ref : chunkStorage.getLocalStorageReferences()) {
						...
						if (localStorage != null && localStorage.getBoundingBox() != null
								&& type.isAssignableFrom(localStorage.getClass())
								&& localStorage.getBoundingBox().intersects(aabb)
								&& (filter == null || filter.apply((T) localStorage))) {
							if (!storages.contains(localStorage)) {
								storages.add((T) localStorage);
							}
	...
	public ILocalStorage loadLocalStorage(LocalStorageReference reference) {
		try {
			...
			TheBetweenlands.logger.error(
					String.format("Failed loading local storage with ID %s at %s", reference.getID().getStringID(),
							"[x=" + reference.getChunk().x + ", z=" + reference.getChunk().z + "]"),
					ex);
		}
		return null;
	}
	...
	private ILocalStorage createLocalStorageFromFile(LocalStorageReference reference) {
		if (!reference.hasRegion()) {
			...
			try {
				...
				TheBetweenlands.logger.error(String.format("Failed reading local storage %s from file: %s",
						reference.getID().getStringID(), file.getAbsolutePath()), ex);
			}
		} else {
