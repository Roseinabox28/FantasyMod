...
public class LocationStorage extends LocalStorageImpl implements ITickable {
	...
	protected static final DataParameter<String> NAME = GenericDataManager.createKey(LocationStorage.class,
			DataSerializers.STRING);
	protected static final DataParameter<Boolean> VISIBLE = GenericDataManager.createKey(LocationStorage.class,
	 ...
	 * @param id
	 */
	public LocationStorage(IWorldStorage worldStorage, StorageID id, @Nullable LocalRegion region, String name,
	...
	protected final boolean isVecInsideOrEdge(AxisAlignedBB aabb, Vec3d vec) {
		return vec.x >= aabb.minX && vec.x <= aabb.maxX
				? (vec.y >= aabb.minY && vec.y <= aabb.maxY ? vec.z >= aabb.minZ && vec.z <= aabb.maxZ : false)
				: false;
	}
