...
public class WorldGenSpiritTree extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos position) {
		...
		for (int xx = -checkRadius; xx <= checkRadius; xx++) {
			for (int zz = -checkRadius; zz <= checkRadius; zz++) {
				for (int yy = 3; yy < 16; yy++) {
					if (!world.isAirBlock(position.add(xx, yy, zz))
							&& world.getBlockState(position.add(xx, yy, zz)).isNormalCube()) {
						return false;
					}
				}
			}
		...
		this.log = BlockRegistry.LOG_SPIRIT_TREE.getDefaultState().withProperty(BlockLog.LOG_AXIS,
				BlockLog.EnumAxis.NONE);
		this.leavesTop = BlockRegistry.LEAVES_SPIRIT_TREE_TOP.getDefaultState()
				.withProperty(BlockLeavesBetweenlands.CHECK_DECAY, false);
		this.leavesMiddle = BlockRegistry.LEAVES_SPIRIT_TREE_MIDDLE.getDefaultState()
				.withProperty(BlockLeavesBetweenlands.CHECK_DECAY, false);
		this.leavesBottom = BlockRegistry.LEAVES_SPIRIT_TREE_BOTTOM.getDefaultState()
				.withProperty(BlockLeavesBetweenlands.CHECK_DECAY, false);
		...
		for (BlockPos rootBlock : rootBlocks) {
			if (rand.nextInt(4) == 0 && new Vec3d(rootBlock.getX() + 0.5D, 0, rootBlock.getZ() + 0.5D)
					.squareDistanceTo(trunkX + 1.0D, 0, trunkZ + 1.0D) >= 9 && world.isAirBlock(rootBlock.up())) {
	...
	private void generateTrunkCrossSection(World world, Random rand, int x, int y, int z, IBlockState log, int trunkX,
			int trunkY, int trunkZ) {
	...
	/*
	 * private List<BlockPos> generateSideBranch(World world, Random rand, BlockPos
	 * start, int dir) { List<BlockPos> branchBlocks = new ArrayList<>();
	 * branchBlocks.addAll(this.generateBranch(world, rand, start, dir, 8 +
	 * rand.nextInt(3), 0.3D, 0.6D, (i, length) -> i < length / 2 ? 1 : (i >= length
	 * - 1 && rand.nextInt(2) == 0 ? -1 : 0), (i, length) -> true)); BlockPos end =
	 * branchBlocks.get(branchBlocks.size() - 1); switch(dir) { case 0:
	 * branchBlocks.addAll(this.generateSideBranch2(world, rand, end, 1));
	 * branchBlocks.addAll(this.generateSideBranch2(world, rand, end, 7)); break;
	 * case 1: branchBlocks.addAll(this.generateSideBranch2(world, rand, end, 0));
	 * branchBlocks.addAll(this.generateSideBranch2(world, rand, end, 2)); break;
	 * case 2: branchBlocks.addAll(this.generateSideBranch2(world, rand, end, 1));
	 * branchBlocks.addAll(this.generateSideBranch2(world, rand, end, 3)); break;
	 * case 3: branchBlocks.addAll(this.generateSideBranch2(world, rand, end, 2));
	 * branchBlocks.addAll(this.generateSideBranch2(world, rand, end, 4)); break;
	 * case 4: branchBlocks.addAll(this.generateSideBranch2(world, rand, end, 3));
	 * branchBlocks.addAll(this.generateSideBranch2(world, rand, end, 5)); break;
	 * case 5: branchBlocks.addAll(this.generateSideBranch2(world, rand, end, 4));
	 * branchBlocks.addAll(this.generateSideBranch2(world, rand, end, 6)); break;
	 * case 6: branchBlocks.addAll(this.generateSideBranch2(world, rand, end, 5));
	 * branchBlocks.addAll(this.generateSideBranch2(world, rand, end, 7)); break;
	 * case 7: branchBlocks.addAll(this.generateSideBranch2(world, rand, end, 6));
	 * branchBlocks.addAll(this.generateSideBranch2(world, rand, end, 1)); break; }
	 * return branchBlocks; }
	 * 
	 * private List<BlockPos> generateSideBranch2(World world, Random rand, BlockPos
	 * start, int dir) { return this.generateBranch(world, rand, start, dir, 4 +
	 * rand.nextInt(3), 0.3D, 0.6D, (i, length) -> i < length / 2 ? 1 : (i >= length
	 * - 1 && rand.nextInt(2) == 0 ? -1 : 0), (i, length) -> true); }
	 */

	private List<BlockPos> generateRoot(World world, Random rand, BlockPos start, int dir, int trunkX, int trunkY,
			int trunkZ) {
		List<BlockPos> root = this.generateBranchPositions(rand, start, dir, 2 + rand.nextInt(2), 0.9D, 0.8D,
				(i, remainingBlocks) -> i < 2 ? 0 : rand.nextInt((i - 2) * 2 + 2) == 0 ? 0 : -1, (i, length) -> true);
	...
	private List<BlockPos> generateSubRootPositions(World world, Random rand, BlockPos start, int dir) {
		return this.generateBranchPositions(rand, start, dir, 1 + rand.nextInt(4), 0.5D, 1,
				(i, remainingBlocks) -> rand.nextInt(i * 2 + 1) == 0 ? 0 : -1, (i, length) -> true);
	}

	private List<BlockPos> generateSideBranch(World world, Random rand, BlockPos start, int dir, int trunkX, int trunkY,
			int trunkZ) {
	...
	private List<BlockPos> generateSideBranchPositions(Random rand, BlockPos start, int dir) {
		return this.generateBranchPositions(rand, start, dir, 6 + rand.nextInt(5), 0.3D, 0.6D,
				(i, remainingBlocks) -> i < remainingBlocks / 2 ? 1
						: (i >= remainingBlocks - 1 && rand.nextInt(2) == 0 ? -1 : 0),
				(i, length) -> true);
	}

	private List<BlockPos> generateTopBranch(World world, Random rand, BlockPos start, int dir, int trunkX, int trunkY,
			int trunkZ) {
		List<BlockPos> branch = this.generateBranchPositions(rand, start, dir, 5 + rand.nextInt(5), 0.1D, 0.2D,
				(i, remainingBlocks) -> i < remainingBlocks - 1 ? 1 : 0, (i, length) -> i >= length - 1);
		for (BlockPos pos : branch) {
			this.setBlock(world, pos, this.log, true, trunkX, trunkY, trunkZ);
		}
		return branch;
	...
	public List<BlockPos> generateBranchPositions(Random rand, BlockPos start, int dir, int length,
			double defaultCurveWeight, double directedCurveWeight, BiFunction<Integer, Double, Integer> heightFunction,
			BiFunction<Integer, Double, Boolean> forceMoveFunction) {
	...
	private void generateBranchLeaves(World world, Random rand, BlockPos start, List<BlockPos> branchBlocks, int trunkX,
			int trunkY, int trunkZ) {
	...
	protected void setBlock(World world, BlockPos pos, IBlockState state, boolean registerSmallFacePositions,
		...
		if (!(pos.getX() >= trunkX && pos.getZ() >= trunkZ && pos.getX() <= trunkX + 1 && pos.getZ() <= trunkZ + 1)
				&& registerSmallFacePositions && this.location != null
				&& state.getBlock() == BlockRegistry.LOG_SPIRIT_TREE) {
