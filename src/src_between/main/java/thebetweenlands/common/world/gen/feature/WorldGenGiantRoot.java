...
public class WorldGenGiantRoot extends WorldGenerator {
	...
	public WorldGenGiantRoot(BlockPos start, BlockPos end, double minWidth, double maxWidth, double randXZOffsetRange,
			double randYOffsetRange, double maxArcHeight, boolean genLeafyBranches, boolean genLeaves, boolean genFungi,
			boolean genSmallerRoots, AxisAlignedBB genBounds) {
		this.start = start;
		this.end = end;
	...
	public boolean generate(IBlockStateAccessOnly worldIn, int chunkX, int chunkZ, boolean checkChunk, Random rand,
			BlockPos position) {
		...
		for (int i = 0; i < randPts; i++) {
			Vec3d pt = new Vec3d(this.start).add(diff.scale(i / (float) randPts)).add(
					offsetDir.x * (rand.nextDouble() - 0.5D) * this.randXZOffsetRange * 2.0D,
					(rand.nextDouble() - 0.5D) * this.randYOffsetRange * 2.0D
							+ Math.sin(i / (float) randPts * Math.PI) * this.maxArcHeight,
					offsetDir.z * (rand.nextDouble() - 0.5D) * this.randXZOffsetRange * 2.0D);
			pts.add(pt);
		}
		...
		int steps = 20 + (int) (this.start.getDistance(this.end.getX(), this.end.getY(), this.end.getZ()) * 4);// MathHelper.ceil(spline.getArcLength())
																												// * 4;
		...
		for (int i = 0; i < steps; i++) {
			...
			if (this.genLeafyBranches && smallerRootRand.nextInt(12) == 0) {
				WorldGenGiantRoot smallRoot = new WorldGenGiantRoot(pos,
						pos.add(offsetDir.x * (smallerRootRand.nextDouble() - 0.5D) * 16,
								smallerRootRand.nextDouble() * 8,
								offsetDir.z * (smallerRootRand.nextDouble() - 0.5D) * 16),
						0.5D, 0.5D, 3, 3, 0, false, true, false, false, this.genBounds);
			...
			if (this.genSmallerRoots && smallerRootRand.nextInt(12) == 0) {
				WorldGenGiantRoot smallRoot = new WorldGenGiantRoot(pos,
						pos.add(offsetDir.x * (smallerRootRand.nextDouble() - 0.5D) * 16,
								-smallerRootRand.nextDouble() * 8,
								offsetDir.z * (smallerRootRand.nextDouble() - 0.5D) * 16),
						0.5D, 0.5D, 3, 3, 0, false, false, false, false, this.genBounds);
				smallRoot.generate(worldIn, chunkX, chunkZ, checkChunk, smallerRootRand, pos);
			}

			double widthMul = 1 - ((i > steps / 2) ? ((steps - i) / (float) steps * 2) : (i / (float) steps * 2));

			int radius = (int) ((rootWidthRand.nextDouble() * (this.maxWidth - this.minWidth) * widthMul)
					+ this.minWidth);
		...
		if (this.genLeaves) {
			...
			for (int i = 0; i < steps; i++) {
				...
				int radius = 1 + (int) ((rootWidthRand.nextDouble() * (this.maxWidth - this.minWidth) * widthMul)
						+ this.minWidth);
					...
					for (int yo = -radius; yo <= radius; yo++) {
						for (int zo = -radius; zo <= radius; zo++) {
							...
							if (this.isInBounds(genPos) && worldIn.getBlockState(genPos) != bark) {
								if (xo * xo + yo * yo + zo * zo > (radius - 1) * (radius - 1)
					...
					for (int yo = -radius + 1; yo <= radius - 1; yo++) {
						for (int zo = -radius; zo <= radius; zo++) {
							...
							if (xo * xo + yo * yo + zo * zo >= radius * radius) {
								...
								fungiRand.setSeed(
										MathHelper.getCoordinateRandom(genPos.getX(), genPos.getY(), genPos.getZ()));
								if (fungiRand.nextInt(160) == 0) {
									...
									for (int fx = -fungiRadius; fx <= fungiRadius; fx++) {
										for (int fz = -fungiRadius; fz <= fungiRadius; fz++) {
											...
											if ((fx + 0.5D) * (fx + 0.5D) + (fz + 0.5D) * (fz + 0.5D) <= fungiRadius
													* fungiRadius && this.isInBounds(fungiPos)
													&& worldIn.getBlockState(fungiPos) != bark) {
		...
		Random foliageRand = new Random(); // Use a seperate rand foliage so that it doesn't mess with the main rand
											// when skipped!
		...
		for (int i = 0; i < steps; i++) {
			...
			int radius = 1
					+ (int) ((rootWidthRand.nextDouble() * (this.maxWidth - this.minWidth) * widthMul) + this.minWidth);
				...
				for (int yo = -radius; yo <= radius; yo++) {
					for (int zo = -radius; zo <= radius; zo++) {
						...
						if (!this.genLeaves) {
							...
							if (this.isInBounds(genPos) && foliageRand.nextInt(60) == 0 && yo >= radius - 2
									&& xo * xo + yo * yo + zo * zo <= (radius - 1) * (radius - 1)
									&& !worldIn.isAirBlock(genPos) && worldIn.isAirBlock(genPos.up())
									&& worldIn.isAirBlock(genPos.up(2))) {
							...
							if (this.isInBounds(genPos) && foliageRand.nextInt(10) == 0 && yo <= radius - 2
									&& xo * xo + yo * yo + zo * zo <= (radius - 1) * (radius - 1)
									&& !worldIn.isAirBlock(genPos) && worldIn.isAirBlock(genPos.down())
									&& worldIn.isAirBlock(genPos.down(2))) {
							...
							// Moss is just one block so it needn't care about other chunks either, if it
							// gets replaced so be it
							if (this.isInBounds(genPos) && foliageRand.nextInt(10) == 0
									&& xo * xo + yo * yo + zo * zo > (radius - 1) * (radius - 1)
									&& worldIn.isAirBlock(genPos)) {
								List<EnumFacing> dirs = Arrays
										.asList(Arrays.copyOf(EnumFacing.VALUES, EnumFacing.VALUES.length));
								Collections.shuffle(dirs, foliageRand);
								for (EnumFacing facing : dirs) {
									// Check if offset pos is giant root
									if ((xo + facing.getXOffset()) * (xo + facing.getXOffset())
											+ (yo + facing.getYOffset()) * (yo + facing.getYOffset())
											+ (zo + facing.getZOffset()) * (zo + facing.getZOffset()) <= (radius - 1)
													* (radius - 1)) {
										worldIn.setBlockState(genPos, BlockRegistry.MOSS.getDefaultState()
												.withProperty(BlockMoss.FACING, facing.getOpposite()), 2);
									}
								}
							}
						}
	...
	protected boolean shouldCheckAtPos(BlockPos pos, int chunkX, int chunkZ, int radius) {
		double dx = pos.getX() >= chunkX * 16 && pos.getX() <= chunkX * 16 + 16 ? 0
				: Math.min(Math.abs(chunkX * 16 - pos.getX()), Math.abs(pos.getX() - (chunkX * 16 + 16)));
		double dz = pos.getZ() >= chunkZ * 16 && pos.getZ() <= chunkZ * 16 + 16 ? 0
				: Math.min(Math.abs(chunkZ * 16 - pos.getZ()), Math.abs(pos.getZ() - (chunkZ * 16 + 16)));
		return dx * dx + dz * dz <= radius * radius;
	}

	protected boolean isInBounds(BlockPos pos) {
		return this.genBounds == null || (this.genBounds.minX <= pos.getX() && this.genBounds.minY <= pos.getY()
				&& this.genBounds.minZ <= pos.getZ() && this.genBounds.maxX >= pos.getX()
				&& this.genBounds.maxY >= pos.getY() && this.genBounds.maxZ >= pos.getZ());
	}
}