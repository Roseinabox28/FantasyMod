...
public class WorldGenSpawner extends WorldGenerator {
	private final List<Block> blackListedBlocks = ImmutableList.of(BlockRegistry.BETWEENSTONE_TILES,
			BlockRegistry.BETWEENSTONE_BRICK_STAIRS, BlockRegistry.BETWEENSTONE_BRICKS,
			BlockRegistry.BETWEENSTONE_BRICK_SLAB);
	private final double size;
	private int minY = 0;
	private DecoratorPositionProvider positionProvider = new DecoratorPositionProvider().setOffsetXZ(-4, 4)
			.setOffsetY(0, 8);
	...
	public boolean generate(World world, Random rand, BlockPos position) {
		...
		if (position.getY() <= 4 + this.minY) {
				...
				for (int zz = 0; zz < 16; ++zz)
					for (int yy = 0; yy < 8; ++yy) {
						...
						if (!world.isBlockLoaded(pos)
								|| this.blackListedBlocks.contains(world.getBlockState(pos).getBlock()))
							return false;
					}
			...
			for (int ox = 0; ox < 16; ++ox) {
				for (int oz = 0; oz < 16; ++oz) {
					for (int oy = 0; oy < 16; ++oy) {
						boolean isOuterBlock = !isInBlob[(ox * 16 + oz) * 16 + oy]
								&& (ox < 15 && isInBlob[((ox + 1) * 16 + oz) * 16 + oy]
										|| ox > 0 && isInBlob[((ox - 1) * 16 + oz) * 16 + oy]
										|| oz < 15 && isInBlob[(ox * 16 + oz + 1) * 16 + oy]
										|| oz > 0 && isInBlob[(ox * 16 + (oz - 1)) * 16 + oy]
										|| oy < 15 && isInBlob[(ox * 16 + oz) * 16 + oy + 1]
										|| oy > 0 && isInBlob[(ox * 16 + oz) * 16 + (oy - 1)]);
				...
				for (int oz = 0; oz < 16; ++oz) {
					for (int oy = 15; oy >= 0; --oy) {
						if (isInBlob[(ox * 16 + oz) * 16 + oy]) {
							this.setBlockAndNotifyAdequately(world, position.add(ox, oy, oz),
							...
							if (oy < 2) {
								if (world.getBlockState(pos.up()).isFullCube()) {
									this.setBlockAndNotifyAdequately(world, pos,
											BlockRegistry.SWAMP_DIRT.getDefaultState());
								} else {
									this.setBlockAndNotifyAdequately(world, pos,
											BlockRegistry.SWAMP_GRASS.getDefaultState());
								}
							} else {
			...
			if (!bigMushroom) {
				for (int i = 0; i < 8; i++) {
					...
					if (BlockRegistry.BULB_CAPPED_MUSHROOM.canPlaceBlockAt(world, pos)) {
						this.setBlockAndNotifyAdequately(world, pos,
								BlockRegistry.BULB_CAPPED_MUSHROOM.getDefaultState());
					}
				}
			}

			this.positionProvider.init(world, world.getBiomeForCoordsBody(center), null, rand, center.getX(),
			...
			for (int i = 0; i < 16; i++) {
				DecorationHelper.GEN_SWAMP_TALLGRASS.generate(world, rand,
						center.add(rand.nextInt(8) - 4, rand.nextInt(8) - 4, rand.nextInt(8) - 4));
			}
			...
			LocationStorage location = new LocationStorage(worldStorage, new StorageUUID(UUID.randomUUID()),
					LocalRegion.getFromBlockPos(position), "underground_dungeon", EnumLocationType.DUNGEON);
