...
public class WorldGenIdolHeads extends WorldGenHelper {
	...
	private final WorldGenerator crackGen = new WorldGenBlockReplacementCluster(
			BlockRegistry.CRAGROCK.getDefaultState(), BlockMatcher.forBlock(BlockRegistry.SMOOTH_CRAGROCK));
	private final WorldGenerator crackGenSlabs = new WorldGenBlockReplacementCluster(
			BlockRegistry.CRAGROCK_BRICK_SLAB.getDefaultState(),
			BlockMatcher.forBlock(BlockRegistry.SMOOTH_CRAGROCK_SLAB)).setInheritProperties(true);
	private final WorldGenerator crackGenStairs = new WorldGenBlockReplacementCluster(
			BlockRegistry.CRAGROCK_BRICK_STAIRS.getDefaultState(),
			BlockMatcher.forBlock(BlockRegistry.SMOOTH_CRAGROCK_STAIRS)).setInheritProperties(true);
	private final WorldGenerator mossGen = new WorldGenBlockReplacementCluster(
			BlockRegistry.CRAGROCK.getDefaultState().withProperty(BlockCragrock.VARIANT, EnumCragrockType.MOSSY_2),
			BlockMatcher.forBlock(BlockRegistry.SMOOTH_CRAGROCK));
	...
	public boolean generateStructure(World world, Random rand, int x, int y, int z) {
			...
			for (int zz = z - width / 2; zz <= z + width / 2; ++zz) {
				for (int yy = y + 1; yy < y + height; ++yy) {
					if (!world.isAirBlock(checkPos.setPos(xx, yy, zz))
							&& !world.getBlockState(checkPos.setPos(xx, yy, zz)).getBlock().isReplaceable(world,
									checkPos.setPos(xx, yy, zz))) {
						return false;
					}
				}
		...
		case 0:// Gold Head
			...
			rotatedCubeVolume(world, xx, y, zz, 2, 2, 1,
					slab.withProperty(BlockSlabBetweenlands.HALF, EnumBlockHalfBL.TOP), 4, 1, 1, direction);
			rotatedCubeVolume(world, xx, y, zz, 1, 1, 1, direction == 0
					? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.WEST)
					: direction == 2 ? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.EAST)
							: direction == 1 ? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.NORTH)
									: stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.SOUTH),
					1, 1, 1, direction); // bottom right
			rotatedCubeVolume(world, xx, y, zz, 6, 1, 1, direction == 0
					? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.EAST)
					: direction == 2 ? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.WEST)
							: direction == 1 ? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.SOUTH)
									: stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.NORTH),
					1, 1, 1, direction); // bottom left
		...
		case 1:// Silver Head
			...
			rotatedCubeVolume(world, xx, y, zz, 1, 6, 1,
					slab.withProperty(BlockSlabBetweenlands.HALF, EnumBlockHalfBL.TOP), 2, 1, 2, direction);
			rotatedCubeVolume(world, xx, y, zz, 3, 7, 1, slab, 2, 1, 6, direction);
			rotatedCubeVolume(world, xx, y, zz, 5, 7, 1, solid, 2, 1, 6, direction);
			rotatedCubeVolume(world, xx, y, zz, 5, 6, 1,
					slab.withProperty(BlockSlabBetweenlands.HALF, EnumBlockHalfBL.TOP), 2, 1, 2, direction);
			...
			rotatedCubeVolume(world, xx, y, zz, 3, 1, 1, direction == 0
					? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.WEST)
					: direction == 2 ? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.EAST)
							: direction == 1 ? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.NORTH)
									: stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.SOUTH),
					1, 1, 1, direction); // bottom right
			rotatedCubeVolume(world, xx, y, zz, 4, 1, 1, direction == 0
					? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.EAST)
					: direction == 2 ? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.WEST)
							: direction == 1 ? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.SOUTH)
									: stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.NORTH),
		...
		case 2:// Bronze Head
			...
			rotatedCubeVolume(world, xx, y, zz, 3, 2, 1,
					slab.withProperty(BlockSlabBetweenlands.HALF, EnumBlockHalfBL.TOP), 2, 1, 1, direction);
			rotatedCubeVolume(world, xx, y, zz, 2, 1, 1, direction == 0
					? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.WEST)
					: direction == 2 ? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.EAST)
							: direction == 1 ? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.NORTH)
									: stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.SOUTH),
					1, 1, 1, direction); // bottom right
			rotatedCubeVolume(world, xx, y, zz, 5, 1, 1, direction == 0
					? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.EAST)
					: direction == 2 ? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.WEST)
							: direction == 1 ? stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.SOUTH)
									: stairs.withProperty(BlockStairsBetweenlands.FACING, EnumFacing.NORTH),
		...
		for (int i = 0; i < 40 + rand.nextInt(160); i++) {
			...
			default:
			case 0:
				this.crackGen.generate(world, rand,
						new BlockPos(x - 3 + rand.nextInt(6), y + 6 - 3 + rand.nextInt(6), z - 3 + rand.nextInt(6)));
				break;
			case 1:
				this.crackGenSlabs.generate(world, rand,
						new BlockPos(x - 3 + rand.nextInt(6), y + 6 - 3 + rand.nextInt(6), z - 3 + rand.nextInt(6)));
				break;
			case 2:
				this.crackGenStairs.generate(world, rand,
						new BlockPos(x - 3 + rand.nextInt(6), y + 6 - 3 + rand.nextInt(6), z - 3 + rand.nextInt(6)));
				break;
			case 3:
				this.mossGen.generate(world, rand,
						new BlockPos(x - 3 + rand.nextInt(6), y + 6 - 3 + rand.nextInt(6), z - 3 + rand.nextInt(6)));
				break;
			}
			...
			if (rand.nextInt(4) == 0) {
				if (rand.nextInt(8) == 0) {
					this.lichenClusterGen.generate(world, rand, new BlockPos(x - 3 + rand.nextInt(6),
							y + 6 - 3 + rand.nextInt(6), z - 3 + rand.nextInt(6)));
				} else {
					this.mossClusterGen.generate(world, rand, new BlockPos(x - 3 + rand.nextInt(6),
							y + 6 - 3 + rand.nextInt(6), z - 3 + rand.nextInt(6)));
				}
			}
		}
		...
		LocationStorage locationStorage = new LocationStorage(worldStorage, new StorageUUID(UUID.randomUUID()),
				LocalRegion.getFromBlockPos(x, z), "idol_head", EnumLocationType.IDOL_HEAD);
