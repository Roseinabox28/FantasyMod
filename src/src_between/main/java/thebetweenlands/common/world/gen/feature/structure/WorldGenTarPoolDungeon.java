...
public class WorldGenTarPoolDungeon extends WorldGenHelper {
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		if (y + height + 1 >= WorldProviderBetweenlands.LAYER_HEIGHT - height
				|| y - 1 <= WorldProviderBetweenlands.CAVE_WATER_HEIGHT + height)
			return false;
			...
			for (int xx = halfSize * -1; xx <= halfSize; ++xx) {
				for (int zz = halfSize * -1; zz <= halfSize; ++zz) {
					...
					if (Math.round(Math.sqrt(dSq)) == halfSize - 2 && yy >= y + 1) {
						world.setBlockState(new BlockPos(x + xx, yy, z + zz),
								BlockRegistry.BETWEENSTONE_TILES.getDefaultState());
						if (rand.nextBoolean() && yy == y + 2)
							if ((xx % 4 == 0 || zz % 4 == 0 || xx % 3 == 0 || zz % 3 == 0)
									&& world.getBlockState(this.getCheckPos(x + xx, y + 6, z + zz))
											.getBlock() == BlockRegistry.BETWEENSTONE)
								placePillar(world, x + xx, y + 3, z + zz, rand);
					}
					if (Math.round(Math.sqrt(dSq)) <= halfSize && Math.round(Math.sqrt(dSq)) >= halfSize - 1
							&& yy <= y + 2)
						world.setBlockState(new BlockPos(x + xx, yy, z + zz),
					...
					if (Math.round(Math.sqrt(dSq)) < halfSize - 2 && yy == y) {
						if (rand.nextBoolean() && rand.nextBoolean())
							world.setBlockState(new BlockPos(x + xx, yy, z + zz),
									BlockRegistry.TAR_SOLID.getDefaultState());
						else
							world.setBlockState(new BlockPos(x + xx, yy, z + zz),
									BlockRegistry.BETWEENSTONE.getDefaultState());
						if (rand.nextInt(18) == 0) {
							world.setBlockState(new BlockPos(x + xx, yy, z + zz), getRandomTarLootPot(rand)
									.withProperty(BlockLootPot.FACING, EnumFacing.HORIZONTALS[rand.nextInt(4)]));
		...
		world.setBlockState(
				new BlockPos(x + rand.nextInt(halfSize - 2) - rand.nextInt(halfSize - 2), y,
						z + rand.nextInt(halfSize - 2) - rand.nextInt(halfSize - 2)),
				BlockRegistry.TAR_BEAST_SPAWNER.getDefaultState());

		BetweenlandsWorldStorage worldStorage = BetweenlandsWorldStorage.forWorld(world);
		LocationStorage location = new LocationStorage(worldStorage, new StorageUUID(UUID.randomUUID()),
				LocalRegion.getFromBlockPos(pos), "tar_pool_dungeon", EnumLocationType.DUNGEON);
		location.addBounds(new AxisAlignedBB(x - halfSize, y - 1, z - halfSize, x + halfSize, y + height, z + halfSize)
				.grow(1, 1, 1));
		location.linkChunks();
		location.setLayer(0);
	...
	private IBlockState getRandomTarLootPot(Random rand) {
		...
		default:
		case 0:
			return BlockRegistry.TAR_LOOT_POT.getDefaultState().withProperty(BlockLootPot.VARIANT, EnumLootPot.POT_1);
		case 1:
			return BlockRegistry.TAR_LOOT_POT.getDefaultState().withProperty(BlockLootPot.VARIANT, EnumLootPot.POT_2);
		case 2:
			return BlockRegistry.TAR_LOOT_POT.getDefaultState().withProperty(BlockLootPot.VARIANT, EnumLootPot.POT_3);
		}
	}
}