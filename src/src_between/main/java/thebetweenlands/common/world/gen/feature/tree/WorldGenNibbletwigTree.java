...
public class WorldGenNibbletwigTree extends WorldGenHelper {
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		if (!this.rotatedCubeCantReplace(world, x, y + 2, z, -2, 0, -2, 5, height, 5, 0)) {
			...
			IBlockState log = BlockRegistry.LOG_NIBBLETWIG.getDefaultState().withProperty(BlockLogBetweenlands.LOG_AXIS,
					BlockLog.EnumAxis.NONE);
			IBlockState leaves = BlockRegistry.LEAVES_NIBBLETWIG_TREE.getDefaultState()
			...
			this.rotatedCubeVolume(world, setPos -> world.isAirBlock(setPos), xo[canopy1] + x, y + canopy1,
					zo[canopy1] + z, -1, 0, -1, leaves, 3, 1, 3, 0);
			for (EnumFacing offset : EnumFacing.HORIZONTALS) {
				BlockPos offsetPos = pos.add(xo[canopy1] + offset.getXOffset(), canopy1 + 1,
						zo[canopy1] + offset.getZOffset());
				if (world.isAirBlock(offsetPos)) {
					this.setBlockAndNotifyAdequately(world, offsetPos, leaves);
				}

				BlockPos droopPos = pos.add(xo[canopy1] + offset.getXOffset() * 2, canopy1,
						zo[canopy1] + offset.getZOffset() * 2);
				...
				if (rand.nextInt(3) == 0) {
					BlockPos ivyPos = pos.add(xo[canopy1] + offset.getXOffset() * 3, canopy1,
							zo[canopy1] + offset.getZOffset() * 3);
					int ivyLength = rand.nextInt(5) + 3;
					for (int yo = 0; yo < ivyLength; yo++) {
						BlockPos ivyPosY = ivyPos.down(yo);
						if (world.isAirBlock(ivyPosY)) {
							this.setBlockAndNotifyAdequately(world, ivyPosY,
									ivy.withProperty(BlockPoisonIvy.getPropertyFor(offset.getOpposite()), true));
						} else {
							break;
						}
					}
			...
			this.rotatedCubeVolume(world, setPos -> world.isAirBlock(setPos), xo[height - 1] + x, y + height - 1,
					zo[height - 1] + z, -1, 0, -2, leaves, 3, 2, 5, 0);
			this.rotatedCubeVolume(world, setPos -> world.isAirBlock(setPos), xo[height - 1] + x, y + height - 1,
					zo[height - 1] + z, -2, 0, -1, leaves, 5, 2, 3, 0);

			this.rotatedCubeVolume(world, setPos -> world.isAirBlock(setPos), xo[height - 1] + x, y + height - 1 + 2,
					zo[height - 1] + z, -1, 0, 0, leaves, 3, 1, 1, 0);
			this.rotatedCubeVolume(world, setPos -> world.isAirBlock(setPos), xo[height - 1] + x, y + height - 1 + 2,
					zo[height - 1] + z, 0, 0, -1, leaves, 1, 1, 3, 0);
			this.rotatedCubeVolume(world, setPos -> world.isAirBlock(setPos), xo[height - 1] + x, y + height - 1 + 3,
					zo[height - 1] + z, 0, 0, 0, leaves, 1, 1, 1, 0);
			...
			for (int i = 0; i < droops; i++) {
				...
				if (Math.abs(droopXO) + Math.abs(droopZO) > 1 && !generatedDroops[droopIndex]
					...
					BlockPos ivyPos = pos.add(xo[height - 1] + droopXO + ivyOffset.getXOffset(), height - 1,
							zo[height - 1] + droopZO + ivyOffset.getZOffset());
					int ivyLength = rand.nextInt(9) + 3;
					for (int yo = 0; yo < ivyLength; yo++) {
						BlockPos ivyPosY = ivyPos.down(yo);
						if (world.isAirBlock(ivyPosY)) {
							this.setBlockAndNotifyAdequately(world, ivyPosY,
									ivy.withProperty(BlockPoisonIvy.getPropertyFor(ivyOffset.getOpposite()), true));
						} else {
							break;
						}
					}
