...
public class ChunkGeneratorBetweenlands implements IChunkGenerator {
	...
	public ChunkGeneratorBetweenlands(World world, long seed, Block baseBlock, Block layerBlock, int layerHeight) {
		...
		ContextBetweenlands ctx = new ContextBetweenlands(minLimitPerlinNoise, maxLimitPerlinNoise, mainPerlinNoise,
		...
		for (int z = 0; z < 16; z++) {
			for (int x = 0; x < 16; x++) {
				...
				if (biome instanceof BiomeBetweenlands) {
					...
					biome.genTerrainBlocks(this.worldObj, this.rand, primer, chunkX * 16 + x, chunkZ * 16 + z,
							baseBlockNoise);
				}
			}
		}
...
//	}
	...
	public void populate(int x, int z) {
		...
		if (biome instanceof BiomeBetweenlands) {
			...
			if (net.minecraftforge.event.terraingen.TerrainGen.populate(this, this.worldObj, this.rand, x, z, false,
	...
	private void debugProvideHandle(int x, int z) {
		...
		for (int i = 0; i < elems.length; i++) {
			...
			if ("net.minecraft.world.gen.ChunkProviderServer".equals(cls) && "provideChunk".equals(method)) {
				...
				if ("net.minecraft.server.management.PlayerChunkMapEntry".equals(cls)
						&& "providePlayerChunk".equals(method)) {
					player = true;
					...
					signatureCauses[signatureIdx].append('#').append(method).append(':').append(elem.getLineNumber())
							.append("\n");
					signature[signatureIdx] = (((signature[signatureIdx] * 31) + cls.hashCode()) * 31
							+ method.hashCode()) * 31 + elem.getLineNumber();
					newSignature = false;
				} else {
					if (!lastOther) {
	...
	public void debugGenerateChunkProvidesImage(boolean open) {
		...
		int pad = 1;
		int unit = tile + pad;
		BufferedImage img = new BufferedImage((maxX - minX + 1) * unit + pad, (maxZ - minZ + 1) * unit + pad,
			...
			for (ChunkProvide p : chunkProvides) {
				...
				for (int i = 0, j = p.signature.length - 1; i < p.signature.length; i++, j--) {
					...
					if (printed.add(p.cause[i])) {
						TheBetweenlands.logger.debug("#%s:%n%s%n", Integer.toHexString(color | 0xFF000000).substring(2),
								p.cause[i]);
					}
				}
			}
