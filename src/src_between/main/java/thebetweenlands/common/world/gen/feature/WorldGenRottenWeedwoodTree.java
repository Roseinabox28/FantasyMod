...
public class WorldGenRottenWeedwoodTree extends WorldGenerator {
		...
		for (int xx = x - maxRadius; xx <= x + maxRadius; xx++)
			for (int zz = z - maxRadius; zz <= z + maxRadius; zz++)
				for (int yy = y + 2; yy < y + height; yy++)
					if (!world.isAirBlock(new BlockPos(xx, yy, zz)) && !world.getBlockState(new BlockPos(xx, yy, zz))
							.getBlock().isReplaceable(world, new BlockPos(xx, yy, zz)))
						return false;
		...
		for (int yy = y; yy < y + height; ++yy) {
			...
			for (int i = radius * -1; i <= radius; ++i)
				for (int j = radius * -1; j <= radius; ++j) {
					...
					if (Math.round(Math.sqrt(dSq)) == radius && yy == y
							|| Math.round(Math.sqrt(dSq)) == radius && yy <= y + height - 1)
			...
			if (yy == y + height / 2 + 2) {
				...
				createBranch(world, rand, x + radius + 1, yy - rand.nextInt(1), z + radius + 1, 5, false,
						rand.nextInt(2) + 3, hasPoisonIvy);
				createBranch(world, rand, x - radius - 1, yy - rand.nextInt(1), z - radius - 1, 6, false,
						rand.nextInt(2) + 3, hasPoisonIvy);
				createBranch(world, rand, x - radius - 1, yy - rand.nextInt(1), z + radius + 1, 7, false,
						rand.nextInt(2) + 3, hasPoisonIvy);
				createBranch(world, rand, x + radius + 1, yy - rand.nextInt(1), z - radius - 1, 8, false,
						rand.nextInt(2) + 3, hasPoisonIvy);
			}
			...
			if (yy == y + 1) {
				...
				createBranch(world, rand, x + radius + 1, yy - rand.nextInt(2), z + radius + 1, 5, true,
						rand.nextInt(2) + 3, hasPoisonIvy);
				createBranch(world, rand, x - radius - 1, yy - rand.nextInt(2), z - radius - 1, 6, true,
						rand.nextInt(2) + 3, hasPoisonIvy);
				createBranch(world, rand, x - radius - 1, yy - rand.nextInt(2), z + radius + 1, 7, true,
						rand.nextInt(2) + 3, hasPoisonIvy);
				createBranch(world, rand, x + radius + 1, yy - rand.nextInt(2), z - radius - 1, 8, true,
						rand.nextInt(2) + 3, hasPoisonIvy);
			}
		}
		return true;
	...
	private void addVines(World world, Random rand, int x, int y, int z, EnumFacing... dirs) {
		...
		if (BlockRegistry.POISON_IVY.canPlaceBlockOnSide(world, new BlockPos(x, y, z), dirs[0].getOpposite())
				&& rand.nextInt(4) != 0) {
