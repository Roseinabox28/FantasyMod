...
public class Marsh1Feature extends BiomeFeature {
	...
	public void generateNoise(int chunkX, int chunkZ, Biome biome) {
		this.islandNoise = this.islandNoiseGen.getRegion(this.islandNoise, (double) (chunkX * 16),
				(double) (chunkZ * 16), 16, 16, 0.08D * 2.0D, 0.08D * 2.0D, 1.0D);
		this.fuzzNoise = this.fuzzNoiseGen.getRegion(this.fuzzNoise, (double) (chunkX * 16), (double) (chunkZ * 16), 16,
				16, 10.5D, 10.5D, 1.0D);
	}
	...
	public void replaceStackBlocks(int x, int z, double baseBlockNoise, ChunkPrimer chunkPrimer,
		...
		if (pass == EnumGeneratorPass.PRE_REPLACE_BIOME_BLOCKS) {
			...
			double noise = (this.islandNoise[x * 16 + z] / 1.4f + this.fuzzNoise[x * 16 + z] / 1.4f)
					* Math.pow(biomeWeight, 4) + 1.8f;
