...
public class WorldGenMossCluster extends WorldGenerator {
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		...
		for (int i = 0; i < this.attempts; ++i) {
			mutablePos.setPos(position.getX() + rand.nextInt(this.offset) - rand.nextInt(this.offset),
					position.getY() + rand.nextInt(this.offset / 2 + 1) - rand.nextInt(this.offset / 2 + 1),
					position.getZ() + rand.nextInt(this.offset) - rand.nextInt(this.offset));

			if (worldIn.isAreaLoaded(mutablePos, 1) && worldIn.isAirBlock(mutablePos)
					&& this.block.canPlaceBlockAt(worldIn, mutablePos)) {
