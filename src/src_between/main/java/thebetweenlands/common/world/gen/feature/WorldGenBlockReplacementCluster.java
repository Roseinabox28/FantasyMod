...
public class WorldGenBlockReplacementCluster extends WorldGenerator {
	...
	public WorldGenBlockReplacementCluster(IBlockState blockState, int offset, int attempts,
			Predicate<IBlockState> matcher) {
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		...
		for (int i = 0; i < this.attempts; ++i) {
			BlockPos blockpos = position.add(rand.nextInt(this.offset) - rand.nextInt(this.offset),
					rand.nextInt(this.offset / 2 + 1) - rand.nextInt(this.offset / 2 + 1),
					rand.nextInt(this.offset) - rand.nextInt(this.offset));
				...
				if (this.matcher.apply(state)) {
					...
					if (this.inheritProperties) {
						...
						for (Entry<IProperty<?>, Comparable<?>> property : properties.entrySet()) {
							...
							IProperty targetProperty = setState.getBlock().getBlockState()
