...
public class MapGenRavineBetweenlands extends MapGenRavine {
	...
	protected void recursiveGenerate(World worldIn, int chunkX, int chunkZ, int p_180701_4_, int p_180701_5_,
		...
		if (this.rand.nextInt(60) == 0) {
			...
			for (int j = 0; j < 1 + (this.rand.nextInt(5) == 0 ? 2 : 0); ++j) {
				...
				this.addTunnel(this.rand.nextLong(), p_180701_4_, p_180701_5_, chunkPrimerIn, rx, ry, rz, f2, f, f1, 0,
						0, 3.0D);
			}
		}
	}
	...
	protected void digBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop) {
		...
		if (state.getBlock() == BlockRegistry.BETWEENSTONE || state.getBlock() == BlockRegistry.PITSTONE
				|| state.getBlock() == top.getBlock() || state.getBlock() == filler.getBlock()) {
