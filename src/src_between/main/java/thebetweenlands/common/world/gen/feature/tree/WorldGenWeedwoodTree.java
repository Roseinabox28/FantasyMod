...
public class WorldGenWeedwoodTree extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		this.bark = BlockRegistry.LOG_WEEDWOOD.getDefaultState().withProperty(BlockLog.LOG_AXIS,
			...
			for (int zz = -maxRadius; zz <= maxRadius; zz++)
				for (int yy = 2; yy < height; yy++)
					if (!world.isAirBlock(pos.add(xx, yy, zz))
							&& world.getBlockState(pos.add(xx, yy, zz)).isNormalCube()) {
						return false;
					}
		...
		for (int yy = 0; yy < height; ++yy) {
			...
			for (int i = radius * -1; i <= radius; ++i)
				for (int j = radius * -1; j <= radius; ++j) {
					...
					if (Math.round(Math.sqrt(dSq)) == radius && yy == 0
							|| Math.round(Math.sqrt(dSq)) == radius && yy <= height - 1)
			...
			if (yy == height / 2 + 2) {
				...
				createBranch(world, rand, pos.add(radius + 1, yy - rand.nextInt(2), radius + 1), 5, false,
						rand.nextInt(2) + 3, hasPoisonIvy);
				createBranch(world, rand, pos.add(-radius - 1, yy - rand.nextInt(2), -radius - 1), 6, false,
						rand.nextInt(2) + 3, hasPoisonIvy);
				createBranch(world, rand, pos.add(-radius - 1, yy - rand.nextInt(2), radius + 1), 7, false,
						rand.nextInt(2) + 3, hasPoisonIvy);
				createBranch(world, rand, pos.add(radius + 1, yy - rand.nextInt(2), -radius - 1), 8, false,
						rand.nextInt(2) + 3, hasPoisonIvy);
			}
			...
			if (yy == 0) {
				...
				createBranch(world, rand, pos.add(radius + 1, yy - rand.nextInt(2), radius + 1), 5, true,
						rand.nextInt(2) + 3, hasPoisonIvy);
				createBranch(world, rand, pos.add(-radius - 1, yy - rand.nextInt(2), -radius - 1), 6, true,
						rand.nextInt(2) + 3, hasPoisonIvy);
				createBranch(world, rand, pos.add(-radius - 1, yy - rand.nextInt(2), radius + 1), 7, true,
						rand.nextInt(2) + 3, hasPoisonIvy);
				createBranch(world, rand, pos.add(radius + 1, yy - rand.nextInt(2), -radius - 1), 8, true,
						rand.nextInt(2) + 3, hasPoisonIvy);
			}
		}
	...
	private void createMainCanopy(World world, Random rand, BlockPos pos, int maxRadius) {
		...
		// Generate leaves after generating logs so that it doesn't trigger breakBlock
		// when replacing blocks, causing BlockLeaves' CHECK_DECAY to become true
		for (int x1 = -maxRadius; x1 <= maxRadius; x1++)
			for (int z1 = -maxRadius; z1 <= maxRadius; z1++)
				for (int y1 = 0; y1 < maxRadius; y1++) {
					double dSq = Math.pow(x1, 2.0D) + Math.pow(z1, 2.0D) + Math.pow(y1, 2.5D);
		...
		if (rand.nextInt(4) != 0) {
			...
			for (int yy = 0; yy < length; ++yy)
				if (world.isAirBlock(pos.down(yy)))
					world.setBlockState(pos.down(yy),
							ivy.withProperty(BlockVineBL.getPropertyFor(facing.getOpposite()), Boolean.valueOf(true)),
							2);
				else
					break;
		}
