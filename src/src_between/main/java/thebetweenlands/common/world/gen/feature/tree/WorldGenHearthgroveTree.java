...
public class WorldGenHearthgroveTree extends WorldGenHelper {
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		if (!this.rotatedCubeCantReplace(world, x, y + 2, z, 0, 0, 0, 1, height, 1, 0)
				&& !this.rotatedCubeCantReplace(world, x, y + 2, z, -canopySize + 2, 0, -canopySize + 2,
						canopySize * 2 - 4, height, canopySize * 2 - 4, 0)) {

			IBlockState log = BlockRegistry.LOG_HEARTHGROVE.getDefaultState()
					.withProperty(BlockLogBetweenlands.LOG_AXIS, BlockLog.EnumAxis.NONE);
			IBlockState logY = BlockRegistry.LOG_HEARTHGROVE.getDefaultState()
					.withProperty(BlockLogBetweenlands.LOG_AXIS, BlockLog.EnumAxis.Y);
			IBlockState leaves = BlockRegistry.LEAVES_HEARTHGROVE_TREE.getDefaultState()
			...
			for (int i = 0; i < 4; i++) {
				blobs.add(new BlockPos(x + rand.nextInt(canopySize * 2) - canopySize, y + height - canopyStart / 2,
						z + rand.nextInt(canopySize * 2) - canopySize));
			}
			...
			if (canopySize > 3) {
				for (int i = 0; i < branches; i++) {
					...
					BlockPos branchEnd = pos.add(
							Math.cos(Math.PI * 2 / (branches - 1) * i + randRot) * (canopySize - 4),
							height - canopyStart,
							Math.sin(Math.PI * 2 / (branches - 1) * i + randRot) * (canopySize - 4));
					...
					for (int j = 0; j < steps; j++) {
						BlockPos branchPos = new BlockPos(
								spline.interpolate(1.0F / (steps - 1) * j).add(0.5, 0.5, 0.5));
						if (!branchPos.equals(setBranchPos) && world.isAirBlock(branchPos)) {
							...
							if (!support && branchPos.distanceSq(x, branchPos.getY(), z) >= 2.5D) {
								EnumFacing supportDir = EnumFacing.getFacingFromVector(branchPos.getX() - x, 0,
										branchPos.getZ() - z);
								...
								for (int k = 0; k < supportHeight; k++) {
									this.setBlockAndNotifyAdequately(world,
											branchPos.add(supportXO, 1 + supportYO, supportZO), logY);
								...
								leavesSupportPoints
										.add(Pair.of(branchPos.add(supportXO, 1 + supportYO, supportZO), 6.0F));
			...
			for (int i = 0; i < canopyStart + 2; i++) {
				...
				this.generateCanopyLayer(world, pos.up(height - canopyStart + i), canopySize - i, leaves, p -> {
					...
					for (Pair<BlockPos, Float> support : leavesSupportPoints) {
						...
						supportRand.setSeed(MathHelper.getCoordinateRandom(supportPos.getX(), supportPos.getY(),
								supportPos.getZ()));
						...
						if ((supportPos.getX() - p.getX()) * (supportPos.getX() - p.getX()) * squashX
								...
								+ (supportPos.getZ() - p.getZ()) * (supportPos.getZ() - p.getZ())
										* squashZ <= support.getValue() + rand.nextFloat() * 0.8F) {
							nearSupport = true;
							break;
						}
					}
					if (!nearSupport)
						return false;
					return world.isAirBlock(p) && (k > 1 || (canopySize - k) < 4
							|| Math.sqrt((p.getX() - x) * (p.getX() - x) * centerSquashX
									+ (p.getZ() - z) * (p.getZ() - z) * centerSquashZ) >= canopySize - k - 3
											- rand.nextFloat() * 0.8F);
				}, p -> {
			...
			for (BlockPos endpoint : subbranchEndpoints) {
				...
				if (world.isAirBlock(branch1))
					this.setBlockAndNotifyAdequately(world, branch1, log);
				if (world.isAirBlock(branch2))
					this.setBlockAndNotifyAdequately(world, branch2, log);
			}
	...
	protected void generateCanopyLayer(World world, BlockPos center, final int size, IBlockState leaves,
			final @Nullable Predicate<BlockPos> pred, @Nullable Consumer<BlockPos> postprocessor) {
		int x = center.getX();
		int y = center.getY();
		int z = center.getZ();
		...
		default:
			...
			for (int i = 0; i <= span; i++) {
				this.rotatedCubeVolume(world, fullPred, x, y, z, -(radius - i), 0, -(span + 2 * i - 1) / 2, leaves,
						(radius - i) * 2 + 1, 1, span + 2 * i, 0, consumer);
				this.rotatedCubeVolume(world, fullPred, x, y, z, -(span + 2 * i - 1) / 2, 0, -(radius - i), leaves,
						span + 2 * i, 1, (radius - i) * 2 + 1, 0, consumer);
			}
			break;
		case 4:
