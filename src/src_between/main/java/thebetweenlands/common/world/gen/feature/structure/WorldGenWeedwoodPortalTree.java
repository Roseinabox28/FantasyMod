...
public class WorldGenWeedwoodPortalTree extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		LocationPortal location = new LocationPortal(worldStorage, new StorageUUID(UUID.randomUUID()),
				LocalRegion.getFromBlockPos(pos), pos);
	...
	private void createBranch(World world, Random rand, BlockPos pos, int dir, boolean root, int branchLength) {
		...
		for (int i = 0; i <= branchLength; ++i) {
			...
			case 5:
				if (!root)
					this.setBlockIfValid(world, pos.east(i - 1).up(y).south(i - 1), bark);
				else {
