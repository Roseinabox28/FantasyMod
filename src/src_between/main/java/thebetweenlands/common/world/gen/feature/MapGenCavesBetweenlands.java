...
public class MapGenCavesBetweenlands extends MapGenBase {
	...
	public MapGenCavesBetweenlands(long seed) {
		...
		noBreakBiomes = Lists.newArrayList(BiomeRegistry.DEEP_WATERS, BiomeRegistry.COARSE_ISLANDS,
				BiomeRegistry.MARSH_0, BiomeRegistry.MARSH_1, BiomeRegistry.PATCHY_ISLANDS, BiomeRegistry.SLUDGE_PLAINS,
				BiomeRegistry.SWAMPLANDS_CLEARING, BiomeRegistry.SLUDGE_PLAINS_CLEARING);
	}
	...
	public void generate(World world, int chunkX, int chunkZ, ChunkPrimer primer) {
		...
		for (int x = 0; x < 16; x++) {
			for (int z = 0; z < 16; z++) {
				this.seaBreakNoiseField[x * 16 + z] = seaLevelBreak.eval((cx + x) * XZ_BREAK_SCALE,
						(cz + z) * XZ_BREAK_SCALE);
			}
		}
				...
				for (int y = 0; y < 128; y++) {
					...
					for (int xo = 0; xo < 2; xo++) {
						...
						for (int zo = 0; zo < 2; zo++) {
							...
							double shouldntBreak = noBreakBiomes.contains(biome) ? SHOULDNT_BREAK
									: (1 - this.biomeWeights.get(bx, bz)) * SHOULDNT_BREAK;
							...
							for (int yo = 0; yo < 1; yo++) {
								...
								if (surfaceDist <= UPPER_BOUND) {
									noise += (shouldntBreak + MathUtils
											.linearTransformd(this.seaBreakNoiseField[bx * 16 + bz], -1, 1, 0, 1))
											* BREAK_SCALE * (1 - surfaceDist / (float) UPPER_BOUND);
								}
								// if (y == level) {
								// primer.setBlockState(bx, 150, bz,
								// Blocks.STAINED_GLASS.getDefaultState().withProperty(BlockStainedGlass.COLOR,
								// EnumDyeColor.byMetadata((int) MathUtils.linearTransformd(noise, -0.5F, 1, 0,
								// 15))));
								// }
								IBlockState state = primer.getBlockState(bx, by, bz);
								if (state.getBlock() == BlockRegistry.SWAMP_WATER && noise < limit + 0.25
										&& noise > limit) {
									...
									primer.setBlockState(bx, by, bz,
											by > WorldProviderBetweenlands.CAVE_WATER_HEIGHT
													? Blocks.AIR.getDefaultState()
													: BlockRegistry.SWAMP_WATER.getDefaultState());
								}
							}
	...
	private double sampleNoise(int x, int y, int z) {
		return this.cave.eval(x * XZ_CAVE_SCALE, y * Y_CAVE_SCALE, z * XZ_CAVE_SCALE)
				+ form.eval(x * XZ_FORM_SCALE, y * Y_FORM_SCALE, z * XZ_FORM_SCALE) * FORM_SCALE;
	}
}
