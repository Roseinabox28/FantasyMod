...
public class WorldGenSapTree extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		this.leaves = BlockRegistry.LEAVES_SAP_TREE.getDefaultState().withProperty(BlockLeavesBetweenlands.CHECK_DECAY,
				false);

		for (int xx = -maxRadius; xx <= maxRadius; xx++)
			for (int zz = -maxRadius; zz <= maxRadius; zz++)
				for (int yy = 2; yy < height; yy++)
					if (!world.isAirBlock(pos.add(xx, yy, zz)) && !world.getBlockState(pos.add(xx, yy, zz)).getBlock()
							.isReplaceable(world, pos.add(xx, yy, zz)))
						return false;
