...
public class DeepWatersFeature extends BiomeFeature {
	...
	public void generateNoise(int chunkX, int chunkZ, Biome biome) {
		this.terrainNoise = this.islandNoiseGen.getRegion(this.terrainNoise, (double) (chunkX * 16),
				(double) (chunkZ * 16), 16, 16, 0.05D * 1.0D, 0.05D * 1.0D, 1.0D);
	}
	...
	public void replaceStackBlocks(int x, int z, double baseBlockNoise, ChunkPrimer chunkPrimer,
		...
		if (pass == EnumGeneratorPass.PRE_REPLACE_BIOME_BLOCKS) {
			...
			for (int y = lowestBlock; y < lerp(
					layerHeight - (layerHeight - lowestBlock) / 2.5f + noise * (layerHeight - lowestBlock) - 2,
					lowestBlock, biomeWeight); y++) {
