...
public class WorldGenRottenLogs extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		this.log = BlockRegistry.LOG_ROTTEN_BARK.getDefaultState().withProperty(BlockLogBetweenlands.LOG_AXIS,
				EnumAxis.NONE);
				...
				for (int zz = z - length; length + z - 1 >= zz; zz++) {
					for (int yy = y + 1; yy <= y + baseRadius * 2; yy++) {
						...
						if (!world.isBlockLoaded(offsetPos)
								|| !world.getBlockState(offsetPos).getBlock().isReplaceable(world, offsetPos)) {
							return false;
						}
					}
				}
				...
				for (int i = baseRadius * -1; i <= baseRadius; ++i)
					for (int j = baseRadius * -1; j <= baseRadius; ++j) {
						...
						if (Math.round(Math.sqrt(dSq)) == baseRadius) {
							...
							if (zz == z - length && rand.nextInt(2) == 0
									|| zz == z + length - 1 && rand.nextInt(2) == 0)
								world.setBlockState(offsetPos, Blocks.AIR.getDefaultState(), 2 | 16);
						} else
							world.setBlockState(offsetPos, Blocks.AIR.getDefaultState(), 2 | 16);
					}
				...
				for (int i = baseRadius * -1; i <= baseRadius; ++i)
					for (int j = baseRadius * -1; j <= baseRadius; ++j) {
						...
						if (Math.round(Math.sqrt(dSq)) == baseRadius) {
							...
							if (rand.nextInt(12) == 0)
								world.setBlockState(offsetPos, Blocks.AIR.getDefaultState(), 2 | 16);
							if (xx == x - length && rand.nextInt(2) == 0
									|| xx == x + length - 1 && rand.nextInt(2) == 0)
								world.setBlockState(offsetPos, Blocks.AIR.getDefaultState(), 2 | 16);
						} else
							world.setBlockState(offsetPos, Blocks.AIR.getDefaultState(), 2 | 16);
					}
