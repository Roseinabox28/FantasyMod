...
public abstract class WorldGenGiantTreeTrunk extends WorldGenerator {
	...
	public static final EnumFacing[] DIRECTIONS = { EnumFacing.NORTH, EnumFacing.SOUTH, EnumFacing.EAST,
			EnumFacing.WEST };

	private static final byte[][] TRUNK_LAYERS = new byte[MAX_TRUNK_RADIUS - MIN_TRUNK_RADIUS + 1][];

	public static IBlockState BARK = BlockRegistry.LOG_WEEDWOOD.getDefaultState().withProperty(BlockLog.LOG_AXIS,
		...
		for (int dy = 0; dy < height; dy++) {
			...
			for (int dx = -radius; dx <= radius; dx++) {
				for (int dz = -radius; dz <= radius; dz++) {
					...
					switch (state) {
					case 0:
						if (world.getBlockState(new BlockPos(blockX + dx, y - 1, blockZ + dz))
								.getBlock() == BlockRegistry.LOG_WEEDWOOD) {
							this.setBlockAndNotifyAdequately(world, new BlockPos(blockX + dx, y - 1, blockZ + dz),
									BARK);
						}
						break;
					case 1:
					...
					case 2:
						...
						if (canFungusGenerateAtY(dy, height) && rand.nextInt(8) == 0) {
							...
							fungi.add(new Fungus(new BlockPos(blockX + dx + fungusX, y, blockZ + dz + fungusZ),
									fungusRadius));
						}
					}
				}
	...
	protected void generateShoots(World world, Random rand, int baseRadius, int height, int blockX, int blockY,
			int blockZ) {
	...
	private void generateRoots(World world, Random rand, int baseRadius, int height, int baseY, int blockX, int blockY,
			int blockZ, int rootCount, boolean high) {
		...
		for (int root = 0; root < rootCount; root++) {
			...
			float posX = MathHelper.cos(yaw) * baseRadius * 0.2F + blockX, posY = blockY,
					posZ = MathHelper.sin(yaw) * baseRadius * 0.2F + blockZ;
			generateRoot(world, rand, posX, posY, posZ, yaw, pitch, length, 2, 1);
		}
	}

	private void generateRoot(World world, Random rand, float posX, float posY, float posZ, float yaw, float pitch,
			int length, float startSize, float endSize) {
		...
		for (int step = 0; step < length; step++) {
			...
			if (step == branchPoint && startSize > 1.75F) {
				...
				generateRoot(world, rand, posX, posY, posZ, yaw + branchYaw + branchAngle, pitch, length - step, size,
						1);
				generateRoot(world, rand, posX, posY, posZ, yaw + branchYaw - branchAngle, pitch, length - step, size,
						1);
				break;
			...
			for (int y = minY; y <= maxY; y++) {
				for (int z = minZ; z <= maxZ; z++) {
					if (world.getBlockState(new BlockPos(x, y, z)) == BARK) {
						IBlockState t;
						if (((t = world.getBlockState(new BlockPos(x + 1, y, z))) == BARK || t == WOOD)
	...
	protected void placeWood(World world, Random rand, int radius, int height, int blockX, int blockY, int blockZ,
			int dx, int dy, int dz) {
		boolean isDentrothyst = dy < 40 ? (rand.nextInt(dy * 2 + 60) == 0 ? true : false) : false;
		if (isDentrothyst) {
			this.setBlockAndNotifyAdequately(world, new BlockPos(blockX + dx, blockY + dy, blockZ + dz),
					rand.nextInt(20) == 0 ? BlockRegistry.DENTROTHYST.getDefaultState()
							.withProperty(BlockDentrothyst.TYPE, EnumDentrothyst.ORANGE)
							: BlockRegistry.DENTROTHYST.getDefaultState());
		} else {
			this.setBlockAndNotifyAdequately(world, new BlockPos(blockX + dx, blockY + dy, blockZ + dz), WOOD);
		}
	}

	protected void placeBark(World world, Random rand, int radius, int height, int blockX, int blockY, int blockZ,
			int dx, int dy, int dz) {
	...
	protected int getRadius(int baseRadius, int dy) {
		...
		return (STEEPNESS * MIN_TRUNK_RADIUS - baseRadius * STEEPNESS)
				/ (-baseRadius * dy - STEEPNESS + MIN_TRUNK_RADIUS * dy) + MIN_TRUNK_RADIUS;
	}
		...
		for (int i = 0; i < TRUNK_LAYERS.length; i++) {
			...
			for (int x = -radius; x <= radius; x++) {
				for (int z = -radius; z <= radius; z++) {
					...
					float fold = (MathHelper.sin(angle * 2 * (float) Math.PI * 2) + 1)
							* (slope * (radius - MIN_TRUNK_RADIUS));
					if (dist <= radius - fold) {
