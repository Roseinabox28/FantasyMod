...
public class WorldGenDruidCircle implements IWorldGenerator {
	private static final IBlockState[] RUNE_STONES = { BlockRegistry.DRUID_STONE_1.getDefaultState(),
			BlockRegistry.DRUID_STONE_2.getDefaultState(), BlockRegistry.DRUID_STONE_3.getDefaultState(),
			BlockRegistry.DRUID_STONE_4.getDefaultState(), BlockRegistry.DRUID_STONE_5.getDefaultState() };
	...
	private void generate(World world, Random random, int startX, int startZ) {
		...
		check: for (int xo = this.baseRadius + 1; xo <= this.checkRadius - (this.baseRadius + 1); xo++) {
			for (int zo = this.baseRadius + 1; zo <= this.checkRadius - (this.baseRadius + 1); zo++) {
				int x = startX + xo;
				int z = startZ + zo;
				if (world.isAreaLoaded(new BlockPos(x - baseRadius - 2, 64, z - baseRadius - 2),
						new BlockPos(x + baseRadius + 3, 64, z + baseRadius + 3))) {
					pos.setPos(x, 0, z);
	...
	public boolean canGenerateAt(World world, BlockPos altar) {
		for (BlockPos p : BlockPos.getAllInBox(altar.add(-this.baseRadius, 1, -this.baseRadius),
				altar.add(this.baseRadius, this.height, this.baseRadius))) {
	...
	private void placeAir(World world, MutableBlockPos pos) {
		...
		for (int k = 0, y = pos.getY(); k <= this.height; k++, pos.setY(y + k)) {
			IBlockState blockState = world.getBlockState(pos);
			if (blockState == biome.fillerBlock || blockState == biome.topBlock
					|| blockState.getMaterial() == Material.ROCK || blockState.getMaterial() == Material.GROUND) {
				world.setBlockToAir(pos.toImmutable());
			}
		}
	}
		...
		for (int k = 0, y = pos.getY(); k <= height; k++, pos.setY(y + k)) {
			...
			if (rand.nextBoolean()) {
				world.setBlockState(pos.toImmutable(),
						getRandomRuneBlock(rand).withProperty(BlockDruidStone.FACING, facing), 3);
			} else {
				world.setBlockState(pos.toImmutable(),
						BlockRegistry.DRUID_STONE_6.getDefaultState().withProperty(BlockDruidStone.FACING, facing));
