...
public class WorldGenGiantTreeDead extends WorldGenGiantTreeTrunk {
	...
	public void addVineToRandomBlockSide(World world, Random rand, int x, int y, int z) {
		...
		if (world.getBlockState(new BlockPos(x + direction.getXOffset(), y, z + direction.getZOffset())).getMaterial()
				.isReplaceable()) {
			world.setBlockState(new BlockPos(x + direction.getXOffset(), y, z + direction.getZOffset()),
					IVY.withProperty(BlockVineBL.getPropertyFor(direction.getOpposite()), Boolean.valueOf(true)), 2);
		}
	}
	...
	private int calculateTilt(int dx, int dz) {
		...
		return (int) ((((MathHelper.sin(dx * TILT_SCALE + tiltOffsetX) * 0.5F
				+ MathHelper.sin(dz * TILT_SCALE + tiltOffsetZ) * 0.5F) + 1) * 0.5F) * tiltHeight);
	}
}
