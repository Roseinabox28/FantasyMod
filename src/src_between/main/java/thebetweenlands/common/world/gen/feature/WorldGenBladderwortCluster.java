...
public class WorldGenBladderwortCluster extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos position) {
		...
		for (int i = 0; i < 128; ++i) {
			...
			if (world.isBlockLoaded(pos) && SurfaceType.WATER.matches(world, pos.up())
					&& SurfaceType.DIRT.matches(world, pos)) {
	...
	private boolean generateBladderwortStack(World world, Random rand, BlockPos pos) {
		...
		for (int yo = 0; yo < 128; yo++) {
			...
			if (world.isAirBlock(checkPos)
					&& world.isAirBlock(checkPos.setPos(pos.getX(), pos.getY() + yo + 1, pos.getZ()))) {
				height = yo;
				break;
			}
		}
