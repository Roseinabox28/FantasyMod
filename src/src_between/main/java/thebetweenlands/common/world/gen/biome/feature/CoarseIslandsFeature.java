...
public class CoarseIslandsFeature extends BiomeFeature {
	private final IBlockState cragrockDefault = BlockRegistry.CRAGROCK.getDefaultState()
			.withProperty(BlockCragrock.VARIANT, EnumCragrockType.DEFAULT);
	private final IBlockState cragrockMossy1 = BlockRegistry.CRAGROCK.getDefaultState()
			.withProperty(BlockCragrock.VARIANT, EnumCragrockType.MOSSY_1);
	private final IBlockState cragrockMossy2 = BlockRegistry.CRAGROCK.getDefaultState()
			.withProperty(BlockCragrock.VARIANT, EnumCragrockType.MOSSY_2);
	...
	public void generateNoise(int chunkX, int chunkZ, Biome biome) {
		this.islandNoise = this.islandNoiseGen.getRegion(this.islandNoise, (double) (chunkX * 16),
				(double) (chunkZ * 16), 16, 16, 0.08D * 0.6D, 0.08D * 0.6D, 1.0D);
		this.cragNoise = this.cragNoiseGen.getRegion(this.cragNoise, (double) (chunkX * 16), (double) (chunkZ * 16), 16,
				16, 0.1D * 4.5D, 0.1D * 4.5D, 1.0D);
	}
	...
	public void replaceStackBlocks(int x, int z, double baseBlockNoise, ChunkPrimer chunkPrimer,
		...
		if (pass == EnumGeneratorPass.PRE_REPLACE_BIOME_BLOCKS) {
			...
			if (islandNoise <= 0
					&& chunkPrimer.getBlockState(x, layerHeight, z).getBlock() == chunkGenerator.layerBlock) {
				int minHeight = 1;
				...
				for (int y = lowestBlock; y < lerp(layerHeight - (layerHeight - lowestBlock) / 3.5f, lowestBlock,
						biomeWeight); y++) {
					chunkPrimer.setBlockState(x, y, z, chunkGenerator.baseBlockState);
				}
			}
