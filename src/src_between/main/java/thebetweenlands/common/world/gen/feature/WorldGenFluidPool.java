				...
				for (int oz = 0; oz < 16; ++oz) {
					for (int oy = 0; oy < 8; ++oy) {
						boolean isOuterBlock = !isInPool[(ox * 16 + oz) * 8 + oy]
								&& (ox < 15 && isInPool[((ox + 1) * 16 + oz) * 8 + oy]
										|| ox > 0 && isInPool[((ox - 1) * 16 + oz) * 8 + oy]
										|| oz < 15 && isInPool[(ox * 16 + oz + 1) * 8 + oy]
										|| oz > 0 && isInPool[(ox * 16 + (oz - 1)) * 8 + oy]
										|| oy < 7 && isInPool[(ox * 16 + oz) * 8 + oy + 1]
										|| oy > 0 && isInPool[(ox * 16 + oz) * 8 + (oy - 1)]);
						...
						if (isOuterBlock) {
							...
							if (oy < 4 && !material.isSolid()
									&& worldIn.getBlockState(position.add(ox, oy, oz)).getBlock() != this.block) {
								return false;
							}
						}
					}
