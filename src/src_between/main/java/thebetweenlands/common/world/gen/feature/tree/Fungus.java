	...
	public void generate(World world, Random rand) {
		for (int y = posY; y > posY - radius; y -= 2) {
			...
			while (!pendingCoords.isEmpty()) {
				...
				if (world.getBlockState(coord).getMaterial().isReplaceable()
						|| world.getBlockState(coord.up()).getMaterial().isReplaceable()) {
				...
				for (EnumFacing direction : WorldGenGiantTreeTrunk.DIRECTIONS) {
					BlockPos neighborCoord = new BlockPos(coord.getX() + direction.getXOffset(), coord.getY(),
							coord.getZ() + direction.getZOffset());
					IBlockState block = world.getBlockState(neighborCoord);
					IBlockState above = world.getBlockState(neighborCoord.up());
					if (!pendingCoords.contains(neighborCoord)
							&& getDistanceBetweenChunkCoordinates(center, neighborCoord) <= radius
							&& (block.getMaterial().isReplaceable() || (above.getMaterial().isReplaceable()
									&& block.getBlock() != BlockRegistry.SHELF_FUNGUS))) {
