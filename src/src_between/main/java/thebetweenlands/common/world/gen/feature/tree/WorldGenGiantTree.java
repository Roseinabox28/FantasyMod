...
public class WorldGenGiantTree extends WorldGenGiantTreeTrunk {
	...
	protected void generateShoots(World world, Random rand, int baseRadius, int height, int blockX, int blockY,
		...
		generateBranches(world, rand, baseRadius, height, endRadius, blockY, blockX, blockY + height / 3 * 2, blockZ,
				true);
		generateBranches(world, rand, baseRadius, height, endRadius, blockY, blockX, blockY + height / 3, blockZ,
				false);
		generateBranch(world, rand, blockX, blockY + height, blockZ, rand.nextFloat() * 2 * (float) Math.PI,
				(float) Math.PI / 2, 6, endRadius, -2);
	}

	protected void generateBranches(World world, Random rand, int baseRadius, int height, int endRadius, int baseY,
			int blockX, int blockY, int blockZ, boolean top) {
		...
		for (int branch = 0; branch < branchCount; branch++) {
			...
			generateBranch(world, rand, blockX, Math.min(blockY + rand.nextInt(10), baseY + height - rand.nextInt(10)),
					blockZ, yaw, pitch, length, (int) (endRadius * 0.85F), (int) (endRadius * 0.4F));
		}
	}

	protected void generateBranch(World world, Random rand, float posX, float posY, float posZ, float yaw, float pitch,
		...
		for (int step = 0; step < length; step++) {
			...
			if (step == branchPoint && startSize > 1 && length > 5) {
				...
				generateBranch(world, rand, posX, posY, posZ, yaw + branchYaw + branchAngle,
						pitch + (float) Math.PI / 6, length - step, size * 0.8F, -1);
				generateBranch(world, rand, posX, posY, posZ, yaw + branchYaw - branchAngle,
						pitch + (float) Math.PI / 6, length - step, size * 0.8F, -1);
			}
			for (int x = -sizeRange; x <= sizeRange; x++) {
				for (int z = -sizeRange; z <= sizeRange; z++) {
					for (int y = -sizeRange; y <= sizeRange; y++) {
						...
						if (dist <= size) {
							IBlockState block = world
									.getBlockState(new BlockPos((int) posX + x, (int) posY + y, (int) posZ + z));
							IBlockState above = world
									.getBlockState(new BlockPos((int) posX + x, (int) posY + y + 1, (int) posZ + z));
							...
							for (int y = -size; y <= size; y++) {
								if (MathHelper.sqrt(x * x + y * y + z * z) <= size) {
									this.setBlockAndNotifyAdequately(world,
											new BlockPos((int) posX + x, (int) posY + y, (int) posZ + z), BARK);
								}
							}
						}
					}
			...
			for (int z = blockZ - maxRadius; z <= blockZ + maxRadius; z++) {
				for (int y = blockY; y < blockY + maxRadius; y++) {
					int dist = (int) Math.round(
							Math.sqrt(Math.pow(x - blockX, 2) + Math.pow(z - blockZ, 2) + Math.pow(y - blockY, 2.5)));
					if (dist < maxRadius - 1 && rand.nextInt(4) == 0 && y > blockY) {
						if (world.getBlockState(new BlockPos(x, y, z)).getBlock() != BlockRegistry.LOG_WEEDWOOD) {
							this.setBlockAndNotifyAdequately(world, new BlockPos(x, y, z),
									BlockRegistry.LOG_WEEDWOOD.getDefaultState());
						}
					}
					if (dist <= maxRadius) {
						if (world.getBlockState(new BlockPos(x, y, z)).getBlock() != BlockRegistry.LOG_WEEDWOOD
								&& world.getBlockState(new BlockPos(x, y, z)) != BARK) {
			...
			for (int y = startY; y > startY - length; y--) {
				if (world.getBlockState(new BlockPos(x, y, z)).getMaterial().isReplaceable()) {
					if (startY - y < 6 && rand.nextInt(5) == 0) {
						this.setBlockAndNotifyAdequately(world, new BlockPos(x, y, z),
								HANGER.withProperty(BlockHanger.SEEDED, true));
					} else {
	...
	protected void generateWorldLocation(World world, Random rand, BlockPos pos) {
		...
		LocationStorage treeLocation = new LocationStorage(worldStorage, new StorageUUID(UUID.randomUUID()),
				LocalRegion.getFromBlockPos(pos), "giant_tree", EnumLocationType.GIANT_TREE);
		treeLocation.addBounds(new AxisAlignedBB(pos.getX() - 32, pos.getY() - 10, pos.getZ() - 32, pos.getX() + 32,
				pos.getY() + 80, pos.getZ() + 32));
