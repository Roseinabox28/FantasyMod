...
public class BiomeGenerator {
	...
	protected IBlockState underLayerTopBlockState = BlockRegistry.MUD.getDefaultState(),
			baseBlockState = BlockRegistry.BETWEENSTONE.getDefaultState();
	...
	public final void runBiomeFeatures(int blockX, int blockZ, int inChunkX, int inChunkZ, double baseBlockNoise,
		...
		for (BiomeFeature feature : this.biomeFeatures) {
			feature.replaceStackBlocks(inChunkX, inChunkZ, baseBlockNoise, chunkPrimer, chunkGenerator,
	...
	public final void replaceBiomeBlocks(int blockX, int blockZ, int inChunkX, int inChunkZ, double baseBlockNoise,
			Random rng, long seed, ChunkPrimer chunkPrimer, ChunkGeneratorBetweenlands chunkGenerator,
			Biome[] biomesForGeneration, BiomeWeights biomeWeights) {
		this.chunkGenerator = chunkGenerator;
		this.biomesForGeneration = biomesForGeneration;

		if (!this.replaceStackBlocks(blockX, blockZ, inChunkX, inChunkZ, baseBlockNoise, chunkPrimer, chunkGenerator,
		...
		for (int y = 255; y >= 0; --y) {
			...
			if (currentBlockState.getBlock() == chunkGenerator.baseBlock
					&& blockAboveState.getBlock() == chunkGenerator.layerBlock) {
				blocksBelowLayer++;
			}
			...
			if (blocksBelowLayer <= this.underLayerBlockHeight && blocksBelowLayer >= 0) {
				...
				chunkPrimer.setBlockState(inChunkX, y, inChunkZ, this.biome.topBlock);
					...
					&& currentBlockState.getBlock() == chunkGenerator.baseBlock) {
		...
		this.replaceStackBlocks(blockX, blockZ, inChunkX, inChunkZ, baseBlockNoise, chunkPrimer, chunkGenerator,
