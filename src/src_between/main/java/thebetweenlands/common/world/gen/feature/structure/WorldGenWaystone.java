...
public class WorldGenWaystone extends WorldGenerator implements IWorldGenerator {
		...
		if (SurfaceType.MIXED_GROUND.matches(world.getBlockState(pos.down()))) {
			...
			if (!state1.getMaterial().isLiquid() && state1.getBlock().isReplaceable(world, pos)
				...
				LocationStorage locationStorage = new LocationStorage(worldStorage, new StorageUUID(UUID.randomUUID()),
						LocalRegion.getFromBlockPos(pos.getX(), pos.getZ()), "waystone", EnumLocationType.WAYSTONE);
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator,
		...
		if (world.provider.getDimension() == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId) {
			...
			int blockX = (regionX << (gridSizePower2 + 4)) + gridBlockSize / 4
					+ seededRandom.nextInt(gridBlockSize / 2 + 1);
			int blockZ = (regionZ << (gridSizePower2 + 4)) + gridBlockSize / 4
					+ seededRandom.nextInt(gridBlockSize / 2 + 1);

			if (blockX > chunkX * 16 + 5 && blockZ > chunkZ * 16 + 5 && blockX < (chunkX + 2) * 16 - 5
					&& blockZ < (chunkZ + 2) * 16 - 5) {
