...
public abstract class WorldGenHelper extends WorldGenerator {
	...
	public enum EnumRotationSequence {
		// TODO: This ought to be removed and replaced with the proper block states at
		// some point
		// E.g. enum.getRotatedBlockState(IProperty propertyThatContainsTheRotation,
		// IBlockState theBlockStateToRotate)

		STAIR(0, 3, 1, 2), UPSIDE_DOWN_STAIR(4, 7, 5, 6), CHEST(2, 5, 3, 4), LOG_SIDEWAYS(4, 8), PILLAR_SIDEWAYS(8, 7);
	...
	/**
	 * @see #rotatedCubeVolume(World, int, int, int, int, int, int, IBlockState,
	 *      int, int, int, int, Predicate, Consumer...)
	 * @param world
	 * @param x
	 * @param y
	 * @param z
	 ...
	 */
	@SafeVarargs
	public final void rotatedCubeVolume(World world, int x, int y, int z, int offsetX, int offsetY, int offsetZ,
			IBlockState blockState, int sizeWidth, int sizeHeight, int sizeDepth, int rotation,
			Consumer<BlockPos>... callbacks) {
		this.rotatedCubeVolume(world, null, x, y, z, offsetX, offsetY, offsetZ, blockState, sizeWidth, sizeHeight,
				sizeDepth, rotation, callbacks);
	}
	 ...
	 * @param predicate  The predicate decides whether a block should be placed or
	 *                   not
	 ...
	 */
	@SafeVarargs
	public final void rotatedCubeVolume(World world, @Nullable Predicate<BlockPos> pred, int x, int y, int z,
			int offsetX, int offsetY, int offsetZ, IBlockState blockState, int sizeWidth, int sizeHeight, int sizeDepth,
			int rotation, Consumer<BlockPos>... callbacks) {
		x -= width / 2;
		z -= depth / 2;
		switch (rotation) {
		case 0:
	 ...
	 * @return
	 */
	public final AxisAlignedBB rotatedAABB(World world, double x, double y, double z, double offsetX, double offsetY,
			double offsetZ, double sizeWidth, double sizeHeight, double sizeDepth, int rotation) {
		x -= width / 2;
		z -= depth / 2;
		switch (rotation) {
		default:
		case 0:
			return new AxisAlignedBB(x + offsetX, y + offsetY, z + offsetZ, x + offsetX + sizeWidth,
					y + offsetY + sizeHeight, z + offsetZ + sizeDepth);
		case 1:
			return new AxisAlignedBB(x + offsetZ, y + offsetY, z + depth - offsetX - sizeWidth - 1,
					x + offsetZ + sizeDepth, y + offsetY + sizeHeight, z + depth - offsetX - 1);
		case 2:
			return new AxisAlignedBB(x + width - offsetX - sizeWidth - 1, y + offsetY,
					z + depth - offsetZ - sizeDepth - 1, x + width - offsetX - 1, y + offsetY + sizeHeight,
					z + depth - offsetZ - 1);
		case 3:
			return new AxisAlignedBB(x + width - offsetZ - sizeDepth - 1, y + offsetY, z + offsetX,
	 ...
	 * @return
	 */
	public final BlockPos rotatePos(World world, int x, int y, int z, int offsetX, int offsetY, int offsetZ,
			int rotation) {
		x -= width / 2;
		z -= depth / 2;
		switch (rotation) {
	 ...
	 * @return
	 */
	public final BlockPos rotatePosNoOffsets(World world, int x, int y, int z, int offsetX, int offsetY, int offsetZ,
			int rotation) {
		x -= width / 2;
		z -= depth / 2;
		switch (rotation) {
	...
	/**
	 * Creates a WorldGenHelper#rotatedCubeVolume that also extends down until it
	 * hits a non replaceable block
	 *
	 * @param world      The world
	 * @param x          x to generate relative from
	 * @param y          y to generate relative from
	 ...
	 */
	@SafeVarargs
	public final void rotatedCubeVolumeExtendedDown(World world, int x, int y, int z, int offsetX, int offsetY,
			int offsetZ, IBlockState blockState, int sizeWidth, int sizeHeight, int sizeDepth, int rotation,
			Consumer<BlockPos>... callbacks) {
		for (int w = 0; w < sizeWidth; w++) {
			for (int d = 0; d < sizeDepth; d++) {
				while (y + offsetY > 0
						&& isReplaceable(world, x, y, z, offsetX + w, offsetY - 1, offsetZ + d, rotation)) {
					offsetY--;
					sizeHeight++;
				}
				rotatedCubeVolume(world, x, y, z, offsetX + w, offsetY, offsetZ + d, blockState, 1, sizeHeight, 1,
						rotation, callbacks);
			}
		}
	}
	 ...
	 */
	public boolean isReplaceable(World world, int x, int y, int z, int offsetX, int offsetY, int offsetZ,
			int rotation) {
		x -= width / 2;
		z -= depth / 2;
		BlockPos pos;
		...
		case 0:
			...
			return world.isBlockLoaded(pos) && (world.getBlockState(pos).getBlock().isReplaceable(world, pos)
					|| (replaceable != null && checkReplaceablePredicates(world.getBlockState(pos))));
		case 1:
	 ...
	 * @return
	 */
	public boolean rotatedCubeMatches(World world, int x, int y, int z, int offsetX, int offsetY, int offsetZ,
			int sizeWidth, int sizeHeight, int sizeDepth, int rotation, SurfaceType type) {
		x -= width / 2;
		z -= depth / 2;
		switch (rotation) {
		case 0:
			for (int yy = y + offsetY; yy < y + offsetY + sizeHeight; yy++)
				for (int xx = x + offsetX; xx < x + offsetX + sizeWidth; xx++)
					for (int zz = z + offsetZ; zz < z + offsetZ + sizeDepth; zz++) {
						if (!world.isBlockLoaded(this.getCheckPos(xx, yy, zz))
								|| !type.matches(world.getBlockState(this.getCheckPos(xx, yy, zz))))
							return false;
					}
			break;
		case 1:
	 ...
	 */
	public void rotatedLootPot(World world, Random rand, int x, int y, int z, int offsetX, int offsetY, int offsetZ,
			int rotation, int min, int max, int chance, ResourceLocation lootTable) {
		x -= width / 2;
		z -= depth / 2;
		...
		case 1:
			...
			break;
		case 2:
			generateLootPot(world, rand, new BlockPos(x + width - offsetX - 1, y + offsetY, z + depth - offsetZ - 1),
					min, max, lootTable);
			break;
		case 3:
	 ...
	 * @param chance  The chance of it actually generating
	 */
	public void rotatedLootChest(World world, Random rand, int x, int y, int z, int offsetX, int offsetY, int offsetZ,
			int rotation, int min, int max, int chance, int sequenceStart, ResourceLocation lootTable) {
		x -= width / 2;
		z -= depth / 2;
		if (rand.nextInt(chance) == 0)
			return;
		switch (rotation) {
		case 0:
			generateLootChest(world, rand, new BlockPos(x + offsetX, y + offsetY, z + offsetZ), min, max,
					getStateFromRotation(sequenceStart, rotation, BlockRegistry.WEEDWOOD_CHEST.getDefaultState(),
							EnumRotationSequence.CHEST),
					lootTable);
			break;
		case 1:
			generateLootChest(world, rand, new BlockPos(x + offsetZ, y + offsetY, z + depth - offsetX - 1), min, max,
					getStateFromRotation(sequenceStart, rotation, BlockRegistry.WEEDWOOD_CHEST.getDefaultState(),
							EnumRotationSequence.CHEST),
					lootTable);
			break;
		case 2:
			generateLootChest(world, rand, new BlockPos(x + width - offsetX - 1, y + offsetY, z + depth - offsetZ - 1),
					min, max, getStateFromRotation(sequenceStart, rotation,
							BlockRegistry.WEEDWOOD_CHEST.getDefaultState(), EnumRotationSequence.CHEST),
					lootTable);
			break;
		case 3:
			generateLootChest(world, rand, new BlockPos(x + width - offsetZ - 1, y + offsetY, z + offsetX), min, max,
					getStateFromRotation(sequenceStart, rotation, BlockRegistry.WEEDWOOD_CHEST.getDefaultState(),
							EnumRotationSequence.CHEST),
					lootTable);
			break;
	 ...
	 */
	public MobSpawnerLogicBetweenlands rotatedSpawner(World world, int x, int y, int z, int offsetX, int offsetY,
			int offsetZ, int rotation, String mob) {
	 ...
	 * @return
	 */
	public boolean rotatedCubeCantReplace(World world, int x, int y, int z, int offsetA, int offsetB, int offsetC,
			int sizeWidth, int sizeHeight, int sizeDepth, int direction) {
		x -= width / 2;
		z -= depth / 2;
		switch (direction) {
		case 0:
			for (int yy = y + offsetB; yy < y + offsetB + sizeHeight; yy++)
				for (int xx = x + offsetA; xx < x + offsetA + sizeWidth; xx++)
					for (int zz = z + offsetC; zz < z + offsetC + sizeDepth; zz++) {
						if (!world.isBlockLoaded(this.getCheckPos(xx, yy, zz))
								|| !isReplaceable(world, xx, yy, zz, 0, 0, 0, 0))
							return true;
					}
			break;
		case 1:
	...
	public IBlockState getStateFromRotation(int start, int rotation, IBlockState state,
			EnumRotationSequence enumRotationSequence) {
		return state.getBlock().getStateFromMeta(
				enumRotationSequence.sequence[(rotation + start) % enumRotationSequence.sequence.length]);
	}
	...
	public IBlockState getRandomLootPot(Random random) {
		...
		case 0:
			return BlockRegistry.LOOT_POT.getDefaultState()
					.withProperty(BlockLootPot.VARIANT, BlockLootPot.EnumLootPot.POT_1)
					.withProperty(BlockLootPot.FACING, EnumFacing.byIndex(randDirection));
		case 1:
			return BlockRegistry.LOOT_POT.getDefaultState()
					.withProperty(BlockLootPot.VARIANT, BlockLootPot.EnumLootPot.POT_2)
					.withProperty(BlockLootPot.FACING, EnumFacing.byIndex(randDirection));
		default:
			return BlockRegistry.LOOT_POT.getDefaultState()
					.withProperty(BlockLootPot.VARIANT, BlockLootPot.EnumLootPot.POT_3)
					.withProperty(BlockLootPot.FACING, EnumFacing.byIndex(randDirection));
		}
	}
