...
public class WorldProviderBetweenlands extends WorldProvider {
	...
	/**
	 * Returns a WorldProviderBetweenlands instance if world is not null and
	 * world#provider is an instance of WorldProviderBetweenlands
	 *
	 * @param world
	 */
	...
	public float getSunBrightness(float partialTicks) {
		...
		return rift.getVisibility(partialTicks) * this.getOverworldSunBrightness(partialTicks) * 0.6F
				+ rift.getVisibility(partialTicks) * 0.2F;
	}

	@Override
	public float getSunBrightnessFactor(float partialTicks) {
		BetweenlandsWorldStorage storage = BetweenlandsWorldStorage.forWorldNullable(world); // Null if called by
	...
	public IChunkGenerator createChunkGenerator() {
		return new ChunkGeneratorBetweenlands(this.world, this.world.getSeed(), BlockRegistry.BETWEENSTONE,
				/* Blocks.WATER */BlockRegistry.SWAMP_WATER, LAYER_HEIGHT);
	}
	...
	public BlockPos getRandomizedSpawnPoint() {
		...
		if (!isAdventure) {
			spawnPos = spawnPos.add(world.rand.nextInt(spawnFuzz) - spawnFuzzHalf, 0,
	...
	public void updateClientLightTable(EntityPlayer player) {
		...
		for (int i = 0; i <= 15; i++) {
			float f1 = 1F - (float) Math.pow(i / 15F, 1.1F + 0.35F * (1.0F - configBrightness));
			surfaceTable[i] = Math.max((1.0F - f1) / (f1 * f1 * (0.75F + configBrightness * 0.6F) + 1.0F)
					* (0.4F + configBrightness * 0.5F) - 0.1F, 0.0F);
		}

		for (int i = 0; i <= 15; i++) {
			float f1 = 1F - (float) Math.pow(i / 15F, 0.4D);
			undergroundTable[i] = Math.max(
					(1.0F - f1) / (f1 * 2.5F * (1.0F - configBrightness) + 1.0F) * 0.425F - 0.05F,
					-0.035F + 0.035F * configBrightness * configBrightness);
		}
		...
		for (int i = 0; i < 16; i++) {
			this.lightBrightnessTable[i] = surfaceTable[i]
					+ (undergroundTable[i] - surfaceTable[i]) * (1.0F - caveMultiplier);
		}
	}
