...
public class EventSpoopy extends SeasonalEnvironmentEvent {
	...
	private static final long SPOOPY_DATE = new GregorianCalendar(Calendar.getInstance().get(Calendar.YEAR), 9, 23, 0,
			0).getTime().getTime();
	...
	public static void onClientTick(ClientTickEvent event) {
		...
		if (world != null && world.provider instanceof WorldProviderBetweenlands) {
			updateModelActiveState(
					((WorldProviderBetweenlands) world.provider).getEnvironmentEventRegistry().spoopy.isActive());
		} else {
