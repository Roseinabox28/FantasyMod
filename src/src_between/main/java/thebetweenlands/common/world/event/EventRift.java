...
public class EventRift extends TimedEnvironmentEvent {
	public static class RiftConfiguration {
		...
		public RiftConfiguration(int riftSeed, float yawComponent, float pitchComponent, float rollComponent,
				float scaleComponent, boolean mirrorU, boolean mirrorV) {
	...
	protected static final DataParameter<RiftConfiguration> RIFT_CONFIGURATION = GenericDataManager
			.createKey(EventRift.class, RiftConfiguration::write, RiftConfiguration::new);

	protected static final DataParameter<Integer> ACTIVATION_TICKS = GenericDataManager.createKey(EventRift.class,
	...
	public void setActive(boolean active) {
		if (!this.getWorld().isRemote && active && !this.isActive() && this.getActivationTicks() == 0) {
			...
			if (this.playRiftOpenSound) {
				TheBetweenlands.networkWrapper.sendToDimension(new MessageRiftSound(RiftSoundType.OPEN),
						BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId);
			}
		}
	...
	protected void setRandomConfiguration() {
		this.dataManager.set(RIFT_CONFIGURATION,
				new RiftConfiguration(this.getWorld().rand.nextInt(Integer.MAX_VALUE), this.getWorld().rand.nextFloat(),
						this.getWorld().rand.nextFloat(), this.getWorld().rand.nextFloat(),
						this.getWorld().rand.nextFloat(), this.getWorld().rand.nextBoolean(),
						this.getWorld().rand.nextBoolean()));
	}
	...
	public void update(World world) {
		...
		if (!this.getWorld().isRemote) {
			...
			if ((!this.isActive() && remainingTicks < 1800 && remainingTicks > 80)
					|| (this.isActive() && remainingTicks < 1800 && remainingTicks > 80)) {
				if (this.soundTicks-- <= 0) {
					TheBetweenlands.networkWrapper.sendToDimension(new MessageRiftSound(RiftSoundType.CREAK),
							BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId);
	...
	public float getVisibility(float partialTicks) {
		return (this.lastActivationTicks + (this.getActivationTicks() - this.lastActivationTicks) * partialTicks)
				/ (float) MAX_ACTIVATION_TICKS;
	}
	...
	public float getRiftScale(float partialTicks) {
		...
		return variant.getMinScale()
				+ this.getRiftConfiguration().scaleComponent * (variant.getMaxScale() - variant.getMinScale());
	}
