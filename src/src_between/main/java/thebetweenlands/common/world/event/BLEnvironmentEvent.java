...
public abstract class BLEnvironmentEvent
		implements IEnvironmentEvent, IRemotelyControllableEnvironmentEvent, IDataManagedObject {
	private final BLEnvironmentEventRegistry registry;
	private final World world;
	private NBTTagCompound nbtt = new NBTTagCompound();
	private boolean loaded = false;

	protected static final DataParameter<Boolean> ACTIVE = GenericDataManager.createKey(BLEnvironmentEvent.class,
	...
	public void setActive(boolean active) {
		...
		if (active)
			for (EntityPlayerMP player : getWorld().getPlayers(EntityPlayerMP.class,
					player -> player.dimension == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId))
				AdvancementCriterionRegistry.EVENT.trigger(player, getEventName());
	}

	@Override
	public void update(World world) {
		if (!world.isRemote && !this.isStateFromRemoteOverridden
				&& (!EnvironmentEventOverridesHandler.isRemoteDataAvailable() || this.hasNoRemoteState)) {
