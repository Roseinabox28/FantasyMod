...
public class EventThunderstorm extends TimedEnvironmentEvent {
	...
	public void update(World world) {
		...
		if (!world.isRemote) {
			...
			if (this.isActive() && world.provider instanceof WorldProviderBetweenlands
				...
				for (Iterator<Chunk> iterator = worldServer.getPersistentChunkIterable(
					...
					if (world.provider.canDoLightning(chunk) && world.rand.nextInt(30000) == 0) {
						...
						BlockPos pos = this.adjustPosToNearbyEntity(worldServer,
								new BlockPos(chunk.x * 16 + (l & 15), 0, chunk.z * 16 + (l >> 8 & 15)));
						if (world.isRainingAt(pos)) {
							world.addWeatherEffect(new EntityLightningBolt(world, (double) pos.getX(),
									(double) pos.getY(), (double) pos.getZ(), false));
						}
					}
				}
			}
	...
	protected BlockPos adjustPosToNearbyEntity(WorldServer world, BlockPos pos) {
		...
		AxisAlignedBB aabb = (new AxisAlignedBB(blockpos,
				new BlockPos(blockpos.getX(), world.getHeight(), blockpos.getZ()))).grow(3.0D);
