...
public class EventSnowfall extends TimedEnvironmentEvent {
	...
	protected static final DataParameter<Float> TARGET_SNOWING_STRENGTH = GenericDataManager
			.createKey(EventSnowfall.class, DataSerializers.FLOAT);
				...
				for (Iterator<Chunk> iterator = worldServer.getPersistentChunkIterable(
					...
					if (world.rand.nextInt(Math.max(20 - (int) (this.getSnowingStrength() / 8.0F * 18.0F), 2)) == 0) {
						...
						if (stateAbove.getBlock() == Blocks.AIR
								&& BlockRegistry.SNOW.canPlaceBlockAt(world, pos.up())) {
							...
							if (layers < 5) {
								...
								for (EnumFacing dir : EnumFacing.HORIZONTALS) {
									checkPos.setPos(pos.getX() + dir.getXOffset(), pos.getY() + 1,
									...
									if (world.isBlockLoaded(checkPos)) {
										...
										if (BlockRegistry.SNOW.canPlaceBlockAt(world, checkPos)
												&& (neighourState.getBlock() != BlockRegistry.SNOW || neighourState
														.getValue(BlockSnowBetweenlands.LAYERS) < layers)) {
								...
								if (hasEnoughSnowAround) {
									world.setBlockState(pos.up(),
											stateAbove.withProperty(BlockSnowBetweenlands.LAYERS, layers + 1));
								}
							}
						}
					}
