...
public abstract class SeasonalEnvironmentEvent extends BLEnvironmentEvent {
	...
	public long getDayDiffFromStartDate() {
		return TimeUnit.DAYS.convert(Calendar.getInstance().getTime().getTime() - this.getStartDateInMs(),
				TimeUnit.MILLISECONDS);
	}

	@Override
	public void setActive(boolean active) {
		if (active && TheBetweenlands.proxy.getClientWorld() != null
				&& (!this.isActive() || this.lastWorld != TheBetweenlands.proxy.getClientWorld())
				&& TheBetweenlands.proxy.getClientPlayer() != null && this.world != null && this.world.isRemote) {
		...
		if (!world.isRemote) {
			if (BetweenlandsConfig.WORLD_AND_DIMENSION.enableSeasonalEvents) {
				...
				if (dayDiff >= 0 && dayDiff <= this.getDurationInDays()
						&& BetweenlandsConfig.WORLD_AND_DIMENSION.enableSeasonalEvents) {
