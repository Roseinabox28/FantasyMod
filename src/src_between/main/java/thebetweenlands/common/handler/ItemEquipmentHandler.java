					...
					if (player.world.isRemote) {
						...
						if (result.isEmpty() || result.getCount() != heldItem.getCount()) {
							...
							player.sendStatusMessage(
									new TextComponentTranslation("chat.equipment.equipped",
											new TextComponentTranslation(heldItem.getTranslationKey() + ".name")),
									true);
				...
				for (EnumEquipmentInventory type : EnumEquipmentInventory.VALUES) {
					...
					for (int i = 0; i < inv.getSizeInventory(); i++) {
						...
						if (!stack.isEmpty()) {
							...
							EntityItem equipmentDrop = new EntityItem(entity.world, entity.posX,
									entity.posY + entity.getEyeHeight(), entity.posZ, stack.copy());
