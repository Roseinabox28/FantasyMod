	...
	public static void onBlockBreak(BlockEvent.BreakEvent event) {
		...
		if (player != null && !event.getWorld().isRemote) {
			...
			if (!player.isCreative()) {
				...
				if (blockState.getBlock() == BlockRegistry.WEEDWOOD
						|| blockState.getBlock() == BlockRegistry.LOG_WEEDWOOD) {
					...
					for (LocationStorage location : locations) {
						if (location.getType() == EnumLocationType.GIANT_TREE) {
							List<EntityPyrad> pyrads = event.getWorld().getEntitiesWithinAABB(EntityPyrad.class,
									location.getEnclosingBounds(), pyrad -> location.isInside(pyrad));
