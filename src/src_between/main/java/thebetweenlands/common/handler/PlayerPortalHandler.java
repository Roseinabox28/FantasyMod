							...
							if (cap.getTicksUntilTeleport() <= 0 || player.capabilities.isCreativeMode) {
								if (player.world instanceof WorldServer) {
									...
									if (targetDim != player.dimension) {
										WorldServer otherDim = ((WorldServer) player.world).getMinecraftServer()
												.getWorld(targetDim);
										if (otherDim != null) {
						...
						if (timer < MAX_PORTAL_TIME) {
							...
							if (timer == MAX_PORTAL_TIME - 1) {
								Minecraft.getMinecraft().getSoundHandler().playSound(
										new PortalSound(SoundRegistry.PORTAL_TRIGGER, SoundCategory.BLOCKS, player));
							}
