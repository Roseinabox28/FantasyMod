...
public class BossHandler<T extends Entity & IBLBoss> {
	...
	private static final ResourceLocation BOSS_BAR_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/gui/boss_health_bar.png");
	private static final ResourceLocation MINIBOSS_BAR_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/gui/miniboss_health_bar.png");
	...
	public static void onBossBarRender(RenderGameOverlayEvent.BossInfo event) {
		...
		if (boss != null) {
			...
			if (boss.getBossType() == BossType.NORMAL_BOSS) {
				...
				double renderHealth = (renderWidth - 16.0F / texWidth * renderWidth
						- (renderWidth - 16.0F / texWidth * renderWidth) * percent);
				...
				GlStateManager.translate(event.getResolution().getScaledWidth() / 2 - renderWidth / 2.0D,
						event.getY() - 2, 0);
				...
				TheBetweenlands.proxy.getCustomFontRenderer().drawString(name.getFormattedText(),
						event.getResolution().getScaledWidth() / 2 - strWidth / 2, event.getY() + 1, 0xFFFFFFFF);
			}
		}
	}
	...
	public static void onRenderLiving(RenderLivingEvent.Specials.Post<EntityLivingBase> event) {
		...
		if (entity instanceof IBLBoss) {
			...
			if (boss.getBossType() == BossType.MINI_BOSS) {
				BossInfo info = Minecraft.getMinecraft().ingameGUI.getBossOverlay().mapBossInfos
						.get(boss.getBossInfoUuid());
				if (info != null) {
					...
					/*
					 * boolean useStencil = false; int stencilBit =
					 * MinecraftForgeClient.reserveStencilBit(); int stencilMask = 1 << stencilBit;
					 * 
					 * if(stencilBit >= 0) { useStencil = fbo.isStencilEnabled() ? true :
					 * fbo.enableStencil(); }
					 */
					...
					try (Stencil stencil = Stencil.reserve(fbo)) {
						if (stencil.valid()) {
							...
							renderTagQuad(tessellator, -width, -height - (height - 0.2D) * emptyPercentage, width,
									height - height * emptyPercentage, 0, 0.5D, 0.5D, 1);
	...
	@SideOnly(Side.CLIENT)
	private static void renderTag(Tessellator tessellator, Framebuffer fbo, TextureManager textureManager, double width,
			double height, double emptyPercentage, Stencil stencil) {
		GL11.glDisable(GL11.GL_STENCIL_TEST);
	...
	@SideOnly(Side.CLIENT)
	private static void renderTagQuad(Tessellator tessellator, double minX, double minY, double maxX, double maxY,
