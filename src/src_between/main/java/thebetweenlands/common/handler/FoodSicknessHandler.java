...
public class FoodSicknessHandler {
	...
	public static boolean isFoodSicknessEnabled(World world) {
		if (GameruleRegistry.getGameRuleBooleanValue(GameruleRegistry.BL_FOOD_SICKNESS)) {
			if (world.provider.getDimension() == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId
					&& BetweenlandsConfig.GENERAL.useFoodSicknessInBetweenlands) {
				return true;
			} else if (world.provider.getDimension() != BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId
					&& BetweenlandsConfig.GENERAL.useFoodSicknessOutsideBetweenlands) {
				return true;
			}
		}
	...
	public static void onClientTick(ClientTickEvent event) {
		...
		if (player != null) {
			if (!lastUsedItem.isEmpty() && (player.getHeldItem(lastHand).isEmpty()
					|| !player.getHeldItem(lastHand).isItemEqual(lastUsedItem))) {
	...
	protected static void addSicknessMessage(EntityPlayer player, ItemStack item, FoodSickness sickness) {
		if (lastUsedItem.isEmpty() || !item.isItemEqual(lastUsedItem) || lastSickness == null
				|| lastSickness != sickness) {
			player.sendStatusMessage(new TextComponentString(
					String.format(sickness.getRandomLine(player.getRNG()), item.getDisplayName())), true);
		}
		lastUsedItem = item;
		lastSickness = sickness;
	}
	...
	public static void onStartItemUse(LivingEntityUseItemEvent.Start event) {
		...
		if (player != null && !itemStack.isEmpty()
				&& FoodSicknessHandler.isFoodSicknessEnabled(event.getEntity().getEntityWorld())
				&& itemStack.getItem() instanceof IFoodSicknessItem
				&& ((IFoodSicknessItem) itemStack.getItem()).canGetSickOf(player, itemStack)) {
				...
				if (cap != null) {
					...
					if (currentSickness == FoodSickness.SICK) {
						if (item instanceof ItemFood) {
							...
							if (player.world.isRemote) {
								...
								player.getFoodStats()
										.addStats(-Math.min(MathHelper.ceil(foodLevel * foodLoss), foodLevel), 0.0F);
							} else {
								int minFoodGain = player.world.rand.nextInt(4) == 0 ? 1 : 0;
								player.getFoodStats().addStats(-Math.min(MathHelper.ceil(foodLevel * foodLoss),
										Math.max(foodLevel - minFoodGain, 0)), 0.0F);
							}
						}
						...
						if (itemStack.getItem() instanceof IDecayFood) {
							...
							if (decayCap != null) {
								...
								if (player.world.isRemote) {
									...
									decayStats.addStats(-Math.min(MathHelper.ceil(decayLevel * decayLoss), decayLevel),
											0.0F);
								} else {
									int minDecayGain = player.world.rand.nextInt(4) == 0 ? 1 : 0;
									decayStats.addStats(-Math.min(MathHelper.ceil(decayLevel * decayLoss),
											Math.max(decayLevel - minDecayGain, 0)), 0.0F);
								}
							}
						}
					...
					if (!player.world.isRemote && player instanceof EntityPlayerMP) {
						if (newSickness != lastSickness) {
							TheBetweenlands.networkWrapper.sendTo(
									new MessageShowFoodSicknessLine(itemStack, newSickness), (EntityPlayerMP) player);
						}
					}
