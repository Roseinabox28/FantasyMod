...
public class AdvancementHandler {
	...
	public static void onPlayerRightClick(PlayerInteractEvent.RightClickBlock event) {
		if (event.getSide() == Side.SERVER && event.getEntityPlayer() instanceof EntityPlayerMP) {
			...
			AdvancementCriterionRegistry.CLICK_BLOCK.trigger(stack, (EntityPlayerMP) event.getEntityPlayer(),
					event.getPos(), state, event.getFace());
		}
	}
		...
		if (server != null) {
			...
			if (manager != null) {
				...
				for (EntityPlayerMP entityPlayerMP : manager.getPlayers()) {
					if (entityPlayerMP.openContainer == container && container.canInteractWith(entityPlayerMP)
							&& container.getCanCraft(entityPlayerMP)) {
