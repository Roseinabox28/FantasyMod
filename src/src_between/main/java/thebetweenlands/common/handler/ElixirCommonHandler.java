...
public class ElixirCommonHandler {
	...
	// This can be used to stop the entities from attacking if the player is out of
	// "sight"
	 ...
	 * if(event.phase == TickEvent.Phase.START) { for(Entity e :
	 * event.world.loadedEntityList) { if(e instanceof EntityLivingBase) {
	 * EntityLivingBase living = (EntityLivingBase)e; if(living.getRevengeTarget()
	 * != null && !ElixirEffectRegistry.EFFECT_MASKING.canEntityBeSeenBy(living.
	 * getRevengeTarget(), living)) { living.setRevengeTarget(null); } if(living
	 * instanceof EntityCreature) { EntityCreature creature =
	 * (EntityCreature)living; if(creature.getAttackTarget() instanceof
	 * EntityLivingBase) {
	 * if(!ElixirEffectRegistry.EFFECT_MASKING.canEntityBeSeenBy(creature.
	 * getAttackTarget(), creature)) { creature.setAttackTarget(null); } } }
	 * if(living instanceof EntityLiving) { EntityLiving entityLiving =
	 * (EntityLiving)living; if(entityLiving.getAttackTarget() != null &&
	 * !ElixirEffectRegistry.EFFECT_MASKING.canEntityBeSeenBy(entityLiving.
	 * getAttackTarget(), entityLiving)) { entityLiving.setAttackTarget(null); } } }
	 * } } }
	 */
	...
	public void onSetAttackTarget(LivingSetAttackTargetEvent event) {
		if (!this.ignoreSetAttackTarget) {
			if (event.getEntityLiving() instanceof EntityLiving
					&& (event.getTarget() != null && !ElixirEffectRegistry.EFFECT_MASKING
							.canEntityBeSeenBy(event.getTarget(), event.getEntityLiving()))) {
	...
	public void onBreakSpeed(PlayerEvent.BreakSpeed event) {
		...
		if (player != null) {
			if (ElixirEffectRegistry.EFFECT_SWIFTARM.isActive(player)
					&& ElixirEffectRegistry.EFFECT_SWIFTARM.getStrength(player) >= 0) {
				event.setNewSpeed(event.getNewSpeed()
						* (1.0F + (ElixirEffectRegistry.EFFECT_SWIFTARM.getStrength(player) + 1) * 0.3F));
			}
			if (ElixirEffectRegistry.EFFECT_SLUGARM.isActive(player)
					&& ElixirEffectRegistry.EFFECT_SLUGARM.getStrength(player) >= 0) {
				event.setNewSpeed(event.getNewSpeed()
						/ (1.0F + (ElixirEffectRegistry.EFFECT_SLUGARM.getStrength(player) + 1) * 0.3F));
			}
		}
	}
	...
	public void onStartUseItem(LivingEntityUseItemEvent.Start event) {
		...
		if (living != null) {
			if (ElixirEffectRegistry.EFFECT_SWIFTARM.isActive(living)
					&& ElixirEffectRegistry.EFFECT_SWIFTARM.getStrength(living) >= 0) {
			...
			if (ElixirEffectRegistry.EFFECT_SLUGARM.isActive(living)
					&& ElixirEffectRegistry.EFFECT_SLUGARM.getStrength(living) >= 0) {
	...
	public void onShootArrow(ArrowLooseEvent event) {
		if (ElixirEffectRegistry.EFFECT_WEAKBOW.isActive(event.getEntityLiving())) {
			...
			event.setCharge((int) (event.getCharge() * (1.0F
					- (ElixirEffectRegistry.EFFECT_WEAKBOW.getStrength(event.getEntityLiving()) + 1) / 4.0F * 0.75F)));
		}
	}

	private static final AttributeModifier FOLLOW_RANGE_MODIFIER = new AttributeModifier(
			"24ce3c60-ae87-4e31-8fc3-bf3f40ab37ca", 10, 0);
	...
	public void onEntityUpdate(LivingEvent.LivingUpdateEvent event) {
		...
		if (ElixirEffectRegistry.EFFECT_SPIDERBREED.isActive(entityLivingBase)) {
			...
			if ((!entityLivingBase.onGround || this.isEntityOnWall(entityLivingBase))
					&& (entityLivingBase.collidedHorizontally || entityLivingBase.collidedVertically)) {
			...
			if (!entityLivingBase.onGround && this.isEntityOnWall(entityLivingBase)) {
				if (entityLivingBase.motionY < 0.0F && (lookVec.y > 0.0F
						|| (entityLivingBase.moveForward == 0.0F && entityLivingBase.moveStrafing == 0.0F))) {
		...
		if (ElixirEffectRegistry.EFFECT_LIGHTWEIGHT.isActive(entityLivingBase) && !entityLivingBase.isInWater()
				&& !entityLivingBase.isSneaking()) {
			IBlockState state = entityLivingBase.world.getBlockState(new BlockPos(entityLivingBase.posX,
					entityLivingBase.getEntityBoundingBox().minY + Math.min(-0.1D, entityLivingBase.motionY),
					entityLivingBase.posZ));
			if (state.getMaterial().isLiquid()) {
				float relStrength = Math
						.min((ElixirEffectRegistry.EFFECT_LIGHTWEIGHT.getStrength(entityLivingBase)) / 4.0F, 1.0F);
		...
		if (ElixirEffectRegistry.EFFECT_HEAVYWEIGHT.isActive(entityLivingBase) && entityLivingBase.isInWater()) {
			if (entityLivingBase.motionY > -0.1F) {
				entityLivingBase.motionY -= 0.005F
						+ 0.035F / 5.0F * (1 + ElixirEffectRegistry.EFFECT_HEAVYWEIGHT.getStrength(entityLivingBase));
			}
		}

		if (!entityLivingBase.world.isRemote) {
			if (ElixirEffectRegistry.EFFECT_CATSEYES.isActive(entityLivingBase)) {
				entityLivingBase.addPotionEffect(new PotionEffect(MobEffects.NIGHT_VISION,
						ElixirEffectRegistry.EFFECT_CATSEYES.getDuration(entityLivingBase),
						ElixirEffectRegistry.EFFECT_CATSEYES.getStrength(entityLivingBase)));
			...
			if (ElixirEffectRegistry.EFFECT_POISONSTING.isActive(entityLivingBase)) {
				entityLivingBase.addPotionEffect(new PotionEffect(MobEffects.POISON,
						ElixirEffectRegistry.EFFECT_POISONSTING.getDuration(entityLivingBase),
						ElixirEffectRegistry.EFFECT_POISONSTING.getStrength(entityLivingBase)));
			...
			if (ElixirEffectRegistry.EFFECT_DRUNKARD.isActive(entityLivingBase)) {
				entityLivingBase.addPotionEffect(new PotionEffect(MobEffects.NAUSEA,
						ElixirEffectRegistry.EFFECT_DRUNKARD.getDuration(entityLivingBase),
						ElixirEffectRegistry.EFFECT_DRUNKARD.getStrength(entityLivingBase)));
			...
			if (ElixirEffectRegistry.EFFECT_BLINDMAN.isActive(entityLivingBase)) {
				entityLivingBase.addPotionEffect(new PotionEffect(MobEffects.BLINDNESS,
						ElixirEffectRegistry.EFFECT_BLINDMAN.getDuration(entityLivingBase),
						ElixirEffectRegistry.EFFECT_BLINDMAN.getStrength(entityLivingBase)));
			...
			if (!(entityLivingBase instanceof EntityPlayer) && entityLivingBase instanceof EntityMob
				...
				IAttributeInstance followRangeAttrib = entityLiving
				...
				if (followRangeAttrib != null) {
					...
					if (stenchingPlayers.isEmpty()) {
						...
						AttributeModifier currentModifier = followRangeAttrib
								.getModifier(FOLLOW_RANGE_MODIFIER.getID());
						if (entityLiving.getAttackTarget() == null || entityLiving.getRevengeTarget() == null) {
							EntityPlayer closestPlayer = null;
							for (EntityPlayer player : stenchingPlayers) {
								if (closestPlayer == null || player.getDistanceSq(entityLiving) < closestPlayer
										.getDistanceSq(entityLiving))
							...
							boolean shouldApplyModifier = currentModifier == null
									|| currentModifier.getAmount() < stenchModifier.getAmount();
	...
	private AttributeModifier getFollowRangeModifier(int strength) {
		return new AttributeModifier(FOLLOW_RANGE_MODIFIER.getID(), FOLLOW_RANGE_MODIFIER.getName() + " " + strength,
				FOLLOW_RANGE_MODIFIER.getAmount() / 4.0D * (Math.min(strength, 4)),
				FOLLOW_RANGE_MODIFIER.getOperation());
	}
	...
	private double getSpottingRange(EntityLivingBase entity, int strength) {
		...
		double spottingRange = followRangeAttrib == null
				? (16.0D + FOLLOW_RANGE_MODIFIER.getAmount() / 4.0D * Math.min(strength, 4))
				: followRangeAttrib.getAttributeValue();
