...
public class PlayerJoinWorldHandler {
	...
	public static void onEntityJoin(EntityJoinWorldEvent event) {
		if (!event.getWorld().isRemote && event.getEntity() instanceof EntityPlayer
				&& BetweenlandsConfig.WORLD_AND_DIMENSION.activateRiftOnFirstJoin
				&& event.getWorld().provider.getDimension() == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId) {
			...
			boolean isFirstTimeInDimension = !(persistentNbt.hasKey(NOT_FIRST_JOIN_NBT, Constants.NBT.TAG_BYTE)
					&& persistentNbt.getBoolean(NOT_FIRST_JOIN_NBT));
	...
	public static void onPlayerLogin(PlayerLoggedInEvent event) {
		if (!event.player.world.isRemote && BetweenlandsConfig.WORLD_AND_DIMENSION.startInBetweenlands
				&& event.player.world.provider.getDimension() != BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId
				&& event.player.world instanceof WorldServer) {
			NBTTagCompound dataNbt = event.player.getEntityData();
			NBTTagCompound persistentNbt = dataNbt.getCompoundTag(EntityPlayer.PERSISTED_NBT_TAG);

			boolean isFirstTimeSpawning = !(persistentNbt.hasKey(NOT_FIRST_SPAWN_NBT, Constants.NBT.TAG_BYTE)
					&& persistentNbt.getBoolean(NOT_FIRST_SPAWN_NBT));
			...
			if (isFirstTimeSpawning) {
				...
				TeleporterHandler.transferToDim(event.player, blWorld,
						BetweenlandsConfig.WORLD_AND_DIMENSION.startInPortal, true);
			}
		}
	}
}
