...
public class AttackDamageHandler {
	...
	public static void onEntityKnockback(LivingKnockBackEvent event) {
		...
		if (attackedEntity instanceof IEntityBL && attacker instanceof EntityLivingBase
				&& ((EntityLivingBase) attacker).getActiveHand() != null) {
	...
	public static void onEntityAttacked(LivingHurtEvent event) {
		...
		if (attackedEntity instanceof IEntityBL && entity instanceof EntityLivingBase
				&& ((EntityLivingBase) entity).getActiveHand() != null) {
				...
				if (OverworldItemHandler.isToolWeakened(heldItem)) {
					...
					if (!attackedEntity.world.isRemote) {
						...
						if (immediateAttacker == null || attacker == immediateAttacker) {
							RayTraceResult result = attackedEntity.getEntityBoundingBox().calculateIntercept(
									attacker.getPositionEyes(1),
									attacker.getPositionEyes(1).add(attacker.getLookVec().scale(10)));
							if (result != null) {
								hitOffset = result.hitVec.subtract(center);
							}
						}
						if (immediateAttacker != null && hitOffset == null) {
							hitOffset = immediateAttacker.getPositionVector().add(0, immediateAttacker.height / 2.0F, 0)
									.subtract(center);
						}
						if (hitOffset != null) {
							Vec3d offsetDirXZ = new Vec3d(hitOffset.x, 0, hitOffset.z).normalize();
							Vec3d offset = offsetDirXZ.scale(attackedEntity.width).add(0,
									hitOffset.y + attackedEntity.height / 2.0F, 0);

							attackedEntity.world.playSound(null, attackedEntity.posX, attackedEntity.posY + 0.5D,
									attackedEntity.posZ, SoundRegistry.DAMAGE_REDUCTION, SoundCategory.PLAYERS, 0.7F,
									0.75F + attackedEntity.world.rand.nextFloat() * 0.3F);

							TheBetweenlands.networkWrapper.sendToAllAround(
									new MessageDamageReductionParticle(attackedEntity, offset,
											offsetDirXZ.scale(attackedEntity.width + 0.2F).normalize()),
									new TargetPoint(attackedEntity.dimension, attackedEntity.posX, attackedEntity.posY,
											attackedEntity.posZ, 32.0D));
						}
					}
				}
			}
		...
		if (entity instanceof EntityLivingBase) {
			...
			if (cap != null) {
				...
				if (rings > 0) {
					TheBetweenlands.networkWrapper.sendToAllAround(new MessagePowerRingParticles(attackedEntity),
							new TargetPoint(attackedEntity.dimension, attackedEntity.posX, attackedEntity.posY,
									attackedEntity.posZ, 32.0D));
				}
	...
	public static void spawnDamageReductionParticle(Entity entity, Vec3d offset, Vec3d dir) {
		BLParticles.DAMAGE_REDUCTION.spawn(entity.world, 0, 0, 0,
				ParticleArgs.get().withScale(2F).withData(entity, offset, dir));
	}

	@SideOnly(Side.CLIENT)
	public static void spawnPowerRingParticles(Entity entityHit) {
		BLParticles.GREEN_FLAME.spawn(entityHit.world, entityHit.posX + entityHit.width / 2.0D,
				entityHit.posY + entityHit.height / 2.0D + 0.5D, entityHit.posZ,
				ParticleArgs.get().withMotion(0.08D, 0.05D, 0));
		BLParticles.GREEN_FLAME.spawn(entityHit.world, entityHit.posX, entityHit.posY + entityHit.height / 2.0D + 0.5D,
				entityHit.posZ + entityHit.width / 2.0D, ParticleArgs.get().withMotion(0, 0.05D, 0.08D));
		BLParticles.GREEN_FLAME.spawn(entityHit.world, entityHit.posX - entityHit.width / 2.0D,
				entityHit.posY + entityHit.height / 2.0D + 0.5D, entityHit.posZ,
				ParticleArgs.get().withMotion(-0.08D, 0.05D, 0));
		BLParticles.GREEN_FLAME.spawn(entityHit.world, entityHit.posX, entityHit.posY + entityHit.height / 2.0D + 0.5D,
