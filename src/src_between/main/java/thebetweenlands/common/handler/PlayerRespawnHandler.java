...
public class PlayerRespawnHandler {
	...
	public static void onPlayerDeath(LivingDeathEvent event) {
		if (event.getEntityLiving() instanceof EntityPlayer) {
			...
			BlockPos adjustedSpawnPos = spawnPos == null ? null
					: EntityPlayer.getBedSpawnLocation(player.world, spawnPos, player.isSpawnForced(player.dimension));

			boolean respawnInBL = BetweenlandsConfig.WORLD_AND_DIMENSION.startInBetweenlands
					&& (!player.world.provider.canRespawnHere() || adjustedSpawnPos == null);
	...
	public static void onRespawn(PlayerRespawnEvent event) {
		if (!event.player.world.isRemote) {
			...
			BlockPos adjustedSpawnPos = spawnPos == null ? null
					: EntityPlayer.getBedSpawnLocation(event.player.world, spawnPos,
							event.player.isSpawnForced(event.player.dimension));

			NBTTagCompound dataNbt = event.player.getEntityData();
			NBTTagCompound persistentNbt = dataNbt.getCompoundTag(EntityPlayer.PERSISTED_NBT_TAG);

			boolean shouldTeleportToBL = (BetweenlandsConfig.WORLD_AND_DIMENSION.startInBetweenlands
					&& event.isEndConquered()) || persistentNbt.getBoolean(RESPAWN_IN_BL_NBT);

			if (shouldTeleportToBL && event.player.world instanceof WorldServer) {
				WorldServer blWorld = ((WorldServer) event.player.world).getMinecraftServer()
						.getWorld(BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId);
				...
				adjustedSpawnPos = spawnPos == null ? null
						: EntityPlayer.getBedSpawnLocation(event.player.world, spawnPos,
								event.player.isSpawnForced(event.player.dimension));

				if (adjustedSpawnPos != null) {
					event.player.setPosition(adjustedSpawnPos.getX() + 0.5D, adjustedSpawnPos.getY(),
							adjustedSpawnPos.getZ() + 0.5D);

					if (event.player instanceof EntityPlayerMP) {
						EntityPlayerMP playerMP = (EntityPlayerMP) event.player;

						playerMP.connection.setPlayerLocation(playerMP.posX, playerMP.posY, playerMP.posZ,
			...
			if (shouldTeleportToBL || event.player.dimension == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId) {
				...
				if (adjustedSpawnPos == null) {
					...
					boolean isValidSpawnBlock = stateDown.getMaterial().blocksMovement()
							&& !stateDown.getBlock().isLeaves(stateDown, event.player.world, adjustedSpawnPos.down())
							&& !stateDown.getBlock().isFoliage(event.player.world, adjustedSpawnPos.down());
				...
				if (newRespawn) {
					if (persistentNbt.hasKey(TeleporterBetweenlands.LAST_PORTAL_POS_NBT, Constants.NBT.TAG_LONG)) {
						BlockPos lastPortal = BlockPos
								.fromLong(persistentNbt.getLong(TeleporterBetweenlands.LAST_PORTAL_POS_NBT));
	...
	public static void respawnNearPos(Entity entity, BlockPos pos) {
		...
		entity.setLocationAndAngles(newSpawn.getX() + 0.5D, newSpawn.getY(), newSpawn.getZ() + 0.5D, entity.rotationYaw,
				entity.rotationPitch);

		while (!entity.world.getCollisionBoxes(entity, entity.getEntityBoundingBox()).isEmpty()
				&& entity.posY < 256.0D) {
		...
		if (entity instanceof EntityPlayerMP) {
			...
			playerMP.connection.setPlayerLocation(playerMP.posX, playerMP.posY, playerMP.posZ, playerMP.rotationYaw,
	...
	public static BlockPos getRespawnPointNearPos(World world, BlockPos pos, int fuzz) {
		...
		if (result == null) {
			...
			result = world.getTopSolidOrLiquidBlock(pos.add(world.rand.nextInt(spawnFuzz) - spawnFuzzHalf, 0,
					world.rand.nextInt(spawnFuzz) - spawnFuzzHalf));
		}
		return result;
	}

	@Nullable
	public static BlockPos getSpawnPointNearPos(World world, BlockPos pos, int fuzz, boolean surface, int yRange,
			int xzSkip) {
		...
		for (int xo = -spawnFuzzHalf; xo <= spawnFuzzHalf; xo += 1 + world.rand.nextInt(xzSkip)) {
			for (int zo = -spawnFuzzHalf; zo <= spawnFuzzHalf; zo += 1 + world.rand.nextInt(xzSkip)) {
				...
				for (int yo = yRange; surface || yo >= -yRange; yo--) {
					...
					if (Math.abs(checkPos.getY() - pos.getY()) <= yRange
							&& EntityPlayer.getBedSpawnLocation(world, checkPos, true) != null) {
						checkPos.setY(checkPos.getY() - 1);

						IBlockState stateDown = chunk.getBlockState(checkPos);
						if (stateDown.getMaterial().blocksMovement()
								&& !stateDown.getBlock().isLeaves(stateDown, world, checkPos)
								&& !stateDown.getBlock().isFoliage(world, checkPos)) {
