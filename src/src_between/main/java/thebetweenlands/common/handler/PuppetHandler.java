...
public class PuppetHandler {
		...
		if (entity instanceof EntityLiving) {
			...
			if (cap != null && cap.hasPuppeteer()) {
				...
				if (!entity.world.isRemote) {
					...
					if (cap.getRemainingTicks() <= 0 || (cap.getPuppeteer() != null
							&& !ItemRingOfRecruitment.isRingActive(cap.getPuppeteer()))) {
						...
						if (EntityAIPuppet.getPuppetAI(living.targetTasks) == null) {
							/*
							 * EntityAINearestAttackableTarget<EntityLiving> aiTarget = new
							 * EntityAINearestAttackableTarget<EntityLiving>(creature, EntityLiving.class,
							 * 0, true, true, living -> {
							 * if(living.hasCapability(CapabilityRegistry.CAPABILITY_PUPPET, null)) {
							 * IPuppetCapability targetCap =
							 * living.getCapability(CapabilityRegistry.CAPABILITY_PUPPET, null);
							 * if(targetCap.getPuppeteer() == cap.getPuppeteer()) { //Don't attack puppets
							 * from same owner return false; } } return living instanceof EntityMob ||
							 * living instanceof IMob; }); aiTarget.setMutexBits(1); //01
							 */

							if (living instanceof EntityCreature) {
								EntityAIAttackMelee aiMelee = new EntityAIAttackMelee((EntityCreature) living, 1.2D,
										true);
					...
					if (entity.world.rand.nextInt(5) == 0) {
						BLParticles.SPAWNER.spawn(living.world, living.posX + living.motionX * 2,
								living.posY + living.height / 2.0D, living.posZ + living.motionZ * 2,
								ParticleArgs.get()
										.withMotion(
												living.motionX
														+ (living.world.rand.nextFloat() - 0.5F) / 8.0F * entity.width,
	...
	public static void onEntityAttacked(LivingHurtEvent event) {
		...
		if (!attackedEntity.world.isRemote) {
			...
			if (source.getTrueSource() instanceof EntityPlayer) {
				IPuppeteerCapability cap = source.getTrueSource().getCapability(CapabilityRegistry.CAPABILITY_PUPPETEER,
						null);
				if (cap != null) {
	...
	public static void onEntityInteract(EntityInteract event) {
		...
		if (event.getHand() == EnumHand.MAIN_HAND && player != null && target instanceof EntityLiving
				&& ItemRingOfRecruitment.isRingActive(player)) {
			...
			if (cap != null) {
				...
				if (cap.hasPuppeteer()) {
					...
					if (ItemRingOfRecruitment.isRingActive(player)) {
						IPuppeteerCapability capPlayer = player.getCapability(CapabilityRegistry.CAPABILITY_PUPPETEER,
								null);
			...
			if (cap != null) {
				...
				if (activatingEntity instanceof EntityLiving) {
					...
					if (!event.player.world.isRemote) {
						if (living.getDistance(event.player) > 5.0D) {
							...
							if (cap.getActivatingTicks() > living.getMaxHealth()) {
								if (ItemRingOfRecruitment.isRingActive(event.player)) {
									IPuppetCapability puppetCap = living
											.getCapability(CapabilityRegistry.CAPABILITY_PUPPET, null);
						...
						Vec3d vec = new Vec3d(living.posX - event.player.posX,
								(living.posY + living.getEyeHeight() * 0.8F)
										- (event.player.posY + event.player.getEyeHeight() * 0.8F),
								living.posZ - event.player.posZ);
						vec = vec.normalize();
						...
						BLParticles.SPAWNER.spawn(event.player.world, event.player.posX,
								event.player.posY + event.player.getEyeHeight() * 0.8F, event.player.posZ,
								ParticleArgs.get().withData(40).withColor(0.2F, 0.8F, 0.4F, 1).withMotion(vec.x, vec.y,
										vec.z));
					}
	...
	public static void onPlayerInteract(PlayerInteractEvent event) {
		if (event.getHand() == EnumHand.MAIN_HAND && event instanceof PlayerInteractEvent.RightClickBlock
				&& ItemRingOfRecruitment.isRingActive(event.getEntityPlayer())) {
			EntityPlayer player = event.getEntityPlayer();
			...
			if (cap != null) {
				...
				if (ordered) {
					...
					if (player.world.isRemote) {
						for (int i = 0; i < 4; i++) {
							BLParticles.SPAWNER.spawn(player.world, target.getX() + 0.5D, target.getY() + 0.5D,
									target.getZ() + 0.5D,
									ParticleArgs.get()
											.withMotion((player.world.rand.nextFloat() - 0.5F) / 16.0F,
	...
	public static void onRenderLivingPost(RenderLivingEvent.Post<EntityLivingBase> event) {
		...
		if (cap != null && cap.hasPuppeteer()) {
			...
			if (puppeteer != null) {
				...
				double dx = puppeteer.lastTickPosX
						+ (puppeteer.posX - puppeteer.lastTickPosX) * WorldRenderHandler.getPartialTicks()
						- (living.lastTickPosX
								+ (living.posX - living.lastTickPosX) * WorldRenderHandler.getPartialTicks());
				double dy = -living.height / 2.0F + puppeteer.height / 2.0D + puppeteer.lastTickPosY
						+ (puppeteer.posY - puppeteer.lastTickPosY) * WorldRenderHandler.getPartialTicks()
						- (living.lastTickPosY
								+ (living.posY - living.lastTickPosY) * WorldRenderHandler.getPartialTicks());
				double dz = puppeteer.lastTickPosZ
						+ (puppeteer.posZ - puppeteer.lastTickPosZ) * WorldRenderHandler.getPartialTicks()
						- (living.lastTickPosZ
								+ (living.posZ - living.lastTickPosZ) * WorldRenderHandler.getPartialTicks());
				...
				for (int i = 0; i < iter; i++) {
					...
					RenderSwordEnergy.renderBeam(
							new Vec3d(sx + prevXOffset + dx / iter * i, sy + prevYOffset + dy / iter * i,
									sz + prevZOffset + dz / iter * i),
							new Vec3d(sx + xOffset + dx / iter * (i + 1), sy + yOffset + dy / iter * (i + 1),
									sz + zOffset + dz / iter * (i + 1)),
