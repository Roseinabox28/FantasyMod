...
public class OverworldItemHandler {
	...
	public static interface ITorchPlaceHandler {
		...
		/**
		 * Returns whether the specified item is a torch that should be destroyed or
		 * replaced
		 * 
		 * @param stack
		 * @return
		...
		/**
		 * Called when a torch is placed, by default replaces the block with a damp
		 * torch
		 * 
		 * @param world
		 * @param pos
	...
	static {
		ROTTING_WHITELIST.put(new ResourceLocation(ModInfo.ID, "config_whitelist"),
				stack -> BetweenlandsConfig.GENERAL.rottenFoodWhitelist.isListed(stack));
		ROTTING_BLACKLIST.put(new ResourceLocation(ModInfo.ID, "config_blacklist"),
				stack -> BetweenlandsConfig.GENERAL.rottenFoodBlacklist.isListed(stack));
		...
		TAINTING_WHITELIST.put(new ResourceLocation(ModInfo.ID, "config_whitelist"),
				stack -> BetweenlandsConfig.GENERAL.taintingWhitelist.isListed(stack));
		TAINTING_BLACKLIST.put(new ResourceLocation(ModInfo.ID, "config_blacklist"),
				stack -> BetweenlandsConfig.GENERAL.taintingBlacklist.isListed(stack));
		TAINTING_BLACKLIST.put(new ResourceLocation(ModInfo.ID, "default_blacklist"),
				stack -> stack.getItem() instanceof ItemPotion);

		FIRE_TOOL_WHITELIST.put(new ResourceLocation(ModInfo.ID, "config_whitelist"),
				stack -> BetweenlandsConfig.GENERAL.fireToolWhitelist.isListed(stack));
		FIRE_TOOL_BLACKLIST.put(new ResourceLocation(ModInfo.ID, "config_blacklist"),
				stack -> BetweenlandsConfig.GENERAL.fireToolBlacklist.isListed(stack));
		FIRE_TOOL_BLACKLIST.put(new ResourceLocation(ModInfo.ID, "default_blacklist"),
				stack -> stack.getItem() instanceof ItemFlintAndSteel);

		FERTILIZER_WHITELIST.put(new ResourceLocation(ModInfo.ID, "config_whitelist"),
				stack -> BetweenlandsConfig.GENERAL.fertilizerWhitelist.isListed(stack));
		FERTILIZER_BLACKLIST.put(new ResourceLocation(ModInfo.ID, "config_blacklist"),
				stack -> BetweenlandsConfig.GENERAL.fertilizerBlacklist.isListed(stack));
		FERTILIZER_BLACKLIST.put(new ResourceLocation(ModInfo.ID, "default_blacklist"),
				stack -> stack.getItem() == Items.DYE);

		TOOL_WHITELIST.put(new ResourceLocation(ModInfo.ID, "config_whitelist"),
				stack -> BetweenlandsConfig.GENERAL.toolWeaknessWhitelist.isListed(stack));
		TOOL_BLACKLIST.put(new ResourceLocation(ModInfo.ID, "config_blacklist"),
				stack -> BetweenlandsConfig.GENERAL.toolWeaknessBlacklist.isListed(stack));
		TOOL_BLACKLIST.put(new ResourceLocation(ModInfo.ID, "default_blacklist"), stack -> {
			return (stack.getItem() instanceof ItemTool || stack.getItem() instanceof ItemSword
					|| stack.getItem() instanceof ItemBow) && stack.getItem() instanceof ItemBLSword == false
					&& stack.getItem() instanceof ItemBLAxe == false
					&& stack.getItem() instanceof ItemBLPickaxe == false
					&& stack.getItem() instanceof ItemBLShovel == false && stack.getItem() instanceof ItemBLBow == false
					&& !ItemRegistry.ITEMS.contains(stack.getItem());
		});

		TORCH_WHITELIST.put(new ResourceLocation(ModInfo.ID, "config_whitelist"),
				stack -> BetweenlandsConfig.GENERAL.torchWhitelist.isListed(stack));
		TORCH_BLACKLIST.put(new ResourceLocation(ModInfo.ID, "config_blacklist"),
				stack -> BetweenlandsConfig.GENERAL.torchBlacklist.isListed(stack));
		...
		ITorchPlaceHandler vanillaTorchPlaceHandler = new ITorchPlaceHandler() {
			...
			public void onTorchBlockPlaced(World world, BlockPos pos, IBlockState state, ItemStack stack,
				...
				if (facing == null) {
					...
					for (EnumFacing dir : dirs) {
						if (dir != EnumFacing.DOWN) {
							...
							if ((dir == EnumFacing.UP && offsetState.getBlock().canPlaceTorchOnTop(offsetState, world,
									pos.offset(dir.getOpposite())))
									|| world.isSideSolid(pos.offset(dir.getOpposite()), dir)) {
								facing = dir;
								break;
							}
						}
				...
				if (facing != null) {
					IBlockState dampTorch = BlockRegistry.DAMP_TORCH.getDefaultState()
							.withProperty(BlockDampTorch.FACING, facing);
					world.setBlockState(pos, dampTorch);
				} else {
					world.setBlockToAir(pos);
					world.spawnEntity(new EntityItem(world, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D,
							new ItemStack(BlockRegistry.DAMP_TORCH)));
				}
				...
				for (ITorchPlaceHandler handler : TORCH_PLACE_HANDLERS.values()) {
					if (handler.isTorchItem(held)) {
						if (!handler.onTorchItemPlaced(event.getWorld(), event.getPos(), held, event.getPlayer())) {
							handler.onTorchBlockPlaced(event.getWorld(), event.getPos(), event.getState(), held,
									event.getPlayer());
						}
						break;
					}
	...
	public static void onUseItem(PlayerInteractEvent.RightClickBlock event) {
		...
		if (!item.isEmpty()
				&& event.getEntityPlayer().dimension == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId) {
			if (isFireToolBlocked(item)) {
				event.setUseItem(Result.DENY);
				event.setCanceled(true);
				if (event.getWorld().isRemote) {
					event.getEntityPlayer().sendStatusMessage(new TextComponentTranslation("chat.flintandsteel",
							new TextComponentTranslation(item.getTranslationKey() + ".name")), true);
				}
			}
		}
	}
	...
	public static void onBonemeal(BonemealEvent event) {
		if (event.getEntityPlayer().dimension == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId) {
			...
			if (!stack.isEmpty() && isFertilizerBlocked(stack)) {
				...
				event.setCanceled(true);
				if (event.getWorld().isRemote) {
					event.getEntityPlayer().sendStatusMessage(new TextComponentTranslation("chat.fertilizer",
							new TextComponentTranslation(stack.getTranslationKey() + ".name")), true);
				}
			}
		}
	}
	...
	public static void onEntitySpawn(EntityJoinWorldEvent event) {
		if (event.getEntity() instanceof EntityPlayer
				&& !((EntityPlayer) event.getEntity()).capabilities.isCreativeMode) {
	...
	public static void onPlayerTick(PlayerTickEvent event) {
		if (!event.player.world.isRemote && event.player.ticksExisted % 5 == 0
	...
	public static void onItemPickup(EntityItemPickupEvent event) {
		...
		if (player != null && !player.world.isRemote && !player.capabilities.isCreativeMode) {
			...
			if (!stack.isEmpty()) {
				...
				} else if (BetweenlandsConfig.GENERAL.reverseRottenFood
						&& stack.getItem() == ItemRegistry.ROTTEN_FOOD) {
					ItemStack originalStack = ItemRegistry.ROTTEN_FOOD.getOriginalStack(stack);
					if (!originalStack.isEmpty()) {
	...
	public static boolean isRotting(ItemStack stack) {
		if (!BetweenlandsConfig.GENERAL.useRottenFood
				|| !GameruleRegistry.getGameRuleBooleanValue(GameruleRegistry.BL_ROTTEN_FOOD)) {
			return false;
		}
	...
	public static boolean isTainting(ItemStack stack) {
		if (!BetweenlandsConfig.GENERAL.usePotionBlacklist
				|| !GameruleRegistry.getGameRuleBooleanValue(GameruleRegistry.BL_POTION_BLACKLIST)) {
			return false;
		}
	...
	public static boolean isFireToolBlocked(ItemStack stack) {
		if (!BetweenlandsConfig.GENERAL.useFireToolBlacklist
				|| !GameruleRegistry.getGameRuleBooleanValue(GameruleRegistry.BL_FIRE_TOOL_BLACKLIST)) {
			return false;
		}
	...
	public static boolean isFertilizerBlocked(ItemStack stack) {
		if (!BetweenlandsConfig.GENERAL.useFertilizerBlacklist
				|| !GameruleRegistry.getGameRuleBooleanValue(GameruleRegistry.BL_FERTILIZER_BLACKLIST)) {
			return false;
		}
	...
	public static boolean isToolWeakened(ItemStack stack) {
		if (!BetweenlandsConfig.GENERAL.useToolWeakness
				|| !GameruleRegistry.getGameRuleBooleanValue(GameruleRegistry.BL_TOOL_WEAKNESS)) {
			return false;
		}
	...
	public static boolean isTorchTurningDamp(ItemStack stack) {
		if (!BetweenlandsConfig.GENERAL.useTorchBlacklist
				|| !GameruleRegistry.getGameRuleBooleanValue(GameruleRegistry.BL_TORCH_BLACKLIST)) {
			return false;
		}
