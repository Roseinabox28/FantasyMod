...
public class PlayerDecayHandler {
	public static final UUID DECAY_HEALTH_MODIFIER_ATTRIBUTE_UUID = UUID
			.fromString("033f5f10-67b3-42f3-8511-67a575fbb099");

	public static boolean isDecayEnabled() {
		return GameruleRegistry.getGameRuleBooleanValue(GameruleRegistry.BL_DECAY)
			...
			if (cap != null) {
				...
				if (attr != null) {
					if (BetweenlandsConfig.GENERAL.decayPercentual) {
						...
						float prevDecayMaxBaseHealthPercentage = cap
								.getMaxPlayerHealthPercentage(stats.getPrevDecayLevel());

						AttributeModifier currentDecayModifier = attr.getModifier(DECAY_HEALTH_MODIFIER_ATTRIBUTE_UUID);

						if (!MathUtils.epsilonEquals(decayMaxBaseHealthPercentage, prevDecayMaxBaseHealthPercentage)
								|| (currentDecayModifier == null && decayMaxBaseHealthPercentage < 1)) {
							attr.removeModifier(DECAY_HEALTH_MODIFIER_ATTRIBUTE_UUID);

							if (decayMaxBaseHealthPercentage < 1) {
								attr.applyModifier(new AttributeModifier(DECAY_HEALTH_MODIFIER_ATTRIBUTE_UUID,
										"Decay health modifier", -1 + decayMaxBaseHealthPercentage, 2));
							}
						}
					} else {
						...
						int prevDecayMaxBaseHealth = (int) (cap.getMaxPlayerHealth(stats.getPrevDecayLevel()) / 2.0F)
								* 2;
						...
						// Only change modifier if deay modifier is missing, decay health modifier value
						// has changed or if player has less than 3 hearts (in which case decay modifier
						// should be reduced or removed)
						if ((currentMaxHealth > BetweenlandsConfig.GENERAL.decayMinHealth && decayHealthDiff != 0
								&& (currentDecayModifier == null
										|| decayHealthDiff != (int) currentDecayModifier.getAmount()))
								|| decayHealthChange || (currentMaxHealth < BetweenlandsConfig.GENERAL.decayMinHealth
										&& currentDecayModifier != null)) {
							attr.removeModifier(DECAY_HEALTH_MODIFIER_ATTRIBUTE_UUID);
							...
							int newHealth = (int) Math.max(currentMaxHealth + decayHealthDiff,
									BetweenlandsConfig.GENERAL.decayMinHealth);

							int attributeHealth = newHealth - currentMaxHealth;

							if (attributeHealth < 0) {
								attr.applyModifier(new AttributeModifier(DECAY_HEALTH_MODIFIER_ATTRIBUTE_UUID,
										"Decay health modifier", attributeHealth, 0));
				...
				if (cap.isDecayEnabled()) {
					...
					if (!event.player.isRiding()) {
						...
						if (player.distanceWalkedModified - player.prevDistanceWalkedModified > 0) {
							decaySpeed += (player.distanceWalkedModified - player.prevDistanceWalkedModified) * 4
									* decayBaseSpeed;
						}

						BetweenlandsWorldStorage storage = BetweenlandsWorldStorage.forWorld(player.world);
						if (storage.getEnvironmentEventRegistry().heavyRain.isActive()
	...
	public static void onPlayerTick(PlayerRespawnEvent event) {
		// Workaround for client not receiving the new MAX_HEALTH attribute after a
		// respawn
		EntityPlayer player = event.player;
		if (!player.world.isRemote && player instanceof EntityPlayerMP
				&& player.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH) != null) {
			((EntityPlayerMP) player).connection.sendPacket(new SPacketEntityProperties(player.getEntityId(),
					ImmutableList.of(player.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH))));
		}
	}
	...
	public static void onUseItemTick(LivingEntityUseItemEvent.Tick event) {
		...
		if (!event.getEntityLiving().getEntityWorld().isRemote && event.getDuration() <= 1) {
			if (!event.getItem().isEmpty() && event.getItem().getItem() instanceof IDecayFood
					&& event.getEntityLiving() instanceof EntityPlayer) {
				EntityPlayer player = (EntityPlayer) event.getEntityLiving();
				IDecayCapability cap = player.getCapability(CapabilityRegistry.CAPABILITY_DECAY, null);
				if (cap != null) {
					IDecayFood food = (IDecayFood) event.getItem().getItem();
					cap.getDecayStats().addStats(-food.getDecayHealAmount(event.getItem()),
							food.getDecayHealSaturation(event.getItem()));
				}
			}
		}
	}
	...
	public static void onStartUsingItem(LivingEntityUseItemEvent.Start event) {
		if (!event.getItem().isEmpty() && event.getEntityLiving() instanceof EntityPlayer) {
			...
			if (isDecayFood) {
				boolean canEatFood = player.getFoodStats().needFood() && event.getItem().getItem() instanceof ItemFood
						&& ((ItemFood) event.getItem().getItem()).getHealAmount(event.getItem()) > 0;
