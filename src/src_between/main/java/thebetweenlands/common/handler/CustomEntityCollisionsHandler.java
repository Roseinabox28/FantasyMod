...
public final class CustomEntityCollisionsHandler {
	public static final class Helper implements CollisionBoxHelper {
		...
		public void getCollisionBoxes(Entity entity, AxisAlignedBB aabb, EntityCollisionPredicate entityPredicate,
			...
			CustomEntityCollisionsHandler.getCollisionBoxes(entity, aabb, entityPredicate, blockPredicate,
					collisionBoxes);
		}
	}
	...
	public static void onGatherCollisionBoxes(GetCollisionBoxesEvent event) {
		if (!gathering) {
			...
			try {
				if (entity != null) {
					IEntityCustomCollisionsCapability cap = entity
					...
					if (cap != null || entity instanceof IEntityCustomBlockCollisions) {
						if (cap != null) {
							...
							((IEntityCustomBlockCollisions) event.getEntity()).getCustomCollisionBoxes(event.getAabb(),
				 ...
				 * while(it.hasNext()) { AxisAlignedBB aabb = it.next(); if(aabb instanceof
				 * ProcessedEntityCollisionBox) { it.remove();
				 * 
				 * AxisAlignedBB processedAabb = ((ProcessedEntityCollisionBox<?>)
				 * aabb).process(event.getEntity(), event.getAabb()); if(processedAabb != null)
				 * { if(processedAabbList == null) { processedAabbList = new ArrayList<>(); }
				 * 
	...
	private static void getCollisionBoxes(Entity entity, AxisAlignedBB aabb, EntityCollisionPredicate entityPredicate,
			BlockCollisionPredicate blockPredicate, List<AxisAlignedBB> collisionBoxes) {
		...
		MinecraftForge.EVENT_BUS.post(
				new net.minecraftforge.event.world.GetCollisionBoxesEvent(entity.world, entity, aabb, collisionBoxes));
	}

	private static void getEntityCollisionBoxes(Entity entity, AxisAlignedBB aabb,
			EntityCollisionPredicate entityPredicate, List<AxisAlignedBB> collisionBoxes) {
		if (entity != null) {
			...
			for (Entity otherEntity : otherEntities) {
				if (!entity.isRidingSameEntity(otherEntity)) {
					...
					if (otherAABB != null && otherAABB.intersects(aabb)
							&& entityPredicate.isColliding(entity, aabb, otherEntity, otherAABB)) {
	...
	private static void getBlockCollisionBoxes(Entity entity, AxisAlignedBB aabb,
			BlockCollisionPredicate blockPredicate, List<AxisAlignedBB> collisionBoxes) {
