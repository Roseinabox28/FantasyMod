				...
				for (IEnvironmentEvent eevent : reg.getEvents().values()) {
					...
					if (dataManager != null) {
						...
						if (dataManager.isDirty()) {
							TheBetweenlands.networkWrapper.sendToDimension(
									new MessageSyncEnvironmentEventData(eevent, false),
				...
				for (IEnvironmentEvent eevent : storage.getEnvironmentEventRegistry().getEvents().values()) {
					if (eevent instanceof BLEnvironmentEvent) {
						TheBetweenlands.networkWrapper.sendTo(
								new MessageSyncEnvironmentEventData((BLEnvironmentEvent) eevent, true),
								(EntityPlayerMP) event.getEntity());
					}
					if (eevent.isActive())
						AdvancementCriterionRegistry.EVENT.trigger((EntityPlayerMP) event.getEntity(),
								eevent.getEventName());
				}
			}
		}
