...
public class ExtendedReachHandler {
	...
	public static void onMouseClick(MouseEvent event) {
		if ((event.getButton() == 0) && event.isButtonstate()) {
			extendedRayTrace(
					trace -> TheBetweenlands.networkWrapper.sendToServer(new MessageExtendedReach(trace.entityHit)));
		}
	}
			...
			extendedRayTrace(trace -> {
				...
				if (mc.pointedEntity == null && mc.gameSettings.thirdPersonView == 0) {
					...
					if (mc.gameSettings.attackIndicator == 1
							&& !(mc.gameSettings.showDebugInfo && !mc.gameSettings.hideGUI
									&& !mc.player.hasReducedDebug() && !mc.gameSettings.reducedDebugInfo)) {
		...
		if (player != null) {
			...
			if (!stack.isEmpty()) {
				if (stack.getItem() instanceof IExtendedReach) {
					...
					if (trace != null && trace.entityHit != null && trace.entityHit.hurtResistantTime == 0
							&& trace.entityHit != player) {
	...
	public static RayTraceResult getExtendedRayTrace(double dist) {
		...
		if (mc.world != null && viewEntity != null) {
			...
			Vec3d hitVec = null;
			List<Entity> list = mc.world.getEntitiesInAABBexcluding(viewEntity,
					viewEntity.getEntityBoundingBox().expand(startVec.x * dist, startVec.y * dist, startVec.z * dist)
							.grow(1.0D, 1.0D, 1.0D),
					Predicates.and(EntitySelectors.NOT_SPECTATING, Predicates.notNull(), Entity::canBeCollidedWith));
			...
			for (Entity entity : list) {
				...
				if (entityAABB.contains(viewPos)) {
					...
					if ((d1 < entityDist) || (entityDist == 0.0D)) {
						if (entity.getLowestRidingEntity() == viewEntity.getLowestRidingEntity()
