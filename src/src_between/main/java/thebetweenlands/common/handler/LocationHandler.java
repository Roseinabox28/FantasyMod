...
public class LocationHandler {
	...
	public static List<LocationStorage> getLocations(Entity entity) {
		...
		return worldStorage.getLocalStorageHandler().getLocalStorages(LocationStorage.class, entity.posX, entity.posZ,
				location -> location.isInside(entity));
	}
	...
	public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
		if (event.phase == Phase.END) {
			...
			if (player != null && !player.world.isRemote) {
				if (player instanceof EntityPlayerMP && ((EntityPlayerMP) player).world.provider
	...
	public static void onBlockBreak(BlockEvent.BreakEvent event) {
		...
		if (player != null) {
			...
			List<LocationStorage> locations = worldStorage.getLocalStorageHandler().getLocalStorages(
					LocationStorage.class, pos.getX(), pos.getZ(), location -> location.isInside(pos));
	...
	public static void onBlockPlace(BlockEvent.PlaceEvent event) {
		...
		if (!player.isCreative()) {
			...
			for (BlockPos pos : positions) {
				...
				for (LocationStorage location : locations) {
					if (location != null && location.getGuard() != null
							&& location.getGuard().isGuarded(player.world, player, pos)) {
	...
	public static void onBlockRightClick(RightClickBlock event) {
		...
		if (!player.isCreative() && !event.getItemStack().isEmpty()
				&& Block.getBlockFromItem(event.getItemStack().getItem()) != Blocks.AIR) {
			...
			for (LocationStorage location : locations) {
				if (location != null && location.getGuard() != null
						&& location.getGuard().isGuarded(player.world, player, resultingPos)) {
					event.setUseItem(Result.DENY);
					if (event.getWorld().isRemote) {
						Vec3d hitVec = event.getHitVec();
						BLParticles.BLOCK_PROTECTION.spawn(event.getWorld(),
								hitVec.x + event.getFace().getXOffset() * 0.025F,
								hitVec.y + event.getFace().getYOffset() * 0.025F,
								hitVec.z + event.getFace().getZOffset() * 0.025F,
								ParticleArgs.get().withData(event.getFace()));
					}
					return;
				}
			}
	...
	public static void onBreakSpeed(PlayerEvent.BreakSpeed event) {
		...
		for (LocationStorage location : locations) {
			if (location != null && location.getGuard() != null
					&& location.getGuard().isGuarded(player.world, player, event.getPos())) {
	...
	private static void spawnBreakSpeedParticle(BlockPos pos, EntityPlayer player) {
		if (player instanceof EntityPlayerSP && Minecraft.getMinecraft().playerController.getIsHittingBlock()) {
			...
			if (rayTrace != null && rayTrace.typeOfHit == RayTraceResult.Type.BLOCK
					&& pos.equals(rayTrace.getBlockPos())) {
				Vec3d hitVec = rayTrace.hitVec;
				BLParticles.BLOCK_PROTECTION.spawn(player.world, hitVec.x + rayTrace.sideHit.getXOffset() * 0.025F,
						hitVec.y + rayTrace.sideHit.getYOffset() * 0.025F,
						hitVec.z + rayTrace.sideHit.getZOffset() * 0.025F,
						ParticleArgs.get().withData(rayTrace.sideHit));
			}
		}
	}
	...
	public static void onExplosion(ExplosionEvent.Detonate event) {
		...
		for (BlockPos pos : explosion.getAffectedBlockPositions()) {
			...
			if (locations == null) {
				locations = worldStorage.getLocalStorageHandler().getLocalStorages(LocationStorage.class, pos.getX(),
						pos.getZ(), storage -> storage.getGuard() != null);
				locationCache.put(chunkId, locations);
			}

			for (LocationStorage location : locations) {
				if (location.getGuard().isGuarded(world, explosion.getExplosivePlacedBy(), pos)
						&& !affectedLocations.contains(location)) {
	...
	public static void onLeftClickBlock(LeftClickBlock event) {
		if (!event.getEntityPlayer().isCreative()) {
			...
			for (LocationStorage location : locations) {
				if (location != null && location.getGuard() != null
						&& location.getGuard().isGuarded(event.getWorld(), event.getEntityPlayer(), event.getPos())) {
					Vec3d hitVec = event.getHitVec();
					BLParticles.BLOCK_PROTECTION.spawn(event.getWorld(),
							hitVec.x + event.getFace().getXOffset() * 0.025F,
							hitVec.y + event.getFace().getYOffset() * 0.025F,
