...
public class TextContainer {
	...
	public TextContainer(final double pageWidth, final double pageHeight, final String text,
			final FontRenderer defaultFont) {
	...
	public static class TextArea {
		...
		public boolean isInside(double offsetX, double offsetY, double testX, double testY) {
			return testX >= x + offsetX && testX < x + offsetX + width && testY >= y + offsetY
					&& testY < y + offsetY + height;
		}
		...
		public boolean equals(Object object) {
			if (object instanceof TextArea) {
				...
				return area.page.equals(this.page) && area.x == this.x && area.y == this.y && area.width == this.width
	...
	/**
	 * A text segment is a text area with the additional properties: text, color,
	 * scale and font
	 */
	public static class TextSegment extends TextArea {
		...
		public TextSegment(TextPage page, String text, double x, double y, double width, double height, int color,
				float scale, FontRenderer font) {
		...
		private TextSegment(TextPage page, String text, double x, double y, double width, double height, int color,
				float scale, FontRenderer font, List<Object> properties) {
		...
		try {
			...
			for (int w = 0; w < wrapSegments.size() - 1; w++) {
				...
				for (ParsedSegment segment : wrapSegment.segments) {
						...
						for (ParsedTag tag : tags) {
							...
							if (textTag == null) {
								...
								if (tag.close) {
									...
									if (newTag.push(this, newTag instanceof RangedTag ? tagStack.peek() : null,
											tagArguments, null, 0) && newTag instanceof RangedTag) {
						...
						for (ParsedTag tag : tags) {
							...
							if (textTag == null) {
								...
								if (!tag.close) {
									Tag newTag = textTag.create();
									if (newTag.push(this, newTag instanceof RangedTag ? tagStack.peek() : null,
											tagArguments, area, 1) && newTag instanceof RangedTag) {
										tagStack.push((RangedTag) newTag);
									}
								}
							}
					...
					// Only add X offset and segment at start of new line if the segment's width is
					// significant when word wrapping
					if (!newLine || segment.significant || this.newLine > 0) {
						//// Create text segment ////
						TextSegment textSegment = new TextSegment(area.page, word, cursorX, cursorY, strWidth,
								strHeight, this.currentColor, this.currentScale, this.currentFont);
						if (word.length() > 0) {
			...
			if (ex instanceof EmptyStackException) {
				throw new RuntimeException(
						"Stack underflow. Stack type: " + currentTagType + " Word: " + currentSegment.text, ex);
			}
			throw ex;
		}
	}
