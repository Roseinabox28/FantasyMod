...
public class PageCreators {
	 ...
	 * @param isHidden        whether or not the pages should be hidden until
	 *                        discovery
	...
	public static ArrayList<Page> TextPages(int x, int y, String unlocalizedName, String pageName, boolean isHidden,
		...
		TextContainer textContainer = parseTextContainer(
				new TextContainer(116, 144, text, Minecraft.getMinecraft().fontRenderer));
	...
	public static ArrayList<Page> AspectPages(IAspectType aspect, Item manualType) {
		...
		TextContainer textContainer = parseTextContainer(new TextContainer(112, 144,
				TranslationHelper.translateToLocal("manual." + aspect.getName().toLowerCase() + ".description"),
				Minecraft.getMinecraft().fontRenderer));

		height += textContainer.getPages().get(0).getSegments()
				.get(textContainer.getPages().get(0).getSegments().size() - 1).y + 18;
		...
		if (height + 18 + 16 < 152) {
			...
			newPages.add(new Page(aspect.getName().toLowerCase(), widgets, false, manualType).setParent()
					.setAspect(aspect).setLocalizedPageName(aspect.getName()));
			widgets.add(new TextWidget(18, 12 + height, "manual.aspect.found.in"));
			height += 16;
			widgets.add(new AspectItemSlideShowWidget(18, 12 + height, aspect));
			height += 18;
		...
		if (height + 10 + 18 < 152) {
			...
			if (newPages.size() > 0)
				newPages.add(new Page(aspect.getName().toLowerCase(), widgets, false, manualType).setAspect(aspect)
						.setLocalizedPageName(aspect.getName()));
			else
				newPages.add(new Page(aspect.getName().toLowerCase(), widgets, false, manualType).setParent()
						.setAspect(aspect).setLocalizedPageName(aspect.getName()));
			widgets.add(new TextWidget(18, 12 + height, "manual.aspect.used.in"));
	...
	public static List<Page> AspectItemPages(AspectItem item, Item manualType) {
		...
		TextContainer textContainer = parseTextContainer(new TextContainer(112, 144,
				TranslationHelper.translateToLocal("manual." + itemStack.getTranslationKey() + ".description"),
				Minecraft.getMinecraft().fontRenderer));

		height += 18 + textContainer.getPages().get(0).getSegments()
				.get(textContainer.getPages().get(0).getSegments().size() - 1).y;
		widgets.add(new TextWidget(18, 12 + height, "manual.has.aspects"));
		height += 18;
		widgets.add(new AspectSlideShowWidget(18, 12 + height, itemStack));
		Page itemPage = new Page(itemStack.getTranslationKey().toLowerCase().replace(" ", ""), widgets, true,
				manualType).setParent().addItem(itemStack).setLocalizedPageName(itemStack.getDisplayName());
		if (!pestleAndMortarInput.isEmpty()) {
	...
	public static ArrayList<Page> elixirPages(ItemStack item, Item manualType, ElixirEffect effect) {
		...
		height += 32;
		TextContainer textContainer = new TextContainer(115, 130,
				TranslationHelper.translateToLocal("manual." + item.getTranslationKey() + ".description"),
				Minecraft.getMinecraft().fontRenderer);
		...
		if (textContainer.getPages().size() > 1) {
			...
			newPages.add(new Page(item.getTranslationKey().toLowerCase().replace(" ", ""),
					(ArrayList<ManualWidgetBase>) widgets.clone(), false, manualType).setParent()
							.setLocalizedPageName(item.getDisplayName()).addItem(item));
			widgets.clear();
			...
			if (ElixirRecipes.getFromEffect(effect) != null && ElixirRecipes.getFromEffect(effect).aspects != null) {
				if (textContainer.getPages().get(1).getSegments()
						.get(textContainer.getPages().get(1).getSegments().size() - 1).y + 38 < 142)
					widgets.add(new AspectSlideShowWidget(18,
							(int) (textContainer.getPages().get(1).getSegments()
									.get(textContainer.getPages().get(1).getSegments().size() - 1).y + 22),
							ElixirRecipes.getFromEffect(effect).aspects));
				else
					temp = new Page(item.getTranslationKey().toLowerCase().replace(" ", ""), false, manualType,
							new AspectSlideShowWidget(15, 12, ElixirRecipes.getFromEffect(effect).aspects));
			}
			newPages.add(new Page(item.getTranslationKey().toLowerCase().replace(" ", ""), widgets, false, manualType)
					.setLocalizedPageName(item.getDisplayName()).addItem(item));
			if (temp != null)
				newPages.add(temp);
		} else {
			widgets.add(
					new TextWidget(15, height, "manual." + item.getTranslationKey() + ".description", 0, 115, 1130));
			if (ElixirRecipes.getFromEffect(effect) != null && ElixirRecipes.getFromEffect(effect).aspects != null) {
				if (height + 24 < 142)
					widgets.add(new AspectSlideShowWidget(18,
							height + (int) (textContainer.getPages().get(0).getSegments()
									.get(textContainer.getPages().get(0).getSegments().size() - 1).y + 8),
							ElixirRecipes.getFromEffect(effect).aspects));
				else
					temp = new Page(item.getTranslationKey().toLowerCase().replace(" ", ""), false, manualType,
							new AspectSlideShowWidget(15, 12, ElixirRecipes.getFromEffect(effect).aspects));
			...
			newPages.add(new Page(item.getTranslationKey().toLowerCase().replace(" ", ""), widgets, false, manualType)
					.setParent().setLocalizedPageName(item.getDisplayName()).addItem(item));
			if (temp != null)
				newPages.add(temp);
		}
	...
	/**
	 * Parses the text container. Used to get the right width and height of the
	 * container
	 *
	 * @param textContainer a unparsed text container
	 * @return a pars ed text container
	 */
