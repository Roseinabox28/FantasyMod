...
public class ManualManager {
	...
	public static boolean hasFoundPage(EntityPlayer player, String page, Item itemManual) {
		...
		if (!stack.isEmpty() && stack.getItem() == ItemRegistry.MANUAL_HL) {
			...
			for (Map.Entry<AspectItem, List<AspectManager.AspectItemEntry>> e : matchedAspects.entrySet()) {
				if (e.getKey() != null) {
					ItemStack itemStack = new ItemStack(e.getKey().getOriginal().getItem(), 1,
							e.getKey().getOriginal().getItemDamage());
					if (itemStack.getTranslationKey().toLowerCase().replace(" ", "").equals(page)) {
						ingredient = new ItemStack(e.getKey().getOriginal().getItem(), 1,
								e.getKey().getOriginal().getItemDamage());
						break;
					}
				}
			}
			ItemAspectContainer aspectContainer = ItemAspectContainer.fromItem(ingredient,
					AspectManager.get(Minecraft.getMinecraft().world));
			return ingredient != null && aspectContainer.getAspects(container).size() > 0;
		}

		return player != null && page != null && getFoundPages(player, itemManual) != null
				&& getFoundPages(player, itemManual).contains(page);
	}
	...
	public static void playerDiscoverPage(EntityPlayer player, String name, Item itemManual) {
		if (!ManualManager.hasFoundPage(player, name, itemManual) && player != null
				&& player.inventory.hasItemStack(new ItemStack(itemManual)) && !player.world.isRemote) {
			if (ManualManager.findPage(player, name, itemManual))
				player.sendStatusMessage(new TextComponentTranslation("chat.manual.discover_page",
						new TextComponentTranslation("manual." + name + ".title")), true);
		}
	}
	...
	public static void setCurrentPage(String category, int pageNumber, Item itemManual, EntityPlayer player,
			EnumHand hand) {
		if (player != null && !player.getHeldItem(hand).isEmpty() && player.getHeldItem(hand).getItem() == itemManual
				&& category != null) {
	...
	/**
	 * Gets the pagenumber you where on last time you closed the manual, or -1 if
	 * the player is opening the manual for the first time
	 * 
	 * @param manual
	 * @return
	 */
