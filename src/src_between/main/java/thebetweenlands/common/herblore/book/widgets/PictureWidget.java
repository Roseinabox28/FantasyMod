...
public class PictureWidget extends ManualWidgetBase {
	...
	public PictureWidget(int xStart, int yStart, String recourseLocation, int width, int height, double textureWidth,
			double textureHeight) {
		super(xStart, yStart);
	...
	public PictureWidget(int xStart, int yStart, String recourseLocation, int width, int height,
			ArrayList<String> toolTips, double textureWidth, double textureHeight) {
		super(xStart, yStart);
		this.recourseLocation = new ResourceLocation(recourseLocation);
		this.width = width;
		this.height = height;
	...
	public void drawForeGround() {
		...
		manual.drawTexture(xStart, yStart, width, height, textureWidth, textureHeight, xIndex, xIndex + width, yIndex,
				yIndex + height);
		GlStateManager.disableBlend();

		if (toolTips.size() > 0 && mouseX >= xStart && mouseX <= xStart + width && mouseY >= yStart
