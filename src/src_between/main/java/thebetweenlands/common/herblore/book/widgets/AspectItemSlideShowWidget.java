...
public class AspectItemSlideShowWidget extends ManualWidgetBase {
	...
	public void drawForeGround() {
		...
		List<ItemStack> subItems = items.subList(currentItems,
				currentItems + ((items.size() - currentItems) > 5 ? 6 : items.size() - currentItems));
		int width = 0;
	...
	public void getItems() {
		...
		for (Map.Entry<AspectItem, List<AspectManager.AspectItemEntry>> entry : AspectManager.getRegisteredItems()
				.entrySet()) {
			List<Aspect> discoveredAspects = manager.getDiscoveredStaticAspects(entry.getKey(),
					DiscoveryContainer.getMergedDiscoveryContainer(Minecraft.getMinecraft().player));
			for (Aspect aspect : discoveredAspects) {
				if (aspect.type.equals(this.aspectType))
					items.add(new ItemStack(entry.getKey().getOriginal().getItem(), 1,
							entry.getKey().getOriginal().getItemDamage()));
			}
		}
	}
}
