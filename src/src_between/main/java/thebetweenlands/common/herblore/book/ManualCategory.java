...
public class ManualCategory {
	...
	/**
	 * @param pages                  all pages in this category
	 * @param number                 a number that shows which where the category is
	 *                               located
	 * @param manualType             the type of manual it is. manualHL or
	 *                               manualGuideBook
	 * @param name                   the name of the category
	 * @param customPageInitializing whether or not it has a custom way of doing
	 *                               pages
	 * @param indexPages             if not needed use -1
	 */
	public ManualCategory(ArrayList<Page> pages, int number, Item manualType, String name,
			boolean customPageInitializing, int indexPages) {
		ArrayList<Page> buttonPagesNew;
	...
	public void init(GuiManualHerblore manual, boolean force) {
		if (currentPageLeft == null || currentPageRight == null || force) {
			...
			for (Page page : pages)
				if (!page.isHidden
						|| ManualManager.hasFoundPage(manual.player, page.unlocalizedPageName, manual.manualType))
