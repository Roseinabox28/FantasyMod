...
public class GuiManualHerblore extends GuiScreen {
	...
	public void initGui() {
		...
		changeCategory(currCategory == null ? HLEntryRegistry.aspectCategory : currCategory,
	...
	public void drawScreen(int mouseX, int mouseY, float renderPartials) {
		...
		if (currentCategory != null) {
			if (currentCategory.getCurrentPage() - 2 >= 1) {
				if (mouseX >= xStart + 15 && mouseX <= xStart + 15 + 19 && mouseY >= yStart + 160
						&& mouseY <= yStart + 160 + 8)
			...
			if (currentCategory.getCurrentPage() + 2 <= currentCategory.getVisiblePages().size()) {
				if (mouseX >= xStart + 256 && mouseX <= xStart + 256 + 19 && mouseY >= yStart + 160
						&& mouseY <= yStart + 160 + 8)
			...
			drawTexture(xStart, yStart + 10, 14, 22 * currentCategory.getCategoryNumber(), 1024.0D, 1024.0D, 293.0D,
					306.0D, 18.0D, 18.0D + 22.0D * currentCategory.getCategoryNumber());
			drawTexture(xStart + 279, yStart + 10 + 22 * currentCategory.getCategoryNumber(), 14,
					44 - 22 * currentCategory.getCategoryNumber(), 1024.0D, 1024.0D, 306.0D, 293.0D,
					18.0D + 22.0D * currentCategory.getCategoryNumber(), 62.0D);
	...
	protected void mouseClicked(int mouseX, int mouseY, int button) {
		if (currentCategory != null) {
			...
			if (mouseX >= xStart + (currentCategory.getCategoryNumber() >= 1 ? 0 : 279)
					&& mouseX <= xStart + (currentCategory.getCategoryNumber() >= 1 ? 0 : 279) + 14
					&& mouseY >= yStart + 11 && mouseY <= yStart + 10 + 20 && button == 0)
				changeCategory(HLEntryRegistry.aspectCategory);
			if (mouseX >= xStart + (currentCategory.getCategoryNumber() >= 2 ? 0 : 279)
					&& mouseX <= xStart + (currentCategory.getCategoryNumber() >= 2 ? 0 : 279) + 14
					&& mouseY >= yStart + 33 && mouseY <= yStart + 32 + 20 && button == 0)
	...
	protected void renderPageNumbers(int mouseX, int mouseY, float partialTicks) {
		if (this.currentCategory != null) {
			int leftPageStrWidth = this.fontRenderer
					.getStringWidth(String.valueOf(this.currentCategory.getCurrentPage()));
			GlStateManager.enableBlend();
			this.fontRenderer.drawString(String.valueOf(this.currentCategory.getCurrentPage()),
					this.xStart + this.WIDTH / 2 - 11 - leftPageStrWidth, this.yStart + this.HEIGHT - 17, 0x804f4314);
			this.fontRenderer.drawString(String.valueOf(this.currentCategory.getCurrentPage() + 1),
					this.xStart + this.WIDTH / 2 + 11, this.yStart + this.HEIGHT - 17, 0x804f4314);
		}
	}
	 ...
	 */
	public void drawTexture(int xStart, int yStart, int width, int height, double textureWidth, double textureHeight,
			double textureXStart, double textureXEnd, double textureYStart, double textureYEnd) {
