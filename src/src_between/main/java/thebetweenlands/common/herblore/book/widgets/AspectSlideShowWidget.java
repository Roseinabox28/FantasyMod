...
public class AspectSlideShowWidget extends ManualWidgetBase {
	...
	public void drawForeGround() {
		List<IAspectType> subItems = aspects.subList(currentItems,
				currentItems + (aspects.size() - currentItems > 5 ? 6 : aspects.size() - currentItems));
		int width = 0;
		for (IAspectType aspect : subItems) {
			...
			if (link.category != null)
				pageLinks.add(link);
			if (mouseX >= xStart + width && mouseX <= xStart + 16 + width && mouseY >= yStart
					&& mouseY <= yStart + 16) {
				List<String> tooltipData = new ArrayList<>();
				tooltipData.add(aspect.getName());
				tooltipData.add(TextFormatting.GRAY + aspect.getType());
				tooltipData.add(TextFormatting.GRAY
						+ (I18n.canTranslate("manual.open_entry") ? I18n.translateToLocal("manual.open_entry")
								: I18n.translateToFallback("manual.open_entry")));
	...
	public void getAspects() {
		aspects.clear();
		ItemAspectContainer container = ItemAspectContainer.fromItem(this.itemStack,
				AspectManager.get(Minecraft.getMinecraft().world));
		List<Aspect> visibleAspects = container
				.getAspects(DiscoveryContainer.getMergedDiscoveryContainer(Minecraft.getMinecraft().player));
