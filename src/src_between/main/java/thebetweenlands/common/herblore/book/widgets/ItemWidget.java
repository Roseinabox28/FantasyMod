...
public class ItemWidget extends ManualWidgetBase {
	...
	public void drawForeGround() {
		...
		render.renderItemAndEffectIntoGUI(stacks.get(currentDisplayItem), (int) (xStart / scale),
				(int) (yStart / scale));
		render.renderItemOverlayIntoGUI(Minecraft.getMinecraft().fontRenderer, stacks.get(currentDisplayItem),
				(int) (xStart / scale), (int) (yStart / scale), null);
		...
		if (mouseX >= xStart && mouseX <= xStart + 16 * scale && mouseY >= yStart && mouseY <= yStart + 16 * scale) {
			if (stacks.get(currentDisplayItem) != null) {
				List<String> tooltipData = stacks.get(currentDisplayItem).getTooltip(Minecraft.getMinecraft().player,
