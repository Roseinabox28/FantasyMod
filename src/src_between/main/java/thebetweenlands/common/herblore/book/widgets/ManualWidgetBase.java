...
public class ManualWidgetBase {
	 ...
	 */
	@SideOnly(Side.CLIENT)
	public static void drawGradientRect(int left, int top, float z, int right, int bottom, int startColor,
			int endColor) {
	...
	public void renderItem(int xPos, int yPos, ItemStack stack, boolean hasSpecialTooltip, boolean addPageLink,
		...
		if (!hasSpecialTooltip && mouseX >= xPos && mouseY >= yPos && mouseX <= xPos + 16 && mouseY <= yPos + 16) {
			if (!stack.isEmpty()) {
				List<String> tooltipData = stack.getTooltip(Minecraft.getMinecraft().player,
						ITooltipFlag.TooltipFlags.NORMAL);
				List<String> parsedTooltip = new ArrayList<>();
				boolean first = true;
				if (addPageLink && shouldShowTooltip)
					tooltipData.add(I18n.canTranslate("manual.open_entry") ? I18n.translateToLocal("manual.open_entry")
	...
	public static class PageLink {
								...
								for (ItemStack stack : page.pageItems) {
									if (stack != null && stack.getItem() == item.getItem()
											&& (stack.getItemDamage() == item.getItemDamage()
													|| stack.getItemDamage() == item.getItemDamage() - 1)) {
		...
		public String toString() {
			return "page number: " + pageNumber + ", category name: " + category.getName() + ", xStart: " + x
					+ ", yStart: " + y + ", width: " + width + ", height: " + height;
		}
