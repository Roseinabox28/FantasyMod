...
public class InventoryItem implements IInventory {
	...
	public void readFromNBT(NBTTagCompound compound) {
		 ...
		 * Constants.NBT.TAG_COMPOUND); for (int i = 0; i < items.tagCount(); ++i) {
		 * NBTTagCompound item = items.getCompoundTagAt(i); int slot =
		 * item.getInteger("Slot"); if (slot >= 0 && slot < this.getSizeInventory()) {
	...
	public void writeToNBT(NBTTagCompound tagcompound) {
		 ...
		 * 
		 * for (int i = 0; i < this.getSizeInventory(); ++i) { if
		 * (this.getStackInSlot(i) != null) { NBTTagCompound item = new
		 * NBTTagCompound(); item.setInteger("Slot", i);
		 * getStackInSlot(i).writeToNBT(item); items.appendTag(item); } }
		 * 
