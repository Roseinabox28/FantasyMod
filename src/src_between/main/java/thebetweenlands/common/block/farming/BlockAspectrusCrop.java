...
public class BlockAspectrusCrop extends BlockGenericCrop implements ICustomItemBlock, ITileEntityProvider {
	...
	protected boolean removePlant(World world, BlockPos pos, EntityPlayer player, boolean canHarvest) {
		return world.setBlockState(pos, BlockRegistry.RUBBER_TREE_PLANK_FENCE.getDefaultState(),
				world.isRemote ? 11 : 3);
	}
	...
	/**
	 * Updates the plant's aspect by looking for nearby aspect fog block producing
	 * an aspect
	 * 
	 * @param world
	 * @param pos
	...
	protected void updateAspectSource(World world, BlockPos pos, IBlockState state) {
		...
		for (BlockPos.MutableBlockPos checkPos : BlockPos.getAllInBoxMutable(pos.add(-6, -this.maxHeight, -6),
				pos.add(6, 0, 6))) {
			if (world.isBlockLoaded(checkPos)) {
				...
				if (offsetBlock instanceof IAspectFogBlock) {
					IAspectType aspectType = ((IAspectFogBlock) offsetBlock).getAspectFogType(world, checkPos,
							offsetState);
