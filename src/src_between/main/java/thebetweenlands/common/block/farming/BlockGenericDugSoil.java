...
public abstract class BlockGenericDugSoil extends BasicBlock implements ITileEntityProvider,
		ISubtypeItemBlockModelDefinition, IStateMappedBlock, ICustomItemBlock, IConnectedTextureBlock {
	...
	public BlockGenericDugSoil(Material material, boolean purified) {
		...
		this.setDefaultState(this.getBlockState().getBaseState().withProperty(COMPOSTED, false)
				.withProperty(DECAYED, false).withProperty(FOGGED, false));
	...
	public static void onBlockBreak(BreakEvent event) {
		...
		if (!event.getWorld().isRemote) {
			...
			if (stateDown.getBlock() instanceof BlockGenericDugSoil) {
				if (event.getState().getBlock() instanceof BlockGenericCrop == false
						&& event.getState().getBlock() instanceof IPlantable) {
	...
	public static void onHarvestBlock(HarvestDropsEvent event) {
		...
		if (!event.getWorld().isRemote) {
			...
			if (stateDown.getBlock() instanceof BlockGenericDugSoil) {
				if (event.getState().getBlock() instanceof BlockGenericCrop == false
						&& event.getState().getBlock() instanceof IPlantable && stateDown.getValue(DECAYED)) {
	...
	protected BlockStateContainer createBlockState() {
		return this.getConnectedTextureBlockStateContainer(
				new ExtendedBlockState(this, new IProperty[] { COMPOSTED, DECAYED, FOGGED }, new IUnlistedProperty[0]));
	}
	...
	public IBlockState getExtendedState(IBlockState oldState, IBlockAccess worldIn, BlockPos pos) {
		...
		return this.getExtendedConnectedTextureState(state, worldIn, pos,
				p -> worldIn.getBlockState(p)
						.getBlock() instanceof BlockGenericDugSoil /* TODO: Add canConnectTo similar to fence? */,
				false);
	}
	...
	public void setStateMapper(AdvancedStateMap.Builder builder) {
		builder.ignore(COMPOSTED).ignore(DECAYED).withPropertySuffix(COMPOSTED, null, "composted")
				.withPropertySuffixExclusions((map) -> {
					...
					if (map != null && map.getValue(COMPOSTED) != null && map.getValue(DECAYED) != null
							&& map.getValue(COMPOSTED) && map.getValue(DECAYED))
	...
	public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
		if (!world.isRemote) {
			...
			if (te != null) {
				if (!this.purified && te.isComposted() && !te.isFullyDecayed()
						&& rand.nextFloat() <= this.getDecayChance(world, pos, state, rand)) {
				...
				if (te.isComposted()) {
					...
					if (stateUp.getBlock() instanceof IFarmablePlant) {
						...
						if (plant.isFarmable(world, pos.up(), stateUp)) {
							...
							if (rand.nextFloat() <= spreadChance
									&& plant.canSpreadTo(world, pos.up(), stateUp, offsetPos, rand)) {
								plant.spreadTo(world, pos.up(), stateUp, offsetPos, rand);
								te.setCompost(Math.max(
										te.getCompost() - plant.getCompostCost(world, pos.up(), stateUp, rand), 0));
							}
						}
					}
				}
	...
	/**
	 * Updates the fogged state, i.e. whether a nearby censer is producing fog or
	 * not. Pretty much like farmland and water.
	 * 
	 * @param world
	 * @param pos
	 * @param state
	...
	protected IBlockState updateFoggedState(World world, BlockPos pos, IBlockState state) {
		...
		for (BlockPos.MutableBlockPos checkPos : BlockPos.getAllInBoxMutable(pos.add(-6, 0, -6), pos.add(6, 1, 6))) {
			if (world.isBlockLoaded(checkPos)) {
				...
				if (offsetBlock instanceof IDungeonFogBlock
						&& ((IDungeonFogBlock) offsetBlock).isCreatingDungeonFog(world, checkPos, offsetState)) {
					shouldBeFogged = true;
					break;
				}
			}
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand,
		...
		if (te != null && te.getCompost() == 0 && !heldItem.isEmpty() && EnumItemMisc.COMPOST.isItemOf(heldItem)) {
			if (!world.isRemote) {
				world.playSound(null, pos.getX() + hitX, pos.getY() + hitY, pos.getZ() + hitZ,
						SoundEvents.BLOCK_GRASS_PLACE, SoundCategory.PLAYERS, 1, 0.5f + world.rand.nextFloat() * 0.5f);
				te.setCompost(30);
		...
		if (stateIn.getValue(FOGGED)) {
			BatchedParticleRenderer.INSTANCE
					.addParticle(DefaultParticleBatches.TRANSLUCENT_GLOWING_NEAREST_NEIGHBOR,
							BLParticles.SMOOTH_SMOKE.create(worldIn, pos.getX() + rand.nextFloat(), pos.getY() + 1,
									pos.getZ() + rand.nextFloat(),
									ParticleArgs.get().withMotion((rand.nextFloat() - 0.5f) * 0.05f,
											rand.nextFloat() * 0.02F + 0.005F, (rand.nextFloat() - 0.5f) * 0.05f)
											.withScale(5.0f).withColor(1, 1, 1, 0.1f).withData(80, true, 0.0F, true)));
		}

		if (stateIn.getValue(DECAYED)) {
			BLParticles.DIRT_DECAY.spawn(worldIn, pos.getX() + rand.nextFloat(), pos.getY() + 1.0F,
					pos.getZ() + rand.nextFloat());
			...
			for (int i = 0; i < 5; i++) {
				...
				if (stateOffset.getBlock() instanceof IFarmablePlant
						&& ((IFarmablePlant) stateOffset.getBlock()).isFarmable(worldIn, offsetPos, stateOffset)) {
					BLParticles.DIRT_DECAY.spawn(worldIn, offsetPos.getX() + rand.nextFloat(), offsetPos.getY(),
							offsetPos.getZ() + rand.nextFloat());
				}
			}
		} else {
			...
			if (te.getDecay() >= 11) {
				if (rand.nextInt(Math.max(120 - (te.getDecay() - 11) * 14, 2)) == 0) {
					...
					for (int i = 0; i < 5; i++) {
						...
						IBlockState stateOffset = worldIn.getBlockState(offsetPos);
						if (stateOffset.getBlock() instanceof IFarmablePlant
								&& ((IFarmablePlant) stateOffset.getBlock()).isFarmable(worldIn, offsetPos,
										stateOffset)) {
							BLParticles.DIRT_DECAY.spawn(worldIn, offsetPos.getX() + rand.nextFloat(), offsetPos.getY(),
									offsetPos.getZ() + rand.nextFloat());
						}
	...
	/**
	 * Returns how often crops can be harvested before the purified soil turns into
	 * unpurified soil
	 * 
	 * @param world
	 * @param pos
	...
	/**
	 * Returns the unpurified variant of this soil. This should also copy the
	 * {@link #COMPOSTED} and {@link #DECAYED} properties.
	 * 
	 * @param world
	 * @param pos
	 * @param state
