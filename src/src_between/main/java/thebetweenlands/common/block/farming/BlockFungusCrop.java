	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		...
		if (this.isDecayed(worldIn, pos)) {
			if (!worldIn.isRemote && state.getValue(AGE) >= 15 && rand.nextInt(6) == 0) {
				...
				sporeling.setLocationAndAngles(pos.getX() + 0.5D, pos.getY(), pos.getZ() + 0.5D,
						rand.nextFloat() * 360.0F, 0.0F);
				worldIn.spawnEntity(sporeling);
				worldIn.setBlockToAir(pos);
				this.harvestAndUpdateSoil(worldIn, pos, 5);

				for (EntityPlayerMP playerMP : worldIn.getEntitiesWithinAABB(EntityPlayerMP.class,
						new AxisAlignedBB(pos, pos).grow(10.0D, 5.0D, 10.0D))) {
