				...
				if (open) {
					...
					if (!interacted && !heldItem.isEmpty()) {
						...
						if (compostRecipe != null) {
							...
							player.sendStatusMessage(new TextComponentTranslation("chat.compost.not.compostable"),
									true);
						}
						interacted = true;
					}
	...
	public void randomDisplayTick(IBlockState stateIn, World world, BlockPos pos, Random rand) {
		if (world.getTileEntity(pos) instanceof TileEntityCompostBin) {
			...
			if (!tile.isOpen() && !tile.isEmpty()) {
				BLParticles.DIRT_DECAY.spawn(world, pos.getX() + 0.2F + rand.nextFloat() * 0.62F,
						pos.getY() + rand.nextFloat() * 0.75F, pos.getZ() + 0.2F + rand.nextFloat() * 0.6F);
			}
		}
	}
}
