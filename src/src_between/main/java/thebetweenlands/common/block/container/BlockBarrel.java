		...
		if (world.getTileEntity(pos) instanceof TileEntityBarrel) {
			...
			if (!heldItem.isEmpty()) {
				IFluidHandler handler = heldItem.getCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY,
						null);
				if (handler != null) {
					IItemHandler playerInventory = player.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY,
							null);
					if (playerInventory != null) {
						...
						if (fluidActionResult.isSuccess()) {
							...
							fluidActionResult = FluidUtil.tryFillContainerAndStow(heldItem, tile, playerInventory,
									Integer.MAX_VALUE, player, !world.isRemote);
							if (fluidActionResult.isSuccess()) {
								if (!world.isRemote) {
									player.setHeldItem(hand, fluidActionResult.getResult());
								}
			...
			if (!world.isRemote && tile != null) {
				player.openGui(TheBetweenlands.instance, CommonProxy.GUI_BARREL, world, pos.getX(), pos.getY(),
						pos.getZ());
			}
		}
		...
		if (!worldIn.isRemote && !player.isCreative() && worldIn.getGameRules().getBoolean("doTileDrops")) {
			...
			if (te instanceof TileEntityBarrel) {
				...
				if (item instanceof ItemBarrel) {
					InventoryHelper.spawnItemStack(worldIn, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D,
							((ItemBarrel) item).fromBarrel((TileEntityBarrel) te));
				}
			}
		}
	}
