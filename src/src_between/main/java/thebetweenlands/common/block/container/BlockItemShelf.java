		...
		/* Ugly check so that it doesn't give 2 items when clicking with empty hand */) {
			...
			if (te instanceof TileEntityItemShelf) {
				...
				RayTraceResult ray = this.rayTrace(pos, player.getPositionEyes(1),
						player.getPositionEyes(1).add(player.getLookVec().scale(10)),
						this.getBoundingBox(state, world, pos));
				if (ray != null) {
					InvWrapper wrapper = new InvWrapper(shelf);

					int slot = this.getSlot(state.getValue(FACING), (float) (ray.hitVec.x - pos.getX()),
							(float) (ray.hitVec.y - pos.getY()), (float) (ray.hitVec.z - pos.getZ()));
