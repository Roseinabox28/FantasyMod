...
public class BlockPresent extends BasicBlock implements ITileEntityProvider, ITintedBlock {
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer,
			ItemStack stack) {
		if (!worldIn.isRemote) {
			state = state.withProperty(COLOR,
					EnumDyeColor.values()[worldIn.rand.nextInt(EnumDyeColor.values().length)]);
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		if (rand.nextInt(20) == 0
				&& worldIn.getClosestPlayer(pos.getX(), pos.getY(), pos.getZ(), 32.0D, false) == null) {
