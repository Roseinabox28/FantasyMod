...
public class BlockGeckoCage extends BlockContainer {
	...
	protected void spawnHeartParticles(World world, BlockPos pos) {
		for (int i = 0; i < 7; ++i) {
			...
			world.spawnParticle(EnumParticleTypes.HEART, pos.getX() + world.rand.nextFloat(),
					pos.getY() + world.rand.nextFloat(), pos.getZ() + world.rand.nextFloat(), d0, d1, d2, new int[0]);
		}
	}
			...
			if (!heldItemStack.isEmpty()) {
				...
				if (heldItem == ItemRegistry.GECKO) {
					if (!tile.hasGecko()) {
						if (!world.isRemote) {
							String name = "";
							if (!(heldItemStack.getDisplayName()
									.equals(TranslationHelper.translateToLocal(heldItemStack.getTranslationKey())))
									&& heldItemStack.hasDisplayName())
								name = heldItemStack.getDisplayName();

							tile.addGecko(
									heldItemStack.hasTagCompound() && heldItemStack.getTagCompound().hasKey("Health")
											? (int) heldItemStack.getTagCompound().getFloat("Health")
											: 12,
									name);
				...
				if (heldItem == ItemRegistry.SAP_SPIT && tile.hasGecko() && tile.getGeckoUsages() < 12) {
					...
					if (tile.hasGecko()) {
						if (DiscoveryContainer.hasDiscoveryProvider(player)) {
							if (!world.isRemote) {
								...
								if (aspects.size() > 0) {
									DiscoveryContainer<?> mergedKnowledge = DiscoveryContainer
											.getMergedDiscoveryContainer(player);
									AspectDiscovery discovery = mergedKnowledge.discover(manager, aspectItem);
									switch (discovery.result) {
									case NEW:
									case LAST:
										DiscoveryContainer.addDiscoveryToContainers(player, aspectItem,
												discovery.discovered.type);
										tile.setAspectType(discovery.discovered.type, 600);
										if (player instanceof EntityPlayerMP) {
											AdvancementCriterionRegistry.GECKO_TRIGGER.trigger((EntityPlayerMP) player,
													true, false);
											if (discovery.result == EnumDiscoveryResult.LAST && DiscoveryContainer
													.getMergedDiscoveryContainer(player).haveDiscoveredAll(manager))
												AdvancementCriterionRegistry.HERBLORE_FIND_ALL
														.trigger((EntityPlayerMP) player);
										}
										player.sendStatusMessage(new TextComponentTranslation("chat.aspect.discovery."
												+ discovery.discovered.type.getName().toLowerCase()), false);
										if (discovery.result == EnumDiscoveryResult.LAST) {
											player.sendStatusMessage(
													new TextComponentTranslation("chat.aspect.discovery.last"), true);
										} else {
											player.sendStatusMessage(
													new TextComponentTranslation("chat.aspect.discovery.more"), true);
										}
										if (!player.capabilities.isCreativeMode)
											heldItemStack.shrink(1);
										return true;
									case END:
										...
										player.sendStatusMessage(
												new TextComponentTranslation("chat.aspect.discovery.end"), true);
										return false;
									default:
										// no aspects
										player.sendStatusMessage(
												new TextComponentTranslation("chat.aspect.discovery.none"), true);
										return false;
									}
								} else {
									player.sendStatusMessage(new TextComponentTranslation("chat.aspect.discovery.none"),
											true);
									return true;
								}
							} else {
							...
							if (!world.isRemote)
								player.sendStatusMessage(
										new TextComponentTranslation("chat.aspect.discovery.book.none"), true);
							return false;
						}
					} else {
						// no gecko
						if (!world.isRemote)
							player.sendStatusMessage(new TextComponentTranslation("chat.aspect.discovery.gecko.none"),
									true);
						return false;
					}
				} else {
					// recovering
					if (!world.isRemote)
						player.sendStatusMessage(new TextComponentTranslation("chat.aspect.discovery.gecko.recovering"),
								true);
					return false;
				}
			}
