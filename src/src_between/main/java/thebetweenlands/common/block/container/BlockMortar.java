	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (worldIn.getTileEntity(pos) instanceof TileEntityMortar) {
			...
			if (!playerIn.getHeldItem(hand).isEmpty()) {
				...
				} else if (playerIn.getHeldItem(hand).getItem() == ItemRegistry.LIFE_CRYSTAL
						&& tile.getStackInSlot(3).isEmpty()) {
					tile.setInventorySlotContents(3, playerIn.getHeldItem(hand));
					playerIn.setHeldItem(hand, ItemStack.EMPTY);
				} else {
					playerIn.openGui(TheBetweenlands.instance, CommonProxy.GUI_PESTLE_AND_MORTAR, worldIn, pos.getX(),
							pos.getY(), pos.getZ());
				}
			} else {
				playerIn.openGui(TheBetweenlands.instance, CommonProxy.GUI_PESTLE_AND_MORTAR, worldIn, pos.getX(),
