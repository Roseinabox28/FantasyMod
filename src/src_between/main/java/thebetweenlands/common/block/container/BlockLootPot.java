...
public class BlockLootPot extends BasicBlock
		implements ITileEntityProvider, ICustomItemBlock, ISubtypeItemBlockModelDefinition, IStateMappedBlock {
	...
	public IBlockState getStateFromMeta(int meta) {
		return this.getDefaultState().withProperty(VARIANT, EnumLootPot.byMetadata(meta)).withProperty(FACING,
				...
				if (!playerIn.getHeldItem(hand).isEmpty()) {
					...
					for (int i = 0; i < wrapper.getSlots(); i++) {
						...
						if (!extracted.isEmpty()) {
							EntityItem item = new EntityItem(worldIn, pos.getX() + 0.5D, pos.getY() + 1.0D,
									pos.getZ() + 0.5D, extracted);
