...
public class BlockAspectVial extends BlockContainer implements BlockRegistry.ICustomItemBlock {
	public static final PropertyEnum<BlockDentrothyst.EnumDentrothyst> TYPE = PropertyEnum.create("type",
			BlockDentrothyst.EnumDentrothyst.class);
	...
	public BlockAspectVial() {
		...
		setDefaultState(this.blockState.getBaseState().withProperty(TYPE, BlockDentrothyst.EnumDentrothyst.GREEN)
				.withProperty(RANDOM_POSITION, false));
	}
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand,
		...
		if (world.getTileEntity(pos) instanceof TileEntityAspectVial) {
			...
			if (!player.getHeldItem(hand).isEmpty()) {
				...
				if (heldItem.getItem() == ItemRegistry.ASPECT_VIAL
						&& (container = ItemAspectContainer.fromItem(heldItem)).getAspects().size() == 1) {
				...
				} else if (heldItem.getItem() == ItemRegistry.DENTROTHYST_VIAL && player.isSneaking()
						&& tile.getAspect() != null && heldItem.getItemDamage() != 1) {
					if (!world.isRemote) {
	...
	public void onBlockHarvested(World world, BlockPos pos, IBlockState state, EntityPlayer player) {
		...
		if (!world.isRemote && !world.restoringBlockSnapshots && !player.capabilities.isCreativeMode) {
			...
			float chance = ForgeEventFactory.fireBlockHarvesting(drops, world, pos, world.getBlockState(pos), 0, 1,
