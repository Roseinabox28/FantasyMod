...
public class BlockWeedwoodWorkbench extends BlockContainer {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand,
		...
		if (!world.isRemote) {
			player.openGui(TheBetweenlands.instance, CommonProxy.GUI_WEEDWOOD_CRAFT, world, pos.getX(), pos.getY(),
					pos.getZ());
		}
		return true;
	}
	...
	public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer,
			ItemStack stack) {
		((TileEntityWeedwoodWorkbench) world.getTileEntity(
				pos)).rotation = (byte) (((MathHelper.floor((double) (placer.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3)
						+ 1) % 4);
	}
	...
	public void breakBlock(World world, BlockPos pos, IBlockState state) {
		...
		for (ItemStack stack : tile.craftingSlots) {
			if (!stack.isEmpty()) {
				...
				EntityItem item = new EntityItem(world, pos.getX() + offsetX, pos.getY() + offsetY,
