	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand,
		...
		if (world.getTileEntity(pos) instanceof TileEntityCenser) {
			...
			if (!world.isRemote && tile != null) {
				player.openGui(TheBetweenlands.instance, CommonProxy.GUI_CENSER, world, pos.getX(), pos.getY(),
						pos.getZ());
			}
		}
		return true;
				...
				if (recipe != null) {
					...
					for (int i = 0; i < 3 + rand.nextInt(5); i++) {
						BatchedParticleRenderer.INSTANCE.addParticle(
								...
								BLParticles.SMOOTH_SMOKE.create(worldIn, pos.getX() + 0.5F, pos.getY() + 0.5F,
										pos.getZ() + 0.5F,
										ParticleArgs.get().withMotion((rand.nextFloat() - 0.5f) * 0.08f,
												rand.nextFloat() * 0.01F + 0.005F, (rand.nextFloat() - 0.5f) * 0.08f)
												.withScale(2.0f + rand.nextFloat() * 8.0F).withColor(r, g, b, 0.1f)
