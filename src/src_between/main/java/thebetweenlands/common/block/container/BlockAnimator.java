...
public class BlockAnimator extends BlockContainer {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand,
		...
		if (world.getTileEntity(pos) instanceof TileEntityAnimator) {
			...
			if (!animator.itemAnimated) {
				player.openGui(TheBetweenlands.instance, CommonProxy.GUI_ANIMATOR, world, pos.getX(), pos.getY(),
						pos.getZ());
			} else {
	...
	public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
		...
		if (te != null && te.isSlotInUse(0) && te.isCrystalInslot() && te.isSulfurInSlot()
				&& te.fuelConsumed < te.requiredFuelCount && te.isValidFocalItem()) {
			...
			points.add(new Vec3d(te.getPos().getX() + 0.5D + (rand.nextFloat() - 0.5F) * 0.3D + xOff,
					te.getPos().getY() + 0.9, te.getPos().getZ() + 0.5 + (rand.nextFloat() - 0.5F) * 0.3D + zOff));
			points.add(new Vec3d(te.getPos().getX() + 0.5D + (rand.nextFloat() - 0.5F) * 0.3D + xOff,
					te.getPos().getY() + 1.36, te.getPos().getZ() + 0.5 + (rand.nextFloat() - 0.5F) * 0.3D + zOff));
			points.add(new Vec3d(te.getPos().getX() + 0.5D, te.getPos().getY() + 1.45D, te.getPos().getZ() + 0.5D));
			BLParticles.ANIMATOR.spawn(worldIn, te.getPos().getX(), te.getPos().getY() + 0.9, te.getPos().getZ() + 0.65,
					ParticleArgs.get().withData(points));
			BLParticles.SMOKE.spawn(worldIn, te.getPos().getX() + 0.5 + rand.nextFloat() * 0.3D - 0.15D,
					te.getPos().getY() + 0.3, te.getPos().getZ() + 0.5 + rand.nextFloat() * 0.3D - 0.15D);
		}
	}
	...
	public int getComparatorInputOverride(IBlockState blockState, World world, BlockPos pos) {
		...
		if (tileEntityAnimator instanceof TileEntityAnimator) {
			return Math.round(((float) ((TileEntityAnimator) tileEntityAnimator).fuelConsumed
					/ (float) ((TileEntityAnimator) tileEntityAnimator).requiredFuelCount) * 16.0f);
		}
		return 0;
	}
