...
public class BlockInfuser extends BlockContainer {
		...
		if (!world.isRemote && tileEntity instanceof TileEntityInfuser) {
			...
			if (!player.isSneaking()) {
				...
				if (!heldItem.isEmpty() && !tile.hasInfusion()) {
					ItemAspectContainer aspectContainer = ItemAspectContainer.fromItem(heldItem,
							AspectManager.get(world));
							...
							if (tile.getStackInSlot(i).isEmpty()) {
								...
								if (tile.getWaterAmount() > 0) {
									world.playSound(null, pos, SoundEvents.ENTITY_GENERIC_SPLASH, SoundCategory.BLOCKS,
											0.3f, 0.9f + world.rand.nextFloat() * 0.3f);
								} else {
									world.playSound(null, pos, SoundEvents.ENTITY_ITEMFRAME_ADD_ITEM,
											SoundCategory.BLOCKS, 0.3f, 0.9f + world.rand.nextFloat() * 0.3f);
								}
								return true;
							}
						}
			...
			if (player.isSneaking()) {
				if (heldItem.getItem() instanceof ItemBLBucket
						&& ((ItemBLBucket) heldItem.getItem()).getFluid(heldItem) == null && tile.hasInfusion()
						&& tile.getWaterAmount() >= Fluid.BUCKET_VOLUME) {
					ItemStack infusionBucket = new ItemStack(ItemRegistry.BL_BUCKET_INFUSION, 1,
				...
				if (!tile.getStackInSlot(TileEntityInfuser.MAX_INGREDIENTS + 1).isEmpty()) {
					EntityItem itemEntity = player
							.dropItem(tile.getStackInSlot(TileEntityInfuser.MAX_INGREDIENTS + 1).copy(), false);
					if (itemEntity != null)
						itemEntity.setPickupDelay(0);
		...
		if (world.getTileEntity(pos) instanceof TileEntityInfuser) {
			...
			if (infuser.getWaterAmount() > 0 && infuser.getTemperature() > 0) {
				...
				if (rand.nextInt((101 - infuser.getTemperature())) / 4 == 0) {
					...
					BLParticles.BUBBLE_INFUSION.spawn(world, xx + 0.3F - rand.nextFloat() * 0.6F, yy,
							zz + 0.3F - rand.nextFloat() * 0.6F,
							ParticleArgs.get().withScale(0.3F).withColor(colors[0], colors[1], colors[2], 1));
					if (rand.nextInt(10) == 0 && infuser.getTemperature() > 70)
						world.playSound(xx, yy, zz, SoundEvents.BLOCK_LAVA_AMBIENT, SoundCategory.BLOCKS,
								1.2F + rand.nextFloat() * 0.2F, 0.9F + rand.nextFloat() * 0.5F, false);
				}
				if (infuser.getTemperature() >= 100) {
					BLParticles.STEAM_PURIFIER.spawn(world, (double) (xx - fixedOffset), (double) y + 0.75D,
							(double) (zz + randomOffset));
					BLParticles.STEAM_PURIFIER.spawn(world, (double) (xx + fixedOffset), (double) y + 0.75D,
							(double) (zz + randomOffset));
					BLParticles.STEAM_PURIFIER.spawn(world, (double) (xx + randomOffset), (double) y + 0.75D,
							(double) (zz - fixedOffset));
					BLParticles.STEAM_PURIFIER.spawn(world, (double) (xx + randomOffset), (double) y + 0.75D,
							(double) (zz + fixedOffset));
				}
			}
	...
	public void fillWithRain(World world, BlockPos pos) {
		if (world.provider.getDimension() == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId
				&& world.getTileEntity(pos) instanceof TileEntityInfuser) {
			TileEntityInfuser tile = (TileEntityInfuser) world.getTileEntity(pos);
