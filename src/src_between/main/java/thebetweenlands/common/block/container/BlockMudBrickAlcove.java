...
public class BlockMudBrickAlcove extends BasicBlock implements ITileEntityProvider {
	...
	protected BlockStateContainer createBlockState() {
		return new ExtendedBlockState(this, new IProperty[] { FACING, HAS_URN },
				new IUnlistedProperty[] { LEVEL, TOP_COBWEB, BOTTOM_COBWEB, SMALL_CANDLE, BIG_CANDLE });
	}

	@Override
	public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		TileEntityMudBrickAlcove tile = StatePropertyHelper.getTileEntityThreadSafe(worldIn, pos,
				TileEntityMudBrickAlcove.class);
		if (tile != null) {
	...
	public IBlockState getExtendedState(IBlockState oldState, IBlockAccess worldIn, BlockPos pos) {
		...
		TileEntityMudBrickAlcove tile = StatePropertyHelper.getTileEntityThreadSafe(worldIn, pos,
				TileEntityMudBrickAlcove.class);
		if (tile != null) {
			extended = extended.withProperty(TOP_COBWEB, tile.topWeb).withProperty(BOTTOM_COBWEB, tile.bottomWeb)
					.withProperty(SMALL_CANDLE, tile.smallCandle).withProperty(BIG_CANDLE, tile.bigCandle)
	...
	public void onBlockClicked(World world, BlockPos pos, EntityPlayer player) {
		if (!world.isRemote) {
			...
			if (tile != null && tile.hasUrn) {
				...
				RayTraceResult ray = this.rayTrace(pos, player.getPositionEyes(1),
						player.getPositionEyes(1).add(player.getLookVec().scale(10)), state.getBoundingBox(world, pos));
				...
				if (ray != null && state.getValue(FACING) == ray.sideHit) {
					...
					if (world.rand.nextInt(3) == 0) {
						...
						entity.setLocationAndAngles(offsetPos.getX() + 0.5D, offsetPos.getY(), offsetPos.getZ() + 0.5D,
								0.0F, 0.0F);
					...
					world.playEvent(null, 2001, pos, Block.getIdFromBlock(BlockRegistry.MUD_FLOWER_POT)); // this will
																											// do unless
																											// we want
																											// specific
																											// particles
