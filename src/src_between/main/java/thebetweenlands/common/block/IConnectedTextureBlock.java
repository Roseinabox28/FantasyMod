...
public interface IConnectedTextureBlock {
	public static interface IConnectionRules {
		...
		/**
		 * Returns whether the face can connect through the specified block pos. The
		 * default implementation lets the texture connect through any non solid sides
		 * 
		 * @param world
		 * @param pos   The position of this connected texture block
		 * @param face  The face of this connected texture block
		 ...
		 * @return
		 */
		public default boolean canConnectThrough(IBlockAccess world, BlockPos pos, EnumFacing face,
				MutableBlockPos to) {
			// The block tries to connect to one of its own other sides
			Axis axis = face.getAxis();
			BlockPos planeOffset = new BlockPos(axis == Axis.X ? 0 : (to.getX() - pos.getX()),
					axis == Axis.Y ? 0 : (to.getY() - pos.getY()), axis == Axis.Z ? 0 : (to.getZ() - pos.getZ()));
			boolean onSamePlane = (axis != Axis.X || (to.getX() - pos.getX()) == 0)
					&& (axis != Axis.Y || (to.getY() - pos.getY()) == 0)
					&& (axis != Axis.Z || (to.getZ() - pos.getZ()) == 0);
			if (Math.abs(planeOffset.getX()) + Math.abs(planeOffset.getY()) + Math.abs(planeOffset.getZ()) > 1) {
				...
				for (EnumFacing offsetFace : EnumFacing.VALUES) {
					...
					if (Math.abs(planeOffset.getX() + offsetFace.getXOffset())
							+ Math.abs(planeOffset.getY() + offsetFace.getYOffset())
							+ Math.abs(planeOffset.getZ() + offsetFace.getZOffset()) == 1) {
						// Check if either sides of the diagonal block that point to a direct neighbour
						// of the connected texture block are solid
						if (onSamePlane) {
							if (world.isSideSolid(to, offsetFace, false) || world.isSideSolid(to, face, false)) {
								return false;
							}
						} else {
							if (world.isSideSolid(to, offsetFace, false)
									|| world.isSideSolid(to, face.getOpposite(), false)) {
								return false;
							}
						}
						directNeighbour.setPos(to.getX() + offsetFace.getXOffset(), to.getY() + offsetFace.getYOffset(),
								to.getZ() + offsetFace.getZOffset());
						// Check if direct neighbour can be connected to, if not it needs to check if it
						// lets a conncetion through
						if (!this.canConnectTo(world, pos, face, directNeighbour)) {
							// Check if either sides of the direct neighbour block of the connected texture
							// block are solid
							if (onSamePlane) {
								if (world.isSideSolid(directNeighbour, offsetFace.getOpposite(), false)
										|| world.isSideSolid(directNeighbour, face, false)) {
									return false;
								}
							} else {
								if (world.isSideSolid(directNeighbour, offsetFace.getOpposite(), false)
										|| world.isSideSolid(directNeighbour, face.getOpposite(), false)) {
									return false;
								}
							}
						}
				...
				// Direct neighbour
				EnumFacing offsetDir = EnumFacing.getFacingFromVector(planeOffset.getX(), planeOffset.getY(),
						planeOffset.getZ());
				if (onSamePlane) {
					return !world.isSideSolid(to, offsetDir.getOpposite(), false)
							&& !world.isSideSolid(to, face, false);
				} else {
					return !world.isSideSolid(to, offsetDir.getOpposite(), false)
							&& !world.isSideSolid(to, face.getOpposite(), false);
				}
	...
	// -1, +1, -1, quadrant 0
	public static final IUnlistedProperty<Integer> TOP_NORTH_WEST_INDEX = new PropertyIntegerUnlisted(
			"top_north_west_index");
	// +1, +1, -1, quadrant 1
	public static final IUnlistedProperty<Integer> TOP_NORTH_EAST_INDEX = new PropertyIntegerUnlisted(
			"top_north_east_index");
	// -1, +1, +1, quadrant 2
	public static final IUnlistedProperty<Integer> TOP_SOUTH_WEST_INDEX = new PropertyIntegerUnlisted(
			"top_south_west_index");
	// +1, +1, +1, quadrant 3
	public static final IUnlistedProperty<Integer> TOP_SOUTH_EAST_INDEX = new PropertyIntegerUnlisted(
			"top_south_east_index");

	// +1, -1, -1, quadrant 0
	public static final IUnlistedProperty<Integer> BOTTOM_NORTH_EAST_INDEX = new PropertyIntegerUnlisted(
			"bottom_north_east_index");
	// -1, -1, -1, quadrant 1
	public static final IUnlistedProperty<Integer> BOTTOM_NORTH_WEST_INDEX = new PropertyIntegerUnlisted(
			"bottom_north_west_index");
	// +1, -1, +1, quadrant 2
	public static final IUnlistedProperty<Integer> BOTTOM_SOUTH_EAST_INDEX = new PropertyIntegerUnlisted(
			"bottom_south_east_index");
	// -1, -1, +1, quadrant 3
	public static final IUnlistedProperty<Integer> BOTTOM_SOUTH_WEST_INDEX = new PropertyIntegerUnlisted(
			"bottom_south_west_index");

	// -1, +1, -1, quadrant 0
	public static final IUnlistedProperty<Integer> NORTH_UP_WEST_INDEX = new PropertyIntegerUnlisted(
			"north_up_west_index");
	// +1, +1, -1, quadrant 1
	public static final IUnlistedProperty<Integer> NORTH_UP_EAST_INDEX = new PropertyIntegerUnlisted(
			"north_up_east_index");
	// -1, -1, -1, quadrant 2
	public static final IUnlistedProperty<Integer> NORTH_DOWN_WEST_INDEX = new PropertyIntegerUnlisted(
			"north_down_west_index");
	// +1, -1, -1, quadrant 3
	public static final IUnlistedProperty<Integer> NORTH_DOWN_EAST_INDEX = new PropertyIntegerUnlisted(
			"north_down_east_index");

	// -1, -1, +1, quadrant 0
	public static final IUnlistedProperty<Integer> SOUTH_DOWN_WEST_INDEX = new PropertyIntegerUnlisted(
			"south_down_west_index");
	// +1, -1, +1, quadrant 1
	public static final IUnlistedProperty<Integer> SOUTH_DOWN_EAST_INDEX = new PropertyIntegerUnlisted(
			"south_down_east_index");
	// -1, +1, +1, quadrant 2
	public static final IUnlistedProperty<Integer> SOUTH_UP_WEST_INDEX = new PropertyIntegerUnlisted(
			"south_up_west_index");
	// +1, +1, +1, quadrant 3
	public static final IUnlistedProperty<Integer> SOUTH_UP_EAST_INDEX = new PropertyIntegerUnlisted(
			"south_up_east_index");

	// -1, -1, -1, quadrant 0
	public static final IUnlistedProperty<Integer> WEST_DOWN_NORTH_INDEX = new PropertyIntegerUnlisted(
			"west_down_north_index");
	// -1, -1, +1, quadrant 1
	public static final IUnlistedProperty<Integer> WEST_DOWN_SOUTH_INDEX = new PropertyIntegerUnlisted(
			"west_down_south_index");
	// -1, +1, -1, quadrant 2
	public static final IUnlistedProperty<Integer> WEST_UP_NORTH_INDEX = new PropertyIntegerUnlisted(
			"west_up_north_index");
	// -1, +1, +1, quadrant 3
	public static final IUnlistedProperty<Integer> WEST_UP_SOUTH_INDEX = new PropertyIntegerUnlisted(
			"west_up_south_index");

	// +1, -1, +1, quadrant 0
	public static final IUnlistedProperty<Integer> EAST_DOWN_SOUTH_INDEX = new PropertyIntegerUnlisted(
			"east_down_south_index");
	// +1, -1, -1, quadrant 1
	public static final IUnlistedProperty<Integer> EAST_DOWN_NORTH_INDEX = new PropertyIntegerUnlisted(
			"east_down_north_index");
	// +1, +1, +1, quadrant 2
	public static final IUnlistedProperty<Integer> EAST_UP_SOUTH_INDEX = new PropertyIntegerUnlisted(
			"east_up_south_index");
	// +1, +1, -1, quadrant 3
	public static final IUnlistedProperty<Integer> EAST_UP_NORTH_INDEX = new PropertyIntegerUnlisted(
			"east_up_north_index");
	...
	public default ExtendedBlockState getConnectedTextureBlockStateContainer(ExtendedBlockState container) {
		...
		return BlockStateContainerHelper.extendBlockstateContainer(container, new IProperty[0],
				props.toArray(new IUnlistedProperty[0]));
	}
	 ...
	 * @return
	 */
	public default IBlockState getExtendedConnectedTextureState(IExtendedBlockState state, IBlockAccess world,
			BlockPos pos, Predicate<MutableBlockPos> canConnectTo, boolean connectToSelf) {
		IConnectionRules connectionRules = new IConnectionRules() {
			...
			public boolean canConnectThrough(IBlockAccess world, BlockPos pos, EnumFacing face, MutableBlockPos to) {
				...
				Axis axis = face.getAxis();
				if ((axis == Axis.X && to.getX() - pos.getX() != 0) || (axis == Axis.Y && to.getY() - pos.getY() != 0)
						|| (axis == Axis.Z && to.getZ() - pos.getZ() != 0)) {
					// Tries to connect through the block that is next to the connected texture
					// face. This should always be true
					// if the block can't connect to its own faces because otherwise it wouldn't be
	 ...
	 * @param world
	 * @param pos
	 * @param connectionRules The connection rules determines which blocks this
	 *                        block can connect to and which blocks let a connection
	 *                        through
	 * @return
	 */
	public default IBlockState getExtendedConnectedTextureState(IExtendedBlockState state, IBlockAccess world,
			BlockPos pos, IConnectionRules connectionRules) {
		world = connectionRules.getBlockAccessCache(world, pos);

		if (this.isFaceConnectedTexture(EnumFacing.UP)) {
			int[] quadrantIndicesUp = getQuadrantIndices(getConnectionArray(world, pos, EnumFacing.UP, connectionRules),
					false);
		...
		if (this.isFaceConnectedTexture(EnumFacing.DOWN)) {
			int[] quadrantIndicesDown = getQuadrantIndices(
					getConnectionArray(world, pos, EnumFacing.DOWN, connectionRules), true);
		...
		if (this.isFaceConnectedTexture(EnumFacing.NORTH)) {
			int[] quadrantIndicesNorth = getQuadrantIndices(
					getConnectionArray(world, pos, EnumFacing.NORTH, connectionRules), false);
		...
		if (this.isFaceConnectedTexture(EnumFacing.SOUTH)) {
			int[] quadrantIndicesSouth = getQuadrantIndices(
					getConnectionArray(world, pos, EnumFacing.SOUTH, connectionRules), false);
		...
		if (this.isFaceConnectedTexture(EnumFacing.WEST)) {
			int[] quadrantIndicesWest = getQuadrantIndices(
					getConnectionArray(world, pos, EnumFacing.WEST, connectionRules), true);
		...
		if (this.isFaceConnectedTexture(EnumFacing.EAST)) {
			int[] quadrantIndicesEast = getQuadrantIndices(
					getConnectionArray(world, pos, EnumFacing.EAST, connectionRules), true);
	...
	/**
	 * Returns the quadrant indices
	 * <p>
	 * <p>
	 *
	 * @param connectionArray
	 *                        <p>
	 *                        Connection states, index 4 is the center:
	 * 
	 *                        <pre>
	 *                                                -------
	 *                                               | 0 1 2 |
	 ...
	 *                        </pre>
	 * 
	 * @param flipXZ          Flips the X and Z axis, used for EAST, WEST and DOWN
	 *                        face
	 * @return
	 *         <p>
	 *         Returned index positions:
	 * 
	 *         <pre>
	 *  -------> 
	 * | 0-----1
			...
			for (int zo = 0; zo <= 2; zo++) {
				...
				if ((xo != 1 && zo == 1) || (xo == 1 && zo != 1)) {
					...
					if (currentNeighbourState) {
						if (xo == 0) {
							if (!connectionArray[getIndex(1, 2, 3)])
								bls = segXPath;
							if (!connectionArray[getIndex(1, 0, 3)])
								tls = segXPath;
						} else if (xo == 2) {
							if (!connectionArray[getIndex(1, 2, 3)])
								brs = segXPath;
							...
							if (!connectionArray[getIndex(0, 1, 3)])
								tls = segZPath;
							if (!connectionArray[getIndex(2, 1, 3)])
								trs = segZPath;
						} else if (zo == 2) {
							if (!connectionArray[getIndex(0, 1, 3)])
								bls = segZPath;
	 ...
	 * @param pos
	 * @param dir               Face
	 * @param canConnectTo      Returns whether this block can connect to the
	 *                          specified block pos
	 * @param canConnectThrough Returns whether this block can connect through the
	 *                          specified block pos;
	 * @return Connection array
	 */
	public static boolean[] getConnectionArray(IBlockAccess world, BlockPos pos, EnumFacing dir,
		...
		for (int xo = xr ? -1 : 0; xo <= (xr ? 1 : 0); xo++) {
			for (int yo = yr ? -1 : 0; yo <= (yr ? 1 : 0); yo++) {
				for (int zo = zr ? -1 : 0; zo <= (zr ? 1 : 0); zo++) {
					...
					if (connectionRules.canConnectThrough(world, pos, dir,
							checkPos.setPos(x + dir.getXOffset(), y + dir.getYOffset(), z + dir.getZOffset()))) {
						Axis axis = dir.getAxis();
						if ((axis == Axis.X && (yo != 0 || zo != 0)) || (axis == Axis.Y && (xo != 0 || zo != 0))
								|| (axis == Axis.Z && (xo != 0 || yo != 0))) {
							MutableBlockPos diagPos = checkPos.setPos(
									axis == Axis.X ? (x + dir.getXOffset()) : (x + xo),
									axis == Axis.Y ? (y + dir.getYOffset()) : (y + yo),
									axis == Axis.Z ? (z + dir.getZOffset()) : (z + zo));
							...
							if (isDiagConnectable || connectionRules.canConnectThrough(world, pos, dir, diagPos)) {
								MutableBlockPos obstructionPos = checkPos.setPos(axis == Axis.X ? x : (x + xo),
										axis == Axis.Y ? y : (y + yo), axis == Axis.Z ? z : (z + zo));
								if (isDiagConnectable
										|| connectionRules.canConnectThrough(world, pos, dir, obstructionPos)) {
									connectionArray[blockIndex] = true;
								} else {
									connectionArray[blockIndex] = connectionRules.canConnectTo(world, pos, dir,
