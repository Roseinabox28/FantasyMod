...
public class BlockWoodenSupportBeam extends BlockHorizontal {
	...
	// Entities should be able to path and walk through this otherwise it blocks the
	// dungeon in some areas.
	@Override
	@Nullable
	...
	public IBlockState getStateForPlacement(World world, BlockPos pos, EnumFacing facing, float hitX, float hitY,
		...
		if (canPlaceAt(world, pos, facing))
			return facing != EnumFacing.DOWN && (facing == EnumFacing.UP || (double) hitY <= 0.5D)
					? state.withProperty(TOP, false)
					: state.withProperty(TOP, true);
