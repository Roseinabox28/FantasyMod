...
public class BlockMudTiles extends Block implements ICustomItemBlock, ISubtypeItemBlockModelDefinition {
	...
	public static final PropertyEnum<EnumMudTileType> VARIANT = PropertyEnum.<EnumMudTileType>create("variant",
			EnumMudTileType.class);
	...
	public BlockMudTiles() {
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(VARIANT, EnumMudTileType.MUD_TILES)
				.withProperty(CONNECTED_DOWN, Boolean.FALSE).withProperty(CONNECTED_EAST, Boolean.FALSE)
	...
	protected BlockStateContainer createBlockState() {
		return new BlockStateContainer(this, new IProperty[] { VARIANT, CONNECTED_DOWN, CONNECTED_UP, CONNECTED_NORTH,
	...
	private boolean isSideConnectable(IBlockAccess world, BlockPos pos, EnumFacing side) {
		...
		if (stateConnection.getBlock() == this && state.getBlock() == this) {
			if (state.getValue(VARIANT) == EnumMudTileType.MUD_TILES
					|| state.getValue(VARIANT) == EnumMudTileType.MUD_TILES_CRACKED)
				if (stateConnection.getValue(VARIANT) == EnumMudTileType.MUD_TILES_DECAY
						|| stateConnection.getValue(VARIANT) == EnumMudTileType.MUD_TILES_CRACKED_DECAY
						|| stateConnection.getValue(VARIANT) == EnumMudTileType.MUD_TILES
						|| stateConnection.getValue(VARIANT) == EnumMudTileType.MUD_TILES_CRACKED)
					return false;
			if (state.getValue(VARIANT) == EnumMudTileType.MUD_TILES_DECAY
					|| state.getValue(VARIANT) == EnumMudTileType.MUD_TILES_CRACKED_DECAY)
				if (stateConnection.getValue(VARIANT) == EnumMudTileType.MUD_TILES
						|| stateConnection.getValue(VARIANT) == EnumMudTileType.MUD_TILES_CRACKED)
					return false;
		}
	...
	public float getSlipperiness(IBlockState state, IBlockAccess world, BlockPos pos, @Nullable Entity entity) {
		if (entity instanceof IEntityBL == false && (state.getValue(VARIANT) == EnumMudTileType.MUD_TILES_DECAY
				|| state.getValue(VARIANT) == EnumMudTileType.MUD_TILES_CRACKED_DECAY)) {
			return 0.98F;
		}
		return 0.6F;
	...
	public static enum EnumMudTileType implements IStringSerializable {
		MUD_TILES, MUD_TILES_DECAY, MUD_TILES_CRACKED, MUD_TILES_CRACKED_DECAY;
