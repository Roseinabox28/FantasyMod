...
public class BlockDruidStone extends BasicBlock implements BlockRegistry.ISubtypeItemBlockModelDefinition {
	...
	public IBlockState getStateFromMeta(int meta) {
		return getDefaultState().withProperty(FACING, EnumFacing.byHorizontalIndex(meta)).withProperty(ACTIVE,
				(meta & 4) != 0);
	}
			...
			for (EnumFacing facing : EnumFacing.VALUES) {
				...
				if (!world.getBlockState(side).isOpaqueCube()) {
					...
					BLParticles.DRUID_CASTING_BIG.spawn(world, particleX, particleY, particleZ,
							ParticleArgs.get()
									.withMotion((rand.nextFloat() - rand.nextFloat()) * 0.1, 0,
											(rand.nextFloat() - rand.nextFloat()) * 0.1)
									.withScale(rand.nextFloat() * 0.5F + 0.5F).withColor(0.8F + rand.nextFloat() * 0.2F,
											0.8F + rand.nextFloat() * 0.2F, 0.8F, 1));
				}
			}
		}
	}
