...
public class BlockDungeonDoorRunes extends BasicBlock implements ITileEntityProvider, IStateMappedBlock {
	...
	public BlockDungeonDoorRunes(Material material, boolean mimic, boolean barrishee) {
		...
		setDefaultState(
				blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH).withProperty(INVISIBLE, false));
		setLightOpacity(255);
	...
	public IBlockState getStateFromMeta(int meta) {
		return getDefaultState().withProperty(FACING, EnumFacing.byIndex(meta & 0b111)).withProperty(INVISIBLE,
				...
				for (int z = -1; z <= 1; z++)
					for (int y = -1; y <= 1; y++)
						if (pos.add(0, y, z) != pos)
							world.setBlockState(pos.add(0, y, z),
									invisiBlock.withProperty(FACING, state.getValue(FACING)));
			}
			if (state.getValue(FACING) == EnumFacing.NORTH || state.getValue(FACING) == EnumFacing.SOUTH) {
				for (int x = -1; x <= 1; x++)
					for (int y = -1; y <= 1; y++) {
						if (pos.add(x, y, 0) != pos)
							world.setBlockState(pos.add(x, y, 0),
									invisiBlock.withProperty(FACING, state.getValue(FACING)));
					}
			}
		}
