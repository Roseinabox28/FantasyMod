...
public class BlockPuffshroom extends Block implements ITileEntityProvider {
	...
	protected boolean tryHarvestWithShears(World world, BlockPos pos, IBlockState state, EntityPlayer player,
			ItemStack stack) {
			...
			if (tile instanceof TileEntityPuffshroom) {
				...
				if (puffshroom.animation_1 >= 1) {
					if (!world.isRemote && world instanceof WorldServer) {
						LootTable lootTable = ((WorldServer) world).getLootTableManager()
								.getLootTableFromLocation(LootTableRegistry.PUFFSHROOM);
						LootContext.Builder lootBuilder = new LootContext.Builder((WorldServer) world);

						List<ItemStack> loot = lootTable.generateLootForPools(((WorldServer) world).rand,
								lootBuilder.build());
						...
						world.setBlockState(pos, BlockRegistry.MUD_TILES.getDefaultState()
								.withProperty(BlockMudTiles.VARIANT, EnumMudTileType.MUD_TILES_CRACKED), 3);
	...
	public boolean removedByPlayer(IBlockState state, World world, BlockPos pos, EntityPlayer player,
		...
		return world.setBlockState(pos, BlockRegistry.MUD_TILES.getDefaultState().withProperty(BlockMudTiles.VARIANT,
				EnumMudTileType.MUD_TILES_CRACKED), world.isRemote ? 11 : 3);
	}
