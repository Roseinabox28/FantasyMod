...
public class BlockSlabBetweenlands extends BasicBlock implements BlockRegistry.ICustomItemBlock {
	public static final PropertyEnum<EnumBlockHalfBL> HALF = PropertyEnum.<EnumBlockHalfBL>create("half",
			EnumBlockHalfBL.class);
	...
	public boolean doesSideBlockRendering(IBlockState state, IBlockAccess world, BlockPos pos, EnumFacing face) {
		return (state.getValue(HALF).equals(EnumBlockHalfBL.BOTTOM) && face == EnumFacing.DOWN)
				|| (state.getValue(HALF).equals(EnumBlockHalfBL.TOP) && face == EnumFacing.UP)
				|| state.getValue(HALF).equals(EnumBlockHalfBL.FULL);
	}
	...
	public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY,
		...
		return state.getValue(HALF).equals(EnumBlockHalfBL.FULL) ? state
				: (facing != EnumFacing.DOWN && (facing == EnumFacing.UP || (double) hitY <= 0.5D)
						? state.withProperty(HALF, EnumBlockHalfBL.BOTTOM)
						: state.withProperty(HALF, EnumBlockHalfBL.TOP));
	}
	...
	public BlockFaceShape getBlockFaceShape(IBlockAccess worldIn, IBlockState state, BlockPos pos, EnumFacing face) {
		if (state.getValue(HALF) == EnumBlockHalfBL.FULL) {
			...
			return face == EnumFacing.DOWN && state.getValue(HALF) == EnumBlockHalfBL.BOTTOM ? BlockFaceShape.SOLID
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (!heldItem.isEmpty() && ((state.getValue(HALF).equals(EnumBlockHalfBL.TOP) && facing.equals(EnumFacing.DOWN))
				|| (state.getValue(HALF).equals(EnumBlockHalfBL.BOTTOM) && facing.equals(EnumFacing.UP)))) {
