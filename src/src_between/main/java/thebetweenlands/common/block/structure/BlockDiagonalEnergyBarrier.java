...
public class BlockDiagonalEnergyBarrier extends Block implements IConnectedTextureBlock {
	...
	protected BlockStateContainer createBlockState() {
		return this.getConnectedTextureBlockStateContainer(
				new ExtendedBlockState(this, new IProperty[] { FLIPPED }, new IUnlistedProperty[0]));
	}
	...
	public IBlockState getExtendedState(IBlockState state, IBlockAccess world, BlockPos pos) {
		...
		return this.getExtendedConnectedTextureState((IExtendedBlockState) state, world, pos, p -> {
			...
			if ((p.getY() != pos.getY() && xzSteps == 0) || xzSteps > 1) {
				return otherState.getBlock() instanceof BlockDiagonalEnergyBarrier
						&& otherState.getValue(FLIPPED) == flipped;
			}
	...
	public RayTraceResult collisionRayTrace(IBlockState blockState, World worldIn, BlockPos pos, Vec3d start,
		...
		if (result != null) {
			...
			Vec3d n = blockState.getValue(FLIPPED) ? new Vec3d(0.70710678118D, 0, -0.70710678118D)
					: new Vec3d(0.70710678118D, 0, 0.70710678118D);

			double d = p0.subtract(start).dotProduct(n) / (dir.dotProduct(n));

			Vec3d intercept = start.add(dir.scale(d));

			if (intercept.x >= pos.getX() && intercept.x <= pos.getX() + 1 && intercept.y >= pos.getY()
					&& intercept.y <= pos.getY() + 1 && intercept.z >= pos.getZ() && intercept.z <= pos.getZ() + 1) {
	...
	public void onEntityCollision(World world, BlockPos pos, IBlockState state, Entity entity) {
		 ...
		 * entity; if (!player.isSpectator()) {
		 * entity.attackEntityFrom(DamageSource.MAGIC, 1); double dx = (entity.posX -
		 * (pos.getX())) * 2 - 1; double dz = (entity.posZ - (pos.getZ())) * 2 - 1; if
		 * (Math.abs(dx) > Math.abs(dz)) dz = 0; else dx = 0; dx = (int) dx; dz = (int)
		 * dz; entity.addVelocity(dx * 0.85D, 0.08D, dz * 0.85D);
