...
public class BlockWaystone extends BlockContainer {
	...
	public BlockWaystone() {
		...
		this.setDefaultState(
				this.blockState.getBaseState().withProperty(PART, Part.BOTTOM).withProperty(ACTIVE, false));
	}
	...
	public IBlockState getStateFromMeta(int meta) {
		return this.getDefaultState().withProperty(PART, Part.fromMeta(meta & 0b11)).withProperty(ACTIVE,
				(meta & 0b100) != 0);
	}
	...
	public boolean canPlaceBlockAt(World world, BlockPos pos) {
		return super.canPlaceBlockAt(world, pos) && super.canPlaceBlockAt(world, pos.up())
				&& super.canPlaceBlockAt(world, pos.up(2));
	}
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
		...
		List<LocationStorage> waystoneLocations = localStorageHandler.getLocalStorages(LocationStorage.class,
				new AxisAlignedBB(pos), storage -> storage.getType() == EnumLocationType.WAYSTONE);
	...
	public boolean isValidWaystone(World world, BlockPos pos, IBlockState state) {
		...
		case TOP: {
			...
			return down1.getBlock() == this && down1.getValue(PART) == Part.MIDDLE && down2.getBlock() == this
					&& down2.getValue(PART) == Part.BOTTOM;
		}
		case MIDDLE: {
			IBlockState down1 = world.getBlockState(pos.down());
			IBlockState up1 = world.getBlockState(pos.up());
			return down1.getBlock() == this && down1.getValue(PART) == Part.BOTTOM && up1.getBlock() == this
					&& up1.getValue(PART) == Part.TOP;
		}
		default:
		case BOTTOM: {
			IBlockState up1 = world.getBlockState(pos.up());
			IBlockState up2 = world.getBlockState(pos.up(2));
			return up1.getBlock() == this && up1.getValue(PART) == Part.MIDDLE && up2.getBlock() == this
					&& up2.getValue(PART) == Part.TOP;
		}
		}
	}
	...
	public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
		for (int i = 0; i < 16; i++) {
			worldIn.spawnParticle(EnumParticleTypes.SUSPENDED_DEPTH,
					pos.getX() + 0.5D + (rand.nextBoolean() ? -1 : 1) * Math.pow(rand.nextFloat(), 2) * 16,
					pos.getY() + 0.5D + rand.nextFloat() * 6 - 3,
					pos.getZ() + 0.5D + (rand.nextBoolean() ? -1 : 1) * Math.pow(rand.nextFloat(), 2) * 16, 0, 0.2D, 0);
		}
	}
