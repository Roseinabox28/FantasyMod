...
public class BlockEnergyBarrier extends Block {
	...
	public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random rand) {
		...
		if (world.getBlockState(pos.add(-1, 0, 0)).getBlock() != this
				&& world.getBlockState(pos.add(1, 0, 0)).getBlock() != this) {
		...
		BLParticles.PORTAL.spawn(world, particleX, particleY, particleZ,
				ParticleFactory.ParticleArgs.get().withMotion(motionX * 0.2F, motionY, motionZ * 0.2F));
	}
								...
								if (blockState.getRenderType() != EnumBlockRenderType.INVISIBLE) {
									for (int i = 0; i < 8; i++) {
										world.spawnParticle(EnumParticleTypes.BLOCK_CRACK,
												offset.getX() + (double) world.rand.nextFloat(),
												offset.getY() + (double) world.rand.nextFloat(),
												offset.getZ() + (double) world.rand.nextFloat(),
												(double) world.rand.nextFloat() - 0.5D,
												(double) world.rand.nextFloat() - 0.5D,
												(double) world.rand.nextFloat() - 0.5D,
