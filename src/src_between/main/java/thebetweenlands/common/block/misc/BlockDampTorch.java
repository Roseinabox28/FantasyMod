	...
	public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
		if (rand.nextInt(4) == 0) {
			...
			if (facing.getAxis().isHorizontal()) {
				...
				BLParticles.SMOKE.spawn(worldIn, x + 0.27D * (double) facingOpposite.getXOffset(), y + 0.22D,
						z + 0.27D * (double) facingOpposite.getZOffset(),
						ParticleArgs.get().withColor(0.2F, 0.2F, 0.2F, 1.0F));
			} else {
