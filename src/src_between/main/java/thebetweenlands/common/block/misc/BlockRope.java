...
public class BlockRope extends Block implements ICustomItemBlock {
	...
	public static final PropertyEnum<EnumRopeVariant> VARIANT = PropertyEnum.<EnumRopeVariant>create("variant",
			EnumRopeVariant.class);
			...
			if (offsetPos.getY() != pos.getY()) {
				if (!world.isRemote) {
					...
					if (!player.capabilities.isCreativeMode
							&& !player.inventory.addItemStackToInventory(new ItemStack(ItemRegistry.ROPE_ITEM))) {
						world.spawnEntity(new EntityItem(world, player.posX, player.posY, player.posZ,
								new ItemStack(ItemRegistry.ROPE_ITEM)));
					}
				}
	...
	public boolean canPlaceBlockAt(World worldIn, BlockPos pos) {
		return !worldIn.getBlockState(pos).getMaterial().isLiquid() && super.canPlaceBlockAt(worldIn, pos)
				&& (worldIn.getBlockState(pos.up()).isSideSolid(worldIn, pos, EnumFacing.DOWN)
