...
public class BlockMudFlowerPot extends BlockContainer {
	...
	public IBlockState getExtendedState(IBlockState state, IBlockAccess world, BlockPos pos) {
		...
		TileEntityMudFlowerPot te = StatePropertyHelper.getTileEntityThreadSafe(world, pos,
				TileEntityMudFlowerPot.class);
			...
			if (itemstack1.isEmpty()) {
				if (this.getPlantBlockStateFromItem(heldItem) != null) {
					...
					if (!worldIn.isRemote) {
						...
						worldIn.playSound(null, pos, SoundType.WOOD.getPlaceSound(), SoundCategory.BLOCKS,
	...
	/**
	 * Returns the blockstate that should be used for the specified item. Return
	 * null if item can't be contained in pot
	 * 
	 * @param itemStack
	 * @return
	 */
			...
			if (block != Blocks.AIR) {
				if (block == Blocks.YELLOW_FLOWER || block == Blocks.RED_FLOWER || block == Blocks.CACTUS
						...
						|| (block == Blocks.TALLGRASS
								&& itemStack.getMetadata() == BlockTallGrass.EnumType.FERN.getMeta())
