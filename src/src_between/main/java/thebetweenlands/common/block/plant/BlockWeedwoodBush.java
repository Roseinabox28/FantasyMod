...
public class BlockWeedwoodBush extends Block implements IShearable, ISickleHarvestable, ITintedBlock, IFarmablePlant {
	...
	protected BlockStateContainer createBlockState() {
		return new ExtendedBlockState(this, new IProperty[] { NORTH, EAST, WEST, SOUTH, UP, DOWN },
				new IUnlistedProperty[] { POS_X, POS_Y, POS_Z });
	}
	...
	public static void onLivingUpdate(LivingUpdateEvent event) {
		...
		if (cap != null) {
			if (entity.distanceWalkedOnStepModified > cap.getNextWeedwoodBushStep()) {
				...
				Iterator<MutableBlockPos> it = BlockPos
						.getAllInBoxMutable(new BlockPos(aabb.minX, aabb.minY, aabb.minZ),
								new BlockPos(aabb.maxX, aabb.maxY, aabb.maxZ))
						.iterator();
				while (it.hasNext()) {
					MutableBlockPos pos = it.next();
					if (entity.world.isBlockLoaded(pos)
							&& entity.world.getBlockState(pos).getBlock() == BlockRegistry.WEEDWOOD_BUSH) {
						inBush = true;
						if (entity.world.isRemote) {
							spawnLeafParticles(entity.world, pos, Math.min(
									(entity.distanceWalkedOnStepModified - cap.getNextWeedwoodBushStep()) * 40, 1));
						}
					}
				}
				if (inBush) {
					entity.world.playSound(null, entity.posX, entity.posY, entity.posZ, SoundRegistry.GECKO_HIDE,
							SoundCategory.BLOCKS, 0.4F, entity.world.rand.nextFloat() * 0.3F + 0.7F);
				}
	...
	protected static void spawnLeafParticles(World world, BlockPos pos, float strength) {
		...
		while (leafCount-- > 0) {
			...
			BLParticles.WEEDWOOD_LEAF.spawn(world, x, y, z,
					ParticleArgs.get().withMotion(dx * mag, dy * mag, dz * mag));
		}
	}
