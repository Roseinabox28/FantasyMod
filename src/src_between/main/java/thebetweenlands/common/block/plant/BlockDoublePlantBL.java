...
public class BlockDoublePlantBL extends BlockBush
		implements IStateMappedBlock, IShearable, ISickleHarvestable, IFarmablePlant, ITintedBlock {
	protected static final AxisAlignedBB PLANT_AABB = new AxisAlignedBB(0.1D, 0.0D, 0.1D, 0.9D, 1.0D, 0.9D);

	public static final PropertyEnum<BlockDoublePlantBL.EnumBlockHalf> HALF = PropertyEnum.<BlockDoublePlantBL.EnumBlockHalf>create(
			"half", BlockDoublePlantBL.EnumBlockHalf.class);
	...
	public BlockDoublePlantBL() {
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(HALF, BlockDoublePlantBL.EnumBlockHalf.LOWER)
	...
	public void placeAt(World worldIn, BlockPos lowerPos, int updateFlags) {
		worldIn.setBlockState(lowerPos,
				this.getDefaultState().withProperty(HALF, BlockDoublePlantBL.EnumBlockHalf.LOWER), updateFlags);
		worldIn.setBlockState(lowerPos.up(),
				this.getDefaultState().withProperty(HALF, BlockDoublePlantBL.EnumBlockHalf.UPPER), updateFlags);
	}
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer,
		...
		worldIn.setBlockState(pos.up(),
				this.getDefaultState().withProperty(HALF, BlockDoublePlantBL.EnumBlockHalf.UPPER).withProperty(FACING,
						EnumFacing.byHorizontalIndex(rot)),
				2);
	}
		...
		if (state.getValue(HALF) == BlockDoublePlantBL.EnumBlockHalf.UPPER) {
			if (worldIn.getBlockState(pos.down()).getBlock() == this) {
				if (!player.capabilities.isCreativeMode) {
					// Stupid workarounds...
					this.harvestBlock(worldIn, player, pos.down(), worldIn.getBlockState(pos.down()),
							worldIn.getTileEntity(pos.down()), player.getHeldItemMainhand());
				}
	...
	public IBlockState getStateFromMeta(int meta) {
		...
		return this.getDefaultState().withProperty(HALF, isUpper ? EnumBlockHalf.UPPER : EnumBlockHalf.LOWER)
	...
	public List<ItemStack> getHarvestableDrops(ItemStack item, IBlockAccess world, BlockPos pos, int fortune) {
		return this.sickleHarvestableDrop != null ? ImmutableList.of(this.sickleHarvestableDrop.copy())
				: ImmutableList.of();
	}
	...
	public boolean canSpreadTo(World world, BlockPos pos, IBlockState state, BlockPos targetPos, Random rand) {
		return world.isAirBlock(targetPos) && world.isAirBlock(targetPos.up())
				&& this.canPlaceBlockAt(world, targetPos);
	}
	...
	public void spreadTo(World world, BlockPos pos, IBlockState state, BlockPos targetPos, Random rand) {
		world.setBlockState(targetPos,
				this.getDefaultState().withProperty(BlockDoublePlantBL.HALF, BlockDoublePlantBL.EnumBlockHalf.LOWER));
		world.setBlockState(targetPos.up(),
				this.getDefaultState().withProperty(BlockDoublePlantBL.HALF, BlockDoublePlantBL.EnumBlockHalf.UPPER));
	}
