...
public class BlockAlgae extends BlockPlant implements IConnectedTextureBlock {
	...
	protected BlockStateContainer createBlockState() {
		return this.getConnectedTextureBlockStateContainer(
				new ExtendedBlockState(this, new IProperty[0], new IUnlistedProperty[0]));
	}
	...
	public IBlockState getExtendedState(IBlockState oldState, IBlockAccess worldIn, BlockPos pos) {
		...
		return this.getExtendedConnectedTextureState(state, worldIn, pos, p -> {
			return p.getY() <= pos.getY()
					&& (worldIn.getBlockState(p).getBlock() == this || worldIn.getBlockState(p.down()).isFullCube());
		}, false);
	}
