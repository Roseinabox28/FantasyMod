...
public class BlockStackablePlantUnderwater extends BlockPlantUnderwater {
	...
	public BlockStackablePlantUnderwater(Fluid fluid, Material materialIn) {
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(LEVEL, 0).withProperty(IS_TOP, true)
				.withProperty(IS_BOTTOM, false));
	}
	...
	protected BlockStateContainer createBlockState() {
		return BlockStateContainerHelper.extendBlockstateContainer((ExtendedBlockState) super.createBlockState(),
				new IProperty[] { AGE, IS_TOP, IS_BOTTOM }, new IUnlistedProperty[0]);
	}
	...
	public boolean removedByPlayer(IBlockState state, World world, BlockPos pos, EntityPlayer player,
		...
		if (harvesting == null || !harvesting) {
			...
			for (height = 1; this.isSamePlant(world.getBlockState(pos.up(height))); ++height)
				;
			for (int offset = height - 1; (this.harvestAll && this.isSamePlant(world.getBlockState(pos.up(offset))))
					|| (!this.harvestAll && offset >= 0); offset--) {
				if (offset != 0) {
					BlockPos offsetPos = pos.up(offset);
					IBlockState blockState = world.getBlockState(offsetPos);
					boolean canHarvest = player.isCreative() ? false
							: blockState.getBlock().canHarvestBlock(world, offsetPos, player);
					boolean otherRemoved = this.removeOtherBlockAsPlayer(world, offsetPos, player, canHarvest);
					if (otherRemoved && canHarvest) {
						ItemStack stack = player.getHeldItemMainhand() == null ? null
								: player.getHeldItemMainhand().copy();
						blockState.getBlock().harvestBlock(world, player, offsetPos, blockState,
								world.getTileEntity(offsetPos), stack);
					}
				} else {
	...
	/**
	 * Sets the maximum height this plant should naturally grow. Set to -1 if the
	 * plant should grow until it reaches the surface.
	 * 
	 * @param maxHeight
	 * @return
	 */
	...
	public void randomTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		...
		if (this.canGrow(worldIn, pos, state)) {
			if (ForgeHooks.onCropsGrowPre(worldIn, pos, state,
					rand.nextFloat() <= this.getGrowthChance(worldIn, pos, state, rand))) {
	...
	protected boolean canGrowUp(World world, BlockPos pos, IBlockState state, int height) {
		return world.getBlockState(pos.up()) != this && world.getBlockState(pos.up()).getMaterial() == Material.WATER
	...
	protected boolean canSustainPlant(IBlockState state) {
		return ((this.maxHeight == -1 || this.maxHeight > 1) && this.isSamePlant(state))
				|| SoilHelper.canSustainUnderwaterPlant(state);
	}
