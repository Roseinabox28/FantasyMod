...
public class BlockNettleFlowered extends BlockPlant {
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		...
		if (rand.nextInt(350) == 0) {
			...
			if (worldIn.isAreaLoaded(randOffset, 0) && worldIn.isAirBlock(randOffset)
					&& canBlockStay(worldIn, randOffset, worldIn.getBlockState(randOffset))) {
	...
	public void onEntityCollision(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
		if (!worldIn.isRemote && entityIn instanceof IEntityBL == false && entityIn instanceof EntityLivingBase
				&& !ElixirEffectRegistry.EFFECT_TOUGHSKIN.isActive((EntityLivingBase) entityIn)) {
