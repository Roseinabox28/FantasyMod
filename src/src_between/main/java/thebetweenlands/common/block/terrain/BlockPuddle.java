...
public class BlockPuddle extends Block implements ITintedBlock, IStateMappedBlock {
			...
			if (amount > 2) {
				...
				for (int xo = -1; xo <= 1; xo++) {
					for (int zo = -1; zo <= 1; zo++) {
						...
						if ((xo == 0 && zo == 0) || xo * xo == zo * zo)
							continue;
						if ((world.isAirBlock(newPos)
								|| world.getBlockState(newPos).getBlock() instanceof BlockGenericCrop)
								&& this.canPlaceBlockAt(world, newPos)) {
							...
							world.setBlockState(newPos,
									state.withProperty(AMOUNT, Math.min(amount + rand.nextInt(6), 15)), 2);
						}
					}
				}
			}
	...
	public boolean canPlaceBlockAt(World world, BlockPos pos) {
		...
		return (block.isReplaceable(world, pos) || block instanceof BlockGenericCrop)
				&& world.isSideSolid(pos.down(), EnumFacing.UP) && world.getBlockState(pos.down())
