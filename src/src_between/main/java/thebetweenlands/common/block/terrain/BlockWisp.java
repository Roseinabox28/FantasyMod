...
public class BlockWisp extends BlockContainer implements IStateMappedBlock {
	...
	public static boolean canSee(World world, BlockPos pos) {
		...
		if (!worldStorage.getLocalStorageHandler().getLocalStorages(LocationCragrockTower.class, pos.getX(), pos.getZ(),
				location -> location.isInside(pos)).isEmpty()) {
			return true;
		}
		if (!worldStorage.getLocalStorageHandler()
				.getLocalStorages(LocationSpiritTree.class, pos.getX(), pos.getZ(), location -> location.isInside(pos))
				.isEmpty()) {
			return true;
		}
		return false;
	...
	public void onPlayerDestroy(World world, BlockPos pos, IBlockState state) {
		if (!world.isRemote && canSee(world, pos)) {
			EntityItem wispItem = new EntityItem(world, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D,
	...
	// Colors can be added here, always add a pair of colors for outer color and
	// inner color
	public static final int[] COLORS = new int[] { 0xFF7F1659, 0xFFFFFFFF, // Pink/White
