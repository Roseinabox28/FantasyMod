...
public class BlockSwampWater extends BlockFluidClassic implements IStateMappedBlock, ITintedBlock, ICustomItemBlock {
	...
	public void addCollisionBoxToList(IBlockState state, World worldIn, BlockPos pos, AxisAlignedBB entityBox,
		...
		if (entityIn instanceof EntityPlayer && ItemMarshRunnerBoots.checkPlayerWalkOnWater((EntityPlayer) entityIn)) {
			addCollisionBoxToList(pos, entityBox, collidingBoxes, new AxisAlignedBB(0, 0, 0, 1,
					((float) this.getQuantaValue(worldIn, pos) / (float) this.quantaPerBlock) * 0.8F + 0.3F, 1));
			return;
		}
	...
	public Vec3d modifyAcceleration(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull Entity entity,
		...
		return vec.add(vec_flow.x, vec_flow.y, vec_flow.z);
	}
	...
	public boolean displaceIfPossible(World world, BlockPos pos) {
		if (world instanceof World && !((World) world).isBlockLoaded(pos))
			return false;
	...
	public float getFluidHeightForRender(IBlockAccess world, BlockPos pos, @Nonnull IBlockState up) {
		...
		if (here.getBlock() instanceof BlockSwampWater) {
			if ((up.getMaterial().isLiquid() || up.getBlock() instanceof IFluidBlock)
					&& (up.getBlock() instanceof BlockSwampWater == false
							|| !((BlockSwampWater) up.getBlock()).isUnderwaterBlock)) {
				return 1;
			}
		...
		return !here.getMaterial().isSolid() && up.getBlock() instanceof BlockSwampWater ? 1
	...
	public Vec3d getFlowVector(IBlockAccess world, BlockPos pos) {
		...
		if (!this.isSourceBlock(world, pos) && world.getBlockState(pos.up()).getBlock() instanceof BlockSwampWater) {
			for (EnumFacing dir : EnumFacing.Plane.HORIZONTAL) {
				if (this.causesDownwardCurrent(world, pos.offset(dir), dir)
						|| this.causesDownwardCurrent(world, pos.offset(dir).up(), dir)) {
	...
	protected void flowIntoBlock(World world, BlockPos pos, int meta) {
		...
		if (displaceIfPossible(world, pos)) {
			world.setBlockState(pos, BlockRegistry.SWAMP_WATER.getBlockState().getBaseState().withProperty(LEVEL, meta),
					3);
		}
	}
	...
	public AxisAlignedBB getCollisionBoundingBox(@Nonnull IBlockState blockState, @Nonnull IBlockAccess worldIn,
		...
		if (blockState.getBlock() instanceof BlockSwampWater
				&& ((BlockSwampWater) blockState.getBlock()).isUnderwaterBlock)
	...
	public boolean canCollideCheck(IBlockState state, boolean fullHit) {
		return state.getBlock() instanceof BlockSwampWater && ((BlockSwampWater) state.getBlock()).isUnderwaterBlock
				|| super.canCollideCheck(state, fullHit);
	}

	@Override
	public boolean isReplaceable(IBlockAccess worldIn, BlockPos pos) {
		IBlockState state = worldIn.getBlockState(pos);
		return !(state.getBlock() instanceof BlockSwampWater)
				|| !((BlockSwampWater) state.getBlock()).isUnderwaterBlock && super.isReplaceable(worldIn, pos);
	}
	...
	public boolean canPlaceBlockAt(World worldIn, BlockPos pos) {
		return super.canPlaceBlockAt(worldIn, pos)
				&& (!this.isUnderwaterBlock || worldIn.getBlockState(pos).getMaterial() == Material.WATER);
	}
	...
	public int getColorMultiplier(IBlockState state, IBlockAccess worldIn, BlockPos pos, int tintIndex) {
		...
		if (pos.getY() > WorldProviderBetweenlands.CAVE_START) {
			...
			if (pos.getY() < WorldProviderBetweenlands.CAVE_WATER_HEIGHT) {
				...
				depth = (pos.getY() - WorldProviderBetweenlands.CAVE_WATER_HEIGHT)
						/ (float) (WorldProviderBetweenlands.CAVE_START - WorldProviderBetweenlands.CAVE_WATER_HEIGHT);
			}
		}
	...
	public boolean shouldSideBeRendered(@Nonnull IBlockState state, @Nonnull IBlockAccess world, @Nonnull BlockPos pos,
		...
		if (this.isUnderwaterBlock) {
			...
			// Ignore AABB check, only render sides if there's no block that actually blocks
			// that side visually!
			return !world.getBlockState(pos.offset(side)).doesSideBlockRendering(world, pos.offset(side),
					side.getOpposite());
		}
