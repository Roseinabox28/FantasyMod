	...
	public static void spreadGrassTo(World world, BlockPos pos) {
		...
		if (te != null) {
			...
			if (state.getBlock() == BlockRegistry.DUG_SWAMP_DIRT) {
				world.setBlockState(pos, BlockRegistry.DUG_SWAMP_GRASS.getDefaultState(), 2); // don't do block update
																								// yet
			}

			if (state.getBlock() == BlockRegistry.DUG_PURIFIED_SWAMP_DIRT) {
				world.setBlockState(pos, BlockRegistry.DUG_PURIFIED_SWAMP_GRASS.getDefaultState(), 2); // don't do block
																										// update yet
			}
