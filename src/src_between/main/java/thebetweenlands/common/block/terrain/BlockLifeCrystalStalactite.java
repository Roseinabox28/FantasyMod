...
public class BlockLifeCrystalStalactite extends BlockSwampWater
		implements BlockRegistry.ICustomItemBlock, BlockRegistry.ISubtypeItemBlockModelDefinition, IStateMappedBlock {
	public static final PropertyEnum<EnumLifeCrystalType> VARIANT = PropertyEnum.<EnumLifeCrystalType>create("variant",
			EnumLifeCrystalType.class);
	...
	protected BlockStateContainer createBlockState() {
		return BlockStateContainerHelper.extendBlockstateContainer((ExtendedBlockState) super.createBlockState(),
				...
				new IUnlistedProperty<?>[] { POS_X, POS_Y, POS_Z, NO_BOTTOM, NO_TOP, DIST_UP, DIST_DOWN });
	}
	...
	public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos,
		...
		return new ItemStack(Item.getItemFromBlock(this), 1,
				((EnumLifeCrystalType) state.getValue(VARIANT)).getMetadata());
	}
	...
	public IBlockState getExtendedState(IBlockState oldState, IBlockAccess worldIn, BlockPos pos) {
		...
		return state.withProperty(POS_X, pos.getX()).withProperty(POS_Y, pos.getY()).withProperty(POS_Z, pos.getZ())
				.withProperty(DIST_UP, distUp).withProperty(DIST_DOWN, distDown).withProperty(NO_TOP, noTop)
				.withProperty(NO_BOTTOM, noBottom);
	}
