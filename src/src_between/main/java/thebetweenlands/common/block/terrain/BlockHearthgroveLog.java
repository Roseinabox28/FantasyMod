...
public class BlockHearthgroveLog extends BlockLogBetweenlands {
	...
	public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
		...
		for (EnumFacing offset : EnumFacing.HORIZONTALS) {
			IBlockState offsetState = worldIn.getBlockState(
					checkPos.setPos(pos.getX() + offset.getXOffset(), pos.getY(), pos.getZ() + offset.getZOffset()));
			IBlockState offsetStateDown = worldIn.getBlockState(checkPos.setPos(pos.getX() + offset.getXOffset(),
					pos.getY() - 1, pos.getZ() + offset.getZOffset()));

			if (offsetStateDown.getMaterial() == Material.WATER && offsetState.getMaterial() != Material.WATER) {
				if (rand.nextInt(8) == 0) {
					for (int i = 0; i < 5; i++) {
						float x = pos.getX() + (offset.getXOffset() > 0 ? 1.05F
								: offset.getXOffset() == 0 ? rand.nextFloat() : -0.05F);
						float y = pos.getY() - 0.1F;
						float z = pos.getZ() + (offset.getZOffset() > 0 ? 1.05F
								: offset.getZOffset() == 0 ? rand.nextFloat() : -0.05F);
			...
			if (offsetState.getMaterial() == Material.WATER) {
				if (rand.nextInt(8) == 0) {
					for (int i = 0; i < 5; i++) {
						float x = pos.getX() + (offset.getXOffset() > 0 ? 1.1F
								: offset.getXOffset() == 0 ? rand.nextFloat() : -0.1F);
						float y = pos.getY() + rand.nextFloat();
						float z = pos.getZ() + (offset.getZOffset() > 0 ? 1.1F
								: offset.getZOffset() == 0 ? rand.nextFloat() : -0.1F);
	...
	protected BlockStateContainer createBlockState() {
		return BlockStateContainerHelper.extendBlockstateContainer(super.createBlockState(),
				new IProperty[] { TARRED });
	}

	@Override
	@SideOnly(Side.CLIENT)
	public void getSubBlocks(CreativeTabs itemIn, NonNullList<ItemStack> list) {
		list.add(new ItemStack(this, 1, this.getMetaFromState(
				this.getDefaultState().withProperty(LOG_AXIS, BlockLog.EnumAxis.Y).withProperty(TARRED, false))));
		list.add(new ItemStack(this, 1, this.getMetaFromState(
				this.getDefaultState().withProperty(LOG_AXIS, BlockLog.EnumAxis.Y).withProperty(TARRED, true))));
		list.add(new ItemStack(this, 1, this.getMetaFromState(
				this.getDefaultState().withProperty(LOG_AXIS, BlockLog.EnumAxis.NONE).withProperty(TARRED, false))));
		list.add(new ItemStack(this, 1, this.getMetaFromState(
				this.getDefaultState().withProperty(LOG_AXIS, BlockLog.EnumAxis.NONE).withProperty(TARRED, true))));
	}
