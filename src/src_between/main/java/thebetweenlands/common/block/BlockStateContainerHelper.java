...
public class BlockStateContainerHelper {
	 ...
	 * @return
	 */
	public static ExtendedBlockState extendBlockstateContainer(ExtendedBlockState original,
			IProperty<?>[] newProperties, IUnlistedProperty<?>[] newUnlistedProperties) {
		...
		return new ExtendedBlockState(original.getBlock(), properties.toArray(new IProperty[0]),
				unlistedProperties.toArray(new IUnlistedProperty[0]));
	}
	 ...
	 * @return
	 */
	public static ExtendedBlockState extendBlockstateContainer(BlockStateContainer original,
			IProperty<?>[] newProperties, IUnlistedProperty<?>[] newUnlistedProperties) {
		Collection<IProperty<?>> properties = new ArrayList<IProperty<?>>();
		properties.addAll(original.getProperties());
		properties.addAll(Arrays.asList(newProperties));
	 ...
	 * @return
	 */
	public static BlockStateContainer extendBlockstateContainer(BlockStateContainer original,
			IProperty<?>... newProperties) {
		Collection<IProperty<?>> properties = new ArrayList<IProperty<?>>();
		properties.addAll(original.getProperties());
		properties.addAll(Arrays.asList(newProperties));
