...
public class RingOfDispersionEntityCapability
	...
	public void getCustomCollisionBoxes(CollisionBoxHelper collisionBoxHelper, AxisAlignedBB aabb,
		...
		if (!stack.isEmpty()) {
			...
			if (item.canPhase(player, stack)) {
				...
				final AxisAlignedBB viewAabb = new AxisAlignedBB(player.posX, player.posY + player.getEyeHeight(),
						player.posZ, player.posX, player.posY + player.getEyeHeight(), player.posZ).grow(0.25D);

				final double checkReach = Math.max(this.getViewObstructionCheckDistance(),
						this.getObstructionCheckDistance());

				collisionBoxHelper.getCollisionBoxes(player,
						aabb.grow(checkReach, 0, checkReach).expand(0, checkReach, 0), EntityCollisionPredicate.ALL,
						new BlockCollisionPredicate() {
							...
							public boolean isColliding(Entity entity, AxisAlignedBB aabb, MutableBlockPos pos,
								...
								if (!isCollisionForced
										&& state.getPlayerRelativeBlockHardness(player, player.world, pos) < 0.0001F) {
									isCollisionForced = true;
								}
								...
								if (!isCollisionForced) {
									...
									if (playerDist < RingOfDispersionEntityCapability.this.getObstructionCheckDistance()
											&& playerDist < RingOfDispersionEntityCapability.this.obstructionDistance) {
									...
									if (viewDist < RingOfDispersionEntityCapability.this
											.getViewObstructionCheckDistance()
											&& viewDist < RingOfDispersionEntityCapability.this.viewObstructionDistance) {
	...
	public static void onLivingAttacked(LivingAttackEvent event) {
		if (event.getEntity() instanceof EntityPlayer
				&& event.getSource().getDamageType().equals(DamageSource.IN_WALL.getDamageType())) {
