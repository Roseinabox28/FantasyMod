...
public class EntityCapabilityHandler {
	 ...
	 */
	public static <T, F extends EntityCapability<F, T, E>, E extends Entity> void registerEntityCapability(
			EntityCapability<F, T, E> entityCapability) {
		...
		Preconditions.checkState(entityCapability.getCapabilityClass().isAssignableFrom(entityCapability.getClass()),
				"Entity capability %s must implement %s", entityCapability.getClass().getName(),
				entityCapability.getCapabilityClass().getName());
		REGISTERED_CAPABILITIES.add(entityCapability);
	}

	/**
	 * Registers all capabilities to the {@link CapabilityManager}. Must be called
	 * during pre init.
	 */
	...
	private static <T, E extends Entity> void registerCapability(EntityCapability<?, T, E> capability) {
		CapabilityManager.INSTANCE.register(capability.getCapabilityClass(), new IStorage<T>() {
			...
			public final NBTBase writeNBT(Capability<T> capability, T instance, EnumFacing side) {
				...
				return new NBTTagCompound(); // This is broken, the doc says returning null if not required, but causes
												// a NPE
			}
			...
			if (entityCapability.isApplicable(entity)) {
				...
				event.addCapability(entityCapability.getID(), new ICapabilitySerializable<NBTTagCompound>() {
					...
					private EntityCapability<?, ?, ?> getNewInstance() {
						EntityCapability<?, ?, ?> entityCapability = (EntityCapability<?, ?, ?>) capabilityInstance
								.getDefaultInstance();
			...
			if (trackers != null) {
				...
				while (trackerIT.hasNext()) {
					...
					if (tracker.getEntityCapability().getEntity() != player) {
						Set<? extends EntityPlayer> vanillaTrackingPlayers = player.getServerWorld().getEntityTracker()
								.getTrackingPlayers(tracker.getEntityCapability().getEntity());

						if (vanillaTrackingPlayers == null || vanillaTrackingPlayers.isEmpty()
								|| !vanillaTrackingPlayers.contains(player)) {
		...
		if (event.phase == Phase.END) {
			...
			if (updateTimer > 20) {
				...
				while (it.hasNext()) {
					...
					// player.getServerWorld().loadedEntityList.size());

					if (player.getServerWorld().getMinecraftServer() != null && !player.getServerWorld()
							.getMinecraftServer().getPlayerList().getPlayers().contains(player)) {
						it.remove();
	...
	public static void onPlayerClone(PlayerEvent.Clone event) {
		...
		for (EntityCapability<?, ?, EntityPlayer> capability : capabilities) {
			if (capability.isPersistent(oldPlayer, newPlayer, event.isWasDeath())
					&& capability instanceof ISerializableCapability) {
				...
				if (newCapability != null && newCapability instanceof ISerializableCapability) {
					capability.clonePersistentData(oldPlayer, newPlayer, event.isWasDeath(),
							(ISerializableCapability) newCapability);
				}
			}
		}
	}
