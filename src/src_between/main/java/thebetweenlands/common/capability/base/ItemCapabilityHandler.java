...
public class ItemCapabilityHandler {
	...
	public static <T, F extends ItemCapability<F, T>> void registerItemCapability(ItemCapability<F, T> itemCapability) {
		...
		Preconditions.checkState(itemCapability.getCapabilityClass().isAssignableFrom(itemCapability.getClass()),
				"Item capability %s must implement %s", itemCapability.getClass().getName(),
				itemCapability.getCapabilityClass().getName());
	...
	public static void onAttachCapabilities(AttachCapabilitiesEvent<ItemStack> event) {
		Item item = event.getObject().getItem(); // I don't know what to use... getItemStack returns a stack with a null
													// item...
			...
			if (itemCapability.isApplicable(item)) {
				...
				event.addCapability(itemCapability.getID(), new ICapabilitySerializable<NBTTagCompound>() {
					...
					private ItemCapability<?, ?> getNewInstance() {
						ItemCapability<?, ?> itemCapability = (ItemCapability<?, ?>) capabilityInstance
								.getDefaultInstance();
