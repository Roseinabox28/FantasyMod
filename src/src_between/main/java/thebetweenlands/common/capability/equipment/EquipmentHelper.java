...
public class EquipmentHelper {
	...
	/**
	 * Tries to equip the specified item in the specified slot, returns the leftover
	 * stack
	 * 
	 * @param player
	 * @param target
	 ...
	 * @return
	 */
	public static ItemStack equipItem(@Nullable EntityPlayer player, Entity target, ItemStack stack, int slot,
			boolean simulate) {
			...
			for (EnumEquipmentInventory type : EnumEquipmentInventory.VALUES) {
				...
				for (int i = 0; i < inv.getSizeInventory(); i++) {
					...
					if (!stack.isEmpty() && stack.getItem() instanceof IEquippable
							&& !((IEquippable) stack.getItem()).canUnequip(stack, player, target,
									cap.getInventory(((IEquippable) stack.getItem()).getEquipmentCategory(stack)))) {
						continue;
					}
	 ...
	 * @return
	 */
	public static ItemStack unequipItem(@Nullable EntityPlayer player, Entity target, EnumEquipmentInventory type,
			int slot, boolean simulate) {
		IEquipmentCapability cap = target.getCapability(CapabilityRegistry.CAPABILITY_EQUIPMENT, null);
		if (cap != null) {
			IInventory inv = cap.getInventory(type);
