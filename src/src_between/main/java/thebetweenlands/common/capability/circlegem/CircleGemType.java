...
public enum CircleGemType {
	...
	GREEN("green", 3, ItemGemSinger.GemSingerTarget.GREEN_MIDDLE_GEM), NONE("none", 0, null);
	 ...
	 * 0: Neutral
	 * <p>
	 * 1: This gem has an advantage over the other gem
	 * <p>
	 * -1: This gem has a disadvantage over the other gem
	 * 
	 ...
	 * @param source       The source (damage source, e.g. the entity that shot the
	 *                     projectile)
	 * @param attacker     The entity that is actually attacking (can be both the
	 *                     owner himself or a projectile)
	 * @param defender     The defending entity
	 * @param strength     Proc strength
	 * @param damageSource The damage source
	 * @param damage       The damage that was dealt
	 */
	public boolean applyProc(boolean isAttacker, Entity owner, Entity source, Entity attacker, Entity defender,
			float strength, DamageSource damageSource, float damage) {
		switch (this) {
		case CRIMSON:
			if (isAttacker) {
				if (defender instanceof EntityLivingBase) {
					...
					double mz;
					for (mz = attacker.posZ - defender.posZ; mx * mx
							+ mz * mz < 1.0E-4D; mz = (Math.random() - Math.random()) * 0.01D) {
					...
					if (attacker instanceof EntityLivingBase) {
						((EntityLivingBase) attacker).addPotionEffect(new PotionEffect(MobEffects.STRENGTH, 110,
								Math.min(MathHelper.floor(strength * 0.2F), 2)));
					}
					if (source != attacker && source instanceof EntityLivingBase) {
						((EntityLivingBase) source).addPotionEffect(new PotionEffect(MobEffects.STRENGTH, 110,
								Math.min(MathHelper.floor(strength * 0.2F), 2)));
					}
					return true;
				}
		...
		case GREEN:
			if (isAttacker) {
				...
				if (defender instanceof EntityLivingBase) {
					((EntityLivingBase) defender).addPotionEffect(new PotionEffect(MobEffects.ABSORPTION, 260,
							Math.min(MathHelper.floor(strength * 0.25F), 2)));
					return true;
				}
			}
			break;
		case AQUA:
			if (isAttacker) {
				if (defender instanceof EntityLivingBase) {
					...
					case 1:
					case 2:
						((EntityLivingBase) defender)
								.addPotionEffect(new PotionEffect(MobEffects.WEAKNESS, 40, amplifier));
						break;
					}
					return true;
				}
				...
				if (defender instanceof EntityLivingBase) {
					((EntityLivingBase) defender).addPotionEffect(new PotionEffect(MobEffects.RESISTANCE, 130,
							Math.min(MathHelper.floor(strength * 0.3F), 2)));
					return true;
				}
			}
			break;
