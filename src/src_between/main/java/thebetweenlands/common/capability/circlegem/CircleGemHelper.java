...
public class CircleGemHelper {
	...
	public static boolean isApplicable(Item item) {
		return item instanceof ItemAmulet || item instanceof ItemArmor || item instanceof ItemSword
				|| item instanceof ItemBow || item instanceof ItemTool || item instanceof ItemShield;
	}
	...
	public static void addGemPropertyOverrides(Item item) {
		item.addPropertyOverride(new ResourceLocation("gem"),
				(stack, worldIn, entityIn) -> CircleGemHelper.getGem(stack).id);
	}
	...
	public static float handleAttack(DamageSource damageSource, EntityLivingBase attackedEntity, float damage) {
		if (attackedEntity.hurtTime == 0 && attackedEntity.deathTime == 0 && damageSource instanceof EntityDamageSource
				&& (attackedEntity instanceof EntityPlayer == false
						|| !((EntityPlayer) attackedEntity).capabilities.disableDamage)) {
			Entity attacker;
			Entity source;
			...
			if (attacker != null && source != null) {
				...
				if (attacker instanceof EntityLivingBase) {
					...
					if (!heldItem.isEmpty())
						attackerItemGem = CircleGemHelper.getGem(heldItem);
				}
				// At this point either userGem or attackerItemGem are set because either
				// there's a user shooting a (non-living) projectile (user != attacker) or the
				// user is attacking directly (user == attacker)
				...
				for (ItemStack equipmentStack : equipment) {
					if (!equipmentStack.isEmpty() && !equipmentStack.equals(getActiveItem(attackedEntity))
							&& equipmentStack.getItem() instanceof ItemArmor) {
				...
				float gemDamageVariation = Math
						.min(((gemRelation != 0 ? Math.signum(gemRelation) * 1 : 0) + gemRelation) / 6.0F
								* MAX_GEM_DAMAGE_VARIATION, MAX_GEM_DAMAGE_VARIATION);
				if (gemDamageVariation != 0.0F) {
					damage = Math.max(damage + gemDamageVariation, 1.0F);
				}
				damage = Math.max(0, damage);

				boolean attackerProc = attacker.world.rand
						.nextFloat() <= (source == attacker && !attacker.onGround && attacker.motionY < 0
								? GEM_PROC_CHANCE * 1.33F
								: GEM_PROC_CHANCE);
				...
				for (CircleGemType gem : attackerGemCounts.keySet()) {
					if (gem != CircleGemType.NONE && applyProc(gem, attacker, source, attacker, attackedEntity,
							attackerProc, defenderProc, getMultipleProcStrength(attackerGemCounts.get(gem), damage),
							damageSource, damage)) {
						attackerProcd = true;
				...
				for (CircleGemType gem : defenderGemCounts.keySet()) {
					if (gem != CircleGemType.NONE && applyProc(gem, attackedEntity, source, attacker, attackedEntity,
							attackerProc, defenderProc, getMultipleProcStrength(defenderGemCounts.get(gem), damage),
							damageSource, damage)) {
						defenderProcd = true;
					...
					if (attackerProcd) {
						for (CircleGemType gem : attackerProcdGems) {
							TheBetweenlands.networkWrapper
									.sendToAllAround(new MessageGemProc(attackedEntity, true, gem), new TargetPoint(dim,
											attackedEntity.posX, attackedEntity.posY, attackedEntity.posZ, 64.0D));
						}
					}
					if (defenderProcd) {
						for (CircleGemType gem : defenderProcdGems) {
							TheBetweenlands.networkWrapper.sendToAllAround(
									new MessageGemProc(attackedEntity, false, gem), new TargetPoint(dim,
											attackedEntity.posX, attackedEntity.posY, attackedEntity.posZ, 64.0D));
						}
					}
					source.world.playSound(null, source.posX, source.posY, source.posZ,
							SoundEvents.ENTITY_ARROW_HIT_PLAYER, SoundCategory.PLAYERS, 1, 1);
					source.world.playSound(null, attackedEntity.posX, attackedEntity.posY, attackedEntity.posZ,
							SoundEvents.ENTITY_ARROW_HIT_PLAYER, SoundCategory.PLAYERS, 1, 1);
				}
			}
		}
	...
	private static boolean applyProc(CircleGemType gem, Entity owner, Entity source, Entity attacker, Entity defender,
			boolean attackerProc, boolean defenderProc, float strength, DamageSource damageSource, float damage) {
