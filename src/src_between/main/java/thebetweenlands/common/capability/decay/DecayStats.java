...
public class DecayStats {
	...
	/**
	 * Adds or removes decay. Negative decay increases saturation and decreases the
	 * decay level while positive decay decreases the saturation and increases the
	 * decay level.
	 * 
	...
	public void addStats(int decay, float saturationModifier) {
		...
		this.decaySaturationLevel = MathHelper.clamp(
				this.decaySaturationLevel + (float) -decay * saturationModifier * 2.0F, 0.0F,
				(float) (20 - this.decayLevel) / 4.0F);
	...
	/**
	 * Adds decay acceleration (once >= 4 is accumulated the decay level is
	 * increased by one)
	 * 
	 * @param acceleration
	 */
