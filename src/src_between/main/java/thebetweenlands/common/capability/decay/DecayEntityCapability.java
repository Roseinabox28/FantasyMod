...
public class DecayEntityCapability extends EntityCapability<DecayEntityCapability, IDecayCapability, EntityPlayer>
		implements IDecayCapability, ISerializableCapability {
	@Override
	...
	public float getMaxPlayerHealthPercentage(int decayLevel) {
		return BetweenlandsConfig.GENERAL.decayMinHealthPercentage
				+ (1.0f - BetweenlandsConfig.GENERAL.decayMinHealthPercentage) * (Math.min(26f - decayLevel, 20f) - 6f)
						/ (20.0f - 6f);
	}
	...
	public boolean isDecayEnabled() {
		return this.getEntity().getEntityWorld().getDifficulty() != EnumDifficulty.PEACEFUL
				...
				&& !this.getEntity().capabilities.isCreativeMode && !this.getEntity().capabilities.disableDamage;
	}
