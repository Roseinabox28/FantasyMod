...
public final class BlockShapeUtils {
	...
	public static List<BlockPos> getCircle(BlockPos pos, int radius, List<BlockPos> list) {
		...
		while (xo >= zo) {
			...
			if (xo != zo)
				s2.add(pos.add(zo, 0, xo));
			if (zo != 0)
				s3.add(pos.add(-zo, 0, xo));
			if (xo != zo)
				s4.add(pos.add(-xo, 0, zo));
			if (zo != 0)
				s5.add(pos.add(-xo, 0, -zo));
			if (xo != zo)
				s6.add(pos.add(-zo, 0, -xo));
			if (zo != 0)
				s7.add(pos.add(zo, 0, -xo));
			if (xo != zo && zo != 0)
				s8.add(pos.add(xo, 0, -zo));
	...
	/**
	 * Adds all blocks within the specified circle/ring segment on the XZ plane to a
	 * list. {@code startAngle} must be < {@code endAngle}. Angles are in radians
	 * 
	 * @param pos
	 * @param startAngle
	 * @param endAngle
	 ...
	 * @return
	 */
	public static List<BlockPos> getRingSegment(BlockPos pos, double startAngle, double endAngle, double innerRadius,
			double outerRadius, boolean includeCenter, List<BlockPos> list) {
