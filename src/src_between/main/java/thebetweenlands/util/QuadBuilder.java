...
public class QuadBuilder {
	static class Vertex {
		...
		private Vertex(Vec3d pos, float u, float v, TextureAtlasSprite sprite, boolean switchUV,
				TRSRTransformation transformation, float[] color, Vec3d normal, int blockLight, int skyLight,
				int tintIndex, EnumFacing cullFace, EnumFacing orientation, boolean diffuse) {
			this.pos = pos;
			this.u = u;
			this.v = v;
			this.sprite = sprite;
	...
	public QuadBuilder setLightmap(int blockLight, int skyLight) {
		if (BetweenlandsConfig.RENDERING.fullbrightBlocks && ForgeModContainer.forgeLightPipelineEnabled
	...
	/**
	 * Sets the normal. Set to null for cross product normal.
	 * 
	 * @param normal
	 * @return
	 */
	...
	public QuadBuilder addVertex(Vec3d pos, float u, float v) {
		this.vertices.add(new Vertex(pos, u, v, this.sprite, this.switchUV, this.transformation, this.color,
				this.normal, this.blockLight, this.skyLight, this.tintIndex, this.cullFace, this.orientation,
	...
	public QuadBuilder addVertex(Vec3d pos) {
		this.vertices.add(new Vertex(pos, 0.0F, 0.0F, this.sprite, this.switchUV, this.transformation, this.color,
				this.normal, this.blockLight, this.skyLight, this.tintIndex, this.cullFace, this.orientation,
				this.diffuseLighting));
		return this;
	}
	...
	private void putVertex(VertexFormat format, UnpackedBakedQuad.Builder builder, Vec3d quadNormal, Vertex vert) {
		boolean hasTransform = vert.transformation != null
				&& !vert.transformation.equals(TRSRTransformation.identity());
		for (int e = 0; e < format.getElementCount(); e++) {
			...
			case NORMAL:
				float[] normalData;
				if (vert.normal != null) {
					normalData = new float[] { (float) vert.normal.x, (float) vert.normal.y, (float) vert.normal.z,
							0f };
				} else {
	...
	private BakedQuad createQuad(VertexFormat format, Vertex vert1, Vertex vert2, Vertex vert3, Vertex vert4,
			@Nullable Consumer<UnpackedBakedQuad.Builder> quadConsumer) {
		Vec3d quadNormal = vert4.normal;
		if (quadNormal == null) {
			// Find 3 vertices that aren't at the exact same position to calculate the quad
			// normal
		...
		if (vert4.orientation != null) {
			...
			builder.setQuadOrientation(
					EnumFacing.getFacingFromVector((float) quadNormal.x, (float) quadNormal.y, (float) quadNormal.z));
		}
