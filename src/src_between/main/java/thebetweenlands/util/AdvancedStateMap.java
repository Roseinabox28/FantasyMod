...
public class AdvancedStateMap extends StateMapperBase {
	...
	private AdvancedStateMap(@Nullable Function<PropertiesMap, String> nameMapper, @Nullable String suffix,
			List<IProperty<?>> ignored, Map<IProperty<?>, Function<?, String>> suffixProperties,
			List<Function<UnmodifiablePropertiesMap, List<IProperty<?>>>> suffixExclusions,
			List<Function<UnmodifiablePropertiesMap, List<IProperty<?>>>> propertyExclusions) {
	...
	protected ModelResourceLocation getModelResourceLocation(IBlockState state) {
		...
		if (this.nameMapper == null) {
			...
			fileName = String.format("%s:%s", Block.REGISTRY.getNameForObject(state.getBlock()).getNamespace(),
					this.nameMapper.apply(wrappedPropertiesMapModifiable));
		}
		...
		for (Entry<IProperty<?>, Comparable<?>> propertyPair : propertiesMap.entrySet()) {
			if (!suffixExclusions.contains(propertyPair.getKey())) {
				...
				Function<Comparable<?>, String> nameMapper = (Function<Comparable<?>, String>) this.suffixProperties
						.get(propertyPair.getKey());
				if (nameMapper != null) {
	...
	public static class Builder {
		...
		/**
		 * Sets the state mapper to set the name to the value returned by the specified
		 * mapper
		 * 
		 * @param exclusions
		 * @return
		...
		/**
		 * Sets the state mapper to use the name provided by the name mapper for the
		 * value of the specified property as a suffix. Return null if no suffix should
		 * be appended. Sorted alphabetically and separated by '_'.
		 * 
		 * @param property
		 * @param nameMapper
		 * @return
		 */
		public <T extends Comparable<T>> AdvancedStateMap.Builder withPropertySuffix(IProperty<T> property,
		 ...
		 * @return
		 */
		public AdvancedStateMap.Builder withPropertySuffix(IProperty<Boolean> property, @Nullable String nameFalse,
				@Nullable String nameTrue) {
		 ...
		 * @return
		 */
		public AdvancedStateMap.Builder withPropertySuffixFalse(IProperty<Boolean> property,
				@Nullable String nameFalse) {
		 ...
		 * @return
		 */
		public AdvancedStateMap.Builder withPropertySuffixExclusions(
				Function<UnmodifiablePropertiesMap, List<IProperty<?>>> exclusions) {
		 ...
		 * @return
		 */
		public AdvancedStateMap.Builder withPropertyExclusions(
				Function<UnmodifiablePropertiesMap, List<IProperty<?>>> exclusions) {
		...
		/**
		 * Add properties that will not be used to compute all possible states of a
		 * block, used for block rendering to ignore some property that does not alter
		 * block's appearance
		 */
		...
		public AdvancedStateMap build() {
			...
			return new AdvancedStateMap(this.nameMapper, this.suffix, this.ignored, this.suffixProperties,
					this.suffixExclusions, this.propertyExclusions);
		}
	}
