...
public class FogGenerator {
	...
	public float[] getFogRange(float min, float max) {
		...
		if (this.fogChunkNoise == null || this.lastCX != cx || this.lastCZ != cz) {
			this.lastCX = cx;
			this.lastCZ = cz;
			this.fogChunkNoise = this.fogNoiseGen.getRegion(this.fogChunkNoise, (double) (cx * 16), (double) (cz * 16),
		...
		float newMin = (float) (noise < 0.75F ? min + diff / 2.0F
				: (min + diff / 2.0F * (1.0F - (noise - 0.75F) * 2.0F)));
