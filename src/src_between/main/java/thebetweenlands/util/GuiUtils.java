...
public class GuiUtils {
	...
	public static void renderTexturedCircleSegment(int segments, double maxAngle, double wrapAngle, double radius,
			double innerRadius, double minU, double maxU, double minV, double maxV) {
		...
		for (int i = 0; i < segments; i++) {
			...
			if (textureU % diffU > nextTextureU % diffU) {
				...
				double interpolatedLen = Math
						.sqrt(interpolatedSin * interpolatedSin + interpolatedCos * interpolatedCos);
				...
				GL11.glVertex2d(interpolatedSin / interpolatedLen * innerRadius,
						interpolatedCos / interpolatedLen * innerRadius);
	...
	public static void renderTexturedCircleSegment(int segments, double maxAngle, double wrapAngle, double wrapRadius,
			double radius, double innerRadius, double minU, double maxU, double minV, double maxV) {
		...
		for (int i = 0; i < MathHelper.ceil(requiredSegments); i++) {
			...
			renderTexturedCircleSegment(segments, maxAngle, wrapAngle, renderOuterRadius + innerRadius,
					renderInnerRadius + innerRadius, minU, maxU, textureV, textureVOuter);
		}
	}

	/**
	 * Renders a texture mapped circle segment with wrapping textures. UVs in the
	 * range [0,1][0,1]
	 * 
	 ...
	 */
	public static void renderMappedCircleSegmentWrapped(int segments, double maxAngle, double wrapAngle,
			double wrapRadius, double radius, double innerRadius, double borderWidth, double sminU, double smaxU,
			double sminV, double smaxV, double b1minU, double b1maxU, double b1minV, double b1maxV, double b2minU,
			double b2maxU, double b2minV, double b2maxV, double b3minU, double b3maxU, double b3minV, double b3maxV,
			double b4minU, double b4maxU, double b4minV, double b4maxV, double c1minU, double c1maxU, double c1minV,
			double c1maxV, double c2minU, double c2maxU, double c2minV, double c2maxV, double c3minU, double c3maxU,
			double c3minV, double c3maxV, double c4minU, double c4maxU, double c4minV, double c4maxV) {
		...
		// Inner segment
		GlStateManager.pushMatrix();
		renderTexturedCircleSegment(segments, innerSegmentMaxAngle, wrapAngle, wrapRadius, radius - borderWidth,
				innerRadius + borderWidth, sminU, smaxU, sminV, smaxV);
		GlStateManager.popMatrix();
		...
		// Border 2
		GlStateManager.pushMatrix();
		renderTexturedCircleSegment(segments, innerSegmentMaxAngle, wrapAngle, radius, radius - borderWidth, b2minU,
				b2maxU, b2maxV, b2minV);
		GlStateManager.popMatrix();
		...
		// Border 4
		GlStateManager.pushMatrix();
		renderTexturedCircleSegment(segments, innerSegmentMaxAngle, wrapAngleInner, innerRadius + borderWidth + 0.05D,
				innerRadius, b4minU, b4maxU, b4maxV, b4minV);
		GlStateManager.popMatrix();
