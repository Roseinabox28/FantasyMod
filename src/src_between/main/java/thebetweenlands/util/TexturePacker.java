...
public class TexturePacker {
	...
	public TexturePacker(ResourceLocation textureName, List<TextureQuadMap> textureMaps) {
		...
		System.arraycopy(TextureUtil.MISSING_TEXTURE.getTextureData(), 0,
				((DataBufferInt) this.missingTexture.getRaster().getDataBuffer()).getData(), 0, 16 * 16);
	}
	...
	protected ResourceLocation generateNewTextureLocation() {
		ResourceLocation rl = new ResourceLocation(this.textureName.getNamespace(),
				this.textureName.getPath() + "_" + this.packedTextureID);
	...
	protected static void copySubImage(BufferedImage source, int x, int y, int w, int h, BufferedImage dest, int x2,
			int y2) {
