...
public class ReparameterizedSpline implements ISpline {
	 ...
	 * @param segmentSubdivs The number of segment subdivisions to approximate
	 *                       length
	 */
	public ReparameterizedSpline init(int subdivs, int segmentSubdivs) {
		...
		for (int i = 1; i <= this.spline.getNumSegments(); i++) {
			this.segmentLengths[i] = this.segmentLengths[i - 1]
					+ this.getArcLength(segmentDelta * (i - 1), segmentDelta * i, subdivs);
		}
	...
	/**
	 * Approximates the interpolation parameter to get to the specified length of
	 * the spline
	...
	private double getCurveParameter(double s) {
		...
		for (int i = 0; i < this.maxNewtonIter; i++) {
			double error = this.segmentLengths[segment - 1]
					+ this.getArcLength(segmentT, segmentT + t, this.segmentSubdivs) - s;
