...
/**
 * OpenGL stencil wrapper class that exposes only safe operations that can't
 * modify unreserved stencil bits. Stencil resources must be released with
 * {@link #close()} and should be used in try-with-resources statements.
 */
public class Stencil implements AutoCloseable {
	...
	/**
	 * Whether the stencil is valid, i.e. has been reserved and is compatible with
	 * the FBO.
	 */
	...
	/**
	 * Applies stencil operation on reserved bit mask, see
	 * {@link GL11#glStencilOp(int, int, int)}
	 * 
	 * @param fail  Stencil action function to be applied when stencil comparison
	 *              function fails
	...
	/**
	 * Applies the same stencil operation to all three cases. See
	 * {@link #op(int, int, int)}
	 * 
	...
	/**
	 * Applies stencil function with reserved bit mask, see
	 * {@link GL11#glStencilFunc(int, int, int)}
	 * 
	...
	/**
	 * Tries to reserve a stencil bit. Check if returned stencil is valid and can be
	 * used with {@link #valid()}. Call {@link #close()} to release the stencil bit.
	 * 
