...
public class BetweenlandsJEIPlugin implements IModPlugin {
	...
	public void register(IModRegistry registry) {
		...
		registry.handleRecipes(ShapelessOverrideDummyRecipe.class,
				recipe -> new ShapelessOverrideRecipeJEI(jeiHelper, recipe), VanillaRecipeCategoryUid.CRAFTING);
		registry.handleRecipes(ShapedOverrideDummyRecipe.class,
				recipe -> new ShapedOverrideRecipeJEI(jeiHelper, recipe), VanillaRecipeCategoryUid.CRAFTING);

		registry.addIngredientInfo(ItemWeedwoodRowboat.getTarred(), VanillaTypes.ITEM,
				"jei.thebetweenlands.tarred_weedwood_boat");
		...
		recipeTranferRegistry.addRecipeTransferHandler(ContainerWeedwoodWorkbench.class,
	...
	private void addDynamicRecipes(IModRegistry registry) {
		...
		// MummyBait
		registry.handleRecipes(RecipeMummyBait.class, recipe -> new MummyBaitRecipeJEI(),
				VanillaRecipeCategoryUid.CRAFTING);

		// Vials
		recipes.add(new ShapelessOreRecipe(null, new ItemStack(ItemRegistry.DENTROTHYST_VIAL, 1, 0),
				new ItemStack(ItemRegistry.ASPECT_VIAL, 1, 0)).setRegistryName(ModInfo.ID,
						RecipeRegistry.ASPECT_VIAL.getPath() + "_green"));
		recipes.add(new ShapelessOreRecipe(null, new ItemStack(ItemRegistry.DENTROTHYST_VIAL, 1, 2),
				new ItemStack(ItemRegistry.ASPECT_VIAL, 1, 1)).setRegistryName(ModInfo.ID,
						RecipeRegistry.ASPECT_VIAL.getPath() + "_orange"));

		// Plant Tonic
		recipes.add(new ShapelessOreRecipe(null, new ItemStack(ItemRegistry.BL_BUCKET_PLANT_TONIC, 1, 0),
				ItemRegistry.BL_BUCKET.withFluid(0, FluidRegistry.SWAMP_WATER), ItemRegistry.SAP_BALL)
						.setRegistryName(ModInfo.ID, RecipeRegistry.PLANT_TONIC.getPath() + "_weedwood"));
		recipes.add(new ShapelessOreRecipe(null, new ItemStack(ItemRegistry.BL_BUCKET_PLANT_TONIC, 1, 1),
				ItemRegistry.BL_BUCKET.withFluid(1, FluidRegistry.SWAMP_WATER), ItemRegistry.SAP_BALL)
						.setRegistryName(ModInfo.ID, RecipeRegistry.PLANT_TONIC.getPath() + "_syrmorite"));

		// Bone Wayfinder
		recipes.add(new ShapelessOreRecipe(null, new ItemStack(ItemRegistry.BONE_WAYFINDER, 1, 0),
				Function.identity().andThen(o -> {
					((ItemBoneWayfinder) ((ItemStack) o).getItem()).setBoundWaystone((ItemStack) o,
							new BlockPos(0, 0, 0));
					return o;
				}).apply(new ItemStack(ItemRegistry.BONE_WAYFINDER, 1, 0)))
						.setRegistryName(RecipeRegistry.CLEAR_BONE_WAYFINDER));
		...
		// Lurker skin
		{
			...
			for (int i = 0; i < 3; i++) {
				...
				recipes.add(new ShapedOreRecipe(null, output.copy(), "LLL", "LPL", "LLL", 'L', skin, 'P', input.copy())
						.setRegistryName(ModInfo.ID, RecipeRegistry.LURKER_POUCH.getPath() + "_" + i));
			}
		}
		...
		// Grappling hook upgrade
		{
			...
			recipes.add(new ShapelessOreRecipe(null, output, tooth, tooth.copy(), tongue, input)
					.setRegistryName(ModInfo.ID, RecipeRegistry.GRAPPLING_HOOK_UPGRADE.getPath() + "_0"));
		}
		...
		// MarshRunnerBoots
		registry.handleRecipes(RecipeMarshRunnerBoots.class, recipe -> new MarshRunnerBootsRecipeJEI(),
				VanillaRecipeCategoryUid.CRAFTING);

		// Life Crystal
		registry.handleRecipes(RecipesLifeCrystal.class, recipe -> new LifeCrystalRecipeJEI(jeiHelper.getGuiHelper()),
				VanillaRecipeCategoryUid.CRAFTING);

		// Coating
		CoatingRecipeJEI.setCoatableItems();
		registry.handleRecipes(RecipesCoating.class, recipe -> new CoatingRecipeJEI(jeiHelper.getGuiHelper()),
				VanillaRecipeCategoryUid.CRAFTING);

		// Sap Spit Cleaning
		registry.handleRecipes(RecipeSapSpitCleanTool.class, recipe -> new SapCleanRecipeJEI(jeiHelper.getGuiHelper()),
				VanillaRecipeCategoryUid.CRAFTING);

		// CircleGems
		CircleGemsRecipeJEI.updateApplicableItems();
		registry.handleRecipes(RecipesCircleGems.class, recipe -> new CircleGemsRecipeJEI(jeiHelper.getGuiHelper()),
				VanillaRecipeCategoryUid.CRAFTING);

		// Book merging
		registry.handleRecipes(BookMergeRecipe.class, recipe -> new BookMergeRecipeJEI(jeiHelper.getGuiHelper()),
				VanillaRecipeCategoryUid.CRAFTING);

		// Tarring recipe
		registry.handleRecipes(HearthgroveTarringRecipe.class, recipe -> new TarringRecipeJEI(jeiHelper.getGuiHelper()),
				VanillaRecipeCategoryUid.CRAFTING);
	...
	public void registerItemSubtypes(ISubtypeRegistry subtypeRegistry) {
		...
		/*
		 * subtypeRegistry.registerSubtypeInterpreter(ItemRegistry.ASPECT_VIAL,
		 * itemStack -> { NBTTagCompound nbtTagCompound = itemStack.getTagCompound(); if
		 * (nbtTagCompound == null || nbtTagCompound.hasNoTags()) { return
		 * itemStack.getItemDamage() + ISubtypeRegistry.ISubtypeInterpreter.NONE; }
	...
	public void registerCategories(IRecipeCategoryRegistration registry) {
		...
		registry.addRecipeCategories(new CompostRecipeCategory(), new AnimatorRecipeCategory(),
				new DruidAltarRecipeCategory(), new PestleAndMortarCategory(), new PurifierRecipeCategory());
	}
			...
			if (slotIndex == ouputIndex) {
				...
				if (showAdvanced) {
					tooltip.add(TextFormatting.DARK_GRAY
							+ I18n.translateToLocalFormatted("jei.tooltip.recipe.id", registryName.toString()));
				}
			}
		});
	}
