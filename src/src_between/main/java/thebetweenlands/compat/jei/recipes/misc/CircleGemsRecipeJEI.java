...
public class CircleGemsRecipeJEI implements ICraftingRecipeWrapper, ICustomCraftingRecipeWrapper {
	...
	public static void updateApplicableItems() {
		...
		for (ItemStack stack : BetweenlandsJEIPlugin.ingredientRegistry.getAllIngredients(VanillaTypes.ITEM)) {
			if (!stack.isEmpty() && CircleGemHelper.isApplicable(stack.getItem())
					&& (BetweenlandsConfig.COMPATIBILITY.showNonBLGemRecipes
							|| ModInfo.ID.equals(stack.getItem().getRegistryName().getNamespace())))
	...
	public void getIngredients(IIngredients ingredients) {
		...
		ItemStack[] gems = new ItemStack[] { new ItemStack(ItemRegistry.CRIMSON_MIDDLE_GEM),
				new ItemStack(ItemRegistry.GREEN_MIDDLE_GEM), new ItemStack(ItemRegistry.AQUA_MIDDLE_GEM) };
	...
	public void setRecipe(IRecipeLayout recipeLayout, IIngredients ingredients) {
		...
		if (focus != null && focus.getValue() instanceof ItemStack
				&& (CircleGemHelper.isApplicable(((ItemStack) focus.getValue()).getItem())
						|| ((ItemStack) focus.getValue()).getItem() instanceof ItemGem)) {
			if (focus.getMode() == IFocus.Mode.INPUT) {
				if (!(((ItemStack) focus.getValue()).getItem() instanceof ItemGem)) {
					...
					while (outIterator.hasNext()) {
						...
						inIterator.next();
						if (CircleGemHelper
								.getGem(stack) != ((ItemGem) ((ItemStack) focus.getValue()).getItem()).type) {
							outIterator.remove();
							inIterator.remove();
						}
					}
				...
				inputs.get(0).removeIf(stack -> stack.getItem() != ((ItemStack) focus.getValue()).getItem()
						|| CircleGemHelper.getGem(stack) != CircleGemType.NONE);
				inputs.get(1)
						.removeIf(stack -> CircleGemHelper.getGem(((ItemStack) focus.getValue())) != CircleGemType.NONE
								&& ((ItemGem) stack.getItem()).type != CircleGemHelper
										.getGem(((ItemStack) focus.getValue())));
				outputs.get(0).removeIf(stack -> stack.getItem() != ((ItemStack) focus.getValue()).getItem());
				if (CircleGemHelper.getGem(((ItemStack) focus.getValue())) != CircleGemType.NONE)
					outputs.get(0).removeIf(stack -> CircleGemHelper
							.getGem(((ItemStack) focus.getValue())) != CircleGemHelper.getGem(stack));
			}
		}
