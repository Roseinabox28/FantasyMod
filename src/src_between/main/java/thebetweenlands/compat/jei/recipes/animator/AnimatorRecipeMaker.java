...
public class AnimatorRecipeMaker {
	...
	public static List<IRecipeWrapper> getRecipes() {
		...
		for (IAnimatorRecipe recipe : AnimatorRecipe.getRecipes()) {
			if (recipe instanceof AnimatorRecipe || (recipe instanceof ToolRepairAnimatorRecipe
					&& isValidToolRecipe((ToolRepairAnimatorRecipe) recipe)))
	...
	/*
	 * @Nonnull public static List<AnimatorRecipeJEI> getRecipesRuntime(World world)
	 * { ArrayList<AnimatorRecipeJEI> recipes = new ArrayList<AnimatorRecipeJEI>();
	 * for (IAnimatorRecipe recipe : AnimatorRecipe.getRecipes()) { if (recipe
	 * instanceof AnimatorRecipe) if (((AnimatorRecipe) recipe).getLootTable() !=
	 * null) recipes.add(new AnimatorRecipeJEI((AnimatorRecipe) recipe, world)); }
	 * return recipes; }
	 */
}
