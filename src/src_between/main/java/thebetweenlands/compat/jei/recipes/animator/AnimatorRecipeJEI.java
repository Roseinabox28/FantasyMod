...
public class AnimatorRecipeJEI implements IRecipeWrapper {
	...
	public void getIngredients(IIngredients ingredients) {
		...
		if (lootTableName != null) {
			ingredients.setOutputLists(VanillaTypes.ITEM, Collections.singletonList(
					LootTableRegistry.getItemsFromTable(lootTableName, Minecraft.getMinecraft().world, true)));
		}
	}
	...
	public void drawInfo(Minecraft minecraft, int recipeWidth, int recipeHeight, int mouseX, int mouseY) {
		if (entity == null && animatorRecipe.getSpawnEntityClass(input) != null) {
			try {
				entity = animatorRecipe.getSpawnEntityClass(input).getConstructor(new Class[] { World.class })
		...
		if (entity != null) {
			...
			Gui.drawRect(fontX - 1, -6, fontX + minecraft.fontRenderer.getStringWidth(entityName) + 1, 10 - 6,
					0xAA000000);
	...
	public List<String> getTooltipStrings(int mouseX, int mouseY) {
		...
		if (mouseX >= 18 && mouseX <= 51 && mouseY >= 42 && mouseY <= 66) {
			float lifeAmount = guiIngredient != null
					? animatorRecipe.getRequiredLife(guiIngredient.getDisplayedIngredient())
					: requiredLife;
			processTooltip.add(TranslationHelper.translateToLocal("jei.thebetweenlands.animator.life",
					LIFE_CRYSTAL_PERCENTAGE.format((lifeAmount / 128F))));
		}
		if (mouseX >= 57 && mouseX <= 90 && mouseY >= 42 && mouseY <= 66) {
			processTooltip.add(TranslationHelper.translateToLocal("jei.thebetweenlands.animator.fuel",
					guiIngredient != null ? animatorRecipe.getRequiredFuel(guiIngredient.getDisplayedIngredient())
							: requiredFuel));
		}
		if (entityName != null && mouseX >= 37 && mouseX <= 71 && mouseY >= 7 && mouseY <= 41) {
			processTooltip
					.add(TranslationHelper.translateToLocal("jei.thebetweenlands.animator.entity_spawn", entityName));
		}
