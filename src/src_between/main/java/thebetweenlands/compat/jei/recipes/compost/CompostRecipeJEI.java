...
public class CompostRecipeJEI implements IRecipeWrapper {
	...
	public void getIngredients(IIngredients ingredients) {
		...
		ingredients.setOutput(VanillaTypes.ITEM,
				new ItemStack(ItemRegistry.ITEMS_MISC, 1, ItemMisc.EnumItemMisc.COMPOST.getID()));
	}
	...
	public List<String> getTooltipStrings(int mouseX, int mouseY) {
		...
		if (mouseX >= 18 && mouseX <= 39 && mouseY >= 3 && mouseY <= 18) {
			...
			processTooltip.add(
					seconds > 0 && minutes > 0 ? TranslationHelper.translateToLocal(processTimeString, minutes, seconds)
							: minutes > 0 ? TranslationHelper.translateToLocal(processTimeMinutesString, minutes)
									: TranslationHelper.translateToLocal(processTimeSecondsString, seconds));
