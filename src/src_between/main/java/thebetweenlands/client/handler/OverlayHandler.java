...
public class OverlayHandler {
	...
	private static final ResourceLocation TAR_OVERLAY = new ResourceLocation(ModInfo.ID,
			"textures/gui/overlay/tar.png");
	private static final ResourceLocation STAGNANT_WATER_OVERLAY = new ResourceLocation(ModInfo.ID,
			"textures/gui/overlay/stagnant_water.png");
	...
	public static void onRenderOverlay(RenderBlockOverlayEvent event) {
		if (event.getOverlayType() == OverlayType.WATER) {
			...
			for (int i = 0; i < 8; ++i) {
				...
				if (checkState.getMaterial() == BLMaterialRegistry.TAR || checkState.getMaterial() == Material.WATER) {
					state = checkState;
				}
			}
