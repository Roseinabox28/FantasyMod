...
public class TextureStitchHandler {
	...
	public void onTextureStitchPre(TextureStitchEvent.Pre e) {
		...
		Map<ResourceLocation, BufferedImage> packedTextures = ModelRegistry.MODEL_TEXTURE_PACKER
				.pack(Minecraft.getMinecraft().getResourceManager());

		TheBetweenlands.logger
				.info("Packed model textures in " + ((System.nanoTime() - packingStartTime) / 1000000.0f) + "ms");
		TheBetweenlands.logger.info("Optimal footprint: " + ModelRegistry.MODEL_TEXTURE_PACKER.getOptimalFootprint()
				+ "px^2, Packed footprint: " + ModelRegistry.MODEL_TEXTURE_PACKER.getPackedFootprint() + "px^2");
		...
		for (Entry<ResourceLocation, BufferedImage> packedTexture : packedTextures.entrySet()) {
			e.getMap()
					.setTextureEntry(new TextureFromData(packedTexture.getKey().toString(), packedTexture.getValue()));
		}
		...
		for (Item item : ItemRegistry.ITEMS) {
			if (item instanceof ICorrodible) {
				...
				for (ResourceLocation variant : variants) {
					try {
						ResourceLocation modelLocation = new ResourceLocation(variant.getNamespace(),
								"item/" + variant.getPath());
						...
						for (ResourceLocation texture : textures) {
							...
							if (path.contains("/")) {
								...
								if (fileName.endsWith(corrodibleSuffix)) {
									ResourceLocation completeBaseTextureLocation = new ResourceLocation(
											texture.getNamespace(),
											String.format("textures/%s.png", texture.getPath()));
									for (int n = 0; n < CorrosionHelper.CORROSION_STAGE_COUNT; n++) {
										String corrosionSpriteName = texture.getNamespace() + ":"
												+ path.substring(0, path.length() - corrodibleSuffix.length())
												+ "_corrosion_" + n;
										TextureCorrosion corrosionTexture = new TextureCorrosion(corrosionSpriteName,
												completeBaseTextureLocation, n, item.getTranslationKey().hashCode());
										// Forcibly sets the texture entry because TextureMap#setTextureEntry doesn't
										// allow
										// overwriting a previously added sprite (usually set in
					...
					try {
						...
						if (cachedFrames != null) {
							...
							if (!sprite.hasAnimationMetadata() || sprite.getFrameCount() == 1) {
								...
								int frameCount = hasFrameMeta ? animationMetadata.getFrameCount()
										: sprite.getFrameCount();
								frames[i] = new Frame[frameCount];
								for (int frame = 0; frame < frameCount; frame++) {
									int duration = hasFrameMeta ? animationMetadata.getFrameTimeSingle(frame)
											: animationMetadata.getFrameTime();
									int index = hasFrameMeta ? animationMetadata.getFrameIndex(frame) : frame;
									int frameData[] = sprite.getFrameTextureData(index)[0]; // Use original non-mipmap
																							// frame
									TextureAtlasSprite frameSprite = new TextureFromData(
											sprite.getIconName() + "_frame_" + frame, frameData, sprite.getIconWidth(),
	...
	private ResourceLocation getResourceLocation(String basePath, TextureAtlasSprite sprite) {
		...
		return new ResourceLocation(resourcelocation.getNamespace(),
				String.format("%s/%s%s", basePath, resourcelocation.getPath(), ".png"));
	}
