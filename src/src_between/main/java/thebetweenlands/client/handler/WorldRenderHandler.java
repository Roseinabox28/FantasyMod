...
public class WorldRenderHandler {
	...
	public static void renderWorld(RenderWorldLastEvent event) {
		...
		if (ShaderHelper.INSTANCE.isWorldShaderActive() && sphereDispList >= 0) {
			...
			if (shader != null) {
				...
				if (gBuffer != null) {
					try (FramebufferStack.State state = FramebufferStack.push()) {
						gBuffer.updateGeometryBuffer(mainFramebuffer.framebufferWidth,
						...
						if (gBuffer.isInitialized()) {
							...
							if (!REPELLER_SHIELDS.isEmpty()) {
								...
								GlStateManager.disableBlend();
								GlStateManager.color(0.0F,
										0.4F + (float) (Math.sin(System.nanoTime() / 500000000.0F) + 1.0F) * 0.2F,
										0.8F - (float) (Math.cos(System.nanoTime() / 400000000.0F) + 1.0F) * 0.2F,
										1.0F);
							...
							if (world != null) {
								for (EntityPlayer player : MC.world.playerEntities) {
									if (player instanceof AbstractClientPlayer
										...
										IEntityCustomCollisionsCapability cap = player.getCapability(
												CapabilityRegistry.CAPABILITY_ENTITY_CUSTOM_BLOCK_COLLISIONS, null);
										...
										if (cap != null) {
											...
											if (obstructionDistance < 0) {
												...
												renderPlayerGlow(clientPlayer, strength, 0.98F,
			...
			GlStateManager.alphaFunc(GL11.GL_GREATER, 0.0F);
			GlStateManager.color(0.0F,
					(0.4F + (float) (Math.sin(System.nanoTime() / 500000000.0F) + 1.0F) * 0.2F) / 3.0F,
					(0.8F - (float) (Math.cos(System.nanoTime() / 400000000.0F) + 1.0F) * 0.2F) / 3.0F, 0.3F);
			for (Entry<Vec3d, Float> e : REPELLER_SHIELDS) {
				Vec3d pos = e.getKey();
				GlStateManager.pushMatrix();
				GlStateManager.translate(pos.x, pos.y, pos.z);
		...
		if (ShaderHelper.INSTANCE.isWorldShaderActive() && (!DefaultParticleBatches.HEAT_HAZE_PARTICLE_ATLAS.isEmpty()
				|| !DefaultParticleBatches.HEAT_HAZE_BLOCK_ATLAS.isEmpty())) {
			ShaderHelper.INSTANCE.require();
		}
			...
			if (fbo != null) {
				try (FramebufferStack.State state = FramebufferStack.push()) {
					...
					if (!DefaultParticleBatches.GAS_CLOUDS_HEAT_HAZE.isEmpty()) {
						...
						BatchedParticleRenderer.INSTANCE.renderBatch(DefaultParticleBatches.GAS_CLOUDS_HEAT_HAZE,
								MC.getRenderViewEntity(), event.getPartialTicks());
					}

					if (!DefaultParticleBatches.HEAT_HAZE_PARTICLE_ATLAS.isEmpty()) {
						BatchedParticleRenderer.INSTANCE.renderBatch(DefaultParticleBatches.HEAT_HAZE_PARTICLE_ATLAS,
								MC.getRenderViewEntity(), event.getPartialTicks());
					}

					if (!DefaultParticleBatches.HEAT_HAZE_BLOCK_ATLAS.isEmpty()) {
						BatchedParticleRenderer.INSTANCE.renderBatch(DefaultParticleBatches.HEAT_HAZE_BLOCK_ATLAS,
								MC.getRenderViewEntity(), event.getPartialTicks());
					}
		...
		if (ShaderHelper.INSTANCE.isWorldShaderActive()) {
			...
			if (shader != null) {
				BetweenlandsWorldStorage worldStorage = BetweenlandsWorldStorage
						.forWorld(Minecraft.getMinecraft().world);
	...
	public static void renderPlayerGlow(AbstractClientPlayer player, float strength, float alpha, float partialTicks) {
		...
		playerRenderer.doRender(player, d0 - MC.getRenderManager().renderPosX, d1 - MC.getRenderManager().renderPosY,
				d2 - MC.getRenderManager().renderPosZ, f, partialTicks);
