...
public class FogHandler {
	...
	public static boolean hasDenseFog(World world) {
		...
		return eeRegistry.denseFog.isActive()
				&& Minecraft.getMinecraft().player.posY > WorldProviderBetweenlands.CAVE_START;
	}
	...
	public static void onFogColor(FogColors event) {
		Entity renderView = Minecraft.getMinecraft().getRenderViewEntity();
		if (renderView != null) {
			IBlockState blockState = ActiveRenderInfo.getBlockStateAtEntityViewpoint(renderView.world, renderView,
			...
			if (blockState.getBlock() instanceof BlockSwampWater) {
				BlockPos pos = new BlockPos(
						ActiveRenderInfo.projectViewFromEntity(renderView, (float) event.getRenderPartialTicks()));
				int colorMultiplier = Minecraft.getMinecraft().getBlockColors().colorMultiplier(blockState,
						renderView.world, pos, 0);
				...
				Vec3d fogColor = provider.getFogColor(
						renderView.getEntityWorld().getCelestialAngle((float) event.getRenderPartialTicks()),
						(float) event.getRenderPartialTicks());
	...
	public static void onFogDensity(FogDensity event) {
		...
		if (renderView != null) {
			Block block = ActiveRenderInfo
					.getBlockStateAtEntityViewpoint(renderView.world, renderView, (float) event.getRenderPartialTicks())
					.getBlock();
			if (block instanceof BlockSwampWater) {
				fogMode = GL11.GL_EXP;
				GlStateManager.setFog(FogMode.EXP);
				if (renderView instanceof EntityLivingBase && ((EntityLivingBase) renderView)
						.isPotionActive(MobEffects.WATER_BREATHING/* Water breathing */)) {
	...
	public static void updateFog(UpdateFogEvent event) {
		...
		if (hasDenseFog(world)) {
			...
			float denseFogStart = state.getFixedFogStart(biomeFog.getStart())
					/ Math.max(8.0f / (1.0F + uncloudedStrength * 4.0F) * lowViewDistanceFogReduction, 1) * range[0];
			float denseFogEnd = state.getFixedFogEnd(biomeFog.getEnd())
					/ Math.max(3.0f / (1.0F + uncloudedStrength * 2.0F) * lowViewDistanceFogReduction, 1) * range[1];
		...
		for (ILocalStorage storage : worldStorage.getLocalStorageHandler().getLoadedStorages()) {
			if (storage instanceof LocationStorage
					&& ((LocationStorage) storage).getType() == EnumLocationType.SPIRIT_TREE) {
		...
		if (closestSpiritTree >= 0) {
			...
			fog.setRed(fog.getRed() + (0.58F - fog.getRed()) * strength)
					.setGreen(fog.getGreen() + (0.58F - fog.getGreen()) * strength)
					.setBlue(fog.getBlue() + (0.58F - fog.getBlue()) * strength);
