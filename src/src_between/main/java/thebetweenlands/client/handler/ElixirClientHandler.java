			...
			if (player != null && player.world != null && player.world.isRemote
				...
				if (ElixirEffectRegistry.EFFECT_HUNTERSSENSE.isActive(player)) {
					...
					List<Entity> entityList = world.getEntitiesWithinAABB(Entity.class,
							player.getEntityBoundingBox().grow(50, 50, 50));
					...
					for (int i = 0; i < pointCount; i++) {
						...
						if ((player.ticksExisted - MathHelper
								.floor((float) crawlTicks / (float) EntityTrail.MAX_CACHE_SIZE * (float) tp.index))
								% crawlTicks == 0) {
							Vec3d pos = tp.pos;
							if (tp.nextPos != null) {
								int subSegments = 10;
								...
								for (int s = 0; s <= subSegments; s++) {
									if ((player.ticksExisted
											- MathHelper.floor((float) crawlTicks / (float) EntityTrail.MAX_CACHE_SIZE
													* (float) (tp.index + s / (float) subSegments)))
											% crawlTicks == 0) {
										...
										BLParticles.PORTAL.spawn(world, tpxi, tpyi, tpzi,
												ParticleFactory.ParticleArgs.get().withScale(0.3F));
									}
								}
							} else {
								double tpx = pos.x + 0.5F;
								double tpy = pos.y + 0.05F;
								double tpz = pos.z + 0.5F;
								BLParticles.PORTAL.spawn(world, tpx, tpy, tpz,
										ParticleFactory.ParticleArgs.get().withScale(0.3F));
							}
						}
					}
					...
					if (Minecraft.getMinecraft().gameSettings.keyBindAttack.isKeyDown()
						...
						try {
							...
							if (target == null || target.entityHit != null
									|| target.typeOfHit == RayTraceResult.Type.MISS) {
	...
	public void onRenderWorld(RenderWorldLastEvent event) {
		...
		if (player != null && player.world != null) {
			if (ElixirEffectRegistry.EFFECT_SAGITTARIUS.isActive(player)) {
				ArrowPredictionRenderer.render(
						Math.min((ElixirEffectRegistry.EFFECT_SAGITTARIUS.getStrength(player) + 1) / 3.0F, 1.0F));
			}
		}
	}
}
