...
public class CameraPositionHandler {
	...
	public void onRenderTickStart(RenderTickEvent event) {
		...
		if (renderViewEntity != null) {
			...
			List<LocationCragrockTower> towers = worldData.getLocalStorageHandler().getLocalStorages(
					LocationCragrockTower.class, renderViewEntity.posX, renderViewEntity.posZ, location -> location
							.getInnerBoundingBox().grow(4, 4, 4).contains(renderViewEntity.getPositionVector()));
			for (LocationCragrockTower tower : towers) {
				if (tower.isCrumbling()) {
					shakeStrength += Math.min(
							Math.pow((tower.getCrumblingTicks() + event.renderTickTime) / 400.0f, 4) * 0.08f, 0.08f);
				}
			}

			// Ring of Summoning
			List<EntityPlayer> nearbyPlayers = renderViewEntity.world.getEntitiesWithinAABB(EntityPlayer.class,
					renderViewEntity.getEntityBoundingBox().grow(32, 32, 32),
					entity -> entity.getDistance(renderViewEntity) <= 32.0D);

			for (EntityPlayer player : nearbyPlayers) {
				ISummoningCapability cap = player.getCapability(CapabilityRegistry.CAPABILITY_SUMMON, null);
				if (cap != null) {
					if (cap.isActive()) {
						shakeStrength += (ItemRingOfSummoning.MAX_USE_TIME - cap.getActiveTicks())
								/ (float) ItemRingOfSummoning.MAX_USE_TIME * 0.1F + 0.01F;
					}
				}
			}
