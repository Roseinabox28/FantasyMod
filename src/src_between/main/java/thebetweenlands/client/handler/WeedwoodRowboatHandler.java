...
public final class WeedwoodRowboatHandler {
	...
	public void onOverlayRender(RenderGameOverlayEvent event) {
		if (isPlayerInRowboat) {
			if (event instanceof RenderGameOverlayEvent.Pre
				...
				// Set render view entity to player during GUI overlay rendering so that HUD
				// renders
	...
	// Do this so that when the player enters the rowboat they are rotated correctly
	// after a SetPassengers packet is recieved
	private void onMacgyveredGameLoop() {
		...
		if (riding instanceof EntityWeedwoodRowboat && riding.getControllingPassenger() == player) {
			if (!isPlayerInRowboat) {
				...
				player.prevRotationYawHead = player.rotationYawHead = player.prevRotationYaw = player.rotationYaw = MathHelper
						.wrapDegrees(riding.rotationYaw - 180);
		...
		public void mouseXYChange() {
			...
			if (entity instanceof RowboatCam) {
				...
				if (riding instanceof EntityWeedwoodRowboat) {
					...
					cam.prevRotationPitch = cam.rotationPitch = MathHelper.clamp(cam.rotationPitch - deltaY * 0.15F, 0,
							90);
	...
	private class RowboatCam extends Entity {
		...
		private double getDistance(World world, double x, double y, double z, float yaw, float pitch) {
			...
			for (int zyx = 0; zyx < 8; zyx++) {
				...
				RayTraceResult vector = world.rayTraceBlocks(new Vec3d(x + dx, y + dy, z + dz),
						new Vec3d(x - extentX + dx, y - extentY + dy, z - extentZ + dz), false, true, false);
				if (vector != null) {
