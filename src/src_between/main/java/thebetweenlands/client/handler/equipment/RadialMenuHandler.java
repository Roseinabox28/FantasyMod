...
public class RadialMenuHandler {
	...
	public void onMouseInput(MouseEvent event) {
		if (this.isOpen) {
			...
			double mouseY = res.getScaledHeight()
					- (this.prevMouseY * res.getScaledHeight()) / Minecraft.getMinecraft().displayHeight;
							...
							if (equippable.getEquipmentCategory(stack) == type) {
								if (equippable.canEquip(stack, player, player)) {
									...
									if (res.isEmpty() || res.getCount() != stack.getCount()) {
										categories.add(new Categories.EquipCategory(
												I18n.format("equipment.menu.equip", stack.getDisplayName()), 0x6010AA10,
												0xDD10AA10, stack, type, i));
									}
								}
							}
						}
				...
				for (EnumEquipmentInventory type : EnumEquipmentInventory.VALUES) {
					...
					for (int i = 0; i < inv.getSizeInventory(); i++) {
						...
						if (!stack.isEmpty()) {
							...
							categories.add(new Categories.UnequipCategory(
									I18n.format("equipment.menu.unequip", stack.getDisplayName()), 0x60AA1010,
									0xDDAA1010, stack, type, i));
						}
					}
				}
			}
	...
	public static class Category {
		...
		public void renderCategory(double centerX, double centerY, double dirX, double dirY, double radius,
				double startX, double startY, double angle, double segmentAngle) {
		}
	...
	public void onTick(ClientTickEvent event) {
		if (this.isOpen) {
			if (event.phase == Phase.START) {
				...
				if (this.isOpen) {
					/*
					 * if(this.currentCategory.getCategories().size() > 2) { for(int i = 0; i < 2;
					 * i++) { if(this.displayedCategories <
					 * this.currentCategory.getCategories().size()) { this.displayedCategories++; }
					 * } } else { this.displayedCategories =
					 * this.currentCategory.getCategories().size(); }
					 */
					this.displayedCategories = this.currentCategory.getCategories().size();
				}
			} else {
	...
	public void renderGui(RenderGameOverlayEvent.Post event) {
		if (event.getType() == ElementType.HOTBAR) {
			...
			for (int i = 0; i < segments; i++) {
				...
				GlStateManager.translate(this.guiX + xOffset * segments * 2.8D, this.guiY + yOffset * segments * 2.8D,
						0);

				GlStateManager.rotate(
						180.0F + 360.0F
								/ Math.min(this.currentCategory.getCategories().size(), this.displayedCategories),
						0, 0, 1);
				...
				GuiUtils.renderMappedCircleSegmentWrapped(subSegments, maxAngle, wrapAngle, wrapRadius, radius,
						MathHelper.clamp(innerRadius - segments * 2.5D, 10, innerRadius), 8,
						// Central piece
			...
			for (int i = 0; i < Math.min(this.currentCategory.getCategories().size(), this.displayedCategories); i++) {
				...
				category.renderCategory(this.guiX + xOffset, this.guiY + yOffset, xOffset / dst, yOffset / dst,
						this.radius - this.innerRadius - 8, xOffset / dst * this.innerRadius,
						yOffset / dst * this.innerRadius, (i + 0.5D) * circleAngle, circleAngle);
			}
			...
			for (int i = 0; i < Math.min(this.currentCategory.getCategories().size(), this.displayedCategories); i++) {
				...
				if (endX < this.guiX - 1) {
					Gui.drawRect((int) this.guiX - 100 - width - 1, (int) endY - 8, (int) this.guiX - 100,
							(int) endY + 1, color);
					mc.fontRenderer.drawString(category.getName(), (int) this.guiX - 100 - width, (int) endY - 7,
							textColor);
				} else if (endX > this.guiX + 1) {
					Gui.drawRect((int) this.guiX + 100 - 1, (int) endY - 8, (int) this.guiX + 100 + width,
							(int) endY + 1, color);
					mc.fontRenderer.drawString(category.getName(), (int) this.guiX + 100, (int) endY - 7, textColor);
				} else {
					Gui.drawRect((int) this.guiX - width / 2 - 1, (int) endY - 1, (int) this.guiX - width / 2 + width,
							(int) endY + 8, color);
			...
			if (this.currentCategory != this.rootCategory) {
				...
				mc.fontRenderer.drawString(categoryName, (int) this.guiX - width / 2,
						(int) this.guiY - this.radius - 60, 0xFFFFFFFF);
			}
	...
	private boolean isInside(int x, int y, int category) {
		...
		int segments = Math.min(this.currentCategory.getCategories().size(), this.displayedCategories);
		if (angle >= (int) (category * circleAngle) && angle < (int) ((category + 1) * circleAngle)
				&& length <= this.radius + segments * 2D
				&& length >= MathHelper.clamp(this.innerRadius - segments * 2.8D, 10, this.innerRadius) + segments * 2D
						- 4) {
			return true;
		}
		return false;
