...
public class ScreenRenderHandler extends Gui {
	...
	private static final ResourceLocation DECAY_BAR_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/gui/decay_bar.png");
	private static final ResourceLocation RING_OF_DISPERSION_OVERLAY_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/gui/overlay/ring_of_dispersion_overlay.png");
	private static final ResourceLocation RING_OF_DISPERSION_OVERLAY_TOP_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/gui/overlay/ring_of_dispersion_overlay_top.png");
	private static final ResourceLocation RING_OF_DISPERSION_OVERLAY_SIDE_TOP_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/gui/overlay/ring_of_dispersion_overlay_side_top.png");
	private static final ResourceLocation RING_OF_DISPERSION_OVERLAY_BOTTOM_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/gui/overlay/ring_of_dispersion_overlay_bottom.png");
	private static final ResourceLocation RING_OF_DISPERSION_OVERLAY_SIDE_BOTTOM_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/gui/overlay/ring_of_dispersion_overlay_side_bottom.png");
	private static final ResourceLocation RING_OF_DISPERSION_INDICATOR_OVERLAY_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/gui/overlay/ring_of_dispersion_indicator_overlay.png");
	...
	public static final ResourceLocation TITLE_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/gui/location_title.png");

	public static final ResourceLocation CAVING_ROPE_CONNECTED = new ResourceLocation(
			"thebetweenlands:textures/gui/caving_rope_connected.png");
	public static final ResourceLocation CAVING_ROPE_DISCONNECTED = new ResourceLocation(
			"thebetweenlands:textures/gui/caving_rope_disconnected.png");
	...
	public static List<LocationStorage> getVisibleLocations(Entity entity) {
		...
		return worldStorage.getLocalStorageHandler().getLocalStorages(LocationStorage.class, entity.posX, entity.posZ,
				location -> location.isInside(entity.getPositionEyes(1)) && location.isVisible(entity));
	}
	...
	public void onClientTick(ClientTickEvent event) {
		if (event.phase == Phase.START && !Minecraft.getMinecraft().isGamePaused()) {
			...
			if (player != null) {
				IEntityCustomCollisionsCapability cap = player
						.getCapability(CapabilityRegistry.CAPABILITY_ENTITY_CUSTOM_BLOCK_COLLISIONS, null);
				...
				if (player.dimension == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId) {
					...
					if (locations.isEmpty()) {
						...
						if (highestLocation.getTitleDisplayCooldown(player) == 0) {
							...
							highestLocation.setTitleDisplayCooldown(player, displayCooldown); // Keep cooldown up until
																								// player leaves
																								// location
						}
					}
					...
					if (this.currentLocation.length() > 0) {
						if (this.currentLocation.contains(":")) {
							...
							try {
								...
								this.currentLocation = this.currentLocation.substring(startIndex + 1,
						...
						if (prevLocation != null && !prevLocation.equals(this.currentLocation)) {
							...
							this.titleContainer = new TextContainer(2048, 2048, this.currentLocation,
									TheBetweenlands.proxy.getCustomFontRenderer());
							...
							this.titleContainer
									.registerTag(new FormatTags.TagSimple("obfuscated", TextFormatting.OBFUSCATED));
							this.titleContainer.registerTag(new FormatTags.TagSimple("italic", TextFormatting.ITALIC));
							this.titleContainer.registerTag(
									new FormatTags.TagSimple("strikethrough", TextFormatting.STRIKETHROUGH));
							this.titleContainer
									.registerTag(new FormatTags.TagSimple("underline", TextFormatting.UNDERLINE));
							try {
	...
	public void onRenderGameOverlay(RenderGameOverlayEvent.Post event) {
		...
		if (event.getType() == RenderGameOverlayEvent.ElementType.CROSSHAIRS) {
			 ...
			 * DestFactor.ONE_MINUS_SRC_ALPHA); GlStateManager.color(1, 1, 1, 1);
			 * 
			 * GlStateManager.enableTexture2D(); GL11.glBindTexture(GL11.GL_TEXTURE_2D,
			 * ShaderHelper.INSTANCE.getWorldShader().getRepellerShieldBuffer().
			 * getDiffuseTexture());
			 * 
			 * Tessellator t = Tessellator.getInstance(); BufferBuilder b = t.getBuffer();
			 * b.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX);
			 * 
			 * b.pos(0, 0, 0).tex(0, 1).endVertex(); b.pos(0,
			 * event.getResolution().getScaledHeight(), 0).tex(0, 0).endVertex();
			 * b.pos(event.getResolution().getScaledWidth(),
			 * event.getResolution().getScaledHeight(), 0).tex(1, 0).endVertex();
			...
			if (BetweenlandsConfig.GENERAL.cavingRopeIndicator && player != null) {
				...
				List<EntityRopeNode> ropeNodes = player.world.getEntitiesWithinAABB(EntityRopeNode.class,
						player.getEntityBoundingBox().grow(32, 32, 32));
				...
				if (connected) {
					...
					GlStateManager.alphaFunc(GL11.GL_GREATER, 0.0F);
					GlStateManager.enableBlend();
					GlStateManager.color(1.0F, 1.0F, 1.0F, MathHelper.clamp(
							this.cavingRopeConnectTicks / 80.0F
									* (0.8F + 0.2F * (float) Math
											.sin((this.cavingRopeConnectTicks + 1 - event.getPartialTicks()) / 2.0F)),
							0, 1));
			...
			if (player != null && !player.isSpectator()) {
				if (BetweenlandsConfig.GENERAL.equipmentVisible) {
					IEquipmentCapability capability = player.getCapability(CapabilityRegistry.CAPABILITY_EQUIPMENT,
							null);
					if (capability != null) {
			...
			if (this.titleTicks > 0 && this.titleContainer != null && !this.titleContainer.getPages().isEmpty()) {
				...
				float fade = Math.min(1.0F,
						((float) this.maxTitleTicks - (float) this.titleTicks)
								/ Math.min(40.0F, this.maxTitleTicks - 5.0F) + 0.02F)
						- Math.max(0, (-this.titleTicks + 5) / 5.0F);
				GlStateManager.alphaFunc(GL11.GL_GREATER, 0.0F);
				GlStateManager.enableBlend();
				...
				for (TextSegment segment : page.getSegments()) {
					...
					segment.font.drawString(segment.text, 0, 0,
							ColorUtils.toHex(rgba[0], rgba[1], rgba[2], rgba[3] * fade));
					GlStateManager.color(1, 1, 1, 1);
					GlStateManager.popMatrix();
				...
				BufferBuilder buffer = tessellator.getBuffer();
				buffer.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX);
				this.renderTexturedRect(buffer, strX - sidePadding * averageScale, sy,
						strX - sidePadding * averageScale + 9 * averageScale, ey, 0, 9 / 128.0D, 0, 1);
				this.renderTexturedRect(buffer, strX - sidePadding * averageScale + 9 * averageScale, sy,
						strX + strWidth / 2.0D - 6 * averageScale, ey, 9 / 128.0D, 58 / 128.0D, 0, 1);
				this.renderTexturedRect(buffer, strX + strWidth / 2.0D - 6 * averageScale, sy,
						strX + strWidth / 2.0D + 6 * averageScale, ey, 58 / 128.0D, 70 / 128.0D, 0, 1);
				this.renderTexturedRect(buffer, strX + strWidth / 2.0D + 6 * averageScale, sy,
						strX + strWidth + sidePadding * averageScale - 9 * averageScale, ey, 70 / 128.0D, 119 / 128.0D,
						0, 1);
				this.renderTexturedRect(buffer, strX + strWidth + sidePadding * averageScale - 9 * averageScale, sy,
						strX + strWidth + sidePadding * averageScale, ey, 119 / 128.0D, 1, 0, 1);
				tessellator.draw();
				GlStateManager.alphaFunc(GL11.GL_GREATER, 0.1F);
			}
			...
			if (player != null) {
				...
				if (cap != null && cap.isInPortal()) {
					this.renderPortal(mc,
							MathHelper.clamp(
									(1.0F - cap.getTicksUntilTeleport() / (float) PlayerPortalHandler.MAX_PORTAL_TIME),
									0, 1),
							event.getResolution());
				}
			}
	...
	private void renderDispersionRingOverlay(Minecraft mc, EntityPlayer player, boolean untexturedQuadOnly,
			float partialTicks) {
		Tessellator tessellator = Tessellator.getInstance();
		...
		if (cap != null) {
			...
			if (viewObstructionDistance < viewFadeStartDistance) {
				float alpha = (float) Math.min(1, Math.max(0, this.prevObstructionPercentage
						+ (this.obstructionPercentage - this.prevObstructionPercentage) * partialTicks));
				...
				if (untexturedQuadOnly) {
					...
					bufferbuilder.pos((double) res.getScaledWidth_double(), (double) res.getScaledHeight_double(), 0.0D)
							.endVertex();
					...
					dispersionWorldFbo = this.ringOfDispersionWorldFramebuffer.getFramebuffer(mainFbo.framebufferWidth,
							mainFbo.framebufferHeight);
					...
					bufferbuilder
							.pos((double) res.getScaledWidth_double(), (double) res.getScaledHeight_double(), -90.0D)
					...
					GlStateManager.translate(0,
							-res.getScaledHeight_double() / 2 + res.getScaledHeight_double() / 2 * alpha, 0);

					bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX);

					// top left
					bufferbuilder.pos(0.0D, res.getScaledHeight_double() / 2, -90.0D).tex(0.0D, 0.5D).endVertex();
					bufferbuilder.pos(centerStart, res.getScaledHeight_double() / 2, -90.0D).tex(0.333D, 0.5D)
							.endVertex();
					...
					bufferbuilder.pos(centerStart, res.getScaledHeight_double() / 2, -90.0D).tex(0.333D, 0.5D)
							.endVertex();
					bufferbuilder.pos(centerEnd, res.getScaledHeight_double() / 2, -90.0D).tex(0.666D, 0.5D)
							.endVertex();
					...
					bufferbuilder.pos(centerEnd, res.getScaledHeight_double() / 2, -90.0D).tex(0.666D, 0.5D)
							.endVertex();
					bufferbuilder.pos(res.getScaledWidth_double(), res.getScaledHeight_double() / 2, -90.0D)
							.tex(1.0D, 0.5D).endVertex();
					...
					// bottom right
					bufferbuilder.pos(centerEnd, res.getScaledHeight_double(), -90.0D).tex(0.666D, 1.0D).endVertex();
					bufferbuilder.pos(res.getScaledWidth_double(), res.getScaledHeight_double(), -90.0D).tex(1.0D, 1.0D)
							.endVertex();
					bufferbuilder.pos(res.getScaledWidth_double(), res.getScaledHeight_double() / 2, -90.0D)
							.tex(1.0D, 0.5D).endVertex();
					bufferbuilder.pos(centerEnd, res.getScaledHeight_double() / 2, -90.0D).tex(0.666D, 0.5D)
		...
		float indicatorAlpha = (float) (this.prevDispersionIndicatorPercentage
				+ (this.dispersionIndicatorPercentage - this.prevDispersionIndicatorPercentage) * partialTicks);
		...
		if (indicatorAlpha > 0.01F) {
			...
			bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX);
			bufferbuilder.pos(0.0D, (double) res.getScaledHeight_double(), -90.0D).tex(0.0D, 1.0D).endVertex();
			bufferbuilder.pos((double) res.getScaledWidth_double(), (double) res.getScaledHeight_double(), -90.0D)
					.tex(1.0D, 1.0D).endVertex();
			bufferbuilder.pos((double) res.getScaledWidth_double(), 0.0D, -90.0D).tex(1.0D, 0.0D).endVertex();
			bufferbuilder.pos(0.0D, 0.0D, -90.0D).tex(0.0D, 0.0D).endVertex();
			tessellator.draw();
	...
	protected void renderPortal(Minecraft mc, float timeInPortal, ScaledResolution scaledRes) {
		...
		TextureAtlasSprite textureatlassprite = mc.getBlockRendererDispatcher().getBlockModelShapes()
				.getTexture(BlockRegistry.TREE_PORTAL.getDefaultState());
	...
	private void renderTexturedRect(BufferBuilder buffer, double x, double y, double x2, double y2, double umin,
	...
	public void onRenderScreen(DrawScreenEvent.Post event) {
		...
		if (GuiScreen.isShiftKeyDown() && mc.currentScreen instanceof GuiContainer && mc.player != null) {
			...
			if (selectedSlot != null && selectedSlot.getHasStack()) {
				...
				double mouseY = resolution.getScaledHeight_double()
						- (Mouse.getY() * resolution.getScaledHeight_double()) / mc.displayHeight - 1;
				GlStateManager.pushMatrix();
				GlStateManager.translate(mouseX + 8, mouseY - 38, 500);
				int yOffset = 0;
				int width = 0;
				List<Aspect> aspects = ItemAspectContainer
						.fromItem(selectedSlot.getStack(), AspectManager.get(mc.world)).getAspects(mc.player);
				GlStateManager.enableTexture2D();
				GlStateManager.enableBlend();
				...
				if (aspects != null && aspects.size() > 0) {
					for (Aspect aspect : aspects) {
						...
						int entryWidth = Math.max(fontRenderer.getStringWidth(aspectText) + 19,
								fontRenderer.getStringWidth(aspectTypeText) + 19);
						if (entryWidth > width) {
							width = entryWidth;
						}
						yOffset -= 21;
