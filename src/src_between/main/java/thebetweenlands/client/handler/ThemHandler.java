	...
	public static void onTick(ClientTickEvent event) {
		if (event.phase == Phase.END && !Minecraft.getMinecraft().isGamePaused()) {
			...
			if (world != null && viewer != null
					&& viewer.dimension == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId
					&& FogHandler.hasDenseFog(world)
					&& (FogHandler.getCurrentFogEnd() + FogHandler.getCurrentFogStart()) / 2 < 65.0F) {
				...
				if (activeParticles.size() < 4) {
					...
					if (viewer.posY >= worldHeight.getY() - 3 && SurfaceType.MIXED_GROUND_AND_UNDERGROUND
							.matches(world.getBlockState(worldHeight.down()))) {
						int probability = (int) (FogHandler.getCurrentFogEnd() + FogHandler.getCurrentFogStart()) / 2
								* 10 + 60;
