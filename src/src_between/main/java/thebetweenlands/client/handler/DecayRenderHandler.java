...
public class DecayRenderHandler {
	public static final ResourceLocation PLAYER_DECAY_TEXTURE = new ResourceLocation(ModInfo.ID,
			"textures/entity/player_decay.png");
		...
		if (player != null) {
			...
			if (cap != null && cap.isDecayEnabled() && cap.getDecayStats().getDecayLevel() > 0) {
				...
				if (isMainHand && !player.isInvisible() && event.getItemStack().isEmpty()) {
					EnumHandSide enumhandside = isMainHand ? player.getPrimaryHand()
							: player.getPrimaryHand().opposite();
					renderArmFirstPersonWithDecay(event.getEquipProgress(), event.getSwingProgress(), enumhandside,
							decay);
	 ...
	 * @param decay
	 */
	private static void renderArmFirstPersonWithDecay(float swingProgress, float equipProgress, EnumHandSide handSide,
			int decay) {
		...
		RenderPlayer renderplayer = (RenderPlayer) renderManager
