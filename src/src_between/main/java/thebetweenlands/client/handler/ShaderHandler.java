...
public class ShaderHandler {
		...
		if (event.phase == Phase.START) {
			...
			if (ShaderHelper.INSTANCE.isShaderSupported() && BetweenlandsConfig.RENDERING.useShader && canUseInWorld) {
				...
				if (ShaderHelper.INSTANCE.isHDRActive()) {
					int colorFormat = ShaderHelper.INSTANCE.isFloatBufferSupported() ? GL30.GL_RGBA16F
							: ShaderHelper.INSTANCE.isARBFloatBufferSupported() ? ARBTextureFloat.GL_RGB16F_ARB : -1;
					if (colorFormat != -1) {
						// Use float buffers for HDR
						GlStateManager.bindTexture(framebuffer.framebufferTexture);
						GlStateManager.glTexImage2D(GL11.GL_TEXTURE_2D, 0, colorFormat,
								framebuffer.framebufferTextureWidth, framebuffer.framebufferTextureHeight, 0,
	...
	private void updateShaders(RenderWorldLastEvent event) {
		if (ShaderHelper.INSTANCE.canUseShaders()) {
			...
			Minecraft.getMinecraft().entityRenderer.renderHand(event.getPartialTicks(),
					MinecraftForgeClient.getRenderPass());
			Minecraft.getMinecraft().entityRenderer.setupCameraTransform(event.getPartialTicks(),
					MinecraftForgeClient.getRenderPass());
