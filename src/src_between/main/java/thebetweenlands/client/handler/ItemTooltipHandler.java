...
public class ItemTooltipHandler {
	...
	public static void onItemTooltip(ItemTooltipEvent event) {
		...
		if (stack.getItem() instanceof IDecayFood) {
			((IDecayFood) stack.getItem()).getDecayFoodTooltip(stack, player != null ? player.world : null, toolTip,
					event.getFlags());
		}

		if (player != null) {
			if (FoodSicknessHandler.isFoodSicknessEnabled(player.getEntityWorld())
					&& stack.getItem() instanceof ItemFood && stack.getItem() instanceof IFoodSicknessItem
					&& ((IFoodSicknessItem) stack.getItem()).canGetSickOf(player, stack)) {
				...
				if (cap != null) {
					...
					((IFoodSicknessItem) stack.getItem()).getSicknessTooltip(stack, sickness, hatred,
							event.getFlags().isAdvanced(), toolTip);
				}
			}

			if (stack.getItem() instanceof IEquippable
					&& ((IEquippable) stack.getItem()).canEquip(stack, player, player)) {
		...
		if (BetweenlandsConfig.GENERAL.itemUsageTooltip) {
			...
			IFluidHandler fluidHandler = stack.getCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY,
					null);
			if (AbstractCenserRecipe.getRecipe(stack) != null || (fluidHandler != null
					&& AbstractCenserRecipe.getRecipe(fluidHandler.drain(Integer.MAX_VALUE, false)) != null)) {
			...
			if (compostRecipe != null) {
				...
				if (event.getFlags().isAdvanced()) {
					debug = " (T: " + COMPOST_AMOUNT_FORMAT.format(compostRecipe.getCompostingTime(stack) / 20.0F)
							+ "s A: " + compostRecipe.getCompostAmount(stack) + ")";
				}
				usedInMachines.add(I18n.format("tooltip.recipes.compost_bin") + debug);
			}

			if (!usedInMachines.isEmpty()) {
				toolTip.add(I18n.format("tooltip.recipes.used_in",
						usedInMachines.stream().collect(Collectors.joining(", "))));
			}
		}
	}
