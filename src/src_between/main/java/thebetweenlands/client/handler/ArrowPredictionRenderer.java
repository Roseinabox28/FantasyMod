...
public class ArrowPredictionRenderer {
	private static class EntityArrowSilent extends EntityArrow {
		...
		public boolean handleWaterMovement() {
			if (this.world.handleMaterialAcceleration(this.getEntityBoundingBox()
					.expand(0.0D, -0.4000000059604645D, 0.0D).contract(0.001D, 0.001D, 0.001D), Material.WATER, this)) {
	...
	public static void render(float quality) {
		...
		if (lastQuality != quality || !randomYawPitchSet) {
			...
			randYaw = (maxOffset / 2.0F - Minecraft.getMinecraft().world.rand.nextFloat() * maxOffset * 2.0F)
					* (1.0F - quality);
			randPitch = (maxOffset / 2.0F - Minecraft.getMinecraft().world.rand.nextFloat() * maxOffset * 2.0F)
					* (1.0F - quality);
		}
		...
		for (int i = 0; i < 1000; i++) {
			...
			if (collisionPoint != null) {
				if (collisionPoint.typeOfHit == RayTraceResult.Type.BLOCK) {
					drawing = false;
					GL11.glVertex3d(ea.posX - rx, ea.posY - ry, ea.posZ - rz);
					GL11.glVertex3d(collisionPoint.hitVec.x - rx, collisionPoint.hitVec.y - ry,
							collisionPoint.hitVec.z - rz);
					GL11.glEnd();
					...
					if (collisionPoint.sideHit == EnumFacing.DOWN) {
						GL11.glVertex3d(collisionPoint.hitVec.x - 0.1 - rx, collisionPoint.hitVec.y - 0.001 - ry,
								collisionPoint.hitVec.z - 0.1 - rz);
						GL11.glVertex3d(collisionPoint.hitVec.x + 0.1 - rx, collisionPoint.hitVec.y - 0.001 - ry,
								collisionPoint.hitVec.z + 0.1 - rz);
						GL11.glVertex3d(collisionPoint.hitVec.x + 0.1 - rx, collisionPoint.hitVec.y - 0.001 - ry,
								collisionPoint.hitVec.z - 0.1 - rz);
						...
						GL11.glVertex3d(collisionPoint.hitVec.x - 0.1 - rx, collisionPoint.hitVec.y - 0.1 - ry,
								collisionPoint.hitVec.z - 0.001 - rz);
						GL11.glVertex3d(collisionPoint.hitVec.x + 0.1 - rx, collisionPoint.hitVec.y + 0.1 - ry,
								collisionPoint.hitVec.z - 0.001 - rz);
						GL11.glVertex3d(collisionPoint.hitVec.x + 0.1 - rx, collisionPoint.hitVec.y - 0.1 - ry,
								collisionPoint.hitVec.z - 0.001 - rz);
						...
						GL11.glVertex3d(collisionPoint.hitVec.x - 0.001 - rx, collisionPoint.hitVec.y - 0.1 - ry,
								collisionPoint.hitVec.z - 0.1 - rz);
						GL11.glVertex3d(collisionPoint.hitVec.x - 0.001 - rx, collisionPoint.hitVec.y + 0.1 - ry,
								collisionPoint.hitVec.z + 0.1 - rz);
						GL11.glVertex3d(collisionPoint.hitVec.x - 0.001 - rx, collisionPoint.hitVec.y - 0.1 - ry,
	...
	private static RayTraceResult getCollision(EntityArrowSilent ea) {
		...
		List entityList = Minecraft.getMinecraft().world.getEntitiesWithinAABBExcludingEntity(ea,
				ea.getEntityBoundingBox().expand(ea.motionX, ea.motionY, ea.motionZ).grow(1.0D, 1.0D, 1.0D));
		...
		for (int c = 0; c < entityList.size(); ++c) {
			...
			if (currentEntity.canBeCollidedWith() && (currentEntity != Minecraft.getMinecraft().player)) {
				AxisAlignedBB entityBoundingBox = currentEntity.getEntityBoundingBox().grow((double) 0.3F,
						(double) 0.3F, (double) 0.3F);
