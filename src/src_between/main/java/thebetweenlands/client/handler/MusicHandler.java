...
public class MusicHandler {
	...
	public void onTick(ClientTickEvent event) {
		if (event.phase == TickEvent.Phase.START) {
			if (this.mc.getSoundHandler() != null && !this.openAlAccess.isErrored()
					&& !this.openAlAccess.isInitialized()) {
				// Needs to be done here because during SoundLoadEvent and SoundSetupEvent the
				// system isn't fully set up yet
				this.openAlAccess.init(this.mc.getSoundHandler().sndManager);
			}

			EntityPlayer player = getPlayer();

			boolean isInMainMenu = (!(mc.currentScreen instanceof GuiWinGame) && mc.player == null)
					&& BetweenlandsConfig.GENERAL.blMainMenu;
			...
			if ((isInBlMainMenu
					|| (player != null && player.dimension == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId))) {
				...
				if (!closestMusicEntityMap.isEmpty()) {
					for (Int2ObjectMap.Entry<IEntityMusic> entry : closestMusicEntityMap.int2ObjectEntrySet()) {
						...
						} else if (currentlyPlaying.getMusicEntity() != entry.getValue()
								&& entry.getValue().canInterruptOtherEntityMusic(player)) {
				...
				if (!this.entityMusicMap.isEmpty()) {
					if (this.mc.getSoundHandler().isSoundPlaying(this.currentSound)) {
						...
						this.timeUntilMusic = Math.min(MathHelper.getInt(this.RNG, MIN_WAIT, MAX_WAIT),
								this.timeUntilMusic);
					}
				} else if (this.mc.gameSettings.getSoundLevel(SoundCategory.MUSIC) > 0.0F) {
					if (this.currentSound != null) {

						if ((!isInBlMainMenu && SoundRegistry.BL_MUSIC_MENU.getSoundName()
								.equals(this.currentSound.getSoundLocation()))
								|| (isInBlMainMenu && SoundRegistry.BL_MUSIC_DIMENSION.getSoundName()
										.equals(this.currentSound.getSoundLocation()))) {
							this.mc.getSoundHandler().stopSound(this.currentSound);
							this.timeUntilMusic = MathHelper.getInt(this.RNG, 0,
									(isInBlMainMenu ? MIN_WAIT_MENU : MIN_WAIT) / 2);
						}
						// Wait for sound track to finish
						if (!this.mc.getSoundHandler().isSoundPlaying(this.currentSound)) {
							this.currentSound = null;
							this.timeUntilMusic = Math
									.min(MathHelper.getInt(this.RNG, (isInBlMainMenu ? MIN_WAIT_MENU : MIN_WAIT),
											(isInBlMainMenu ? MAX_WAIT_MENU : MAX_WAIT)), this.timeUntilMusic);
						}
					}
	...
	public void onPlaySound(PlaySoundEvent event) {
		...
		if ((isInBlMainMenu
				...
				&& !this.isBetweenlandsMusic(event.getSound())) {
	...
	public List<Sound> getBetweenlandsMenuMusicTracks() {
		if (this.musicMenuTrackAccessors == null) {
			try {
				...
				SoundEventAccessor soundEventAccessor = this.mc.getSoundHandler()
						.getAccessor(SoundRegistry.BL_MUSIC_MENU.getSoundName());
	...
	public List<Sound> getBetweenlandsMusicTracks() {
		if (this.musicDimTrackAccessors == null) {
			try {
				...
				SoundEventAccessor soundEventAccessor = this.mc.getSoundHandler()
						.getAccessor(SoundRegistry.BL_MUSIC_DIMENSION.getSoundName());
				if (soundEventAccessor != null) {
					List<ISoundEventAccessor<Sound>> soundAccessors = soundEventAccessor.accessorList;
					for (ISoundEventAccessor<Sound> accessor : soundAccessors) {
						if (accessor instanceof Sound) {
	...
	/**
	 * Plays a random Betweenlands sound track. The previously played sound track
	 * will be excluded.
	 */
	public void playRandomSoundTrack() {
		List<Sound> availableSounds = new ArrayList<>(
				isInBlMainMenu ? this.getBetweenlandsMenuMusicTracks() : this.getBetweenlandsMusicTracks());
		if (!availableSounds.isEmpty()) {
			...
			if (weight != 0) {
				...
				ISound parentSound = PositionedSoundRecord.getMusicRecord(
						isInBlMainMenu ? SoundRegistry.BL_MUSIC_MENU : SoundRegistry.BL_MUSIC_DIMENSION);
	...
	public static class SoundWrapper implements ISound {
		...
		/**
		 * Creates a new sound with the properties of the specified parent sound (-pool)
		 * but only the specified sound is played
		 * 
		 * @param parent
		 * @param sound
		 * @return
		...
		public SoundEventAccessor createAccessor(SoundHandler handler) {
			...
			SoundEventAccessor accessor = new SoundEventAccessor(this.sound.getSoundLocation(),
					subtitle != null ? subtitle.getFormattedText() : null);
			return accessor;
		}
