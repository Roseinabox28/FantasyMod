...
public class GuiCenser extends GuiContainer {
	...
	private static final ResourceLocation CENSER_GUI_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/gui/censer.png");
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (currentRecipe != null) {
			...
			if (!effect.isEmpty()) {
				...
				effectTooltip.add("");
				effectTooltip
						.add(TextFormatting.DARK_PURPLE + I18n.translateToLocalFormatted("tooltip.bl.censer_effect"));
			...
			if (amount > 0) {
				...
				int effectColor = currentRecipe.getEffectColor(this.censer.getCurrentRecipeContext(), this.censer,
						EffectColorType.GUI);
				...
				this.drawTexturedModalRectWithGradient(this.guiLeft + 67,
						this.guiTop + 18 + maxBarHeight - barHeightBackground3, 195,
						1 + maxBarHeight - barHeightBackground3, 47, barHeightBackground3, 0xFF202020, 0xFF202020);
				this.drawTexturedModalRectWithGradient(this.guiLeft + 67,
						this.guiTop + 18 + maxBarHeight - barHeightBackground2, 195,
						1 + maxBarHeight - barHeightBackground2, 47, barHeightBackground2, 0xFF505050, 0xFF505050);
				this.drawTexturedModalRectWithGradient(this.guiLeft + 67,
						this.guiTop + 18 + maxBarHeight - barHeightBackground, 195,
						1 + maxBarHeight - barHeightBackground, 47, barHeightBackground, 0xFF808080, 0xFF808080);
				this.drawTexturedModalRectWithGradient(this.guiLeft + 67, this.guiTop + 18 + maxBarHeight - barHeight,
						195, 1 + maxBarHeight - barHeight, 47, barHeight, effectColor, 0xFFFFFFFF);
		...
		if (fuelTicks > 0) {
			...
			this.drawTexturedModalRect(this.guiLeft + 81, this.guiTop + 122 + maxBarHeight - barHeight, 177,
					20 + maxBarHeight - barHeight, 14, barHeight);
		...
		if (!internalStack.isEmpty()) {
			...
			this.itemRender.renderItemAndEffectIntoGUI(this.mc.player, internalStack, this.guiLeft + internalSlot.xPos,
					this.guiTop + internalSlot.yPos);
			this.itemRender.renderItemOverlayIntoGUI(this.fontRenderer, internalStack, this.guiLeft + internalSlot.xPos,
					this.guiTop + internalSlot.yPos, null);
		...
		if (fluidStack != null) {
			...
			if (fluidTexture != null) {
				...
				this.drawTexturedModalRect(this.guiLeft + internalSlot.xPos - 3, this.guiTop + internalSlot.yPos - 3,
						sprite, 20, 20);
			} else {
				...
				drawRect(this.guiLeft + internalSlot.xPos - 3, this.guiTop + internalSlot.yPos - 3,
						this.guiLeft + internalSlot.xPos - 3 + 20, this.guiTop + internalSlot.yPos - 3 + 20, color);
			...
			// Render cover
			this.mc.getTextureManager().bindTexture(CENSER_GUI_TEXTURE);
			this.drawTexturedModalRect(this.guiLeft + internalSlot.xPos - 3, this.guiTop + internalSlot.yPos - 3, 176,
					100, 20, 20);
		...
		if ((fluidStack == null || fluidStack.amount == 0) && internalStack.isEmpty()) {
			...
			this.mc.getTextureManager().bindTexture(CENSER_GUI_TEXTURE);
			this.drawTexturedModalRect(this.guiLeft + internalSlot.xPos - 1, this.guiTop + internalSlot.yPos - 1, 176,
					1, 18, 18);
	...
	protected void drawTexturedModalRectWithGradient(int x, int y, int textureX, int textureY, int width, int height,
