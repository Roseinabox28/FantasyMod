	...
	protected void drawGuiContainerBackgroundLayer(float partialTickTime, int x, int y) {
		...
		if (tile.isCrystalInslot()) {
			...
			if (tile.isValidFocalItem()) {
				...
				GlStateManager.color(1.0F, 0.1F, 0.1F,
						0.35F + (float) (Math.cos((this.updateTicks + partialTickTime) / 10.0F) + 1.0F) / 2.0F * 0.65F);
				this.drawTexturedModalRect(k + 39, l + 8 + lifeCrystalCount, 175, 2 + lifeCrystalCount, 6,
						requiredLifeCrystal);
		...
		if (tile.isSlotInUse(0) && tile.isCrystalInslot() && tile.isSulfurInSlot()
				&& tile.fuelConsumed < tile.requiredFuelCount && tile.lifeCrystalLife >= tile.requiredLifeCount
				&& tile.isValidFocalItem()) {
			// Fuel bar
			int fuelBurnProgress = tile.fuelBurnProgress;
			drawTexturedModalRect(k + 129, l + 8 + fuelBurnProgress, 175, 2 + fuelBurnProgress, 6, 42);

			double relTotalProgress = (tile.fuelConsumed + (tile.fuelBurnProgress / 42.0D))
					/ (double) tile.requiredFuelCount;
