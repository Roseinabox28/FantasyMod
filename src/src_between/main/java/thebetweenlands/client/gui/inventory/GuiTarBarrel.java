...
public class GuiTarBarrel extends GuiContainer {
	...
	private static final ResourceLocation TAR_BARREL_GUI_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/gui/tar_barrel.png");
	...
	protected void drawGuiContainerBackgroundLayer(float partialTickTime, int x, int y) {
		...
		if (fluidStack != null) {
			...
			this.fontRenderer.drawString(
					this.numberFormat.format(fluidStack != null ? fluidStack.amount / 1000.0f : 0.0f), xx + 127,
					yy + 26, 0xFFFFFFFF);
	...
	protected void drawTexturedBar(int xCoord, int yCoord, TextureAtlasSprite textureSprite, int widthIn, int heightIn,
			int fullHeight) {
		if (heightIn <= 0) {
			return;
		}
		...
		while (yStart < fullHeight) {
			...
			bufferbuilder.pos((double) (xCoord + widthIn), (double) (yOff + yCoord + 0), (double) this.zLevel)
					.tex(maxU, maxV).endVertex();
			bufferbuilder.pos((double) (xCoord + 0), (double) (yOff + yCoord + 0), (double) this.zLevel).tex(minU, maxV)
					.endVertex();
			bufferbuilder
					.pos((double) (xCoord + 0), (double) (yOff + yCoord + heightIn * fraction), (double) this.zLevel)
					.tex(minU, minV).endVertex();
			bufferbuilder.pos((double) (xCoord + widthIn), (double) (yOff + yCoord + heightIn * fraction),
					(double) this.zLevel).tex(maxU, minV).endVertex();
