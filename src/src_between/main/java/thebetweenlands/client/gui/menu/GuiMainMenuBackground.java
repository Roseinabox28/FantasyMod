...
public class GuiMainMenuBackground extends GuiScreen {
	...
	public void updateScreen() {
		...
		for (int i = 0; i < 3; i++) {
			...
			while (it.hasNext()) {
				...
				if (firefly.getPosY() >= this.width + 40 || firefly.getPosY() <= -40 || firefly.getPosX() <= -40) {
					it.remove();
				} else {
					firefly.update();
				}
			...
			if (this.random.nextInt(32) == 0 && layer.size() < 5) {
				layer.add(new GuiFirefly(this.width + 50, this.random.nextInt(this.height),
						-this.random.nextFloat() * 0.8F, (this.random.nextFloat() - this.random.nextFloat()) * 1.5F));
			}
		}
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		for (int i = 0; i < this.layerTextures.length; i++) {
			...
			double u = (((this.layerTick + partialTicks) / (float) (this.layerTextures.length - i)) / (float) (i + 1)
					+ 1024 * i / 4.0F) / 4000.0F;
	...
	protected void drawStarfield(float partialTicks) {
		if (ShaderHelper.INSTANCE.canUseShaders() && this.starfieldEffect != null && this.starfieldTextureFBO != null) {
			...
			this.starfieldEffect.create(this.starfieldTextureFBO)
					...
					.setRenderDimensions(renderDimension, renderDimension).render(partialTicks);
