...
public class GuiBLMainMenu extends GuiMainMenu {
	...
	private GuiMainMenuBackground background = new GuiMainMenuBackground(
			new ResourceLocation(ModInfo.ID, "textures/gui/main/layer"), 4);
	...
	public void initGui() {
		...
		this.buttonList.add(this.realmsButton = new GuiButtonMainMenu(14, this.width / 2 + 2, j + 24 * 2, 98, 20,
				I18n.format("menu.online", new Object[0]).replace("Minecraft", "").trim()));

		this.buttonList.add(this.modButton = new GuiButtonMainMenu(6, this.width / 2 - 100, j + 24 * 2, 98, 20,
				I18n.format("fml.menu.mods")));
		this.buttonList.add(new GuiButtonMainMenu(5, this.width / 2 - 124, j + 72 + 12, 20, 20, "L"));
		this.buttonList
				.add(new GuiButtonMainMenu(0, this.width / 2 - 100, j + 72 + 12, 98, 20, I18n.format("menu.options")));
		this.buttonList
				.add(new GuiButtonMainMenu(4, this.width / 2 + 2, j + 72 + 12, 98, 20, I18n.format("menu.quit")));
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.drawTexturedModalRect(this.width / 2 - 161 / 2,
				20 + MathHelper.sin(((float) this.ticks + partialTicks) / 16.0F) * 6.0F, 0, 0, 161, 79, 256, 256);
	...
	protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
		...
		synchronized (this.threadLock) {
			...
			if (!this.openGLWarning1.isEmpty() && !StringUtils.isNullOrEmpty(openGLWarningLink)
