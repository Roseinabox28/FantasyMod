		...
		if (OpenGlHelper.isFramebufferEnabled()) {
			...
			if (rift.getActivationTicks() > 0 && rift.getVisibility(partialTicks) > 0) {
				...
				try (FramebufferStack.State state = FramebufferStack.push()) {
					skyFbo = overworldSkyFbo.getFramebuffer(state.getMinecraftFbo().framebufferWidth,
							state.getMinecraftFbo().framebufferHeight);
					...
					if (OpenGlHelper.openGL14) {
						GlStateManager.tryBlendFuncSeparate(SourceFactor.ZERO, DestFactor.ONE, SourceFactor.ZERO,
								DestFactor.ONE_MINUS_SRC_ALPHA);
					} else {
						GlStateManager.blendFunc(SourceFactor.ZERO, DestFactor.ONE_MINUS_SRC_ALPHA); // Still decent
																										// looking
																										// fallback
					}
