...
public class BLSkyRenderer extends IRenderHandler implements IBetweenlandsSky {
	public static final ResourceLocation SKY_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/sky/sky_texture.png");
	public static final ResourceLocation SKY_SPOOPY_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/sky/spoopy.png");
	public static final ResourceLocation FOG_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/sky/fog_texture.png");
	...
	public BLSkyRenderer() {
		if (clipPlaneBuffer == null) {
			clipPlaneBuffer = new GeometryBuffer(Minecraft.getMinecraft().getTextureManager(),
					WorldShader.CLIP_PLANE_DIFFUSE_TEXTURE, WorldShader.CLIP_PLANE_DEPTH_TEXTURE, true);
		}
	...
	protected Mesh createStarMesh() {
		...
		for (int i = 0; i < 1500; ++i) {
			...
			if (centerDistance < 1.0D && centerDistance > 0.01D) {
				...
				Vertex v1 = this.getQuadPoint(0, randSize, randRotX, randRotY, distYRotX, distYRotZ, xzRotX, xzRotY,
						farX, farY, farZ, color);
				Vertex v2 = this.getQuadPoint(1, randSize, randRotX, randRotY, distYRotX, distYRotZ, xzRotX, xzRotY,
						farX, farY, farZ, color);
				Vertex v3 = this.getQuadPoint(2, randSize, randRotX, randRotY, distYRotX, distYRotZ, xzRotX, xzRotY,
						farX, farY, farZ, color);
				Vertex v4 = this.getQuadPoint(3, randSize, randRotX, randRotY, distYRotX, distYRotZ, xzRotX, xzRotY,
						farX, farY, farZ, color);
	...
	protected Vertex getQuadPoint(int vertex, double randSize, double randRotX, double randRotY, double distYRotX,
			double distYRotZ, double xzRotX, double xzRotY, double farX, double farY, double farZ, int color) {
	...
	protected void renderSky(float partialTicks, WorldClient world, Minecraft mc) {
		...
		boolean useShaderSky = ShaderHelper.INSTANCE.isWorldShaderActive()
				&& ShaderHelper.INSTANCE.getWorldShader() != null
				&& ShaderHelper.INSTANCE.getWorldShader().getStarfieldTexture() >= 0;

		float starBrightness = (world.getStarBrightness(partialTicks) + 0.5F) * invRainStrength * invRainStrength
				* invRainStrength;
		float fade = 1.0F;
		...
		if (world.provider.isSkyColored()) {
			GlStateManager.color(skyR * 0.2F + 0.04F, skyG * 0.2F + 0.04F, skyB * 0.6F + 0.1F,
					starBrightness / (!useShaderSky ? 1.5F : 1.0F));
		} else {
	...
	protected void renderFog(float partialTicks, WorldClient world, Minecraft mc) {
		...
		if (Minecraft.getMinecraft().gameSettings.fancyGraphics) {
			...
			GlStateManager.scale(1.0F / 50.0F * renderRadius, 1.0F / 50.0F * renderRadius, 1.0F / 50.0F * renderRadius);
	...
	protected void renderSkyDome() {
		...
		// Renders tiles and then normalizes their vertices to create a texture mapped
		// dome
	...
	protected void resetRenderingStates() {
		// Value used while rendering the world, but is only set once before rendering
		// the sky
	...
	protected void renderAuroras(float partialTicks, WorldClient world, Minecraft mc) {
		if (!this.auroras.isEmpty()) {
			...
			GlStateManager.enableBlend();
			GlStateManager.pushMatrix();
			GlStateManager.translate(-mc.getRenderManager().viewerPosX, -mc.getRenderManager().viewerPosY,
		...
		if (storage != null) {
			...
			if (reg.auroras.isActive()) {
				...
				if (minDist > 150 || this.auroras.isEmpty()) {
					...
					this.auroras.add(new AuroraRenderer(newAuroraPosX, newAuroraPosY + rand.nextInt(100), newAuroraPosZ,
							new Vector2d(rand.nextFloat() * 2.0F - 1.0F, rand.nextFloat() * 2.0F - 1.0F),
							rand.nextInt(40) + 15, gradients));
				}
			} else {
				for (AuroraRenderer aurora : this.auroras) {
					aurora.setActive(false);
