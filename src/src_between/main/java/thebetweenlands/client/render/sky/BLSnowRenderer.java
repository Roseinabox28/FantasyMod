...
public class BLSnowRenderer extends IRenderHandler {
	...
	public void render(float partialTicks, WorldClient world, Minecraft mc) {
		...
		float snowingStrength = this.prevSnowingStrength
				+ (this.snowingStrength - this.prevSnowingStrength) * partialTicks;
		...
		if (snowingStrength > 0.0F) {
			...
			float interpTicks = this.prevRenderUpdateTicks
					+ (this.renderUpdateTicks - this.prevRenderUpdateTicks) * partialTicks;
			...
			for (int layerZ = pz - layers; layerZ <= pz + layers; ++layerZ) {
				for (int layerX = px - layers; layerX <= px + layers; ++layerX) {
					...
					if (layerY != maxLayerY) {
						this.random.setSeed((long) (layerX * layerX * 3121 + layerX * 45238971
								^ layerZ * layerZ * 418711 + layerZ * 13761));

						double uvShiftY = (double) (-(interpTicks % 512) / 512.0F);
						double randUvShiftX = this.random.nextDouble()
								+ (double) interpTicks * 0.01D * (double) ((float) this.random.nextGaussian());
						double randUvShiftY = this.random.nextDouble()
								+ (double) (interpTicks * (float) this.random.nextGaussian()) * 0.001D;
						...
						float visibility = MathHelper
								.clamp(((1.0F - distance * distance) * 0.5F + 0.5F) * snowingStrength / 5.0F, 0, 1);
						...
						builder.pos((double) layerX - rainXCoord + 0.5D, (double) maxLayerY,
								(double) layerZ - rainYCoord + 0.5D)
								.tex(0.0D + randUvShiftX, (double) layerY * 0.25D + uvShiftY + randUvShiftY)
								.color(1.0F, 1.0F, 1.0F, visibility).lightmap(lightmapX, lightmapY).endVertex();
						builder.pos((double) layerX + rainXCoord + 0.5D, (double) maxLayerY,
								(double) layerZ + rainYCoord + 0.5D)
								.tex(1.0D + randUvShiftX, (double) layerY * 0.25D + uvShiftY + randUvShiftY)
								.color(1.0F, 1.0F, 1.0F, visibility).lightmap(lightmapX, lightmapY).endVertex();
						builder.pos((double) layerX + rainXCoord + 0.5D, (double) layerY,
								(double) layerZ + rainYCoord + 0.5D)
								.tex(1.0D + randUvShiftX, (double) maxLayerY * 0.25D + uvShiftY + randUvShiftY)
								.color(1.0F, 1.0F, 1.0F, visibility).lightmap(lightmapX, lightmapY).endVertex();
						builder.pos((double) layerX - rainXCoord + 0.5D, (double) layerY,
								(double) layerZ - rainYCoord + 0.5D)
								.tex(0.0D + randUvShiftX, (double) maxLayerY * 0.25D + uvShiftY + randUvShiftY)
								.color(1.0F, 1.0F, 1.0F, visibility).lightmap(lightmapX, lightmapY).endVertex();
					}
				}
			}
