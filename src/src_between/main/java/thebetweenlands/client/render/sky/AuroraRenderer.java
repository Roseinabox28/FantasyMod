...
public class AuroraRenderer {
	private static final ResourceLocation AURORA_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/sky/aurora.png");
	...
	private float getNoiseGradient(int noisePosRounded) {
		return getNoise(noisePosRounded) / 2.0F + getNoise(noisePosRounded - 1) / 4.0F
				+ getNoise(noisePosRounded + 1) / 4.0F;
	}

	private float getNoise(int noisePosRounded) {
		noisePosRounded = (noisePosRounded << 13) ^ noisePosRounded;
		return (float) (1.0
				- ((noisePosRounded * (noisePosRounded * noisePosRounded * 15731 + 789221) + 1376312589) & 0x7fffffff)
						/ 1073741824.0);
	}
	...
	public void render(float partialTicks, float alphaMultiplier) {
		...
		for (int i = 0; i < segments; i++) {
			...
			for (int si = 0; si < subSegments; si++) {
				...
				float dirXNoise = this
						.interpolatedNoise(randNoiseOffset * 10 + 0.01F
								* ((i + (si) / (float) subSegments + (float) (System.nanoTime() / 7000000000.0D))))
						* 0.1F - 0.05F;
				float dirYNoise = this
						.interpolatedNoise(randNoiseOffset * 20 + 0.01F
								* ((i + (si) / (float) subSegments + (float) (System.nanoTime() / 7000000000.0D)) * 5))
						* 0.1F - 0.05F;
				...
				float offset1 = this.interpolatedNoise(randNoiseOffset + (i + (si) / (float) subSegments) * 2
						+ (float) (System.nanoTime() / 4000000000.0D)) * 10;
				float offset2 = this.interpolatedNoise(randNoiseOffset + (i + (si + 1) / (float) subSegments) * 2
						+ (float) (System.nanoTime() / 4000000000.0D)) * 10;

				double segStartX = this.x + prevDirection.x * (i + (si) / (float) subSegments) * segmentWidth
						+ this.getRotatedVec(offset1, prevDirection).x;
				double segStartZ = this.z + prevDirection.y * (i + (si) / (float) subSegments) * segmentWidth
						+ this.getRotatedVec(offset1, prevDirection).y;

				double segStopX = this.x + this.currDirection.x * (i + (si + 1) / (float) subSegments) * segmentWidth
						+ this.getRotatedVec(offset2, this.currDirection).x;
				double segStopZ = this.z + this.currDirection.y * (i + (si + 1) / (float) subSegments) * segmentWidth
						+ this.getRotatedVec(offset2, this.currDirection).y;
				...
				for (int gi = 0; gi < cGradients - 1; gi++) {
					...
					float[][] interpolatedUVs = ATLAS.getInterpolatedUVs(textureSegment, (float) relUMin, (float) vmin,
							(float) relUMax, (float) vmax);
					...
					double camDist = renderView != null ? renderView.getDistance(segStopX, renderView.posY, segStopZ)
							: 0.0D;
					double camDistNext = renderView != null
							? renderView.getDistance(segStartX, renderView.posY, segStartZ)
							: 0.0D;
					...
					// Front face
					vertexBuffer.pos(segStartX - this.x, segStopY, segStartZ - this.z).tex(umax, vmax)
							.lightmap(238, 238).color(topGradient.x, topGradient.y, topGradient.z,
									topGradient.w * alphaGradMultiplier * alphaMultiplier)
							.endVertex();
					vertexBuffer.pos(segStopX - this.x, segStopY, segStopZ - this.z).tex(umin, vmax).lightmap(238, 238)
							.color(topGradient.x, topGradient.y, topGradient.z,
									topGradient.w * alphaGradMultiplierNext * alphaMultiplier)
							.endVertex();
					vertexBuffer.pos(segStopX - this.x, segStartY, segStopZ - this.z).tex(umin, vmin).lightmap(238, 238)
							.color(bottomGradient.x, bottomGradient.y, bottomGradient.z,
									bottomGradient.w * alphaGradMultiplierNext * alphaMultiplier)
							.endVertex();
					vertexBuffer.pos(segStartX - this.x, segStartY, segStartZ - this.z).tex(umax, vmin)
							.lightmap(238, 238).color(bottomGradient.x, bottomGradient.y, bottomGradient.z,
									bottomGradient.w * alphaGradMultiplier * alphaMultiplier)
							.endVertex();
