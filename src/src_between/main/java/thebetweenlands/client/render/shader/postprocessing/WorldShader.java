...
/**
 * TODO: Make lighting and other spacial effects use "correct" deferred
 * rendering
 */
public class WorldShader extends PostProcessingEffect<WorldShader> {
	...
	public static final ResourceLocation REPELLER_DIFFUSE_TEXTURE = new ResourceLocation(ModInfo.ID,
			"repeller_diffuse");
	public static final ResourceLocation REPELLER_DEPTH_TEXTURE = new ResourceLocation(ModInfo.ID, "repeller_depth");
	public static final ResourceLocation GAS_PARTICLES_DIFFUSE_TEXTURE = new ResourceLocation(ModInfo.ID,
			"gas_particles_diffuse");
	public static final ResourceLocation GAS_PARTICLES_DEPTH_TEXTURE = new ResourceLocation(ModInfo.ID,
			"gas_particles_depth");
	public static final ResourceLocation CLIP_PLANE_DIFFUSE_TEXTURE = new ResourceLocation(ModInfo.ID,
			"clip_plane_diffuse");
	public static final ResourceLocation CLIP_PLANE_DEPTH_TEXTURE = new ResourceLocation(ModInfo.ID,
			"clip_plane_depth");
	...
	protected ResourceLocation[] getShaders() {
		return new ResourceLocation[] { new ResourceLocation("thebetweenlands:shaders/postprocessing/world/world.vsh"),
				new ResourceLocation("thebetweenlands:shaders/postprocessing/world/world.fsh") };
	}
	...
	protected boolean initEffect() {
		...
		this.repellerShieldBuffer = new GeometryBuffer(textureManager, REPELLER_DIFFUSE_TEXTURE, REPELLER_DEPTH_TEXTURE,
				true);
		this.gasParticlesBuffer = new GeometryBuffer(textureManager, GAS_PARTICLES_DIFFUSE_TEXTURE,
				GAS_PARTICLES_DEPTH_TEXTURE, true);

		// Initialize gas textures and effect
		this.gasTextureFramebuffer = new Framebuffer(64, 64, false);
		Minecraft.getMinecraft().getTextureManager().loadTexture(GAS_PARTICLE_TEXTURE,
				new GLTextureObjectWrapper(this.gasTextureFramebuffer.framebufferTexture));
		...
		this.starfieldTextureFramebuffer = new Framebuffer(BetweenlandsConfig.RENDERING.skyResolution,
				BetweenlandsConfig.RENDERING.skyResolution, false);
	...
	protected void uploadUniforms(float partialTicks) {
		...
		final int renderedLightSources = Math.min(MAX_LIGHT_SOURCES_PER_PASS,
				this.lightSources.size() - this.currentRenderPass * MAX_LIGHT_SOURCES_PER_PASS);
		...
		for (int i = 0; i < renderedLightSources; i++) {
			...
			this.uploadFloat(this.lightSourcePositionUniformIDs[i], (float) (lightSource.x - renderPosX),
					(float) (lightSource.y - renderPosY), (float) (lightSource.z - renderPosZ));
		...
		Vec3d camPos = renderView != null
				? ActiveRenderInfo.projectViewFromEntity(Minecraft.getMinecraft().getRenderViewEntity(), partialTicks)
				: Vec3d.ZERO;
		this.uploadFloat(this.viewPosUniformID, (float) (camPos.x - renderPosX), (float) (camPos.y - renderPosY),
				(float) (camPos.z - renderPosZ));
	...
	/**
	 * Updates following matrices: MV (Modelview), PM (Projection), MVP (Modelview x
	 * Projection), INVMVP (Inverted MVP)
	 */
	...
	private void applyGroundFog(float partialTicks) {
		if (!this.groundFogVolumes.isEmpty()) {
			...
			Framebuffer blitFramebuffer = this.blitBuffer.getFramebuffer(mainFramebuffer.framebufferWidth,
					mainFramebuffer.framebufferHeight);

			this.groundFogEffect.setDepthBufferTexture(this.getDepthBuffer().getGlTextureId());
			this.groundFogEffect.create(mainFramebuffer).setSource(mainFramebuffer.framebufferTexture)
					.setBlitFramebuffer(blitFramebuffer).setPreviousFramebuffer(mainFramebuffer).setMirrorY(true)
					.render(partialTicks);
		}
	}
	...
	private void applyBloodSky(float partialTicks) {
		...
		World world = Minecraft.getMinecraft().world;
		if (world != null) {
			BLEnvironmentEventRegistry eeRegistry = BetweenlandsWorldStorage.forWorld(world)
					.getEnvironmentEventRegistry();
		...
		Projection projection = GLUProjection.getInstance().project(lightPos.x, lightPos.y, lightPos.z,
				ClampMode.ORTHOGONAL, false);
		Projection projectionUnclamped = GLUProjection.getInstance().project(lightPos.x, lightPos.y, lightPos.z,
				ClampMode.NONE, false);
		...
		if (depthTexture < 0 || clipPlaneBuffer < 0)
			return; // FBOs not yet ready

		Framebuffer mainFramebuffer = Minecraft.getMinecraft().getFramebuffer();
		Framebuffer blitFramebuffer = this.blitBuffer.getFramebuffer(mainFramebuffer.framebufferWidth,
				mainFramebuffer.framebufferHeight);
		Framebuffer occlusionFramebuffer = this.occlusionBuffer.getFramebuffer(mainFramebuffer.framebufferWidth,
				mainFramebuffer.framebufferHeight);
		...
		this.occlusionExtractor.create(occlusionFramebuffer)
				...
				// .setRenderDimensions(Minecraft.getMinecraft().displayWidth,
		...
		float beat = 0.0F;
		if (hasBeat) {
			beat = Math.abs(((float) Math.sin(System.nanoTime() / 100000000.0D) / 3.0F)
					/ (Math.abs((float) Math.sin(System.nanoTime() / 4000000000.0D)
							* (float) Math.sin(System.nanoTime() / 4000000000.0D)
							* (float) Math.sin(System.nanoTime() / 4000000000.0D + 0.05F) * 120.0F) * 180.0F + 15.5F)
					* 30.0F) / 4.0F;
		}
		...
		this.godRayEffect.setOcclusionMap(occlusionFramebuffer)
				...
				.setRayPos(rayX, rayY).create(blitFramebuffer).setSource(mainFramebuffer.framebufferTexture)
				.setPreviousFramebuffer(mainFramebuffer).render(partialTicks);
	...
	private void applySwirl(float partialTicks) {
		...
		if (interpolatedSwirlAngle != 0.0F) {
			...
			this.swirlEffect.create(mainFramebuffer).setSource(mainFramebuffer.framebufferTexture)
					.setBlitFramebuffer(blitFramebuffer).setPreviousFramebuffer(mainFramebuffer).render(partialTicks);
		}
	}

	private void updateGasParticlesTexture(World world, float partialTicks) {
		boolean hasCloud = DefaultParticleBatches.GAS_CLOUDS_TEXTURED.getParticles().size() > 0
				|| DefaultParticleBatches.GAS_CLOUDS_HEAT_HAZE.getParticles().size() > 0;
		if (!hasCloud) {
		...
		if (hasCloud) {
			...
			this.gasWarpEffect.create(this.gasTextureFramebuffer)
					...
					.setPreviousFramebuffer(Minecraft.getMinecraft().getFramebuffer()).render(partialTicks);
		}
	}
	...
	private void updateStarfieldTexture(float partialTicks) {
		...
		this.starfieldEffect.create(this.starfieldTextureFramebuffer)
				...
				.setRenderDimensions(BetweenlandsConfig.RENDERING.skyResolution,
						BetweenlandsConfig.RENDERING.skyResolution)
				.render(partialTicks);
		GlStateManager.alphaFunc(GL11.GL_GREATER, 0.1F);
	}
}
