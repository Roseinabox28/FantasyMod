...
public abstract class PostProcessingEffect<T extends PostProcessingEffect<?>> {
		...
		if (!this.initialized) {
			...
			if (!this.initEffect() || this.shaderProgramID < 0) {
				throw new RuntimeException(
						"Couldn't initialize shaders for post processing effect: " + this.toString());
			}
	...
	public static final class EffectBuilder<T extends PostProcessingEffect<?>> {
		 ...
		 * <p>
		 * <b>Note:</b> If the source is the same as the destination a blit buffer is
		 * required
		 * 
		 * @param src
		 * @return
		...
		/**
		 * Sets the render dimensions for this effect. Uses the destination FBO
		...
		/**
		 * Sets whether the GL states should be restored after applying the effect. True
		 * by default
		 * 
		 * @param restore
		 * @return
		...
		public void render(float partialTicks) {
			...
			this.effect.render(partialTicks, this.src, this.dst, this.blitFrfamebuffer, this.prevFramebuffer,
					renderWidth, renderHeight, this.restore, this.mirrorX, this.mirrorY, this.clearDepth,
					this.clearColor);
		}
	}

	private final void render(float partialTicks, int src, Framebuffer dst, Framebuffer blitBuffer, Framebuffer prev,
			double renderWidth, double renderHeight, boolean restore, boolean mirrorX, boolean mirrorY,
			boolean clearDepth, boolean clearColor) {
		...
		GlStateManager.ortho(0.0D, intermediateDst.framebufferWidth, intermediateDst.framebufferHeight, 0.0D, 1000.0D,
				3000.0D);
		GlStateManager.matrixMode(GL11.GL_MODELVIEW);
		GlStateManager.loadIdentity();
		...
		if (blitBuffer != null && this.stages != null && this.stages.length > 0) {
			for (PostProcessingEffect<?> stage : this.stages) {
				...
				stage.render(partialTicks, intermediateDst.framebufferTexture, blitBuffer, intermediateDst, null,
						renderWidth, renderHeight, false, false, false, clearDepth, clearColor);
		...
		if (this.shaderProgramID == -1) {
			...
			try {
				...
				for (int i = 0; i < 2; i++) {
					...
					IOUtils.copy(Minecraft.getMinecraft().getResourceManager().getResource(shaderLocations[i])
			...
			if (this.shaderProgramID != -1 && vertexShaderID != -1 && fragmentShaderID != -1) {
				...
				// Check for errors
				if (OpenGlHelper.glGetProgrami(this.shaderProgramID,
	...
	private static int createShader(String shaderCode, int shaderType) throws Exception {
		...
		try {
			...
			if (OpenGlHelper.glGetShaderi(shader, ARBShaderObjects.GL_OBJECT_COMPILE_STATUS_ARB) == GL11.GL_FALSE) {
				String shaderTypeName = shaderType == ARBVertexShader.GL_VERTEX_SHADER_ARB ? "vertex"
						: shaderType == ARBFragmentShader.GL_FRAGMENT_SHADER_ARB ? "fragment" : "";
	 ...
	 * <p>
	 * <b>Note:</b> If additional stages are used a blit buffer is required
	 * 
	 * @return
	 */
	...
	/**
	 * Used to initialize the effect with additional things such as getting uniform
	 * locations. Return false if something during initializiation goes wrong.
	 */
	...
	/**
	 * Uploads a sampler. Texture unit 0 is reserved for the default diffuse sampler
	 * 
	 * @param uniform
	 * @param texture
	 * @param textureUnit
