...
public class ShaderHelper implements IResourceManagerReloadListener {
	...
	public boolean canUseShaders() {
		if (this.isShaderSupported()) {
			...
			if (BetweenlandsConfig.RENDERING.dimensionShaderOnly) {
				canUseInWorld = Minecraft.getMinecraft().world != null && Minecraft.getMinecraft().world.provider
						.getDimension() == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId;
			}
			return this.shaderError == null && OpenGlHelper.isFramebufferEnabled()
					&& BetweenlandsConfig.RENDERING.useShader && canUseInWorld;
		} else {
	...
	private void checkCapabilities() {
		if (!this.checked) {
			...
			boolean supported = contextCapabilities.OpenGL21 || (contextCapabilities.GL_ARB_vertex_shader
			...
			int maxTextureUnits = arbMultitexture ? GL11.glGetInteger(ARBMultitexture.GL_MAX_TEXTURE_UNITS_ARB)
	...
	public void renderShaders(float partialTicks) {
		if (this.shadersUpdated && this.worldShader != null && this.isRequired() && this.canUseShaders()) {
			...
			try (FramebufferStack.State state = FramebufferStack.push()) {
				blitFramebuffer = this.blitBuffer.getFramebuffer(mainFramebuffer.framebufferWidth,
						mainFramebuffer.framebufferHeight);
				;
				targetFramebuffer1 = mainFramebuffer;
				targetFramebuffer2 = blitFramebuffer;

				int renderPasses = MathHelper
						.floor(this.worldShader.getLightSourcesAmount() / WorldShader.MAX_LIGHT_SOURCES_PER_PASS) + 1;
				...
				for (int i = 0; i < renderPasses; i++) {
					...
					this.worldShader.create(targetFramebuffer2).setSource(targetFramebuffer1.framebufferTexture)
							.setRestoreGlState(true).setMirrorY(false).setClearDepth(true).setClearColor(false)
							.render(partialTicks);
			...
			if (targetFramebuffer1 != mainFramebuffer) {
				...
				vb.pos((double) targetFramebuffer1.framebufferTextureWidth,
						(double) targetFramebuffer1.framebufferTextureHeight, 500.0D).tex(1, 0).endVertex();
			...
			if (this.toneMappingShader != null) {
				...
				this.toneMappingShader.create(mainFramebuffer).setSource(mainFramebuffer.framebufferTexture)
						.setBlitFramebuffer(blitFramebuffer).setRestoreGlState(true).setMirrorY(false)
						.setClearDepth(false).setClearColor(false).render(partialTicks);
			}
	...
	/**
	 * Enables the shaders to be used in the next/current render tick. The shaders
	 * are always rendered in the BL dimension, but if something in another
	...
	private boolean isRequired() {
		...
		return mc.world != null
				&& mc.world.provider.getDimension() == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId;
	}
