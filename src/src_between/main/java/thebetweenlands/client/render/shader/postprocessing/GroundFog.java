...
public class GroundFog extends PostProcessingEffect<GroundFog> {
	public static final ResourceLocation TEXTURE_GROUND_FOG_HEIGHT_MAP = new ResourceLocation(ModInfo.ID,
			"textures/shaders/ground_fog_height_map.png");
	...
	public static class GroundFogVolume {
		...
		public GroundFogVolume(Vec3d position, Vec3d size, float inScattering, float extinction, int color) {
			this(position, size, inScattering, extinction, (float) (color >> 16 & 0xff) / 255F,
					(float) (color >> 8 & 0xff) / 255F, (float) (color & 0xff) / 255F);
		}

		public GroundFogVolume(Vec3d position, Vec3d size, float inScattering, float extinction, float r, float g,
				float b) {
	...
	protected ResourceLocation[] getShaders() {
		return new ResourceLocation[] {
				new ResourceLocation("thebetweenlands:shaders/postprocessing/ground_fog/ground_fog.vsh"),
				new ResourceLocation("thebetweenlands:shaders/postprocessing/ground_fog/ground_fog.fsh") };
	}
	...
	protected void uploadUniforms(float partialTicks) {
		...
		for (int i = 0; i < renderedVolumes; i++) {
			...
			this.uploadFloat(this.fogVolumePositionUniformIDs[i], (float) (volume.position.x - renderPosX),
					(float) (volume.position.y - renderPosY), (float) (volume.position.z - renderPosZ));
			this.uploadFloat(this.fogVolumeColorUniformIDs[i], volume.r, volume.g, volume.b);
			this.uploadFloat(this.fogVolumeSizeUniformIDs[i], (float) volume.size.x, (float) volume.size.y,
					(float) volume.size.z);
