...
public final class VanillaParticleFactory<T extends Particle> extends ParticleFactory<VanillaParticleFactory<T>, T> {
	...
	protected T createParticle(ImmutableParticleArgs args) {
		...
		if (data.length > 1) {
			...
			return (T) this.factory.createParticle((int) data[0], args.world, args.x, args.y, args.z, args.motionX,
					args.motionY, args.motionZ, additionalArgs);
		} else {
			return (T) this.factory.createParticle((int) data[0], args.world, args.x, args.y, args.z, args.motionX,
					args.motionY, args.motionZ);
		}
	}
