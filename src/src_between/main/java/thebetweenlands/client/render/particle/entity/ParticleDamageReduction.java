...
public class ParticleDamageReduction extends ParticleAnimated implements IParticleSpriteReceiver {
	...
	protected ParticleDamageReduction(World world, double x, double y, double z, double mx, double my, double mz,
			Entity entity, Vec3d offset, Vec3d normal, float scale, int maxAge) {
	...
	@Override
	public void renderParticle(BufferBuilder buff, Entity entityIn, float partialTicks, float rx, float rz, float ryz,
			float rxy, float rxz) {
		...
		Vec3d[] vertices = new Vec3d[] {
				perpendicular.add(perpendicular2.scale(-1)).add(perpendicular.scale(yOffset)).scale(scale),
				perpendicular.scale(-1).add(perpendicular2.scale(-1)).add(perpendicular.scale(yOffset)).scale(scale),
				perpendicular.scale(-1).add(perpendicular2).add(perpendicular.scale(yOffset)).scale(scale),
				perpendicular.add(perpendicular2).add(perpendicular.scale(yOffset)).scale(scale) };
		...
		if (this.particleAngle != 0.0F) {
			...
			for (int l = 0; l < 4; ++l) {
				vertices[l] = vec3d.scale(2.0D * vertices[l].dotProduct(vec3d))
						.add(vertices[l].scale((double) (f9 * f9) - vec3d.dotProduct(vec3d)))
						.add(vec3d.crossProduct(vertices[l]).scale((double) (2.0F * f9)));
			}
		}

		buff.pos((double) rpx + vertices[0].x, (double) rpy + vertices[0].y, (double) rpz + vertices[0].z)
				.tex((double) maxU, (double) maxV)
				.color(this.particleRed, this.particleGreen, this.particleBlue, this.particleAlpha)
				.lightmap(lightmapX, lightmapY).endVertex();
		buff.pos((double) rpx + vertices[1].x, (double) rpy + vertices[1].y, (double) rpz + vertices[1].z)
				.tex((double) maxU, (double) minV)
				.color(this.particleRed, this.particleGreen, this.particleBlue, this.particleAlpha)
				.lightmap(lightmapX, lightmapY).endVertex();
		buff.pos((double) rpx + vertices[2].x, (double) rpy + vertices[2].y, (double) rpz + vertices[2].z)
				.tex((double) minU, (double) minV)
				.color(this.particleRed, this.particleGreen, this.particleBlue, this.particleAlpha)
				.lightmap(lightmapX, lightmapY).endVertex();
		buff.pos((double) rpx + vertices[3].x, (double) rpy + vertices[3].y, (double) rpz + vertices[3].z)
	...
	public static final class Factory extends ParticleFactory<Factory, ParticleDamageReduction> {
		public Factory() {
			super(ParticleDamageReduction.class,
					ParticleTextureStitcher
							.create(ParticleDamageReduction.class,
									new ResourceLocation("thebetweenlands:particle/damage_reduction"))
							.setSplitAnimations(true));
		}

		@Override
		public ParticleDamageReduction createParticle(ImmutableParticleArgs args) {
			return new ParticleDamageReduction(args.world, args.x, args.y, args.z, args.motionX, args.motionY,
					args.motionZ, args.data.getObject(Entity.class, 0), args.data.getObject(Vec3d.class, 1),
					args.data.getObject(Vec3d.class, 2), args.scale, args.data.getInt(3));
		}
