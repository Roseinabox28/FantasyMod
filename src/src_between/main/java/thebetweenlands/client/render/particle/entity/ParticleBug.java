...
public class ParticleBug extends Particle implements IParticleSpriteReceiver {
	...
	protected ParticleBug(World world, double x, double y, double z, double mx, double my, double mz, int maxAge,
			float speed, float jitter, float scale, boolean underwater) {
	...
	public void onUpdate() {
		...
		this.move(this.world.rand.nextFloat() * this.jitter * 2 - this.jitter,
				this.world.rand.nextFloat() * this.jitter * 2 - this.jitter,
				this.world.rand.nextFloat() * this.jitter * 2 - this.jitter);
		double distToTarget = Math.sqrt((this.tx - this.posX) * (this.tx - this.posX)
				+ (this.ty - this.posY) * (this.ty - this.posY) + (this.tz - this.posZ) * (this.tz - this.posZ));
		Block currBlock = this.world.getBlockState(
				new BlockPos((int) Math.floor(this.posX), (int) Math.floor(this.posY), (int) Math.floor(this.posZ)))
				.getBlock();
		if (this.underwater == (currBlock instanceof BlockSwampWater == false)) {
			...
			if (distToTarget <= this.speed + this.jitter) {
				...
				Block targetBlock = this.world.getBlockState(new BlockPos((int) Math.floor(this.tx),
						(int) Math.floor(this.ty + 0.5D), (int) Math.floor(this.tz))).getBlock();
				...
				this.move(-(this.posX - this.tx) / distToTarget * this.speed,
						-(this.posY - this.ty) / distToTarget * this.speed,
						-(this.posZ - this.tz) / distToTarget * this.speed);
			}
		}
	}
	...
	public static final class FlyFactory extends ParticleFactory<FlyFactory, ParticleBug> {
		public FlyFactory() {
			super(ParticleBug.class,
					ParticleTextureStitcher
							.create(ParticleBug.class, new ResourceLocation("thebetweenlands:particle/fly"))
							.setSplitAnimations(true));
		}

		@Override
		public ParticleBug createParticle(ImmutableParticleArgs args) {
			return new ParticleBug(args.world, args.x, args.y, args.z, args.motionX, args.motionY, args.motionZ,
					args.data.getInt(0), args.data.getFloat(1), args.data.getFloat(2), args.scale,
					args.data.getBool(3));
		}
	...
	public static final class MosquitoFactory extends ParticleFactory<MosquitoFactory, ParticleBug> {
		public MosquitoFactory() {
			super(ParticleBug.class,
					ParticleTextureStitcher
							.create(ParticleBug.class, new ResourceLocation("thebetweenlands:particle/mosquito"))
							.setSplitAnimations(true));
		}
	...
	public static final class WaterBugFactory extends ParticleFactory<WaterBugFactory, ParticleBug> {
		public WaterBugFactory() {
			super(ParticleBug.class,
					ParticleTextureStitcher
							.create(ParticleBug.class, new ResourceLocation("thebetweenlands:particle/water_bug"))
							.setSplitAnimations(true));
		}
