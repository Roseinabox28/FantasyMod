...
public class ParticleSimple extends Particle implements IParticleSpriteReceiver {
	...
	public ParticleSimple(World worldIn, double xCoordIn, double yCoordIn, double zCoordIn, double xSpeedIn,
			double ySpeedIn, double zSpeedIn, int maxAge, float scale, boolean fade, float gravity,
			boolean exactMotion) {
	...
	public static final class GenericFactory extends ParticleFactory<GenericFactory, ParticleSimple> {
		...
		public ParticleSimple createParticle(ImmutableParticleArgs args) {
			return new ParticleSimple(args.world, args.x, args.y, args.z, args.motionX, args.motionY, args.motionZ,
					args.data.getInt(0), args.scale, args.data.getBool(1), args.data.getFloat(2), args.data.getBool(3));
		}
