...
public class ParticleThem extends Particle {
	...
	public void renderParticle(BufferBuilder vertexBuffer, Entity entity, float partialTicks, float rotationX,
		...
		float particleDist = renderView == null ? 0.0F
				: (float) renderView.getDistance(this.posX, this.posY, this.posZ);
		...
		vertexBuffer
				.pos((double) ipx + rotation[0].x, (double) ipy + rotation[0].y * 1.8F, (double) ipz + rotation[0].z)
				.tex((double) umax, (double) vmax).color(this.particleRed, this.particleGreen, this.particleBlue, alpha)
				.lightmap(lightmapX, lightmapY).endVertex();
		vertexBuffer
				.pos((double) ipx + rotation[1].x, (double) ipy + rotation[1].y * 1.8F, (double) ipz + rotation[1].z)
				.tex((double) umax, (double) vmin).color(this.particleRed, this.particleGreen, this.particleBlue, alpha)
				.lightmap(lightmapX, lightmapY).endVertex();
		vertexBuffer
				.pos((double) ipx + rotation[2].x, (double) ipy + rotation[2].y * 1.8F, (double) ipz + rotation[2].z)
				.tex((double) umin, (double) vmin).color(this.particleRed, this.particleGreen, this.particleBlue, alpha)
				.lightmap(lightmapX, lightmapY).endVertex();
		vertexBuffer
				.pos((double) ipx + rotation[3].x, (double) ipy + rotation[3].y * 1.8F, (double) ipz + rotation[3].z)
				.tex((double) umin, (double) vmax).color(this.particleRed, this.particleGreen, this.particleBlue, alpha)
				.lightmap(lightmapX, lightmapY).endVertex();
	...
	public void onUpdate() {
		...
		Entity renderView = Minecraft.getMinecraft().getRenderViewEntity();
		if (renderView != null) {
			Vec3d diff = renderView.getPositionVector().subtract(this.posX, this.posY - renderView.getEyeHeight(),
					this.posZ);
			if (diff.length() < 2.0F) {
				this.setExpired();
			}
