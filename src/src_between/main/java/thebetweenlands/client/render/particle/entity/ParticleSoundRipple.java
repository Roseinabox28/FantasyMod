...
public class ParticleSoundRipple extends Particle {
	public static final ResourceLocation TEXTURE = new ResourceLocation(ModInfo.ID,
			"textures/particle/sound_ripple.png");
	...
	protected ParticleSoundRipple(World world, double x, double y, double z, float scale, int delay,
			boolean spawnMore) {
	...
	@Override
	public void renderParticle(BufferBuilder vertexBuffer, Entity entity, float partialTicks, float rotationX,
		...
		if (!this.spawnMore) {
			...
			float ipx = (float) (this.prevPosX + (this.posX - this.prevPosX) * (double) partialTicks
					- (interpPosX + camPos.x));
			float ipy = (float) (this.prevPosY + (this.posY - this.prevPosY) * (double) partialTicks
					- (interpPosY + camPos.y));
			float ipz = (float) (this.prevPosZ + (this.posZ - this.prevPosZ) * (double) partialTicks
					- (interpPosZ + camPos.z));
			...
			if (this.particleAngle != 0.0F) {
				float interpolatedRoll = this.particleAngle
				...
				for (int l = 0; l < 4; ++l) {
					rotation[l] = look.scale(2.0D * rotation[l].dotProduct(look))
							.add(rotation[l].scale((double) (cos * cos) - look.dotProduct(look)))
							.add(look.crossProduct(rotation[l]).scale((double) (2.0F * cos)));
				}
			}
			...
			vertexBuffer.pos((double) ipx + rotation[0].x, (double) ipy + rotation[0].y, (double) ipz + rotation[0].z)
					.tex((double) umax, (double) vmax)
					.color(this.particleRed, this.particleGreen, this.particleBlue, alpha).lightmap(240, 240)
					.endVertex();
			vertexBuffer.pos((double) ipx + rotation[1].x, (double) ipy + rotation[1].y, (double) ipz + rotation[1].z)
					.tex((double) umax, (double) vmin)
					.color(this.particleRed, this.particleGreen, this.particleBlue, alpha).lightmap(240, 240)
					.endVertex();
			vertexBuffer.pos((double) ipx + rotation[2].x, (double) ipy + rotation[2].y, (double) ipz + rotation[2].z)
					.tex((double) umin, (double) vmin)
					.color(this.particleRed, this.particleGreen, this.particleBlue, alpha).lightmap(240, 240)
					.endVertex();
			vertexBuffer.pos((double) ipx + rotation[3].x, (double) ipy + rotation[3].y, (double) ipz + rotation[3].z)
		...
		if (this.spawnMore && this.particleAge >= this.delay) {
			if (this.particleAge == this.delay && Minecraft.getMinecraft().player != null) {
				Minecraft.getMinecraft().getSoundHandler()
						.playSound(new GemSingerEchoSound(new Vec3d(this.posX, this.posY, this.posZ))
								.setVolumeAndPitch(0.7f, 0.98f + this.world.rand.nextFloat() * 0.06f - 0.03f));
			}
			if ((this.particleAge - this.delay) % 10 == 0) {
				BatchedParticleRenderer.INSTANCE.addParticle(DefaultParticleBatches.UNBATCHED, new ParticleSoundRipple(
						this.world, this.posX, this.posY, this.posZ, this.particleScale, 0, false));
			}
		}
