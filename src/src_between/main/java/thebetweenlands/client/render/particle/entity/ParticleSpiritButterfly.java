...
public class ParticleSpiritButterfly extends ParticleBug {
	protected ParticleSpiritButterfly(World world, double x, double y, double z, double mx, double my, double mz,
			int maxAge, float speed, float jitter, float scale) {
	...
	public static final class Factory extends ParticleFactory<Factory, ParticleSpiritButterfly> {
		public Factory() {
			super(ParticleSpiritButterfly.class,
					ParticleTextureStitcher
							.create(ParticleSpiritButterfly.class,
									new ResourceLocation("thebetweenlands:particle/spirit_butterfly"))
							.setSplitAnimations(true));
		}

		@Override
		public ParticleSpiritButterfly createParticle(ImmutableParticleArgs args) {
			return new ParticleSpiritButterfly(args.world, args.x, args.y, args.z, args.motionX, args.motionY,
					args.motionZ, args.data.getInt(0), args.data.getFloat(1), args.data.getFloat(2), args.scale);
		}
