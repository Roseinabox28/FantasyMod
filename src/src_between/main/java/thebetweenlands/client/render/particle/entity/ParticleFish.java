...
public class ParticleFish extends ParticleBug {
	...
	protected ParticleFish(World world, double x, double y, double z, double mx, double my, double mz, int maxAge,
			float speed, float jitter, float scale, int texture) {
	...
	public static final class Factory extends ParticleFactory<Factory, ParticleFish> {
		public Factory() {
			super(ParticleFish.class,
					ParticleTextureStitcher.create(ParticleFish.class,
							new ResourceLocation[] { new ResourceLocation("thebetweenlands:particle/fish_1"),
		...
		public ParticleFish createParticle(ImmutableParticleArgs args) {
			return new ParticleFish(args.world, args.x, args.y, args.z, args.motionX, args.motionY, args.motionZ,
					args.data.getInt(0), args.data.getFloat(1), args.data.getFloat(2), args.scale, args.data.getInt(3));
		}
		...
		protected void setDefaultArguments(World world, double x, double y, double z, ParticleArgs<?> args) {
			args.withScale(0.18F * world.rand.nextFloat() + 0.1F).withDataBuilder().setData(3, world.rand.nextInt(3))
					.buildData();
		}
	}
}
