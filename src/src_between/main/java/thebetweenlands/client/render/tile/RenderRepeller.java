...
public class RenderRepeller extends TileEntitySpecialRenderer<TileEntityRepeller> {
	...
	protected static final ResourceLocation TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/tiles/repeller.png");
	...
	@Override
	public void render(TileEntityRepeller tile, double x, double y, double z, float partialTicks, int destroyStage,
			float alpha) {
		EnumFacing facing = StatePropertyHelper.getStatePropertySafely(tile, BlockRepeller.class, BlockRepeller.FACING,
				EnumFacing.NORTH);
		...
		if (tile != null && tile.hasShimmerstone()) {
			...
			this.renderShine(tile.renderTicks + partialTicks, 20, 1.0F, 0.8F, 0.0F, 0.0F, 1.0F, 0.8F, 0.0F, 1.0F);
		...
		if (tile != null && tile.isRunning()) {
			...
			WorldRenderHandler.REPELLER_SHIELDS
					.add(Pair.of(new Vec3d(x + 0.5F + xOff, y + 1.15F, z + 0.5F - zOff), tile.getRadius(partialTicks)));
		}
	}

	protected void renderShine(float ticks, int iterations, float or, float og, float ob, float oa, float ir, float ig,
			float ib, float ia) {
		...
		for (int i = 0; (float) i < iterations; ++i) {
			...
			buffer.pos(-0.866D * (double) pos2, (double) pos1, (double) (-0.5F * pos2))
					.color(or, og, ob, oa * brightness).endVertex();
			buffer.pos(0.866D * (double) pos2, (double) pos1, (double) (-0.5F * pos2))
					.color(or, og, ob, oa * brightness).endVertex();
