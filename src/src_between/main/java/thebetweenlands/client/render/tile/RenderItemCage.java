...
public class RenderItemCage extends TileEntitySpecialRenderer<TileEntityItemCage> {
	...
	private static final ResourceLocation FORCE_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/tiles/item_cage_power.png");
	private static final ResourceLocation CAGE_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/tiles/item_cage.png");
	private final ModelItemCage model = new ModelItemCage();

	@Override
	public void render(TileEntityItemCage tile, double x, double y, double z, float partialTicks, int destroyStage,
			float alpha) {
		...
		if (ShaderHelper.INSTANCE.isWorldShaderActive() && tile != null) {
			...
			ShaderHelper.INSTANCE.getWorldShader()
					.addLight(new LightSource(tile.getPos().getX() + 0.5D, tile.getPos().getY() + 0.5D,
							tile.getPos().getZ() + 0.5D, 3.5f, 5.0f / 255.0f * 16.0F, 40.0f / 255.0f * 16.0F,
							60.0f / 255.0f * 16.0F));
		}
