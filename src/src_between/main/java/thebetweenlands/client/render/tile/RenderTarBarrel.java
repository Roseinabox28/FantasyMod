...
public class RenderTarBarrel extends FastTESR<TileEntityBarrel> {
	...
	@Override
	public void renderTileEntityFast(TileEntityBarrel te, double x, double y, double z, float partialTicks,
		...
		if (fluid != null && fluid.amount > 0) {
			...
			buffer.pos(offX, height, 1 - offZ).color(r, g, b, a).tex(minU, minV).lightmap(skyLight, blockLight)
					.endVertex();
			buffer.pos(1 - offX, height, 1 - offZ).color(r, g, b, a).tex(maxU, minV).lightmap(skyLight, blockLight)
					.endVertex();
			buffer.pos(1 - offX, height, offZ).color(r, g, b, a).tex(maxU, maxV).lightmap(skyLight, blockLight)
					.endVertex();
