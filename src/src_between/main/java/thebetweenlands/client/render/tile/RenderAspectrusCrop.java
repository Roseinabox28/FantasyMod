...
public class RenderAspectrusCrop extends TileEntitySpecialRenderer<TileEntityAspectrusCrop> {
	...
	protected static final ResourceLocation TEXTURE_0 = new ResourceLocation(
			"thebetweenlands:textures/tiles/aspectrus_crop_0.png");
	protected static final ResourceLocation TEXTURE_1 = new ResourceLocation(
			"thebetweenlands:textures/tiles/aspectrus_crop_1.png");
	protected static final ResourceLocation TEXTURE_2 = new ResourceLocation(
			"thebetweenlands:textures/tiles/aspectrus_crop_2.png");
	protected static final ResourceLocation TEXTURE_3 = new ResourceLocation(
			"thebetweenlands:textures/tiles/aspectrus_crop_3.png");
	protected static final ResourceLocation TEXTURE_4 = new ResourceLocation(
			"thebetweenlands:textures/tiles/aspectrus_crop_4.png");
	protected static final ResourceLocation TEXTURE_5 = new ResourceLocation(
			"thebetweenlands:textures/tiles/aspectrus_crop_5.png");
	protected static final ResourceLocation TEXTURE_6 = new ResourceLocation(
			"thebetweenlands:textures/tiles/aspectrus_crop_6.png");

	@Override
	public void render(TileEntityAspectrusCrop tile, double x, double y, double z, float partialTicks, int destroyStage,
			float alpha) {
		...
		int index = StatePropertyHelper.getStatePropertySafely(tile, BlockAspectrusCrop.class,
				BlockRegistry.ASPECTRUS_CROP.getStageProperty(), 0, true, false) / 2;
		if (index <= 4) {
			index = index / 2;
		} else {
			index = index - 2;
		}
		if (index >= 5 && StatePropertyHelper.getStatePropertySafely(tile, BlockAspectrusCrop.class,
				BlockGenericCrop.DECAYED, false, true, false)) {
			index = 6;
		}
		...
		if (index >= 4 && ShaderHelper.INSTANCE.isWorldShaderActive() && aspect != null) {
			...
			float brightness = ((float) Math.sin((tile.glowTicks + partialTicks) / 15.0F)
					* (float) Math.cos((tile.glowTicks + partialTicks + 4) / 80.0F) + 1.0F) / 2.0F;
			ShaderHelper.INSTANCE.getWorldShader()
					.addLight(new LightSource(tile.getPos().getX() + 0.5D, tile.getPos().getY() + 0.5D,
							tile.getPos().getZ() + 0.5D, brightness * brightness * 4.0F,
							rgba[0] * brightness * brightness * 2.5F, rgba[1] * brightness * brightness * 2.5F,
