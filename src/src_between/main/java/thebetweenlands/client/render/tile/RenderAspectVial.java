...
public class RenderAspectVial extends TileEntitySpecialRenderer<TileEntityAspectVial> {
	...
	public static final ResourceLocation TEXTURE1 = new ResourceLocation(
			"thebetweenlands:textures/tiles/vial_block_green.png");
	public static final ResourceLocation TEXTURE2 = new ResourceLocation(
			"thebetweenlands:textures/tiles/vial_block_orange.png");

	@Override
	public void render(TileEntityAspectVial te, double x, double y, double z, float partialTicks, int destroyStage,
			float alpha) {
		BlockDentrothyst.EnumDentrothyst type = StatePropertyHelper.getStatePropertySafely(te, BlockAspectVial.class,
				BlockAspectVial.TYPE, EnumDentrothyst.GREEN);
		switch (type) {
		default:
		case GREEN:
		...
		if (StatePropertyHelper.getStatePropertySafely(te, BlockAspectVial.class, BlockAspectVial.RANDOM_POSITION,
				true)) {
			long posRand = (long) (te.getPos().getY() * 224856) ^ (te.getPos().getX() * 3129871)
					^ (long) te.getPos().getZ() * 116129781L;
