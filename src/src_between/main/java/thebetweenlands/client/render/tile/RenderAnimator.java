...
public class RenderAnimator extends TileEntitySpecialRenderer<TileEntityAnimator> {
	...
	@Override
	public void render(TileEntityAnimator te, double x, double y, double z, float partialTicks, int destroyStage,
			float alpha) {
		int meta = 0;
		...
		if (te != null) {
			SplittableRandom rand = new SplittableRandom(
					(long) (te.getPos().getX() + te.getPos().getY() + te.getPos().getZ()));
			...
			if (!te.getStackInSlot(2).isEmpty()) {
				...
				for (int i = 0; i < items; i++) {
					GlStateManager.pushMatrix();
					GlStateManager.translate(rand.nextDouble() / 3.0D - 1.0D / 6.0D, 0.0D,
							rand.nextDouble() / 3.0D - 1.0D / 6.0D);
			...
			if (!input.isEmpty()) {
				...
				if (recipe != null) {
					if (!(input.getItem() instanceof ItemMonsterPlacer)
							&& (recipe == null || recipe.getRenderEntity(input) == null)) {
