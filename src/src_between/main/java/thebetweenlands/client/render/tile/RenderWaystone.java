...
public class RenderWaystone extends TileEntitySpecialRenderer<TileEntityWaystone> {
	...
	private static final ResourceLocation TEXTURE_ACTIVE = new ResourceLocation(
			"thebetweenlands:textures/tiles/waystone_active.png");
	private static final ResourceLocation TEXTURE_INACTIVE = new ResourceLocation(
			"thebetweenlands:textures/tiles/waystone_inactive.png");
	private static final ResourceLocation TEXTURE_GRASS = new ResourceLocation(
			"thebetweenlands:textures/tiles/waystone_grass.png");

	@Override
	public void render(TileEntityWaystone te, double x, double y, double z, float partialTicks, int destroyStage,
			float alpha) {
		BlockWaystone.Part part = StatePropertyHelper.getStatePropertySafely(te, BlockWaystone.class,
				BlockWaystone.PART, BlockWaystone.Part.BOTTOM);
		boolean active = StatePropertyHelper.getStatePropertySafely(te, BlockWaystone.class, BlockWaystone.ACTIVE,
				false);
		...
		if (part == BlockWaystone.Part.BOTTOM || isBreakingAnimation) {
			...
			if (isBreakingAnimation) {
				...
				case TOP:
					...
					if (te.getWorld() != null)
						master = te.getWorld().getTileEntity(te.getPos().down(2));
					break;
				case MIDDLE:
					GlStateManager.translate(0, 1, 0);
					if (te.getWorld() != null)
						master = te.getWorld().getTileEntity(te.getPos().down(1));
					break;
				case BOTTOM:
					master = te;
					break;
				...
				if (active && te != null && ShaderHelper.INSTANCE.isWorldShaderActive()) {
					...
					if (closestPlayer != null) {
						...
						ShaderHelper.INSTANCE.getWorldShader().addLight(new LightSource(px, py, pz, 8F,
								0.44f * brightness, 0.8f * brightness, 1.0f * brightness));
					}
				}
			}
			...
			if (isBreakingAnimation) {
				...
				int grassColor = te != null ? BiomeColorHelper.getGrassColorAtPos(te.getWorld(), te.getPos())
						: ColorizerGrass.getGrassColor(0.5D, 1.0D);

				GlStateManager.color((float) (grassColor >> 16 & 0xff) / 255F, (float) (grassColor >> 8 & 0xff) / 255F,
						(float) (grassColor & 0xff) / 255F);
