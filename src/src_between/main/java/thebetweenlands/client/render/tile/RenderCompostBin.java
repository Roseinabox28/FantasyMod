...
public class RenderCompostBin extends TileEntitySpecialRenderer<TileEntityCompostBin> {
	public static final ResourceLocation TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/tiles/compost_bin.png");
	public static final ModelCompostBin MODEL = new ModelCompostBin();

	@Override
	public void render(TileEntityCompostBin te, double x, double y, double z, float partialTicks, int destroyStage,
			float alpha) {
		int meta = te != null ? te.getBlockMetadata() : 0;
		RenderItem renderItem = Minecraft.getMinecraft().getRenderItem();

		float compostHeight = te != null
				? Math.min(te.getCompostedAmount() / (float) TileEntityCompostBin.MAX_COMPOST_AMOUNT, 0.82f)
				: 0;
		...
		if (compostHeight > 0.01f) {
			...
			GlStateManager.translate(-(float) blockPos.getX() - 0.5D, -(float) blockPos.getY(),
			...
			blockRenderer.getBlockModelRenderer().renderModel(te.getWorld(), blockRenderer.getModelForState(compost),
					compost, blockPos, vertexbuffer, false, MathHelper.getPositionRandom(blockPos));
			tessellator.draw();
