...
public class RenderInfuser extends TileEntitySpecialRenderer<TileEntityInfuser> {
	...
	@Override
	public void render(TileEntityInfuser infuser, double x, double y, double z, float partialTicks, int destroyStage,
			float alpha) {
		bindTexture(TEXTURE);
		...
		EnumFacing facing = StatePropertyHelper.getStatePropertySafely(infuser, BlockInfuser.class, BlockInfuser.FACING,
				EnumFacing.NORTH);
		GlStateManager.pushMatrix();
		...
		if (BetweenlandsConfig.DEBUG.debug) {
			...
			renderStirCount("Evap: " + infuser.getEvaporation() + " Temp: " + infuser.getTemperature() + " Time: "
					+ infuser.getInfusionTime() + " Recipe: " + elixirName, x, y, z);
		}
		...
		for (int i = 0; i <= TileEntityInfuser.MAX_INGREDIENTS; i++) {
			...
			renderItemInSlot(infuser, i, 0, itemY, 0,
					amount >= 100 ? (i % 2 == 0 ? (itemBob * 0.01D) : ((-itemBob + 20) * 0.01D)) : 0.0D, -rot);
			GlStateManager.popMatrix();
		}

		renderItemInSlot(infuser, TileEntityInfuser.MAX_INGREDIENTS + 1, x + 0.5, y + 1.43D, z + 0.5D, itemBob * 0.01D,
				crystalRotation);

		if (amount >= 100) {
			Tessellator tess = Tessellator.getInstance();
			BufferBuilder vb = tess.getBuffer();
			TextureAtlasSprite waterSprite = Minecraft.getMinecraft().getTextureMapBlocks()
					.getAtlasSprite(FluidRegistry.SWAMP_WATER.getStill().toString());
			GlStateManager.pushMatrix();
			...
			if (infuser.getInfusionColorGradientTicks() > 0) {
				targetColor = new float[] {
						infuser.prevInfusionColor[0] + (infuser.currentInfusionColor[0] - infuser.prevInfusionColor[0])
								/ 30.0F * infuser.getInfusionColorGradientTicks(),
						infuser.prevInfusionColor[1] + (infuser.currentInfusionColor[1] - infuser.prevInfusionColor[1])
								/ 30.0F * infuser.getInfusionColorGradientTicks(),
						infuser.prevInfusionColor[2] + (infuser.currentInfusionColor[2] - infuser.prevInfusionColor[2])
								/ 30.0F * infuser.getInfusionColorGradientTicks(),
						infuser.prevInfusionColor[3] + (infuser.currentInfusionColor[3] - infuser.prevInfusionColor[3])
								/ 30.0F * infuser.getInfusionColorGradientTicks() };
			} else {
				targetColor = infuser.currentInfusionColor;
			}
			vb.pos(0.1875, 0, 0.1875).tex(waterSprite.getMinU(), waterSprite.getMinV())
					.color(targetColor[0], targetColor[1], targetColor[2], targetColor[3]).endVertex();
			vb.pos(0.1875, 0, 0.8125).tex(waterSprite.getMinU(), waterSprite.getMaxV())
					.color(targetColor[0], targetColor[1], targetColor[2], targetColor[3]).endVertex();
			vb.pos(0.8125, 0, 0.8125).tex(waterSprite.getMaxU(), waterSprite.getMaxV())
					.color(targetColor[0], targetColor[1], targetColor[2], targetColor[3]).endVertex();
			vb.pos(0.8125, 0, 0.1875).tex(waterSprite.getMaxU(), waterSprite.getMinV())
					.color(targetColor[0], targetColor[1], targetColor[2], targetColor[3]).endVertex();
	...
	private void renderItemInSlot(TileEntityInfuser infuser, int slotIndex, double x, double y, double z,
			double itemBob, double rotation) {
