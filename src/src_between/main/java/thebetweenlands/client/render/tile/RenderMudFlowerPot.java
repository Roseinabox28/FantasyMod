...
public class RenderMudFlowerPot extends TileEntitySpecialRenderer<TileEntityMudFlowerPot> {
	...
	@Override
	public void render(TileEntityMudFlowerPot te, double x, double y, double z, float partialTicks, int destroyStage,
			float alpha) {
		IBlockState flowerBlockState = StatePropertyHelper.getStatePropertySafely(te, BlockMudFlowerPot.class,
				BlockMudFlowerPot.FLOWER, null, false, true);
		...
		if (flowerBlockState != null && flowerBlockState.getBlock() != Blocks.AIR) {
			...
			IBakedModel model = Minecraft.getMinecraft().getBlockRendererDispatcher()
					.getModelForState(flowerBlockState);
			...
			BLOCK_RENDERER.setLighting((IBlockState blockState, @Nullable EnumFacing facing) -> {
				return world.getBlockState(pos.up()).getPackedLightmapCoords(world,
						facing != null ? pos.up().offset(facing) : pos.up());
			}).setTint((IBlockState blockState, int tintIndex) -> {
				return Minecraft.getMinecraft().getBlockColors().colorMultiplier(flowerBlockState, world, pos.up(),
						tintIndex);
			});

			BLOCK_RENDERER.renderModel(te.getWorld(), BlockPos.ORIGIN, model, flowerBlockState,
					MathHelper.getPositionRandom(te.getPos()), vertexBuffer);
