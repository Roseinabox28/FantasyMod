...
public class RenderSpikeTrap extends TileEntitySpecialRenderer<TileEntitySpikeTrap> implements IFastTESRBakedModels {
	...
	public RenderSpikeTrap() {
		this(new ResourceLocation("thebetweenlands:spike_trap"),
				new ResourceLocation("thebetweenlands:textures/tiles/spike_block_spikes_1.png"));
	}
	...
	public RenderSpikeTrap(ResourceLocation overlayModel, ResourceLocation spikeTexture) {
		...
		for (EnumFacing facing : EnumFacing.VALUES) {
			ModelResourceLocation location = new ModelResourceLocation(overlayModel,
					"facing=" + facing.getName() + ",overlay=true");
	...
	@Override
	public void render(TileEntitySpikeTrap tile, double x, double y, double z, float partialTicks, int destroyStage,
			float alpha) {
		this.renderFast(tile, x, y, z, partialTicks, destroyStage, alpha);
	}

	private void renderFast(TileEntitySpikeTrap tile, double x, double y, double z, float partialTicks,
	...
	private void renderSpikes(TileEntitySpikeTrap tile, double x, double y, double z, float partialTicks,
			int destroyStage, float alpha) {
		if (tile.animationTicks > 0) {
			EnumFacing facing = StatePropertyHelper.getStatePropertySafely(tile, BlockSpikeTrap.class,
					BlockSpikeTrap.FACING, EnumFacing.UP);
		...
		if (state.getBlock() instanceof BlockSpikeTrap) {
			if (tile.type != 0) {
				...
				if (state.getBlock() instanceof BlockSpikeTrap) {
					...
					blockRenderer.getBlockModelRenderer().renderModel(world, this.overlayModels[facing.ordinal()],
							state, pos, buffer, false);
				}
			}
