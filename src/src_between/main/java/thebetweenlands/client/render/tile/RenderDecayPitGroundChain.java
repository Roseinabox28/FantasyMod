...
public class RenderDecayPitGroundChain extends TileEntitySpecialRenderer<TileEntityDecayPitGroundChain> {
	public static final ResourceLocation CHAIN_TEXTURE = new ResourceLocation(ModInfo.ID,
			"textures/entity/decay_pit_chain.png");
	private final ModelDecayPitChain CHAIN_MODEL = new ModelDecayPitChain();

	@Override
	public void render(TileEntityDecayPitGroundChain tile, double x, double y, double z, float partialTicks,
			int destroyStage, float alpha) {
		if (tile == null || !tile.hasWorld())
			return;

		float scroll = tile.animationTicksChainPrev * 0.0078125F
				+ (tile.animationTicksChain * 0.0078125F - tile.animationTicksChainPrev * 0.0078125F) * partialTicks;
		EnumFacing facing = StatePropertyHelper.getStatePropertySafely(tile, BlockDecayPitGroundChain.class,
				BlockDecayPitGroundChain.FACING, EnumFacing.NORTH);
