...
public class RenderBeamOrigin extends TileEntitySpecialRenderer<TileEntityBeamOrigin> {
	...
	@Override
	public void render(TileEntityBeamOrigin te, double x, double y, double z, float partialTicks, int destroyStage,
			float alpha) {
		...
		this.renderMirrorTri(te, m.transformVec(v0, mid), m.transformVec(v1, mid), m.transformVec(v2, mid), visibility,
				partialTicks);
		this.renderMirrorTri(te, m.transformVec(v0, mid), m.transformVec(v1.add(-w * 2, 0, 0), mid),
				m.transformVec(v2.add(0, 0, w * 2), mid), visibility, partialTicks);
		this.renderMirrorTri(te, m.transformVec(v0, mid), m.transformVec(v1.add(-w * 2, 0, -w * 2), mid),
				m.transformVec(v2.add(-w * 2, 0, w * 2), mid), visibility, partialTicks);
		this.renderMirrorTri(te, m.transformVec(v0, mid), m.transformVec(v1.add(0, 0, -w * 2), mid),
				m.transformVec(v2.add(-w * 2, 0, 0), mid), visibility, partialTicks);
	...
	protected void renderMirrorTri(TileEntityBeamOrigin te, Vec3d v0, Vec3d v1, Vec3d v2, float visibility,
			float partialTicks) {
		...
		GlStateManager.color(visibility * 0.65f, visibility * 0.65f, visibility * 0.65f,
				0.75f + 0.25f * (1 - visibility));
	...
	protected void renderMirror(TileEntityBeamOrigin te, Vec3d planePos, Vec3d planeNormal, Runnable mirrorRenderer,
			float partialTicks) {
		GlStateManager.enableBlend();
		...
		try (Stencil stencil1 = Stencil.reserve(fbo); Stencil stencil2 = Stencil.reserve(fbo)) {
			if (stencil1.valid() && stencil2.valid()) {
				...
				// set stencil bit 2 (when something is rendered on that pixel and z test
				// passes)
				...
				// The previous preparations have the effect that the mirror world is only
				// rendered
	...
	protected void renderMirrorWorld(TileEntityBeamOrigin te, float partialTicks) {
		...
		for (EntityPlayer entity : te.getWorld().getEntitiesWithinAABB(EntityPlayer.class,
				new AxisAlignedBB(te.getPos()).grow(20))) {
			...
			if (renderer != null) {
				...
				double ex = (entity.prevPosX + (entity.posX - entity.prevPosX) * partialTicks
						- (te.getPos().getX() + 0.5D));
				double ey = (entity.prevPosY + (entity.posY - entity.prevPosY) * partialTicks
						- (te.getPos().getY() - 1.5D));
		...
		if (needsRebuild) {
			...
			IBakedModel modelBrazier = Minecraft.getMinecraft().getBlockRendererDispatcher()
					.getModelForState(stateBrazier);
			Tessellator tessellator = Tessellator.getInstance();
			...
			BLOCK_RENDERER.renderModel(te.getWorld(), te.getPos(), modelFire, stateFire,
					MathHelper.getPositionRandom(te.getPos()), bufferBuilder);

			bufferBuilder.setTranslation(-0.5D + brazierPos.x - 3, -0.5D + brazierPos.y, -0.5D + brazierPos.z - 3);
			BLOCK_RENDERER.renderModel(te.getWorld(), te.getPos(), modelBrazier, stateBrazier,
					MathHelper.getPositionRandom(te.getPos()), bufferBuilder);
