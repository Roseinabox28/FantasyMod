...
public class RenderDruidAltar extends TileEntitySpecialRenderer<TileEntityDruidAltar> {
	...
	private final ResourceLocation ACTIVE = new ResourceLocation(
			"thebetweenlands:textures/tiles/druid_altar_active.png");
	private final ResourceLocation ACTIVEGLOW = new ResourceLocation(
			"thebetweenlands:textures/tiles/druid_altar_active_glow.png");
	private final ResourceLocation NORMAL = new ResourceLocation("thebetweenlands:textures/tiles/druid_altar.png");
	private final ResourceLocation NORMALGLOW = new ResourceLocation(
			"thebetweenlands:textures/tiles/druid_altar_glow.png");
	...
	private void renderTile(TileEntityDruidAltar tile, double x, double y, double z, float partialTicks) {
		...
		double yOff = 1.2D;
		if (StatePropertyHelper.getStatePropertySafely(tile, BlockDruidAltar.class, BlockDruidAltar.ACTIVE, false)
				&& tile.craftingProgress != 0) {
			yOff = Math.min(tile.renderYOffset + (tile.renderYOffset - tile.prevRenderYOffset) * partialTicks,
					TileEntityDruidAltar.FINAL_HEIGHT + 1.0D);

			GlStateManager.pushMatrix();
			GlStateManager.translate(x + 0.5D, y + 3.1D, z + 0.5D);
			GlStateManager.rotate(renderRotation * 2.0f, 0f, 1f, 0f);
			double shineScale = 0.04f * Math.pow(
					1.0D - (TileEntityDruidAltar.FINAL_HEIGHT + 1.0D - yOff) / TileEntityDruidAltar.FINAL_HEIGHT, 12);
			GlStateManager.scale(shineScale, shineScale, shineScale);
			this.renderShine((float) Math.sin(Math.toRadians(renderRotation)) / 2.0f - 0.2f, (int) (80 * Math.pow(
					1.0D - (TileEntityDruidAltar.FINAL_HEIGHT + 1.0D - yOff) / TileEntityDruidAltar.FINAL_HEIGHT, 12)));
			GlStateManager.popMatrix();
		}
		for (int xi = 0; xi < 2; xi++) {
			for (int zi = 0; zi < 2; zi++) {
				...
				double rProgress = 1.0D - Math.pow(
						1.0D - (TileEntityDruidAltar.FINAL_HEIGHT + 1.0D - yOff) / TileEntityDruidAltar.FINAL_HEIGHT,
						6);
				diffVec.x *= rProgress;
				diffVec.z *= rProgress;
				midVec.x += diffVec.x;
	...
	@Override
	public void render(TileEntityDruidAltar te, double x, double y, double z, float partialTicks, int destroyStage,
			float alpha) {
