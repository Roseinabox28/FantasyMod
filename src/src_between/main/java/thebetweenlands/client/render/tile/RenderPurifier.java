...
public class RenderPurifier extends TileEntitySpecialRenderer<TileEntityPurifier> {
	...
	@Override
	public void render(TileEntityPurifier tile, double x, double y, double z, float partialTicks, int destroyStage,
			float alpha) {
		...
		int rotation = StatePropertyHelper
				.getStatePropertySafely(tile, BlockPurifier.class, BlockPurifier.FACING, EnumFacing.NORTH)
		...
		if (!tile.getStackInSlot(2).isEmpty()) {
			...
			for (int i = 0; i < items; i++) {
				GlStateManager.pushMatrix();
				GlStateManager.translate(rand.nextFloat() / 3.0D - 1.0D / 6.0D, -0.25D,
						rand.nextFloat() / 3.0D - 1.0D / 6.0D);
		...
		if (amount >= 100) {
			...
			TextureAtlasSprite waterIcon = Minecraft.getMinecraft().getTextureMapBlocks()
					.getAtlasSprite("thebetweenlands:fluids/swamp_water_still");
			GlStateManager.pushMatrix();
			...
			buffer.pos(0.1F, 0.0F, 0.1F).tex(waterIcon.getMinU(), waterIcon.getMinV()).color(0.2F, 0.6F, 0.4F, 1.0F)
					.endVertex();
			buffer.pos(0.1F, 0.0F, 0.9F).tex(waterIcon.getMinU(), waterIcon.getMaxV()).color(0.2F, 0.6F, 0.4F, 1.0F)
					.endVertex();
			buffer.pos(0.9F, 0.0F, 0.9F).tex(waterIcon.getMaxU(), waterIcon.getMaxV()).color(0.2F, 0.6F, 0.4F, 1.0F)
					.endVertex();
			buffer.pos(0.9F, 0.0F, 0.1F).tex(waterIcon.getMaxU(), waterIcon.getMinV()).color(0.2F, 0.6F, 0.4F, 1.0F)
					.endVertex();
			tesselator.draw();
