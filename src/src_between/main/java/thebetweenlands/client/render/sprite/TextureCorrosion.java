...
/**
 * Generates a corrosion overlay for the specified item. Sprite naming goes as
 * follows: some_item_corrodible -> some_item_corrosion_[0,5]
 */
public class TextureCorrosion extends TextureAtlasSprite {
	public static final ResourceLocation CORROSION_TEXTURE = new ResourceLocation(ModInfo.ID,
			"textures/items/tool_corrosion.png");
	...
	private void loadCorrosionPixels(IResourceManager manager) {
		...
		try {
			...
			corrosionImg.getRGB(0, 0, corrosionImg.getWidth(), corrosionImg.getHeight(), corrosionPixels, 0,
					corrosionImg.getWidth());
	...
	public boolean load(IResourceManager manager, ResourceLocation location,
		...
		try {
			...
			LOGGER.error((String) ("Unable to parse metadata from " + this.loadedResourceLocation),
					(Throwable) runtimeexception);
		} catch (IOException ioexception) {
			LOGGER.error((String) ("Using missing texture, unable to load " + this.loadedResourceLocation),
	...
	public void loadSpriteFrames(IResource resource, int mipmapLevels) throws IOException {
		...
		spriteTexture.getRGB(0, 0, spriteTexture.getWidth(), spriteTexture.getHeight(), mipmappedFrames[0], 0,
				spriteTexture.getWidth());
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				...
				if (pixel >>> 24 != 0 && RANDOM.nextFloat() > 0.2F) {
					corrosion = corrosionPixels[(x % width % corrosionWidth)
							+ (y % width % corrosionWidth) * corrosionWidth];
				}
				float alpha = (this.corrosionAmount / (float) (CorrosionHelper.CORROSION_STAGE_COUNT - 1)
						* ((corrosion >>> 24 & 0xFF) / 255.0F)) * (0.5F + RANDOM.nextFloat() * 0.5F);
				mipmappedFrames[0][x + y * width] = (((int) (alpha * 255.0F) & 0xFF) << 24)
						| ((corrosion >> 16 & 0xFF) << 16) | ((corrosion >> 8 & 0xFF) << 8) | (corrosion & 0xFF);
			}
		}
		...
		if (spriteMetadata == null) {
			...
			if (spriteMetadata.getFrameCount() > 0) {
				...
				while (frameIndexIterator.hasNext()) {
					...
					this.framesTextureData.set(frameIndex,
							getFrameTextureData(mipmappedFrames, frameWidth, frameHeight, frameIndex));
				}
				...
				this.parentAnimationMetadata = new AnimationMetadataSection(animationFrames, width, height,
						spriteMetadata.getFrameTime(), spriteMetadata.isInterpolate());
			}
		}
	}
	...
	private static int[][] getFrameTextureData(int[][] framesData, int width, int height, int offset) {
		...
		for (int frame = 0; frame < framesData.length; ++frame) {
			...
			if (frameData != null) {
				...
				System.arraycopy(frameData, offset * newFrameData[frame].length, newFrameData[frame], 0,
						newFrameData[frame].length);
			}
		}
	...
	public void updateAnimation() {
		if (this.parentSprite == null) {
			...
			if (this.tickCounter >= this.parentAnimationMetadata.getFrameTimeSingle(this.frameCounter)) {
				...
				int frameCount = this.parentAnimationMetadata.getFrameCount() == 0 ? this.framesTextureData.size()
						: this.parentAnimationMetadata.getFrameCount();
				...
				if (frameIndex != nextFrameIndex && nextFrameIndex >= 0
						&& nextFrameIndex < this.framesTextureData.size()) {
					TextureUtil.uploadTextureMipmap((int[][]) this.framesTextureData.get(nextFrameIndex), this.width,
	...
	private void updateAnimationInterpolated() {
		double interpolatedFrameIndex = 1.0D - (double) this.tickCounter
				/ (double) this.parentAnimationMetadata.getFrameTimeSingle(this.frameCounter);
		int frameIndex = this.parentAnimationMetadata.getFrameIndex(this.frameCounter);
		int frameCount = this.parentAnimationMetadata.getFrameCount() == 0 ? this.framesTextureData.size()
				: this.parentAnimationMetadata.getFrameCount();
			...
			for (int mipmapLevel = 0; mipmapLevel < frame.length; ++mipmapLevel) {
				...
				if (mipmapLevel < nextFrame.length && nextFrame[mipmapLevel].length == frame[mipmapLevel].length) {
					for (int framePixelIndex = 0; framePixelIndex < frame[mipmapLevel].length; ++framePixelIndex) {
						...
						int interpR = this.interpolateColor(interpolatedFrameIndex, pixel >> 16 & 255,
								nextPixel >> 16 & 255);
						int interpG = this.interpolateColor(interpolatedFrameIndex, pixel >> 8 & 255,
								nextPixel >> 8 & 255);
						int interpB = this.interpolateColor(interpolatedFrameIndex, pixel & 255, nextPixel & 255);
						this.interpolatedFrameData[mipmapLevel][framePixelIndex] = pixel & -16777216 | interpR << 16
								| interpG << 8 | interpB;
					}
				}
			}
