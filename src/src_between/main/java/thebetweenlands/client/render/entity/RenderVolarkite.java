...
public class RenderVolarkite extends Render<EntityVolarkite> {
	protected static final ResourceLocation TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/entity/volarkite.png");
	...
	public void doRender(EntityVolarkite entity, double x, double y, double z, float yaw, float partialTicks) {
		...
		GlStateManager.rotate((float) -interpolate(entity.prevRotationYaw, entity.rotationYaw, partialTicks) + 180, 0,
				1, 0);
		GlStateManager.rotate((float) -interpolate(entity.prevRotationRoll, entity.rotationRoll, partialTicks), 0, 0,
				1);
		GlStateManager.rotate((float) -interpolate(entity.prevRotationPitch, entity.rotationPitch, partialTicks), 1, 0,
				0);
		GlStateManager.translate(0, 0.5D, 0);
	...
	public static void onPlayerRenderPre(RenderPlayerEvent.Pre event) {
		if (event.getEntityPlayer() instanceof AbstractClientPlayer) {
			...
			if (ridingEntity instanceof EntityVolarkite
					&& event.getRenderer() instanceof RenderPlayerVolarkite == false) {
				...
				float kiteYaw = (float) interpolate(kite.prevRotationYaw, kite.rotationYaw,
						event.getPartialRenderTick());

				GlStateManager.rotate(-kiteYaw, 0, 1, 0);
				GlStateManager.translate(0, 1.0D, 0);
				GlStateManager.rotate(
						(float) interpolate(kite.prevRotationRoll, kite.rotationRoll, event.getPartialRenderTick()), 0,
						0, 1);
				GlStateManager.rotate(
						(float) interpolate(kite.prevRotationPitch, kite.rotationPitch, event.getPartialRenderTick()),
						1, 0, 0);
				GlStateManager.translate(0, -1.0D, 0);
				GlStateManager.rotate(kiteYaw, 0, 1, 0);

				GlStateManager.rotate((float) interpolate(player.prevRenderYawOffset, player.renderYawOffset,
						event.getPartialRenderTick()) - kiteYaw, 0, 1, 0);
				...
				if (isSmallArms) {
					if (renderPlayerSmallArmsVolarkite == null) {
						renderPlayerSmallArmsVolarkite = new RenderPlayerVolarkite(
								event.getRenderer().getRenderManager(), true);
					}
					playerRenderer = renderPlayerSmallArmsVolarkite;
				} else {
					if (renderPlayerNormalArmsVolarkite == null) {
						renderPlayerNormalArmsVolarkite = new RenderPlayerVolarkite(
								event.getRenderer().getRenderManager(), false);
					}
					playerRenderer = renderPlayerNormalArmsVolarkite;
				}

				playerRenderer.doRender(player, event.getX(), event.getY(), event.getZ(),
						(float) interpolate(player.prevRotationYaw, player.rotationYaw, event.getPartialRenderTick()),
						event.getPartialRenderTick());
