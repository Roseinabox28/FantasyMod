...
public class RenderSwordEnergy extends Render<EntitySwordEnergy> {
	private static final ResourceLocation FORCE_TEXTURE = new ResourceLocation(
	...
	@Override
	public void doRender(EntitySwordEnergy entity, double x, double y, double z, float rotationYaw,
			float partialTickTime) {
		renderSwordEnergy(entity, x, y, z, rotationYaw, partialTickTime);
	}

	public void renderSwordEnergy(EntitySwordEnergy energyBall, double x, double y, double z, float rotationYaw,
			float partialTickTime) {
		if (ShaderHelper.INSTANCE.isWorldShaderActive()) {
			ShaderHelper.INSTANCE.require();
			ShaderHelper.INSTANCE.getWorldShader().addLight(new LightSource(energyBall.posX, energyBall.posY + 0.5D,
					energyBall.posZ, 2f, 5.0f / 255.0f * 13.0F, 40.0f / 255.0f * 13.0F, 60.0f / 255.0f * 13.0F));
		}
		...
		renderBeam(new Vec3d(x, y + 0.85F, z), new Vec3d(x - interpPos1 - 0.1F, y + 0.9F, z - interpPos1 - 0.1F), 0.05F,
				0.25F, true, true);
		GlStateManager.pushMatrix();
		...
		if (energyBall.pos1 < 3.5F)
			model.render(0.0625F);
		GlStateManager.popMatrix();
		renderBeam(new Vec3d(x, y + 0.85F, z), new Vec3d(x + interpPos2 + 0.1F, y + 0.9F, z - interpPos2 - 0.1F), 0.05F,
				0.25F, true, true);
		GlStateManager.pushMatrix();
		...
		if (energyBall.pos2 < 3.5F)
			model.render(0.0625F);
		GlStateManager.popMatrix();
		renderBeam(new Vec3d(x, y + 0.85F, z), new Vec3d(x + interpPos3 + 0.1F, y + 0.9F, z + interpPos3 + 0.1F), 0.05F,
				0.25F, true, true);
		GlStateManager.pushMatrix();
		...
		if (energyBall.pos3 < 3.5F)
			model.render(0.0625F);
		GlStateManager.popMatrix();
		renderBeam(new Vec3d(x, y + 0.85F, z), new Vec3d(x - interpPos4 - 0.1F, y + 0.9F, z + interpPos4 + 0.1F), 0.05F,
				0.25F, true, true);
		GlStateManager.pushMatrix();
	...
	public static void renderBeam(Vec3d start, Vec3d end, float startWidth, float endWidth, boolean renderStartCap,
			boolean renderEndCap) {
		...
		vertexbuffer.pos(start.x + (localSide.x + localUp.x) * startWidth,
				start.y + (localSide.y + localUp.y) * startWidth, start.z + (localSide.z + localUp.z) * startWidth)
				.tex(0, minVStart).endVertex();
		vertexbuffer.pos(start.x + (localSide.x - localUp.x) * startWidth,
				start.y + (localSide.y - localUp.y) * startWidth, start.z + (localSide.z - localUp.z) * startWidth)
				.tex(0, maxVStart).endVertex();
		vertexbuffer.pos(end.x + (localSide.x - localUp.x) * endWidth, end.y + (localSide.y - localUp.y) * endWidth,
				end.z + (localSide.z - localUp.z) * endWidth).tex(maxU, maxVEnd).endVertex();
		vertexbuffer.pos(end.x + (localSide.x + localUp.x) * endWidth, end.y + (localSide.y + localUp.y) * endWidth,
				end.z + (localSide.z + localUp.z) * endWidth).tex(maxU, minVEnd).endVertex();
		...
		vertexbuffer.pos(end.x + (localUp.x + localSide.x) * endWidth, end.y + (localUp.y + localSide.y) * endWidth,
				end.z + (localUp.z + localSide.z) * endWidth).tex(maxU, minVEnd).endVertex();
		vertexbuffer.pos(end.x + (localUp.x - localSide.x) * endWidth, end.y + (localUp.y - localSide.y) * endWidth,
				end.z + (localUp.z - localSide.z) * endWidth).tex(maxU, maxVEnd).endVertex();
		vertexbuffer.pos(start.x + (localUp.x - localSide.x) * startWidth,
				start.y + (localUp.y - localSide.y) * startWidth, start.z + (localUp.z - localSide.z) * startWidth)
				.tex(0, maxVStart).endVertex();
		vertexbuffer.pos(start.x + (localUp.x + localSide.x) * startWidth,
				start.y + (localUp.y + localSide.y) * startWidth, start.z + (localUp.z + localSide.z) * startWidth)
				.tex(0, minVStart).endVertex();
		...
		if (renderStartCap) {
			vertexbuffer.pos(start.x + (localUp.x - localSide.x) * startWidth,
					...
					.tex(0, 1).endVertex();
			vertexbuffer.pos(start.x + (-localUp.x - localSide.x) * startWidth,
					start.y + (-localUp.y - localSide.y) * startWidth,
					start.z + (-localUp.z - localSide.z) * startWidth).tex(1, 1).endVertex();
			vertexbuffer.pos(start.x + (-localUp.x + localSide.x) * startWidth,
					start.y + (-localUp.y + localSide.y) * startWidth,
					start.z + (-localUp.z + localSide.z) * startWidth).tex(1, 0).endVertex();
			vertexbuffer.pos(start.x + (localUp.x + localSide.x) * startWidth,
					start.y + (localUp.y + localSide.y) * startWidth, start.z + (localUp.z + localSide.z) * startWidth)
					.tex(0, 0).endVertex();
		}
		...
		if (renderEndCap) {
			vertexbuffer.pos(end.x + (localUp.x + localSide.x) * endWidth, end.y + (localUp.y + localSide.y) * endWidth,
					end.z + (localUp.z + localSide.z) * endWidth).tex(0, 0).endVertex();
			vertexbuffer.pos(end.x + (-localUp.x + localSide.x) * endWidth,
					end.y + (-localUp.y + localSide.y) * endWidth, end.z + (-localUp.z + localSide.z) * endWidth)
					.tex(1, 0).endVertex();
			vertexbuffer.pos(end.x + (-localUp.x - localSide.x) * endWidth,
					end.y + (-localUp.y - localSide.y) * endWidth, end.z + (-localUp.z - localSide.z) * endWidth)
					.tex(1, 1).endVertex();
			vertexbuffer.pos(end.x + (localUp.x - localSide.x) * endWidth, end.y + (localUp.y - localSide.y) * endWidth,
					end.z + (localUp.z - localSide.z) * endWidth).tex(0, 1).endVertex();
		}
		tessellator.draw();
	}
}