...
public class RenderPyrad extends RenderLiving<EntityPyrad> {
	...
	public RenderPyrad(RenderManager manager) {
		...
		this.addLayer(this.glow = new LayerOverlay<EntityPyrad>(this,
				new ResourceLocation("thebetweenlands:textures/entity/pyrad_glow.png")).setGlow(true));
	}
	...
	public void doRender(EntityPyrad entity, double x, double y, double z, float entityYaw, float partialTicks) {
		this.glow.setAlpha(entity.getActiveTicks(partialTicks) / 60.0F
				+ (!entity.isActive() ? entity.getHitTicks(partialTicks) / 20.0F * 0.45F : 0.0F));

		GlStateManager.pushMatrix();
		GlStateManager.translate(0,
				(Math.sin((entity.ticksExisted + partialTicks) / 10.0F) / 4.0F - 0.25D)
						* entity.getActiveTicks(partialTicks) / 60.0D
						- 0.9D * (1.0D - entity.getActiveTicks(partialTicks) / 60.0D),
				0);
		...
		if (!entity.isActive() && entity.getHitTicks(partialTicks) > 8 && ShaderHelper.INSTANCE.isWorldShaderActive()) {
			...
			ShaderHelper.INSTANCE.getWorldShader()
					.addLight(new LightSource(rx, ry + 0.5D, rz, 1.8f, 134.0f / 255.0f * 3.0F * brightness,
							214.0f / 255.0f * 3.0F * brightness, 55.0f / 255.0f * 3.0F * brightness));
		}
		...
		if (entity.getGlowTicks(partialTicks) > 0 && ShaderHelper.INSTANCE.isWorldShaderActive()) {
			...
			ShaderHelper.INSTANCE.getWorldShader()
					.addLight(new LightSource(rx, ry + entity.getEyeHeight(), rz, 4.75F * glow + 1.0F,
							134.0f / 255.0f * 10.0F * glow, 214.0f / 255.0f * 10.0F * glow,
