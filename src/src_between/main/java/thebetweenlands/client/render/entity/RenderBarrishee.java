...
public class RenderBarrishee extends RenderLiving<EntityBarrishee> {
	public static final ResourceLocation TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/entity/barrishee.png");
	public int fudge = 0;

	public RenderBarrishee(RenderManager renderManagerIn) {
		super(renderManagerIn, new ModelBarrishee(), 1F);
		addLayer(new LayerOverlay<EntityBarrishee>(this,
				new ResourceLocation("thebetweenlands:textures/entity/barrishee_face.png")).setGlow(true));
	}
		...
		if (entity.isAmbushSpawn() || entity.isSlamming())
			if (entity.isScreaming()) {
				GlStateManager.translate(0F,
						-0.5F + entity.standingAngle * 0.5F - getTimerFudge(entity) * 0.00625F - 0.0625F, 0F);
	...
	public void lightUpStuff(EntityBarrishee entity, float partialTickTime) {
		if (ShaderHelper.INSTANCE.isWorldShaderActive()) {
			...
			ShaderHelper.INSTANCE.getWorldShader()
					.addLight(new LightSource(entity.posX, entity.posY + 1.25D, entity.posZ,
							(fudge + partialTickTime) / 2F + 1F, 255f / 255.0f * 4.0F, 102f / 255.0f * 4.0F,
							0f / 255.0f * 4.0F));
			ShaderHelper.INSTANCE.getWorldShader()
					.addLight(new LightSource(entity.posX, entity.posY + 1.25D, entity.posZ,
							(getTimerFudge(entity) + partialTickTime) / 2F + 0.6F, 105f / 255.0f * 4.0F,
							26f / 255.0f * 4.0F, 0f / 255.0f * 4.0F));
		}
	}
