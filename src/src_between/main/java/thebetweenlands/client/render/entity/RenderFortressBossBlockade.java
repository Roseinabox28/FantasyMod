...
public class RenderFortressBossBlockade extends Render<EntityFortressBossBlockade> {
	...
	@Override
	public void doRender(EntityFortressBossBlockade entity, double x, double y, double z, float yaw,
			float partialTicks) {
		...
		for (int l = 0; l < layers; l++) {
			...
			tu3 = cu + tu3 * sin;
			tv3 = cv + tv3 * cos;
			buffer.pos(vertices[0].x, vertices[0].y, vertices[0].z).tex(tu1, tv1).color(0.5F, 0.6F, 1F, 0.5F)
					.endVertex();
			buffer.pos(vertices[1].x, vertices[1].y, vertices[1].z).tex(tu2, tv2).color(0.5F, 0.6F, 1F, 0.5F)
					.endVertex();
			buffer.pos(vertices[2].x, vertices[2].y, vertices[2].z).tex(tu3, tv3).color(0.5F, 0.6F, 1F, 0.5F)
					.endVertex();
		}
		tessellator.draw();
