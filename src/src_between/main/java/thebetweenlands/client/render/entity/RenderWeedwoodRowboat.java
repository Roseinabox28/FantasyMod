...
public class RenderWeedwoodRowboat extends Render<EntityWeedwoodRowboat> {
	private static final ResourceLocation TEXTURE = new ResourceLocation(ModInfo.ID,
			"textures/entity/weedwood_rowboat.png");

	private static final ResourceLocation TEXTURE_TARRED = new ResourceLocation(ModInfo.ID,
			"textures/entity/weedwood_rowboat_tarred.png");
	...
	private EnumMap<ShipSide, ArmArticulation> arms = ShipSide.newEnumMap(ArmArticulation.class, new ArmArticulation(),
			new ArmArticulation());
	...
	public void renderMultipass(EntityWeedwoodRowboat rowboat, double x, double y, double z, float yaw, float delta) {
		...
		/*
		 * proof of concept double bpx = rowboat.prevPosX + (rowboat.posX -
		 * rowboat.prevPosX) * delta; double bpy = rowboat.prevPosY + (rowboat.posY -
		 * rowboat.prevPosY) * delta; double bpz = rowboat.prevPosZ + (rowboat.posZ -
		 * rowboat.prevPosZ) * delta; int bx = MathHelper.floor_double(bpx); int by =
		 * MathHelper.floor_double(bpy); int bz = MathHelper.floor_double(bpz);
		 * OpenSimplexNoise waveRng = new OpenSimplexNoise(6354); TextureMap texturemap
		 * = Minecraft.getMinecraft().getTextureMapBlocks(); TextureAtlasSprite sprite =
		 * texturemap.getAtlasSprite("minecraft:blocks/water_still");
		 * bindTexture(TextureMap.LOCATION_BLOCKS_TEXTURE); GlStateManager.pushMatrix();
		 * float fracX = MathHelper.positiveModulo((float) bpx, 1); float fracZ =
		 * MathHelper.positiveModulo((float) bpz, 1); GlStateManager.translate(x -
		 * fracX, y - MathHelper.positiveModulo((float) bpy, 1) + 14.22 / 16, z -
		 * fracZ); GlStateManager.enableAlpha(); GlStateManager.enableBlend();
		 * GlStateManager.blendFunc(SourceFactor.SRC_ALPHA,
		 * DestFactor.ONE_MINUS_SRC_ALPHA); GlStateManager.glBegin(GL11.GL_TRIANGLES);
		 * float roughness = 0.15F, scale = 0.5F; double t =
		 * (rowboat.worldObj.getTotalWorldTime() + delta) * 0.03; int range = 4; //
		 * scale up waves to normal level from number of ticks the rowboat has existed
		 * for (int dx = -range; dx <= range; dx++) { for (int dz = -range; dz <= range;
		 * dz++) { float d0 = scale(dx, dz, fracX, fracZ, range); float d1 = scale(dx,
		 * dz + 1, fracX, fracZ, range); float d2 = scale(dx + 1, dz, fracX, fracZ,
		 * range); float d3 = scale(dx + 1, dz + 1, fracX, fracZ, range); float sy0 =
		 * (float) waveRng.eval((bx + dx) * scale, t, (bz + dz) * scale) * roughness *
		 * d0; float sy1 = (float) waveRng.eval((bx + dx) * scale, t, (bz + dz + 1) *
		 * scale) * roughness * d1; float sy2 = (float) waveRng.eval((bx + dx + 1) *
		 * scale, t, (bz + dz) * scale) * roughness * d2; float sy3 = (float)
		 * waveRng.eval((bx + dx + 1) * scale, t, (bz + dz + 1) * scale) * roughness *
		 * d3; float b0 = MathUtils.linearTransformf(sy0, -roughness, roughness, 0, 1);
		 ...
		 * float u0 = sprite.getInterpolatedU(0); float u1 =
		 * sprite.getInterpolatedU(16); float v0 = sprite.getInterpolatedV(0); float v1
		 * = sprite.getInterpolatedV(16); Vec3d s0 = new Vec3d(0, sy0, 0); Vec3d s1 =
		 * new Vec3d(0, sy1, 1); Vec3d s2 = new Vec3d(1, sy2, 0); Vec3d s3 = new
		 * Vec3d(1, sy3, 1); Vec3d normal0 =
		 * s3.subtract(s1).crossProduct(s0.subtract(s1)).normalize();
		 * GlStateManager.glNormal3f((float) normal0.xCoord, (float) normal0.yCoord,
		 * (float) normal0.zCoord); GlStateManager.glTexCoord2f(u0, v0);
		 * GlStateManager.glVertex3f(dx, sy0, dz); GlStateManager.glTexCoord2f(u0, v1);
		 * GlStateManager.glVertex3f(dx, sy1, dz + 1); GlStateManager.glTexCoord2f(u1,
		 * v1); GlStateManager.glVertex3f(dx + 1, sy3, dz + 1); Vec3d normal1 =
		 * s3.subtract(s1).crossProduct(s0.subtract(s1)).normalize();
		 * GlStateManager.glNormal3f((float) normal1.xCoord, (float) normal1.yCoord,
		 * (float) normal1.zCoord); GlStateManager.glTexCoord2f(u0, v0);
		 * GlStateManager.glVertex3f(dx, sy0, dz); GlStateManager.glTexCoord2f(u1, v1);
		 * GlStateManager.glVertex3f(dx + 1, sy3, dz + 1);
		 * GlStateManager.glTexCoord2f(u1, v0); GlStateManager.glVertex3f(dx + 1, sy2,
		 * dz); } } GlStateManager.glEnd(); GlStateManager.disableAlpha();
		 * GlStateManager.disableBlend(); GlStateManager.colorMask(false, false, false,
		 * false); GlStateManager.glBegin(GL11.GL_QUADS);
		 ...
		 * GlStateManager.glVertex3f(range + 1, 0, -range); GlStateManager.glEnd();
	...
	private void roll(EntityWeedwoodRowboat rowboat, float yaw, float delta) {
		...
		if (timeSinceHit > 0) {
			...
			GlStateManager.rotate(
					MathHelper.sin(timeSinceHit) * timeSinceHit * damageTaken / 10 * rowboat.getForwardDirection(), 0,
					0, 1);
			GlStateManager.translate(0, 1, 0);
		}
	}
	...
	public void onLivingRender(RenderPlayerEvent.Pre event) {
		...
		if (riding instanceof EntityWeedwoodRowboat && riding.getControllingPassenger() == e) {
			...
			render.renderPilot(player, arms.get(ShipSide.STARBOARD), arms.get(ShipSide.PORT), bodyRotateAngleX,
					bodyRotateAngleY, event.getX(), event.getY(), event.getZ(), delta);
		}
	}
	...
	private void articulateArm(ShipSide side, float yaw) {
		...
		float shoulderAngle = (float) Math
				.acos((upperArmLen * upperArmLen + targetLen * targetLen - lowerArmLen * lowerArmLen)
						/ (2 * upperArmLen * targetLen));
		float flexionAngle = (float) Math
				.acos((upperArmLen * upperArmLen + lowerArmLen * lowerArmLen - targetLen * targetLen)
						/ (2 * upperArmLen * lowerArmLen));
		ArmArticulation armArt = arms.get(side);
		// If shoulderAngle is NaN then the target is out of reach so the arm should
		// simply point in that direction.
		armArt.shoulderAngleX = (shoulderAngle != shoulderAngle ? -MathUtils.PI / 2 - targetPitch
				: (shoulderAngle - MathUtils.PI / 2 - targetPitch)) - bodyRotateAngleX;
