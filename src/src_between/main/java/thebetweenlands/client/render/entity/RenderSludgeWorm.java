...
public class RenderSludgeWorm<T extends EntitySludgeWorm> extends RenderLiving<T> {
	public static final ResourceLocation TEXTURE_HEAD = new ResourceLocation(
			"thebetweenlands:textures/entity/smol_sludge_worm_head.png");
	public static final ResourceLocation TEXTURE_BODY = new ResourceLocation(
			"thebetweenlands:textures/entity/smol_sludge_worm_body.png");
	...
	public void doRender(T entity, double x, double y, double z, float yaw, float partialTicks) {
		...
		for (int i = 1; i < entity.parts.length - 1; i++) {
			renderBodyPart(entity, entity.parts[i], entity.parts[i - 1], rx, ry, rz, i, avgWibbleStrength,
					zOffset -= 0.001F, partialTicks);
		}

		renderTailPart(entity, entity.parts[entity.parts.length - 1], entity.parts[entity.parts.length - 2], rx, ry, rz,
				entity.parts.length - 1, avgWibbleStrength, partialTicks);
	...
	private void renderHead(T entity, int frame, double x, double y, double z, float yaw, float avgWibbleStrength,
			float partialTicks) {
		double yawDiff = (yaw - entity.parts[1].rotationYaw) % 360.0F;
		double yawInterpolant = 2 * yawDiff % 360.0F - yawDiff;
		float wibbleStrength = Math.min(avgWibbleStrength,
				MathHelper.clamp(1.0F - (float) Math.abs(yawInterpolant) / 60.0F, 0, 1));
	...
	protected void renderBodyPart(T entity, MultiPartEntityPart part, MultiPartEntityPart prevPart, double rx,
			double ry, double rz, int frame, float avgWibbleStrength, float zOffset, float partialTicks) {
	...
	protected void renderTailPart(T entity, MultiPartEntityPart part, MultiPartEntityPart prevPart, double rx,
			double ry, double rz, int frame, float avgWibbleStrength, float partialTicks) {
		double x = part.lastTickPosX + (part.posX - part.lastTickPosX) * (double) partialTicks - rx;
		double y = part.lastTickPosY + (part.posY - part.lastTickPosY) * (double) partialTicks - ry;
		double z = part.lastTickPosZ + (part.posZ - part.lastTickPosZ) * (double) partialTicks - rz;
