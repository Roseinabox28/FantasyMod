...
public class RenderGrapplingHookNode extends Render<EntityGrapplingHookNode> {
	...
	protected static final ResourceLocation TEXTURE_ROPE = new ResourceLocation(
			"thebetweenlands:textures/entity/grappling_hook_rope.png");
	protected static final ResourceLocation TEXTURE_SHAMBLER = new ResourceLocation(
			"thebetweenlands:textures/entity/shambler.png");
	...
	public void doRender(EntityGrapplingHookNode entity, double x, double y, double z, float yaw, float partialTicks) {
		...
		if (this.getRenderManager().isDebugBoundingBox() && entity.isMountNode()) {
			...
			DebugHandlerClient.drawBoundingBox(
					new AxisAlignedBB(weightPos.x - 0.1D, weightPos.y - 0.1D, weightPos.z - 0.1D, weightPos.x + 0.1D,
							weightPos.y + 0.1D, weightPos.z + 0.1D).offset(-camPosX, -camPosY, -camPosZ));
		...
		if (prevNode instanceof EntityGrapplingHookNode) {
			if (!this.renderManager.getEntityRenderObject(prevNode).shouldRender(prevNode, this.frustum, camPosX,
					camPosY, camPosZ)) {
				// Previous node not rendered, render rope
				GlStateManager.pushMatrix();
				double renderOffsetX = interpolate(prevNode.lastTickPosX - entity.lastTickPosX,
						prevNode.posX - entity.posX, partialTicks);
				double renderOffsetY = interpolate(prevNode.lastTickPosY - entity.lastTickPosY,
						prevNode.posY - entity.posY, partialTicks);
				double renderOffsetZ = interpolate(prevNode.lastTickPosZ - entity.lastTickPosZ,
						prevNode.posZ - entity.posZ, partialTicks);
		...
		if (nextNode instanceof EntityGrapplingHookNode) {
			...
			double dx = interpolate(entity.lastTickPosX, entity.posX, partialTicks)
					- interpolate(nextNode.lastTickPosX, nextNode.posX, partialTicks);
			double dy = interpolate(entity.lastTickPosY, entity.posY, partialTicks)
					- interpolate(nextNode.lastTickPosY, nextNode.posY, partialTicks);
			double dz = interpolate(entity.lastTickPosZ, entity.posZ, partialTicks)
					- interpolate(nextNode.lastTickPosZ, nextNode.posZ, partialTicks);
	...
	protected void renderConnection(Entity node1, Entity node2, Tessellator tessellator, BufferBuilder buffer, double x,
		...
		if (node2 != null) {
			...
			if (node2 instanceof EntityGrapplingHookNode && ((EntityGrapplingHookNode) node2).isMountNode()
					&& node2.getControllingPassenger() != null) {
				Entity controller = node2.getControllingPassenger();

				double yaw;
				if (controller instanceof EntityLivingBase) {
					yaw = interpolate(((EntityLivingBase) controller).prevRenderYawOffset,
							((EntityLivingBase) controller).renderYawOffset, partialTicks);
				} else {
				...
				if (controller instanceof EntityLivingBase) {
					activeHand = !((EntityLivingBase) controller).getHeldItem(EnumHand.OFF_HAND).isEmpty()
							&& ((EntityLivingBase) controller).getHeldItem(EnumHand.OFF_HAND)
									.getItem() instanceof ItemGrapplingHook ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND;
				}
				...
				double yOffset = 0;
				if (controller instanceof EntityLivingBase
						&& !((EntityLivingBase) controller).getHeldItem(activeHand).isEmpty()) {
					yOffset += 0.2D;
				}
				...
				/*
				 * GlStateManager.rotate(-bodyYaw, 0, 1, 0); GlStateManager.translate(0.6D, 0,
				 * -0.4D); GlStateManager.rotate(bodyYaw, 0, 1, 0);
				 */
				/*
				 * GlStateManager.rotate(yaw, 0, 1, 0); GlStateManager.rotate(pitch, 0, 0, 1);
				...
				offset = offset.add(matrix.transformVec(
						new Vec3d(activeHand == EnumHand.MAIN_HAND ? 0.6D : -0.6D, 0, -0.4D), Vec3d.ZERO));
				...
				endX = interpolate(controller.lastTickPosX - camPosX, controller.posX - camPosX, partialTicks)
						+ offset.x;
				endY = interpolate(controller.lastTickPosY - camPosY, controller.posY - camPosY, partialTicks)
						+ offset.y;
				endZ = interpolate(controller.lastTickPosZ - camPosZ, controller.posZ - camPosZ, partialTicks)
						+ offset.z;
			}
			...
			ParticleBeam.buildBeam(x + diffX, y + diffY, z + diffZ, new Vec3d(-diffX, -diffY, -diffZ), 0.05F, 0, 2F,
					ActiveRenderInfo.getRotationX(), ActiveRenderInfo.getRotationZ(), ActiveRenderInfo.getRotationYZ(),
					ActiveRenderInfo.getRotationXY(), ActiveRenderInfo.getRotationXZ(), (vx, vy, vz, u, v) -> {
	...
	public static void onPlayerRenderPre(RenderPlayerEvent.Pre event) {
		...
		if (ridingEntity instanceof EntityGrapplingHookNode) {
			...
			if (prevNode != null) {
				...
				EnumHand activeHand = !player.getHeldItem(EnumHand.OFF_HAND).isEmpty()
						&& player.getHeldItem(EnumHand.OFF_HAND).getItem() instanceof ItemGrapplingHook
								? EnumHand.OFF_HAND
								: EnumHand.MAIN_HAND;
