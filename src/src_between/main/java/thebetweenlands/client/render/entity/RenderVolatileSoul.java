...
public class RenderVolatileSoul extends Render<EntityVolatileSoul> {
	...
	protected static final ResourceLocation TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/entity/wight_face.png");
	protected static final ResourceLocation TEXTURE_TRAIL = new ResourceLocation(
			"thebetweenlands:textures/entity/volatile_soul_trail.png");
	...
	public void doRender(EntityVolatileSoul entity, double x, double y, double z, float yaw, float partialTicks) {
		...
		buffer.pos((double) (x - rx * scale - ryz * scale), (double) (y - rxz * scale),
				(double) (z - rz * scale - rxy * scale)).tex((double) maxU, (double) maxV).color(255, 255, 255, 255)
				.endVertex();
		buffer.pos((double) (x - rx * scale + ryz * scale), (double) (y + rxz * scale),
				(double) (z - rz * scale + rxy * scale)).tex((double) maxU, (double) minV).color(255, 255, 255, 255)
				.endVertex();
		buffer.pos((double) (x + rx * scale + ryz * scale), (double) (y + rxz * scale),
				(double) (z + rz * scale + rxy * scale)).tex((double) minU, (double) minV).color(255, 255, 255, 255)
				.endVertex();
		buffer.pos((double) (x + rx * scale - ryz * scale), (double) (y - rxz * scale),
				(double) (z + rz * scale - rxy * scale)).tex((double) minU, (double) maxV).color(255, 255, 255, 255)
				.endVertex();
		...
		for (Vec3d trailPos : entity.getTrail()) {
			if (prevTrailPos != null) {
				for (int s = 0; s < interpSteps; s++) {
					...
					buffer.pos((double) (tx - rx * scale - ryz * scale), (double) (ty - rxz * scale),
							(double) (tz - rz * scale - rxy * scale)).tex((double) maxU, (double) maxV)
							.color(alpha, alpha, alpha, 1).endVertex();
					buffer.pos((double) (tx - rx * scale + ryz * scale), (double) (ty + rxz * scale),
							(double) (tz - rz * scale + rxy * scale)).tex((double) maxU, (double) minV)
							.color(alpha, alpha, alpha, 1).endVertex();
					buffer.pos((double) (tx + rx * scale + ryz * scale), (double) (ty + rxz * scale),
							(double) (tz + rz * scale + rxy * scale)).tex((double) minU, (double) minV)
							.color(alpha, alpha, alpha, 1).endVertex();
					buffer.pos((double) (tx + rx * scale - ryz * scale), (double) (ty - rxz * scale),
							(double) (tz + rz * scale - rxy * scale)).tex((double) minU, (double) maxV)
							.color(alpha, alpha, alpha, 1).endVertex();
					scale -= scaleDecr;
					alpha -= alphaDecr;
				}
