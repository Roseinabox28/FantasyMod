...
public class RenderLeech extends RenderLiving<EntityLeech> {
	private static final ResourceLocation texture = new ResourceLocation(
			"thebetweenlands:textures/entity/leech_hungry.png");
	...
	protected void preRenderCallback(EntityLeech leech, float partialTickTime) {
		if (!leech.isRiding())
			GlStateManager.scale(1 + leech.getBloodConsumed() * 0.1F, 1 + leech.getBloodConsumed() * 0.1F,
					(leech.moveProgress + 3F) / 3F);

		if (leech.isRiding()) {
			GlStateManager.scale(leech.moveProgress * leech.moveProgress / 2 + 0.5F,
					leech.moveProgress * leech.moveProgress / 2 + 0.5F, 1F);
