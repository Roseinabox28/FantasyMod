...
public class RenderAngler extends RenderLiving<EntityAngler> {
	...
	public RenderAngler(RenderManager manager) {
		...
		this.addLayer(new LayerOverlay<EntityAngler>(this,
				new ResourceLocation("thebetweenlands:textures/entity/angler_glow.png")).setGlow(true));
	}
	...
	public void doRender(EntityAngler entity, double x, double y, double z, float entityYaw, float partialTicks) {
		...
		if (ShaderHelper.INSTANCE.isWorldShaderActive()) {
			...
			ShaderHelper.INSTANCE.getWorldShader().addLight(new LightSource(rx + xOff, ry + 0.95f, rz + zOff, 2.6f,
					30.0f / 255.0f * 13.0F, 90.0f / 255.0f * 13.0F, 60.0f / 255.0f * 13.0F));
		}
	}
}
