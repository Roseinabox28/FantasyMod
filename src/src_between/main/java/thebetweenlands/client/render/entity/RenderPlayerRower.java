...
public class RenderPlayerRower extends RenderLivingBase<AbstractClientPlayer> {
	...
	public void renderPilot(AbstractClientPlayer player, ArmArticulation leftArm, ArmArticulation rightArm,
			float bodyRotateAngleX, float bodyRotateAngleY, double x, double y, double z, float delta) {
		...
		doRender(player, x, y, z, player.prevRotationYaw + (player.rotationYaw - player.prevRotationYaw) * delta,
				delta);
	}
	...
	@Override
	protected void renderEntityName(AbstractClientPlayer player, double x, double y, double z, String name,
			double distance) {
		if (distance < 100) {
