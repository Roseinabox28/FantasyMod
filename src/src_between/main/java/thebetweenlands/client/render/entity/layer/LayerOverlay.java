...
public class LayerOverlay<T extends EntityLivingBase> implements LayerRenderer<T> {
		...
		for (ModelBase model : this.getModels(entity)) {
			...
			if (texture != null) {
				...
				renderOverlay(entity, () -> {
					this.renderOverlay(entity, model, limbSwing, limbSwingAmount, partialTicks, ageInTicks, netHeadYaw,
							headPitch, scale);
	...
	public static void renderOverlay(@Nullable Entity entity, Runnable renderer, boolean glow, float red, float green,
	...
	protected void renderOverlay(T entity, ModelBase model, float limbSwing, float limbSwingAmount, float partialTicks,
