	...
	public void renderShockwaveBlock(EntityShockwaveBlock entity, double x, double y, double z, float yaw, float tick) {
		if (ShaderHelper.INSTANCE.isWorldShaderActive()) {
			...
			ShaderHelper.INSTANCE.getWorldShader()
					.addLight(new LightSource(entity.posX, entity.posY + 0.5D, entity.posZ,
							(entity.ticksExisted + tick) / 12.0F + 1F, 10.0f / 255.0f * 4.0F, 40.0f / 255.0f * 4.0F,
							160.0f / 255.0f * 4.0F));
			ShaderHelper.INSTANCE.getWorldShader().addLight(new LightSource(entity.posX, entity.posY + 0.5D,
					entity.posZ, (entity.ticksExisted + tick) / 35.0F + 0.6F, -3.4F, -3.4F, -3.4F));
		}
		...
		if (model != null) {
			...
			blockRenderer.setLighting((IBlockState blockState, @Nullable EnumFacing facing) -> {
				return state.getPackedLightmapCoords(entity.world,
						facing != null ? entity.origin.up().offset(facing) : entity.origin.up());
			}).setTint((IBlockState blockState, int tintIndex) -> {
				if (blockState.getBlock() == entity.block)
					return Minecraft.getMinecraft().getBlockColors().colorMultiplier(state, entity.world, entity.origin,
							tintIndex);
				else
					return Minecraft.getMinecraft().getBlockColors().colorMultiplier(state, null, null, tintIndex);
			});

			blockRenderer.renderModel(entity.world, entity.origin, model, state,
					MathHelper.getPositionRandom(entity.origin), buffer);
