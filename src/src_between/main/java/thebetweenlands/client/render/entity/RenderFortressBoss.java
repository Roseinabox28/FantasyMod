...
public class RenderFortressBoss extends Render<EntityFortressBoss> {
	...
	private static final ResourceLocation MODEL_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/entity/fortress_boss.png");
	...
	public void doRender(EntityFortressBoss entity, double x, double y, double z, float yaw, float partialTicks) {
		...
		if (ShaderHelper.INSTANCE.isWorldShaderActive()) {
			...
			if (boss.hurtResistantTime == 0) {
				...
				if (lightIntensity > 0.0F) {
					...
					shader.addLight(new LightSource(boss.posX, boss.posY, boss.posZ, 16.0F,
							3.4F / 4.0F * MathHelper.clamp(lightIntensity, 0.0F, 4.0F),
							0.0F / 4.0F * MathHelper.clamp(lightIntensity, 0.0F, 4.0F),
							3.6F / 4.0F * MathHelper.clamp(lightIntensity, 0.0F, 4.0F)));
				}
			} else {
				ShaderHelper.INSTANCE.require();
				shader.addLight(new LightSource(boss.posX, boss.posY, boss.posZ, 16.0F,
						1.5F / boss.maxHurtResistantTime * (boss.hurtResistantTime + partialTicks), 0, 0));
			}
		}

		/*
		 * if(this.getRenderManager().isDebugBoundingBox()) {
		 * GlStateManager.depthMask(false); GlStateManager.disableTexture2D();
		 * GlStateManager.disableCull(); GlStateManager.disableBlend();
		 * RenderGlobal.func_189697_a(boss.coreBoundingBox.offset(boss.posX - x,
		 * boss.posY - y, boss.posZ - z), 1, 1, 1, 1); GlStateManager.enableLighting();
		...
		GlStateManager.rotate(
				entity.prevRotationPitch + (entity.rotationPitch - entity.prevRotationPitch) * partialTicks, 1, 0, 0);
		GlStateManager.translate(0, -0.2D, 0);
		GlStateManager.translate(Math.sin((entity.ticksExisted + partialTicks) / 5.0D) * 0.1F,
				Math.cos((entity.ticksExisted + partialTicks) / 7.0D) * 0.1F,
				Math.cos((entity.ticksExisted + partialTicks) / 6.0D) * 0.1F);
		...
		GlStateManager.pushMatrix();
		GlStateManager.translate(x + EntityFortressBoss.SHIELD_OFFSET_X, y + EntityFortressBoss.SHIELD_OFFSET_Y,
				z + EntityFortressBoss.SHIELD_OFFSET_Z);
		...
		for (int i = 0; i <= 19; i++) {
			...
			if (shieldAnimationTicks > 0 && shieldAnimationTicks <= 20) {
				...
				g = 00.5F - 0.5F / 20 * (shieldAnimationTicks);
			...
			for (int l = 0; l < ((shieldAnimationTicks > 0 && shieldAnimationTicks <= 20) ? 4 : layers); l++) {
				...
				tu3 = cu + tu3 * sin;
				tv3 = cv + tv3 * cos;
				buffer.pos(v1[0] + centerX / len * explode, v1[1] + centerY / len * explode,
						v1[2] + centerZ / len * explode).tex(tu1, tv1).color(r, g, b, a).endVertex();
				buffer.pos(v2[0] + centerX / len * explode, v2[1] + centerY / len * explode,
						v2[2] + centerZ / len * explode).tex(tu2, tv2).color(r, g, b, a).endVertex();
				buffer.pos(v3[0] + centerX / len * explode, v3[1] + centerY / len * explode,
						v3[2] + centerZ / len * explode).tex(tu3, tv3).color(r, g, b, a).endVertex();
			}
		...
		for (int i = 0; i <= 19; i++) {
			...
			double len = Math.sqrt(centerX * centerX + centerY * centerY + centerZ * centerZ);
			buffer.pos(v1[0] + centerX / len * explode, v1[1] + centerY / len * explode,
					v1[2] + centerZ / len * explode).color(0.5F, 0.6F, 1F, 0.5F).endVertex();
			buffer.pos(v2[0] + centerX / len * explode, v2[1] + centerY / len * explode,
					v2[2] + centerZ / len * explode).color(0.5F, 0.6F, 1F, 0.5F).endVertex();
			buffer.pos(v3[0] + centerX / len * explode, v3[1] + centerY / len * explode,
					v3[2] + centerZ / len * explode).color(0.5F, 0.6F, 1F, 0.5F).endVertex();
		}
		tessellator.draw();
		...
		for (int p = 0; p < 2; p++) {
			...
			for (int i = 0; i <= 19; i++) {
				...
				buffer.pos(v1[0] + v1Normalized.x * vertexExplode, v1[1] + v1Normalized.y * vertexExplode,
						v1[2] + v1Normalized.z * vertexExplode).color(0.05F, 0.05F, 0.05F, 0.45F).endVertex();
				buffer.pos(v2[0] + v2Normalized.x * vertexExplode, v2[1] + v2Normalized.y * vertexExplode,
						v2[2] + v2Normalized.z * vertexExplode).color(0.05F, 0.05F, 0.05F, 0.45F).endVertex();
				buffer.pos(v3[0] + v3Normalized.x * vertexExplode, v3[1] + v3Normalized.y * vertexExplode,
						v3[2] + v3Normalized.z * vertexExplode).color(0.05F, 0.05F, 0.05F, 0.45F).endVertex();
			}
			tessellator.draw();
		}
		// }
		...
		if (this.getRenderManager().isDebugBoundingBox()) {
			...
			for (int i = 0; i <= 19; i++) {
				...
				double centerZ = (v1[2] + v2[2] + v3[2]) / 3;
				double len = Math.sqrt(centerX * centerX + centerY * centerY + centerZ * centerZ);
				Vec3d vec1 = new Vec3d(v1[0] + centerX / len * explode, v1[1] + centerY / len * explode,
						v1[2] + centerZ / len * explode);
				Vec3d vec2 = new Vec3d(v2[0] + centerX / len * explode, v2[1] + centerY / len * explode,
						v2[2] + centerZ / len * explode);
				Vec3d vec3 = new Vec3d(v3[0] + centerX / len * explode, v3[1] + centerY / len * explode,
						v3[2] + centerZ / len * explode);
				Vec3d normal = vec2.subtract(vec1).crossProduct(vec3.subtract(vec1));
				buffer.pos(centerX + centerX / len * explode, centerY + centerY / len * explode,
						centerZ + centerZ / len * explode).color(0.8F, 0.0F, 1F, 0.5F).endVertex();
				buffer.pos(normal.x + centerX + centerX / len * explode, normal.y + centerY + centerY / len * explode,
						normal.z + centerZ + centerZ / len * explode).color(0.8F, 0.0F, 1F, 0.5F).endVertex();
			}
			tessellator.draw();
		}
		...
		for (int i = 0; i <= 19; i++) {
			...
			double len = Math.sqrt(centerX * centerX + centerY * centerY + centerZ * centerZ);
			buffer.pos(v1[0] + centerX / len * explode, v1[1] + centerY / len * explode,
					v1[2] + centerZ / len * explode).color(0.5F, 0.75F, 1F, 1.0F).endVertex();
			buffer.pos(v2[0] + centerX / len * explode, v2[1] + centerY / len * explode,
					v2[2] + centerZ / len * explode).color(0.5F, 0.75F, 1F, 1.0F).endVertex();
			buffer.pos(v2[0] + centerX / len * explode, v2[1] + centerY / len * explode,
					v2[2] + centerZ / len * explode).color(0.5F, 0.75F, 1F, 1.0F).endVertex();
			buffer.pos(v3[0] + centerX / len * explode, v3[1] + centerY / len * explode,
					v3[2] + centerZ / len * explode).color(0.5F, 0.75F, 1F, 1.0F).endVertex();
			buffer.pos(v3[0] + centerX / len * explode, v3[1] + centerY / len * explode,
					v3[2] + centerZ / len * explode).color(0.5F, 0.75F, 1F, 1.0F).endVertex();
			buffer.pos(v1[0] + centerX / len * explode, v1[1] + centerY / len * explode,
					v1[2] + centerZ / len * explode).color(0.5F, 0.75F, 1F, 1.0F).endVertex();
		...
		if (this.getRenderManager().isDebugBoundingBox()) {
			...
			if (hitShield >= 0) {
				...
				buffer.begin(GL11.GL_TRIANGLES, DefaultVertexFormats.POSITION_COLOR);
				buffer.pos(v1[0] + centerX / len * explode, v1[1] + centerY / len * explode,
						v1[2] + centerZ / len * explode).color(0F, 0F, 0F, 1.0F).endVertex();
				buffer.pos(v2[0] + centerX / len * explode, v2[1] + centerY / len * explode,
						v2[2] + centerZ / len * explode).color(0F, 0F, 0F, 1.0F).endVertex();
				buffer.pos(v3[0] + centerX / len * explode, v3[1] + centerY / len * explode,
						v3[2] + centerZ / len * explode).color(0F, 0F, 0F, 1.0F).endVertex();
				tessellator.draw();
			}
			GlStateManager.enableCull();
			GlStateManager.blendFunc(SourceFactor.SRC_ALPHA, DestFactor.ONE);
		...
		if (boss.getGroundAttackTicks() > 0) {
			...
			GlStateManager.color(0.8F / 20.0F * boss.getGroundAttackTicks(), 0.6F / 20.0F * boss.getGroundAttackTicks(),
					0.4F / 20.0F * boss.getGroundAttackTicks(), 1.0F);
