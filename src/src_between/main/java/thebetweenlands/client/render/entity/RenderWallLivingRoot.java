...
public class RenderWallLivingRoot extends RenderWallHole<EntityWallLivingRoot> {
	private static final ResourceLocation ROOT_TEXTURE = new ResourceLocation(ModInfo.ID,
			"textures/blocks/root_middle.png");
	...
	@Override
	protected void renderEntityModel(EntityWallLivingRoot entity, float limbSwing, float limbSwingAmount,
			float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, float partialTicks) {
		if (this.mainModel == BLANK_MODEL) {
			this.renderRootModel(entity, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scaleFactor,
					partialTicks);
		} else {
			super.renderEntityModel(entity, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scaleFactor,
					partialTicks);
		}
	}

	protected void renderRootModel(EntityWallLivingRoot entity, float limbSwing, float limbSwingAmount,
			float ageInTicks, float netHeadYaw, float headPitch, float scale, float partialTicks) {
