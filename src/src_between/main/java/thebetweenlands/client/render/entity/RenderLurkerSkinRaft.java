...
public class RenderLurkerSkinRaft extends Render<EntityBoat> {
	private static final ResourceLocation TEXTURE = new ResourceLocation(ModInfo.ID,
			"textures/entity/lurker_skin_raft.png");
	...
	public void setupRotation(EntityBoat entity, float yaw, float partialTicks) {
		...
		if (wobbleTimer > 0.0F) {
			GlStateManager.rotate(MathHelper.sin(wobbleTimer) * wobbleTimer * wobbleStrength / 10.0F
					* (float) entity.getForwardDirection(), 1.0F, 0.0F, 0.0F);
		}
