...
public class RenderFirefly extends RenderLiving<EntityFirefly> {
	public static final ResourceLocation GLOW_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/entity/firefly_glow.png");
	public static final ResourceLocation TEXTURE = new ResourceLocation("thebetweenlands:textures/entity/firefly.png");
	public static final ResourceLocation TEXTURE_OVERLAY = new ResourceLocation(
			"thebetweenlands:textures/entity/firefly_glow_overlay.png");
	...
	public void doRender(EntityFirefly entity, double x, double y, double z, float yaw, float partialTicks) {
		if (MinecraftForgeClient.getRenderPass() == 0) {
			double glowStrength = (float) entity.getEntityAttribute(EntityFirefly.GLOW_STRENGTH_ATTRIB)
					.getAttributeValue();
			...
			if (ShaderHelper.INSTANCE.isWorldShaderActive()) {
				...
				if (radius > 0.1F) {
					...
					ShaderHelper.INSTANCE.getWorldShader().addLight(new LightSource(interpX, interpY, interpZ,
							...
							12.0f / 255.0f * 60.0F + entity.world.rand.nextFloat() * 0.1f, 8.0f / 255.0f * 60.0F));
				}
			}
		} else {
	...
	public void renderFireflyGlow(EntityFirefly entity, double x, double y, double z, float partialTicks) {
		...
		for (int i = 0; i < 10; i++) {
			...
			buffer.pos(ipx - rx * currentScale - ryz * currentScale, ipy - rxz * currentScale,
					ipz - rz * currentScale - rxy * currentScale).tex(0.0, 1.0).color(red, green, blue, alpha)
					.endVertex();
			buffer.pos(ipx - rx * currentScale + ryz * currentScale, ipy + rxz * currentScale,
					ipz - rz * currentScale + rxy * currentScale).tex(1.0D, 1.0D).color(red, green, blue, alpha)
					.endVertex();
			buffer.pos(ipx + rx * currentScale + ryz * currentScale, ipy + rxz * currentScale,
					ipz + rz * currentScale + rxy * currentScale).tex(1.0D, 0.0D).color(red, green, blue, alpha)
					.endVertex();
			buffer.pos(ipx + rx * currentScale - ryz * currentScale, ipy - rxz * currentScale,
					ipz + rz * currentScale - rxy * currentScale).tex(0.0D, 0.0D).color(red, green, blue, alpha)
					.endVertex();
		}
		...
		for (int i = 0; i < 10; i++) {
			...
			buffer.pos(ipx - rx * currentScale - ryz * currentScale, ipy - rxz * currentScale,
					ipz - rz * currentScale - rxy * currentScale).tex(0.0D, 1.0D).color(red, green, blue, alpha)
					.endVertex();
			buffer.pos(ipx - rx * currentScale + ryz * currentScale, ipy + rxz * currentScale,
					ipz - rz * currentScale + rxy * currentScale).tex(1.0D, 1.0D).color(red, green, blue, alpha)
					.endVertex();
