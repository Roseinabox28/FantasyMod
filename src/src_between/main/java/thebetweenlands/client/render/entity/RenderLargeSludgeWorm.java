...
public class RenderLargeSludgeWorm extends RenderLiving<EntityLargeSludgeWorm> {
	public static final ResourceLocation MODEL_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/entity/large_sludge_worm.png");
	public static final ResourceLocation HULL_TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/entity/large_sludge_worm_hull.png");
	public static final ResourceLocation EGG_SAC_TEXTURE = new ResourceLocation(ModInfo.ID,
			"textures/entity/worm_egg_sac.png");
	...
	@Override
	public void renderMultipass(EntityLargeSludgeWorm entity, double x, double y, double z, float yaw,
			float partialTicks) {
		this.renderPass(entity, x, y, z, yaw, partialTicks, true);
	}

	protected void renderPass(EntityLargeSludgeWorm entity, double x, double y, double z, float yaw, float partialTicks,
			boolean isMultiPass) {
		...
		for (HullSegment segment2 : entity.segments) {
			...
			if (segment1 != null) {
				...
				for (int vertIndex = 0; vertIndex < hullVerts; vertIndex++) {
					...
					float contraction1 = this.calculateHullContraction(entity,
							(i - 1) / (float) (entity.segments.length - 1), partialTicks);
					float contraction2 = this.calculateHullContraction(entity, i / (float) (entity.segments.length - 1),
							partialTicks);
	...
	protected float calculateHullContraction(EntityLargeSludgeWorm entity, float percent, float partialTicks) {
		...
		float contraction = ((float) Math
				.sin(percent * entity.spineySpliney.getArcLength() * 4 - (entity.ticksExisted + partialTicks) * 0.25F)
				+ 1.0F) / 2.0F * 0.2F + 0.8F;
	...
	protected void renderSpine(EntityLargeSludgeWorm entity, float partialTicks) {
		...
		for (SpineBone bone : entity.bones) {
			...
			GlStateManager.pushMatrix();
			GlStateManager.translate(x, y + Math.sin(-(entity.ticksExisted + partialTicks) * 0.25F + i * 0.2F) * 0.05F,
					z);
