...
public class RenderRopeNode extends Render<EntityRopeNode> {
	...
	protected static final ResourceLocation TEXTURE = new ResourceLocation(
			"thebetweenlands:textures/blocks/bulb_capped_mushroom.png");
	...
	public void doRender(EntityRopeNode ropeNode, double x, double y, double z, float yaw, float partialTicks) {
		...
		AxisAlignedBB boundingBox = ropeNode.getEntityBoundingBox().offset(-ropeNode.posX, -ropeNode.posY,
				-ropeNode.posZ);
		...
		if (ShaderHelper.INSTANCE.isWorldShaderActive()) {
			...
			if (dstSq <= 256.0D) {
				...
				ShaderHelper.INSTANCE.getWorldShader().addLight(
						new LightSource(rx, ry, rz, 1.25F, 1.5F * brightness, 2.0F * brightness, 4.0F * brightness));
			}
		}
		...
		if (prevNode != null) {
			if (!this.renderManager.getEntityRenderObject(prevNode).shouldRender(prevNode, this.frustum, camPosX,
				...
				GlStateManager.pushMatrix();
				double renderOffsetX = this.interpolate(prevNode.lastTickPosX - ropeNode.lastTickPosX,
						prevNode.posX - ropeNode.posX, partialTicks);
				double renderOffsetY = this.interpolate(prevNode.lastTickPosY - ropeNode.lastTickPosY,
						prevNode.posY - ropeNode.posY, partialTicks);
				double renderOffsetZ = this.interpolate(prevNode.lastTickPosZ - ropeNode.lastTickPosZ,
						prevNode.posZ - ropeNode.posZ, partialTicks);
	...
	protected void renderConnection(Entity node1, Entity node2, Tessellator tessellator, BufferBuilder buffer, double x,
		...
		if (node2 != null) {
			...
			for (int i = 0; i <= 24; ++i) {
				...
				buffer.pos(x + diffX * (double) percentage + 0.0D, y + diffY * (double) (yMult + percentage) * 0.5D,
						z + diffZ * (double) percentage).color(r, g, b, 1).endVertex();
				buffer.pos(x + diffX * (double) percentage + 0.025D,
						y + diffY * (double) (yMult + percentage) * 0.5D + 0.025D, z + diffZ * (double) percentage)
			...
			for (int i = 0; i <= 24; ++i) {
				...
				buffer.pos(x + diffX * (double) percentage + 0.0D,
						y + diffY * (double) (yMult + percentage) * 0.5D + 0.025D, z + diffZ * (double) percentage)
						.color(r, g, b, 1).endVertex();
				buffer.pos(x + diffX * (double) percentage + 0.025D, y + diffY * (double) (yMult + percentage) * 0.5D,
						z + diffZ * (double) percentage + 0.025D).color(r, g, b, 1).endVertex();
			}
			tessellator.draw();
