...
public abstract class RenderWallHole<T extends EntityWallFace> extends RenderWallFace<T> {
	...
	private static final ResourceLocation WALL_TEXTURE_OVERLAY = new ResourceLocation(ModInfo.ID,
			"textures/entity/wall_hole_overlay.png");

	private static final ResourceLocation WALL_TEXTURE = new ResourceLocation(ModInfo.ID,
	...
	public RenderWallHole(RenderManager renderManager, ModelBase model, ResourceLocation modelTexture) {
		...
		this.addLayer(new LayerOverlay<T>(this) {
			...
			@Override
			protected void renderOverlay(T entity, ModelBase model, float limbSwing, float limbSwingAmount,
					float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
				this.preRenderDamagedBlocks();
				super.renderOverlay(entity, model, limbSwing, limbSwingAmount, partialTicks, ageInTicks, netHeadYaw,
						headPitch, scale);
	...
	@Override
	protected void renderModel(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw,
		...
		if (wallSprite != null) {
			...
			GlStateManager.scale(wallSprite.getMaxU() - wallSprite.getMinU(),
					wallSprite.getMaxV() - wallSprite.getMinV(), 1);
			GlStateManager.matrixMode(GL11.GL_MODELVIEW);
		}

		this.renderEntityModel(entity, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scaleFactor,
				this.partialTicks);
	...
	protected void renderEntityModel(T entity, float limbSwing, float limbSwingAmount, float ageInTicks,
	...
	protected void applyDeathAndEasterEggRotations(T entity) {
		...
		if (entity.deathTime > 0) {
			...
			GlStateManager.rotate(deathPercent * this.getDeathMaxRotation(entity)
					* this.getMainModelVisibilityPercent(entity, this.partialTicks), 0.0F, 0.0F, 1.0F);
		} else {
