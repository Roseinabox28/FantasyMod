...
public class RingOfDispersionWorldRenderer {
	...
	protected VertexBatchRenderer getBatchRenderer(BlockRenderLayer layer) {
		...
		if (renderer == null) {
			this.batchRenderers.put(layer,
					renderer = new VertexBatchRenderer(DefaultVertexFormats.BLOCK, OpenGlHelper.useVbo()));
		}
		return renderer;
	}
	...
	public void render() {
		...
		GlStateManager.translate(this.renderedPos.getX() - mc.getRenderManager().renderPosX,
				this.renderedPos.getY() - mc.getRenderManager().renderPosY,
				this.renderedPos.getZ() - mc.getRenderManager().renderPosZ);
	...
	protected void renderBuffers(Minecraft mc) {
		...
		mc.getTextureManager().getTexture(TextureMap.LOCATION_BLOCKS_TEXTURE).setBlurMipmap(false,
				mc.gameSettings.mipmapLevels > 0); // FORGE: fix flickering leaves when mods mess up the blurMipmap
													// settings
			...
			for (int yo = -this.rangeY; yo <= 0; yo++) {
				for (int zo = -this.rangeXZ; zo <= this.rangeXZ; zo++) {
					...
					if (!state.getBlock().isAir(state, this.world, pos)
							&& (renderType == EnumBlockRenderType.MODEL || renderType == EnumBlockRenderType.LIQUID)) {
						...
						if (blockModel != null) {
							for (BlockRenderLayer layer : BlockRenderLayer.values()) {
								if (state.getBlock().canRenderInLayer(state, layer)) {
									...
									bufferBuilder.setTranslation(-this.renderedPos.getX(), -this.renderedPos.getY(),
											-this.renderedPos.getZ());

									switch (renderType) {
									case MODEL:
										blockRenderer.getBlockModelRenderer().renderModel(this.world, blockModel, state,
												pos, bufferBuilder, layer == BlockRenderLayer.TRANSLUCENT);
										break;
									case LIQUID:
