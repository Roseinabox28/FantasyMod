...
public class IsolatedBlockModelRenderer {
	...
	public static interface AmbientOcclusionProvider {
		 ...
		 * @return
		 */
		public AmbientOcclusionFace getAmbientOcclusion(IBlockState state, @Nullable EnumFacing facing,
	 ...
	 * @return
	 */
	public boolean renderModel(IBlockAccess world, BlockPos pos, IBakedModel model, IBlockState state, long rand,
			BufferBuilder buffer) {
		@SuppressWarnings("deprecation")
		boolean useAO = Minecraft.isAmbientOcclusionEnabled() && state.getLightValue() == 0
				&& model.isAmbientOcclusion() && this.ao != null;

		try {
			return useAO ? this.renderModelSmooth(world, pos, model, state, buffer, rand)
					: this.renderModelFlat(world, pos, model, state, buffer, rand);
	...
	private boolean renderModelSmooth(IBlockAccess world, BlockPos pos, IBakedModel model, IBlockState state,
			BufferBuilder buffer, long rand) {
		boolean flag = false;
		 ...
		 * 
		 * if (!list1.isEmpty()) { this.renderQuadsSmooth(pos, state, buffer, list1,
		 * blockBounds, bitset); flag = true; }
		 */

		return flag;
	}

	private boolean renderModelFlat(IBlockAccess world, BlockPos pos, IBakedModel model, IBlockState state,
			BufferBuilder buffer, long rand) {
		boolean flag = false;
		BitSet bitset = new BitSet(3);
		 ...
		 * List<BakedQuad> list1 = model.getQuads(state, (EnumFacing)null, rand);
		 * 
		 * if (!list1.isEmpty()) { this.renderQuadsFlat(state, pos, -1, true, buffer,
		 * list1, bitset, lighting); flag = true; }
		 */

		return flag;
	}

	private void renderQuadsSmooth(IBlockAccess world, BlockPos pos, IBlockState state, BufferBuilder vertexBuffer,
			List<BakedQuad> quads, float[] blockBounds, BitSet blockBoundsState) {
		double blockX = 0.0D;
		double blockY = 0.0D;
		double blockZ = 0.0D;
		...
		for (int j = quads.size(); l < j; ++l) {
			...
			vertexBuffer.putBrightness4(aoFace.vertexBrightness[0], aoFace.vertexBrightness[1],
					aoFace.vertexBrightness[2], aoFace.vertexBrightness[3]);
			...
			if (bakedQuad.hasTintIndex()) {
				...
				vertexBuffer.putColorMultiplier(aoFace.vertexColorMultiplier[0] * tintRed,
						aoFace.vertexColorMultiplier[0] * tintGreen, aoFace.vertexColorMultiplier[0] * tintBlue, 4);
				vertexBuffer.putColorMultiplier(aoFace.vertexColorMultiplier[1] * tintRed,
						aoFace.vertexColorMultiplier[1] * tintGreen, aoFace.vertexColorMultiplier[1] * tintBlue, 3);
				vertexBuffer.putColorMultiplier(aoFace.vertexColorMultiplier[2] * tintRed,
						aoFace.vertexColorMultiplier[2] * tintGreen, aoFace.vertexColorMultiplier[2] * tintBlue, 2);
				vertexBuffer.putColorMultiplier(aoFace.vertexColorMultiplier[3] * tintRed,
						aoFace.vertexColorMultiplier[3] * tintGreen, aoFace.vertexColorMultiplier[3] * tintBlue, 1);
			} else {
				vertexBuffer.putColorMultiplier(aoFace.vertexColorMultiplier[0], aoFace.vertexColorMultiplier[0],
						aoFace.vertexColorMultiplier[0], 4);
				vertexBuffer.putColorMultiplier(aoFace.vertexColorMultiplier[1], aoFace.vertexColorMultiplier[1],
						aoFace.vertexColorMultiplier[1], 3);
				vertexBuffer.putColorMultiplier(aoFace.vertexColorMultiplier[2], aoFace.vertexColorMultiplier[2],
						aoFace.vertexColorMultiplier[2], 2);
				vertexBuffer.putColorMultiplier(aoFace.vertexColorMultiplier[3], aoFace.vertexColorMultiplier[3],
	...
	private void renderQuadsFlat(IBlockAccess world, IBlockState state, BlockPos pos, int brightness,
			boolean updateBrightness, BufferBuilder vertexBuffer, List<BakedQuad> quads, BitSet blockBoundsState,
			@Nullable LightingProvider lighting) {
		double blockX = 0.0D;
		double blockY = 0.0D;
		double blockZ = 0.0D;
		...
		for (int i1 = quads.size(); l < i1; ++l) {
			...
			if (updateBrightness) {
				this.fillQuadBounds(state, bakedQuad.getVertexData(), bakedQuad.getFace(), (float[]) null,
						blockBoundsState);
				brightness = lighting.getPackedLightmapCoords(state,
						blockBoundsState.get(0) ? bakedQuad.getFace() : null);
			}
	...
	private void fillQuadBounds(IBlockState state, int[] vertexData, EnumFacing facing, @Nullable float[] blockBounds,
			BitSet blockBoundsState) {
		float f = 32.0F;
		float f1 = 32.0F;
		float f2 = 32.0F;
		float f3 = -32.0F;
	...
	public static class AmbientOcclusionFace {
		...
		public void updateVertexBrightness(IBlockAccess worldIn, IBlockState state, BlockPos centerPos,
			...
			IsolatedBlockModelRenderer.EnumNeighborInfo blockmodelrenderer$enumneighborinfo = IsolatedBlockModelRenderer.EnumNeighborInfo
			...
			float f8 = shapeState.get(0) ? worldIn.getBlockState(blockpos).getAmbientOcclusionLightValue()
					: worldIn.getBlockState(centerPos).getAmbientOcclusionLightValue();
			IsolatedBlockModelRenderer.VertexTranslations blockmodelrenderer$vertextranslations = IsolatedBlockModelRenderer.VertexTranslations
		...
		protected int getVertexBrightness(int p_178203_1_, int p_178203_2_, int p_178203_3_, int p_178203_4_,
	...
	public static enum EnumNeighborInfo {
		DOWN(new EnumFacing[] { EnumFacing.WEST, EnumFacing.EAST, EnumFacing.NORTH, EnumFacing.SOUTH }, 0.5F, true,
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.FLIP_WEST,
						IsolatedBlockModelRenderer.Orientation.SOUTH, IsolatedBlockModelRenderer.Orientation.FLIP_WEST,
						IsolatedBlockModelRenderer.Orientation.FLIP_SOUTH, IsolatedBlockModelRenderer.Orientation.WEST,
						IsolatedBlockModelRenderer.Orientation.FLIP_SOUTH, IsolatedBlockModelRenderer.Orientation.WEST,
						IsolatedBlockModelRenderer.Orientation.SOUTH },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.FLIP_WEST,
						IsolatedBlockModelRenderer.Orientation.NORTH, IsolatedBlockModelRenderer.Orientation.FLIP_WEST,
						IsolatedBlockModelRenderer.Orientation.FLIP_NORTH, IsolatedBlockModelRenderer.Orientation.WEST,
						IsolatedBlockModelRenderer.Orientation.FLIP_NORTH, IsolatedBlockModelRenderer.Orientation.WEST,
						IsolatedBlockModelRenderer.Orientation.NORTH },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.FLIP_EAST,
						IsolatedBlockModelRenderer.Orientation.NORTH, IsolatedBlockModelRenderer.Orientation.FLIP_EAST,
						IsolatedBlockModelRenderer.Orientation.FLIP_NORTH, IsolatedBlockModelRenderer.Orientation.EAST,
						IsolatedBlockModelRenderer.Orientation.FLIP_NORTH, IsolatedBlockModelRenderer.Orientation.EAST,
						IsolatedBlockModelRenderer.Orientation.NORTH },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.FLIP_EAST,
						IsolatedBlockModelRenderer.Orientation.SOUTH, IsolatedBlockModelRenderer.Orientation.FLIP_EAST,
						IsolatedBlockModelRenderer.Orientation.FLIP_SOUTH, IsolatedBlockModelRenderer.Orientation.EAST,
						IsolatedBlockModelRenderer.Orientation.FLIP_SOUTH, IsolatedBlockModelRenderer.Orientation.EAST,
						IsolatedBlockModelRenderer.Orientation.SOUTH }),
		UP(new EnumFacing[] { EnumFacing.EAST, EnumFacing.WEST, EnumFacing.NORTH, EnumFacing.SOUTH }, 1.0F, true,
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.EAST,
						IsolatedBlockModelRenderer.Orientation.SOUTH, IsolatedBlockModelRenderer.Orientation.EAST,
						IsolatedBlockModelRenderer.Orientation.FLIP_SOUTH,
						IsolatedBlockModelRenderer.Orientation.FLIP_EAST,
						IsolatedBlockModelRenderer.Orientation.FLIP_SOUTH,
						IsolatedBlockModelRenderer.Orientation.FLIP_EAST,
						IsolatedBlockModelRenderer.Orientation.SOUTH },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.EAST,
						IsolatedBlockModelRenderer.Orientation.NORTH, IsolatedBlockModelRenderer.Orientation.EAST,
						IsolatedBlockModelRenderer.Orientation.FLIP_NORTH,
						IsolatedBlockModelRenderer.Orientation.FLIP_EAST,
						IsolatedBlockModelRenderer.Orientation.FLIP_NORTH,
						IsolatedBlockModelRenderer.Orientation.FLIP_EAST,
						IsolatedBlockModelRenderer.Orientation.NORTH },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.WEST,
						IsolatedBlockModelRenderer.Orientation.NORTH, IsolatedBlockModelRenderer.Orientation.WEST,
						IsolatedBlockModelRenderer.Orientation.FLIP_NORTH,
						IsolatedBlockModelRenderer.Orientation.FLIP_WEST,
						IsolatedBlockModelRenderer.Orientation.FLIP_NORTH,
						IsolatedBlockModelRenderer.Orientation.FLIP_WEST,
						IsolatedBlockModelRenderer.Orientation.NORTH },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.WEST,
						IsolatedBlockModelRenderer.Orientation.SOUTH, IsolatedBlockModelRenderer.Orientation.WEST,
						IsolatedBlockModelRenderer.Orientation.FLIP_SOUTH,
						IsolatedBlockModelRenderer.Orientation.FLIP_WEST,
						IsolatedBlockModelRenderer.Orientation.FLIP_SOUTH,
						IsolatedBlockModelRenderer.Orientation.FLIP_WEST,
		...
		NORTH(new EnumFacing[] { EnumFacing.UP, EnumFacing.DOWN, EnumFacing.EAST, EnumFacing.WEST }, 0.8F, true,
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.UP,
						IsolatedBlockModelRenderer.Orientation.FLIP_WEST, IsolatedBlockModelRenderer.Orientation.UP,
						IsolatedBlockModelRenderer.Orientation.WEST, IsolatedBlockModelRenderer.Orientation.FLIP_UP,
						IsolatedBlockModelRenderer.Orientation.WEST, IsolatedBlockModelRenderer.Orientation.FLIP_UP,
						IsolatedBlockModelRenderer.Orientation.FLIP_WEST },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.UP,
						IsolatedBlockModelRenderer.Orientation.FLIP_EAST, IsolatedBlockModelRenderer.Orientation.UP,
						IsolatedBlockModelRenderer.Orientation.EAST, IsolatedBlockModelRenderer.Orientation.FLIP_UP,
						IsolatedBlockModelRenderer.Orientation.EAST, IsolatedBlockModelRenderer.Orientation.FLIP_UP,
						IsolatedBlockModelRenderer.Orientation.FLIP_EAST },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.DOWN,
						IsolatedBlockModelRenderer.Orientation.FLIP_EAST, IsolatedBlockModelRenderer.Orientation.DOWN,
						IsolatedBlockModelRenderer.Orientation.EAST, IsolatedBlockModelRenderer.Orientation.FLIP_DOWN,
						IsolatedBlockModelRenderer.Orientation.EAST, IsolatedBlockModelRenderer.Orientation.FLIP_DOWN,
						IsolatedBlockModelRenderer.Orientation.FLIP_EAST },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.DOWN,
						IsolatedBlockModelRenderer.Orientation.FLIP_WEST, IsolatedBlockModelRenderer.Orientation.DOWN,
						IsolatedBlockModelRenderer.Orientation.WEST, IsolatedBlockModelRenderer.Orientation.FLIP_DOWN,
						IsolatedBlockModelRenderer.Orientation.WEST, IsolatedBlockModelRenderer.Orientation.FLIP_DOWN,
						IsolatedBlockModelRenderer.Orientation.FLIP_WEST }),
		SOUTH(new EnumFacing[] { EnumFacing.WEST, EnumFacing.EAST, EnumFacing.DOWN, EnumFacing.UP }, 0.8F, true,
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.UP,
						IsolatedBlockModelRenderer.Orientation.FLIP_WEST,
						IsolatedBlockModelRenderer.Orientation.FLIP_UP,
						IsolatedBlockModelRenderer.Orientation.FLIP_WEST,
						IsolatedBlockModelRenderer.Orientation.FLIP_UP, IsolatedBlockModelRenderer.Orientation.WEST,
						IsolatedBlockModelRenderer.Orientation.UP, IsolatedBlockModelRenderer.Orientation.WEST },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.DOWN,
						IsolatedBlockModelRenderer.Orientation.FLIP_WEST,
						IsolatedBlockModelRenderer.Orientation.FLIP_DOWN,
						IsolatedBlockModelRenderer.Orientation.FLIP_WEST,
						IsolatedBlockModelRenderer.Orientation.FLIP_DOWN, IsolatedBlockModelRenderer.Orientation.WEST,
						IsolatedBlockModelRenderer.Orientation.DOWN, IsolatedBlockModelRenderer.Orientation.WEST },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.DOWN,
						IsolatedBlockModelRenderer.Orientation.FLIP_EAST,
						IsolatedBlockModelRenderer.Orientation.FLIP_DOWN,
						IsolatedBlockModelRenderer.Orientation.FLIP_EAST,
						IsolatedBlockModelRenderer.Orientation.FLIP_DOWN, IsolatedBlockModelRenderer.Orientation.EAST,
						IsolatedBlockModelRenderer.Orientation.DOWN, IsolatedBlockModelRenderer.Orientation.EAST },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.UP,
						IsolatedBlockModelRenderer.Orientation.FLIP_EAST,
						IsolatedBlockModelRenderer.Orientation.FLIP_UP,
						IsolatedBlockModelRenderer.Orientation.FLIP_EAST,
						IsolatedBlockModelRenderer.Orientation.FLIP_UP, IsolatedBlockModelRenderer.Orientation.EAST,
						IsolatedBlockModelRenderer.Orientation.UP, IsolatedBlockModelRenderer.Orientation.EAST }),
		WEST(new EnumFacing[] { EnumFacing.UP, EnumFacing.DOWN, EnumFacing.NORTH, EnumFacing.SOUTH }, 0.6F, true,
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.UP,
						...
						IsolatedBlockModelRenderer.Orientation.FLIP_UP,
						IsolatedBlockModelRenderer.Orientation.FLIP_SOUTH,
						IsolatedBlockModelRenderer.Orientation.FLIP_UP, IsolatedBlockModelRenderer.Orientation.SOUTH },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.UP,
						IsolatedBlockModelRenderer.Orientation.NORTH, IsolatedBlockModelRenderer.Orientation.UP,
						IsolatedBlockModelRenderer.Orientation.FLIP_NORTH,
						IsolatedBlockModelRenderer.Orientation.FLIP_UP,
						IsolatedBlockModelRenderer.Orientation.FLIP_NORTH,
						IsolatedBlockModelRenderer.Orientation.FLIP_UP, IsolatedBlockModelRenderer.Orientation.NORTH },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.DOWN,
		...
		EAST(new EnumFacing[] { EnumFacing.DOWN, EnumFacing.UP, EnumFacing.NORTH, EnumFacing.SOUTH }, 0.6F, true,
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.FLIP_DOWN,
						IsolatedBlockModelRenderer.Orientation.SOUTH, IsolatedBlockModelRenderer.Orientation.FLIP_DOWN,
						IsolatedBlockModelRenderer.Orientation.FLIP_SOUTH, IsolatedBlockModelRenderer.Orientation.DOWN,
						IsolatedBlockModelRenderer.Orientation.FLIP_SOUTH, IsolatedBlockModelRenderer.Orientation.DOWN,
						IsolatedBlockModelRenderer.Orientation.SOUTH },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.FLIP_DOWN,
						IsolatedBlockModelRenderer.Orientation.NORTH, IsolatedBlockModelRenderer.Orientation.FLIP_DOWN,
						IsolatedBlockModelRenderer.Orientation.FLIP_NORTH, IsolatedBlockModelRenderer.Orientation.DOWN,
						IsolatedBlockModelRenderer.Orientation.FLIP_NORTH, IsolatedBlockModelRenderer.Orientation.DOWN,
						IsolatedBlockModelRenderer.Orientation.NORTH },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.FLIP_UP,
						IsolatedBlockModelRenderer.Orientation.NORTH, IsolatedBlockModelRenderer.Orientation.FLIP_UP,
						IsolatedBlockModelRenderer.Orientation.FLIP_NORTH, IsolatedBlockModelRenderer.Orientation.UP,
						IsolatedBlockModelRenderer.Orientation.FLIP_NORTH, IsolatedBlockModelRenderer.Orientation.UP,
						IsolatedBlockModelRenderer.Orientation.NORTH },
				new IsolatedBlockModelRenderer.Orientation[] { IsolatedBlockModelRenderer.Orientation.FLIP_UP,
						IsolatedBlockModelRenderer.Orientation.SOUTH, IsolatedBlockModelRenderer.Orientation.FLIP_UP,
						IsolatedBlockModelRenderer.Orientation.FLIP_SOUTH, IsolatedBlockModelRenderer.Orientation.UP,
						IsolatedBlockModelRenderer.Orientation.FLIP_SOUTH, IsolatedBlockModelRenderer.Orientation.UP,
						IsolatedBlockModelRenderer.Orientation.SOUTH });
		...
		private EnumNeighborInfo(EnumFacing[] p_i46236_3_, float p_i46236_4_, boolean p_i46236_5_,
				IsolatedBlockModelRenderer.Orientation[] p_i46236_6_,
				IsolatedBlockModelRenderer.Orientation[] p_i46236_7_,
				IsolatedBlockModelRenderer.Orientation[] p_i46236_8_,
				IsolatedBlockModelRenderer.Orientation[] p_i46236_9_) {
