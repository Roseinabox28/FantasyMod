...
public class ModelDungeonDoorRunes extends ModelBase {
	...
	public void render(TileEntityDungeonDoorRunes tile, float scale, float partialTicks) {
		top.rotateAngleX = 0F + (tile.lastTickTopRotate + (tile.top_rotate - tile.lastTickTopRotate) * partialTicks)
				/ (180F / (float) Math.PI);
		mid.rotateAngleX = 0F + (tile.lastTickMidRotate + (tile.mid_rotate - tile.lastTickMidRotate) * partialTicks)
				/ (180F / (float) Math.PI);
		bottom.rotateAngleX = 0F
				+ (tile.lastTickBottomRotate + (tile.bottom_rotate - tile.lastTickBottomRotate) * partialTicks)
						/ (180F / (float) Math.PI);

		if (tile.isMimic()) {
			slate1.rotateAngleX = 0F + (tile.last_tick_slate_1_rotate
					+ (tile.slate_1_rotate - tile.last_tick_slate_1_rotate) * partialTicks) / (180F / (float) Math.PI);
			slate2.rotateAngleX = 0F + (tile.last_tick_slate_2_rotate
					+ (tile.slate_2_rotate - tile.last_tick_slate_2_rotate) * partialTicks) / (180F / (float) Math.PI);
			slate3.rotateAngleX = 0F + (tile.last_tick_slate_3_rotate
					+ (tile.slate_3_rotate - tile.last_tick_slate_3_rotate) * partialTicks) / (180F / (float) Math.PI);
		}
		if (!tile.isMimic()) {
		...
		if (!tile.hide_slate_1) {
			if (!tile.isMimic()) {
				...
				GlStateManager
						.translate(
								0F, 0F + 0.1375F
										* (tile.last_tick_slate_1_rotate
												+ (tile.slate_1_rotate - tile.last_tick_slate_1_rotate) * partialTicks)
										* scale,
								0F + 0.275F
										* (tile.last_tick_recess_pos
												+ (tile.recess_pos - tile.last_tick_recess_pos) * partialTicks)
										* scale);
				slate1.render(scale);
		...
		if (!tile.hide_slate_2) {
			if (!tile.isMimic()) {
				...
				GlStateManager
						.translate(
								0F, 0F + 0.1375F
										* (tile.last_tick_slate_2_rotate
												+ (tile.slate_2_rotate - tile.last_tick_slate_2_rotate) * partialTicks)
										* scale,
								0F + 0.275F
										* (tile.last_tick_recess_pos
												+ (tile.recess_pos - tile.last_tick_recess_pos) * partialTicks)
		...
		if (!tile.hide_slate_3) {
			if (!tile.isMimic()) {
				...
				GlStateManager
						.translate(
								0F, 0F + 0.1375F
										* (tile.last_tick_slate_3_rotate
												+ (tile.slate_3_rotate - tile.last_tick_slate_3_rotate) * partialTicks)
										* scale,
								0F + 0.275F
										* (tile.last_tick_recess_pos
												+ (tile.recess_pos - tile.last_tick_recess_pos) * partialTicks)
		...
		if (!tile.isMimic() && tile.is_in_dungeon) {
			if (tile.animate_tile_recess) {
				...
				GlStateManager
						.translate(0F, 0F,
								0F + 0.275F * (tile.last_tick_recess_pos_tile_1
										+ (tile.tile_1_recess_pos - tile.last_tick_recess_pos_tile_1) * partialTicks)
										* scale);
				...
				GlStateManager
						.translate(0F, 0F,
								0F + 0.275F * (tile.last_tick_recess_pos_tile_2
										+ (tile.tile_2_recess_pos - tile.last_tick_recess_pos_tile_2) * partialTicks)
										* scale);
				...
				GlStateManager
						.translate(0F, 0F,
								0F + 0.275F * (tile.last_tick_recess_pos_tile_3
										+ (tile.tile_3_recess_pos - tile.last_tick_recess_pos_tile_3) * partialTicks)
										* scale);
