...
public class ModelWallHole extends ModelBase {
	public static class BlockTexturedModelRenderer extends ModelRenderer {
		...
		@Override
		public ModelRenderer addBox(String partName, float offX, float offY, float offZ, int width, int height,
				int depth) {
			partName = this.boxName + "." + partName;
			TextureOffset textureoffset = this.baseModel.getTextureOffset(partName);
			this.setTextureOffset(textureoffset.textureOffsetX, textureoffset.textureOffsetY);
			this.cubeList.add((new BlockTexturedModelBox(this, this.textureOffsetX, this.textureOffsetY, offX, offY,
					offZ, width, height, depth, 0.0F)).setBoxName(partName));
			return this;
		}
		...
		public ModelRenderer addBox(float offX, float offY, float offZ, int width, int height, int depth,
				boolean mirrored) {
			this.cubeList.add(new BlockTexturedModelBox(this, this.textureOffsetX, this.textureOffsetY, offX, offY,
					offZ, width, height, depth, 0.0F, mirrored));
			return this;
		}
		...
		public void addBox(float offX, float offY, float offZ, int width, int height, int depth, float scaleFactor) {
			this.cubeList.add(new BlockTexturedModelBox(this, this.textureOffsetX, this.textureOffsetY, offX, offY,
					offZ, width, height, depth, scaleFactor));
		}
	}
	...
	public static class BlockTexturedModelBox extends ModelBox {
		...
		public BlockTexturedModelBox(BlockTexturedModelRenderer renderer, int texU, int texV, float x, float y, float z,
				int dx, int dy, int dz, float delta) {
		...
		public BlockTexturedModelBox(BlockTexturedModelRenderer renderer, int texU, int texV, float x, float y, float z,
			...
			this.quadList[0] = new TexturedQuad(
					new PositionTextureVertex[] { new PositionTextureVertex(v4, 0, 0),
							new PositionTextureVertex(v0, 0, 0), new PositionTextureVertex(v1, 0, 0),
							new PositionTextureVertex(v5, 0, 0) },
					0, 0, 0, 0, renderer.textureWidth, renderer.textureHeight);
			this.quadList[1] = new TexturedQuad(
					new PositionTextureVertex[] { new PositionTextureVertex(v7, 0, 0),
							new PositionTextureVertex(v3, 0, 0), new PositionTextureVertex(v6, 0, 0),
							new PositionTextureVertex(v2, 0, 0) },
					0, 0, 0, 0, renderer.textureWidth, renderer.textureHeight);
			this.quadList[2] = new TexturedQuad(
					new PositionTextureVertex[] { new PositionTextureVertex(v4, 0, 0),
							new PositionTextureVertex(v3, 0, 0), new PositionTextureVertex(v7, 0, 0),
							new PositionTextureVertex(v0, 0, 0) },
					0, 0, 0, 0, renderer.textureWidth, renderer.textureHeight);
			this.quadList[3] = new TexturedQuad(
					new PositionTextureVertex[] { new PositionTextureVertex(v1, 0, 0),
							new PositionTextureVertex(v2, 0, 0), new PositionTextureVertex(v6, 0, 0),
							new PositionTextureVertex(v5, 0, 0) },
					0, 0, 0, 0, renderer.textureWidth, renderer.textureHeight);
			this.quadList[4] = new TexturedQuad(
					new PositionTextureVertex[] { new PositionTextureVertex(v0, 0, 0),
							new PositionTextureVertex(v7, 0, 0), new PositionTextureVertex(v2, 0, 0),
							new PositionTextureVertex(v1, 0, 0) },
					0, 0, 0, 0, renderer.textureWidth, renderer.textureHeight);
			this.quadList[5] = new TexturedQuad(
					new PositionTextureVertex[] { new PositionTextureVertex(v3, 0, 0),
							new PositionTextureVertex(v4, 0, 0), new PositionTextureVertex(v5, 0, 0),
							new PositionTextureVertex(v6, 0, 0) },
					0, 0, 0, 0, renderer.textureWidth, renderer.textureHeight);
		...
		private void setVertexBlockUVs(TexturedQuad quad, BlockTexturedModelRenderer renderer, BlockPlane plane) {
			for (int i = 0; i < quad.vertexPositions.length; i++) {
				...
				quad.vertexPositions[i] = vertex.setTexturePosition((float) up / renderer.textureWidth,
						(float) vp / renderer.textureHeight);
			}
		}
	...
	private ModelRenderer createModelRenderer(ModelBase model, int texOffX, int texOffY, boolean root,
			boolean blockTextured) {
		return blockTextured ? new BlockTexturedModelRenderer(model, texOffX, texOffY, root)
				: new ModelRenderer(model, texOffX, texOffY);
	}
