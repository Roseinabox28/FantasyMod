...
public class ModelProcessorLoaderExtension extends LoaderExtension {
	...
	public IModel loadModel(IModel original, ResourceLocation location, String arg) {
		...
		if (json.has("custom")) {
			...
			ImmutableMap<String, String> dataMap = parseJsonElementList(
					JsonUtils.getJsonObject(json.get("custom"), "custom"));
		...
		if (json.has("smooth_lighting")) {
			...
			processedModel = processedModel
					.smoothLighting(JsonUtils.getBoolean(json.get("smooth_lighting"), "smooth_lighting"));
		}
		...
		if (json.has("textures")) {
			...
			ImmutableMap<String, String> dataMap = parseJsonElementList(
					JsonUtils.getJsonObject(json.get("textures"), "textures"));
