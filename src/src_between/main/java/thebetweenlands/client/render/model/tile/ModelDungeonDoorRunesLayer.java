...
public class ModelDungeonDoorRunesLayer extends ModelBase {
	...
	public void renderTopOverlay(TileEntity tile, ResourceLocation glow, int ticks, float scale, float partialTicks) {
		if (tile instanceof TileEntityDungeonDoorRunes) {
			...
			top_overlay.rotateAngleX = 0F
					+ (tileDoor.lastTickTopRotate + (tileDoor.top_rotate - tileDoor.lastTickTopRotate) * partialTicks)
							/ (180F / (float) Math.PI);
	...
	public void renderMidOverlay(TileEntity tile, ResourceLocation glow, int ticks, float scale, float partialTicks) {
		if (tile instanceof TileEntityDungeonDoorRunes) {
			...
			if (tileDoor.hide_lock)
				return;
			mid_overlay.rotateAngleX = 0F
					+ (tileDoor.lastTickMidRotate + (tileDoor.mid_rotate - tileDoor.lastTickMidRotate) * partialTicks)
							/ (180F / (float) Math.PI);
		} else {
	...
	public void renderBottomOverlay(TileEntity tile, ResourceLocation glow, int ticks, float scale,
			float partialTicks) {
		if (tile instanceof TileEntityDungeonDoorRunes) {
			TileEntityDungeonDoorRunes tileDoor = (TileEntityDungeonDoorRunes) tile;
			if (tileDoor.hide_lock)
				return;
			bottom_overlay.rotateAngleX = 0F + (tileDoor.lastTickBottomRotate
					+ (tileDoor.bottom_rotate - tileDoor.lastTickBottomRotate) * partialTicks)
					/ (180F / (float) Math.PI);
		} else {
	...
	private void renderRuneGlow(ModelRenderer box, int ticks, float scale, float partialTicks) {
		...
		for (int i = 0; i < passes; i++) {
			...
			GlStateManager.color(1, 1, 1,
					0.3f + (float) (Math.sin(renderTicks / 10.0f + i * Math.PI * 2.0f / passes) + 1) / 2.0f * 0.3f);
