...
public class ModelWeedwoodShieldBurning extends ModelFromModelBase {
	...
	public ModelWeedwoodShieldBurning(TexturePacker packer) {
		super(packer, new ModelWeedwoodShield(), new ResourceLocation("thebetweenlands:items/shields/weedwood_shield"),
				new ResourceLocation("thebetweenlands:particle/item/weedwood_shield_particle"), 64, 64,
				ModelRegistry.SHIELD_VERTEX_PROCESSOR);
	...
	public IBakedModel bake(IModelState state, VertexFormat format,
		...
		return new ModelBakedWeedwoodShieldBurning(this.vertexProcessor, state.apply(Optional.empty()), map, format,
				this.convertedModel, bakedTextureGetter, bakedTextureGetter.apply(this.texture),
				bakedTextureGetter.apply(FIRE_TEXTURE_LOCATION), this.width, this.height);
	}
	...
	public static class ModelBakedWeedwoodShieldBurning extends ModelBakedModelBase {
		protected ModelBakedWeedwoodShieldBurning(IVertexProcessor vertexProcessor,
				...
				VertexFormat format, Model convertedModel,
				Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter, TextureAtlasSprite particleTexture,
				TextureAtlasSprite flameSprite, int width, int height) {
			super(vertexProcessor, transformation, transforms, format, convertedModel, bakedTextureGetter,
					particleTexture, width, height, true);
