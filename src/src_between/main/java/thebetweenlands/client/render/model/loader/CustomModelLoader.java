...
public final class CustomModelLoader implements ICustomModelLoader {
			...
			if (result.type != LoaderType.NORMAL) {
				if (BetweenlandsConfig.DEBUG.debugModelLoader)
					TheBetweenlands.logger.info(
							String.format("Accepting model %s (full path: %s) through loader extension %s with args %s",
									result.actualLocation, modelLocation, result.extension.getName(), result.args));
				return true;
			}
		}

		// Check for registered model providers
		for (Entry<ResourceLocation, Function<ResourceLocation, IModel>> entry : this.manager
				.getRegisteredModelProviders().entrySet()) {
			ResourceLocation registeredModel = entry.getKey();
			if (registeredModel.getNamespace().equals(modelLocation.getNamespace())
					&& modelLocation.getPath().startsWith(registeredModel.getPath())) {
				...
				if (suffix.length() == 0 || suffix.startsWith("#")) {
					if (BetweenlandsConfig.DEBUG.debugModelLoader)
						TheBetweenlands.logger.info(String.format("Accepting model %s as %s through model registry",
								modelLocation, registeredModel));
					return true;
				}
			}
		}
	...
	public IModel loadModel(ResourceLocation modelLocation) throws Exception {
		...
		if (modelLocation.getPath().contains("$")) {
			...
			if (result.type == LoaderType.EXTENSION) {
				...
				try {
					if (BetweenlandsConfig.DEBUG.debugModelLoader)
						TheBetweenlands.logger.info(String.format(
								"Loading model %s (full path: %s) through loader extension %s with args %s",
								result.actualLocation, modelLocation, result.extension.getName(), result.args));
			...
			for (Entry<ResourceLocation, Function<ResourceLocation, IModel>> entry : this.manager
				...
				if (registeredModel.getNamespace().equals(modelLocation.getNamespace())
					...
					if (suffix.length() == 0 || suffix.startsWith("#")) {
						accepted = true;
						if (BetweenlandsConfig.DEBUG.debugModelLoader)
							TheBetweenlands.logger.info(String.format("Loading model %s as %s through model registry",
									modelLocation, registeredModel));
		...
		for (TileEntitySpecialRenderer<?> renderer : TileEntityRendererDispatcher.instance.renderers.values()) {
			if (renderer instanceof IFastTESRBakedModels) {
				...
				for (ModelResourceLocation location : locations) {
					IModel model = ModelLoaderRegistry.getModelOrLogError(location,
							"Failed loading model '" + location + "' for FastTESR");
		...
		for (TileEntitySpecialRenderer<?> renderer : TileEntityRendererDispatcher.instance.renderers.values()) {
			if (renderer instanceof IFastTESRBakedModels) {
				...
				for (ModelResourceLocation location : locations) {
					IModel model = ModelLoaderRegistry.getModelOrLogError(location,
							"Failed loading model '" + location + "' for FastTESR");
					IBakedModel bakedModel = model.bake(model.getDefaultState(), DefaultVertexFormats.BLOCK,
		...
		for (ModelResourceLocation modelLocation : modelRegistry.getKeys()) {
			...
			if (model instanceof IBakedModelDependant) {
				...
				for (ModelResourceLocation dependencyLocation : dependencies) {
					...
					if (bakedModel == null) {
						ResourceLocation dependencyLocationNoVariants = new ResourceLocation(
								dependencyLocation.getNamespace(), dependencyLocation.getPath());
						try {
							IModel externalModel = ModelLoaderRegistry.getModel(dependencyLocationNoVariants);
							bakedModel = externalModel.bake(dependant.getModelState(externalModel),
									dependant.getVertexFormat(externalModel),
									dependant.getTextureGetter(externalModel));
							loadedModels.add(Pair.of(dependencyLocation, bakedModel));
						} catch (Exception ex) {
							throw new RuntimeException("Failed to load model dependency " + dependencyLocationNoVariants
									+ " for model " + modelLocation, ex);
						}
					}
			...
			if (loadedModel.getValue() != null) {
				if (BetweenlandsConfig.DEBUG.debugModelLoader)
					TheBetweenlands.logger
							.info(String.format("Registering additional baked model %s", loadedModel.getKey()));
				modelRegistry.putObject(loadedModel.getKey(), loadedModel.getValue());
			} else {
				if (BetweenlandsConfig.DEBUG.debugModelLoader)
					TheBetweenlands.logger
							.warn(String.format("Additional baked model %s is null!", loadedModel.getKey()));
			}
		}
	...
	private void throwLoaderException(LoaderExtension extension, Throwable cause) {
		throw new LoaderExtensionException(
				String.format("Model loader extension %s failed loading a model", extension.getName()), cause);
	}
	...
	private <K, T> void replaceRegistryObjects(IRegistry<K, T> registry, Map<K, T> map) {
		...
		/*
		 * Map<T, K> objectsToRemove = new HashMap<T, K>(map.size()); Set<K>
		 * replacementKeys = map.keySet();
		 * 
		 * //Gather registered objects for(K replacementKey : replacementKeys) {
		 * if(ConfigHandler.DEBUG.debugModelLoader)
		 * TheBetweenlands.logger.info(String.format("Searching model for %s",
		 * replacementKey)); T obj = registry.getObject(replacementKey); if(obj != null)
		 * { if(ConfigHandler.DEBUG.debugModelLoader)
		 * TheBetweenlands.logger.info(String.format("Found model %s", obj));
		 * objectsToRemove.put(obj, replacementKey); } }
		 * 
		 * //Remove registered objects Iterator<T> it = registry.iterator(); T obj =
		 * null; while(it.hasNext()) { obj = it.next();
		 * if(objectsToRemove.containsKey(obj)) {
		 * if(ConfigHandler.DEBUG.debugModelLoader) TheBetweenlands.logger.info(String.
		 * format("Removed model %s, %s from registry", obj, objectsToRemove.get(obj)));
		 * it.remove(); } }
		 * 
		 * //Add replacement objects for(Entry<K, T> replacement : map.entrySet()) {
		 * if(replacement.getValue() != null) { if(ConfigHandler.DEBUG.debugModelLoader)
		 * TheBetweenlands.logger.info(String.format("Replaced model %s",
		 * replacement.getKey())); registry.putObject(replacement.getKey(),
		 * replacement.getValue()); } else { if(ConfigHandler.DEBUG.debugModelLoader)
		 * TheBetweenlands.logger.info(String.format("Removed model %s",
		 * replacement.getKey())); } }
		 */
	}
}