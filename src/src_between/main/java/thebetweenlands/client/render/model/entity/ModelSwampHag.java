...
public class ModelSwampHag extends MowzieModelBase {
	...
	public ModelSwampHag() {
		...
		parts = new MowzieModelRenderer[] { body_base, neck, body_top, toadstool1, mushroomstem, armright, legright1,
				legleft1, body_inner, vines2, toadstool1b, toadstool2, toadstool3, vines1, toadstool2b, mushroomhat,
				mushroomhat2, legright2, legleft2, head1, head2, jaw, toadstool4, brain, modelCore };
		setInitPose();
	}
...
//        limbSwingAngle = 0.4F;
		...
		float limpSwing = (float) ((-(inverseFrequency * Math.sin((2 * limbSwing - 1) / inverseFrequency)
				- 4 * limbSwing + Math.PI) / 4) - 0.5 * Math.PI);

		limbSwing = limpSwing;

//        bob(modelCore, 1 * globalSpeed, 1 * globalDegree, false, limbSwing, limbSwingAngle);
		walk(body_base, 1F * globalSpeed, 0.1F * globalDegree, true, 0, 0.05F * globalDegree, limbSwing,
				limbSwingAngle);
		...
		walk(body_top, 1F * globalSpeed, 0.1F * globalDegree, true, -1f, 0.05F * globalDegree, limbSwing,
				limbSwingAngle);
		...
		walk(armright, 1 * globalSpeed, 0.2f * globalDegree, true, -1f, -0.3f * globalDegree, limbSwing,
				limbSwingAngle);
