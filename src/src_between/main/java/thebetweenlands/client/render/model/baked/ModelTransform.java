			...
			if (!part.isPresent()) {
				if (transformation.isPresent()) {
					transformation = Optional
							.of(transformation.get().compose(TRSRTransformation.blockCenterToCorner(this.transform)));
				} else {
	...
	public IModel process(ImmutableMap<String, String> customData) {
		...
		if (customData.containsKey("transformed_model") || transformedModel == null) {
			ResourceLocation baseModelLocation = new ResourceLocation(
					JsonUtils.getString(parser.parse(customData.get("transformed_model")), "transformed_model"));
			transformedModel = ModelLoaderRegistry.getModelOrLogError(baseModelLocation,
					"Could not find model for transform model");
		}
