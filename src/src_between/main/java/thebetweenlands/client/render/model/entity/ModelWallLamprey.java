...
/**
 * BLWallLamprey - TripleHeadedSheep Created using Tabula 7.0.1
 */
public class ModelWallLamprey extends MowzieModelBase {
	...
	public ModelWallLamprey() {
		...
		parts = new MowzieModelRenderer[] { body_base, head_base, head_mid, eye_left, eye_right, head_front,
				fancyflap_left1, fancyflap_right1, mouthpart_top, teeth4, nostril_left1, nostril_right1,
				mouthpart_bottom, mouthpart_left, mouthpart_right, teeth1, teeth2, teeth3, mouthpart_corner1,
				mouthpart_corner2, mouthpart_corner3, mouthpart_corner4, nostril_left2, nostril_left3, nostril_right2,
				nostril_right3 };
	...
	public void setLivingAnimations(EntityLivingBase entity, float limbSwing, float limbSwingAmount,
		...
		relYaw = this.easeOutCubic(Math.min(Math.PI * 0.7F, Math.abs(relYaw)) / (Math.PI * 0.7F)) * 0.61F
				* Math.signum(relYaw) / 4.0F;
		relPitch = this.easeOutCubic(Math.min(Math.PI * 0.7F, Math.abs(relPitch)) / (Math.PI * 0.7F)) * 0.61F
				* Math.signum(relPitch) / 4.0F;
