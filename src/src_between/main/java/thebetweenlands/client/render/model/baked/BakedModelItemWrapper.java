...
public class BakedModelItemWrapper implements IBakedModel {
	...
	/**
	 * Creates a baked model that returns the quads of quadModel and the transforms
	 * and item overrides list of transformsModel.
	 * 
	 * @param transformsModel The model with the transforms and item overrides list
	 *                        to be used
	...
	/**
	 * Sets whether override models returned by
	 * {@link ItemOverrideList#handleItemState(IBakedModel, ItemStack, World, EntityLivingBase)}
	 * should be cached
	 * 
	 * @param cached
	 * @return
	...
	private static final class WrappedItemOverrideList extends ItemOverrideList {
		...
		public IBakedModel handleItemState(IBakedModel originalModel, ItemStack stack, World world,
			...
			// Make sure that the correct baked model is passed in here, so that if the
			// object is cast later on it doesn't cause issues
			...
			if (newModel instanceof BakedModelItemWrapper == false) {
				...
				if (wrapper.shouldCacheOverrideModels) {
					...
					if (cachedModel == null)
						this.cachedModels.put(newModel,
								cachedModel = new BakedModelItemWrapper(wrapper.transformsModel, newModel)
										.setInheritOverrides(wrapper.shouldInheritOverrides));
					return cachedModel;
				}
				return new BakedModelItemWrapper(wrapper.transformsModel, newModel)
						.setInheritOverrides(wrapper.shouldInheritOverrides);
			}
			return newModel;
		}
