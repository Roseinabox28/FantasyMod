...
public class ModelWeedwoodBush implements IModel {
	...
	public Collection<ResourceLocation> getTextures() {
		return Collections.unmodifiableCollection(
				Arrays.asList(new ResourceLocation[] { this.leavesTexture, this.sticksTexture }));
	}
	...
	public IBakedModel bake(IModelState state, VertexFormat format,
		...
		return new ModelBakedWeedwoodBush(format, state.apply(Optional.empty()), map,
				bakedTextureGetter.apply(this.leavesTexture), bakedTextureGetter.apply(this.sticksTexture),
				this.leavesTintIndex);
	}
	...
	public ModelWeedwoodBush process(ImmutableMap<String, String> customData) {
		...
		if (customData.containsKey("texture_leaves")) {
			leaves = new ResourceLocation(
					JsonUtils.getString(parser.parse(customData.get("texture_leaves")), "texture_leaves"));
		}
		...
		if (customData.containsKey("texture_sticks")) {
			sticks = new ResourceLocation(
					JsonUtils.getString(parser.parse(customData.get("texture_sticks")), "texture_sticks"));
		}
	...
	public static class ModelBakedWeedwoodBush implements IBakedModel {
		...
		private final LoadingCache<Long, ModelBakedWeedwoodBush> modelCache = CacheBuilder.newBuilder().maximumSize(128)
				.build(new CacheLoader<Long, ModelBakedWeedwoodBush>() {
					@Override
					public ModelBakedWeedwoodBush load(Long key) throws Exception {
						return new ModelBakedWeedwoodBush(format, Optional.ofNullable(transformation), transforms,
								textureLeaves, textureSticks, leavesTintIndex, key);
					}
				});

		private ModelBakedWeedwoodBush(VertexFormat format, Optional<TRSRTransformation> transformation,
				ImmutableMap<TransformType, TRSRTransformation> transforms, TextureAtlasSprite textureLeaves,
				TextureAtlasSprite textureSticks, int leavesTintIndex) {
			this(format, transformation, transforms, textureLeaves, textureSticks, leavesTintIndex, -1);
		}

		private ModelBakedWeedwoodBush(VertexFormat format, Optional<TRSRTransformation> transformation,
				ImmutableMap<TransformType, TRSRTransformation> transforms, TextureAtlasSprite textureLeaves,
				TextureAtlasSprite textureSticks, int leavesTintIndex, long index) {
