...
public class ModelBoxCustomizable extends ModelBox {
	...
	public ModelBoxCustomizable(ModelRenderer model, int u, int v, float x1, float y1, float z1, int width, int height,
			int depth, float expand) {
		this(model, u, v, x1, y1, z1, width, height, depth, expand, 0);
	}

	public ModelBoxCustomizable(ModelRenderer model, int u, int v, float x1, float y1, float z1, int width, int height,
			int depth, float expand, int topVOffset) {
		...
		quadList[0] = new TexturedQuad(new PositionTextureVertex[] { v101, v100, v110, v111 }, u + depth + width,
				v + depth, u + depth + width + depth, v + depth + height, model.textureWidth, model.textureHeight);
		quadList[1] = new TexturedQuad(new PositionTextureVertex[] { v000, v001, v011, v010 }, u, v + depth, u + depth,
				v + depth + height, model.textureWidth, model.textureHeight);
		quadList[2] = new TexturedQuad(new PositionTextureVertex[] { v101, v001, v000, v100 }, u + depth,
				v + topVOffset, u + depth + width, v + depth + topVOffset, model.textureWidth, model.textureHeight);
		quadList[3] = new TexturedQuad(new PositionTextureVertex[] { v110, v010, v011, v111 }, u + depth + width,
				v + depth + topVOffset, u + depth + width + width, v + topVOffset, model.textureWidth,
				model.textureHeight);
		quadList[4] = new TexturedQuad(new PositionTextureVertex[] { v100, v000, v010, v110 }, u + depth, v + depth,
				u + depth + width, v + depth + height, model.textureWidth, model.textureHeight);
		quadList[5] = new TexturedQuad(new PositionTextureVertex[] { v001, v101, v111, v011 },
