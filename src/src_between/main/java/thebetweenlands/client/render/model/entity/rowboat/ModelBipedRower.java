...
public class ModelBipedRower extends ModelBiped {
	...
	public ModelBipedRower(float expand, boolean expandJointed, boolean slimArms, int textureWidth, int textureHeight,
			BipedTextureUVs uvs) {
	...
	private ModelBipedLimb createReplacementArm(ModelRenderer oldLimb, int textureOffsetX, int textureOffsetY,
			boolean slimArms, float expand) {
	...
	private ModelRenderer createExpandReplacementArm(ModelRenderer oldLimb, int textureOffsetX, int textureOffsetY,
			boolean slimArms, float expand) {
		...
		removeCuboids(oldLimb);
		ModelBoxCustomizable arm = new ModelBoxCustomizable(limb, textureOffsetX, textureOffsetY, -2, -2, -2,
				slimArms ? 3 : 4, 6, 4, expand);
		...
		ModelBoxCustomizable forearm = new ModelBoxCustomizable(lowerLimb, textureOffsetX, textureOffsetY + 6, -2, 0,
				-2, slimArms ? 3 : 4, 6, 4, expand * 0.75F, -6);
	...
	@Override
	public void setRotationAngles(float swing, float speed, float age, float yaw, float pitch, float scale,
			Entity entity) {
	}
	...
	public static class BipedTextureUVs {
		...
		public BipedTextureUVs(int armLeftU, int armLeftV, int armRightU, int armRightV, int legLeftU, int legLeftV,
				int legRightU, int legRightV) {
			this(armLeftU, armLeftV, armRightU, armRightV, legLeftU, legLeftV, legRightU, legRightV, 16, 16);
		}

		public BipedTextureUVs(int armLeftU, int armLeftV, int armRightU, int armRightV, int legLeftU, int legLeftV,
				int legRightU, int legRightV, int bodyU, int bodyV) {
