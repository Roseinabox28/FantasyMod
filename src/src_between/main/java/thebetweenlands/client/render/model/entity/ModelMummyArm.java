...
public class ModelMummyArm extends MowzieModelBase {
	...
	public void setLivingAnimations(EntityLivingBase entity, float yaw, float pitch, float partialTicks) {
		...
		if (arm.attackSwing > 0) {
			walk(armleft, 1, -0.4F, false, offset, 0, ((float) Math.abs(arm.attackSwing - 10) / 2.0F
					+ Math.signum(arm.attackSwing - 10) * -1 * partialTicks / 2.0F), 1);
			walk(armleft2, 1, -0.6F, false, offset, 0, ((float) Math.abs(arm.attackSwing - 10) / 2.0F
					+ Math.signum(arm.attackSwing - 10) * -1 * partialTicks / 2.0F), 1);
		}
		if (arm.hurtTime > 0) {
