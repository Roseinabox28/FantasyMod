...
public class MowzieModelBase extends ModelBase {
	...
	/**
	 * Saves the initial rotate angles and initial rotation points. Note: Call this
	...
	/**
	 * Resets the rotate angles and rotation points to its original value if they
	 * were saved before. Note: Call this at the beginning of setRotationAngles.
	 */
	 ...
	 * <p/>
	 * Note: When parenting a chain of boxes, such as a head to a neck to a body,
	 * the end of the chain should start first. In this case the head should be
	 * parented to the neck before parenting the neck to the body.
	 * <p/>
	...
	protected void addChildTo(ModelRenderer child, ModelRenderer parent) {
		float distance = (float) Math.sqrt(Math.pow((child.rotationPointZ - parent.rotationPointZ), 2)
		...
		float parentToChildAngle = (float) Math
				.atan((child.rotationPointZ - parent.rotationPointZ) / (child.rotationPointY - parent.rotationPointY));
	...
	/**
	 * Don't use this yet. I'm trying to refine the parenting method, but it's not
	 * ready yet.
	 */
	protected void newAddChildTo(ModelRenderer child, ModelRenderer parent) {
		float distance = (float) Math.sqrt(Math.pow((child.rotationPointZ - parent.rotationPointZ), 2)
				+ Math.pow((child.rotationPointY - parent.rotationPointY), 2));
		float angle = (float) Math.atan2(child.rotationPointY - parent.rotationPointY,
	 ...
	 *
	 * @param f  is the number of boxes being used. (i.e. if you are using this on a
	 *           head and neck, set it to 2. Just a head, 1);
	 ...
	 * @param weight will make the animation favor one direction more based on how
	 ...
	 * @param f1     is the walk speed.
	 */
	public void walk(MowzieModelRenderer box, float speed, float degree, boolean invert, float offset, float weight,
			float f, float f1) {
		int inverted = 1;
		if (invert)
			inverted = -1;
	 ...
	 * @param f1     is the walk speed.
	 */
	public void swing(MowzieModelRenderer box, float speed, float degree, boolean invert, float offset, float weight,
			float f, float f1) {
		int inverted = 1;
		if (invert)
			inverted = -1;
	 ...
	 * @param f1     is the walk speed.
	 */
	public void flap(MowzieModelRenderer box, float speed, float degree, boolean invert, float offset, float weight,
			float f, float f1) {
		int inverted = 1;
		if (invert)
			inverted = -1;
	...
	/**
	 * Swings a chain of parented boxes back and forth (rotateAngleY). Useful for
	 * tails.
	 * <p/>
	 * Note: Just keep f and f1 from the setRotationAngles() method.
	 *
	 ...
	 * @param speed      is how fast the animation runs;
	 * @param degree     is how far the box will move;
	 * @param rootOffset changes the delay between boxes. Try values from 0.0D to
	 *                   5.0D or so until you like the effect;
	 * @param f          is the walked distance;
	 * @param f1         is the walk speed.
	 */
	public void chainSwing(MowzieModelRenderer[] boxes, float speed, float degree, double rootOffset, float f,
			float f1) {
	 ...
	 * @param f1         is the walk speed.
	 */
	public void chainWave(MowzieModelRenderer[] boxes, float speed, float degree, double rootOffset, float f,
			float f1) {
		int numberOfSegments = boxes.length;
		float offset = (float) ((rootOffset * Math.PI) / (2 * numberOfSegments));
		for (int i = 0; i < numberOfSegments; i++)
	 ...
	 * @param f1         is the walk speed.
	 */
	public void chainFlap(MowzieModelRenderer[] boxes, float speed, float degree, double rootOffset, float f,
			float f1) {
		int numberOfSegments = boxes.length;
		float offset = (float) ((rootOffset * Math.PI) / (2 * numberOfSegments));
		for (int i = 0; i < numberOfSegments; i++)
