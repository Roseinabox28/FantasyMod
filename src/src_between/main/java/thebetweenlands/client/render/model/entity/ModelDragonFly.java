...
public class ModelDragonFly extends MowzieModelBase {
	...
	private void init() {
		...
		parts = new MowzieModelRenderer[] { leftback1, leftback2, rightback1, rightback2, leftfront1, leftfront2,
				rightfront2, rightfront1, leftmid1, leftmid2, rightmid1, rightmid2, tail1, tail2, tail3, tail4,
				stinger };
	...
	@Override
	public void render(Entity entity, float swing, float speed, float ticksExisted, float yaw, float pitch,
			float scale) {
		torso.render(scale);
		back.render(scale);
		tail1.render(scale);
