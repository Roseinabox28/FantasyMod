...
public class ModelAlcove implements IModel {
	...
	private static final Predicate<ModelRenderer> TOP_COBWEB_PREDICATE = part -> {
		return part == ALCOVE_MODEL_BASE.cobweb1 || part == ALCOVE_MODEL_BASE.cobweb2
				|| part == ALCOVE_MODEL_BASE.cobweb2b;
	};
	...
	private static final Predicate<ModelRenderer> SMALL_CANDLE_PREDICATE = part -> {
		return part == ALCOVE_MODEL_BASE.candle2 || part == ALCOVE_MODEL_BASE.wicker2
				|| part == ALCOVE_MODEL_BASE.drip_texture2;
	};

	private static final Predicate<ModelRenderer> BIG_CANDLE_PREDICATE = part -> {
		return part == ALCOVE_MODEL_BASE.candle1 || part == ALCOVE_MODEL_BASE.wicker1
				|| part == ALCOVE_MODEL_BASE.drip_texture1;
	};

	private static final Predicate<ModelRenderer> BASE_PREDICATE = part -> {
		return !OUTCROP_PREDICATE.test(part) && !TOP_COBWEB_PREDICATE.test(part) && !BOTTOM_COBWEB_PREDICATE.test(part)
				&& !SMALL_CANDLE_PREDICATE.test(part) && !BIG_CANDLE_PREDICATE.test(part);
	};
	...
	public ModelAlcove(TexturePacker packer, ResourceLocation[] baseTextures, ResourceLocation particleTexture,
		...
		for (int i = 0; i < baseTextures.length; i++) {
			this.baseModel[i] = new ModelFromModelBase(packer, ALCOVE_MODEL_BASE, baseTextures[i], particleTexture,
					width, height, new PredicateVertexProcessor(BASE_PREDICATE));

			this.topCobwebModel[i] = new ModelFromModelBase(this.baseModel[i], particleTexture, width, height,
					new PredicateVertexProcessor(TOP_COBWEB_PREDICATE));
			this.bottomCobwebModel[i] = new ModelFromModelBase(this.baseModel[i], particleTexture, width, height,
					new PredicateVertexProcessor(BOTTOM_COBWEB_PREDICATE));
			this.smallCandleModel[i] = new ModelFromModelBase(this.baseModel[i], particleTexture, width, height,
					new PredicateVertexProcessor(SMALL_CANDLE_PREDICATE));
			this.bigCandleModel[i] = new ModelFromModelBase(this.baseModel[i], particleTexture, width, height,
					new PredicateVertexProcessor(BIG_CANDLE_PREDICATE));
		}
	...
	public IBakedModel bake(IModelState state, VertexFormat format,
		...
		return new ModelBakedAlcove(PerspectiveMapWrapper.getTransforms(state), bakedBaseModel, bakedTopCobwedModel,
				bakedBottomCobwebModel, bakedSmallCandleModel, bakedBigCandleModel,
				bakedTextureGetter.apply(this.particleTexture));
	}
	...
	public static class ModelBakedAlcove implements IBakedModel {
		...
		private ModelBakedAlcove(ImmutableMap<TransformType, TRSRTransformation> transforms,
				IBakedModel[] bakedBaseModel, IBakedModel[] bakedTopCobwedModel, IBakedModel[] bakedBottomCobwebMode,
				IBakedModel[] bakedSmallCandleModel, IBakedModel[] bakedBigCandleModel,
