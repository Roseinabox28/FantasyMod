...
public class ModelSporeling extends ModelBase {
	...
	public void setRotationAngles(float limbSwing, float prevLimbSwing, float entityTickTime, float rotationYaw,
		...
		super.setRotationAngles(limbSwing, prevLimbSwing, entityTickTime, rotationYaw, rotationPitch, unitPixel,
				entity);
		armleft.rotateAngleX = MathHelper.cos(limbSwing * 1.5F + (float) Math.PI) * 2.0F * prevLimbSwing * 0.5F
				- (MathHelper.cos(entityTickTime / 10.0F) + 0.7F) / 2.0F * 0.3F;
		armright.rotateAngleX = MathHelper.cos(limbSwing * 1.5F) * 2.0F * prevLimbSwing * 0.5F
				- (MathHelper.cos(entityTickTime / 11.0F) + 0.7F) / 2.0F * 0.3F;
