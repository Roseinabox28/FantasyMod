...
public class ModelSlant implements IModel {
	...
	public IBakedModel bake(IModelState state, VertexFormat format,
		...
		return new ModelBakedSlant(state.apply(Optional.empty()), map, format,
				bakedTextureGetter.apply(this.textureSlant), bakedTextureGetter.apply(this.textureSide),
				bakedTextureGetter.apply(this.textureBase));
	}
	...
	public IModel retexture(ImmutableMap<String, String> textures) {
		...
		if (slant != null || side != null || base != null) {
			return new ModelSlant(slant != null ? slant : this.textureSlant, side != null ? side : this.textureSide,
					base != null ? base : this.textureBase);
		}
		return this;
	}
	...
	public static class ModelBakedSlant implements IBakedModel {
		...
		private ModelBakedSlant(Optional<TRSRTransformation> transformation,
		...
		private final LoadingCache<Integer, ModelBakedSlant> modelCache = CacheBuilder.newBuilder().maximumSize(256)
				.build(new CacheLoader<Integer, ModelBakedSlant>() {
					@Override
					public ModelBakedSlant load(Integer key) throws Exception {
						return new ModelBakedSlant(
								ModelBakedSlant.this.transformation != null
										? Optional.of(ModelBakedSlant.this.transformation)
										: Optional.empty(),
								ModelBakedSlant.this.transforms, ModelBakedSlant.this.format,
								ModelBakedSlant.this.textureSlant, ModelBakedSlant.this.textureSide,
								ModelBakedSlant.this.textureBase, key);
					}
				});
