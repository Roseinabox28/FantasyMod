...
public class ModelLurker extends ModelBase {
	...
	public void setLivingAnimations(EntityLivingBase entity, float swing, float speed, float delta) {
		...
		hindfinLeft.rotateAngleX = MathHelper.cos(swing * 0.8F - MathUtils.PI / 2) * speed * 1.0F - 0.260246F
				+ speed * 0.8F;
		...
		forefinRightDistal.rotateAngleY = MathHelper.sin(swing * 0.8F + MathUtils.PI / 4) * speed * 0.7F - 0.2F
				+ MathUtils.PI;
		hindfinRight.rotateAngleX = MathHelper.sin(swing * 0.8F + MathUtils.PI / 2) * speed * 1.0F - 0.260246F
				+ speed * 0.8F;
		...
		for (int i = 0; i < tail.length; i++) {
			...
			segment.rotateAngleY = yaw
					+ MathHelper.sin(swing * 0.4F - i * 1.6F) * speed * ((i / (float) tail.length * 2 + 0.1F)) * 0.6F;
