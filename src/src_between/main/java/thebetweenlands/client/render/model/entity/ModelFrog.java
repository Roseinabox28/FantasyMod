...
public class ModelFrog extends ModelBase {
	...
	public void setRotationAngles(float limbSwing, float prevLimbSwing, float entityTickTime, float rotationYaw,
		...
		head.rotateAngleX = rotationPitch / (180F / (float) Math.PI) - 0.07435719668865202F
				+ (float) Math.sin(entityTickTime / 8.0F) / 15.0F;
	}

	@Override
	public void setLivingAnimations(EntityLivingBase entity, float swing, float speed, float partialRenderTicks) {
		EntityFrog frog = (EntityFrog) entity;
		float leapProgress = frog.prevJumpAnimationTicks
				+ (frog.jumpAnimationTicks - frog.prevJumpAnimationTicks) * partialRenderTicks;
		...
		if (!frog.isInWater()) {
			// Idle animation
			this.torso.rotateAngleX = -0.55F
					- (float) (Math.sin((entity.ticksExisted + partialRenderTicks) / 10.0F) + 1) / 35.0F;
			this.torso.rotationPointY = 19
					+ (float) Math.sin((entity.ticksExisted + partialRenderTicks) / 8.0F) / 15.0F;
			...
			// Idle animation
			this.torso.rotateAngleX = -0.1F
					- (float) (Math.sin((entity.ticksExisted + partialRenderTicks) / 10.0F) + 1) / 35.0F;

			// Water bobbing animation
			this.torso.rotationPointY = 19 + (float) Math.sin((entity.ticksExisted + partialRenderTicks) / 8.0F) / 2.0F;
			this.legbackleft1.rotationPointY = 21
					+ (float) Math.sin((entity.ticksExisted + partialRenderTicks) / 8.0F) / 2.0F;
			this.legbackright1.rotationPointY = 21
					+ (float) Math.sin((entity.ticksExisted + partialRenderTicks) / 8.0F) / 2.0F;
