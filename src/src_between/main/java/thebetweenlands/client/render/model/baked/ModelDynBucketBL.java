...
public class ModelDynBucketBL implements IModel {
	...
	public ModelDynBucketBL(ResourceLocation baseLocation, ResourceLocation liquidLocation,
			ResourceLocation coverLocation, Fluid fluid, boolean flipGas) {
	...
	public IModel process(ImmutableMap<String, String> customData) {
		...
		if (customData.containsKey("base_texture")) {
			baseLocation = new ResourceLocation(
					JsonUtils.getString(parser.parse(customData.get("base_texture")), "base_texture"));
		} else {
			baseLocation = this.baseLocation;
		}

		if (customData.containsKey("liquid_template")) {
			liquidLocation = new ResourceLocation(
					JsonUtils.getString(parser.parse(customData.get("liquid_template")), "liquid_template"));
		} else {
			liquidLocation = this.liquidLocation;
		}

		if (customData.containsKey("cover_texture")) {
			coverLocation = new ResourceLocation(
					JsonUtils.getString(parser.parse(customData.get("cover_texture")), "cover_texture"));
		} else {
