...
public class ModelEventSelection implements IModel {
	...
	public IModel process(ImmutableMap<String, String> customData) {
		...
		if (customData.containsKey("model_base") || baseModel == null) {
			ResourceLocation baseModelLocation = new ResourceLocation(
					JsonUtils.getString(parser.parse(customData.get("model_base")), "model_base"));
			baseModel = ModelLoaderRegistry.getModelOrLogError(baseModelLocation,
					"Could not find base model for event selection model");
		}

		if (customData.containsKey("model_base_data")) {
			baseModel = baseModel.process(
					LoaderExtension.parseJsonElementList(parser, customData.get("model_base_data"), "model_base_data"));
		}

		IModel altModel = this.altModel;

		if (customData.containsKey("model_active") || altModel == null) {
			ResourceLocation additionalModelLocation = new ResourceLocation(
					JsonUtils.getString(parser.parse(customData.get("model_active")), "model_active"));
			altModel = ModelLoaderRegistry.getModelOrLogError(additionalModelLocation,
					"Could not find active model for event selection model");
		}

		if (customData.containsKey("model_active_data")) {
			altModel = altModel.process(LoaderExtension.parseJsonElementList(parser,
					customData.get("model_active_data"), "model_active_data"));
		}
	...
	public static class BakedEventSelectionModel implements IBakedModel {
		...
		public BakedEventSelectionModel(BooleanSupplier isActive, IBakedModel baseBakedModel,
				IBakedModel altBakedModel) {
		...
		public Pair<? extends IBakedModel, Matrix4f> handlePerspective(TransformType cameraTransformType) {
			...
			if (this.baseBakedModel instanceof PerspectiveMapWrapper) {
				...
				result = PerspectiveMapWrapper.handlePerspective(this,
						this.getItemCameraTransforms().getTransform(cameraTransformType), cameraTransformType);
