...
public class ModelRubberTapLiquid implements IModel {
	...
	public IBakedModel bake(IModelState state, VertexFormat format,
		...
		return new BakedModelRubberTapLiquid(format, state.apply(Optional.empty()), map,
				this.fluidTexture != null ? bakedTextureGetter.apply(this.fluidTexture) : null, this.height);
	}
	...
	private static final class BakedModelRubberTapLiquid implements IBakedModel {
		...
		private BakedModelRubberTapLiquid(VertexFormat format, Optional<TRSRTransformation> transformation,
				ImmutableMap<TransformType, TRSRTransformation> transforms, TextureAtlasSprite fluidSprite,
				int height) {
			...
			if (height > 0) {
				QuadBuilder builder = new QuadBuilder(format).setSprite(this.fluidSprite)
