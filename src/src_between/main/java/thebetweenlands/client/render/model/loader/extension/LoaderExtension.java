...
public abstract class LoaderExtension {
	 ...
	 * <p>
	 * E.g. someExtension.getLocationWithExtension("domain:path/file", "someArg") ->
	 * "domain:path/file$someExtension(someArg)"
	 * 
	 * @param location
	 * @param arg
	 * @return
	...
	public final ResourceLocation getLocationWithExtension(ResourceLocation location, String arg) {
		return new ResourceLocation(location.getNamespace(),
				location.getPath() + "$" + this.getName() + "(" + arg + ")");
	}
	 ...
	 * <p>
	 * <b>Do NOT return an {@link IBakedModel} from an {@link IModel} that wasn't
	 * obtained through {@link ModelLoaderRegistry#getModel(ResourceLocation)} at
	 * some point!</b>
	 * 
	 * @param location
	 * @param original
	...
	protected final void throwLoaderException(@Nullable String reason, @Nullable Throwable cause) {
		if (reason != null)
			reason = String.format("Model loader extension %s failed loading a model. Reason: %s", this.getName(),
					reason);
		else
	...
	/**
	 * Reads the metadata from the specified String. If the metadata starts with
	 * "mlmeta:" the metadata is read from an external .mlmeta file with the
	 * following name after "mlmeta:"
	 * 
	 * @param metadata
	 * @return
	 */
