...
public class AdvancedModelRenderer extends MowzieModelRenderer {
	...
	@Override
	public AdvancedModelRenderer addBox(String name, float posX, float posY, float posZ, int width, int height,
			int depth) {
		name = boxName + "." + name;
		...
		cubeList.add(new ModelBox(this, textureOffsetX, textureOffsetY, posX, posY, posZ, width, height, depth, 0.0F)
				.setBoxName(name));
		return this;
	}
	...
	public void postRender(float scale) {
		if (!isHidden) {
			if (showModel) {
				...
				rotationOrder.rotate(rotateAngleX * MathUtils.RAD_TO_DEG, rotateAngleY * MathUtils.RAD_TO_DEG,
						rotateAngleZ * MathUtils.RAD_TO_DEG);
				secondaryRotationOrder.rotate(secondaryRotateAngleX * MathUtils.RAD_TO_DEG,
						secondaryRotateAngleY * MathUtils.RAD_TO_DEG, secondaryRotateAngleZ * MathUtils.RAD_TO_DEG);
		...
		if (isGlowing) {
			...
			for (ModelBox box : boxModels) {
				...
				for (int i = 0; i < bri; i++) {
					...
					GL11.glPushMatrix();
					GL11.glTranslatef((box.posX1 - expand - scaleX * box.posX1) / 16,
							(box.posY1 - (isMeteorLightGlow ? meteorExpandY : expand) - scaleY * box.posY1) / 16,
