	...
	public IModel process(ImmutableMap<String, String> customData) {
		...
		if (customData.containsKey("model_base") || baseModel == null) {
			...
			baseModel = ModelLoaderRegistry.getModelOrLogError(baseModelLocation,
					"Could not find base model for combined model");
		}
		...
		if (customData.containsKey("model_additional") || additionalModel == null) {
			ResourceLocation additionalModelLocation = new ResourceLocation(
					JsonUtils.getString(parser.parse(customData.get("model_additional")), "model_additional"));
			additionalModel = ModelLoaderRegistry.getModelOrLogError(additionalModelLocation,
					"Could not find additional model for combined model");
		}

		if (customData.containsKey("model_additional_data")) {
			additionalModel = additionalModel.process(LoaderExtension.parseJsonElementList(parser,
					customData.get("model_additional_data"), "model_additional_data"));
		}
