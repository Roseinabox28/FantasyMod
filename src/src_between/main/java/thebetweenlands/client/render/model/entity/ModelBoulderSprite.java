...
/**
 * BLBoulderSprite - TripleHeadedSheep Created using Tabula 7.0.0
 */
public class ModelBoulderSprite extends MowzieModelBase {
	public static class StalactitesModelRenderer extends MowzieModelRenderer {
		public static final ResourceLocation SPRITE_BOTTOM = new ResourceLocation(ModInfo.ID,
				"blocks/stalactite_bottom");
		...
		public void setStalactites(@Nullable ResourceLocation texture, @Nullable List<SpikeRenderer> stalactites,
				@Nullable ResourceLocation prevTexture) {
	...
	public ModelBoulderSprite() {
		...
		parts = new MowzieModelRenderer[] { stalactites, block_main, brow1, nosebridge, mosstache_left1,
				mosstache_right1, lebushybeard, leg_left, leg_right, brow_left, brow_right, nosetip, nosewings,
				mosstache_left2, mossbush_left1, mosstache_right2, mossbush_right1, lebushybeard_sideleft,
				lebushybeard_sideright, actualbush, extrabush };
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5) {
		...
		this.stalactites.setStalactites(TextureMap.LOCATION_BLOCKS_TEXTURE, sprite.stalactites,
				RenderBoulderSprite.TEXTURE);
	...
	public void setLivingAnimations(EntityLivingBase entitylivingbaseIn, float limbSwing, float limbSwingAngle,
		...
		swing(this.block_main, globalSpeed * 0.5f, globalDegree * 0.6f, false, 2.8f, 0.0f, limbSwing + 1.5F,
				limbSwingAngle);
		flap(this.block_main, globalSpeed * 0.5f, globalDegree * 0.3f, false, 2.8f, 0.0f, limbSwing - 1.25F,
				limbSwingAngle);
		walk(this.block_main, globalSpeed * 0.5f, globalDegree * 0.15f, false, 0.2f, 0.0f, limbSwing * 1.1F,
				limbSwingAngle);
		...
		this.block_main.offsetY = (-(float) Math.abs(Math.sin((rollAnimation + 0.6F) * Math.PI)) * 0.8F + 0.3F)
				* bobWeight;
		float rollRotX = (float) ((rollAnimation
				+ ((Math.sin((rollAnimation + 0.5F) * Math.PI * 2) + 1) * 0.05F + 0.5F) * 1.1F) * Math.PI * 2)
				% ((float) Math.PI * 2);
