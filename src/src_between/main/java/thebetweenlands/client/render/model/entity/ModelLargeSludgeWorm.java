...
/**
 * BLWormFront - TripleHeadedSheep Created using Tabula 7.0.0
 */
public class ModelLargeSludgeWorm extends ModelBase {
	...
	private static class HeartModelRenderer extends ModelRenderer {
		...
		public void render(float scale) {
			...
			if (this.entity != null) {
				...
				float beat = (float) (a
						* (Math.exp(-(x + d) * (x + d) / (2 * w)) + Math.exp(-(x - d) * (x - d) / (2 * w))
								+ (h - Math.abs(x / s) - x) * Math.exp(-(7 * x) * (7 * x) / 2)));
	...
	public void renderHead(EntityLargeSludgeWorm worm, int frame, float wibbleStrength, float partialTicks,
			boolean renderSolids) {
		float smoothedTicks = worm.ticksExisted + frame
				+ (worm.ticksExisted + frame - (worm.ticksExisted + frame - 1)) * partialTicks;
	...
	public void renderTail(EntityLargeSludgeWorm worm, int frame, float wibbleStrength, float partialTicks,
			boolean renderSolids) {
		if (renderSolids) {
			float smoothedTicks = worm.ticksExisted + frame
					+ (worm.ticksExisted + frame - (worm.ticksExisted + frame - 1)) * partialTicks;
