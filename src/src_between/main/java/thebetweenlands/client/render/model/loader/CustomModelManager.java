...
public enum CustomModelManager {
	...
	/**
	 * Registers a model getter. The getter is called when a block is requesting the
	 * model with the ModelResourceLocation of that block as parameter.
	 * 
	 * @param modelLocation
	 * @param modelGetter
	 */
	public void registerModelProvider(@Nonnull ResourceLocation modelLocation,
			@Nonnull Function<ResourceLocation, IModel> modelGetter) {
	...
	public void registerModel(@Nonnull ResourceLocation modelLocation, @Nonnull IModel model) {
		...
		if (model instanceof IModelVariantProvider)
			this.registerModelProvider(modelLocation,
					(location) -> (((IModelVariantProvider) model).getModelVariant(location)));
		else
