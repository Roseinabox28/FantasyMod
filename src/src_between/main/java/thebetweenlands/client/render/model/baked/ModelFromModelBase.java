...
public class ModelFromModelBase implements IModel, ITexturePackable {
	...
	public ModelFromModelBase(TexturePacker packer, ModelBase model, ResourceLocation texture, int width, int height,
			@Nullable IVertexProcessor vertexProcessor) {
		this(packer, model, texture, texture, width, height, vertexProcessor);
	}

	public ModelFromModelBase(TexturePacker packer, ModelBase model, ResourceLocation texture,
			ResourceLocation particleTexture, int width, int height) {
		this(packer, model, texture, particleTexture, width, height, null);
	}

	public ModelFromModelBase(TexturePacker packer, ModelBase model, ResourceLocation texture,
			ResourceLocation particleTexture, int width, int height, @Nullable IVertexProcessor vertexProcessor) {
	 ...
	 */
	public ModelFromModelBase(ModelFromModelBase parent, ResourceLocation texture, ResourceLocation particleTexture,
			int width, int height, @Nullable IVertexProcessor vertexProcessor) {
		this(parent.derivativeModelCache, parent.packer, parent.model, texture, particleTexture, width, height,
				vertexProcessor);
	}

	/**
	 * Constructor used to create derived models without changing the main texture
	 * (i.e. reusing the already converted model).
	 */
	public ModelFromModelBase(ModelFromModelBase parent, ResourceLocation particleTexture, int width, int height,
			@Nullable IVertexProcessor vertexProcessor) {
		this(parent.derivativeModelCache, parent.model, parent.convertedModel, parent.texture, particleTexture, width,
				height, vertexProcessor);
	}

	private ModelFromModelBase(Map<ModelCacheKey, Model> modelCache, TexturePacker packer, ModelBase model,
			ResourceLocation texture, ResourceLocation particleTexture, int width, int height,
			@Nullable IVertexProcessor vertexProcessor) {
	...
	private ModelFromModelBase(Map<ModelCacheKey, Model> modelCache, ModelBase model, Model convertedModel,
			ResourceLocation texture, ResourceLocation particleTexture, int width, int height,
			@Nullable IVertexProcessor vertexProcessor) {
		this.derivativeModelCache = modelCache;
		this.packer = null;
	...
	public IBakedModel bake(IModelState state, VertexFormat format,
		...
		return new ModelBakedModelBase(this.vertexProcessor, state.apply(Optional.empty()), map, format,
				this.convertedModel, bakedTextureGetter, bakedTextureGetter.apply(this.particleTexture), this.width,
				this.height, this.ambientOcclusion);
	}
	...
	public static class ModelBakedModelBase implements IBakedModel {
		...
		protected ModelBakedModelBase(IVertexProcessor vertexProcessor, Optional<TRSRTransformation> transformation,
				ImmutableMap<TransformType, TRSRTransformation> transforms, VertexFormat format, Model convertedModel,
				Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter, TextureAtlasSprite particleTexture,
				int width, int height, boolean ambientOcclusion) {
	...
	public ModelFromModelBase process(ImmutableMap<String, String> customData) {
		...
		if (customData.containsKey("particle_texture")) {
			particleTexture = new ResourceLocation(
					JsonUtils.getString(parser.parse(customData.get("particle_texture")), "particle_texture"));
		}
		...
		if (customData.containsKey("ambient_occlusion")) {
			ambientOcclusion = JsonUtils.getBoolean(parser.parse(customData.get("ambient_occlusion")),
					"ambient_occlusion");
		}
		...
		if (texture == this.texture) {
			return new ModelFromModelBase(this.derivativeModelCache, this.model, this.convertedModel, texture,
					particleTexture, this.width, this.height, this.vertexProcessor)
							.setAmbientOcclusion(ambientOcclusion);
		} else {
			return new ModelFromModelBase(this.derivativeModelCache, this.packer, this.model, texture, particleTexture,
					this.width, this.height, this.vertexProcessor).setAmbientOcclusion(ambientOcclusion);
		}
	}
}