...
public class ModelGeckoNormal extends MowzieModelBase {
	...
	public ModelGeckoNormal() {
		...
		parts = new MowzieModelRenderer[] { body_base, head, legleft_f1, legright_f1, legleft_b1, legright_b1, tail1,
				tail2, tail3, crane, tongue, legleft_f2, legright_f2, legleft_b2, legright_b2 };
		setInitPose();
	}

	/*
	 * @Override public void render(Entity entity, float swing, float speed, float
	 * age, float yaw, float pitch, float scale) { setRotationAngles(swing, speed,
	 * age, yaw, pitch, scale, entity); legright_b1.render(scale);
	 * legright_f1.render(scale); legleft_f1.render(scale); head.render(scale);
	 * body_base.render(scale); legleft_b1.render(scale); }
	 */
	...
	/*
	 * @Override public void setRotationAngles(float swing, float speed, float age,
	 * float yaw, float pitch, float scale, Entity entity) { faceTarget(head, 1,
	 * yaw, pitch);
	 * 
	 * if (speed > 0.4) { speed = 0.4F; }
	 * 
	 * float globalSpeed = 1, globalDegree = 1, frontOffset = 0.85F;
	 * 
	 * walk(legleft_f1, globalSpeed, globalDegree, false, frontOffset, 0, swing,
	 * speed); walk(legleft_f2, globalSpeed, 1.5F * globalDegree, false, 2 +
	 * frontOffset, -0.4F * globalDegree, swing, speed); walk(legright_f1,
	 * globalSpeed, globalDegree, true, frontOffset, 0, swing, speed);
	 * walk(legright_f2, globalSpeed, 1.5F * globalDegree, true, 2 + frontOffset,
	 * -0.4F * globalDegree, swing, speed);
	 * 
	 * walk(legleft_b1, globalSpeed, globalDegree, false, 0, -2.5F * globalDegree,
	 * swing, speed); walk(legleft_b2, globalSpeed, globalDegree, false, -1, 2.5F *
	 * globalDegree, swing, speed); walk(legright_b1, globalSpeed, globalDegree,
	 * true, 0, -2.5F * globalDegree, swing, speed); walk(legright_b2, globalSpeed,
	 * globalDegree, true, -1, 2.5F * globalDegree, swing, speed); }
	 * 
	 * @Override public void setLivingAnimations(EntityLivingBase entity, float yaw,
	 * float pitch, float delta) { setToInitPose(); float frame =
	 * entity.ticksExisted + delta; float tongueControl = (int)
	 * ((MathHelper.sin(0.15F * frame - MathHelper.cos(0.15F * frame)) + 1) / 2 +
	 * 0.5F); tongue.isHidden = tongueControl == 0; walk(tongue, 2, 1, false, 0, 0,
	 * frame, 1); chainWave(tail, 0.2F, 0.1f, 0, frame, 1); chainSwing(tail, 0.4F,
	 * 0.15f, 3, frame, 1); }
	 */
}
