...
public class ModelDarkDruid extends ModelBase {
	...
	@Override
	public void render(Entity entity, float limbSwing, float limbSwingAngle, float ticksExisted, float rotationYaw,
	...
	public void setLivingAnimations(EntityLivingBase entity, float swing, float speed, float partialRenderTicks) {
		...
		float pitch = -MathUtils.PI / 2
				+ (entity.prevRotationPitch + (entity.rotationPitch - entity.prevRotationPitch) * partialRenderTicks)
						* MathUtils.DEG_TO_RAD;
		float ticksExisted = entity.ticksExisted + partialRenderTicks;
		rightCuff.rotateAngleX = rightArm.rotateAngleX = pitch * attackAnimationTime
				+ MathHelper.cos(swing * 0.6662F + MathUtils.PI) * 2 * speed * 0.5F * (1 - attackAnimationTime);
		leftCuff.rotateAngleX = leftArm.rotateAngleX = pitch * attackAnimationTime
				+ MathHelper.cos(swing * 0.6662F) * 2.0F * speed * 0.5F * (1 - attackAnimationTime);
		rightCuff.rotateAngleX = rightArm.rotateAngleX += MathHelper.sin(ticksExisted * 0.4F) * 0.1F
				* attackAnimationTime;
		leftCuff.rotateAngleX = leftArm.rotateAngleX += MathHelper.cos(ticksExisted * 0.4F) * 0.1F
				* attackAnimationTime;
		rightCuff.rotateAngleZ = rightArm.rotateAngleZ = -MathHelper.sin(ticksExisted * 0.2F) * 0.1F
				* attackAnimationTime;
