...
public class ModelLifeCrystalStalactite implements IModel {
	...
	public static final ResourceLocation TEXTURE_ORE_BACKGROUND = new ResourceLocation(ModInfo.ID,
			"blocks/life_crystal_ore_background");
	...
	public Collection<ResourceLocation> getTextures() {
		return Collections.unmodifiableCollection(
				Arrays.asList(new ResourceLocation[] { TEXTURE_DEFAULT, TEXTURE_ORE_BACKGROUND, TEXTURE_ORE }));
	}

	@Override
	public IBakedModel bake(IModelState state, VertexFormat format,
			java.util.function.Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter) {
		return new ModelBakedLifeCrystalOre(format, bakedTextureGetter.apply(TEXTURE_DEFAULT),
				bakedTextureGetter.apply(TEXTURE_ORE_BACKGROUND), bakedTextureGetter.apply(TEXTURE_ORE));
	}
	...
	public static class ModelBakedLifeCrystalOre implements IBakedModel {
		...
		private ModelBakedLifeCrystalOre(VertexFormat format, TextureAtlasSprite textureDefault,
				TextureAtlasSprite textureOreBackground, TextureAtlasSprite textureOre) {
			this.format = format;
		...
		public List<BakedQuad> getQuads(IBlockState state, EnumFacing side, long rand) {
			...
			if (side == null) {
				EnumLifeCrystalType type = StatePropertyHelper
						.getPropertyOptional(state, BlockLifeCrystalStalactite.VARIANT)
						.orElse(EnumLifeCrystalType.DEFAULT);
				int distUp = StatePropertyHelper.getPropertyOptional(state, BlockLifeCrystalStalactite.DIST_UP)
						.orElse(0);
				int distDown = StatePropertyHelper.getPropertyOptional(state, BlockLifeCrystalStalactite.DIST_DOWN)
						.orElse(0);
				boolean noTop = StatePropertyHelper.getPropertyOptional(state, BlockLifeCrystalStalactite.NO_TOP)
						.orElse(true);
				boolean noBottom = StatePropertyHelper.getPropertyOptional(state, BlockLifeCrystalStalactite.NO_BOTTOM)
						.orElse(true);
				...
				int scaledValBottom = (int) (Math.pow(distToMidBottom, squareAmount) / halfTotalHeightSQ
						* (8 - minValBottom)) + minValBottom;
				int scaledValTop = (int) (Math.pow(distToMidTop, squareAmount) / halfTotalHeightSQ * (8 - minValTop))
						+ minValTop;
				...
				// This is necessary because vertices that are on the exact same point seem to
				// cause problems with the lighting
				...
				for (int i = 0; i < (type == EnumLifeCrystalType.DEFAULT ? 1 : 2); i++) {
					...
					if (distUp == 0) {
						builder.addVertex(core.tX - halfSize1, height, core.tZ - halfSize1, umin, vmin);
						builder.addVertex(core.tX - halfSize1, height, core.tZ + halfSize1, umin + halfSizeTex1 * 2,
								vmin);
						builder.addVertex(core.tX + halfSize1, height, core.tZ + halfSize1, umin + halfSizeTex1 * 2,
								vmin + halfSizeTex1 * 2);
						builder.addVertex(core.tX + halfSize1, height, core.tZ - halfSize1, umin,
								vmin + halfSizeTex1 * 2);
					}
					...
					if (distDown == 0) {
						...
						builder.addVertex(core.bX + halfSize, 0, core.bZ + halfSize, umin + halfSizeTexW * 2,
								vmin + halfSizeTexW * 2);
					}
				}
