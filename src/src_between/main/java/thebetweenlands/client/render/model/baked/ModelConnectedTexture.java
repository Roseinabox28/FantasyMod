...
public class ModelConnectedTexture implements IModel {
	protected static class Vertex {
		...
		protected Vertex add(Vertex vertex) {
			return new Vertex(this.pos.x + vertex.pos.x, this.pos.y + vertex.pos.y, this.pos.z + vertex.pos.z,
					this.uv.x + vertex.uv.x, this.uv.y + vertex.uv.y);
		}
		...
		protected Vertex scale(float scale) {
			return new Vertex(this.pos.x * scale, this.pos.y * scale, this.pos.z * scale, this.uv.x * scale,
					this.uv.y * scale);
		}
	...
	protected static class ConnectedTextureQuad {
		...
		protected ConnectedTextureQuad(ResourceLocation[] textures, String[] indices, Vertex[] verts,
				EnumFacing cullFace, int tintIndex, float minU, float minV, float maxU, float maxV) {
		...
		public void bake(Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter,
			...
			float ecp = this.extrapolateClamp(v0.uv.x + (v1.uv.x - v0.uv.x) * e01, v2.uv.x + (v3.uv.x - v2.uv.x) * e23,
					(this.maxU + this.minU) / 2.0F);
			...
			this.quads[0] = this.bakeTextureVariants(format, transformation, sprites,
					new Vertex[] { v0, p01, cp, p30 });
			this.quads[1] = this.bakeTextureVariants(format, transformation, sprites,
					new Vertex[] { p01, v1, p12, cp });
			this.quads[2] = this.bakeTextureVariants(format, transformation, sprites,
					new Vertex[] { cp, p12, v2, p23 });
			this.quads[3] = this.bakeTextureVariants(format, transformation, sprites,
					new Vertex[] { p30, cp, p23, v3 });
		}
	...
	public ModelConnectedTexture(ResourceLocation particleTexture, boolean ambientOcclusion,
			List<ConnectedTextureQuad> textures) {
	...
	public static class ModelBakedConnectedFace implements IBakedModel {
		...
		private ModelBakedConnectedFace(Optional<TRSRTransformation> transformation,
				ImmutableMap<TransformType, TRSRTransformation> transforms, VertexFormat format,
				TextureAtlasSprite particleTexture, boolean ambientOcclusion,
				List<ConnectedTextureQuad> connectedTextures) {
			this.ambientOcclusion = ambientOcclusion;
			this.format = format;
			...
			for (int i = 0; i < EnumFacing.VALUES.length + 1; i++) {
				...
				this.connectedTextures[i == 0 ? 0 : face.getIndex() + 1] = connectedTextureQuads
						.toArray(new ConnectedTextureQuad[0]);
			}
		}
	...
	public IModel process(ImmutableMap<String, String> customData) {
		...
		if (customData.containsKey("connected_texture_faces")) {
			...
			Preconditions.checkState(jsonObj != null && !jsonObj.isJsonNull() && jsonObj.isJsonArray(),
					"Connected texture model must provide a connected_texture_faces array");
			...
			for (JsonElement element : array) {
				...
				Preconditions.checkState(
						ctJson.has("indices") && ctJson.get("indices").isJsonArray()
								&& ctJson.get("indices").getAsJsonArray().size() == 4,
						"Connected texture face must provide 4 indices");
				...
				Preconditions.checkState(
						ctJson.has("vertices") && ctJson.get("vertices").isJsonArray()
								&& ctJson.get("vertices").getAsJsonArray().size() == 4,
						"Connected texture face must provide 4 vertices");
				Vertex[] vertices = new Vertex[4];
				JsonArray verticesArray = ctJson.get("vertices").getAsJsonArray();
				for (int i = 0; i < 4; i++) {
					JsonObject vertexJson = verticesArray.get(i).getAsJsonObject();
					vertices[i] = new Vertex(vertexJson.get("x").getAsDouble(), vertexJson.get("y").getAsDouble(),
							vertexJson.get("z").getAsDouble(), vertexJson.get("u").getAsFloat(),
							vertexJson.get("v").getAsFloat());
				}

				Preconditions.checkState(
						ctJson.has("connected_textures") && ctJson.get("connected_textures").isJsonArray()
								&& ctJson.get("connected_textures").getAsJsonArray().size() == 5,
						"Connected texture face must provide 5 textures");
				...
				connectedTextures.add(new ConnectedTextureQuad(textures, indices, vertices, cullFace, tintIndex, minU,
						minV, maxU, maxV));
			}
		}
		...
		if (changed) {
			return new ModelConnectedTexture(
					particleTexture == null ? TextureMap.LOCATION_MISSING_TEXTURE : particleTexture, ambientOcclusion,
					connectedTextures);
		}
