...
public class SpikeRenderer {
	...
	public SpikeRenderer(int length, float widthScale, float heightScale, float offsetScale, long seed, double x,
			double y, double z) {
		this.length = length;
	...
	public SpikeRenderer build(VertexFormat format, TextureAtlasSprite bottomSprite, TextureAtlasSprite midSprite) {
		...
		for (int y = 0; y < this.length; y++) {
			...
			// front
			builder.addVertex(core.bX - halfSize, this.y + (y) * this.heightScale, core.bZ - halfSize,
					umin + halfSizeTexW * 2, vmax);
			builder.addVertex(core.bX - halfSize, this.y + (y) * this.heightScale, core.bZ + halfSize, umin, vmax);
			builder.addVertex(core.tX - halfSize1, this.y + (y + height) * this.heightScale, core.tZ + halfSize1, umin,
					vmin);
			builder.addVertex(core.tX - halfSize1, this.y + (y + height) * this.heightScale, core.tZ - halfSize1,
					umin + halfSizeTex1 * 2, vmin);
			// back
			builder.addVertex(core.bX + halfSize, this.y + (y) * this.heightScale, core.bZ + halfSize,
					umin + halfSizeTexW * 2, vmax);
			builder.addVertex(core.bX + halfSize, this.y + (y) * this.heightScale, core.bZ - halfSize, umin, vmax);
			...
			if (distUp == 0) {
				...
				builder.addVertex(core.tX - halfSize1, this.y + y + height, core.tZ + halfSize1,
						umin + halfSizeTex1 * 2, vmin);
				builder.addVertex(core.tX + halfSize1, this.y + y + height, core.tZ + halfSize1,
						umin + halfSizeTex1 * 2, vmin + halfSizeTex1 * 2);
				builder.addVertex(core.tX + halfSize1, this.y + y + height, core.tZ - halfSize1, umin,
						vmin + halfSizeTex1 * 2);
			}
			...
			if (distDown == 0) {
				...
				builder.addVertex(core.bX + halfSize, this.y + y, core.bZ + halfSize, umin + halfSizeTexW * 2,
						vmin + halfSizeTexW * 2);
			}
		}
