...
public class ModelRubberTapCombined implements IModel {
	...
	public ModelRubberTapCombined(TexturePacker packer, ResourceLocation texture, ResourceLocation particleTexture,
			ResourceLocation fluidTexture, int height) {
		this.combinedTapModel = new ModelCombined(this.tapModel = new ModelFromModelBase(packer, new ModelRubberTap(),
				texture, particleTexture, 128, 128), new ModelRubberTapLiquid(fluidTexture, height));
		this.particleTexture = particleTexture;
		this.height = height;
		this.fluidTexture = fluidTexture;
	}

	protected ModelRubberTapCombined(ModelFromModelBase parent, ResourceLocation particleTexture,
			ResourceLocation fluidTexture, int height) {
		this.combinedTapModel = new ModelCombined(
				this.tapModel = new ModelFromModelBase(parent, particleTexture, 128, 128, null),
				new ModelRubberTapLiquid(fluidTexture, height));
		this.particleTexture = particleTexture;
		this.height = height;
		this.fluidTexture = fluidTexture;
	}
	...
	public ModelRubberTapCombined process(ImmutableMap<String, String> customData) {
		...
		if (customData.containsKey("fluid_texture")) {
			fluidTexture = new ResourceLocation(
					JsonUtils.getString(parser.parse(customData.get("fluid_texture")), "fluid_texture"));
		}
		...
		if (customData.containsKey("particle_texture")) {
			...
			particleTexture = new ResourceLocation(
					JsonUtils.getString(parser.parse(particleTextureJsonStr), "particle_texture"));
		}
