...
public class ModelPeatMummy extends MowzieModelBase {
	...
	public ModelPeatMummy() {
		...
		parts = new MowzieModelRenderer[] { body_base, buttJoint, shoulderJoint, sexybutt, legright, legleft, legright2,
				legleft2, shoulderBase, shoulderright, shoulderleft, neck, armright, armleft, head1, head2, jaw, teeth2,
				hair, teeth1, cheecktissueright, cheecktissue2, armright2, armleft2, legleftJoint, legrightJoint,
				armleftJoint, armrightJoint, modelBase };
		setInitPose();
	}
	...
	public void setLivingAnimations(EntityLivingBase entity, float yaw, float pitch, float delta) {
		...
		if (screamProgress != 0) {
			...
			cheecktissue2.rotateAngleX -= 2.4 * controller
					+ controller2 * 0.5 * Math.cos(4 * (mummy.ticksExisted + delta));
			cheecktissue2.rotationPointY += 10 * controller;
			cheecktissueright.rotateAngleX -= 2.4 * controller
					+ controller2 * 0.5 * Math.cos(4 * (mummy.ticksExisted + delta));
