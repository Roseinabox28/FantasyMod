...
/**
 * Allows json item models to load non-json models and specify certain
 * properties or provide a .mlmeta file
 */
public class AdvancedItemLoaderExtension extends LoaderExtension {
	...
	protected final class ModelContext {
		 ...
		 */
		protected ModelContext(ModelResourceLocation source, ResourceLocation replacement, String customData,
				boolean inheritsOverrides, boolean cacheOverrides) {
			this.source = source;
	...
	protected ModelContext parseContextData(ResourceLocation location, String metadata) {
		...
		ImmutableMap<String, String> dataMap = parseJsonElementList(parser, metadata,
				location.toString() + " model metadata");
		...
		if (dataMap.containsKey("inherit_overrides")) {
			try {
				inheritOverrides = JsonUtils.getBoolean(parser.parse(dataMap.get("inherit_overrides")),
						"inherit_overrides");
			} catch (Exception ex) {
		...
		if (dataMap.containsKey("cache_overrides")) {
			try {
				shouldCacheOverrides = JsonUtils.getBoolean(parser.parse(dataMap.get("cache_overrides")),
						"cache_overrides");
			} catch (Exception ex) {
		...
		ModelResourceLocation sourceLocation = new ModelResourceLocation(
				new ResourceLocation(childModel.getNamespace(), childModel.getPath()), "inventory");
	...
	public IBakedModel getModelReplacement(ModelResourceLocation location, IBakedModel original) {
		...
		if (context != null) {
			...
			IBakedModel bakedModel = replacementModel.bake(replacementModel.getDefaultState(),
					DefaultVertexFormats.ITEM,
					(loc) -> Minecraft.getMinecraft().getTextureMapBlocks().getAtlasSprite(loc.toString()));

			// Return wrapped model
			return new BakedModelItemWrapper(original, bakedModel).setInheritOverrides(context.inheritOverrides)
					.setCacheOverrideModels(context.cacheOverrides);
		}
		// Nothing to replace
		return null;
	}
	...
	private IModel findReplacementModelAndRegister(ResourceLocation location, ModelContext context) {
		...
		if (context.hasCustomData()) {
			...
			replacementModelLocation = CustomModelLoader.MODEL_PROCESSOR_LOADER_EXTENSION
					.getLocationWithExtension(replacementModelLocation, "{\"custom\": " + context.customData + "}");
		}

		// Retrieve replacement model
		try {
			// Makes sure that the model is loaded through the model loader and that the
			// textures are registered properly
			return ModelLoaderRegistry.getModel(replacementModelLocation);
		} catch (Exception ex) {
			throw new RuntimeException(
					"Failed to load model " + replacementModelLocation + " for child model " + location, ex);
		}
	}
}
