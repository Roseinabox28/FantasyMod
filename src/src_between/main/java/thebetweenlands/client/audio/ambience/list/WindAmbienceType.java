...
public class WindAmbienceType extends AmbienceType {
	...
	private double getSoundVariation(Entity entity) {
		return (Math.sin(entity.ticksExisted * 0.1D) + 1) / 2.0D * (Math.cos(entity.ticksExisted * 0.2D) + 1) / 2.0D
				* (Math.sin(entity.ticksExisted * 0.3D) + 1) / 2.0D * (Math.cos(entity.ticksExisted * 0.4D) + 1) / 2.0D;
	}
	...
	public float getVolume() {
		...
		if (ridingEntity != null) {
			return (1.0F - (float) Math.pow(1.0F - Math.min((float) this.getVelocity(ridingEntity) * 1.5F, 2.0F) / 2.0F,
					1 / 2.0F)) * 1.25F * (float) (1 - this.getSoundVariation(this.getPlayer()) * 0.5D);
		}
		return 0.0F;
	}
	...
	public float getPitch() {
		Entity ridingEntity = this.getPlayer().getRidingEntity();
		if (ridingEntity != null) {
			return 0.9F + (1.0F - (float) Math
					.pow(1.0F - Math.min((float) this.getVelocity(ridingEntity) * 1.5F, 4.0F) / 4.0F, 1 / 2.0F)) * 1.8F
					* (float) (1 - this.getSoundVariation(this.getPlayer()) * 0.5D);
		}
		return 1.0F;
	}
