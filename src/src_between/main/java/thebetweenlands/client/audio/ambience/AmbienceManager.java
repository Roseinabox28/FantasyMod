...
public class AmbienceManager {
	...
	public void update() {
		...
		while (delayedAmbiencesIT.hasNext()) {
			...
			boolean soundPlaying = Minecraft.getMinecraft().getSoundHandler().isSoundPlaying(sound)
					|| sound.isDonePlaying();
			if (soundPlaying)
		...
		while (playingAmbiencesIT.hasNext()) {
			...
			boolean soundPlaying = Minecraft.getMinecraft().getSoundHandler().isSoundPlaying(sound)
					|| this.delayedAmbiences.contains(sound); // Check if sound is playing or scheduled
			if (sound.isDonePlaying() || !soundPlaying) {
				if (soundPlaying)
					Minecraft.getMinecraft().getSoundHandler().stopSound(sound); // Stop sound because it's not supposed
																					// to play anymore
		...
		if (player != null) {
			for (AmbienceLayer layer : this.ambienceRegistry.keySet()) {
				...
				if (!availableAmbiences.isEmpty()) {
					...
					for (int typeIndex = 0, i = 0; i < sorted.size() && typeIndex < maxTracks; i++) {
						...
						if (type.isActive() && type.isActiveInWorld(player.world)) {
							...
							if (!isPlaying && typeIndex <= lowestPlayedAmbience) {
								if (this.lastSoundPlayTicks <= 0) {
									this.playSound(
											new AmbienceSound(type.getSound(), type.getCategory(), type, player, this),
											type.getDelay());
					...
					// Stop or set any ambient tracks to lower priority if they don't have priority
					// or shouldn't play
					for (AmbienceSound sound : this.playingAmbiences) {
						...
						for (int i = 0; i < sorted.size() && typeIndex < maxTracks; i++) {
							...
							if (otherType.isActive() && otherType.isActiveInWorld(player.world)) {
								...
								if (otherType.getLowerPriorityVolume() > 0.0F
										&& otherType.getLowerPriorityVolume() < 1.0F)
									lowerPriority = true;
							}
						}
