...
public class PresentAmbienceType extends AmbienceType {
			...
			for (int cz = sz; cz <= ez; cz++) {
				...
				for (Entry<BlockPos, TileEntity> entry : chunk.getTileEntityMap().entrySet()) {
					...
					if (tile instanceof TileEntityPresent) {
						...
						if (dstSq <= range * range && (closest == null
								|| dstSq <= closest.getPos().distanceSq(player.posX, player.posY, player.posZ))) {
	...
	public boolean isActive() {
		return EventWinter.isFroooosty(this.getPlayer().world)
				&& this.getClosestPresent(this.getPlayer(), 32.0D) != null;
	}
	...
	public float getVolume() {
		...
		if (present != null) {
			float volume = (1 - MathHelper.clamp((float) Math
					.sqrt(present.getDistanceSq(this.getPlayer().posX, this.getPlayer().posY, this.getPlayer().posZ))
					/ 64.0F, 0, 1));
