...
public class GreeblingMusicSound extends EntityMusicSound<EntityGreebling> {
	...
	public GreeblingMusicSound(int type, EntityGreebling entity, float volume) {
		super(type == 0 ? SoundRegistry.GREEBLING_MUSIC_1 : SoundRegistry.GREEBLING_MUSIC_2, SoundCategory.NEUTRAL,
				entity, volume, AttenuationType.NONE);
		this.type = type;
	}
	...
	public void update() {
		if (!this.synced) {
			...
			if (this.isSoundReady) {
				final IEntitySound otherSound = MusicHandler.INSTANCE
						.getEntityMusic(this.type == 0 ? EntityMusicLayers.GREEBLING_2 : EntityMusicLayers.GREEBLING_1);
		...
		if (!this.mustFadeOut && !this.fadeOut) {
			...
			if (player != null) {
				this.volume = (float) MathHelper
						.clamp(this.originalVolume * (1.0D - player.getDistance(this.xPosF, this.yPosF, this.zPosF)
								/ ((EntityGreebling) this.getMusicEntity()).getMusicRange(player)), 0, 1);
			}
		}
	}
}
