...
public class CorrosionHelper {
	...
	public static void addCorrosionPropertyOverrides(Item item) {
		item.addPropertyOverride(new ResourceLocation("corrosion"),
				(stack, worldIn, entityIn) -> getCorrosionStage(stack));
	...
	/**
	 * Returns the dig speed of an item at the amount of corrosion of the specified
	 * item
	 * 
	...
	public static boolean shouldCauseBlockBreakReset(ItemStack oldStack, ItemStack newStack) {
		return !(newStack.getItem() == oldStack.getItem() && areItemStackTagsEqual(newStack, oldStack)
				&& (newStack.isItemStackDamageable() || newStack.getMetadata() == oldStack.getMetadata()));
	}

	/**
	 * Returns whether the item reequip animation should be played. Only works for
	 * the default NBT tag {@link #ITEM_CORROSION_NBT_TAG}
	 * 
	 * @param oldStack
	 * @param newStack
	 * @return
	...
	public static boolean areItemStackTagsEqual(ItemStack oldStack, ItemStack newStack) {
		return NBTHelper.areItemStackTagsEqual(newStack, oldStack,
				ImmutableList.of(ITEM_CORROSION_NBT_TAG, ITEM_COATING_NBT_TAG));
	}

	/**
	 * Returns a map with the attack damage attribute affected by the corrosion
	 * multiplier
	 * 
	 * @param map
	 * @param slot
	 ...
	 * @return
	 */
	public static Multimap<String, AttributeModifier> getAttributeModifiers(Multimap<String, AttributeModifier> map,
			EntityEquipmentSlot slot, ItemStack stack, UUID uuid, float damageVsEntity) {
		if (slot == EntityEquipmentSlot.MAINHAND) {
			map.removeAll(SharedMonsterAttributes.ATTACK_DAMAGE.getName());
			map.put(SharedMonsterAttributes.ATTACK_DAMAGE.getName(),
					new AttributeModifier(uuid, "Tool modifier", damageVsEntity * getModifier(stack), 0));
		}
		return map;
	}
	...
	public static boolean isCorrosionEnabled() {
		return GameruleRegistry.getGameRuleBooleanValue(GameruleRegistry.BL_CORROSION)
				&& BetweenlandsConfig.GENERAL.useCorrosion;
	}
	...
	public static void updateCorrosion(ItemStack stack, World world, Entity holder, int slot, boolean isHeldItem) {
		...
		if (!world.isRemote && holder.dimension == BetweenlandsConfig.WORLD_AND_DIMENSION.dimensionId
				&& !(holder instanceof EntityPlayer && ((EntityPlayer) holder).isCreative())) {
			if (!stack.isEmpty() && stack.getItem() instanceof ICorrodible) {
				ICorrodible corrodible = (ICorrodible) stack.getItem();
				int corrosion = corrodible.getCorrosion(stack);
