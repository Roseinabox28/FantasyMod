...
public class Fog {
	...
	protected float density, red, green, blue, colorMultiplier, start, end, colorIncrement, distanceIncrementMultiplier,
			densityIncrement;

	public Fog(FogType type, float density, float red, float green, float blue, float colorMultiplier, float start,
			float end, float colorIncrement, float distanceIncrementMultiplier, float densityIncrement) {
		this.type = type;
	...
	public Fog(Fog fog) {
		this(fog.type, fog.density, fog.red, fog.green, fog.blue, fog.colorMultiplier, fog.start, fog.end,
				fog.colorIncrement, fog.distanceIncrementMultiplier, fog.densityIncrement);
	}
	...
	/**
	 * Returns the fog density (only for {@link FogType#EXP} and
	 * {@link FogType#EXP2})
	 * 
	 * @return
	 */
	...
	public static class MutableFog extends Fog {
		public MutableFog(FogType type, float density, float red, float green, float blue, float colorMultiplier,
				float start, float end) {
			super(type, density, red, green, blue, colorMultiplier, start, end);
		}

		public MutableFog(FogType type, float density, float red, float green, float blue, float colorMultiplier,
				float start, float end, float colorIncrement, float distanceIncrementMultiplier,
				float densityIncrement) {
			super(type, density, red, green, blue, colorMultiplier, start, end, colorIncrement,
					distanceIncrementMultiplier, densityIncrement);
		}
		...
		public MutableFog set(Fog fog) {
			this.setType(fog.type).setDensity(fog.density).setRed(fog.getRed()).setGreen(fog.getGreen())
					.setBlue(fog.getBlue()).setStart(fog.start).setEnd(fog.end)
					.setColorIncrement(fog.getColorIncrement())
					.setDistanceIncrementMultiplier(fog.getDistanceIncrementMultiplier())
					.setDensityIncrement(fog.getDensityIncrement());
			return this;
		}
