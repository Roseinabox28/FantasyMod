...
public class FogState {
	...
	public Fog getFog(float partialTicks) {
		...
		if (partialTicks == 0.0F) {
			...
			float density = this.prevTickFog.getDensity()
					+ (this.currentTickFog.getDensity() - this.prevTickFog.getDensity()) * partialTicks;
			float colorIncrement = this.prevTickFog.getColorIncrement()
					+ (this.currentTickFog.getColorIncrement() - this.prevTickFog.getColorIncrement()) * partialTicks;
			float distanceIncrement = this.prevTickFog.getDistanceIncrementMultiplier()
					+ (this.currentTickFog.getDistanceIncrementMultiplier()
							- this.prevTickFog.getDistanceIncrementMultiplier()) * partialTicks;
			float densityIncrement = this.prevTickFog.getDensityIncrement()
					+ (this.currentTickFog.getDensityIncrement() - this.prevTickFog.getDensityIncrement())
							* partialTicks;
			float red = this.prevTickFog.getRed()
					+ (this.currentTickFog.getRed() - this.prevTickFog.getRed()) * partialTicks;
			float green = this.prevTickFog.getGreen()
					+ (this.currentTickFog.getGreen() - this.prevTickFog.getGreen()) * partialTicks;
			float blue = this.prevTickFog.getBlue()
					+ (this.currentTickFog.getBlue() - this.prevTickFog.getBlue()) * partialTicks;
			float colorMultiplier = this.prevTickFog.getColorMultiplier()
					+ (this.currentTickFog.getColorMultiplier() - this.prevTickFog.getColorMultiplier()) * partialTicks;
			float start = this.prevTickFog.getStart()
					+ (this.currentTickFog.getStart() - this.prevTickFog.getStart()) * partialTicks;
			float end = this.prevTickFog.getEnd()
					+ (this.currentTickFog.getEnd() - this.prevTickFog.getEnd()) * partialTicks;
			return new Fog(type, density, red, green, blue, colorMultiplier, start, end, colorIncrement,
					distanceIncrement, densityIncrement);
		}
	}
	...
	public Fog getBiomeFog(World world, Vec3d position, float farPlaneDistance, int mode) {
		MutableFog defaultBiomeFog = new MutableFog().setType(FogType.LINEAR).setDensity(0.0F).setColorIncrement(0.001F)
				.setDistanceIncrementMultiplier(1.0F).setRed(0.5F).setGreen(0.5F).setBlue(0.5F)
				.setStart(farPlaneDistance).setEnd(farPlaneDistance).setColorMultiplier(1);
		...
		if (biome instanceof BiomeBetweenlands) {
			...
			defaultBiomeFog.setStart(biomeBl.getFogStart(farPlaneDistance, mode))
					.setEnd(biomeBl.getFogEnd(farPlaneDistance, mode));
		}
	...
	public Fog getAmbientFog(Fog biomeFog, World world, Vec3d position) {
		...
		if (position.y < WorldProviderBetweenlands.CAVE_START) {
			float fogColorMultiplier = ((float) (WorldProviderBetweenlands.CAVE_START - position.y)
					/ WorldProviderBetweenlands.CAVE_START);
			...
			if (position.y <= WorldProviderBetweenlands.PITSTONE_HEIGHT) {
				...
				if (fogColorMultiplier < targettedMultiplier) {
					fogColorMultiplier += Math
							.pow(((targettedMultiplier - fogColorMultiplier) / WorldProviderBetweenlands.PITSTONE_HEIGHT
									* (WorldProviderBetweenlands.PITSTONE_HEIGHT - position.y)), 0.85F);
				}
			}
			...
			fogColorMultiplier = fogColorMultiplier
					+ (1.0F - fogColorMultiplier) * (float) Math.pow(lowViewDistanceFogReduction, 2.25D);
	...
	public void update(World world, Vec3d position, float farPlaneDistance, int mode) {
		...
		MinecraftForge.EVENT_BUS
				.post(new UpdateFogEvent(this, defaultBiomeFog, defaultAmbientFog, position, world, farPlaneDistance));
		...
		float[] fogPositions = new float[] { this.currentTickFog.getStart(), target.getStart(),
				this.currentTickFog.getEnd(), target.getEnd() };
		...
		float[] currentFogColor = new float[] { this.currentTickFog.getRed(), this.currentTickFog.getGreen(),
				this.currentTickFog.getBlue() };
		float[] targetFogColor = new float[] { target.getRed() * target.getColorMultiplier(),
				target.getGreen() * target.getColorMultiplier(), target.getBlue() * target.getColorMultiplier() };
		...
		this.currentTickFog = new Fog(target.getFogType(), currentFogDensity, currentFogColor[0], currentFogColor[1],
				currentFogColor[2], 1.0F, fogPositions[0], fogPositions[2], target.getColorIncrement(),
				target.getDistanceIncrementMultiplier(), target.getDensityIncrement());
	}
}
