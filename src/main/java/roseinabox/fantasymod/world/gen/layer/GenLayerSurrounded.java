...
public class GenLayerSurrounded extends GenLayerFantasy{
	private final int checkRange;
	private final float spawnChance;
	private final int surrounding, biome;

	public GenLayerSurrounded(InstancedIntCache cache, long seed, GenLayer parentGenLayer, Biome surrounding,
			Biome biome, int checkRange, float spawnChance) {
		super(cache, seed);
	...
	public int[] getInts(int areaX, int areaZ, int sizeX, int sizeZ) {
		int[] currentBiomeInts = this.parent.getInts(areaX - this.checkRange, areaZ - this.checkRange,
				sizeX + this.checkRange * 2, sizeZ + this.checkRange * 2);
		...
		for (int zz = 0; zz < sizeZ; ++zz) {
			for (int xx = 0; xx < sizeX; ++xx) {
				...
				biomeInts[xx + zz * sizeX] = currentBiomeInts[xx + this.checkRange
						+ (zz + this.checkRange) * (sizeX + this.checkRange * 2)];
				...
				check: for (int xo = -this.checkRange; xo <= this.checkRange; xo++) {
					for (int zo = -this.checkRange; zo <= this.checkRange; zo++) {
						if (xo * xo + zo * zo <= this.checkRange * this.checkRange + 1) {
							int biomeID = currentBiomeInts[xx + this.checkRange + xo
									+ (zz + this.checkRange + zo) * (sizeX + this.checkRange * 2)];
