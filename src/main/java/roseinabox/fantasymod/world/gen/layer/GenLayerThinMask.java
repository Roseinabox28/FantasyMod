...
public class GenLayerThinMask extends GenLayerFantasy{
	private final int id, range;
	private final float removeChance;

	public GenLayerThinMask(InstancedIntCache cache, long seed, GenLayer parentGenLayer, int id, int range,
			float removeChance) {
		super(cache, seed);
	...
	public int[] getInts(int areaX, int areaZ, int sizeX, int sizeZ) {
		int[] currentInts = this.parent.getInts(areaX - this.range, areaZ - this.range, sizeX + this.range * 2,
				sizeZ + this.range * 2);
					...
					check: for (int xo = -this.range; xo <= this.range; xo++) {
						for (int zo = -this.range; zo <= this.range; zo++) {
							if ((xo != 0 || zo != 0) && xo * xo + zo * zo <= this.range * this.range + 1) {
								if (currentInts[xx + this.range + xo
										+ (zz + this.range + zo) * (sizeX + this.range * 2)] == this.id
										&& this.nextInt(10000) <= this.removeChance * 10000) {
									ints[xx + zz * sizeX] = -1;
									break check;
								}
							}
	...
	public static GenLayer thin(InstancedIntCache cache, long seed, GenLayer layer, int id, int range,
			float removeChance, int num) {
