...
public class DecoratorPositionProvider {
	...
	public BlockPos getRandomPosSeaGround(int padding) {
		return new BlockPos(this.x + this.offsetXZ(padding), this.seaGroundY + this.offsetY(),
				this.z + this.offsetXZ(padding));
	}
	...
	public void init(World world, Biome biome, @Nullable IChunkGenerator generator, Random rand, int x, int y, int z) {
		...
		/*if (this.y <= WorldProviderBetweenlands.LAYER_HEIGHT
				&& world.getBlockState(new BlockPos(this.x, this.y, this.z)).getMaterial().isLiquid()) {
